...
;((DocsAPI, window, document, undefined) => {
  /*
  ...
  DocsAPI.DocEditor = function (placeholderId, config) {
    ...
    const _checkConfigParams = () => {
      if (_config.document) {
        ...
        if (typeof _config.document.fileType === "string" && _config.document.fileType !== "") {
          ...
          const type =
            /^(?:(xls|xlsx|ods|csv|tsv|gsheet|xlsm|xlt|xltm|xltx|fods|ots|xlsb|sxc|et|ett|numbers)|(pps|ppsx|ppt|pptx|odp|gslides|pot|potm|potx|ppsm|pptm|fodp|otp|sxi|dps|dpt|key|odg)|(pdf|djvu|xps|oxps)|(doc|docx|odt|gdoc|txt|rtf|mht|htm|html|mhtml|epub|docm|dot|dotm|dotx|fodt|ott|fb2|xml|oform|docxf|sxw|stw|wps|wpt|pages|hwp|hwpx|md|hml)|(vsdx|vssx|vstx|vsdm|vssm|vstm))$/.exec(
          ...
          if (typeof _config.documentType !== "string" || _config.documentType === "") {
            ...
            else if (typeof type[2] === "string") _config.documentType = "slide"
            else if (typeof type[3] === "string") _config.documentType = "pdf"
            else if (typeof type[4] === "string") _config.documentType = "word"
            else if (typeof type[5] === "string") _config.documentType = "diagram"
          }
        }

        const type = /^(?:(djvu|xps|oxps))$/.exec(_config.document.fileType)
        ...
        if (!_config.document.key) {
          _config.document.key = "xxxxxxxxxxxxxxxxxxxx".replace(/[x]/g, (c) => {
        ...
        if (_config.editorConfig.user?.id && typeof _config.editorConfig.user.id === "number") {
    ...
    if (target && _checkConfigParams()) {
      ...
      if (
        ...
        window.AscEmbed?.initWorker(iframe)
    ...
    const _destroyEditor = (cmd) => {
      ...
      if (iframe) {
        ...
        _detachMouseEvents()
        iframe.parentNode?.replaceChild(target, iframe)
      }
    }

    const _sendCommand = (cmd, buffer) => {
      if (iframe?.contentWindow) postMessage(iframe.contentWindow, cmd, buffer)
    }

    const _init = (editorConfig) => {
      _sendCommand({
        command: "init",
        data: {
          config: editorConfig,
    ...
    const _openDocumentFromBinary = (doc) => {
      doc &&
        _sendCommand(
          {
    ...
    const _applyEditRights = (allowed, message) => {
      _sendCommand({
    ...
    const _processRightsChange = (enabled, message) => {
      _sendCommand({
    ...
    const _denyEditingRights = (message) => {
      _sendCommand({
        command: "processRightsChange",
        data: {
          enabled: false,
    ...
    const _refreshHistory = (data, message) => {
      _sendCommand({
    ...
    const _setHistoryData = (data, message) => {
      _sendCommand({
    ...
    const _processMailMerge = (enabled, message) => {
      _sendCommand({
    ...
    const _showSharingSettings = (data) => {
      _sendCommand({
    ...
    const _setSharingSettings = (data) => {
      _sendCommand({
    ...
    const _setMailMergeRecipients = (data) => {
      _sendCommand({
    ...
    const _setRequestedDocument = (data) => {
      _sendCommand({
    ...
    const _setRequestedSpreadsheet = (data) => {
      _sendCommand({
    ...
    const _setReferenceSource = (data) => {
      _sendCommand({
    ...
    const _serviceCommand = (command, data) => {
      _sendCommand({
  ...
  DocsAPI.DocEditor.version = () => "{{PRODUCT_VERSION}}"

  DocsAPI.DocEditor.warmUp = (id) => {
    const target = document.getElementById(id)
    if (target) {
      let path = extendAppPath({}, getBasePath())
  ...
  function getShardkey(config) {
    ...
    for (let i = scripts.length - 1; i >= 0; i--) {
      if (scripts[i].src.match(/(.*)api\/documents\/api.js/i)) {
        const shardkey = /[\?\&]shardkey=([^&]+)&?/.exec(scripts[i].src)
        shardkey?.length && (config.editorConfig.shardkey = shardkey[1])
        break
      }
    }
  }
  ...
  function getExtensionPath() {
    if ("undefined" === typeof extensionParams || null == extensionParams.url) return null
  ...
  function getAppPath(config) {
    ...
    if (config.document) {
      if (typeof config.document.fileType === "string")
        type =
          /^(?:(pdf)|(djvu|xps|oxps)|(xls|xlsx|ods|csv|tsv|xlst|xlsy|gsheet|xlsm|xlt|xltm|xltx|fods|ots|xlsb|numbers)|(pps|ppsx|ppt|pptx|odp|pptt|ppty|gslides|pot|potm|potx|ppsm|pptm|fodp|otp|key|odg)|(oform|docxf)|(vsdx|vssx|vstx|vsdm|vssm|vstm))$/.exec(
    ...
    const corrected_type = correct_app_type(config)
    if (type && typeof type[2] === "string") {
      ...
      if (typeof config.documentType === "string") appType = config.documentType.toLowerCase()
      else {
  ...
  function getAppParameters(config) {
    ...
    if (config.editorConfig?.lang) params += `&lang=${config.editorConfig.lang}`
    ...
    if (config.editorConfig && config.editorConfig.targetApp !== "desktop") {
      if (
        ...
        if (config.editorConfig.customization.loaderName !== "none")
          params += `&customer=${encodeURIComponent(config.editorConfig.customization.loaderName)}`
      ...
      if (typeof config.editorConfig.customization === "object") {
        ...
        if (config.editorConfig.customization.logo) {
          if (config.editorConfig.customization.logo.visible === false) {
            ...
            config.editorConfig.customization.logo.imageDark &&
              (params += `&headerlogodark=${encodeURIComponent(config.editorConfig.customization.logo.imageDark)}`)
            config.editorConfig.customization.logo.imageLight &&
              (params += `&headerlogolight=${encodeURIComponent(config.editorConfig.customization.logo.imageLight)}`)
          } else if (
            config.type !== "embedded" &&
            (config.editorConfig.customization.logo.image ||
              config.editorConfig.customization.logo.imageDark ||
    ...
    const type = config.document ? /^(?:(pdf)|(oform|docxf))$/.exec(config.document.fileType) : null
    const isPdf = type && typeof type[1] === "string"
    const oldForm = type && typeof type[2] === "string"
    ...
    if (
      (isPdf || oldForm) &&
      ((config.document?.permissions && config.document.permissions.edit === false) ||
        (config.editorConfig && config.editorConfig.mode === "view"))
    )
    ...
    if (config.editorConfig?.customization && !!config.editorConfig.customization.compactHeader)
      params += "&compact=true"

    if (config.editorConfig?.customization?.features?.tabBackground) {
    ...
    if (config.editorConfig?.customization && config.editorConfig.customization.toolbar === false)
    ...
    if (config.document?.fileType) params += `&fileType=${config.document.fileType}`
    ...
    for (const prop in src) {
      if (src.hasOwnProperty(prop)) {
        if (typeof dest[prop] === "undefined") {
          dest[prop] = src[prop]
        } else if (typeof dest[prop] === "object" && typeof src[prop] === "object") {
  ...
  ;(function () {
    if (document.currentScript) {
      const scriptDirectory = document.currentScript.src
      const cacheWarmupId = /[?&]placeholder=([^&#]*)?/.exec(scriptDirectory)
      if (cacheWarmupId?.length) {
