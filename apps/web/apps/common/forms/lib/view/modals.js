...
define(["common/main/lib/component/Window", "common/main/lib/component/MetricSpinner"], () => {
  Common.Views.EmbedDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 300,
        ...
        initialize: function (options) {
          ...
          this.embedCode =
            '<iframe allowtransparency="true" frameborder="0" scrolling="no" src="{0}" width="{1}" height="{2}"></iframe>'
        ...
        updateEmbedCode: function () {
          this.textareaInput.setValue(
            Common.Utils.String.format(
              this.embedCode,
              this.embedConfig.embedUrl,
              this.spnWidth.getNumberValue(),
              this.spnHeight.getNumberValue(),
            ),
          )
        },
  ...
  Common.Views.ShareDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 300,
        ...
        updateShareCode: function () {
          const _encoded = encodeURIComponent(this.embedConfig.shareUrl)
          const docTitle = this.embedConfig.docTitle || ""
          const _mailto = `mailto:?subject=I have shared a document with you: ${docTitle}&body=I have shared a document with you: ${_encoded}`

          this.getChild()
            .find(".share-buttons > span")
            .on("click", (e) => {
              let _url
              switch ($(e.target).attr("data-name")) {
                case "facebook":
                  _url = `https://www.facebook.com/sharer/sharer.php?u=${this.embedConfig.shareUrl}&t=${encodeURI(docTitle)}`
                  window.open(
                    _url,
                    "",
