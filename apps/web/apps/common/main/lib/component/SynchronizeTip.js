...
define([
    ...
    Common.UI.SynchronizeTip = Common.UI.BaseView.extend(_.extend((() => ({
            options : {
            ...
            applyPlacement: function (repeatOnce) {
                const target = this.target && this.target.length>0 ? this.target : $(document.body);

                if (!target.is(':visible') && !repeatOnce) {
                    setTimeout(()=> { this.applyPlacement(true); }, 100);
                    return;
                }
                ...
                if (this.position && typeof this.position === 'object') {
                    ...
                    let bottom = this.position.bottom;
    ...
    Common.UI.TooltipManager = new(function() {
        ...
        const _addTips = (arr) => {
            for (const step in arr) {
                if (arr.hasOwnProperty(step) && !Common.localStorage.getItem(arr[step].name) && !(_helpTips[step]?.tip?.isVisible())) { // don't replace tip when it's visible
                    _helpTips[step] = arr[step];
                }
            }
        };

        const _removeTip = (step) => {
            if (_helpTips[step]) {
        ...
        const _getNeedShow = (step) => _helpTips[step] && !(_helpTips[step].name && Common.localStorage.getItem(_helpTips[step].name));

        const _applyPlacement = (step) => {
            if (_helpTips[step]?.tip?.isVisible())
                _helpTips[step].tip.applyPlacement();
        };

        const _closeTip = (step, force, preventNext) => {
            const steps = typeof step === 'string' ? [step] : step;
            steps?.forEach((step) => {
                const props = _helpTips[step];
                if (props) {
        ...
        const _showTip = (step) => {
            ...
            if (_getNeedShow(step) && !(_helpTips[step].prev && _getNeedShow(_helpTips[step].prev))) { // show current tip if previous tip has already been shown
                ...
                const targetEl = _findTarget(target);

                if (props.tip?.isVisible())
                    return true;
