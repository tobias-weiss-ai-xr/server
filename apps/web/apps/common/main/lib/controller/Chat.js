...
define([
  ...
  Common.Controllers.Chat = Backbone.Controller.extend(
    _.extend(
      {
        models: [],
          ...
          if (this.api) {
            if (this.mode.canCoAuthoring && this.mode.canChat)
              this.api.asc_registerCallback(
                ...
                _.bind(this.onReceiveMessage, this),
              )
            ...
            if (!this.mode.isEditDiagram && !this.mode.isEditMailMerge && !this.mode.isEditOle) {
              ...
              this.api.asc_registerCallback(
                ...
                _.bind(this.onUsersChanged, this),
              )
              this.api.asc_registerCallback(
        ...
        onUsersChanged: function (users, currentUserId) {
          ...
          const usersStore = this.getApplication().getCollection("Common.Collections.Users")
          ...
          if (usersStore) {
            ...
            for (name in users) {
              if (undefined !== name) {
                user = users[name]
                if (user) {
                  const avatar = Common.UI.ExternalUsers.getImage(user.asc_getIdOriginal())
                  const usermodel = new Common.Models.User({
                    id: user.asc_getId(),
                    ...
                    initials: Common.Utils.getUserInitials(
                      AscCommon.UserInfoParser.getParsedName(user.asc_getUserName()),
                    ),
                    online: true,
                    ...
                    hidden: !(
                      user.asc_getIdOriginal() === this.currentUserId ||
        ...
        onUserConnection: function (change) {
          ...
          if (usersStore) {
            const user = usersStore.findUser(change.asc_getId())
            const avatar = Common.UI.ExternalUsers.getImage(change.asc_getIdOriginal())
            if (!user) {
              usersStore.add(
                new Common.Models.User({
                  ...
                  initials: Common.Utils.getUserInitials(
                    AscCommon.UserInfoParser.getParsedName(change.asc_getUserName()),
                  ),
                  ...
                  hidden: !(
                    change.asc_getIdOriginal() === this.currentUserId ||
                    AscCommon.UserInfoParser.isUserVisible(change.asc_getUserName())
                  ),
                }),
              )
            } else {
            ...
            avatar === undefined &&
              Common.UI.ExternalUsers.get("info", [change.asc_getIdOriginal()])
          }
        },
        ...
        avatarsUpdate: function (type, users) {
          ...
          const usersStore = this.getApplication().getCollection("Common.Collections.Users")
          const msgStore = this.getApplication().getCollection("Common.Collections.ChatMessages")
          let needrender = false
          if (users && users.length > 0) {
            _.each(users, (item) => {
              usersStore?.findOriginalUsers(item.id).forEach((user) => {
                user.set({ avatar: item.image })
              })
              msgStore?.where({ userid: item.id }).forEach((msg) => {
        ...
        onSendMessage: function (panel, text) {
          if (text.length > 0) {
            const splitString = (string, chunkSize) => {
              const chunks = []
            ...
            splitString(text, 2048).forEach((message) => {
              this.api.asc_coAuthoringChatSendMessage(message)
            })
          }
        },
