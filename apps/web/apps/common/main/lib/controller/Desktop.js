...
define(["core"], () => {
  const webapp = window.DE || window.PE || window.SSE || window.PDFE || window.VE
  ...
  const native = window.desktop || window.AscDesktopEditor
  !!native && native.execCommand("webapps:features", JSON.stringify(features))

  const Desktop = () => {
    const config = { version: "{{PRODUCT_VERSION}}" }
    let titlebuttons
    const btnsave_icons = {
      "btn-save": "save",
    ...
    if (native) {
      ...
      window.on_native_message = (cmd, param) => {
        if (/^style:change/.test(cmd)) {
        ...
        } else if (/window:features/.test(cmd)) {
          const obj = JSON.parse(param)
          if (_.isNumber(obj.skiptoparea)) {
            if (
              $(".asc-window.modal").length &&
              $(".asc-window.modal").position().top < obj.skiptoparea
            )
          ...
          if (obj.singlewindow !== undefined) {
            ...
            if (config.isFillFormApp) {
              ...
              titlebuttons?.home?.btn.setVisible(obj.singlewindow)
            }

            native.features.singlewindow = obj.singlewindow
            Common.NotificationCenter.trigger("desktop:window", {
              compositetitle: native.features.singlewindow,
            })
          }
        } else if (/editor:config/.test(cmd)) {
          if (param === "request") {
            const opts = {
              user: config.user,
              title: { buttons: [] },
            ...
            } else if (!config.callback_editorconfig) {
        ...
        } else if (/button:click/.test(cmd)) {
          const obj = JSON.parse(param)
          if (!!obj.action && !!titlebuttons[obj.action]) {
            titlebuttons[obj.action].btn.click()
          }
        } else if (/theme:changed/.test(cmd)) {
          Common.UI.Themes.setTheme(param, "native")
        } else if (/^uitheme:added/.test(cmd)) {
          if (!nativevars.localthemes) nativevars.localthemes = []
        ...
        } else if (/renderervars:changed/.test(cmd)) {
        ...
        } else if (/element:show/.test(cmd)) {
          const _mr = /title:(?:(true|show)|(false|hide))/.exec(param)
          if (_mr) {
            if (_mr[1]) $("#app-title").show()
            else if (_mr[2]) $("#app-title").hide()
          }
        } else if (/althints:show/.test(cmd)) {
          if (/false|hide/.test(param)) Common.NotificationCenter?.trigger("hints:clear")
        } else if (/file:print/.test(cmd)) {
          webapp.getController("Main").onPrint()
        } else if (/printer:config/.test(cmd)) {
          ...
          if (ctrl_print) ctrl_print.setPrintersInfo(currentPrinter, printers)
        } else if (/file:saveas/.test(cmd)) {
          webapp.getController("Main").api.asc_DownloadAs()
        } else if (/file:save/.test(cmd)) {
      ...
      if (window.native_message_cmd) {
        for (const c in window.native_message_cmd) {
    ...
    const _serializeHeaderButton = (action, config) => ({
      action: action,
    ...
    const _onTitleButtonDisabled = (action, e, status) => {
      const _buttons = {}
      _buttons[action] = status
      native.execCommand("title:button", JSON.stringify({ disabled: _buttons }))
    }

    const _onSaveIconChanged = (e, opts) => {
      native.execCommand(
        "title:button",
        JSON.stringify({ "icon:changed": { save: btnsave_icons[opts.next] } }),
      )
    }

    const _onModalDialog = (status) => {
      if (status === "open") {
    ...
    const _onHintsShow = (visible, level) => {
      const info = {
      ...
      if (titlebuttons) {
        info.hints = {}
        !!titlebuttons.print &&
          (info.hints.print = titlebuttons.print.btn.btnEl.attr("data-hint-title-lang"))
        !!titlebuttons.quickprint &&
          (info.hints.quickprint = titlebuttons.quickprint.btn.btnEl.attr("data-hint-title-lang"))
        !!titlebuttons.undo &&
          (info.hints.undo = titlebuttons.undo.btn.btnEl.attr("data-hint-title-lang"))
        !!titlebuttons.redo &&
          (info.hints.redo = titlebuttons.redo.btn.btnEl.attr("data-hint-title-lang"))
        !!titlebuttons.save &&
          (info.hints.save = titlebuttons.save.btn.btnEl.attr("data-hint-title-lang"))
        !!titlebuttons.startover &&
          (info.hints.startover = titlebuttons.startover.btn.btnEl.attr("data-hint-title-lang"))
      }
    ...
    const _onApplySettings = (menu) => {
      if (titlebuttons.quickprint) {
        const var_name = window.SSE
          ? "sse-settings-quick-print-button"
          : window.PE
            ? "pe-settings-quick-print-button"
            : window.PDFE
              ? "pdfe-settings-quick-print-button"
              : window.VE
                ? "ve-settings-quick-print-button"
                : "de-settings-quick-print-button"
    ...
    const _checkHelpAvailable = function () {
      const build_url = (arg1, arg2, arg3) => {
        const re_ls = /\/$/
      ...
      fetch(build_url("resources/help/", Common.Locale.getDefaultLanguage(), "/Contents.json"))
        .then((response) => {
          if (response.ok) {
            /* local help avail */
            fetch(
              build_url("resources/help/", Common.Locale.getCurrentLanguage(), "/Contents.json"),
            )
              .then((response) => {
                if (response.ok)
        ...
        .catch((e) => {
          if (this.helpUrl()) {
            fetch(build_url(this.helpUrl(), Common.Locale.getDefaultLanguage(), "/Contents.json"))
              .then((response) => {
                // if ( response.ok )
              ...
              .then((text) => {
                /* remote help avail */
                fetch(
                  build_url(this.helpUrl(), Common.Locale.getCurrentLanguage(), "/Contents.json"),
                )
                  .then((response) => {
                    // if ( response.ok )
                    return response.json()
                  ...
                  .then((t) => {
                    helpUrl = build_url(this.helpUrl(), Common.Locale.getCurrentLanguage(), "")
                  })
                  .catch((e) => {
                    helpUrl = build_url(this.helpUrl(), Common.Locale.getDefaultLanguage(), "")
                  })
              })
              .catch((e) => {})
          }
    ...
    const _onDocumentReady = () => {
      if (config.isEdit) {
        function get_locked_message(t) {
          switch (t) {
            ...
            case Asc.c_oAscLocalRestrictionType.ReadOnly:
              return Common.Locale.get("tipFileReadOnly", {
                name: "Common.Translation",
                default:
                  "Document is read only. You can make changes and save its local copy later.",
              })
            default:
              return Common.Locale.get("tipFileLocked", {
                name: "Common.Translation",
                default:
                  "Document is locked for editing. You can make changes and save its local copy later.",
              })
          }
        }
        ...
        const locktype = api.asc_getLocalRestrictions
          ? api.asc_getLocalRestrictions()
          : Asc.c_oAscLocalRestrictionType.None
        if (Asc.c_oAscLocalRestrictionType.None !== locktype) {
          ...
          api.asc_registerCallback("asc_onDocumentName", () => {
    ...
    const _onHidePreloader = function (mode) {
      ...
      if (!features.viewmode) {
...

        {
          ...
          header.btnHome = new Common.UI.Button({
            ...
            hint: Common.Locale.get("hintBtnHome", {
              name: "Common.Controllers.Desktop",
          ...
          $("#id-box-doc-name").on({
            dblclick: (e) => {
              native.execCommand(
                "title:dblclick",
                JSON.stringify({ x: e.originalEvent.screenX, y: e.originalEvent.screenY }),
              )
            },
            mousedown: (e) => {
              native.execCommand(
        ...
        if (header.btnSave) {
          ...
          const iconname = /\s?([^\s]+)$/.exec(titlebuttons.save.btn.$icon.attr("class"))
          !!iconname && iconname.length && (titlebuttons.save.icon = btnsave_icons[iconname])
        }

        if (header.btnPrint) titlebuttons.print = { btn: header.btnPrint }
        ...
        if (header.btnUndo) titlebuttons.undo = { btn: header.btnUndo }

        if (header.btnRedo) titlebuttons.redo = { btn: header.btnRedo }

        if (header.btnStartOver) {
          titlebuttons.startover = { btn: header.btnStartOver }
        }

        if (header.btnQuickAccess) titlebuttons.quickaccess = { btn: header.btnQuickAccess }
      ...
      if (config.callback_editorconfig) {
        config.callback_editorconfig()
        config.callback_editorconfig = undefined
      }
      ...
      for (const i in rawarray) {
        ...
        if (utils.matchFileFormat(_f_.type)) {
          if (_re_name.test(_f_.path)) {
            ...
            const dir = _f_.path.slice(0, _f_.path.length - name.length - 1)
    ...
    const _onOpenRecent = (menu, url, record) => {
      console.log("open recent")
    }

    const _onChangeQuickAccess = (props) => {
    ...
    const _tr = (id, defvalue) =>
      Common.Locale.get(id, { name: "Common.Controllers.Desktop", default: defvalue })
    ...
    return {
      init: function (opts) {
        ...
        if (config.isDesktopApp) {
          const is_win_xp = nativevars && nativevars.os === "winxp"
              ...
              FileMenu: {
                "item:click": (menu, action, isopts) => {
                  if (action === "file:exit") {
                    ...
                    menu.hide()
                  } else if (action === "create:fromtemplate") {
                    native.execCommand(
                      "create:new",
                      `template:${
                        window.SSE
      ...
      requestClose: () => {
        if (config.isDesktopApp && !!native) {
          native.execCommand(
            "editor:event",
            JSON.stringify({ action: "file:close", url: config.customization.goback.url }),
          )
        }
      },
      removeRecent: () => {
      ...
      isFeatureAvailable: (feature) => !!native && !!native[feature],
      isWinXp: () => nativevars && nativevars.os === "winxp",
      call: function (name) {
        if (native[name]) {
      ...
      isHelpAvailable: () => !!helpUrl,
      getDefaultPrinterName: () => (nativevars ? nativevars.defaultPrinterName : ""),
      systemThemeType: () =>
        nativevars.theme && !!nativevars.theme.system
          ? nativevars.theme.system
          : window.matchMedia("(prefers-color-scheme: dark)").matches
            ? "dark"
            : "light",
      systemThemeSupported: () => nativevars.theme && nativevars.theme.system !== "disabled",

      localThemes: () => (nativevars ? nativevars.localthemes : undefined),
      recentFiles: () => recents,
      ...
      uiRtlSupported: () => nativevars && nativevars.rtl !== undefined,
      systemLangs: () => (nativevars?.keyboard ? nativevars.keyboard.langs : undefined),
    }
  }
  ...
  utils.parseFileFormat = (format) => {
    switch (format) {
      ...
      case utils.defines.FileFormat.FILE_DOCUMENT_DOTM:
        return "dotm"
      case utils.defines.FileFormat.FILE_DOCUMENT_XML:
        return "xml"
      case utils.defines.FileFormat.FILE_DOCUMENT_PAGES:
        return "pages"
      case utils.defines.FileFormat.FILE_DOCUMENT_HWP:
        return "hwpx"
      case utils.defines.FileFormat.FILE_DOCUMENT_HWPX:
        return "hwp"
      ...
      case utils.defines.FileFormat.FILE_SPREADSHEET_XLSM:
        return "xlsm"
      case utils.defines.FileFormat.FILE_SPREADSHEET_ODS_FLAT:
        return "fods"
      case utils.defines.FileFormat.FILE_SPREADSHEET_NUMBERS:
        return "numbers"
      ...
      case utils.defines.FileFormat.FILE_PRESENTATION_POTM:
        return "potm"
      case utils.defines.FileFormat.FILE_PRESENTATION_ODP_FLAT:
        return "fodp"
      case utils.defines.FileFormat.FILE_PRESENTATION_ODG:
        return "odg"
      case utils.defines.FileFormat.FILE_PRESENTATION_KEY:
        return "key"
      ...
      case utils.defines.FileFormat.FILE_CROSSPLATFORM_DJVU:
        return "djvu"
      case utils.defines.FileFormat.FILE_CROSSPLATFORM_XPS:
        return "xps"
      ...
      case utils.defines.FileFormat.FILE_DRAW_VSDX:
        return "vsdx"
    }
  ...
  utils.matchFileFormat = (t) => {
    ...
    if (window.DE) {
      return (
        (t > utils.defines.FileFormat.FILE_DOCUMENT &&
          !(t > utils.defines.FileFormat.FILE_PRESENTATION)) ||
    ...
    if (window.VE) {
      return (
        t === utils.defines.FileFormat.FILE_DRAW_VSDX ||
