...
define([
  ...
  Common.Controllers.History = Backbone.Controller.extend(
    _.extend(
      {
        models: [],
        ...
        onResetStore: function () {
          const hasChanges = this.panelHistory.storeHistory.hasChanges()
        ...
        onSelectRevision: function (picker, item, record, e) {
          if (e) {
            const btn = $(e.target)
            if (btn?.hasClass("revision-restore")) {
              if (!record.get("hasParent"))
                Common.Gateway.requestRestore(
                  record.get("revision"),
                  undefined,
                  record.get("fileType"),
                )
                ...
                const fileType =
                  Asc.c_oAscFileType[(record.get("fileType") || "").toUpperCase()] ||
          ...
          if (!picker && record) this.panelHistory.viewHistoryList.scrollToRecord(record)

          const url = record.get("url")
          const rev = record.get("revision")
          ...
          if (_.isEmpty(url) || urlGetTime - record.get("urlGetTime") > 5 * 60000) {
            ...
            const urlDiff = record.get("urlDiff")
            ...
            const reviewController = this.getApplication().getController(
              "Common.Controllers.ReviewChanges",
            )
            if (reviewController) reviewController.onApiShowChange()
          }
        },
        ...
        onSetHistoryData: function (opts) {
          ...
          if (!opts.data || opts.data.error) {
            ...
            if (historyStore && data !== null) {
              let rev
              const revisions = historyStore.findRevisions(data.version)
              const urlGetTime = new Date()
              const diff =
                !opts.data.previous || this.currentChangeId === undefined
                  ? null
                  : opts.data.changesUrl // if revision has changes, but serverVersion !== app.buildVersion -> hide revision changes
              const url =
                !_.isEmpty(diff) && opts.data.previous ? opts.data.previous.url : opts.data.url
              const fileType =
                !_.isEmpty(diff) && opts.data.previous
                  ? opts.data.previous.fileType
                  : opts.data.fileType
              const docId = opts.data.key ? opts.data.key : this.currentDocId
              const docIdPrev = opts.data.previous?.key
                ? opts.data.previous.key
                : this.currentDocIdPrev
              const token = opts.data.token
        ...
        onClickExpand: function () {
          const store = this.panelHistory.storeHistory
          const needExpand = store.hasCollapsed()
          ...
          this.panelHistory.btnExpand.setCaption(
            needExpand ? this.panelHistory.textHideAll : this.panelHistory.textShowAll,
          )
        },

        onShowBeforeButtonMenu: function () {
          if (this.api && this.panelHistory.chHighlightDeleted) {
            this.panelHistory.chHighlightDeleted.setChecked(
              this.api.asc_isShowedDeletedTextInVersionHistory(),
              true,
            )
          }
        },
        ...
        avatarsUpdate: function (type, users) {
          ...
          if (users && users.length > 0) {
            this.panelHistory.storeHistory.each((item) => {
              const user = _.findWhere(users, { id: item.get("userid") })
        ...
        onHashError: function () {
          ...
          const rec = store.findWhere({ revision: this.currentRev })
