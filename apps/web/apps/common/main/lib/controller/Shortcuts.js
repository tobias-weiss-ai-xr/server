...
define(["core"], () => {
  Common.Controllers.Shortcuts = Backbone.Controller.extend(
    _.extend(
      {
        initialize: function () {
        ...
        resetAllShortcuts: function () {
          ...
          for (const actionType in Asc.c_oAscDefaultShortcuts) {
            ...
            actionItem.shortcuts = Asc.c_oAscDefaultShortcuts[actionType].map((ascShortcut) => ({
              keys: this._getAscShortcutKeys(ascShortcut),
              isCustom: false,
        ...
        updateShortcutsForAction: function (actionType, updatedShortcuts) {
          ...
          originalShortcuts.forEach((item) => {
            const existsInUpdated = _.some(
              updatedShortcuts,
              (el) =>
                el.ascShortcut.asc_GetShortcutIndex() === item.ascShortcut.asc_GetShortcutIndex(),
            )
          ...
          updatedShortcuts.forEach((item) => {
            const existsInOriginal = _.some(
              originalShortcuts,
              (el) =>
                el.ascShortcut.asc_GetShortcutIndex() === item.ascShortcut.asc_GetShortcutIndex(),
            )
          ...
          const removableIndexes = updatedShortcuts.map((item) =>
            item.ascShortcut.asc_GetShortcutIndex(),
          )
          ...
          for (const type in this.getActionsMap()) {
            ...
            const foundIndex = _.findIndex(shortcuts, (shortcut) => {
              const foundIndex = _.indexOf(
                removableIndexes,
                shortcut.ascShortcut.asc_GetShortcutIndex(),
              )
          ...
          this.api.asc_applyAscShortcuts(resultShortcuts.map((shortcut) => shortcut.ascShortcut))

          //Filter for save in local storage
          const savedInStorage = {
            [actionType]: _.filter(
              resultShortcuts,
              (item) => item.isCustom !== item.ascShortcut.asc_IsHidden(),
            ).map((shortcut) => shortcut.ascShortcut),
          }
        ...
        getShortcutsByActionType: function (actionType, withHidden) {
          ...
          if (!withHidden && shortcuts) {
            shortcuts = shortcuts.filter((shortcut) => !shortcut.ascShortcut.asc_IsHidden())
          }
          return shortcuts
        },
            ...
            if (item) {
              const callback = function () {
                ...
                const firstShortcut = action
                  ...
                  : null
                const hintText =
                  (item.label || "") + (firstShortcut ? ` (${firstShortcut.keys.join("+")})` : "")
        ...
        _eventsTrigger: function (actionTypes) {
          let lists = []
          if (actionTypes) {
            lists = actionTypes.map((type) => {
              type =
                typeof +type === "number" ? this._getDefaultShortcutActionsInvert()[type] : type
        ...
        _fillActionsMap: function () {
          ...
          _.pairs(Asc.c_oAscDefaultShortcuts).forEach((item) => {
            const actionType = item[0]
            const shortcuts = item[1]
            const actionItem = this.actionsMap[actionType]
          ...
          _.pairs(this._getModifiedShortcuts()).forEach((item) => {
            const actionType = item[0]
            const shortcuts = item[1]
            const actionItem = this.actionsMap[actionType]
            ...
            if (actionItem) {
              shortcuts.forEach((ascShortcut) => {
                ...
                const defaultShortcutIndex = _.findIndex(
        ...
        _applyShortcutsInSDK: function () {
          const applyMethod = function (storage) {
            storage = JSON.parse(
              storage || Common.localStorage.getItem(this.localStorageKey) || "{}",
            )
        ...
        _saveModifiedShortcuts: function (savedActionsMap, removedActionsMap) {
          ...
          for (const actionType in removedActionsMap || {}) {
            ...
            customShortcuts[actionType] = _.filter(
              customShortcuts[actionType],
              (ascShortcut) =>
                !_.some(
                  removed,
                  (r) => ascShortcut.asc_GetShortcutIndex() === r.asc_GetShortcutIndex(),
                ),
            )
          }

          for (const actionType in customShortcuts) {
            customShortcuts[actionType] = customShortcuts[actionType].map((ascShortcut) =>
              ascShortcut.asc_ToJson(),
            )
          }
