...
define(["core"], () => {
  ...
  Common.UI.TabStyler = new (function () {
    let _customization
    let _canChangeStyle = true
    let _canChangeBackground = true

    const _init = (customization) => {
      _customization = customization
      _canChangeStyle = Common.UI.FeaturesManager.canChange("tabStyle", true)
      _canChangeBackground =
        !Common.Utils.isIE && Common.UI.FeaturesManager.canChange("tabBackground", true)

      let value = Common.UI.FeaturesManager.getInitValue("tabStyle", true)
      if (_canChangeStyle && Common.localStorage.itemExists("settings-tab-style")) {
        // get from local storage
        const themeid = Common.UI.Themes.currentThemeId()
        const isNew =
          themeid === "theme-system" || themeid === "theme-white" || themeid === "theme-night"
        value =
          isNew && !Common.localStorage.itemExists("settings-tab-style-newtheme")
            ? "line"
            : Common.localStorage.getItem("settings-tab-style")
      } else if (value === undefined) {
        value =
          _customization && typeof _customization === "object" && _customization.toolbarNoTabs
            ? "line"
            : Common.UI.Themes.getThemeProps("tab-style")
      }
      ...
      if (_canChangeBackground && Common.localStorage.itemExists("settings-tab-background")) {
        // get from local storage
        value = Common.localStorage.getItem("settings-tab-background")
      } else if (
        value === undefined &&
        _customization &&
        typeof _customization === "object" &&
        _customization.toolbarNoTabs
      ) {
        value = "toolbar"
      }
      ...
      _customization &&
        ...
        _customization.toolbarNoTabs &&
        console.log(
          "Obsolete: The 'toolbarNoTabs' parameter of the 'customization' section is deprecated. Please use 'tabStyle' and 'tabBackground' parameters in the 'customization.features' section instead.",
        )
    ...
    const _refreshStyle = () => {
      if (
        Common.localStorage.getItem("settings-tab-style") !==
        Common.Utils.InternalSettings.get("settings-tab-style")
      ) {
    ...
    const _refreshBackground = () => {
      if (
        Common.localStorage.getItem("settings-tab-background") !==
        Common.Utils.InternalSettings.get("settings-tab-background")
      ) {
    ...
    const _setBackground = (background) => {
