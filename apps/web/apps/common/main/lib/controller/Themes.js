...
define(["core"], () => {
  ...
  Common.UI.Themes = new (function (locale) {
    ...
    const get_current_theme_colors = (c) => {
    ...
    const create_colors_css = (id, colors) => {
      if (!!colors && !!id) {
        const _css_array = [":root .", id, "{"]
    ...
    const normalize_theme_icons = (t) => {
      if (t.src?.icons) {
        ...
        if (t.src.icons["style-class-selector"]) t.icons.src = t.src.icons["style-class-selector"]
      }
    }

    const parse_themes_object = (obj) => {
      const curr_lang = Common.Locale.getCurrentLanguage()
      let theme_label
      if (!!obj.themes && Array.isArray(obj.themes)) {
        obj.themes.forEach((item) => {
          if (item.id) {
    ...
    const check_launched_custom_theme = () => {
      ...
      if (themes_map[theme_id]) {
        if (themes_map[theme_id].source !== "static") {
          ...
          if (!m) document.body.classList.add(`theme-type-${themes_map[theme_id].type}`)
          else if (m.length)
            document.body.className = document.body.className.replace(
              /theme-type-(?:dark|ligth)/i,
      ...
      if (!s || get_ui_theme_name(s) !== theme_id) {
        ...
        if (themes_map[theme_id].icons) theme_obj.icons = themes_map[theme_id].icons

        Common.localStorage.setItem("ui-theme", JSON.stringify(theme_obj))
      }
    }

    const get_themes_config = (url) => {
      Common.Utils.loadConfig(url, (obj) => {
        if (obj !== "error") {
    ...
    const on_document_ready = function (el) {
    ...
    const get_ui_theme_name = (objtheme) => {
      if (typeof objtheme === "string" && objtheme.startsWith("{") && objtheme.endsWith("}")) {
    ...
    const apply_theme = function (id) {
      ...
      if (themes_map[theme_id].icons) {
        ...
        else if (icons_base_url) themes_map[theme_id].icons.basepath = icons_base_url
      ...
      if (validate_vars(colors_obj)) {
        ...
        if (!(Common.Utils.isIE10 || Common.Utils.isIE11)) {
          ...
          if (theme_id !== id) {
            ...
            const theme_obj = Object.assign({ id: id, colors: colors_obj }, themes_map[id])
            theme_obj.source = undefined
    ...
    return {
      init: function (api) {
        ...
        if (api.asc_setContentDarkMode) api.asc_setContentDarkMode(is_content_dark)
        ...
        if (themes_map[theme_id]?.icons) {
          if (
            ...
            if (icons_base_url) window.uitheme.apply_icons_from_url(theme_id, icons_base_url)
          }
      ...
      map: () => {
        if (
          ...
          new_map["theme-system"] = undefined
      ...
      get: (id) => themes_map[id],

      currentThemeId: () =>
        themes_map[window.uitheme.id] ? window.uitheme.id : window.uitheme.DEFAULT_LIGHT_THEME_ID,

      currentThemeColor: (token) => getComputedStyle(document.body).getPropertyValue(token),
      ...
      isContentThemeDark: () => window.uitheme.iscontentdark,

      setContentTheme: function (mode, force, keep) {
        const set_dark = mode === "dark"
      ...
      toggleContentTheme: function () {
        ...
        if (this.isDarkTheme()) this.api.asc_setContentDarkMode(window.uitheme.iscontentdark)
      ...
      getThemeProps: (prop) => {
        if (prop === "font") {
          ...
          return Common.Utils.isIE || Common.Controllers.Desktop?.isWinXp()
            ? "fill"
            : window
          ...
          if (!theme_props[prop]) {
            theme_props[prop] =
              window.getComputedStyle(document.body).getPropertyValue("--x-small-btn-size") ||
              "20px"
          }
        }
