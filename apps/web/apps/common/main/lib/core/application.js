define(["underscore"], (_) => {
  const resolveNamespace = (className, root) => {
    const parts = className.split(".")
    let current = root || window

    for (let a = 0, b = parts.length; a < b; a++) {
  ...
  const Application = function (options) {
    ...
    if (this.autoCreate !== false) $($.proxy(this.onReady, this))
  }
  ...
  _.extend(Application.prototype, {
    ...
    getClasseRefs: function (type, classes) {
      const hashMap = {}
      const allocationMap = this.allocationMap[type]
      const root = this[allocationMap]
    ...
    initializeControllers: function (controllers) {
      ...
      _.each(
        controllers,
        function (ctrl) {
          const root = ctrl.indexOf(".") > -1 ? window : this[this.allocationMap.controller]
          const classReference = resolveNamespace(ctrl, root)
          const id = ctrl.split(".").pop()

          const controller = new classReference({
            id: ctrl,
            application: this,
          })
    ...
    getModel: function (name) {
      ...
      let model = this._modelsCache[name]
      const modelClass = this.getModelConstructor(name)
    ...
    createModel: function (name, options) {
      ...
      if (modelClass) model = new modelClass(_.extend(options || {}))
    ...
    getCollection: function (name) {
      ...
      let collection = this._collectionsCache[name]
      const collectionClass = this.getCollectionConstructor(name)
    ...
    createCollection: function (name) {
      ...
      if (collectionClass) collection = new collectionClass()
  ...
  _.extend(Controller.prototype, {
    ...
    createView: function (name, options) {
      const view = this.getViewConstructor(name)
   ...
   * @private
   */
  const EventBus = function (options) {
    const me = this
  ...
  _.extend(EventBus.prototype, {
    ...
    addListeners: function (selectors, controller) {
      ...
      const pool = this.pool[controller.id]
    ...
    fireEvent: function (selector, event, args) {
      ...
      _.each(
        this.pool,
        (eventsPoolByAlias, controllerId) => {
          const events = eventsPoolByAlias[selector]

          if (events) {
            const listeners = events[event]
            const controller = application.getController(controllerId)
