...
function onDropDownKeyDown(e) {
  ...
  const beforeEvent = jQuery.Event("keydown.before.bs.dropdown", { keyCode: e.keyCode })
  const afterEvent = jQuery.Event("keydown.after.bs.dropdown", { keyCode: e.keyCode })

  $parent.trigger(beforeEvent)

  if ($parent.hasClass("no-stop-propagate")) {
    if (arguments.length > 1 && arguments[1] instanceof KeyboardEvent) e = arguments[1]
    ...
    if (
      !(
        (/^(13|9|32)/.test(e.keyCode) || (e.keyCode === 27 && !$parent.hasClass("open"))) &&
        !e.ctrlKey &&
        !e.altKey
      )
    ) {
...
function checkFocusedItem(cmp, item) {
  const innerHeight = cmp.innerHeight()
  const padding = (innerHeight - cmp.height()) / 2
  const pos = Common.Utils.getPosition(item).top
  const itemHeight = item.outerHeight()
  if (pos < 0) cmp.scrollTop(cmp.scrollTop() + pos - padding)
...
function patchDropDownKeyDown(e) {
  ...
  const isActive = $parent.hasClass("open") || $parent.hasClass("over")
  ...
  let $items = $("> [role=menu] > li:not(.divider):not(.disabled):visible", $parent).find("> a")
  ...
  if (e.keyCode === 39) {
    ...
    const li = $items.eq(index).parent()
    if (li.hasClass("dropdown-submenu") && !li.hasClass("over")) {
      // open submenu and select first <li> item
      li.mouseenter()
      li.addClass("focused-submenu")
      _.delay(() => {
        const mnu = $("> [role=menu]", li)
        const $subitems = mnu.find("> li:not(.divider):not(.disabled):visible > a")
        const $dataviews = mnu.find("> li:not(.divider):not(.disabled):visible .dataview")
        const $palette = mnu.find(
          "> li:not(.divider):not(.disabled):visible .theme-colorpalette.focused",
        )
        const $internal_menu = mnu.find(
          "> li:not(.divider):not(.disabled):visible ul.internal-menu",
        )
        if (
          $subitems.length > 0 &&
          $dataviews.length < 1 &&
          $internal_menu.length < 1 &&
          $palette.length < 1
        )
...
function patchDropDownKeyDownAdditional(e) {
  // only for formula menu when typing in cell editor
  if (!/(38|40|27|37|39)/.test(e.keyCode)) return
  ...
  const $items = $("> [role=menu] > li:not(.divider):not(.disabled):visible", $parent).find("> a")
  ...
  if (previndex !== index) {
    let tip = $items.eq(previndex).parent().data("bs.tooltip")
    if (tip) {
      tip.hide()
    }
...
"ontouchstart" in document.documentElement &&
  $(document).on("click.bs.dropdown.data-api", "[data-toggle=dropdown]", toggle)
/*
 *      workaround closing menu by right click
 * */
;(() => {
  const eventsObj = $._data($(document).get(0), "events")
  let clickDefHandler
  if (eventsObj?.click) {
    eventsObj.click.every((e, i, a) => {
  ...
  function onDropDownClick(e) {
    if (e.which === 1 || e.which === undefined) clearMenus(/form-control/.test(e.target.className))
  }
