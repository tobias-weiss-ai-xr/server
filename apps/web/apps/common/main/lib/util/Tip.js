...
const tip = ($) => {
  ...
  const Tip = function (element, options) {
  ...
  Tip.prototype = $.extend({}, $.fn.tooltip.Constructor.prototype, {
    ...
    init: function () {
      ...
      Common.NotificationCenter.on({
        "layout:changed": (e) => {
          if (!this.options.keepvisible && !this.options.hideonclick && this.tip().is(":visible"))
            this.hide()
        },
      })
    ...
    mousemove: function (e) {
      const x = e.clientX * Common.Utils.zoom()
      const y = e.clientY * Common.Utils.zoom()
    ...
    show: function (at) {
      if (this.hasContent() && this.enabled && !this.dontShow) {
        ...
        if (!at && placementEx && !placementEx[2].length) {
          ...
          if (typeof this.options.placement === "function") {
            ...
            switch (placementEx[1]) {
              case "bottom":
                tp = { top: pos.top + pos.height + 10 }
                break
              case "top":
                tp = { top: pos.top - actualHeight - 10 }
                break
            }
        ...
        this.timeout = setTimeout(() => {
          if (this.hoverState === "in") this.hide()
          this.dontShow = false
        }, 5000)
      }
    },
    ...
    moveArrow: function () {
      const $arrow = this.tip().find(".tooltip-arrow, .arrow")
      const new_arrow_position = 10
    ...
    enter: function (obj) {
      ...
      if (
        $target.is("[role=menu]") ||
        ($target.parentsUntil(obj.currentTarget, "[role=menu]").length &&
          obj.target !== obj.currentTarget) ||
        this.tip().is(":visible")
      ) {
        return
      }
      ...
      const self =
        obj instanceof this.constructor
          ? obj
          : $(obj.currentTarget)[this.type](this.getDelegateOptions()).data("bs.tooltip")
    ...
    replaceArrow: function (delta, dimension, position) {
      this.options.arrow === false
        ? this.arrow().hide()
        : _superclass.prototype.replaceArrow.apply(this, arguments)
    },

    getCalculatedOffset: function (placement, pos, actualWidth, actualHeight) {
      const out = _superclass.prototype.getCalculatedOffset.apply(this, arguments)
