...
if (window.AscDesktopEditor) {
  ...
  window.on_native_message = (cmd, param) => {
    ...
    } else window.native_message_cmd[cmd] = param
  }
  ...
  if (window.RendererProcessVariable) {
    ...
    if (theme) {
      ...
      if (/dark|light/.test(theme.system)) {
        window.uitheme.is_system_theme_dark = () => theme.system === "dark"
      }

      if (map_themes?.[theme.id]) {
    ...
    if (window.RendererProcessVariable.rtl !== undefined) {
      window.nativeprocvars = {
        rtl:
          window.RendererProcessVariable.rtl === true ||
          window.RendererProcessVariable.rtl === "yes" ||
          window.RendererProcessVariable.rtl === "true",
      }
    }
  }
  ...
  if (!params || !params.internal) {
    ...
    window.desktop.execCommand(
      "webapps:entry",
      (window.features && JSON.stringify(window.features)) || "",
    )
  }
}
