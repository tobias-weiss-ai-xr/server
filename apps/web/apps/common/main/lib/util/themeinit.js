...
;+(function init_themes() {
  ...
  if (!window.uitheme.is_system_theme_dark)
    window.uitheme.is_system_theme_dark = () =>
  ...
  window.uitheme.apply_icons_from_url = (themeid, url) => {
    if (!url) return
    ...
    if (base_url.lastIndexOf("../img/", 0) === 0) base_url = base_url.replace("..", "./resources")

    if (window.Common?.Utils)
      window.Common.Utils.injectSvgIcons(
        [
  ...
  if (objtheme) {
    ...
    if (objtheme) {
      if (window.uitheme.id && window.uitheme.id !== objtheme.id) {
        ...
        if (objtheme.colors) {
          header_tokens.forEach((i) => {
            !!objtheme.colors[i] &&
              document.documentElement.style.setProperty(`--${i}`, objtheme.colors[i])
          })

          const colors = []
          for (const c in objtheme.colors) {
