...
define(["common/main/lib/component/BaseView", "common/main/lib/component/Scroller"], () => {
  Common.Views.About = Common.UI.BaseView.extend(
    _.extend(
      {
        menu: undefined,
        rendered: false,
        ...
        renderRow: (type, label, value) => {
          if (!value || !value.trim()) return ""
          const td = '<td colspan="3" align="center" class="padding-small">'
          switch (type) {
            case "heading":
              return `<tr><td colspan="3" align="center" style="padding: 20px 0 10px 0;"><label class="asc-about-companyname">${value}</label></td></tr>`
            case "email":
              return `<tr>${td}<label class="asc-about-desc-name">${label}</label><a href="mailto:${value}">${value}</a></td></tr>`
            case "link":
              return `<tr><td colspan="3" align="center"><a href="${value}" target="_blank">${value.replace(/https?:\/\//, "").replace(/\/$/, "")}</a></td></tr>`
            case "phone":
              return `<tr>${td}<label class="asc-about-desc-name">${label}</label><label class="asc-about-desc" dir="ltr">${value}</label></td></tr>`
            default:
              return `<tr>${td}<label class="asc-about-desc-name">${label}</label><label class="asc-about-desc">${value}</label></td></tr>`
          }
        },
        ...
        render: function () {
          if (!this.rendered) {
            ...
            if (this.licData) this.setLicInfo(this.licData)
        ...
        changeLogo: function () {
          ...
          const customer = this.licData.customer
        ...
        setMode: function (mode) {
          if (mode.isLightVersion) {
            $("#id-about-licensor-version-name").text(
              this.txtEdition + this.txtVersion + this.txtVersionNum,
            )
            $("#id-about-licensee-version-name").text(
              this.txtEdition + this.txtVersion + this.txtVersionNum,
            )
          }
        },
