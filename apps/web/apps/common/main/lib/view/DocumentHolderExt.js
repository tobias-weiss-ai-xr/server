...
define([], () => {
  const _editor = window.DE || window.PDFE || window.PDFE || window.PE || window.SSE || window.VE
  if (_editor?.Views?.DocumentHolder) {
    const dh = _editor.Views.DocumentHolder.prototype

    dh.addEquationMenu = function (equationMenu, insertIdx) {
      if (!equationMenu) return
      this.clearEquationMenu(equationMenu, insertIdx)

      const menuItems = this.initEquationMenu()
      if (menuItems.length > 0) {
        _.each(menuItems, (menuItem, index) => {
          if (menuItem.menu) {
            _.each(menuItem.menu.items, (item) => {
              item.on("click", _.bind(this.equationCallback, this, item.options.equationProps))
            })
          } else
            menuItem.on(
              "click",
              _.bind(this.equationCallback, this, menuItem.options.equationProps),
            )
    ...
    dh.clearEquationMenu = (equationMenu, insertIdx) => {
      for (let i = insertIdx; i < equationMenu.items.length; i++) {
        if (equationMenu.items[i].options.equation) {
          if (equationMenu.items[i].menu) {
            _.each(equationMenu.items[i].menu.items, (item) => {
              item.off("click")
            })
          } else equationMenu.items[i].off("click")
    ...
    dh.initEquationMenu = function () {
      const me = this._currentTranslateObj // in sse - controller, in other - view
      if (!me._currentMathObj) return
      const type = me._currentMathObj.get_Type()
      const value = me._currentMathObj
      let mnu
      const arr = []
      ...
      switch (type) {
        ...
        case Asc.c_oAscMathInterfaceType.Bar:
          ...
          mnu = new Common.UI.MenuItem({
            caption:
              value.get_Pos() === Asc.c_oAscMathInterfaceBarPos.Top
                ? me.txtUnderbar
                : me.txtOverbar,
            equation: true,
            disabled: me._currentParaObjDisabled,
            equationProps: {
              ...
              value:
                value.get_Pos() === Asc.c_oAscMathInterfaceBarPos.Top
                  ? Asc.c_oAscMathInterfaceBarPos.Bottom
                  : Asc.c_oAscMathInterfaceBarPos.Top,
            },
          })
          arr.push(mnu)
          break
        case Asc.c_oAscMathInterfaceType.Script: {
          const scripttype = value.get_ScriptType()
          if (scripttype === Asc.c_oAscMathInterfaceScript.PreSubSup) {
            ...
            if (
              scripttype === Asc.c_oAscMathInterfaceScript.SubSup ||
              scripttype === Asc.c_oAscMathInterfaceScript.Sub
            ) {
              mnu = new Common.UI.MenuItem({
            ...
            if (
              scripttype === Asc.c_oAscMathInterfaceScript.SubSup ||
              scripttype === Asc.c_oAscMathInterfaceScript.Sup
            ) {
              mnu = new Common.UI.MenuItem({
                ...
                equationProps: {
                  ...
                  value:
                    scripttype === Asc.c_oAscMathInterfaceScript.SubSup
                      ? Asc.c_oAscMathInterfaceScript.Sub
                      : Asc.c_oAscMathInterfaceScript.None,
                },
              })
              arr.push(mnu)
        ...
        case Asc.c_oAscMathInterfaceType.Fraction: {
          const fraction = value.get_FractionType()
          if (
            fraction === Asc.c_oAscMathInterfaceFraction.Skewed ||
            fraction === Asc.c_oAscMathInterfaceFraction.Linear
          ) {
            mnu = new Common.UI.MenuItem({
          ...
          if (
            fraction === Asc.c_oAscMathInterfaceFraction.Bar ||
            fraction === Asc.c_oAscMathInterfaceFraction.Linear
          ) {
            mnu = new Common.UI.MenuItem({
          ...
          if (
            fraction === Asc.c_oAscMathInterfaceFraction.Bar ||
            fraction === Asc.c_oAscMathInterfaceFraction.NoBar
          ) {
            mnu = new Common.UI.MenuItem({
              caption:
                fraction === Asc.c_oAscMathInterfaceFraction.Bar
                  ? me.txtRemFractionBar
                  : me.txtAddFractionBar,
              equation: true,
              disabled: me._currentParaObjDisabled,
              equationProps: {
                type: type,
                ...
                value:
                  fraction === Asc.c_oAscMathInterfaceFraction.Bar
                    ? Asc.c_oAscMathInterfaceFraction.NoBar
                    : Asc.c_oAscMathInterfaceFraction.Bar,
              },
            })
            arr.push(mnu)
          }
        ...
        case Asc.c_oAscMathInterfaceType.Limit:
          mnu = new Common.UI.MenuItem({
            caption:
              value.get_Pos() === Asc.c_oAscMathInterfaceLimitPos.Top
                ? me.txtLimitUnder
                : me.txtLimitOver,
            equation: true,
            disabled: me._currentParaObjDisabled,
            equationProps: {
              ...
              value:
                value.get_Pos() === Asc.c_oAscMathInterfaceLimitPos.Top
                  ? Asc.c_oAscMathInterfaceLimitPos.Bottom
                  : Asc.c_oAscMathInterfaceLimitPos.Top,
            },
          })
          arr.push(mnu)
          mnu = new Common.UI.MenuItem({
            ...
            menu: new Common.UI.Menu({
              ...
              items: [
                ...
                },
                {
                  ...
                  checkable: true,
                  checked:
                    value.get_MatrixAlign() === Asc.c_oAscMathInterfaceMatrixMatrixAlign.Center,
                  equationProps: {
                    type: type,
                    callback: "put_MatrixAlign",
                ...
                },
                {
                  ...
                  checkable: true,
                  checked:
                    value.get_MatrixAlign() === Asc.c_oAscMathInterfaceMatrixMatrixAlign.Bottom,
                  equationProps: {
                    type: type,
                    callback: "put_MatrixAlign",
              ...
              items: [
                {
                  ...
                  checkable: true,
                  checked:
                    value.get_ColumnAlign() === Asc.c_oAscMathInterfaceMatrixColumnAlign.Left,
                  equationProps: {
                    type: type,
                ...
                },
                {
                  ...
                  checkable: true,
                  checked:
                    value.get_ColumnAlign() === Asc.c_oAscMathInterfaceMatrixColumnAlign.Center,
                  equationProps: {
                    type: type,
                    callback: "put_ColumnAlign",
                ...
                },
                {
                  ...
                  checkable: true,
                  checked:
                    value.get_ColumnAlign() === Asc.c_oAscMathInterfaceMatrixColumnAlign.Right,
                  equationProps: {
                    type: type,
                    callback: "put_ColumnAlign",
        ...
        case Asc.c_oAscMathInterfaceType.LargeOperator:
          mnu = new Common.UI.MenuItem({
            ...
            equationProps: {
              ...
              value:
                value.get_LimitLocation() === Asc.c_oAscMathInterfaceNaryLimitLocation.UndOvr
                  ? Asc.c_oAscMathInterfaceNaryLimitLocation.SubSup
                  : Asc.c_oAscMathInterfaceNaryLimitLocation.UndOvr,
            },
          })
          arr.push(mnu)
        ...
        case Asc.c_oAscMathInterfaceType.Delimiter:
          ...
          arr.push(mnu)
          mnu = new Common.UI.MenuItem({
            caption: value.get_HideClosingBracket()
              ? me.txtShowCloseBracket
              : me.txtHideCloseBracket,
            equation: true,
            disabled: me._currentParaObjDisabled,
            equationProps: {
              type: type,
        ...
        case Asc.c_oAscMathInterfaceType.GroupChar:
          if (value.can_ChangePos()) {
            mnu = new Common.UI.MenuItem({
              caption:
                value.get_Pos() === Asc.c_oAscMathInterfaceGroupCharPos.Top
                  ? me.txtGroupCharUnder
                  : me.txtGroupCharOver,
              equation: true,
              disabled: me._currentParaObjDisabled,
              equationProps: {
                ...
                value:
                  value.get_Pos() === Asc.c_oAscMathInterfaceGroupCharPos.Top
                    ? Asc.c_oAscMathInterfaceGroupCharPos.Bottom
                    : Asc.c_oAscMathInterfaceGroupCharPos.Top,
              },
            })
            arr.push(mnu)
            mnu = new Common.UI.MenuItem({
    ...
    dh.updateCustomItems = function (menu, data) {
      ...
      const lang = this.mode?.lang ? this.mode.lang.split(/[\-_]/)[0] : "en"

      this._preventCustomClick && clearTimeout(this._preventCustomClick)
      this._hasCustomItems &&
        (this._preventCustomClick = setTimeout(() => {
          this._preventCustomClick = null
        }, 500)) // set delay only on update existing items
      this._hasCustomItems = true

      const findCustomItem = (guid, id) => {
        if (menu && menu.items.length > 0) {
          for (let i = menu.items.length - 1; i >= 0; i--) {
            if (
              menu.items[i].isCustomItem &&
              ((id === undefined && menu.items[i].options.guid === guid) ||
                (menu.items[i].options.guid === guid && menu.items[i].value === id))
            ) {
              return menu.items[i]
            }
          }
      ...
      const getMenu = (items, guid, toMenu) => {
        let hasIcons = false
        if (toMenu) toMenu.removeAll()
        else {
          ...
          toMenu.on("item:custom-click", (menu, item, e) => {
            !this._preventCustomClick &&
              this.api &&
              this.api.onPluginContextMenuItemClick &&
              this.api.onPluginContextMenuItemClick(item.options.guid, item.value)
          })
        ...
        items.forEach((item) => {
          ...
          item.text &&
            toMenu.addItem(
              new Common.UI.MenuItemCustom({
                caption:
                  (typeof item.text === "object" ? item.text[lang] || item.text.en : item.text) ||
                  "",
                value: item.id,
                guid: guid,
        ...
        hasIcons &&
          (toMenu.cmpEl
            ? toMenu.cmpEl.toggleClass("shifted-right", true)
            : (toMenu.options.cls = "shifted-right"))
        return toMenu
      }

      let focused
      data.forEach((plugin) => {
        const isnew = !findCustomItem(plugin.guid)
        if (plugin?.items && plugin.items.length > 0) {
          plugin.items.forEach((item) => {
            if (item.separator && isnew) {
              // add separator only to new plugins menu
              menu.addItem(
                new Common.UI.MenuItemCustom({
                  caption: "--",
                  guid: plugin.guid,
            ...
            if (!item.text) return
            const mnu = findCustomItem(plugin.guid, item.id)
            const caption =
              (typeof item.text === "object" ? item.text[lang] || item.text.en : item.text) || ""
            if (mnu) {
              ...
              if (item.items) {
                if (mnu.menu) {
                  if (
                    mnu.menu.isVisible() &&
                    mnu.menu.cmpEl
                      .find(" > li:not(.divider):not(.disabled):visible")
                      .find("> a")
                      .filter(":focus").length > 0
                  ) {
                    mnu.menu.isOver = true
                    focused = mnu.cmpEl
                  }
                  getMenu(item.items, plugin.guid, mnu.menu)
                } else mnu.setMenu(getMenu(item.items, plugin.guid))
              }
            } else {
              const mnu = new Common.UI.MenuItemCustom({
                ...
                guid: plugin.guid,
                menu:
                  item.items && item.items.length >= 0 ? getMenu(item.items, plugin.guid) : false,
                iconsSet: item.icons,
                baseUrl: "", // base url is included in icon path
                disabled: !!item.disabled,
              }).on("click", (item, e) => {
      ...
      if (focused) {
        const $subitems = $("> [role=menu]", focused).find(
