...
define([], () => {
  Common.Views.DocumentPropertyDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 320,
        ...
        initialize: function (options) {
          ...
          this.asc2type = (ascType) => {
            ...
            if (ascType === AscCommon.c_oVariantTypes.vtBool) {
              return "boolean"
            }
            if (ascType === AscCommon.c_oVariantTypes.vtFiletime) {
              return "date"
            }
          }
        ...
        render: function () {
          ...
          this.inputTitle = new Common.UI.InputField({
            ...
            validation: (value) => {
              if (this.options.nameValidator !== undefined) {
                return this.options.nameValidator(value)
              }
            },
          })
          ...
          const currentType = this.options.defaultValue.type
            ? this.asc2type(this.options.defaultValue.type)
            : "text"
          this.comboboxType.setValue(currentType)
          this.comboboxType.on(
            "selected",
            _.bind(function (cmb, record) {
              this.inputTextOrNumber.setVisible(
                record.value === "text" || record.value === "number",
              )
          ...
          this.inputTextOrNumber = new Common.UI.InputField({
            ...
            validation: function (value) {
              if (
                this.comboboxType.getValue() === "number" &&
                typeof value !== "number" &&
          ...
          this.inputTextOrNumber.setVisible(
            this.options.defaultValue.type
              ? this.options.defaultValue.type !== AscCommon.c_oVariantTypes.vtFiletime &&
                  this.options.defaultValue.type !== AscCommon.c_oVariantTypes.vtBool
              : currentType === "text" || currentType === "number",
          )
          ...
          this.comboboxBoolean.setValue(
            this.options.defaultValue.value !== undefined && currentType === "boolean"
              ? this.options.defaultValue.value
                ? 1
                : 0
              : 1,
          )
          this.comboboxBoolean.setVisible(
            this.options.defaultValue.type
              ? this.options.defaultValue.type === AscCommon.c_oVariantTypes.vtBool
              : currentType === "boolean",
          )
        ...
        getFocusedComponents: function () {
          return [
            this.inputTitle,
            this.comboboxType,
            this.inputTextOrNumber,
            this.comboboxBoolean,
            this.datepicker,
        ...
        _handleInput: function (state) {
          if (this.options.handler) {
            if (state === "ok") {
              ...
              const type = this.comboboxType.getValue()
              let ascValue
              let ascType
              if (type === "boolean") {
                ...
                if (!ascValue) {
                  ...
                  if (!ascValue || !(ascValue instanceof Date) || Number.isNaN(ascValue))
                    ascValue = undefined
                }
                if (!ascValue) {
                  const me = this
                  Common.UI.warning({
                    ...
                    callback: function (btn) {
                      if (btn === "ok") {
                        me.options.handler.call(
                          this,
                          state,
                          title,
                          AscCommon.c_oVariantTypes.vtLpwstr,
                          me.datepicker.getValue(),
                        )
                        me.close()
                      } else me.datepicker.focus()
                    },
                  })
                  return
                }
                ...
                let value = this.inputTextOrNumber.getValue()
                if (type === "text") {
                  ...
                  if (value % 1 === 0) {
                    ascType = AscCommon.c_oVariantTypes.vtI4
                    ascValue = Number.parseInt(value)
                  } else {
                    ascType = AscCommon.c_oVariantTypes.vtR8
                    ascValue = Number.parseFloat(value)
                  }
                }
              }
        ...
        dateToString: function (value) {
          ...
          if (value) {
            ...
            let lang = (this.options.lang || "en").replace("_", "-").toLowerCase()
            try {
              if (lang === "ar-SA".toLowerCase()) lang = `${lang}-u-nu-latn-ca-gregory`
              text = value.toLocaleString(lang, {
                year: "numeric",
                month: "2-digit",
                day: "2-digit",
              })
        ...
        errorDate:
          "You can choose a value from the calendar to store the value as Date.<br>If you enter a value manually, it will be stored as Text.",
      },
