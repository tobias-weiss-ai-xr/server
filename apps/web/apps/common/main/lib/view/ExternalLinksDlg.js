...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  Common.Views.ExternalLinksDlg = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
        ...
        render: function () {
          ...
          this.linksList = new Common.UI.ListView({
            ...
            simpleAddMode: true,
            headers: [
              { name: this.textSource, width: 240 },
              { name: this.textStatus, width: 175 },
            ],
          ...
          const el = $(this.btnUpdate.cmpEl.find("button")[0])
          ...
          this.chUpdate.on(
            ...
            _.bind(function (field, newValue, oldValue, eOpts) {
              this.api?.asc_setUpdateLinks(field.getValue() === "checked")
            }, this),
          )
        ...
        afterRender: function () {
          ...
          this.api.asc_registerCallback(
            "asc_onUpdateExternalReferenceList",
            this.wrapEvents.onUpdateExternalReferenceList,
          )
        ...
        _setDefaults: function (props) {
          this.refreshList()
          if (!window.SSE) this.chUpdate.setVisible(false)
          else if (this.api) this.chUpdate.setValue(this.api.asc_getUpdateLinks(), true)
        },
          ...
          if (links) {
            for (let i = 0; i < links.length; i++) {
              arr.push({
                ...
                value: (links[i].asc_getSource() || "").replace(/%20/g, " "),
                idx: i,
                externalRef: links[i],
        ...
        onUpdate: function () {
          ...
          const rec = this.linksList.getSelectedRec()
          if (rec) {
        ...
        onUpdateMenu: function (menu, item) {
          ...
          if (item.value === 1) {
            const arr = []
            this.linksList.store.each((item) => {
              arr.push(item.get("externalRef"))
              this.isOffline && this.setLinkStatus(item.get("linkid"), this.textOk)
            }, this)
        ...
        onOpen: function () {
          const rec = this.linksList.getSelectedRec()
          if (rec) {
            let data = this.api.asc_openExternalReference(rec.get("externalRef"))
            if (data) {
        ...
        onChange: function () {
          const rec = this.linksList.getSelectedRec()
          if (rec) {
            if (this.isOffline) this.api.asc_changeExternalReference(rec.get("externalRef"))
            else this.fireEvent("change:source", this, rec.get("externalRef"))
          }
        },

        updateButtons: function () {
          const selected = this.linksList.store.length > 0 && !!this.linksList.getSelectedRec()
        ...
        addTooltips: function (dataview, view, record) {
          ...
          this.checkTextOfItem(
            ...
            $(divs[0]),
            record.get("value"),
            this.linksList.options.headers[0].width,
          )
          this.checkTextOfItem(
            el,
            view,
            $(divs[1]),
            record.get("status"),
            this.linksList.options.headers[1].width,
          )
