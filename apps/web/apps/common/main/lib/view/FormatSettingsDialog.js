...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  Common.Views.FormatSettingsDialog = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 284,
        ...
        initialize: function (options) {
          ...
          const height = this.linked ? 275 : 265
          _.extend(
            this.options,
            {
              ...
              contentTemplate: _.template(
                [
                  ...
                  `<label class="format-sample margin-right-4" style="vertical-align: middle;">${this.txtSample}</label>`,
        ...
        render: function () {
          ...
          this.listType = new Common.UI.ListView({
            ...
            itemTemplate: _.template(
              '<div id="<%= id %>" class="list-item" style="pointer-events:none;overflow: hidden; text-overflow: ellipsis;"><%= Common.Utils.String.htmlEncode(displayValue) %></div>',
            ),
          })
          ...
          this.codesList = new Common.UI.ListView({
            ...
            tabindex: 1,
            itemTemplate: _.template(
              '<div id="<%= id %>" class="list-item" style="pointer-events:none;overflow: hidden; text-overflow: ellipsis;"><%= Common.Utils.String.htmlEncode(value) %></div>',
            ),
          })
          ...
          this.inputCustomFormat = new Common.UI.InputField({
            ...
            this.Format = this.api.asc_convertNumFormatLocal2NumFormat(value)
            this.lblExample.text(this.api.asc_getLocaleExample(this.Format))
            this.chLinked.setValue(false, true)
            if (!this._state.warning) {
              input.showWarning([this.txtCustomWarning])
              this._state.warning = true
            }
          })
          ...
          this.chLinked = new Common.UI.CheckBox({
            ...
            this.props.linked = field.getValue() === "checked"
            if (this.props.linked) {
              this.props.chartFormat.putSourceLinked(true)
              this.props.format = this.props.chartFormat.getFormatCode()
              this.props.formatInfo = this.props.chartFormat.getFormatCellsInfo()
        ...
        _setDefaults: function (props) {
          if (props?.formatInfo) {
            if (props.langId) this.langId = props.langId

            const type = props.formatInfo.asc_getType()
            let item = this.cmbLang.store.findWhere({
              value:
                type === Asc.c_oAscNumFormatType.Date || type === Asc.c_oAscNumFormatType.Time
                  ? props.formatInfo.asc_getSymbol() || this.langId
                  : this.langId,
            })
            ...
            this.onFormatSelect(
              this.cmbFormat,
              this.cmbFormat.getSelectedRecord(),
              null,
              props.formatInfo,
            )
            ...
            if (this._state.hasSymbols)
              this.cmbSymbols.setValue(
                props.formatInfo.asc_getSymbol() || props.formatInfo.asc_getCurrencySymbol(),
              )

            if (props.format) {
              if (this._state.hasNegative) {
                const selectedItem = this.cmbNegative.store.findWhere({ value: props.format })
                if (selectedItem) this.cmbNegative.selectRecord(selectedItem)
                else this.cmbNegative.setValue(this.api.asc_getLocaleExample(props.format))
              } else if (this._state.hasType) {
                let selectedItem = this.listType.store.findWhere({ value: props.format })
                if (selectedItem) {
                  ...
                  const defaultNumber = type === Asc.c_oAscNumFormatType.Time ? 1.534 : 38822
        ...
        getSettings: function () {
          return {
            ...
            linked: this.chLinked.getValue() === "checked",
          }
        },
        ...
        onSymbolsSelect: function (combo, record) {
          ...
          typeof record.value === "string"
            ? info.asc_setCurrencySymbol(record.value)
            : info.asc_setSymbol(record.value)

          const format = this.api.asc_getFormatCells(info)
          const data = []
          format.forEach((item) => {
            data.push({
              value: item,
              displayValue: this.api.asc_getLocaleExample(item, -1234.1234567890123456789),
            })
          })
          this.cmbNegative.setData(data)
          this.cmbNegative.selectRecord(this.cmbNegative.store.at(0))
          this.cmbNegative.cmpEl
            .find("li:nth-child(2) a, li:nth-child(4) a")
            .css({ color: "#ff0000" })
        ...
        onDecimalChange: function (field, newValue, oldValue, eOpts) {
          ...
          info.asc_setSeparator(
            this.FormatType === Asc.c_oAscNumFormatType.Number
              ? this.chSeparator.getValue() === "checked"
              : false,
          )
          if (
            this.FormatType === Asc.c_oAscNumFormatType.Currency ||
            this.FormatType === Asc.c_oAscNumFormatType.Accounting
          ) {
            const value = this.cmbSymbols.getValue()
            typeof value === "string"
              ? info.asc_setCurrencySymbol(value)
              : info.asc_setSymbol(value)
          } else info.asc_setSymbol(false)
        ...
        onFormatSelect: function (combo, record, e, initFormatInfo) {
          ...
          const isDateTime =
            record.value === Asc.c_oAscNumFormatType.Date ||
            record.value === Asc.c_oAscNumFormatType.Time
          const hasDecimal =
            record.value === Asc.c_oAscNumFormatType.Number ||
            record.value === Asc.c_oAscNumFormatType.Scientific ||
            record.value === Asc.c_oAscNumFormatType.Accounting ||
            record.value === Asc.c_oAscNumFormatType.Currency ||
            record.value === Asc.c_oAscNumFormatType.Percent
          const hasNegative =
            record.value === Asc.c_oAscNumFormatType.Number ||
            record.value === Asc.c_oAscNumFormatType.Currency ||
            record.value === Asc.c_oAscNumFormatType.Accounting
          const hasSeparator = record.value === Asc.c_oAscNumFormatType.Number
          const hasType = isDateTime || record.value === Asc.c_oAscNumFormatType.Fraction
          const hasSymbols =
            record.value === Asc.c_oAscNumFormatType.Accounting ||
            record.value === Asc.c_oAscNumFormatType.Currency
          const hasCode = record.value === Asc.c_oAscNumFormatType.Custom
          const valDecimal = initFormatInfo
            ? initFormatInfo.asc_getDecimalPlaces()
            : this.spnDecimal.getNumberValue()
          const valSeparator = initFormatInfo
            ? initFormatInfo.asc_getSeparator()
            : this.chSeparator.getValue() === "checked"
          const valSymbol = initFormatInfo
            ? initFormatInfo.asc_getSymbol() || initFormatInfo.asc_getCurrencySymbol()
            : this.langId
          ...
          if (record.value !== Asc.c_oAscNumFormatType.Custom) {
            ...
            if (hasNegative || isDateTime) {
              if (hasSymbols) {
                if (!this.CurrencySymbolsData) {
                  this.CurrencySymbolsData = []
                  let symbolssarr = this.api.asc_getCurrencySymbols()
                  for (const code in symbolssarr) {
                    if (symbolssarr.hasOwnProperty(code)) {
                      this.CurrencySymbolsData.push({
                        value: Number.parseInt(code),
                        displayValue: `${symbolssarr[code]} ${Common.util.LanguageInfo.getLocalLanguageName(code)[1]}`,
                      })
                    }
                  }
                  this.CurrencySymbolsData.sort((a, b) => {
                    if (a.displayValue < b.displayValue) return -1
                    if (a.displayValue > b.displayValue) return 1
                    return 0
                  })
                  this.CurrencySymbolsData.unshift({ value: null, displayValue: this.txtNone })
                  symbolssarr = this.api.asc_getAdditionalCurrencySymbols()
                  symbolssarr.forEach((item) => {
                    this.CurrencySymbolsData.push({ value: item, displayValue: item })
                  })
              ...
              const formatsarr = this.api.asc_getFormatCells(info)
              const data = []
              const exampleVal =
                record.value === Asc.c_oAscNumFormatType.Date
                  ? 38822
                  : record.value === Asc.c_oAscNumFormatType.Time
                    ? 1.534
                    : Number.parseFloat("-1234.12345678901234567890")
              formatsarr.forEach((item) => {
                data.push({
                  value: item,
                  displayValue: this.api.asc_getLocaleExample(item, exampleVal),
                })
              })
              if (hasNegative) {
                this.cmbNegative.setData(data)
            ...
            const formatsarr = this.api.asc_getFormatCells(info)
            const data = []
            let isCustom = !!this.CustomFormat
            formatsarr.forEach((item) => {
              const rec = new Common.UI.DataViewModel()
              rec.set({
                value: this.api.asc_convertNumFormat2NumFormatLocal(item),
                format: item,
              })
              data.push(rec)
              if (this.CustomFormat === item) isCustom = false
            })
            if (isCustom) {
              const rec = new Common.UI.DataViewModel()
              rec.set({
                value: this.api.asc_convertNumFormat2NumFormatLocal(this.CustomFormat),
                format: this.CustomFormat,
              })
              data.push(rec)
            }
            this.codesList.store.reset(data, { silent: false })
            const rec = this.codesList.store.findWhere({ value: this.Format })
            rec && this.codesList.selectRecord(rec)
            rec && this.codesList.scrollToRecord(rec)
            this.inputCustomFormat.setValue(
              this.api.asc_convertNumFormat2NumFormatLocal(this.Format),
            )
          }
        ...
        onSelectLang: function (lang) {
          ...
          const data = []
          const exampleVal =
            this.FormatType === Asc.c_oAscNumFormatType.Date
              ? 38822
              : this.FormatType === Asc.c_oAscNumFormatType.Time
                ? 1.534
                : Number.parseFloat("-1234.12345678901234567890")
          formatsarr.forEach((item) => {
            data.push({
        ...
        txtCustomWarning:
          "Please enter the custom number format carefully. Spreadsheet Editor does not check custom formats for errors that may affect the xlsx file.",
        textLocale: "Locale",
      },
