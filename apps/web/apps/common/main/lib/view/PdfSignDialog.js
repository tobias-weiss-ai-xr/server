...
define([], () => {
  Common.Views.PdfSignDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 535,
        ...
        initialize: function (options) {
          ...
          this.template = [
            ...
            '<div id="pdf-sign-bold" class="img-type hidden margin-left-5"></div>',
            '<div id="pdf-sign-italic" class="img-type hidden margin-left-5" ></div>',
        ...
        render: function () {
          ...
          Common.UI.GroupedButtons([this.btnUpload, this.btnDraw, this.btnType])
          ...
          this.chRemBack.on("change", (field, newValue, oldValue, eOpts) => {
            this.props?.put_RemoveBackground(field.getValue() === "checked")
          })
          ...
          this.cmbFonts = new Common.UI.ComboBoxFonts({
            ...
            this.props?.put_TypeFont(record.name)
          })
          ...
          this.cmbFontSize = new Common.UI.ComboBox({
            ...
            this.props?.put_TypeFontSize(record.value)
          })
          ...
          this.btnBold.on("click", (btn, e) => {
            ...
            this.props?.put_TypeBold(btn.pressed)
          })
          ...
          this.btnItalic.on("click", (btn, e) => {
            ...
            this.props?.put_TypeItalic(btn.pressed)
          })
          ...
          this.btnLineColor.setMenu(
            new Common.UI.Menu({
              style: "min-width: 100px;",
              items: [
                {
                  template: _.template(
                    '<div id="id-sign-window-draw-line-color" style="width: 174px; display: inline-block;" class="palette-large"></div>',
                  ),
                ...
                },
                {
                  ...
                  template: _.template(
                    `<a tabindex="-1" type="menuitem" style="">${this.btnLineColor.textNewColor}</a>`,
                  ),
                },
              ],
            }),
          ...
          this.btnLineColor.menu.cmpEl.find("#sign-draw-line-color-new").on("click", () => {
            picker.addNewColor(this.btnLineColor.currentColor)
          })
          ...
          this.cmbLineSize.on("selected", (combo, record) => {
            this.props?.put_LineSize(record.value)
          })
          ...
          this.btnUndo = new Common.UI.Button({
            ...
            cls: "btn-toolbar",
            iconCls: is_svg_icon
              ? "svg-icon undo icon-rtl scaling-off"
              : "toolbar__icon btn-undo icon-rtl",
            hint: this.tipUndo,
          ...
          this.btnRedo = new Common.UI.Button({
            ...
            cls: "btn-toolbar",
            iconCls: is_svg_icon
              ? "svg-icon redo icon-rtl scaling-off"
              : "toolbar__icon btn-redo icon-rtl",
            hint: this.tipRedo,
        ...
        afterRender: function () {
          ...
          const onApiImgLoaded = () => {
            this.isImageLoaded = true
            this.uploadEmptyPnl.toggleClass("hidden", true)
            this.uploadPreviewPnl.toggleClass("hidden", false)
            this.props.updateView(0)
            this.btnOk.setDisabled(false)
          }
          this.api.asc_registerCallback("asc_onSignatureImageLoaded", onApiImgLoaded)

          const onCanUndoChanged = (canUndo) => {
            this.btnUndo.setDisabled(!canUndo)
          }
          const onCanRedoChanged = (canRedo) => {
            this.btnRedo.setDisabled(!canRedo)
          }
          ...
          const insertImageFromStorage = (data) => {
            if (data?._urls && data.c === "signature") {
          ...
          this.keydownHandler = (e) => {
            if (!this.$window?.is(":visible")) return
            ...
            if (e.keyCode === 90 && !e.shiftKey) {
              if (!this.btnUndo.isDisabled()) {
                this.onUndo()
                e.preventDefault()
              }
            } else if (e.keyCode === 89 || (e.keyCode === 90 && e.shiftKey)) {
              if (!this.btnRedo.isDisabled()) {
                this.onRedo()
                e.preventDefault()
              }
            }
          ...
          this.on("close", (obj) => {
            this.api.asc_unregisterCallback("asc_onSignatureImageLoaded", onApiImgLoaded)
            this.api.asc_unregisterCallback("asc_CanUndoSignature", onCanUndoChanged)
            this.api.asc_unregisterCallback("asc_CanRedoSignature", onCanRedoChanged)
        ...
        ShowHideElem: function (mode) {
          ...
          _.delay(() => {
            this.props.updateView(mode)
            mode === 1
              ? this.cmbLineSize.focus()
              : mode === 2
        ...
        onColorsLineSelect: function (btn, color) {
          ...
          this.props?.put_LineColor(Common.Utils.ThemeColor.getRgbColor(color))
        },
        ...
        onFontSizeChanged: function (before, combo, record, e) {
          ...
          if (before) {
            ...
            value = value > maxvalue ? maxvalue : value < 1 ? 1 : Math.floor((value + 0.4) * 2) / 2

            combo.setRawValue(value)
            this.font.size = value
            this.props?.put_TypeFontSize(value)
          }
        },
        ...
        txtUploadDesc:
          "You can upload images in JPEG, JPG, GIF and PNG formats with a max size of 30 Mb",
        textBefore:
