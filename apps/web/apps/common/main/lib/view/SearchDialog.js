...
define([], () => {
  Common.UI.SearchDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 550,
        ...
        render: function () {
          ...
          else if (this.options.markresult.applied) this.miHighlight.setChecked(true, true)
          ...
          this.$window
            .find(".btn[result=replace]")
            .on("click", _.bind(this.onBtnClick, this, "replace"))
          this.$window
            .find(".btn[result=replaceall]")
            .on("click", _.bind(this.onBtnClick, this, "replaceall"))
          this.$window
            .find("label[result=replaceshow]")
            .on("click", _.bind(this.onShowReplace, this))
        ...
        focus: function (type) {
          const field = type === "replace" ? this.txtReplace : this.txtSearch
          setTimeout(() => {
            field.focus()
            field.select()
          }, 10)
        ...
        onKeyPress: function (event) {
          if (!this.isLocked()) {
            if (event.keyCode === Common.UI.Keys.RETURN) {
              if (event.data === "search") this.onBtnClick("next", event)
              else if (event.data === "replace" && this.mode === "replace") {
                this.onBtnClick("replace", event)
              }

              event.preventDefault()
              event.stopPropagation()
            } else if (event.keyCode === Common.UI.Keys.ESC && $(".asc-loadmask").length < 1) {
              this.hide()
              event.preventDefault()
              event.stopPropagation()
            }
        ...
        setMode: function (m) {
          this.mode = m
          const $inputs = this.$window.find(".input-row")
          ...
          if (m === "no-replace") {
            ...
            this.menuLookin?.menu.items[1].setDisabled(false)
        ...
        setSearchText: function (value) {
          this.txtSearch?.val(value)
        },

        onShowReplace: function (e) {
          this.setMode(this.mode === "replace" ? "search" : "replace")
          _.defer(() => {
            this.mode === "replace" ? this.txtReplace.focus() : this.txtSearch.focus()
          }, 300)
        },
