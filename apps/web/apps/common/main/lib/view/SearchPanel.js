...
define([
  ...
  Common.Views.SearchPanel = Common.UI.BaseView.extend(
    _.extend(
      {
        ...
        initialize: function (options) {
          ...
          window.SSE &&
            (this.extendedOptions = Common.localStorage.getBool(
              "sse-search-options-extended",
              true,
            ))
        },
        ...
        render: function (el) {
          if (!this.rendered) {
            ...
            this.chCaseSensitive = new Common.UI.CheckBox({
              ...
              this.fireEvent("search:options", ["case-sensitive", field.getValue() === "checked"])
            })
            ...
            this.chMatchWord = new Common.UI.CheckBox({
              ...
              this.fireEvent("search:options", ["match-word", field.getValue() === "checked"])
            })
            ...
            Common.NotificationCenter.on(
              ...
              _.bind(this.disableNavButtons, this),
            )
            ...
            if (window.SSE) {
              this.cmbWithin = new Common.UI.ComboBox({
                ...
                this.fireEvent("search:options", ["within", record.value])
              })
              ...
              this.cmbSearch = new Common.UI.ComboBox({
                ...
                this.fireEvent("search:options", ["search", !record.value])
              })
              ...
              this.cmbLookIn = new Common.UI.ComboBox({
                ...
                this.fireEvent("search:options", ["lookIn", !record.value])
              })
        ...
        focus: function (type) {
          const el =
            type === "replace"
              ? this.inputReplace.$el
              : type === "range"
                ? this.inputSelectRange.$el
                : this.inputText.$el
          setTimeout(() => {
            el.find("input").focus()
        ...
        setSearchMode: function (mode) {
          if (this.mode !== mode) {
            this.$el
              .find(".edit-setting")
              [mode !== "no-replace" && mode !== "redact" ? "show" : "hide"]()
            this.$el
              .find(".redact-setting")
              [mode === "redact" && this.options.mode.isPDFEdit ? "show" : "hide"]()
            this.$el
              .find(".redact-no-replace-btn")
              [mode === "no-replace" && this.options.mode.isPDFEdit === true ? "show" : "hide"]()
            this.$el
              .find("#search-adv-title")
              .text(
                mode === "no-replace"
                  ? this.textFind
                  : mode === "redact"
                    ? this.textFindAndRedact
                    : this.textFindAndReplace,
              )
            this.mode = mode
          }
        },
        ...
        updateResultsContainerHeight: function () {
          if (this.$resultsContainer) {
            this.$resultsContainer.outerHeight(
              Math.max(
                $("#search-box").outerHeight() -
                  $("#search-header").outerHeight() -
                  $("#search-adv-settings").outerHeight(),
                112,
              ),
            )
        ...
        updateResultsNumber: function (current, count) {
          ...
          text =
            current === "no-results"
              ...
              : current === "stop"
                ? this.textSearchHasStopped
                : current === "content-changed"
                  ? `${this.textContentChanged} ${Common.Utils.String.format(this.textSearchAgain, '<a class="search-again">', "</a>")}`
                  : current === "replace-all"
                    ? Common.Utils.String.format(this.textItemsSuccessfullyReplaced, count)
                    : current === "replace"
                      ? Common.Utils.String.format(
                          this.textPartOfItemsNotReplaced,
                          count[0],
                          count[1],
                          count[2],
                        )
                      : !count
                        ? this.textNoMatches
                        : Common.Utils.String.format(this.textSearchResults, current + 1, count)
          if (current === "content-changed") {
            this.$reaultsNumber.html(text)
            this.$reaultsNumber.find(".search-again").on("click", () => {
              this.fireEvent("search:next", [this.inputText.getValue(), true])
            })
          } else {
        ...
        showToManyResults: function () {
          ...
          this.$resultsContainer.append(
            `<div class="many-results">${this.textTooManyResults}</div>`,
          )
        ...
        onReplaceClick: function (action) {
          this.fireEvent(`search:${action}`, [
            this.inputText.getValue(),
            this.inputReplace.getValue(),
          ])
        },
        ...
        onModeChanged: function (config) {
          ...
          this.$el
            .find(".redact-no-replace-btn")
            [this.mode === "no-replace" && isEdit ? "show" : "hide"]()
        },
        ...
        disableNavButtons: function (resultNumber, allResults) {
          const disable = (this.inputText._input.val() === "" && !window.SSE) || !allResults
        ...
        textPartOfItemsNotReplaced:
          "{0}/{1} items replaced. Remaining {2} items are locked by other users.",
