...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  Common.Views.ShortcutsDialog = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 550,
        ...
        render: function () {
          ...
          Common.NotificationCenter.on(
            ...
            _.bind(function () {
              this._setDefaults()
              this.shortcutsEditDialog?.renderShortcutsWarning()
            }, this),
          )
          this._setDefaults()
        },
        ...
        _updateActionsList: function () {
          const selectedActionIndex = this.actionsList.store.findIndex((item) =>
            item.get("selected"),
          )
        ...
        _filterActionsList: function () {
          ...
          if (this._state.searchValue) {
            filteredData = actionsData.filter((item) =>
              (item.action.name || "").toLowerCase().includes(this._state.searchValue),
            )
          } else {
        ...
        onClickActionItem: function (list, item, record, event) {
          if (
            $(event.target).hasClass("action-edit") ||
            $(event.target).parent().hasClass("action-edit")
          ) {
        ...
        onEditActionItem: function (list, item, record) {
          ...
          this.shortcutsEditDialog.on("close", () => {
            this.shortcutsEditDialog = null
        ...
        onResetAll: function () {
          Common.UI.warning({
            ...
            callback: (btn) => {
              if (btn === "ok") {
                this._shortcutsController.resetAllShortcuts()
              }
            },
          })
        },
        ...
        onDlgBtnClick: function (event) {
          ...
          if (state === "ok") {
            this.handler?.call(this, state, state === "ok" ? null : undefined)
          }
