...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  Common.Views.ShortcutsEditDialog = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          height: "auto",
        ...
        getFocusedComponents: function () {
          ...
          this.shortcutsCollection.each((record) => {
        ...
        getDefaultFocusableComponent: function () {
          const firstFocusableItem = this.shortcutsCollection.find((item) => item.get("removeBtn"))
        ...
        _setDefaults: function () {
          ...
          this.shortcutsCollection.on(
            "reset",
            function (newCollection, details) {
          ...
          this.shortcutsCollection.on(
            "add",
            function (record, newCollection) {
              const prevCollection = newCollection.filter((item) => item !== record)
          ...
          this.shortcutsCollection.on(
            "add remove reset change:keys",
            this.renderShortcutsWarning,
            this,
          )

          //Get shortcuts for the current action and copy the instances so as not to modify the original instances
          let shortcuts = _.filter(
            this._getOriginalShortcuts(),
            (shortcut) => !shortcut.ascShortcut.asc_IsHidden(),
          )
        ...
        _isDefaultShortcut: (ascShortcut) => {
          const shortcutIndex = ascShortcut.asc_GetShortcutIndex()
          return _.some(
            Asc.c_oAscDefaultShortcuts[ascShortcut.asc_GetType()],
            (someAscShortcut) => shortcutIndex === someAscShortcut.asc_GetShortcutIndex(),
          )
        },
        ...
        _getDefaultShortcuts: function () {
          ...
          return ascShortcuts.map((ascShortcut) => {
            const copyAscShortcut = new Asc.CAscShortcut()
            copyAscShortcut.asc_FromJson(ascShortcut.asc_ToJson())
            copyAscShortcut.asc_SetIsHidden(false)
            return {
              keys: this._shortcutsController._getAscShortcutKeys(copyAscShortcut),
              ascShortcut: copyAscShortcut,
            }
          })
        },
        ...
        _renderShortcutsList: function (prevCollection) {
          ...
          this.shortcutsCollection.each((item, index) => {
            ...
            // }
            this.$window.find("#shortcuts-list").append($item)

            const keysInput = new Common.UI.InputField({
              el: $item.find(".keys-input"),
              value: keysStr,
              editable: false,
              placeHolder: this.txtInputPlaceholder,
              disabled: isLocked,
            })
            ...
            if (isLocked) {
              $item.find(".lock-info-icon").tooltip({
                title: this.txtCantBeEdited,
                placement: "cursor",
                zIndex: Number.parseInt(this.$window.css("z-index")) + 10,
              })
            } else {
            ...
            $keysInput.on("keydown", (e) => {
              const alowedSingleKeys = [
                ...
                Common.UI.Keys.F11,
                Common.UI.Keys.F12,
                Common.UI.Keys.INSERT,
                Common.UI.Keys.HOME,
                Common.UI.Keys.PAGEUP,
                Common.UI.Keys.DELETE,
                Common.UI.Keys.END,
                Common.UI.Keys.PAGEDOWN,
                Common.UI.Keys.LEFT,
                Common.UI.Keys.UP,
                Common.UI.Keys.RIGHT,
                Common.UI.Keys.DOWN,
              ]
              ...
              if (forbiddensKeys.includes(e.keyCode)) {
                ...
                if (e.keyCode === Common.UI.Keys.TAB && this._prevKeysForActiveInput.length) {
                  item.set("keys", this._prevKeysForActiveInput)
                  $item.find("input").val(this._prevKeysForActiveInput.join(" + "))
                }
                return
              }
              ...
              if (e.metaKey && Common.Utils.isMac)
                keys.push(this._shortcutsController.keyCodeToKeyName(Common.UI.Keys.META))
              if (e.ctrlKey)
                keys.push(this._shortcutsController.keyCodeToKeyName(Common.UI.Keys.CTRL))
              if (e.altKey)
                keys.push(this._shortcutsController.keyCodeToKeyName(Common.UI.Keys.ALT))
              if (e.shiftKey)
                keys.push(this._shortcutsController.keyCodeToKeyName(Common.UI.Keys.SHIFT))

              if (
                ![
                  Common.UI.Keys.CTRL,
                  Common.UI.Keys.SHIFT,
                  Common.UI.Keys.ALT,
                  Common.UI.Keys.META,
                ].includes(e.keyCode)
              ) {
                keys.push(this._shortcutsController.keyCodeToKeyName(e.keyCode))
            ...
            const removeKeysIfOnlyModifiers = (removedKeys) => {
              const modifierKeys = [
                this._shortcutsController.keyCodeToKeyName(Common.UI.Keys.CTRL),
                this._shortcutsController.keyCodeToKeyName(Common.UI.Keys.SHIFT),
                this._shortcutsController.keyCodeToKeyName(Common.UI.Keys.ALT),
                this._shortcutsController.keyCodeToKeyName(Common.UI.Keys.META),
              ]
              const keys = item.get("keys")
              const hasExtra = _.some(keys, (k) => !_.contains(modifierKeys, k))

              if (!hasExtra && removedKeys && removedKeys.length) {
                const filteredKeys = keys.filter((k) => !_.contains(removedKeys, k))
            ...
            $keysInput.on("keyup", (e) => {
              const modifierKeyMap = {
                [Common.UI.Keys.CTRL]: this._shortcutsController.keyCodeToKeyName(
                  Common.UI.Keys.CTRL,
                ),
                [Common.UI.Keys.ALT]: this._shortcutsController.keyCodeToKeyName(
                  Common.UI.Keys.ALT,
                ),
                [Common.UI.Keys.SHIFT]: this._shortcutsController.keyCodeToKeyName(
                  Common.UI.Keys.SHIFT,
                ),
                [Common.UI.Keys.META]: this._shortcutsController.keyCodeToKeyName(
                  Common.UI.Keys.META,
                ),
              }
              ...
              if (!modifierKey) {
                this._prevKeysForActiveInput = item.get("keys")
              }
            })

            $keysInput.on("focusin", () => {
              this._prevKeysForActiveInput = item.get("keys")
            })

            $keysInput.on("focusout", () => {
              this._prevKeysForActiveInput = []
              removeKeysIfOnlyModifiers(item.get("keys"))
            })

            $item.find(".remove-btn").on("click", () => {
              const removedIndex = this.shortcutsCollection.findIndex((record) => record === item)
              this.shortcutsCollection.remove(item)
              if (this.shortcutsCollection.length === 0) {
                this.onAddShortcut()
              }

              const newFocusableIndex =
                removedIndex < this.shortcutsCollection.length ? removedIndex : removedIndex - 1
              if (this.shortcutsCollection.at(newFocusableIndex).get("removeBtn")) {
                this.shortcutsCollection.at(newFocusableIndex).get("removeBtn").focus()
              } else {
                this.getFooterButtons()[0].focus()
              }
            })
          })
          this.fixHeight(true)
        ...
        renderShortcutsWarning: function () {
          ...
          this.shortcutsCollection.each((item) => {
            const ascShortcut = item.get("ascShortcut")
            const assignedActionNames = []
            const assignedItem = this._findAssignedActions(ascShortcut, {
              actionType: this.options.action.type,
              shortcuts: this.shortcutsCollection
                .toJSON()
                .slice(0, _.indexOf(this.shortcutsCollection.models, item)),
            })
            const isDefaultShortcut = this._isDefaultShortcut(ascShortcut)
            const isDisabled =
              !isDefaultShortcut &&
              _.some(
                assignedItem,
                (item) => item.action.isLocked || item.shortcut.ascShortcut.asc_IsLocked(),
              )
            ...
            item
              .get("keysInput")
              .showWarning(
                txtKey ? [this[txtKey].replace("%1", assignedActionNames.join(", "))] : null,
              )
          })
        ...
        onAddShortcut: function () {
          const lastShortcutRecord = this.shortcutsCollection.at(
            this.shortcutsCollection.length - 1,
          )
        ...
        onReset: function () {
          Common.UI.warning({
            ...
            msg: `${this.txtRestoreDescription.replace("%1", this.options.action.name)}<br/>${this.txtRestoreContinue}`,
            buttons: ["ok", "cancel"],
            width: 400,
            callback: (btn) => {
              if (btn === "ok") {
        ...
        onDlgBtnClick: function (event) {
          ...
          if (state === "ok") {
            ...
            ;(this.shortcutsCollection.toJSON() || []).forEach((item) => {
              if (!item.keys.length) return

              const idx = item.ascShortcut.asc_GetShortcutIndex()
              if (seen[idx]) return
            ...
            this._shortcutsController.updateShortcutsForAction(
              this.options.action.type,
              filteredShortcuts,
            )
          }
