...
define([], () => { 
    ...
    Common.Views.SignSettingsDialog = Common.UI.Window.extend(_.extend({
        ...
        render: function() {
            ...
            this.inputEmail = new Common.UI.InputField({
                el          : $('#id-dlg-sign-settings-email'),
                style       : 'width: 100%;',
                disabled    : this.type==='view'
            });

            this.textareaInstructions = new Common.UI.TextareaField({
        ...
        setSettings: function (props) {
            if (props) {
                ...
                let value = props.asc_getSigner1();
                this.inputName.setValue(value ? value : '');
                value = props.asc_getSigner2();
                this.inputTitle.setValue(value ? value : '');
                value = props.asc_getEmail();
                this.inputEmail.setValue(value ? value : '');
                value = props.asc_getInstructions();
                this.textareaInstructions.setValue(value ? value : '');
                this.chDate.setValue(props.asc_getShowDate());

                this._currentGuid = props.asc_getGuid();
            }
        },

        getSettings: function () {
            const props = new AscCommon.asc_CSignatureLine();

            props.asc_setSigner1(this.inputName.getValue());
            props.asc_setSigner2(this.inputTitle.getValue());
            props.asc_setEmail(this.inputEmail.getValue());
            props.asc_setInstructions(this.textareaInstructions.getValue());
            props.asc_setShowDate(this.chDate.getValue()==='checked');
            (this._currentGuid!==undefined) && props.asc_setGuid(this._currentGuid);
