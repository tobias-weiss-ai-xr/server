...
;((root, factory) => {
  ...
  return (lunr) => {
    ...
    lunr.hi = function () {
      this.pipeline.reset()
      this.pipeline.add(lunr.hi.trimmer, lunr.hi.stopWordFilter, lunr.hi.stemmer)
    ...
    lunr.hi.stemmer = (() => (word) => {
      // for lunr version 2
      if (typeof word.update === "function") {
        return word.update((word) => word)
      }
      return word
    })()

    const segmenter = lunr.wordcut
    segmenter.init()
    lunr.hi.tokenizer = (obj) => {
      if (!arguments.length || obj == null || obj === undefined) return []
      if (Array.isArray(obj))
        return obj.map((t) => (isLunr2 ? new lunr.Token(t.toLowerCase()) : t.toLowerCase()))

      const str = obj.toString().toLowerCase().replace(/^\s+/, "")
