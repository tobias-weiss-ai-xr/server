...
;(function () {
  ...
  lunr.tokenizer = (obj, metadata) => {
    ...
    if (Array.isArray(obj)) {
      return obj.map(
        (t) => new lunr.Token(lunr.utils.asString(t).toLowerCase(), lunr.utils.clone(metadata)),
      )
    }
  ...
  lunr.generateStopWordFilter = (stopWords) => {
    const words = stopWords.reduce((memo, stopWord) => {
   ...
   * @see lunr.Pipeline
   */
  lunr.trimmer = (token) => token.update((s) => s.replace(/^\W+/, "").replace(/\W+$/, ""))
  ...
  lunr.TokenSet.fromFuzzyString = (str, editDistance) => {
    ...
    while (stack.length) {
      ...
      if ("*" in frame.node.edges) {
        const insertionNode = frame.node.edges["*"]
      } else {
        const insertionNode = new lunr.TokenSet()
      ...
      if (frame.str.length >= 1) {
        if ("*" in frame.node.edges) {
          const substitutionNode = frame.node.edges["*"]
        } else {
          const substitutionNode = new lunr.TokenSet()
      ...
      if (frame.str.length > 1) {
        ...
        const charB = frame.str.charAt(1)
        let transposeNode
  ...
  lunr.TokenSet.prototype.intersect = function (b) {
    ...
    while (stack.length) {
      ...
      const nEdges = Object.keys(frame.node.edges)
      const nLen = nEdges.length
      ...
      for (let q = 0; q < qLen; q++) {
        ...
        for (let n = 0; n < nLen; n++) {
          ...
          if (nEdge === qEdge || qEdge === "*") {
            ...
            const qNode = frame.qNode.edges[qEdge]
            const final = node.final && qNode.final
            let next = undefined
  ...
  lunr.Index.prototype.query = function (fn) {
    ...
    const matchingFields = Object.create(null)
    const queryVectors = Object.create(null)
    const termFieldCache = Object.create(null)
    const requiredMatches = Object.create(null)
    const prohibitedMatches = Object.create(null)
    ...
    for (let i = 0; i < query.clauses.length; i++) {
      ...
      let terms = null
      let clauseMatches = lunr.Set.empty
      ...
      for (let m = 0; m < terms.length; m++) {
        ...
        const expandedTerms = this.tokenSet.intersect(termTokenSet).toArray()
        ...
        for (let j = 0; j < expandedTerms.length; j++) {
          ...
          const termIndex = posting._index
          ...
          for (let k = 0; k < clause.fields.length; k++) {
            ...
            const matchingDocumentRefs = Object.keys(fieldPosting)
            const termField = `${expandedTerm}/${field}`
            const matchingDocumentsSet = new lunr.Set(matchingDocumentRefs)
            ...
            for (let l = 0; l < matchingDocumentRefs.length; l++) {
               ...
               * results
               */
              const matchingDocumentRef = matchingDocumentRefs[l]
              const matchingFieldRef = new lunr.FieldRef(matchingDocumentRef, field)
              const metadata = fieldPosting[matchingDocumentRef]
              let fieldMatch
    ...
    let allProhibitedMatches = lunr.Set.empty
    ...
    if (query.isNegated()) {
      ...
      for (let i = 0; i < matchingFieldRefs.length; i++) {
        ...
        const fieldRef = lunr.FieldRef.fromString(matchingFieldRef)
    ...
    for (let i = 0; i < matchingFieldRefs.length; i++) {
       ...
       */
      const fieldRef = lunr.FieldRef.fromString(matchingFieldRefs[i])
      ...
      const score = queryVectors[fieldRef.fieldName].similarity(fieldVector)
      let docMatch
  ...
  lunr.Index.prototype.toJSON = function () {
    ...
    const fieldVectors = Object.keys(this.fieldVectors).map(function (ref) {
  ...
  lunr.Index.load = (serializedIndex) => {
    const attrs = {}
    const fieldVectors = {}
    const serializedVectors = serializedIndex.fieldVectors
    const invertedIndex = Object.create(null)
    const serializedInvertedIndex = serializedIndex.invertedIndex
    const tokenSetBuilder = new lunr.TokenSet.Builder()
    const pipeline = lunr.Pipeline.load(serializedIndex.pipeline)
    ...
    for (let i = 0; i < serializedVectors.length; i++) {
      const tuple = serializedVectors[i]
      const ref = tuple[0]
    ...
    for (let i = 0; i < serializedInvertedIndex.length; i++) {
      const tuple = serializedInvertedIndex[i]
      const term = tuple[0]
      const posting = tuple[1]
  ...
  lunr.Builder.prototype.add = function (doc, attributes) {
    ...
    for (let i = 0; i < fields.length; i++) {
      ...
      const field = extractor ? extractor(doc) : doc[fieldName]
  ...
  lunr.Builder.prototype.calculateAverageFieldLengths = function () {
    ...
    const numberOfFields = fieldRefs.length
    const accumulator = {}
    const documentsWithField = {}
  ...
  lunr.Builder.prototype.createFieldVectors = function () {
    ...
    const fieldRefsLength = fieldRefs.length
    const termIdfCache = Object.create(null)

    for (let i = 0; i < fieldRefsLength; i++) {
      const fieldRef = lunr.FieldRef.fromString(fieldRefs[i])
      const fieldName = fieldRef.fieldName
      const fieldLength = this.fieldLengths[fieldRef]
      const fieldVector = new lunr.Vector()
      const termFrequencies = this.fieldTermFrequencies[fieldRef]
      const terms = Object.keys(termFrequencies)
      const termsLength = terms.length

      const fieldBoost = this._fields[fieldName].boost || 1
      const docBoost = this._documents[fieldRef.docRef].boost || 1

      for (let j = 0; j < termsLength; j++) {
        const term = terms[j]
        const tf = termFrequencies[term]
        const termIndex = this.invertedIndex[term]._index
        let idf
        let score
        let scoreWithPrecision
  ...
  lunr.QueryParser.parsePresence = (parser) => {
    ...
    switch (lexeme.str) {
      ...
      case "+":
        ...
        break
      default: {
        const errorMessage = `unrecognised presence operator'${lexeme.str}'`
        throw new lunr.QueryParseError(errorMessage, lexeme.start, lexeme.end)
      }
    }
    ...
    switch (nextLexeme.type) {
      ...
      case lunr.QueryLexer.TERM:
        return lunr.QueryParser.parseTerm
      default: {
        const errorMessage = `expecting term or field, found '${nextLexeme.type}'`
  ...
  lunr.QueryParser.parseField = (parser) => {
    ...
    if (parser.query.allFields.indexOf(lexeme.str) === -1) {
      const possibleFields = parser.query.allFields.map((f) => `'${f}'`).join(", ")
      const errorMessage = `unrecognised field '${lexeme.str}', possible fields: ${possibleFields}`
    ...
    switch (nextLexeme.type) {
      case lunr.QueryLexer.TERM:
        return lunr.QueryParser.parseTerm
      default: {
        const errorMessage = `expecting term, found '${nextLexeme.type}'`
        throw new lunr.QueryParseError(errorMessage, nextLexeme.start, nextLexeme.end)
      }
    }
  }
  ...
  lunr.QueryParser.parseEditDistance = (parser) => {
    ...
    const editDistance = Number.parseInt(lexeme.str, 10)

    if (Number.isNaN(editDistance)) {
  ...
  lunr.QueryParser.parseBoost = (parser) => {
    ...
    const boost = Number.parseInt(lexeme.str, 10)
