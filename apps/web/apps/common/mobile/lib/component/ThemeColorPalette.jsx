          ...
          return (
            <div key={`tc-row-${rowIndex}`} className="row">
              {row.map((effect, index) => {
                return (
                  <a
                    key={`tc-${rowIndex}-${index}`}
                    className={
                      curColor &&
                      !isTypeCustomColors &&
                      !isTypeColors &&
                      ((curColor.color === effect.color &&
                        curColor.effectValue === effect.effectValue) ||
                        curColor === effect.color)
                        ? "active"
                        : ""
                    }
...
const StandartColors = ({ options, standartColors, onColorClick, curColor }) => {
  return (
    <div className="palette">
      {standartColors?.length &&
        standartColors.map((color, index) => {
          return index === 0 && options.transparent ? (
            <a
              key={`sc-${index}`}
            ...
            <a
              key={`sc-${index}`}
              className={
                curColor && (curColor?.color === color?.color || curColor === color?.color)
                  ? " active"
                  : ""
              }
...
const CustomColors = ({ options, customColors, isTypeColors, onColorClick, curColor }) => {
  ...
  const indexCurColor = colors.indexOf(curColor)
        ...
        ? colors.map((color, index) => {
            return (
              <a
                key={`dc-${index}`}
                className={
                  curColor && curColor === color && index === indexCurColor && !isTypeColors
                    ? "active"
                    : ""
                }
...
const ThemeColorPalette = (props) => {
  ...
  const isTypeColors = standartColors?.length && standartColors.some((value) => value === curColor)
  // custom color
  ...
  const isTypeCustomColors = customColors.some((value) => value === curColor)
    ...
    <div className={`color-palettes${props.cls ? ` ${props.cls}` : ""}`}>
      <List>
        <ListItem className="theme-colors">
          ...
          <ThemeColors
            isTypeCustomColors={isTypeCustomColors}
            isTypeColors={isTypeColors}
            themeColors={themeColors}
            onColorClick={props.changeColor}
            curColor={curColor}
          />
        </ListItem>
...
const CustomColorPicker = (props) => {
  ...
  const addNewColor = (color) => {
    ...
    props.onAddNewColor?.(colors, newColor)
  }
