...
class ContextMenuController extends Component {
  ...
  onDocumentReady() {
    ...
    if (!this.$targetEl.length) {
      ...
      this.$targetEl = $$(
        `<div id="${idCntextMenuTargetElement.substr(1)}" style="position:absolute;"></div>`,
      )
  ...
  offsetPopoverTop(popover) {
    ...
    let safeAreaLeft = Number.parseInt($("html").css("--f7-safe-area-left"), 10)
    let safeAreaRight = Number.parseInt($("html").css("--f7-safe-area-right"), 10)
  ...
  onApiOpenContextMenu(x, y) {
    if (
      !this.state.opened &&
      $$(
        ".dialog.modal-in, .popover.modal-in, .sheet-modal.modal-in, .popup.modal-in, #pe-preview, .add-comment-popup, .actions-modal.modal-in",
      ).length < 1
    ) {
      ...
      if (this.state.items.length > 0) {
        ...
        if (Device.android) this.offsetPopoverTop(popover)
  ...
  render() {
    return (
      <div>
        ...
        />
        <ActionsWithExtraItems
          items={this.state.extraItems}
          onMenuItemClick={this.onMenuItemClick}
