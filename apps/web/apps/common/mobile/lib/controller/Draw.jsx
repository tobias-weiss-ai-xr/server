...
export const DrawController = inject("storeAppOptions")(
  observer(({ storeAppOptions }) => {
    ...
    const toolActions = React.useMemo(
      () => ({
        pen: (api, settings) =>
          api.asc_StartDrawInk(
            createStroke(settings.pen.color, settings.pen.lineSize, settings.pen.opacity),
          ),
        highlighter: (api, settings) =>
          api.asc_StartDrawInk(
            createStroke(
              settings.highlighter.color,
              settings.highlighter.lineSize,
              settings.highlighter.opacity,
            ),
          ),
    ...
    const removeBackdropSwipeClose = () => {
      document
        ...
        ?.removeEventListener("touchmove", closeBackdropSheet)
    }

    return storeAppOptions.isDrawMode ? (
      <DrawView
