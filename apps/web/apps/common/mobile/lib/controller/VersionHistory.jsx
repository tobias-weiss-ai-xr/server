...
const VersionHistoryController = inject(
  "storeAppOptions",
  "storeVersionHistory",
)(
  observer((props) => {
        ...
        f7.dialog
          .create({
            ...
            text: opts.data.error
              ? opts.data.error
              : t("Common.VersionHistory.txtErrorLoadHistory"),
            buttons: [
              {
          ...
          if (version) {
            ...
            if (!user) {
              const color = getUserColor(
                version.user.id || version.user.name || t("Common.VersionHistory.textAnonymous"),
                true,
              )
                ...
                for (i = changes.length - 2; i >= 0; i--) {
                  ...
                  if (!user) {
                    const color = getUserColor(
                      change.user.id ||
                        change.user.name ||
                        t("Common.VersionHistory.textAnonymous"),
                      true,
                    )
    ...
    const onSetHistoryData = (opts) => {
      ...
      if (opts.data.error) {
        ...
        if (data !== null) {
          ...
          const diff =
            !opts.data.previous || currentChangeId === undefined ? null : opts.data.changesUrl
          const url = !diff && opts.data.previous ? opts.data.previous.url : opts.data.url
          const fileType =
            !diff && opts.data.previous ? opts.data.previous.fileType : opts.data.fileType
          const docId = opts.data.key ? opts.data.key : currentDocId
          const docIdPrev = opts.data.previous?.key ? opts.data.previous.key : currentDocIdPrev
    ...
    const onRestoreRevision = (revision) => {
      ...
      if (isRevision) {
        ...
        const fileType =
          Asc.c_oAscFileType[(revision.fileType || "").toUpperCase()] || Asc.c_oAscFileType.DOCX
    ...
    return props.isNavigate ? (
      ...
      <Popover
        id="version-history-popover"
        closeByOutsideClick={false}
        className="popover__titled"
        onPopoverClosed={() => props.onclosed()}
      >
      ...
      <Sheet
        id="version-history-sheet"
        backdrop={true}
        closeByOutsideClick={false}
        onSheetClosed={() => props.onclosed()}
      >
        <VersionHistoryView
          onSetHistoryData={onSetHistoryData}
          onSelectRevision={onSelectRevision}
