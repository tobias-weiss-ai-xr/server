...
export class storeComments {
  ...
  addComment(comment) {
    comment.groupName
      ? this.groupCollectionComments.push(comment)
      : this.collectionComments.push(comment)
  }

  removeComment(id) {
    const collection =
      this.collectionComments.length > 0 ? this.collectionComments : this.groupCollectionComments
  ...
  findComment(id) {
    const collection =
      this.collectionComments.length > 0 ? this.collectionComments : this.groupCollectionComments
    const comment = collection.find((item) => {
      return item.uid === id
    })
    return comment
  }
