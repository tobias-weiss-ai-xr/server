...
const PageAbout = (props) => {
  ...
  const publisherPrintUrl = publisherUrl.replace(/https?:\/{2}|\/$/g, "")
  ...
  return (
    <Page
      className={`about${!Device.phone ? " about_tablet" : Device.ios ? " about_ios" : " about_android"}`}
    >
      ...
      {licInfo && typeof licInfo === "object" && typeof customer === "object" ? (
        <Fragment>
          ...
          {mailCustomer || phoneCustomer ? (
            <div className="about__customer">
              {mailCustomer?.length ? (
                <p className="about__text">
                  <Link id="settings-about-email" external={true} href={`mailto:${mailCustomer}`}>
                    {mailCustomer}
                  </Link>
                </p>
              ) : null}
              {phoneCustomer?.length ? (
                <p className="about__text">
                  <Link id="settings-about-tel" external={true} href={`tel:${phoneCustomer}`}>
                    {phoneCustomer}
                  </Link>
                </p>
              ) : null}
          ...
          {addressCustomer?.length ? (
            <div className="about__customer">
              <p className="about__text">
                <Link id="settings-about-address" external={true}>
                  {addressCustomer}
                </Link>
              </p>
            </div>
          ...
          {(nameCustomer?.length || infoCustomer?.length || urlCustomer?.length) && (
            <div className="about__customer">
              {nameCustomer?.length && (
                <p id="settings-about-name" className="about__text">
                  {nameCustomer}
                </p>
              )}
              {infoCustomer?.length && <p className="about__text">{infoCustomer}</p>}
              {urlCustomer?.length && (
                <p className="about__text">
                  <Link
                    id="settings-about-url"
                    external={true}
                    target="_blank"
          ...
          <div className="about__licensor" id="settings-about-licensor">
            ...
            <p className="about__text">
              <Link external={true} target="_blank" href={publisherUrl}>
                {publisherPrintUrl}
              </Link>
            </p>
          </div>
        ...
        <Fragment>
          ...
          <div className="about__licensor">
            <p className="about__text" id="settings-about-name">
              {publisherName}
            </p>
            <p className="about__text">
              <Link id="settings-about-url" external={true} target="_blank" href={publisherUrl}>
                {publisherPrintUrl}
              </Link>
            </p>
          </div>
