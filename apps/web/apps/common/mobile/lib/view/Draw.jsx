...
import { Button, Icon, Range, Sheet, f7 } from "framework7-react"
  ...
  return (
    <React.Fragment>
      {isDrawingTool && (
        <>
          <Sheet
            className="draw-sheet draw-sheet--color-picker"
            backdrop
            swipeToClose
            onSheetOpened={() => {
              attachBackdropSwipeClose()
          ...
          >
            <div className="draw-sheet-label">
              <span>{_t.textCustomColor}</span>
            </div>
            <WheelColorPicker
              ref={pickerRef}
          ...
          <Sheet
            className="draw-sheet draw-sheet--settings"
            backdrop
            swipeToClose
            onSheetOpen={() => {
              document.activeElement?.blur()
            }}
            onSheetOpened={attachBackdropSwipeClose}
            onSheetClosed={removeBackdropSwipeClose}
            style={{ height: "auto" }}
          >
            ...
            <div className="draw-sheet-item">
              ...
              <Range
                ...
                max={10}
                step={0.5}
                value={settings[currentTool].lineSize}
            ...
            <div className="draw-sheet-label">
              <span>{_t.textOpacity}</span>
            </div>
            <div className="draw-sheet-item">
              <input
                style={{ "--color": settings[currentTool].color }}
                className={
                  Device.android ? "opacity-range-input--android" : "opacity-range-input--ios"
                }
                type="range"
                min={0}
                max={100}
                ...
                onChange={(e) => setSettings({ opacity: Number.parseInt(e.target.value) })}
              />
            </div>
          </Sheet>
        </>
      ...
      <div className="draw-toolbar">
        ...
        <div className="draw-toolbar-item">
          <Button
            type="button"
            sheetOpen={isDrawingTool ? ".draw-sheet--settings" : undefined}
        ...
        <div className="draw-toolbar-item">
          <Button
            type="button"
            disabled={!enableErasing}
            fill={currentTool === "eraser"}
        ...
        <div className="draw-toolbar-item">
          <Button
            type="button"
            disabled={!enableErasing}
            onClick={() => setTool("eraseEntireScreen")}
          >
