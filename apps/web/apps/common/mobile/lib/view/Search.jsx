...
class SearchSettingsView extends Component {
  ...
  render() {
    ...
    const content = <View style={show_popover ? popoverStyle : null}>{extra}</View>
    return show_popover ? (
      <Popover id="idx-search-settings" className="popover__titled">
        {content}
      </Popover>
    ) : (
      <Popup id="idx-search-settings" className="search-settings-popup">
        {content}
      </Popup>
    )
  }
...
class SearchView extends Component {
      ...
      this.searchbar = f7.searchbar.create({
        ...
        on: {
          ...
          searchbarEnable: (sb) => {
            ...
            if (this.searchbar?.enabled && !this.props.isViewer) {
              searchOptions.usereplace || searchOptions.isReplaceAll
                ? this.searchbar.el.classList.add("replace")
  ...
  onEditorTouchEnd(e) {
    ...
    if (this.searchbar.enabled) {
      ...
      if (this.startPoint) {
        distance =
          this.startPoint.x === undefined || this.startPoint.y === undefined
            ? 0
            : Math.sqrt(
                (endPoint.x -= this.startPoint.x) * endPoint.x +
                  (endPoint.y -= this.startPoint.y) * endPoint.y,
              )
      } else {
        distance = 0
      }
    ...
    if (text && this.state.searchQuery !== text) {
      ...
      if (this.searchTimer === undefined) {
        this.searchTimer = setInterval(() => {
          ...
          if (
            !(this.state.searchQuery === "" || this.props.onSearchQuery(this.searchParams(), true))
          ) {
            this.props.onSearchQuery(this.searchParams(), true)
            clearInterval(this.searchTimer)
            this.searchTimer = undefined
  ...
  render() {
    ...
    if (this.searchbar?.enabled && !isViewer) {
      searchOptions.usereplace || searchOptions.isReplaceAll
        ? this.searchbar.el.classList.add("replace")
        : this.searchbar.el.classList.remove("replace")
    }
    ...
    return (
      <form className="searchbar">
        {isIos ? <div className="searchbar-bg" /> : null}
        <div className="searchbar-inner">
          <div className="buttons-row searchbar-inner__left">
            <a
              id="idx-btn-search-settings"
              className="link icon-only no-fastclick"
          ...
          <div className="searchbar-inner__center">
            <div className="searchbar-input-wrap">
              <input
                className={`searchbar-input ${searchQuery.length > 0 ? "input-with-value" : ""}`}
                value={searchQuery}
                placeholder={_t.textSearch}
                type="search"
                maxLength="255"
                ...
                ref={(el) => (this.refSearchbarInput = el)}
              />
            ...
            <div
              ...
              style={(usereplace || isReplaceAll) && !isViewer ? null : hidden}
            >
              <input
                value={replaceQuery}
                placeholder={_t.textReplace}
                type="text"
                maxLength="255"
                id="idx-replace-val"
                onChange={(e) => {
          ...
          <div className="buttons-row searchbar-inner__right">
            {!isViewer && (
              <div className="buttons-row buttons-row-replace">
                {isReplaceAll ? (
                  <a
                    id="replace-all-link"
                    className={`link ${searchQuery.trim().length && replaceQuery.trim().length ? "" : "disabled"}`}
                    onClick={() => this.onReplaceAllClick()}
                  >
                    {_t.textReplaceAll}
                  </a>
                ) : usereplace ? (
            ...
            <div className="buttons-row">
              <a
                className={`link icon-only prev no-fastclick ${searchQuery.trim().length ? "" : "disabled"}`}
                onClick={() => this.onSearchClick(SEARCH_BACKWARD)}
              >
                {Device.ios ? (
              ...
              <a
                className={`link icon-only next no-fastclick ${searchQuery.trim().length ? "" : "disabled"}`}
                onClick={() => this.onSearchClick(SEARCH_FORWARD)}
              >
                {Device.ios ? (
...
export {
  SearchViewWithObserver as SearchView,
  SearchSettingsViewWithObserver as SearchSettingsView,
}
