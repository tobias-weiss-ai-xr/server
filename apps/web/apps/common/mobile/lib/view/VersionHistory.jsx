...
import { BlockTitle, Icon, Link, List, ListItem, NavRight, Navbar, Page } from "framework7-react"
import { inject, observer } from "mobx-react"
import React, { useCallback, useEffect, useState } from "react"
import { useTranslation } from "react-i18next"
import { Device } from "../../utils/device"

const VersionHistoryView = inject(
  "storeVersionHistory",
  "users",
)(
  observer((props) => {
    ...
    return (
      <Page className="page-version-history">
        <Navbar
          title={t("Common.VersionHistory.textVersionHistory")}
          backLink={!Device.phone && isNavigate ? t("Common.VersionHistory.textBack") : null}
        >
          {Device.phone ? (
            <NavRight>
                ...
                <React.Fragment key={`block-${index}`}>
                  ...
                  >
                    {versions.map((version, index) => {
                      return (
                        <ListItem
                          className={`version-history__item ${version === currentVersion ? "version-history__item_active" : ""}`}
                          key={`version-${index}`}
                          link="#"
                          title={version.created}
                          subtitle={AscCommon.UserInfoParser.getParsedName(version.username)}
                          onClick={() => handleClickRevision(version)}
                        >
                          <div
                            slot="media"
                            className="version-history__user"
                            style={{
                              backgroundColor: usersVersions.find(
                                (user) => user.id === version.userid,
                              ).color,
                            }}
                          >
                            {usersStore.getInitials(
                              AscCommon.UserInfoParser.getParsedName(version.username),
                            )}
                          </div>
                            ...
                            version.canRestore && (
                              <div slot="inner">
                                <button
                                  ...
                                  onClick={() => props.onRestoreRevision(version)}
                                >
                                  {t("Common.VersionHistory.textRestore")}
                                </button>
                              </div>
                            )}
                        </ListItem>
