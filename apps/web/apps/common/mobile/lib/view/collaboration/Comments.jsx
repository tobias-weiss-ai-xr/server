...
const AddCommentPopup = inject("storeComments")(
  observer((props) => {
    ...
    return (
      <Popup className="add-comment-popup">
        <Navbar>
          ...
          <NavRight>
            <Link
              className={stateText.length === 0 && "disabled"}
              onClick={() => {
                f7.popup.close(".add-comment-popup")
                setTimeout(() => {
                  props.closeAddComment()
            ...
            >
              {Device.android ? (
                <SvgIcon
                  symbolId={IconDoneCommentWhite.id}
                  className={"icon icon-svg icon icon-svg__white"}
                />
              ) : (
                _t.textDone
              )}
        ...
        <div className="wrap-comment">
          <div className="comment-header">
            {Device.android && (
              <div className="initials" style={{ backgroundColor: `${userInfo.color}` }}>
                {userInfo.initials}
              </div>
            )}
          ...
          <div className="wrap-textarea">
            <textarea
              placeholder={_t.textAddComment}
              value={stateText}
              onChange={(event) => {
                setText(event.target.value)
              }}
              ref={(el) => (refInputComment = el)}
            />
          </div>
        </div>
      </Popup>
...
const AddCommentDialog = inject("storeComments")(
  observer((props) => {
    const { t } = useTranslation()
    const _t = t("Common.Collaboration", { returnObjects: true })
    const userInfo = props.userInfo
...
const AddComment = (props) => {
  return Device.phone ? <AddCommentPopup {...props} /> : <AddCommentDialog {...props} />
}
        ...
        {comment && (
          <Fragment>
            {comment.editable && (
              <ActionsButton
                onClick={() => {
                  onCommentMenuClick("editComment", comment)
                }}
              >
                {_t.textEdit}
              </ActionsButton>
            ...
            {!comment.resolved && comment.editable ? (
              <ActionsButton
                onClick={() => {
                  onCommentMenuClick("resolve", comment)
                }}
              >
                {_t.textResolve}
              </ActionsButton>
            ...
            <ActionsButton
              onClick={() => {
                onCommentMenuClick("addReply", comment)
              }}
            >
              {_t.textAddReply}
            </ActionsButton>
            {comment.removable && (
              <ActionsButton
                color="red"
                onClick={() => {
                  onCommentMenuClick("deleteComment", comment)
                }}
              >
        ...
        {reply && (
          <Fragment>
            {reply.editable && (
              <ActionsButton
                onClick={() => {
                  onCommentMenuClick("editReply", comment, reply)
                }}
              >
                {_t.textEdit}
              </ActionsButton>
            ...
            {reply.removable && (
              <ActionsButton
                color="red"
                onClick={() => {
                  onCommentMenuClick("deleteReply", comment, reply)
                }}
              >
                {_t.textDeleteReply}
              </ActionsButton>
...
// Edit comment
const EditCommentPopup = inject("storeComments")(
  observer(({ storeComments, comment, onEditComment }) => {
    const { t } = useTranslation()
    const _t = t("Common.Collaboration", { returnObjects: true })
    ...
    return (
      <Popup className="edit-comment-popup">
        ...
        <div className="wrap-comment">
          <div className="comment-header">
            {Device.android && (
              <div className="initials" style={{ backgroundColor: `${comment.userColor}` }}>
          ...
          <div className="wrap-textarea">
            <textarea
              placeholder={_t.textEditComment}
              value={stateText}
              onChange={(event) => {
                setText(event.target.value)
              }}
              ref={(el) => (refInputEditComment = el)}
            />
          </div>
        </div>
      </Popup>
...
const EditCommentDialog = inject("storeComments")(
  observer(({ storeComments, comment, onEditComment }) => {
    const { t } = useTranslation()
    const _t = t("Common.Collaboration", { returnObjects: true })
...
const AddReplyPopup = inject("storeComments")(
  observer(({ storeComments, userInfo, comment, onAddReply }) => {
    const { t } = useTranslation()
    const _t = t("Common.Collaboration", { returnObjects: true })
    const [stateText, setText] = useState("")
    ...
    return (
      <Popup className="add-reply-popup">
        ...
        <div className="wrap-comment">
          ...
          <div className="wrap-textarea">
            <textarea
              placeholder={_t.textAddReply}
              value={stateText}
              onChange={(event) => {
                setText(event.target.value)
              }}
              ref={(el) => (refInputReplyComment = el)}
            />
          </div>
        </div>
      </Popup>
...
const AddReplyDialog = inject("storeComments")(
  observer(({ storeComments, userInfo, comment, onAddReply }) => {
    const { t } = useTranslation()
    const _t = t("Common.Collaboration", { returnObjects: true })
    const templateInitials = `<div class="initials" style="background-color: ${userInfo.color};">${userInfo.initials}</div>`
...
const EditReplyPopup = inject("storeComments")(
  observer(({ storeComments, comment, reply, onEditReply }) => {
    const { t } = useTranslation()
    const _t = t("Common.Collaboration", { returnObjects: true })
    ...
    return (
      <Popup className="edit-reply-popup">
        ...
        <div className="wrap-comment">
          <div className="comment-header">
            {Device.android && (
              <div className="initials" style={{ backgroundColor: `${reply.userColor}` }}>
                {reply.userInitials}
              </div>
            )}
            <div>
          ...
          <div className="wrap-textarea">
            <textarea
              placeholder={_t.textEditReply}
              value={stateText}
              onChange={(event) => {
                setText(event.target.value)
              }}
              ref={(el) => (relInputReplyComment = el)}
            />
          </div>
        </div>
      </Popup>
...
const EditReplyDialog = inject("storeComments")(
  observer(({ storeComments, comment, reply, onEditReply }) => {
    const { t } = useTranslation()
    const _t = t("Common.Collaboration", { returnObjects: true })
    ...
    return (
      <div
        id="edit-reply-dialog"
        ref={(el) => (refContainerDialog = el)}
        className="edit-reply-dialog"
      />
    )
  }),
)
...
const pickLink = (message) => {
  ...
  message.replace(Common.Utils.ipStrongRe, (subStr) => {
    ...
    if (result) subStr = subStr.substring(0, result.index)
  ...
  message.replace(Common.Utils.emailStrongRe, (subStr) => {
    ...
    if (!elem)
      arrayComment.push({
        ...
        str: (
          <a onClick={() => window.open(ref)} href={ref}>
            {subStr}
          </a>
        ),
      })
  ...
  arrayComment = arrayComment.sort((item1, item2) => item1.start - item2.start)

  let str_res =
    arrayComment.length > 0 ? (
      <label>
        {message.substring(0, arrayComment[0].start)}
        {arrayComment[0].str}
      </label>
    ) : (
      <label>{message}</label>
  ...
  for (let i = 1; i < arrayComment.length; i++) {
    str_res = (
      <label>
        {str_res}
        {Common.Utils.String.htmlEncode(
          message.substring(arrayComment[i - 1].end, arrayComment[i].start),
        )}
        {arrayComment[i].str}
      </label>
    )
  ...
  if (arrayComment.length > 0) {
    str_res = (
      <label>
        {str_res}
        {Common.Utils.String.htmlEncode(message.substring(arrayComment[i - 1].end, message.length))}
      </label>
    )
  }
...
const ViewComments = inject(
  ...
  observer(
    ({
      ...
      const sortComments =
        comments.length > 0 ? [...comments].sort((a, b) => (a.time > b.time ? -1 : 1)) : null
      const isProtected = storeAppOptions.isProtected
      const typeProtection = storeAppOptions.typeProtection
      const isAvailableCommenting =
        !isProtected ||
        typeProtection === Asc.c_oAscEDocProtect.TrackedChanges ||
        typeProtection === Asc.c_oAscEDocProtect.Comments
      ...
      return (
        <Page>
            ...
            <List className="comment-list">
              {sortComments.map((comment, indexComment) => {
                return (
                  !comment.hide && (
                    <ListItem
                      key={`comment-${indexComment}`}
                      onClick={(e) => {
                        !e.target.closest(".comment-menu") && !e.target.closest(".reply-menu")
                          ? showComment(comment)
                          : null
                      }}
                    >
                        ...
                        <div className="left">
                          {isAndroid && (
                            <div
                              className="initials"
                              style={{
                                backgroundColor: `${comment.userColor ? comment.userColor : "#cfcfcf"}`,
                              }}
                            >
                              {comment.userInitials}
                            </div>
                        ...
                        {isEdit && !viewMode && (
                          <div className="right">
                            {comment.editable &&
                              displayMode === "markup" &&
                              !wsProps?.Objects &&
                              (!isViewer || canEditComments) &&
                              isAvailableCommenting && (
                                <div
                                ...
                                >
                                  {Device.ios ? (
                                    comment.resolved ? (
                                      <SvgIcon
                                        symbolId={IconResolveCommentCheckIos.id}
                                        className={"icon icon-svg"}
                                      />
                                    ) : (
                                      <SvgIcon
                                        symbolId={IconResolveCommentIos.id}
                                        className={"icon icon-svg"}
                                      />
                                    )
                                  ) : comment.resolved ? (
                                    <SvgIcon
                                      symbolId={IconResolveCommentCheckAndroid.id}
                                      className={"icon icon-svg"}
                                    />
                                  ) : (
                                    <SvgIcon
                                      symbolId={IconResolveCommentAndroid.id}
                                      className={"icon icon-svg"}
                                    />
                                  )}
                                </div>
                            ...
                            {displayMode === "markup" &&
                              ...
                              isAvailableCommenting && (
                                ...
                                >
                                  {Device.ios ? (
                                    <SvgIcon
                                      symbolId={IconMenuCommentIos.id}
                                      className={"icon icon-svg"}
                                    />
                                  ) : (
                                    <SvgIcon
                                      symbolId={IconMenuCommentAndroid.id}
                                      className={"icon icon-svg"}
                                    />
                                  )}
                                </div>
                      ...
                      <div slot="footer">
                        {comment.quote && (
                          <div className="comment-quote">{sliceQuote(comment.quote)}</div>
                        )}
                        <div className="comment-text">
                          <pre>{pickLink(comment.comment)}</pre>
                        </div>
                        {comment.replies.length > 0 && (
                          <ul className="reply-list">
                            {comment.replies.map((reply, indexReply) => {
                              return (
                                <li
                                  key={`reply-${indexComment}-${indexReply}`}
                                          ...
                                          <div className="left">
                                            {isAndroid && (
                                              <div
                                                className="initials"
                                                style={{
                                                  backgroundColor: `${reply.userColor ? reply.userColor : "#cfcfcf"}`,
                                                }}
                                              >
                                                {reply.userInitials}
                                              </div>
          ...
          <CommentActions
            ...
            opened={commentActionsOpened}
            openActionComment={openActionComment}
          />
          <ReplyActions
            comment={clickComment}
            reply={reply}
            onCommentMenuClick={onCommentMenuClick}
            opened={replyActionsOpened}
            openActionReply={openActionReply}
          />
        </Page>
      )
    },
      ...
      return (
        <Fragment>
          <Toolbar position="bottom">
            {isEdit && !viewMode && (
              <Link
                className={`btn-add-reply${((wsProps?.Objects || isViewer) && !canEditComments) || !isAvailableCommenting ? " disabled" : ""}`}
                href="#"
                onClick={() => {
                  onCommentMenuClick("addReply", comment)
                }}
...
const ViewCommentSheet = ({ closeCurComments, onCommentMenuClick, onResolveComment, wsProps }) => {
  ...
  const handleTouchStart = (event) => {
    ...
    setStartY(Number.parseInt(touchObj.clientY))
  }
  const handleTouchMove = (event) => {
    const touchObj = event.changedTouches[0]
    const dist = Number.parseInt(touchObj.clientY) - stateStartY
    if (dist < 0) {
      // to top
      setHeight("90%")
      setOpacity(1)
  ...
  const handleTouchEnd = (event) => {
    const touchObj = event.changedTouches[0]
    const swipeEnd = Number.parseInt(touchObj.clientY)
  ...
  return (
    <Sheet
      id="view-comment-sheet"
      style={{ height: `${stateHeight}`, opacity: `${stateOpacity}` }}
      backdrop={true}
...
const ViewCurrentComments = (props) => {
  return Device.phone ? <ViewCommentSheet {...props} /> : <ViewCommentPopover {...props} />
}

export { AddComment, EditComment, AddReply, EditReply, ViewComments, ViewCurrentComments }
