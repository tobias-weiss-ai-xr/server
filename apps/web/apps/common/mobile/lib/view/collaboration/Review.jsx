...
const PageReview = (props) => {
  ...
  const isDisableAllSettings =
    props.isReviewOnly || props.displayMode === "final" || props.displayMode === "original"
  ...
  return (
    <Page>
      ...
      <List>
        {canReview && (
          <ListItem
            title={_t.textTrackChanges}
            className={isDisableAllSettings ? "disabled" : ""}
            disabled={isProtected}
          >
            <Toggle
              checked={props.trackChanges}
              onToggleChange={() => props.onTrackChanges(!props.trackChanges)}
            />
          </ListItem>
        )}
        {!props.isRestrictedEdit && (
          <ListItem
            title={_t.textDisplayMode}
            link={"/display-mode/"}
            routeProps={{
      ...
      <List>
        <ListItem title={_t.textReviewChange} link={"/review-change/"}>
          {Device.ios ? (
            ...
            <SvgIcon
              slot="media"
              symbolId={IconReviewChangesAndroid.id}
        ...
        {canReview && !props.canUseReviewPermissions && !isProtected && (
          <ListItem
            title={_t.textAcceptAllChanges}
            link="#"
            className={`no-indicator${isDisableAllSettings ? " disabled" : ""}`}
            onClick={() => {
              props.onAcceptAll()
            }}
          >
            {Device.ios ? (
              <SvgIcon
                slot="media"
                symbolId={IconAcceptChangesIos.id}
                className={"icon icon-svg"}
              />
            ) : (
              <SvgIcon
                slot="media"
                symbolId={IconAcceptChangesAndroid.id}
                className={"icon icon-svg"}
              />
            )}
          </ListItem>
        ...
        {canReview && !props.canUseReviewPermissions && !isProtected && (
          ...
          >
            {Device.ios ? (
              <SvgIcon
                slot="media"
                symbolId={IconRejectChangesIos.id}
                className={"icon icon-svg"}
              />
            ) : (
              <SvgIcon
                slot="media"
                symbolId={IconRejectChangesAndroid.id}
                className={"icon icon-svg"}
              />
            )}
          </ListItem>
...
const PageReviewChange = inject("storeAppOptions")(
  observer((props) => {
    ...
    const isLockAcceptReject =
      !change ||
      (change && !change.editable) ||
      displayMode === "final" ||
          ...
          <span className="change-buttons row">
            ...
            {!props.isReviewOnly && change && change?.editable && (
              <span className="delete">
                <Link
                  href="#"
                  id="btn-delete-change"
                  onClick={() => {
                    props.onDeleteChange()
                  }}
                >
                  {_t.textDelete}
                </Link>
          ...
          <span className="next-prev row">
            ...
            >
              {Device.ios ? (
                ...
                <SvgIcon
                  slot="media"
                  symbolId={IconPrevChangeAndroid.id}
                  className={"icon icon-svg"}
                />
              )}
            </Link>
            ...
            >
              {Device.ios ? (
                ...
                <SvgIcon
                  slot="media"
                  symbolId={IconNextChangeAndroid.id}
                  className={"icon icon-svg"}
                />
              )}
            </Link>
        ...
        {change ? (
          <div className="block-description">
            <div className="header-change">
              {isAndroid && (
                <div className="initials" style={{ backgroundColor: `#${change.color}` }}>
                  {change.initials}
                </div>
              )}
              <div className="info">
