class CThumbnailLoader {
  ...
  openBinary(arrayBuffer) {
    ...
    if (!isOffsets) this.data = new Uint8ClampedArray(memorySize)
    ...
    if (!this.offsets) this.binaryFormat = undefined
  ...
  getImage = function (index, canvas, ctx) {
    ...
    if (!this.offsets) {
      ...
      while (pixelsCount > 0) {
        ...
        if (0 === tmpValue) {
          len0 = binaryAlpha[binaryIndex++]
          if (len0 > pixelsCount) len0 = pixelsCount
          while (len0 > 0) {
            len0--
            imagePixels[alphaChannel] =
              imagePixels[alphaChannel + 1] =
              imagePixels[alphaChannel + 2] =
                255
