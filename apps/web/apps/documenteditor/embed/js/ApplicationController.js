...
DE.ApplicationController = new (function () {
  ...
  function loadConfig(data) {
    ...
    if (typeof config.customization === "object") {
      if (typeof config.customization.goback === "object" && config.canBackToFolder !== false) {
        _canback =
          config.customization.close === undefined
            ? config.customization.goback.url ||
              (config.customization.goback.requestClose && config.canRequestClose)
      ...
      if (config.customization.close && typeof config.customization.close === "object")
        config.canCloseEditor =
          config.customization.close.visible !== false &&
          config.canRequestClose &&
          !config.isDesktopApp
    }
  ...
  function loadDocument(data) {
    ...
    if (docConfig) {
      ...
      const canRenameAnonymous = !(
        typeof config.customization === "object" &&
        typeof config.customization.anonymous === "object" &&
        config.customization.anonymous.request === false
      )
      const guestName =
        typeof config.customization === "object" &&
        typeof config.customization.anonymous === "object" &&
        typeof config.customization.anonymous.label === "string" &&
        config.customization.anonymous.label.trim() !== ""
          ? common.utils.htmlEncode(config.customization.anonymous.label)
          : me.textGuest
      const value = canRenameAnonymous ? common.localStorage.getItem("guest-username") : null
      const user = common.utils.fillUserInfo(
        config.user,
        config.lang,
        value ? `${value} (${guestName})` : me.textAnonymous,
      ...
      const type = /^(?:(pdf|djvu|xps|oxps))$/.exec(docConfig.fileType)
  ...
  function onLongActionBegin(type, id) {
    ...
    if (type === Asc.c_oAscAsyncActionType.BlockInteraction) {
      if (!me.loadMask) me.loadMask = new common.view.LoadMask()
  ...
  function onLongActionEnd(type, id) {
    if (id === Asc.c_oAscAsyncAction.Submit) {
      ...
      if (!_submitFail) {
        if (!$submitedTooltip) {
          $submitedTooltip = $(
            `<div class="submit-tooltip" style="display:none;">${me.textSubmited}</div>`,
          )
          $(document.body).append($submitedTooltip)
          $submitedTooltip.on("click", () => {
            $submitedTooltip.hide()
          })
        }
    ...
    if (data) {
      ...
      if (
        ...
        if (type === Asc.c_oAscMouseMoveDataTypes.Hyperlink) {
          const hyperProps = data.get_Hyperlink()
          if (!hyperProps) return
          if (hyperProps.get_NoCtrl?.()) str = hyperProps.get_ToolTip() || hyperProps.get_Value()
          else str = me.txtPressLink.replace("%1", common.utils.isMac ? "⌘" : me.textCtrl)
        } else str = data.get_FormHelpText()
        ...
        if (!$tooltip) $tooltip = $ttEl.data("bs.tooltip").tip()

        if (!$tooltip.is(":visible")) {
          const tip = $ttEl.data("bs.tooltip")
          tip.options.title = str
          tip.show([-1000, -1000])
        } else $tooltip.find(".tooltip-inner").text(str)
        ...
        if ($ttEl.ttpos[0] + ttWidth + 10 > bodyWidth) {
          ...
          if ($ttEl.ttpos[1] < 0) $ttEl.ttpos[1] += ttHeight + ttOffset[1] + 20
      ...
      api.asc_Print(
        new Asc.asc_CDownloadOptions(
          ...
          $.browser.chrome ||
            $.browser.safari ||
            $.browser.opera ||
            ($.browser.mozilla && $.browser.versionNumber > 86),
        ),
      )
  }
  ...
  function onShowListActions(obj, x, y) {
    ...
    const menuContainer = DE.ApplicationView.getMenuForm()

    if (!$listControlMenu) {
      $listControlMenu = menuContainer.find("ul")
      $listControlMenu.on("click", "li", (e) => {
        let value = $(e.target).attr("value")
        if (value) {
          value = Number.parseInt(value)
          setTimeout(() => {
            value !== -1 &&
              api.asc_SelectContentControlListItem(
                listControlItems[value],
                listObj.get_InternalId(),
              )
          }, 1)
        }
      })
      $("#editor_sdk").on("click", (e) => {
        if (e.target.localName === "canvas") {
          if (me._preventClick) me._preventClick = false
          else {
            $listControlMenu?.hide()
            api.asc_UncheckContentControlButtons()
          }
    ...
    if (specProps) {
      ...
      if (formProps) {
        if (!formProps.get_Required() || count < 1) {
          ...
          $listControlMenu.append(
            `<li><a tabindex="-1" type="menuitem" style="opacity: 0.6" value="0">${text.trim() !== "" ? text : me.txtEmpty}</a></li>`,
          )
      ...
      for (let i = 0; i < count; i++) {
        if (specProps.get_ItemValue(i) !== "" || !formProps) {
          $listControlMenu.append(
            `<li><a tabindex="-1" type="menuitem" value="${i + k}">${common.utils.htmlEncode(specProps.get_ItemDisplayText(i))}</a></li>`,
          )
      ...
      if (!formProps && listControlItems.length < 1) {
        $listControlMenu.append(
          `<li><a tabindex="-1" type="menuitem" value="0">${me.txtEmpty}</a></li>`,
        )
  ...
  function onDocumentContentReady() {
    ...
    const zf = config.customization?.zoom ? Number.parseInt(config.customization.zoom) : -2
    zf === -1 ? api.zoomFitToPage() : zf === -2 ? api.zoomFitToWidth() : api.zoom(zf > 0 ? zf : 100)

    api.asc_setViewerTargetType(
      config.customization && config.customization.pointerMode === "hand" ? "hand" : "select",
    )
    ...
    if (itemsCount < 1) $("#box-tools").addClass("hidden")
    ...
    DE.ApplicationView.tools.get("#idt-fullscreen").on("click", () => {
      common.utils.openLink(embedConfig.fullscreenUrl)
    })

    DE.ApplicationView.tools.get("#idt-download").on("click", () => {
    ...
    DE.ApplicationView.tools.get("#idt-print").on("click", () => {
      const printCallback = () => {
        api.asc_Print(
          new Asc.asc_CDownloadOptions(
            null,
      ...
      if (api?.asc_CheckPrintPassword && api.asc_CheckPrintPassword() === false) {
    ...
    DE.ApplicationView.tools.get("#idt-close").on("click", () => {
    ...
    DE.ApplicationView.tools.get("#idt-download-docx").on("click", () => {
      downloadAs(Asc.c_oAscFileType.DOCX)
    })
    DE.ApplicationView.tools.get("#idt-download-pdf").on("click", () => {
      downloadAs(Asc.c_oAscFileType.PDF)
    })

    DE.ApplicationView.tools.get("#idt-search").on("click", () => {
    ...
    $pagenum.on({
      keyup: (e) => {
        if (e.keyCode === 13) {
          let newPage = Number.parseInt($("#page-number").val())
    ...
    if (
      appOptions.canSubmitForms &&
      appOptions.canFillForms &&
      !api.asc_IsAllRequiredFormsFilled()
    ) {
      const sgroup = $("#id-submit-group")
      btnSubmit.attr({ disabled: true })
      btnSubmit.css("pointer-events", "none")
      if (!common.localStorage.getItem("de-embed-hide-submittip")) {
        const offset = common.utils.getOffset(btnSubmit)
        $requiredTooltip = $(
          `<div class="required-tooltip bottom-left" style="display:none;"><div class="tip-arrow bottom-left"></div><div>${me.textRequired}</div><div class="close-div">${me.textGotIt}</div></div>`,
        )
        $(document.body).append($requiredTooltip)
        $requiredTooltip.css({
          top: `${offset.top + btnSubmit.height()}px`,
          left: `${offset.left + btnSubmit.outerWidth() / 2 - $requiredTooltip.outerWidth()}px`,
        })
        $requiredTooltip.find(".close-div").on("click", () => {
  ...
  function onEditorPermissions(params) {
    ...
    if (
      ...
      common.controller.modals.showWarning({
        title:
          Asc.c_oLicenseResult.NotBefore === licType
            ? me.titleLicenseNotActive
            : me.titleLicenseExp,
        message:
          Asc.c_oLicenseResult.NotBefore === licType ? me.warnLicenseBefore : me.warnLicenseExp,
        buttons: [],
      })
    ...
    if (!appOptions.canFillForms) {
      ...
      $("#id-btn-prev-field").on("click", () => {
        api.asc_MoveToFillingForm(false)
      })
      $("#id-btn-next-field").on("click", () => {
        api.asc_MoveToFillingForm(true)
      })
      $("#id-btn-clear-fields").on("click", () => {
    ...
    if (_left_width < _right_width)
      $parent.css(
        "padding-left",
        Number.parseFloat($parent.css("padding-left")) + _right_width - _left_width,
      )
    else
      $parent.css(
        "padding-right",
        Number.parseFloat($parent.css("padding-right")) + _left_width - _right_width,
      )
  ...
  function onOpenDocument(progress) {
    ...
    me.loadMask?.setTitle(
      `${me.textLoadingDocument}: ${common.utils.fixedDigits(Math.min(Math.round(proc * 100), 100), 3, "  ")}%`,
    )
  }
  ...
  function onAdvancedOptions(type, advOptions, mode, formatOptions) {
    if (type === Asc.c_oAscAdvancedOptionsID.DRM) {
      ...
      const submitPassword = (val) => {
        api?.asc_setAdvancedOptions(
          Asc.c_oAscAdvancedOptionsID.DRM,
          new Asc.asc_CDRMAdvancedOptions(val),
        )
        me.loadMask?.show()
        if (!isCustomLoader) {
      ...
      api?.asc_setAdvancedOptions(
        Asc.c_oAscAdvancedOptionsID.TXT,
        advOptions.asc_getRecommendedSettings() || new Asc.asc_CTextOptions(),
      )
      onLongActionEnd(Asc.c_oAscAsyncActionType.BlockInteraction, LoadingDocument)
    }
  ...
  function onError(id, level, errData) {
    ...
    switch (id) {
      ...
      case Asc.c_oAscError.ID.ConvertationOpenFormat:
        ...
        else message = me.errorInconsistentExt
        break
    ...
    common.controller.modals.showWarning({
      title:
        level === Asc.c_oAscError.Level.Critical ? me.criticalErrorTitle : me.notcriticalErrorTitle,
      message: message,
      buttons: [me.txtClose],
      callback: (btn) => {
  ...
  function onProcessMouse(data) {
    if (data.type === "mouseup") {
      ...
      if (e) {
        ...
        api.OnMouseUp(data.x - r.left, data.y - r.top)
      }
    }
  }
  ...
  function setBranding(value) {
    if (value?.logo) {
      ...
      if (value.logo.image || value.logo.imageEmbedded) {
        logo.html(
          `<img src="${value.logo.image || value.logo.imageEmbedded}" style="max-width:300px; max-height:20px;"/>`,
        )
        logo.css({ "background-image": "none", width: "auto", height: "auto" })

        value.logo.imageEmbedded &&
          console.log(
            "Obsolete: The 'imageEmbedded' parameter of the 'customization.logo' section is deprecated. Please use 'image' parameter instead.",
          )
      }
  ...
  function createController() {
    ...
    $(document.body)
      ...
      .on("hidden.bs.dropdown", ".dropdown", (e) => {
        if (!ismodalshown) api.asc_enableKeyEvents(true)
      })
    ...
    const result = /[\?\&]fileType=\b(pdf)|(djvu|xps|oxps)\b&?/i.exec(window.location.search)
    const isPDF =
      (!!result && result.length && typeof result[2] === "string") ||
      (!!result && result.length && typeof result[1] === "string" && !window.isPDFForm)
  ...
  return {
    ...
    txtOpenWarning:
      "Clicking this link can be harmful to your device and data.To protect you computer, click only those hyperlinks from trusted sources. This location may be unsafe:<br>%1<br>Are you sure you want to continue?",
    txtYes: "Yes",
    txtNo: "No",
    textOk: "OK",
    textCancel: "Cancel",
    txtSecurityWarningLink:
      'This document is trying to connect to %1.<br>If you trust this site, press "OK" while holding down the ctrl key.',
    txtSecurityWarningOpenFile: 'This document is trying to open file dialog, press "OK" to open.',
    txtSecurityWarningLinkOk:
      'This document is trying to connect to %1.<br>If you trust this site, press "OK".',
    errorCopyDisabled:
