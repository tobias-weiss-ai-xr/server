...
define(["core", "documenteditor/main/app/view/FormsTab"], () => {
  DE.Controllers.FormsTab = Backbone.Controller.extend(
    _.extend(
      {
        models: [],
        collections: [],
        ...
        setConfig: function (config) {
          ...
          const dirRight = Common.UI.isRTL() ? "left" : "right"
          const dirLeft = Common.UI.isRTL() ? "right" : "left"
          this._helpTips = {
            ...
            key: {
              name: "de-form-tip-settings-key",
              placement: `${dirLeft}-bottom`,
              text: this.view.tipFormKey,
              link: {
                text: this.view.tipFieldsLink,
                src: "UsageInstructions/CreateFillableForms.htm",
              },
              target: "#form-combo-key",
              showButton: true,
            },
            "group-key": {
              name: "de-form-tip-settings-group",
              placement: `${dirLeft}-bottom`,
              text: this.view.tipFormGroupKey,
              link: false,
              target: "#form-combo-group-key",
              showButton: true,
            },
            settings: {
              name: "de-form-tip-settings",
              placement: `${dirLeft}-top`,
              text: this.view.tipFieldSettings,
              link: {
                text: this.view.tipFieldsLink,
                src: "UsageInstructions/CreateFillableForms.htm",
              },
              target: "#id-right-menu-form",
              showButton: true,
            },
            ...
            save: this.appConfig.canDownloadForms
              ? {
                  name: "de-form-tip-save",
                  placement: `bottom-${dirLeft}`,
                  text: this.view.tipSaveFile,
                  link: false,
                  target: "#slot-btn-form-save",
                  showButton: true,
                }
              : undefined,
            submit: this.appConfig.isRestrictedEdit
              ? {
                  name: "de-form-tip-submit",
                  placement: `bottom-${dirLeft}`,
                  text: this.view.textRequired,
                  link: false,
                  target: "#slot-btn-header-form-submit",
                  callback: () => {
                    this.api.asc_MoveToFillingForm(true, true, true)
                    this.view.btnSubmit.updateHint(this.view.textRequired)
                  },
                  showButton: true,
                }
              : undefined,
            "submit-required": this.appConfig.isRestrictedEdit
              ? {
                  placement: `bottom-${dirLeft}`,
                  text: this.view.textRequired,
                  link: false,
          ...
          !Common.localStorage.getItem(this._helpTips.key.name) &&
            this.addListeners({ RightMenu: { rightmenuclick: this.onRightMenuClick } })
          this.addListeners({
            FormsTab: {
          ...
          this.appConfig.isPDFSignatureSupport &&
            this.appConfig.isRestrictedEdit &&
            this.api &&
            this.api.asc_registerCallback(
          ...
          this.appConfig.isEdit &&
            ...
            !this.appConfig.isOForm &&
            this.api &&
            this.api.asc_registerCallback(
              "asc_onOFormChangeFinal",
              _.bind(this.onOFormChangeFinal, this),
            )
        },
        ...
        onApiFocusObject: function (selectedObjects) {
          ...
          const control_plain =
            in_control && control_props
              ? control_props.get_ContentControlType() === Asc.c_oAscSdtLevelType.Inline &&
                !control_props.get_ComplexFormPr()
              : false
          ...
          Common.Utils.lockControls(
            Common.enumLock.complexForm,
            in_control && !!control_props && !!control_props.get_ComplexFormPr(),
            { array: [this.view.btnComplexField, this.view.btnImageField] },
          )
          ...
          if (control_props?.get_FormPr()) {
            const isRadio =
              control_props.get_SpecificType() === Asc.c_oAscContentControlSpecificType.CheckBox &&
              control_props.get_CheckBoxPr() &&
              typeof control_props.get_CheckBoxPr().get_GroupKey() === "string"
            isRadio ? this.closeHelpTip("key") : this.closeHelpTip("group-key")
            setTimeout(() => {
              if (this._state.formRadioAdded && isRadio) {
                if (this.showHelpTip("group-key")) {
                  this._state.formRadioAdded = false
                  this.closeHelpTip("settings", true)
                } else this.showHelpTip("settings")
              } else if (this._state.formAdded && !isRadio) {
                if (this.showHelpTip("key")) {
                  this._state.formAdded = false
                  this.closeHelpTip("settings", true)
                } else this.showHelpTip("settings")
              }
            }, 500)
        ...
        onControlsSelect: function (type, options) {
          if (!(this.toolbar.mode?.canFeatureContentControl && this.toolbar.mode.canFeatureForms))
            return

          let oPr
          const oFormPr = new AscCommon.CSdtFormPr()
          oFormPr.put_Role(
            Common.Utils.InternalSettings.get("de-last-form-role") || this._state.lastRoleInList,
          )
          this.toolbar.toolbar.fireEvent("insertcontrol", this.toolbar.toolbar)
          this._state.formAdded === undefined &&
            type !== "radiobox" &&
            (this._state.formAdded = true)
          this._state.formRadioAdded === undefined &&
            type === "radiobox" &&
            (this._state.formRadioAdded = true)
          if (type === "picture") this.api.asc_AddContentControlPicture(oFormPr)
            ...
            const props = new AscCommon.CContentControlPr()
            const datePr = new AscCommon.CSdtDatePickerPr()
            ...
            if (options) {
              if (options.reg) oPr.put_RegExpFormat(options.reg)
              else if (options.mask) oPr.put_MaskFormat(options.mask)
              if (options.placeholder) props.put_PlaceholderText(options.placeholder)
              if (options.fixed !== undefined) oFormPr.put_Fixed?.(options.fixed)
            }
          ...
          } else if (type === "signature") this.api.asc_AddContentControlSignature(oFormPr)
          if (!this._state.formCount) {
            // add first form
        ...
        onModeClick: function (state, lastViewRole) {
          if (this.api) {
            ...
            !state &&
              this.view &&
              Common.Utils.lockControls(Common.enumLock.viewFormFinal, false, {
                array: [this.view.btnViewFormRoles],
              })
            !state &&
              this.view &&
              Common.Utils.lockControls(Common.enumLock.viewFormNotFinal, false, {
                array: [this.view.btnFinal],
              })
          }
        ...
        changeViewFormMode: function (state, saveFlag) {
          if (
            this.view &&
            ((this.view.btnViewFormRoles && state !== this.view.btnViewFormRoles.isActive()) ||
              (this.view.btnFinal?.isActive() && !state))
          ) {
            const btnview = this.view.btnViewFormRoles
            const btnfinal = this.view.btnFinal
            if (btnview && state !== btnview.isActive()) {
              btnview.toggle(state, true)
              if (state && btnview.menu) {
                let current = btnview.menu.getChecked()
                if (current) {
            ...
            } else if (btnfinal?.isActive() && !state) {
        ...
        onPreviewClick: function (state, lastViewRole) {
          ...
          state &&
            this.view &&
            Common.Utils.lockControls(Common.enumLock.viewFormNotFinal, true, {
              array: [this.view.btnFinal],
            })
        },
        ...
        onFinalClick: function (state, saveFlag) {
          ...
          state &&
            this.view &&
            Common.Utils.lockControls(Common.enumLock.viewFormFinal, true, {
              array: [this.view.btnViewFormRoles],
            })
        },
        ...
        onOFormChangeFinal: function (isFinal) {
          ...
          if (this.view) {
            if (this.view.btnViewFormRoles?.isActive())
              // off view form mode
        ...
        onGoTo: function (type) {
          if (this.api) this.api.asc_MoveToFillingForm(type === "next")
          Common.NotificationCenter.trigger("edit:complete", this.toolbar)
        },
        ...
        onSaveFormClick: function () {
          ...
          if (this.api && this.appConfig.canDownload) {
            this.appConfig.isRestrictedEdit && this.appConfig.canFillForms
              ? callback()
              : this.showRolesList(callback)
          }
        },
        ...
        onLongActionBegin: function (type, id) {
          if (id === Asc.c_oAscAsyncAction.Submit && this.view.btnSubmit) {
            ...
            Common.Utils.lockControls(Common.enumLock.submit, true, {
              array: [this.view.btnSubmit],
            })
          }
        },
        ...
        onLongActionEnd: function (type, id) {
          if (id === Asc.c_oAscAsyncAction.Submit && this.view.btnSubmit) {
            Common.Utils.lockControls(Common.enumLock.submit, !this._submitFail, {
              array: [this.view.btnSubmit],
            })
            if (!this._submitFail) {
              ...
              const text =
                typeof this.appConfig.customization.submitForm === "object"
                  ? this.appConfig.customization.submitForm.resultMessage
                  : this.view.textSubmitOk
              if (text === "") return
            ...
            } else Common.NotificationCenter.trigger("doc:mode-apply", "view-form", true, true)
          }
        },
        ...
        onAppReady: function (config) {
          new Promise((accept, reject) => {
            ...
            config.isEdit &&
              config.canFeatureContentControl &&
              config.isFormCreator &&
              !config.isOForm &&
              this.showHelpTip("create") // show tip only when create form in docxf
            if (config.isRestrictedEdit && config.canFillForms && config.isPDFForm && this.api) {
              ...
              if (oform && config.user.roles) {
                if (config.user.roles.length > 0 && oform.asc_canFillRole(config.user.roles[0])) {
                  ...
                  this.view?.showFillingForms(true)
                } else {
                  role.put_OFormNoRole(true)
                  this.view &&
                    config.canRequestFillingStatus &&
                    Common.UI.TooltipManager.showTip({
                      step: "showFillStatus",
                      name: "de-help-tip-fill-status",
                      text: this.view.helpTextFillStatus,
                      target: "#slot-btn-fill-status",
              ...
              else this.view?.showFillingForms(true)
              this.api.asc_setRestriction(Asc.c_oAscRestrictionType.OnlyForms, role)
            }
            if (config.isRestrictedEdit && this.view && this.view.btnSubmit && this.api) {
              if (this.api.asc_IsAllRequiredFormsFilled())
            ...
            if (this.view?.cmbRoles?.cmpEl) {
              const width = Math.max(this.view.lblRoles.$label.width(), 130)
              this.view.cmbRoles.setWidth(width)
              this.view.cmbRoles.cmpEl.find(".form-control").css("width", `${width}px`)
              this.view.cmbRoles.cmpEl.find(".dropdown-menu").css("min-width", `${width}px`)
            }
            this.onRefreshRolesList()
            this.onChangeProtectDocument()
          })
        },

        closeHelpTip: function (step, force) {
          const props = this._helpTips[step]
          if (props) {
            props.tip?.close()
            props.tip = undefined
        ...
        showHelpTip: function (step) {
          ...
          if (
            !(this._helpTips[step].name && Common.localStorage.getItem(this._helpTips[step].name))
          ) {
            const props = this._helpTips[step]
            let target = props.target

            if (props.tip?.isVisible()) return true
        ...
        onRefreshRolesList: function (roles) {
          ...
          this._state.lastRoleInList =
            roles && roles.length > 0
              ? roles[roles.length - 1].asc_getSettings().asc_getName()
              : undefined
          if (this.view) {
        ...
        showRolesList: function (callback) {
          ...
          oform &&
            new DE.Views.SaveFormDlg({
              handler: (result, settings) => {
                if (result === "ok") callback.call(this)
        ...
        requestStartFilling: function () {
          ...
          for (let i = 0; i < roles.length; i++) {
            const role = roles[i].asc_getSettings()
        ...
        onChangeProtectDocument: function (props) {
          ...
          if (props) {
            ...
            if (this.view) {
              const arr = this.view.getButtons()
              ...
              Common.Utils.lockControls(Common.enumLock.docLockReview, props.isReviewOnly, {
                array: arr,
              })
        ...
        onRightMenuClick: function (menu, type, minimized, event) {
          if (!minimized && event && type === Common.Utils.documentSettingsType.Form) {
            this.closeHelpTip("settings", true)
            ;(this._state.formRadioAdded || this._state.formAdded) &&
        ...
        onCountPages: function (count) {
          ...
          this.view?.fieldPages?.setFixedValue(`/ ${count}`)
        },

        onCurrentPage: function (value) {
          if (this.view?.fieldPages) {
            this.view.fieldPages.setValue(value + 1)
            Common.Utils.lockControls(Common.enumLock.firstPage, value < 1, {
              array: [this.view.btnFirstPage, this.view.btnPrevPage],
            })
            Common.Utils.lockControls(
              Common.enumLock.lastPage,
              value >= this._state.pageCount - 1,
              { array: [this.view.btnLastPage, this.view.btnNextPage] },
            )
          }
        },
        ...
        onCurrentRoleChanged: function () {
          this.view?.cmbRoles?.setValue(Common.Utils.InternalSettings.get("de-last-form-role"))
        },

        onRequestRoles: function (tab) {
          if (this._isDocReady) this.requestStartFilling()
          else this._state.needToStartFilling = true
        },
        ...
        showSignatureTooltip: function (valid) {
          ...
          Common.Utils.lockControls(Common.enumLock.formSigned, hasForm, {
            array: [this.view.btnSubmit, this.view.btnClear],
          })
        },
      },
