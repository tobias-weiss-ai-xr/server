...
define([
  ...
  DE.Controllers.LeftMenu = Backbone.Controller.extend(
    _.extend(
      {
        ...
        initialize: function () {
          this.addListeners({
            ...
            FileMenu: {
              ...
              "saveas:format": _.bind(function (menu, format, ext) {
                ...
                } else this.clickSaveAsFormat(menu, format, ext)
              }, this),
          ...
          Common.NotificationCenter.on(
            ...
            _.bind(function () {
              if (!this.leftMenu.panelHistory.isVisible()) this.clickMenuFileItem(null, "history")
            }, this),
          )
          Common.NotificationCenter.on(
            "protect:doclock",
        ...
        setApi: function (api) {
          ...
          if (this.mode.canCoAuthoring) {
            ...
            if (this.mode.canComments) {
              ...
              this.api.asc_registerCallback(
                "asc_onAddComments",
                _.bind(this.onApiAddComments, this),
              )
              const collection = this.getApplication().getCollection("Common.Collections.Comments")
              const resolved = Common.Utils.InternalSettings.get("de-settings-resolvedcomment")
              for (let i = 0; i < collection.length; ++i) {
                const comment = collection.at(i)
                if (
                  !comment.get("hide") &&
                  comment.get("userid") !== this.mode.user.id &&
                  comment.get("userid") !== "" &&
          ...
          this.leftMenu.setOptionsPanel(
            "advancedsearch",
            this.getApplication().getController("Search").getView("Common.Views.SearchPanel"),
          )
          return this
        },
        ...
        createDelayedElements: function () {
          /** coauthoring begin **/
          if (this.mode.canCoAuthoring) {
            this.leftMenu.btnComments[
              this.mode.canViewComments && !this.mode.isLightVersion ? "show" : "hide"
            ]()
            if (this.mode.canViewComments)
              this.leftMenu.setOptionsPanel(
                "comment",
                this.getApplication().getController("Common.Controllers.Comments").getView(),
              )
          ...
          if (this.mode.canUseHistory)
            this.leftMenu.setOptionsPanel(
              ...
              this.getApplication()
                .getController("Common.Controllers.History")
                .getView("Common.Views.History"),
            )

          this.leftMenu.setOptionsPanel(
            "navigation",
            this.getApplication().getController("Navigation").getView("Navigation"),
          )

          if (this.mode.canUseThumbnails) {
            this.leftMenu.setOptionsPanel(
              "thumbnails",
              this.getApplication().getController("PageThumbnails").getView("PageThumbnails"),
            )
          } else {
          ...
          switch (action) {
            ...
            case "edit":
              this.getApplication()
                ...
                .setStatusCaption(this.requestEditRightsText)
        ...
        _saveAsFormat: function (menu, format, ext, textParams, wopiPath) {
          const needDownload = !!ext
          const options = new Asc.asc_CDownloadOptions(format, needDownload)
          ...
          if (menu) {
            ...
            if (format === Asc.c_oAscFileType.TXT || format === Asc.c_oAscFileType.RTF) {
              Common.UI.warning({
                title: this.notcriticalErrorTitle,
                msg:
                  format === Asc.c_oAscFileType.TXT ? this.warnDownloadAs : this.warnDownloadAsRTF,
                buttons: ["ok", "cancel"],
                  ...
                  if (btn === "ok") {
                    ...
                    if (format === Asc.c_oAscFileType.TXT)
                      Common.NotificationCenter.trigger(
                        ...
                        this.api.asc_getAdvancedOptions(),
                        2,
                        options,
                      )
                    else this.api.asc_DownloadAs(options)
                    menu.hide()
                  }
                }, this),
              })
            } else if (
              format === Asc.c_oAscFileType.DOCX ||
              format === Asc.c_oAscFileType.DOCM ||
              format === Asc.c_oAscFileType.DOTX
            ) {
              if (
                !Common.Utils.InternalSettings.get("de-settings-compatible") &&
                !Common.localStorage.getBool("de-hide-save-compatible") &&
                this.api.asc_isCompatibilityMode()
              ) {
                Common.UI.warning({
                  closable: false,
                  width: 600,
        ...
        clickSaveAsFormat: function (menu, format, ext, wopiPath) {
          // ext isn't undefined for save copy as
          const fileType = this.getApplication().getController("Main").document.fileType
          ...
          if (/^pdf|xps|oxps|djvu$/.test(fileType)) {
            if (format === undefined) {
              ...
              format === Asc.c_oAscFileType.PDF ||
              format === Asc.c_oAscFileType.PDFA ||
              format === Asc.c_oAscFileType.JPG ||
              format === Asc.c_oAscFileType.PNG
            )
              this._saveAsFormat(menu, format, ext, undefined, wopiPath)
            else {
              if (format === Asc.c_oAscFileType.TXT || format === Asc.c_oAscFileType.RTF)
                // don't show message about pdf/xps/oxps
                this._saveAsFormat(
                  menu,
                  format,
                  ext,
                  new AscCommon.asc_CTextParams(Asc.c_oAscTextAssociation.PlainLine),
                  wopiPath,
                )
              else {
                Common.UI.warning({
          ...
          } else this._saveAsFormat(menu, format, ext, undefined, wopiPath)
        },
        ...
        applySettings: function (menu) {
          ...
          let fast_coauth = Common.Utils.InternalSettings.get("de-settings-coauthmode")
          /** coauthoring begin **/
          ...
          if (
            this.mode.canViewComments &&
            this.leftMenu.panelComments &&
            this.leftMenu.panelComments.isVisible()
          )
            value = resolved = true
          value ? this.api.asc_showComments(resolved) : this.api.asc_hideComments()
          this.getApplication()
            .getController("Common.Controllers.ReviewChanges")
            .commentsShowHide(value ? "show" : "hide")
          /** coauthoring end **/
          ...
          if (this.mode.isEdit) {
            if (this.mode.canChangeCoAuthoring || !fast_coauth) {
              // can change co-auth. mode or for strict mode
              value = Number.parseInt(Common.localStorage.getItem("de-settings-autosave"))
            ...
            if (Common.UI.FeaturesManager.canChange("spellcheck")) {
              ...
              const spprops = new AscCommon.CSpellCheckSettings()
            ...
            value = Number.parseInt(Common.localStorage.getItem("de-settings-paste-button"))
          ...
          /* update zoom */
          const newZoomValue = Common.localStorage.getItem("de-settings-zoom")
          const oldZoomValue = Common.Utils.InternalSettings.get("de-settings-zoom")
          const lastZoomValue = Common.Utils.InternalSettings.get("de-last-zoom")
          ...
          value = Number.parseInt(Common.localStorage.getItem("de-settings-numeral"))
        ...
        changeToolbarSaveState: function (state) {
          const btnSave = this.leftMenu.menuFile.getButton("save")
          btnSave?.setDisabled(state)
        },
        ...
        updatePreviewMode: function () {
          ...
          this.leftMenu.panelSearch?.setSearchMode(this.viewmode ? "no-replace" : "search")
        ...
        SetDisabled: function (disable, options) {
          ...
          if (!options || options.comments?.disable) this.leftMenu.btnComments.setDisabled(disable)
          if (!options || options.chat) this.leftMenu.btnChat.setDisabled(disable)
          if (!options || options.navigation?.disable)
        ...
        onApiAddComment: function (id, data) {
          ...
          if (
            data &&
            data.asc_getUserId() !== this.mode.user.id &&
            (resolved || !data.asc_getSolved()) &&
            AscCommon.UserInfoParser.canViewComment(data.asc_getUserName())
          )
        ...
        onApiAddComments: function (data) {
          ...
          for (let i = 0; i < data.length; ++i) {
            if (
              data[i].asc_getUserId() !== this.mode.user.id &&
              (resolved || !data[i].asc_getSolved()) &&
              AscCommon.UserInfoParser.canViewComment(data.asc_getUserName())
            ) {
              this.leftMenu.markCoauthOptions("comments")
              break
        ...
        onAppAddComment: function (sender) {
          if (this.api.can_AddQuotedComment() === false) {
            new Promise((resolve, reject) => {
              ...
              const ctrl = DE.getController("Common.Controllers.Comments")
        ...
        commentsShowHide: function (mode) {
          const value = Common.Utils.InternalSettings.get("de-settings-livecomment")
          const resolved = Common.Utils.InternalSettings.get("de-settings-resolvedcomment")

          if (!value || !resolved) {
            mode === "show"
              ? this.api.asc_showComments(true)
              : value
          ...
          } else Common.UI.TooltipManager.closeTip("commentFilter")
        ...
        aboutShowHide: function (value) {
          ...
          if (!value) {
            ...
            if (this.mode?.isPDFForm) {
              ...
              const rightmenu = this.getApplication().getController("RightMenu")
              if (rightmenu?.rightmenu?.signatureSettings) {
        ...
        onShortcut: function (s, e) {
          ...
          switch (s) {
            ...
            case "escape": {
              ...
              const statusbar = DE.getController("Statusbar")
        ...
        onMenuSearchBar: function (obj, show) {
          if (show) {
            const mode = this.mode.isEdit && !this.viewmode ? undefined : "no-replace"
        ...
        isCommentsVisible: function () {
          return this.leftMenu?.panelComments?.isVisible()
        },
        ...
        showHelp: function (src) {
          this.leftMenu?.showMenu("file:help", src)
        },
        ...
        warnDownloadAsRTF:
          "If you continue saving in this format some of the formatting might be lost.<br>Are you sure you want to continue?",
        txtUntitled: "Untitled",
        txtCompatible:
          "The document will be saved to the new format. It will allow to use all the editor features, but might affect the document layout.<br>Use the 'Compatibility' option of the advanced settings if you want to make the files compatible with older MS Word versions.",
        warnDownloadAsPdf:
          "Your {0} will be converted to an editable format. This may take a while. The resulting document will be optimized to allow you to edit the text, so it might not look exactly like the original {0}, especially if the original file contained lots of graphics.",
