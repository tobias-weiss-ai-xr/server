...
define([
  ...
  DE.Controllers.Navigation = Backbone.Controller.extend(
    _.extend(
      {
        models: [],
            ...
            Navigation: {
              show: function () {
                if (!this.canUseViwerNavigation) {
                  ...
                  if (me.panelNavigation?.viewNavigationList?.scroller)
        ...
        setApi: function (api) {
          ...
          this.api.asc_registerCallback(
            ...
            _.bind(this.updateNavigation, this),
          )
          this.api.asc_registerCallback(
            "asc_onDocumentOutlineCurrentPosition",
            _.bind(this.onChangeOutlinePosition, this),
          )
          this.api.asc_registerCallback(
            "asc_onDocumentOutlineUpdateAdd",
            _.bind(this.updateNavigation, this),
          )
          this.api.asc_registerCallback(
            "asc_onDocumentOutlineUpdateChange",
            _.bind(this.updateChangeNavigation, this),
          )
          this.api.asc_registerCallback(
          ...
          this.api.asc_registerCallback(
            "asc_onViewerBookmarksUpdate",
            _.bind(this.updateViewerNavigation, this),
          )
          return this
        },
        ...
        setMode: function (mode) {
          ...
          if (this.panelNavigation?.viewNavigationList) {
            this.panelNavigation.viewNavigationList.setEmptyText(
              this.mode.isEdit
                ? this.panelNavigation.txtEmpty
                : this.panelNavigation.txtEmptyViewer,
            )
        ...
        onAfterRender: function (panelNavigation) {
          ...
          panelNavigation.viewNavigationList.on(
            "item:contextmenu",
            _.bind(this.onItemContextMenu, this),
          )
          ...
          panelNavigation.navigationMenu.items[11].menu.on(
            "item:click",
            _.bind(this.onMenuLevelsItemClick, this),
          )
          panelNavigation.btnSettingsMenu.on(
            "item:click",
            _.bind(this.onMenuSettingsItemClick, this),
          )
          panelNavigation.btnSettingsMenu.items[2].menu.on(
            "item:click",
            _.bind(this.onMenuLevelsItemClick, this),
          )
          panelNavigation.btnSettingsMenu.items[4].menu.on(
            "item:click",
            _.bind(this.onMenuFontSizeClick, this),
          )
          ...
          viewport.hlayout.on("layout:resizedrag", () => {
            if (panelNavigation.viewNavigationList?.scroller)
        ...
        updateNavigation: function () {
          ...
          const count = this._navigationObject.get_ElementsCount()
          let prev_level = -1
          let header_level = -1
          const first_header = !this._navigationObject.isFirstItemNotHeader()
          const arr = []
          for (let i = 0; i < count; i++) {
            const level = this._navigationObject.get_Level(i)
            let hasParent = true
            if (level > prev_level && i > 0) arr[i - 1].set("hasSubItems", true)
            if (header_level < 0 || level <= header_level) {
              if (i > 0 || first_header) header_level = level
              hasParent = false
            }
            arr.push(
          ...
          const me = this
          const store = this.getApplication().getCollection("Navigation")
          ...
          function addToPanel() {
            ...
            me.timerUpdateId = setTimeout(() => {
              const added = arr.splice(0, 100)
              added.forEach((item) => {
                const idx = item.get("index")
        ...
        updateChangeNavigation: function (index) {
          ...
          const navList = this.getApplication().getCollection("Navigation")
          if (navList.length <= index) return

          const item = navList.at(index)
          if (
            item.get("level") !== this._navigationObject.get_Level(index) ||
            (index === 0 &&
              item.get("isNotHeader") !== this._navigationObject.isFirstItemNotHeader())
          ) {
        ...
        onChangeOutlinePosition: function (index) {
          if (index < this.panelNavigation.viewNavigationList.store.length)
            this.panelNavigation.viewNavigationList.scrollToRecord(
              this.panelNavigation.viewNavigationList.selectByIndex(index),
            )
          else this._currentPos = index
        },

        onItemContextMenu: function (picker, item, record, e) {
          let showPoint
          const menu = this.panelNavigation.navigationMenu
          if (menu.isVisible()) {
            menu.hide()
          }

          const parentOffset = Common.Utils.getOffset(this.panelNavigation.$el)
          const top = e.clientY * Common.Utils.zoom()
          ...
          const isNotHeader = record.get("isNotHeader")
          ...
          if (showPoint !== undefined) {
            let menuContainer = this.panelNavigation.$el.find("#menu-navigation-container")
            if (!menu.rendered) {
              if (menuContainer.length < 1) {
                menuContainer = $(
                  '<div id="menu-navigation-container" style="position: absolute; z-index: 10000;"><div class="dropdown-toggle" data-toggle="dropdown"></div></div>',
                  menu.id,
                )
        ...
        updateViewerNavigation: function (bookmarks) {
          ...
          if (this._viewerNavigationObject) {
            const count = this._viewerNavigationObject.length
            let prev_level = -1
            let header_level = -1
            const first_header = true //!this._navigationObject.isFirstItemNotHeader(),
            const arr = []
            for (let i = 0; i < count; i++) {
              const level = this._viewerNavigationObject[i].level - 1
              let hasParent = true
              if (level > prev_level && i > 0) arr[i - 1].set("hasSubItems", true)
              if (header_level < 0 || level <= header_level) {
                if (i > 0 || first_header) header_level = level
            ...
            if (this.panelNavigation?.viewNavigationList?.scroller)
