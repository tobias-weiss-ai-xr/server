...
define(["core", "documenteditor/main/app/view/RightMenu"], () => {
  DE.Controllers.RightMenu = Backbone.Controller.extend({
    ...
    onRightMenuAfterRender: function (rightMenu) {
      rightMenu.imageSettings.application =
        rightMenu.shapeSettings.application =
        rightMenu.textartSettings.application =
          this.getApplication()
      ...
      this._settings[Common.Utils.documentSettingsType.Form] = {
        panelId: "id-form-settings",
        panel: rightMenu.formSettings,
        btn: rightMenu.btnForm,
        hidden: 1,
        props: {},
        locked: false,
      }
    ...
    onRightMenuClick: function (menu, type, minimized, event) {
      if (!minimized && this.editMode) {
        ...
        const panel = this._settings[type].panel
        const props = this._settings[type].props
        if (props && panel) {
          panel.ChangeSettings.call(
            panel,
            type === Common.Utils.documentSettingsType.MailMerge ||
    ...
    onFocusObject: function (SelectedObjects, forceSignature, forceOpen) {
      if (!this.editMode && !forceSignature) return

      const open = this._initSettings
        ? !Common.localStorage.getBool(
            "de-hide-right-settings",
            this.rightmenu.defaultHideRightMenu,
          )
        : !!forceOpen
      this._initSettings = false

      let can_add_table = false
      let in_equation = false
      const needhide = true
      for (let i = 0; i < this._settings.length; i++) {
        if (
          i === Common.Utils.documentSettingsType.MailMerge ||
          i === Common.Utils.documentSettingsType.Signature
        )
          continue
        if (this._settings[i]) {
      ...
      for (i = 0; i < SelectedObjects.length; i++) {
        ...
        const eltype = SelectedObjects[i].get_ObjectType()
        const value = SelectedObjects[i].get_ObjectValue()
        let settingsType = this.getDocumentSettingsType(eltype)
        if (eltype === Asc.c_oAscTypeSelectElement.Math) in_equation = true
        else if (eltype === Asc.c_oAscTypeSelectElement.UnProtectedRegion) {
          //(unprotected region)
          unprotectedRegion = {
        ...
        if (
          ...
          settingsType >= this._settings.length ||
          this._settings[settingsType] === undefined
        )
          continue
        ...
        if (settingsType === Common.Utils.documentSettingsType.Image) {
          ...
          if (value.get_ChartProperties() !== null) {
            ...
            if (value.get_ShapeProperties().asc_getTextArtProperties()) {
              ...
              this._settings[Common.Utils.documentSettingsType.TextArt].locked =
                value.get_Locked() || content_locked || isProtected
            }
          }
          control_lock = control_lock || value.get_Locked()
        } else if (
          settingsType === Common.Utils.documentSettingsType.Paragraph &&
          !is_form?.get_Fixed()
        ) {
        ...
        if (settingsType === Common.Utils.documentSettingsType.Paragraph) {
          // unlock paraphaph in unprotected regions
          this._settings[settingsType].locked =
            value.get_Locked() ||
            content_locked ||
            (this._state.docProtection.isReadOnly && !unprotectedRegion.canEditPara) ||
            this._state.docProtection.isFormsOnly ||
            (this._state.docProtection.isCommentsOnly && !unprotectedRegion.canEditPara)
        } else
          this._settings[settingsType].locked = value.get_Locked() || content_locked || isProtected

        if (!this._settings[Common.Utils.documentSettingsType.MailMerge].locked)
          // lock MailMerge-InsertField, if at least one object is locked
          this._settings[Common.Utils.documentSettingsType.MailMerge].locked =
            value.get_Locked() || isProtected
        if (!this._settings[Common.Utils.documentSettingsType.Signature].locked)
          // lock Signature, if at least one object is locked
      ...
      if (!this._settings[Common.Utils.documentSettingsType.MailMerge].locked) {
        // disable MailMerge-InsertField when disable btnInsertTable
        this._settings[Common.Utils.documentSettingsType.MailMerge].locked =
          !can_add_table || in_equation
      }

      let lastactive = -1
      let currentactive
      let priorityactive = -1
      const activePane = this.rightmenu.GetActivePane()
      for (i = 0; i < this._settings.length; i++) {
        const pnl = this._settings[i]
        ...
        if (pnl.hidden) {
          ...
          if (activePane === pnl.panelId) currentactive = -1
        } else {
          if (pnl.btn.isDisabled()) {
          ...
          if (pnl.needShow) {
            pnl.needShow = false
            priorityactive = i
          } else if (activePane === pnl.panelId) currentactive = i
          pnl.panel.setLocked(pnl.locked)
        }
      }
      if (!this._settings[Common.Utils.documentSettingsType.MailMerge].hidden)
        this._settings[Common.Utils.documentSettingsType.MailMerge].panel.setLocked(
          this._settings[Common.Utils.documentSettingsType.MailMerge].locked,
        )
      ...
      if (!this.rightmenu.minimizedMode || open) {
        ...
        if (
          priorityactive < 0 &&
          !this._settings[Common.Utils.documentSettingsType.Form].hidden &&
          (!this._settings[Common.Utils.documentSettingsType.Paragraph].hidden ||
            !this._settings[Common.Utils.documentSettingsType.Image].hidden ||
            !this._settings[Common.Utils.documentSettingsType.Shape].hidden)
        ) {
          const imageactive = Common.Utils.InternalSettings.get("de-rightpanel-active-image") || 0
          const shapeactive = Common.Utils.InternalSettings.get("de-rightpanel-active-shape") || 0
          const paraactive = Common.Utils.InternalSettings.get("de-rightpanel-active-para") || 0
          const formactive = Common.Utils.InternalSettings.get("de-rightpanel-active-form") || 0

          if (!this._settings[Common.Utils.documentSettingsType.Paragraph].hidden) {
            priorityactive =
              formactive > paraactive
                ? Common.Utils.documentSettingsType.Form
                : Common.Utils.documentSettingsType.Paragraph
          } else if (
            !this._settings[Common.Utils.documentSettingsType.Paragraph].Image ||
            !this._settings[Common.Utils.documentSettingsType.Shape].hidden
          ) {
            ...
            else priorityactive = Common.Utils.documentSettingsType.Image
          }
        }
        ...
        if (priorityactive > -1) active = priorityactive
          ...
          forceSignature &&
          !this._settings[Common.Utils.documentSettingsType.Signature].hidden
        )
          active = Common.Utils.documentSettingsType.Signature
        else if (
          !this.rightmenu.GetActivePluginPane() &&
          !this._settings[Common.Utils.documentSettingsType.MailMerge].hidden
        )
          active = Common.Utils.documentSettingsType.MailMerge

        if (active === undefined && open && lastactive >= 0) active = lastactive

        if (active !== undefined) {
          this.rightmenu.SetActivePane(active, open)
          if (active === Common.Utils.documentSettingsType.Form)
            this._settings[active].panel.ChangeSettings.call(
              this._settings[active].panel,
              this._settings[active].props,
              isShape,
            )
          else if (
            active !== Common.Utils.documentSettingsType.MailMerge &&
          ...
          else this._settings[active].panel.ChangeSettings.call(this._settings[active].panel)
          active !== currentactive && this.rightmenu.updateScroller()
        } else if (activePane) {
          // lock active pane if no selected objects (ex. drawing)
          for (let i = 0; i < this._settings.length; i++) {
    ...
    updateMetricUnit: function () {
      ...
      this.rightmenu.formSettings?.updateMetricUnit()
    },
    ...
    createDelayedElements: function () {
      if (this.api) {
        ...
        this.api.asc_registerCallback(
          ...
          _.bind(this.onDoubleClickOnObject, this),
        )
        if (this.rightmenu.mergeSettings) {
          this.rightmenu.mergeSettings.setDocumentName(
            this.getApplication()
    ...
    onDoubleClickOnObject: function (obj) {
      ...
      const eltype = obj.get_ObjectType()
      let settingsType = this.getDocumentSettingsType(eltype)
      ...
      const value = obj.get_ObjectValue()
      if (settingsType === Common.Utils.documentSettingsType.Image) {
        if (value.get_ChartProperties() !== null) {
          settingsType = Common.Utils.documentSettingsType.Chart
        } else if (value.get_ShapeProperties() !== null) {
      ...
      if (settingsType !== Common.Utils.documentSettingsType.Paragraph) {
        ...
        this._settings[settingsType].panel.ChangeSettings.call(
          this._settings[settingsType].panel,
          this._settings[settingsType].props,
        )
        this.rightmenu.updateScroller()
      }
    },
    ...
    onStartMailMerge: function () {
      const type = Common.Utils.documentSettingsType.MailMerge
    ...
    onError: function (id, level, errData) {
      if (id === Asc.c_oAscError.ID.MailMergeLoadFile) {
        ...
        this.rightmenu.setDisabledMoreMenuItem(
          this._settings[Common.Utils.documentSettingsType.MailMerge].btn,
          true,
        )
        const selectedElements = this.api.getSelectedElements()
        if (selectedElements.length > 0) this.onFocusObject(selectedElements)
      }
    },
    ...
    onApiUpdateSignatures: function (valid, requested) {
      ...
      const disabled = (!valid || valid.length < 1) && (!requested || requested.length < 1)
      const type = Common.Utils.documentSettingsType.Signature
      ...
      this._settings[type].panel.setLocked(this._settings[type].locked)
      this._settings[type].panel.setProtected(
    ...
    SetDisabled: function (disabled, allowMerge, allowSignature) {
      ...
      if (this.rightmenu?.paragraphSettings) {
        ...
        this.rightmenu.formSettings?.disableControls(disabled)
        ...
        if (disabled) {
          ...
          this.rightmenu.btnForm?.setDisabled(disabled)
          ...
          if (selectedElements.length > 0)
            this.onFocusObject(
              selectedElements,
              false,
              !Common.Utils.InternalSettings.get("de-hide-right-settings") && // user didn't close panel
                !Common.Utils.InternalSettings.get("de-hidden-rightmenu"),
            ) // user didn't hide right menu
        }
      }
    },

    getDocumentSettingsType: (type) => {
      switch (type) {
        ...
        if (status) {
          ...
          if (selectedElements.length > 0)
            this.onFocusObject(
              selectedElements,
              false,
              !Common.Utils.InternalSettings.get("de-hide-right-settings"),
            )
        } else {
          this.rightmenu.signatureSettings?.hideSignatureTooltip()
        }
    ...
    onRightMenuOpen: function (type) {
      if (
        this._settings[type] === undefined ||
        this._settings[type].hidden ||
        this._settings[type].btn.isDisabled() ||
        this._settings[type].panelId === this.rightmenu.GetActivePane()
      )
        return

      this.tryToShowRightMenu()
      this.rightmenu.SetActivePane(type, true)
      this._settings[type].panel.ChangeSettings.call(
        this._settings[type].panel,
        this._settings[type].props,
      )
      this.rightmenu.updateScroller()
    },
    ...
    tryToShowRightMenu: function () {
      if (
        this.rightmenu?.mode &&
        (!this.rightmenu.mode.canBrandingExt ||
          !this.rightmenu.mode.customization ||
          this.rightmenu.mode.customization.rightMenu !== false) &&
