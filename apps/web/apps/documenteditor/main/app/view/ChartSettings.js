...
define([
  ...
  DE.Views.ChartSettings = Backbone.View.extend(
    _.extend(
      {
        ...
        setApi: function (api) {
          ...
          if (this.api) {
            this.api.asc_registerCallback(
              ...
              _.bind(this._ChartWrapStyleChanged, this),
            )
            this.api.asc_registerCallback(
            ...
            this.api.asc_registerCallback(
              "asc_onStartUpdateExternalReference",
              _.bind(this.onStartUpdateExternalReference, this),
            )
          }
          return this
        },
        ...
        ChangeSettings: function (props) {
          ...
          if (props?.get_ChartProperties()) {
            ...
            const externalRef = this.chartProps.getExternalReference()
            ...
            const text = externalRef ? (externalRef.asc_getSource() || "").replace(/%20/g, " ") : ""
            this.linkExternalSrc.text(text)
            this.OpenLinkSettings.toggleClass(
              "settings-hidden",
              !text || !(this.mode.canRequestOpen || this.mode.isOffline),
            )
            ...
            if (this._state.SeveralCharts && value) {
              ...
              const type = this.chartProps.getType()
              if (this._state.ChartType !== type) {
                const record = this.mnuChartTypePicker
                  ? this.mnuChartTypePicker.store.findWhere({ type: type })
                  : null
            ...
            const props3d = this.chartProps ? this.chartProps.getView3d() : null
            ...
            if (props3d) {
              ...
              if (
                ((this._state.X === undefined || value === undefined) && this._state.X !== value) ||
              ...
              if (
                ((this._state.Y === undefined || value === undefined) && this._state.Y !== value) ||
          ...
          this.btnChartType = new Common.UI.Button({
            ...
            menu: new Common.UI.Menu({
              ...
              items: [
                {
                  template: _.template(
                    '<div id="id-chart-menu-type" class="menu-insertchart" ></div>',
                  ),
                },
              ],
            }),
          ...
          this.chRightAngle.on(
            ...
            _.bind(function (field, newValue, oldValue, eOpts) {
              if (this.api) {
                ...
                if (this.chartProps) {
                  const props = new Asc.asc_CImgProperty()
                  const oView3D = this.chartProps.getView3d()
                  if (oView3D) {
        ...
        createChartTypeMenu: function () {
          ...
          const record = this.mnuChartTypePicker.store.findWhere({ type: this._state.ChartType })
          this.mnuChartTypePicker.selectRecord(record, true)
        ...
        onUpdateData: function () {
          if (!Common.Controllers.LaunchController.isScriptLoaded()) return
          Common.NotificationCenter.trigger("data:updatereferences", [
            this.chartProps.getExternalReference(),
          ])
        },
          ...
          if (this.api && !this._locked) {
            ...
            if (selectedElements && selectedElements.length > 0) {
              ...
              for (let i = selectedElements.length - 1; i >= 0; i--) {
                ...
                const isChart = !!elValue?.get_ChartProperties()
                if (Asc.c_oAscTypeSelectElement.Image === elType) {
                  new DE.Views.ImageSettingsAdvanced({
                    imageProps: elValue,
                    chartSettings: isChart ? this.api.asc_getChartSettings() : null,
        ...
        onSelectType: function (btn, picker, itemView, record) {
          ...
          if (this.api && !this._noApply && this.chartProps) {
            ...
            if (isCombo && this.chartProps.getSeries().length < 2) {
              ...
              this.mnuChartTypePicker.selectRecord(
                this.mnuChartTypePicker.store.findWhere({ type: this.chartProps.getType() }),
                true,
              )
            } else {
        ...
        onStartUpdateExternalReference: function (status) {
          ...
          const externalRef = this.chartProps.getExternalReference()
          this.btnEditData.setDisabled(
            this._locked || (externalRef && this._state.isUpdatingReference),
          )
          this.btnUpdateData.setDisabled(this._locked || this._state.isUpdatingReference)
          this.linkExternalSrc.toggleClass(
            "disabled",
            this._locked || !!this._state.isUpdatingReference,
          )
        },
        ...
        disableControls: function (disable) {
          ...
          if (this._state.DisabledControls !== disable) {
            ...
            this.linkExternalSrc.toggleClass(
              "disabled",
              disable || !!this._state.isUpdatingReference,
            )
          }
        },
