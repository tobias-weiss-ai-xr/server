...
define([
  ...
  DE.Views.ControlSettingsDialog = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 310,
          ...
          _.extend(
            this.options,
            {
              ...
              items: [
                ...
                {
                  panelId: "id-adv-control-settings-additional",
                  panelCaption: this.textAdditional,
                },
              ],
        ...
        render: function () {
          ...
          this.textareaHelp.keydown((event) => {
            if (event.keyCode === Common.UI.Keys.RETURN) {
              event.stopPropagation()
            }
            this.isHelpChanged = true
          })
        ...
        getFocusedComponents: function () {
          return this.btnsCategory
            .concat([
              ...
              this.cmbShow,
              this.btnColor,
              this.btnApplyAll, // 0 tab
              this.chLockDelete,
              this.chLockEdit, // 1 tab
              this.list,
              this.btnAdd,
              this.btnChange,
              this.btnDelete,
        ...
        onCategoryClick: function (btn, index) {
          ...
          setTimeout(() => {
            if (index === 0) {
              ...
              this.chLockDelete.focus()
            } else if (index === 2) {
              this.list.focus()
            } else if (index === 3) this.txtDate.focus()
            else if (index === 4) this.btnEditChecked.focus()
          }, 100)
        },
        ...
        _setDefaults: function (props) {
          if (props) {
            ...
            this.chLockDelete.setValue(
              val === Asc.c_oAscSdtLockType.SdtContentLocked ||
                val === Asc.c_oAscSdtLockType.SdtLocked,
            )
            this.chLockEdit.setValue(
              val === Asc.c_oAscSdtLockType.SdtContentLocked ||
                val === Asc.c_oAscSdtLockType.ContentLocked,
            )
            ...
            if (
              ...
              this.btnsCategory[2].setCaption(
                type === Asc.c_oAscContentControlSpecificType.ComboBox
                  ? this.textCombobox
                  : this.textDropDown,
              )
            ...
            if (type === Asc.c_oAscContentControlSpecificType.DateTime) {
              ...
              if (specProps) {
                ...
                const lang = specProps.get_LangId() || this.options.controlLang
                if (lang) {
                ...
                const format = specProps.get_DateFormat()
                const rec = this.listFormats.store.findWhere({ format: format })
            ...
            if (type === Asc.c_oAscContentControlSpecificType.CheckBox) {
              ...
              if (specProps) {
                let code = specProps.get_CheckedSymbol()
                let font = specProps.get_CheckedFont()
            ...
            if (formTextPr) {
              const code = formTextPr.get_PlaceHolderSymbol()
              const font = formTextPr.get_PlaceHolderFont()
            ...
            if (
              ...
              // standart checkbox or picture
              this.txtPlaceholder.cmpEl?.closest("tr").hide()
            }
          }
        },
        ...
        getSettings: function () {
          ...
          if (this.btnColor.isAutoColor()) {
            ...
            let color = this.colors.getColor() || this.btnColor.color
            if (color) {
          ...
          let lock = Asc.c_oAscSdtLockType.Unlocked

          if (
            this.chLockDelete.getValue() === "checked" &&
            this.chLockEdit.getValue() === "checked"
          )
          ...
          if (
            ...
            this.type === Asc.c_oAscContentControlSpecificType.DropDownList
          ) {
            specProps =
              this.type === Asc.c_oAscContentControlSpecificType.ComboBox
                ? this.props.get_ComboBoxPr()
                : this.props.get_DropDownListPr()
            specProps.clear()
          ...
          if (this.type === Asc.c_oAscContentControlSpecificType.CheckBox) {
            if (this.checkedBox?.changed || this.uncheckedBox?.changed) {
          ...
          if (this.btnsCategory[6].isVisible()) {
            const formPr = new AscCommon.CSdtFormPr()
            ...
            if (this.isHelpChanged) formPr.put_HelpText(this.textareaHelp.val())

            if (
              this.type === Asc.c_oAscContentControlSpecificType.CheckBox &&
              !this.$window.find(".group-key").hasClass("hidden")
            ) {
              specProps = this.props.get_CheckBoxPr()
              if (specProps) {
          ...
          if (this.btnsCategory[5].isVisible()) {
            ...
            if (this.spnWidth.getValue()) {
              ...
              formTextPr.put_Width(
                value <= 0
                  ? 0
                  : Number.parseInt((Common.Utils.Metric.fnRecalcToMM(value) * 72 * 20) / 25.4),
              )
            } else formTextPr.put_Width(0)

            if (this.placeholder?.changed) {
            ...
            if (checked) formTextPr.put_MaxCharacters(this.spnMaxChars.getNumberValue() || 10)
        ...
        disableListButtons: function (disabled) {
          if (disabled === undefined) disabled = !this.list.getSelectedRec()
        ...
        onAddItem: function () {
          const win = new DE.Views.EditListItemDialog({
            store: this.list.store,
            handler: (result, name, value) => {
              if (result === "ok") {
                const rec = this.list.store.add({
                  value: value,
                  name: name,
                })
                if (rec) {
        ...
        updateFormats: function (lang) {
          if (this.datetime) {
            ...
            const rec = this.listFormats.getSelectedRec()
            this.listFormats.scrollToRecord(rec)
        ...
        onEditCheckbox: function (checked) {
          if (this.api) {
            const props = checked ? this.checkedBox : this.uncheckedBox
            const cmp = checked ? this.btnEditChecked : this.btnEditUnchecked
        ...
        onEditPlaceholder: function () {
          if (this.api) {
            const props = this.placeholder
            const cmp = this.btnEditPlaceholder
            const handler = (dlg, result, settings) => {
              if (result === "ok") {
                props.changed = true
                props.code = settings.code
