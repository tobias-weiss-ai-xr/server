 ...
 */
define([], () => {
  DE.Views.CrossReferenceDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 400,
        ...
        render: function () {
          ...
          this.refList = new Common.UI.ListView({
            ...
            itemTemplate: _.template(
              '<div id="<%= id %>" class="list-item" style="overflow: hidden; text-overflow: ellipsis;white-space: pre;"><%= Common.Utils.String.htmlEncode(value) %></div>',
            ),
          })
          this.refList
        ...
        afterRender: function () {
          this._setDefaults()
          const onApiEndCalculate = () => {
            this.refreshReferences(this.cmbType.getSelectedRecord(), true)
          }
          this.api.asc_registerCallback("asc_onEndCalculate", onApiEndCalculate)
          this.on("close", (obj) => {
            this.api.asc_unregisterCallback("asc_onEndCalculate", onApiEndCalculate)
          })
        },
        ...
        _handleInput: function (state, fromButton) {
          ...
          if (state === "ok") {
            if (
              ...
              /area_id/.test(document.activeElement.id)
            ) {
              return
            }
        ...
        insertReference: function () {
          const record = this.refList.getSelectedRec()
          const typeRec = this.cmbType.getSelectedRecord()
          const type = typeRec.type === 1 || typeRec.value > 4 ? 5 : typeRec.value
          const reftype = this.cmbReference.getValue()
          const link = this.chInsertAs.getValue() === "checked"
          const below = this.chBelowAbove.getValue() === "checked"
          const separator =
            this.chSeparator.getValue() === "checked" ? this.inputSeparator.getValue() : undefined

          switch (type) {
            case 0: // paragraph
            case 1: // heading
              this.api.asc_AddCrossRefToParagraph(
                record.get("para"),
                reftype,
                link,
                below,
              ...
              break
            case 2: // bookmark
              this.api.asc_AddCrossRefToBookmark(
                record.get("value"),
                reftype,
                link,
                below,
            ...
            case 5: // caption
              if (
                reftype === Asc.c_oAscDocumentRefenceToType.OnlyCaptionText &&
                record.get("para").asc_canAddRefToCaptionText(typeRec.displayValue) === false
              ) {
                Common.UI.warning({
                  msg: this.textEmpty,
                })
              } else
                this.api.asc_AddCrossRefToCaption(
                  typeRec.displayValue,
                  record.get("para"),
                  reftype,
                  link,
                  below,
        ...
        refreshReferenceTypes: function (record, currentRef) {
          ...
          if (record.type === 1 || record.value > 4) {
            ...
            arr = [
              ...
              {
                value: Asc.c_oAscDocumentRefenceToType.OnlyLabelAndNumber,
                displayValue: this.textLabelNum,
              },
              {
                value: Asc.c_oAscDocumentRefenceToType.OnlyCaptionText,
                displayValue: this.textOnlyCaption,
              },
              { value: Asc.c_oAscDocumentRefenceToType.PageNum, displayValue: this.textPageNum },
              {
                value: Asc.c_oAscDocumentRefenceToType.AboveBelow,
                displayValue: this.textAboveBelow,
              },
            ]
          } else {
            switch (record.value) {
              case 0: // paragraph
                arr = [
                  {
                    value: Asc.c_oAscDocumentRefenceToType.PageNum,
                    displayValue: this.textPageNum,
                  },
                  {
                    value: Asc.c_oAscDocumentRefenceToType.ParaNum,
                    displayValue: this.textParaNum,
                  },
                  {
                    value: Asc.c_oAscDocumentRefenceToType.ParaNumNoContext,
                    displayValue: this.textParaNumNo,
                  },
                  {
                    value: Asc.c_oAscDocumentRefenceToType.ParaNumFullContex,
                    displayValue: this.textParaNumFull,
                  },
              ...
              case 1: // heading
                arr = [
                  {
                    value: Asc.c_oAscDocumentRefenceToType.Text,
                    displayValue: this.textHeadingText,
                  },
                  {
                    value: Asc.c_oAscDocumentRefenceToType.PageNum,
                    displayValue: this.textPageNum,
                  ...
                  {
                    value: Asc.c_oAscDocumentRefenceToType.ParaNum,
                    displayValue: this.textHeadingNum,
                  },
                  {
                    value: Asc.c_oAscDocumentRefenceToType.ParaNumNoContext,
                    displayValue: this.textHeadingNumNo,
                  },
                  {
                    value: Asc.c_oAscDocumentRefenceToType.ParaNumFullContex,
                    displayValue: this.textHeadingNumFull,
                  },
                  {
                    value: Asc.c_oAscDocumentRefenceToType.AboveBelow,
                    displayValue: this.textAboveBelow,
              ...
              case 2: // bookmark
                arr = [
                  {
                    value: Asc.c_oAscDocumentRefenceToType.Text,
                    displayValue: this.textBookmarkText,
                  },
                  {
                    value: Asc.c_oAscDocumentRefenceToType.PageNum,
                    displayValue: this.textPageNum,
              ...
              case 3: // note
                arr = [
                  {
                    value: Asc.c_oAscDocumentRefenceToType.NoteNumber,
                    displayValue: this.textNoteNum,
                  },
                  {
                    value: Asc.c_oAscDocumentRefenceToType.PageNum,
                    displayValue: this.textPageNum,
                  ...
                  {
                    value: Asc.c_oAscDocumentRefenceToType.NoteNumberFormatted,
                    displayValue: this.textNoteNumForm,
                  },
                ]
              ...
              case 4: // end note
                arr = [
                  {
                    value: Asc.c_oAscDocumentRefenceToType.NoteNumber,
                    displayValue: this.textEndNoteNum,
                  },
                  {
                    value: Asc.c_oAscDocumentRefenceToType.PageNum,
                    displayValue: this.textPageNum,
                  ...
                  {
                    value: Asc.c_oAscDocumentRefenceToType.NoteNumberFormatted,
                    displayValue: this.textEndNoteNumForm,
                  },
                ]
          ...
          const rec = this.cmbReference.store.findWhere({ value: currentRef })
        ...
        refreshReferences: function (record, reselect) {
          ...
          const store = this.refList.store
          const type = record.type === 1 || record.value > 4 ? 5 : record.value
          const arr = []
          let props
          const oldlength = store.length
          const oldidx = _.indexOf(store.models, this.refList.getSelectedRec())
          ...
          switch (type) {
            ...
            case 4: // endnote
              ...
              break
            case 5: // caption
              props = this.api.asc_GetAllCaptionParagraphs(
                this.cmbType.getSelectedRecord().displayValue,
              )
              break
          }
          if (type === 2) {
            // bookmark
            const count = props.asc_GetCount()
            for (let i = 0; i < count; i++) {
              const name = props.asc_GetName(i)
          ...
          if (store.length > 0) {
            const rec =
              reselect && store.length === oldlength && oldidx >= 0 && oldidx < store.length
                ? store.at(oldidx)
                : store.at(0)
        ...
        onReferenceSelected: function (combo, record) {
          ...
          const refType = record.value
          const typeRec = this.cmbType.getSelectedRecord()
          const type = typeRec.type === 1 || typeRec.value > 4 ? 5 : typeRec.value
          let disable =
            type === 1 ||
            (type === 5 && refType !== Asc.c_oAscDocumentRefenceToType.PageNum) ||
            (type < 5 &&
              (refType === Asc.c_oAscDocumentRefenceToType.Text ||
                refType === Asc.c_oAscDocumentRefenceToType.AboveBelow))
          if (type === 1) this.chBelowAbove.setValue(false)
          this.chBelowAbove.setDisabled(disable)
          disable =
            !(type === 0 || type === 2) ||
            refType !== Asc.c_oAscDocumentRefenceToType.ParaNumFullContex
