...
define([], () => {
  DE.Views.DateTimeDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 350,
        ...
        render: function () {
          ...
          let value = Common.Utils.InternalSettings.get("de-date-auto-update")
          ...
          this.listFormats = new Common.UI.ListView({
            ...
            itemTemplate: _.template(
              '<div id="<%= id %>" class="list-item"><span dir="ltr"><%= value %></span></div>',
            ),
          })
            ...
            _.bind(function (btn, e) {
              ...
              Common.UI.warning({
                msg: Common.Utils.String.format(
                  this.confirmDefault,
                  Common.util.LanguageInfo.getLocalLanguageName(this.cmbLang.getValue())[1],
                  rec ? rec.get("value") : "",
                ),
                buttons: ["yes", "no"],
                primary: "yes",
                callback: _.bind(function (btn) {
                  if (btn === "yes") {
                    ...
                    const arr = []
                    for (const name in this.defaultFormats) {
                      if (name) {
          ...
          if (
            this.chUpdate.$el.outerWidth() + this.btnDefault.$el.outerWidth() >
            this.$window.find(".box").width()
          ) {
        ...
        afterRender: function () {
          const value = Common.Utils.InternalSettings.get("de-settings-datetime-default")
          const arr = value ? JSON.parse(value) : []
          this.defaultFormats = []
          arr.forEach((item) => {
            if (item.lang) this.defaultFormats[Number.parseInt(item.lang)] = item.format
          })
        ...
        getFocusedComponents: function () {
          return [this.cmbLang, this.listFormats, this.chUpdate, this.btnDefault].concat(
        ...
        _setDefaults: function () {
          ...
          if (this.lang) {
            let item = this.cmbLang.store.findWhere({ value: this.lang })
        ...
        updateFormats: function (lang) {
          ...
          const formats = this.props.get_FormatsExamples()
          const arr = []
          const store = this.listFormats.store
          ...
          store.reset(arr)
          const format = this.defaultFormats[lang]
          let rec = format ? store.findWhere({ format: format }) : null
          !rec && (rec = store.at(0))
