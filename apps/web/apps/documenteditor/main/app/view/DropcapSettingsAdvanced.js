...
define([
  ...
  DE.Views.DropcapSettingsAdvanced = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 320,
        ...
        render: function () {
          ...
          this.btnBorderColor.on(
            "color:select",
            _.bind(function (btn, color) {
              this.tableStyler.setVirtualBorderColor(
          ...
          this.btnBackColor.on(
            ...
            _.bind((btn, color) => {
              ...
              if (me._changedProps) {
                if (
                  me._changedProps.get_Shade() === undefined ||
                  me._changedProps.get_Shade() === null
                ) {
          ...
          this.spnMarginTop = new Common.UI.MetricSpinner({
            ...
            _.bind(function (field, newValue, oldValue) {
              if (!this._noApply) {
                if (_.isUndefined(this.Margins)) this.Margins = {}
          ...
          this.cmbHeight = new Common.UI.ComboBox({
            ...
            _.bind(function (combo, record) {
              if (me._changedProps) {
                ...
                me._changedProps.put_HRule(
                  record.value === 0
                    ? Asc.linerule_Auto
                    : record.value === 1
                      ? Asc.linerule_Exact
                      : Asc.linerule_AtLeast,
                )
                if (record.value > 0)
                  this._changedProps.put_H(
          ...
          this.spnHeight = new Common.UI.MetricSpinner({
            ...
            _.bind((field, newValue, oldValue) => {
              if (me._changedProps) {
                let type = Asc.linerule_Auto
                if (me.cmbHeight.getValue() === me._arrHeight[1].value) type = Asc.linerule_Exact
              ...
              _.bind((combo, record) => {
                if (me._changedProps) {
                  if (combo.getSelectedRecord()) {
                    ...
                    const number = Common.Utils.String.parseFloat(record.value)
                ...
                if (me._changedProps) {
                  if (combo.getSelectedRecord()) {
                    ...
                    if (!Number.isNaN(number)) {
                      ...
                      me._changedProps.put_Y(
                        Common.Utils.Metric.fnRecalcToMM(
                          Common.Utils.String.parseFloat(record.value),
                        ),
                      )
                    }
                  }
          ...
          this.chMove = new Common.UI.CheckBox({
            ...
            _.bind(function (checkbox, state) {
              if (me._changedProps) {
                const rec = this.cmbVRelative.store.at(state === "checked" ? 2 : 1)
        ...
        afterRender: function () {
          ...
          if (this.isFrame) this.setInnerHeight(415)
        ...
        onShowDialog: function (dlg) {
          if (!this.isFrame && this.btnNone.pressed) this._DisableElem(Asc.c_oAscDropCap.None)
        ...
        _setDefaults: function (props) {
          if (props) {
            ...
            const frame_props = props.get_FramePr()
            if (frame_props) {
              let value
              if (this.isFrame) {
                value = frame_props.get_W()
                this.cmbWidth.setValue(
                  value === undefined ? this._arrWidth[0].value : this._arrWidth[1].value,
                )
                this.spnWidth.setValue(
                  value !== undefined ? Common.Utils.Metric.fnRecalcFromMM(value) : "",
                )

                value = frame_props.get_HRule()
                if (value !== undefined) {
                  this.cmbHeight.setValue(
                    value === Asc.linerule_Exact
                      ? this._arrHeight[1].value
                      : this._arrHeight[2].value,
                  )
                ...
                this.spnX.setValue(
                  value !== undefined ? Common.Utils.Metric.fnRecalcFromMM(value) : 0,
                )
                ...
                if (value !== undefined) {
                  ...
                  this.cmbHAlign.setValue(
                    `${Common.Utils.Metric.fnRecalcFromMM(value !== undefined ? value : 0)} ${Common.Utils.Metric.getCurrentMetricName()}`,
                  )
                }

                value = frame_props.get_VAnchor()
                for (let i = 0; i < this._arrVRelative.length; i++) {
                ...
                if (value !== undefined) {
                  for (let i = 0; i < this._arrVAlign.length; i++) {
                  ...
                  this.cmbVAlign.setValue(
                    `${Common.Utils.Metric.fnRecalcFromMM(value !== undefined ? value : 0)} ${Common.Utils.Metric.getCurrentMetricName()}`,
                  )
                }

                value = frame_props.get_Wrap()
                if (value === false) this.btnFrameInline.toggle(true, false)
                else this.btnFrameFlow.toggle(true, false)
              } else {
                this.spnRowHeight.setValue(
                  frame_props.get_Lines() !== null ? frame_props.get_Lines() : "",
                )
                this.numDistance.setValue(
                  frame_props.get_HSpace() !== null
                    ? Common.Utils.Metric.fnRecalcFromMM(frame_props.get_HSpace())
                    : "",
                )
                value = frame_props.get_DropCap()
                if (value === Asc.c_oAscDropCap.Drop) this.btnInText.toggle(true, false)
                else if (value === Asc.c_oAscDropCap.Margin) this.btnInMargin.toggle(true, false)
                else this.btnNone.toggle(true, false)

                value = frame_props.get_FontFamily()
                if (value) {
                  const rec = this.cmbFonts.store.findWhere({ name: value.get_Name() })
                  if (rec) this.cmbFonts.setValue(rec.get("name"))
                  else this.cmbFonts.setRawValue(value.get_Name())
                }
              }
              ...
              const shd = frame_props.get_Shade()
        ...
        _DisableElem: function (btnId) {
          let disabled = btnId === Asc.c_oAscDropCap.None || btnId === c_oAscFrameWrap.None

          _.each(this.btnsCategory, (btn) => {
            if (
              btn.options.contentTarget === "id-adv-dropcap-borders" ||
        ...
        _UpdateBorders: function () {
          ...
          if (this.Borders.get_Between() !== null) {
            for (let i = 0; i < this.tableStyler.rows - 1; i++) {
              this._UpdateCellBorder(
                this.Borders.get_Between(),
                this.tableStyler.getCellBorder(i, -1),
              )
            }
          }
        ...
        _UpdateBorder: function (BorderParam, borderName) {
          if (null !== BorderParam && undefined !== BorderParam) {
            if (
              ...
              this.tableStyler.setBordersSize(
                borderName,
                this._BorderPt2Px((BorderParam.get_Size() * 72) / 25.4),
              )
              this.tableStyler.setBordersColor(
                borderName,
                `rgb(${BorderParam.get_Color().get_r()},${BorderParam.get_Color().get_g()},${BorderParam.get_Color().get_b()})`,
              )
            } else this.tableStyler.setBordersSize(borderName, 0)
          } else {
