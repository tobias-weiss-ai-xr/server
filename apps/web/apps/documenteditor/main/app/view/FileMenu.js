...
define([
  ...
  DE.Views.FileMenu = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#file-menu-panel",
        rendered: false,
        ...
        applyMode: function () {
          ...
          if (!this.panels) {
            this.panels = {
              opts: new DE.Views.FileMenuPanels.Settings({ menu: this }).render(
                this.$el.find("#panel-settings"),
              ),
              info: new DE.Views.FileMenuPanels.DocumentInfo({ menu: this }).render(
                this.$el.find("#panel-info"),
              ),
              rights: new DE.Views.FileMenuPanels.DocumentRights({ menu: this }).render(
          ...
          const canEdit =
            this.mode.canRequestEditRights &&
            ((!this.mode.isEdit && this.mode.canEdit) ||
              (this.mode.isPDFForm && this.mode.canFillForms && this.mode.isRestrictedEdit))
          ...
          if (this.mode.isSignatureSupport || this.mode.isPasswordSupport) {
            !this.panels.protect &&
              (this.panels.protect = new DE.Views.FileMenuPanels.ProtectDoc({
                menu: this,
              }).render())
          ...
          if (this.mode.canDownload) {
            !this.panels.saveas &&
              (this.panels.saveas = new DE.Views.FileMenuPanels.ViewSaveAs({
                menu: this,
                fileType: this.document ? this.document.fileType : undefined,
                mode: this.mode,
              }).render())
          ...
          if (this.mode.canDownload && (this.mode.canRequestSaveAs || this.mode.saveAsUrl)) {
            !this.panels["save-copy"] &&
              (this.panels["save-copy"] = new DE.Views.FileMenuPanels.ViewSaveCopy({
                menu: this,
                fileType: this.document ? this.document.fileType : undefined,
                mode: this.mode,
              }).render())
          ...
          if (this.mode.canPreviewPrint) {
            const printPanel = DE.getController("Print").getView("PrintWithPreview")
