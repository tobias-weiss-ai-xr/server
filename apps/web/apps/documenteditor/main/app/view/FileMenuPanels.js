...
define([], () => {
  ...
  DE.Views.FileMenuPanels.ViewSaveAs = Common.UI.BaseView.extend({
    ...
    onFormatClick: function (e) {
      ...
      if (!_.isUndefined(type) && this.menu) {
        this.menu.fireEvent("saveas:format", [
          this.menu,
          type.value ? Number.parseInt(type.value) : undefined,
        ])
      }
    },
  ...
  DE.Views.FileMenuPanels.ViewSaveCopy = Common.UI.BaseView.extend({
    ...
    render: function () {
      if (/^pdf$/.test(this.fileType)) {
        ...
        this.formats[0].push({
          name: this.fileType.toUpperCase(),
          imgCls: this.fileType,
          type: "",
          ext: true,
        }) // original xps/oxps
      } else if (/^djvu$/.test(this.fileType)) {
  ...
  DE.Views.FileMenuPanels.Settings = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-settings",
        menu: undefined,
        ...
        template: _.template(
          [
            ...
            "</tr>",
            '<tr class="divider coauth changes-show"></tr>',
        ...
        render: function (node) {
          ...
          this.chSpell = new Common.UI.CheckBox({
            ...
            this.chIgnoreUppercase.setDisabled(field.getValue() !== "checked")
            this.chIgnoreNumbers.setDisabled(field.getValue() !== "checked")
          })
          ...
          this.chAutosave = new Common.UI.CheckBox({
            ...
            if (field.getValue() !== "checked" && this.rbCoAuthModeFast.getValue()) {
              ...
              this.onChangeCoAuthMode(this.rbCoAuthModeFast.getValue() ? 1 : 0)
            }
          })
          ...
          this.rbCoAuthModeFast.on("change", (field, newValue, eOpts) => {
            if (newValue) {
              this.chAutosave.setValue(1)
              this.onChangeCoAuthMode(1)
            }
          })
          ...
          this.rbCoAuthModeStrict.on("change", (field, newValue, eOpts) => {
            newValue && this.onChangeCoAuthMode(0)
          })
          ...
          this.cmbTheme = new Common.UI.ComboBox({
            ...
            this.chDarkMode.setDisabled(!Common.UI.Themes.isDarkTheme(record.value))
          })
        ...
        setMode: function (mode) {
          ...
          const fast_coauth = Common.Utils.InternalSettings.get("de-settings-coauthmode")
          ...
          $("tr.collaboration", this.el)[
            mode.canCoAuthoring || mode.canViewReview ? "show" : "hide"
          ]()
          $("tr.coauth", this.el)[mode.isEdit && mode.canCoAuthoring ? "show" : "hide"]()
          $("tr.coauth.changes-mode", this.el)[
            mode.isEdit && !mode.isOffline && mode.canCoAuthoring && mode.canChangeCoAuthoring
              ? "show"
              : "hide"
          ]()
          $("tr.coauth.changes-show", this.el)[
            mode.isEdit && !mode.isOffline && mode.canCoAuthoring ? "show" : "hide"
          ]()
          ...
          $("tr.spellcheck", this.el)[
            mode.isEdit && Common.UI.FeaturesManager.canChange("spellcheck") ? "show" : "hide"
          ]()
        ...
        updateSettings: function () {
          ...
          let value = Common.Utils.InternalSettings.get("de-settings-zoom")
          value =
            value !== null
          ...
          this.chResolvedComment.setValue(
            Common.Utils.InternalSettings.get("de-settings-resolvedcomment"),
          )
          ...
          value = Common.Utils.InternalSettings.get(
            fast_coauth ? "de-settings-showchanges-fast" : "de-settings-showchanges-strict",
          )
          ...
          if (Common.UI.FeaturesManager.canChange("spellcheck")) {
            ...
            this.chIgnoreUppercase.setValue(
              Common.Utils.InternalSettings.get("de-spellcheck-ignore-uppercase-words"),
            )
            this.chIgnoreNumbers.setValue(
              Common.Utils.InternalSettings.get("de-spellcheck-ignore-numbers-words"),
            )
          }
          this.chAlignGuides.setValue(
            Common.Utils.InternalSettings.get("de-settings-showsnaplines"),
          )
          this.chCompatible.setValue(Common.Utils.InternalSettings.get("de-settings-compatible"))

          item = this.cmbMacros.store.findWhere({
            value: Common.Utils.InternalSettings.get("de-macros-mode"),
          })
          ...
          this.chSmartSelection.setValue(
            Common.Utils.InternalSettings.get("de-settings-smart-selection"),
          )
          ...
          for (const t in Common.UI.Themes.map()) {
            data.push({
              value: t,
              displayValue: Common.UI.Themes.get(t).text,
              themeType: Common.UI.Themes.get(t).type,
            })
          }
        ...
        applySettings: function () {
          ...
          if (
            !this.chDarkMode.isDisabled() &&
            this.chDarkMode.isChecked() !== Common.UI.Themes.isContentThemeDark()
          )
          ...
          Common.Utils.InternalSettings.set(
            "de-settings-show-alt-hints",
            Common.localStorage.getBool("de-settings-show-alt-hints"),
          )
          ...
          if (this.mode.isEdit && !this.mode.isOffline && this.mode.canCoAuthoring) {
            ...
            Common.localStorage.setItem(
              this.rbCoAuthModeFast.getValue()
                ? "de-settings-showchanges-fast"
                : "de-settings-showchanges-strict",
              this.rbShowChangesNone.getValue()
                ? "none"
                : this.rbShowChangesLast.getValue()
                  ? "last"
                  : "all",
            )
          } else if (
          ...
          if (
            this.mode.isEdit &&
            (this.mode.canChangeCoAuthoring ||
              !Common.Utils.InternalSettings.get("de-settings-coauthmode"))
          )
          ...
          if (Common.UI.FeaturesManager.canChange("spellcheck") && this.mode.isEdit) {
            ...
            Common.localStorage.setBool(
              ...
              this.chIgnoreUppercase.isChecked(),
            )
            Common.localStorage.setBool(
              "de-spellcheck-ignore-numbers-words",
              this.chIgnoreNumbers.isChecked(),
            )
          }
          Common.localStorage.setItem(
            "de-settings-compatible",
            this.chCompatible.isChecked() ? 1 : 0,
          )
          Common.Utils.InternalSettings.set(
            "de-settings-compatible",
            this.chCompatible.isChecked() ? 1 : 0,
          ...
          if (this.mode.canViewReview) {
            const val = this.rbChangesTip.getValue()
          ...
          Common.localStorage.setItem(
            ...
            this.chSmartSelection.isChecked() ? 1 : 0,
          )
          ...
          if (Common.Utils.InternalSettings.get("de-settings-western-font-size") !== undefined) {
            const val = this.cmbFontSizeType.getValue()
        ...
        fillShowChanges: function (fastmode) {
          if (fastmode) this.rbShowChangesLast.$el.parent().hide()
          else this.rbShowChangesLast.$el.parent().show()
        },
        ...
        textOldVersions:
          "Make the files compatible with older MS Word versions when saved as DOCX, DOTX",
  ...
  DE.Views.FileMenuPanels.CreateNew = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-createnew",
        menu: undefined,
  ...
  DE.Views.FileMenuPanels.DocumentInfo = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-info",
        menu: undefined,
        ...
        updateInfo: function (doc) {
          ...
          const pdfProps = this.api ? this.api.asc_getPdfProps() : null
          ...
          } else if (pdfProps) this.updatePdfInfo(pdfProps)
        },
        ...
        updatePdfInfo: function (props) {
          ...
          if (props) {
            ...
            if (
              props.PageWidth &&
              props.PageHeight &&
              typeof props.PageWidth === "number" &&
              typeof props.PageHeight === "number"
            ) {
              let w = props.PageWidth
              let h = props.PageHeight
              switch (Common.Utils.Metric.getCurrentMetric()) {
                case Common.Utils.Metric.c_MetricUnits.cm:
                  w = Number.parseFloat(((w * 25.4) / 72000).toFixed(2))
                  h = Number.parseFloat(((h * 25.4) / 72000).toFixed(2))
                  break
                case Common.Utils.Metric.c_MetricUnits.pt:
                  w = Number.parseFloat((w / 100).toFixed(2))
                  h = Number.parseFloat((h / 100).toFixed(2))
                  break
                case Common.Utils.Metric.c_MetricUnits.inch:
                  w = Number.parseFloat((w / 7200).toFixed(2))
                  h = Number.parseFloat((h / 7200).toFixed(2))
                  break
              }
            ...
            } else this._ShowHideInfoItem(this.lblPageSize, false)
            ...
            if (value !== undefined) this.lblFastWV.text(value === true ? this.txtYes : this.txtNo)
        ...
        setApi: function (o) {
          ...
          this.api.asc_registerCallback(
            ...
            _.bind(this._onGetDocInfoStart, this),
          )
        ...
        _onGetDocInfoStart: function () {
          ...
          this.timerLoading = setTimeout(() => {
            this.lblStatPages.text(this.txtLoading)
            this.lblStatWords.text(this.txtLoading)
            this.lblStatParagraphs.text(this.txtLoading)
            this.lblStatSymbols.text(this.txtLoading)
            this.lblStatSpaces.text(this.txtLoading)
          }, 2000)
        },
        ...
        SetDisabled: function () {
          ...
          const disable =
            !this.mode.isEdit || this._state._locked || isProtected || this._state.disableEditing
        ...
        onAddPropertyClick: function () {
          new Common.Views.DocumentPropertyDialog({
            ...
            handler: (result, title, type, value) => {
              if (result === "ok") {
                this.api.asc_addCustomProperty(title, type, value)
                this.renderCustomProperties()
              }
            },
          }).show()
  ...
  DE.Views.FileMenuPanels.DocumentRights = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-rights",
        menu: undefined,
  ...
  DE.Views.FileMenuPanels.ProtectDoc = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-protect",
        menu: undefined,
        ...
        render: function () {
          ...
          const protection = DE.getController("Common.Controllers.Protection").getView()
        ...
        onView: function () {
          this.menu?.hide()
          DE.getController("RightMenu").rightmenu.SetActivePane(
        ...
        txtSigned:
          "Valid signatures has been added to the document. The document is protected from editing.",
        txtSignedInvalid:
          "Some of the digital signatures in document are invalid or could not be verified. The document is protected from editing.",
        txtRequestedSignatures: "This document needs to be signed.",
        notcriticalErrorTitle: "Warning",
        txtEditWarning:
          "Editing will remove the signatures from the document.<br>Are you sure you want to continue?",
        ...
        txtAddSignature:
          "Ensure the integrity of the document by adding an<br>invisible digital signature",
        txtAddedSignature:
          "Valid signatures have been added to the document.<br>The document is protected from editing.",
      },
      DE.Views.FileMenuPanels.ProtectDoc || {},
    ),
  )

  DE.Views.PrintWithPreview = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-print",
        menu: undefined,
        ...
        initialize: function (options) {
          ...
          this._defaultPaperSizeList = [
            ...
            {
              value: 13,
              displayValue: ["A4", "84,1", "118,9", "cm"],
              caption: "A0",
              size: [841, 1189],
            },
            {
              value: 14,
              displayValue: ["A4", "59,4", "84,1", "cm"],
              caption: "A1",
              size: [594, 841],
            },
            {
              value: 16,
              displayValue: ["A4", "42", "59,4", "cm"],
              caption: "A2",
              size: [420, 594],
            },
            {
              value: 17,
              displayValue: ["A4", "10,5", "14,8", "cm"],
              caption: "A6",
              size: [105, 148],
            },
          ]
        ...
        render: function (node) {
          ...
          this.cmbPaperMargins = new Common.UI.ComboBox({
            ...
            data: [
              {
                value: 0,
                displayValue: this.textMarginsNormal,
                size: /^(ca|us)$/i.test(Common.Utils.InternalSettings.get("de-config-region"))
                  ? [25.4, 25.4, 25.4, 25.4]
                  : [20, 30, 20, 15],
              },
        ...
        updateScroller: function () {
          if (this.scroller) {
            ...
            const scrolled =
              this.$el.height() <
              this.pnlTable.height() + 25 + this.$el.find(".main-header").outerHeight(true)
            this.pnlSettings.css("overflow", scrolled ? "hidden" : "visible")
            this.scroller.update()
          }
        },
