...
define(["common/main/lib/component/BaseView"], () => {
  DE.Views.FootnoteTip = Common.UI.BaseView.extend(
    _.extend(
      (() => {
        let tipEl
        ...
        return {
          ...
          render: function () {
            ...
            tipEl.find(".show-link label").on(
              "click",
              _.bind(function () {
                this.trigger("settingsclick")
              }, this),
            )
            this.btnPrev.on(
              "click",
          ...
          applyPlacement: function (target) {
            if (!target) target = { top: 0, left: 0, width: 1, height: 1 }
            const docHeight = Common.Utils.innerHeight()
            const elHeight = tipEl.height()
            const elWidth = tipEl.width()
            let placement = this.placement

            if (this.placement === "bottom") {
              if (target.top + target.height + elHeight > docHeight) placement = "top"
            } else if (this.placement === "top") {
              if (target.top - elHeight < 0) placement = "bottom"
            }
            ...
            placement === "top"
              ? tipEl.css({
                  top: "auto",
                  bottom: `${docHeight - target.top}px`,
                  left: `${target.left + target.width / 2 - elWidth / 2}px`,
                })
              : tipEl.css({
