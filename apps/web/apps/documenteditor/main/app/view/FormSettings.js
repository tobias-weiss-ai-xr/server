...
define([
  ...
  DE.Views.FormSettings = Backbone.View.extend(
    _.extend(
      {
        el: "#id-form-settings",
        ...
        render: function () {
          ...
          !Common.UI.FeaturesManager.isFeatureEnabled("roles", true) &&
            el.find("#form-combo-roles").closest("tr").hide().next("tr").hide()
        },
        ...
        createDelayedElements: function () {
          ...
          const showtip = () => {
            ...
            this.cmbKey.off("show:before", showtip)
            this.cmbKey.off("combo:focusin", showtip)
          }
          this.cmbKey.on("show:before", showtip)
          this.cmbKey.on("combo:focusin", showtip)
          ...
          this.txtPlaceholder.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          this.txtPlaceholder.cmpEl.on("focus", "input.form-control", () => {
            setTimeout(() => {
              this.txtPlaceholder._input?.select()
            }, 1)
          })
          ...
          this.txtTag.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          this.txtTag.cmpEl.on("focus", "input.form-control", () => {
            setTimeout(() => {
              this.txtTag._input?.select()
            }, 1)
          })
          ...
          this.textareaHelp.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          ...
          this.txtDefValue.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          this.txtDefValue.cmpEl.on("focus", "input.form-control", () => {
            setTimeout(() => {
              this.txtDefValue._input?.select()
            }, 1)
          })
          ...
          this.txtDateDefValue.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          this.txtDateDefValue.on("date:click", this.onDateDefClick.bind(this))
          this.txtDateDefValue.cmpEl.on("focus", "input.form-control", () => {
            setTimeout(() => {
              this.txtDateDefValue._input?.select()
            }, 1)
          })
          ...
          this.spnMaxChars.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          ...
          const showGrouptip = () => {
            ...
            Common.NotificationCenter.trigger("forms:close-help", "settings", true)
            this.cmbGroupKey.off("show:before", showGrouptip)
            this.cmbGroupKey.off("combo:focusin", showGrouptip)
          }
          this.cmbGroupKey.on("show:before", showGrouptip)
          this.cmbGroupKey.on("combo:focusin", showGrouptip)

          this.txtChoice = new Common.UI.InputField({
          ...
          this.txtChoice.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          this.txtChoice.cmpEl.on("focus", "input.form-control", () => {
            setTimeout(() => {
              this.txtChoice._input?.select()
            }, 1)
          })
          ...
          this.txtLabel.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          this.txtLabel.cmpEl.on("focus", "input.form-control", () => {
            setTimeout(() => {
              this.txtLabel._input?.select()
            }, 1)
          })
          ...
          this.txtNewValue = new Common.UI.InputField({
            ...
            this.btnListAdd.setDisabled(value.length < 1 || this._state.DisabledControls)
          })
          this.lockedControls.push(this.txtNewValue)
          this.txtNewValue.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          this.txtNewValue._input.on("keydown", _.bind(this.onNewValueKeydown, this))
          this.txtNewValue.cmpEl.on("focus", "input.form-control", () => {
            setTimeout(() => {
              this.txtNewValue._input?.select()
            }, 1)
          })
          ...
          this.btnLockForm.on(
            ...
            _.bind(function (btn) {
              if (this.api && !this._noApply) {
                const props = this._originalProps || new AscCommon.CContentControlPr()
                props.put_Lock(
                  !this._state.LockDelete
                    ? Asc.c_oAscSdtLockType.SdtLocked
          ...
          this.cmbScale = new Common.UI.ComboBox({
            ...
            editable: false,
            data: [
              { displayValue: this.textAlways, value: Asc.c_oAscPictureFormScaleFlag.Always },
          ...
          this.sldrPreviewPositionX.on(
            "changecomplete",
            _.bind(this.onImagePositionChangeComplete, this, "x"),
          )
          ...
          this.sldrPreviewPositionY.on(
            "changecomplete",
            _.bind(this.onImagePositionChangeComplete, this, "y"),
          )

          const xValue = this.sldrPreviewPositionX.getValue()
          const yValue = this.sldrPreviewPositionY.getValue()
          ...
          this.cmbRoles = new Common.UI.ComboBoxCustom({
            ...
            updateFormControl: function (record) {
              ...
              if (record) {
                formcontrol[0].innerHTML = `<span class="color" style="background: ${record.get("color")};"></span><div style="overflow: hidden; text-overflow: ellipsis;">${Common.Utils.String.htmlEncode(record.get("displayValue"))}</div>`
          ...
          const showRolesTip = () => {
            ...
            this.cmbRoles.off("show:before", showRolesTip)
          }
          this.cmbRoles.on("show:before", showRolesTip)
          ...
          this.cmbFormat = new Common.UI.ComboBox({
            ...
            editable: false,
            data: [
              { displayValue: this.textNone, value: Asc.TextFormFormatType.None },
          ...
          this.txtRegExp.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          this.txtRegExp.cmpEl.on("focus", "input.form-control", () => {
            setTimeout(() => {
              this.txtRegExp._input?.select()
            }, 1)
          })
          ...
          this.cmbMask.cmpEl.on("focus", "input.form-control", () => {
            setTimeout(() => {
              this.cmbMask._input?.select()
            }, 1)
          })
          ...
          this.txtFormatSymbols.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          this.txtFormatSymbols.cmpEl.on("focus", "input.form-control", () => {
            setTimeout(() => {
              this.txtFormatSymbols._input?.select()
            }, 1)
          })
          ...
          this.cmbDateFormat.cmpEl.on("focus", "input.form-control", () => {
            setTimeout(() => {
              this.cmbDateFormat._input?.select()
            }, 1)
          })
          ...
          this.cmbLang.on("selected", (combo, record) => {
            this.updateDateFormats(record.value)
          })
        ...
        setApi: function (api) {
          ...
          if (this.api) {
            ...
            this.api.asc_registerCallback(
              "asc_onUpdateOFormRoles",
              _.bind(this.onRefreshRolesList, this),
            )
          }
        ...
        onKeyChanged: function (combo, record) {
          if (this.api && !this._noApply) {
            ...
            const props = this._originalProps || new AscCommon.CContentControlPr()
            const formPr = this._originalFormProps || new AscCommon.CSdtFormPr()
        ...
        onPlaceholderChanged: function (input, newValue, oldValue, e) {
          if (this.api && !this._noApply && newValue !== oldValue) {
            ...
            if (
              !e.relatedTarget ||
              (e.relatedTarget.localName !== "input" && e.relatedTarget.localName !== "textarea") ||
              !/form-control/.test(e.relatedTarget.className)
            )
              this.fireEvent("editcomplete", this)
          }
        },
        ...
        onDateDefClick: function (input, date) {
          if (this.api && !this._noApply) {
            const formDatePr = this._originalDateProps || new AscCommon.CSdtDatePickerPr()
        ...
        onChMaxCharsChanged: function (field, newValue, oldValue, eOpts) {
          ...
          this.spnMaxChars.setDisabled(
            !checked ||
              this._state.FormatType === Asc.TextFormFormatType.Mask ||
              this._state.DisabledControls,
          )
          if (!checked) {
          ...
          if (this.api && !this._noApply) {
            const props = this._originalProps || new AscCommon.CContentControlPr()
            const formTextPr = this._originalTextFormProps || new AscCommon.CSdtTextFormPr()
            !checked && formTextPr.put_Comb(checked)
            formTextPr.put_MaxCharacters(
              checked ? this.spnMaxChars.getNumberValue() || 10 : checked,
            )
        ...
        onMaxCharsChange: function (field, newValue, oldValue, eOpts) {
          if (this.api && !this._noApply) {
            ...
            const formTextPr = this._originalTextFormProps || new AscCommon.CSdtTextFormPr()
            const checked =
              this.chMaxChars.getValue() === "checked" || this.chComb.getValue() === "checked"
        ...
        onChCombChanged: function (field, newValue, oldValue, eOpts) {
          ...
          this.cmbWidthRule.setDisabled(
            !checked || this._state.Fixed || this._state.DisabledControls,
          )
          this.spnWidth.setDisabled(
            !checked ||
              this._state.WidthRule === Asc.CombFormWidthRule.Auto ||
              this._state.DisabledControls,
          )
          if (this.api && !this._noApply) {
            const props = this._originalProps || new AscCommon.CContentControlPr()
            ...
            if (checked) {
              ...
              if (this.spnWidth.getValue()) {
                const value = this.spnWidth.getNumberValue()
                formTextPr.put_Width(
                  value <= 0
                    ? 0
                    : Number.parseInt(
                        (Common.Utils.Metric.fnRecalcToMM(value) * 72 * 20) / 25.4 + 0.5,
                      ),
                )
              } else formTextPr.put_Width(0)
            }
            props.put_TextFormPr(formTextPr)
            this.api.asc_SetContentControlProperties(props, this.internalId)
            this.fireEvent("editcomplete", this)
        ...
        onChFixed: function (field, newValue, oldValue, eOpts) {
          if (this.api && !this._noApply) {
            const props = this._originalProps || new AscCommon.CContentControlPr()
            this.cmbWidthRule.setDisabled(
              !this._state.Comb || field.getValue() === "checked" || this._state.DisabledControls,
            )
        ...
        onChAspect: function (field, newValue, oldValue, eOpts) {
          if (this.api && !this._noApply) {
            const props = this._originalProps || new AscCommon.CContentControlPr()
            const pictPr = this._originalPictProps || new AscCommon.CSdtPictureFormPr()
        ...
        onGroupKeyChanged: function (combo, record) {
          if (this.api && !this._noApply && record.value !== "") {
            const props = this._originalProps || new AscCommon.CContentControlPr()
            const specProps = this._originalCheckProps || new AscCommon.CSdtCheckBoxPr()
        ...
        fillListProps: function () {
          if (this.api && !this._noApply) {
            const props = this._originalProps || new AscCommon.CContentControlPr()
            const specProps = this._originalListProps || new AscCommon.CSdtComboBoxPr()
            specProps.clear()
            this.list.store.each((item, index) => {
              specProps.add_Item(item.get("name"), item.get("value"))
            })
            this.type === Asc.c_oAscContentControlSpecificType.ComboBox
              ? props.put_ComboBoxPr(specProps)
              : props.put_DropDownListPr(specProps)
            this.api.asc_SetContentControlProperties(props, this.internalId)
          }
        },
        ...
        onAddItem: function () {
          const store = this.list.store
          const value = this.txtNewValue.getValue()
          if (value !== "") {
            const rec = store.findWhere({ value: value })
            if (!rec) {
        ...
        onDeleteItem: function (btn, eOpts) {
          const rec = this.list.getSelectedRec()
          if (rec) {
            const store = this.list.store
        ...
        onColorPickerSelect: function (btn, color) {
          ...
          if (this.api && !this._noApply) {
            ...
            if (color === "transparent") {
              ...
              let brd = formPr.get_Border()
              if (!brd) brd = new Asc.asc_CTextBorder()
              brd.put_Value(1)
        ...
        ChangeSettings: function (props, isShape) {
          ...
          if (this._themeChanged) this.onThemeChanged()
          ...
          if (props) {
            ...
            let val = props.get_PlaceholderText()
            ...
            if (
              this._state.LockDelete !==
              (val === Asc.c_oAscSdtLockType.SdtContentLocked ||
                val === Asc.c_oAscSdtLockType.SdtLocked)
            ) {
              this._state.LockDelete =
                val === Asc.c_oAscSdtLockType.SdtContentLocked ||
                val === Asc.c_oAscSdtLockType.SdtLocked
            ...
            const type = props.get_SpecificType()
            let connected = false
            let specProps
            //for list controls
            if (
              type === Asc.c_oAscContentControlSpecificType.ComboBox ||
              type === Asc.c_oAscContentControlSpecificType.DropDownList
            ) {
              this.labelFormName.text(
                type === Asc.c_oAscContentControlSpecificType.ComboBox
                  ? this.textCombobox
                  : this.textDropDown,
              )
              ...
              if (specProps) {
                ...
                if (
                  ...
                  (this._state.listValue !== undefined || this._state.listIndex !== undefined)
                ) {
                  if (this._state.listIndex !== undefined) {
                    this._state.listIndex >= this.list.store.length &&
                      (this._state.listIndex = this.list.store.length - 1)
                  }
                  rec =
                    this._state.listValue !== undefined
                      ? this.list.store.findWhere({ value: this._state.listValue })
                      : this.list.store.at(this._state.listIndex)
                }
                if (rec) {
            ...
            if (formPr) {
              ...
              let data = this.api.asc_GetFormKeysByType(type)
              if (
                !this._state.arrKey ||
                this._state.arrKey.length !== data.length ||
                _.difference(this._state.arrKey, data).length > 0
              ) {
                const arr = []
                data.forEach((item) => {
              ...
              if (type === Asc.c_oAscContentControlSpecificType.CheckBox && specProps) {
                ...
                const ischeckbox = typeof val !== "string"
                if (!ischeckbox) {
                  data = this.api.asc_GetRadioButtonGroupKeys()
                  if (
                    !this._state.arrGroupKey ||
                    this._state.arrGroupKey.length !== data.length ||
                    _.difference(this._state.arrGroupKey, data).length > 0
                  ) {
                    const arr = []
                    data.forEach((item) => {
                      arr.push({ displayValue: item, value: item })
                ...
                this.chDefValue.setCaption(
                  ischeckbox ? this.textCheckDefault : this.textRadioDefault,
                )
              ...
              if (
                type !== Asc.c_oAscContentControlSpecificType.Picture &&
                type !== Asc.c_oAscContentControlSpecificType.Signature
              ) {
              ...
              const brd = formPr.get_Border()
              if (brd) {
                const color = brd.get_Color()
                if (color) {
                  if (color.get_type() === Asc.c_oAscColor.COLOR_TYPE_SCHEME) {
                    ...
                    this.BorderColor = Common.Utils.ThemeColor.getHexColor(
                      color.get_r(),
                      color.get_g(),
                      color.get_b(),
                    )
              ...
              let type1 = typeof this.BorderColor
              let type2 = typeof this._state.BorderColor
              if (
                type1 !== type2 ||
                (type1 === "object" &&
                  (this.BorderColor.effectValue !== this._state.BorderColor.effectValue ||
                    this._state.BorderColor.color.indexOf(this.BorderColor.color) < 0)) ||
                ...
                this.BorderColor !== "transparent" &&
                  this.mnuColorPicker.selectByRGB(
                    typeof this.BorderColor === "object"
                      ? this.BorderColor.color
                      : this.BorderColor,
                    true,
                  )
              ...
              const shd = formPr.get_Shd()
              if (shd) {
                const bgColor = shd.get_Color()
                if (bgColor) {
                  if (bgColor.get_type() === Asc.c_oAscColor.COLOR_TYPE_SCHEME) {
                    this.BackgroundColor = {
                      ...
                      effectValue: bgColor.get_value(),
                    }
                  } else {
                    this.BackgroundColor = Common.Utils.ThemeColor.getHexColor(
                      bgColor.get_r(),
                      bgColor.get_g(),
                      bgColor.get_b(),
                    )
              ...
              if (
                type1 !== type2 ||
                (type1 === "object" &&
                  (this.BackgroundColor.effectValue !== this._state.BackgroundColor.effectValue ||
                    this._state.BackgroundColor.color.indexOf(this.BackgroundColor.color) < 0)) ||
                (type1 !== "object" &&
                  this._state.BackgroundColor.indexOf(this.BackgroundColor) < 0)
              ) {
                this.btnBGColor.setColor(this.BackgroundColor)
                Common.Utils.ThemeColor.selectPickerColorByEffect(
                  this.BackgroundColor,
                  this.mnuBGColorPicker,
                )
                this._state.BackgroundColor = this.BackgroundColor
              }
            }

            const pictPr = props.get_PictureFormPr()
            const isSignature = type === Asc.c_oAscContentControlSpecificType.Signature
            if (pictPr) {
              ...
              this.imagePositionLabel.text(
                `${Math.round(this._state.imgPositionX)},${Math.round(this._state.imgPositionY)}`,
              )
              ...
              this.chAspect.setDisabled(
                this._state.scaleFlag === Asc.c_oAscPictureFormScaleFlag.Never ||
                  this._state.DisabledControls,
              )
              const disableSliders =
                (this._state.scaleFlag === Asc.c_oAscPictureFormScaleFlag.Always &&
                  !this._state.Aspect) ||
                this._state.DisabledControls
            ...
            const formTextPr = props.get_TextFormPr()
            const needUpdateTextControls =
            ...
            if (formTextPr) {
              ...
              this.chMulti.setDisabled(
                !this._state.Fixed || this._state.Comb || this._state.DisabledControls,
              )
              ...
              this.chAutofit.setDisabled(
                !this._state.Fixed || this._state.Comb || this._state.DisabledControls,
              )

              this.cmbWidthRule.setDisabled(
                !this._state.Comb || this._state.Fixed || this._state.DisabledControls,
              )
              ...
              if (
                ...
                Math.abs(this._state.WidthPlaceholder - val) > 0.01
              ) {
                this.spnWidth.setDefaultValue(
                  val !== undefined && val !== null
                    ? Common.Utils.Metric.fnRecalcFromMM(((val + 1) * 25.4) / 20 / 72.0)
                    : this.spnWidth.getMinValue(),
                )
                this._state.WidthPlaceholder = val
              }
              ...
              val =
                ...
                val === undefined ||
                val === 0
                  ? this._state.WidthPlaceholder
                  : val
              if (
                ((val === undefined || this._state.Width === undefined) &&
                  this._state.Width !== val) ||
                Math.abs(this._state.Width - val) > 0.1
              ) {
                this.spnWidth.setValue(
                  val !== 0 && val !== undefined
                    ? Common.Utils.Metric.fnRecalcFromMM((val * 25.4) / 20 / 72.0)
                    : "",
                  true,
                )
                this._state.Width = val
              ...
              if (this._state.FormatType !== val) {
                this.cmbFormat.setValue(
                  val !== null && val !== undefined ? val : Asc.TextFormFormatType.None,
                )
              ...
              if (
                ((val === undefined || this._state.MaxChars === undefined) &&
                  this._state.MaxChars !== val) ||
                Math.abs(this._state.MaxChars - val) > 0.1
              ) {
            ...
            } else this._originalTextFormProps = null

            const datePr = props.get_DateTimePr()
            if (datePr && type === Asc.c_oAscContentControlSpecificType.DateTime) {
              ...
              const lang = datePr.get_LangId() || this.options.controlLang
              if (lang) {
                let item = this.cmbLang.store.findWhere({ value: lang })
                item = item ? item.get("value") : 0x0409
                this.cmbLang.setValue(item)
              }
              this.updateDateFormats(this.cmbLang.getValue())
              const format = datePr.get_DateFormat()
            ...
            const isComplex = !!props.get_ComplexFormPr() // is complex form
            const isSimpleInsideComplex = !!this.api.asc_GetCurrentComplexForm() && !isComplex
            ...
            this.TextOnlySettingsMask.toggleClass(
              "hidden",
              !(type === Asc.c_oAscContentControlSpecificType.None && !!formTextPr) ||
                this._state.FormatType !== Asc.TextFormFormatType.Mask,
            )
            this.TextOnlySettingsRegExp.toggleClass(
              "hidden",
              !(type === Asc.c_oAscContentControlSpecificType.None && !!formTextPr) ||
                this._state.FormatType !== Asc.TextFormFormatType.RegExp,
            )
            if (
              this.type !== type ||
              this.isSimpleInsideComplex !== isSimpleInsideComplex ||
              needUpdateTextControls ||
              type === Asc.c_oAscContentControlSpecificType.CheckBox ||
        ...
        updateMetricUnit: function () {
          if (this.spinners) {
            ...
            const val = this._state.Width
            this.spnWidth?.setMinValue(Common.Utils.Metric.fnRecalcFromMM(1))
            this.spnWidth?.setValue(
              val !== 0 && val !== undefined
                ? Common.Utils.Metric.fnRecalcFromMM((val * 25.4) / 20 / 72.0)
                : "",
        ...
        UpdateThemeColors: function () {
          ...
          this.mnuBGColorPicker.updateColors(
        ...
        disableControls: function (disable) {
          ...
          if (this._state.DisabledControls !== (this._state.LockDelete || disable)) {
            ...
            _.each(this.lockedControls, (item) => {
              item.setDisabled(this._state.DisabledControls)
            })
          }
          this.chMaxChars.setDisabled(
            this._state.FormatType === Asc.TextFormFormatType.Mask || this._state.DisabledControls,
          )
          this.spnMaxChars.setDisabled(
            this.chMaxChars.getValue() !== "checked" ||
              this._state.FormatType === Asc.TextFormFormatType.Mask ||
              this._state.DisabledControls,
          )
          this.cmbWidthRule.setDisabled(
          ...
          const disableSliders =
            this._state.scaleFlag === Asc.c_oAscPictureFormScaleFlag.Always && !this._state.Aspect
        ...
        showHideControls: function (
          ...
          textProps,
          specProps,
          isSimpleInsideComplex,
          isSignature,
        ) {
          let textOnly = false
          let checkboxOnly = false
          let radioboxOnly = false
          let listOnly = false
          let imageOnly = false
          let dateOnly = false
          if (
            ...
            type === Asc.c_oAscContentControlSpecificType.None ||
            type === Asc.c_oAscContentControlSpecificType.Complex
          ) {
            textOnly = !!textProps
          }
          ...
          const value = checkboxOnly || radioboxOnly
          this.PlaceholderSettings.toggleClass("hidden", value)
          this.CheckOnlySettings.toggleClass("hidden", !value)
          this.FixedSettings.toggleClass(
            "hidden",
            imageOnly || isSimpleInsideComplex || isSignature,
          )
          this.NotInComplexSettings.toggleClass("hidden", isSimpleInsideComplex)
          this.DateOnlySettings.toggleClass("hidden", !dateOnly)
          this.DefValueText.toggleClass(
            "hidden",
            !(type === Asc.c_oAscContentControlSpecificType.ComboBox || textOnly),
          )
          this.DefValueDropDown.toggleClass(
            "hidden",
            type !== Asc.c_oAscContentControlSpecificType.DropDownList,
          )
        ...
        onDisconnect: function () {
          if (
            this.type === Asc.c_oAscContentControlSpecificType.CheckBox &&
            typeof (this._originalCheckProps || new AscCommon.CSdtCheckBoxPr()).get_GroupKey() ===
              "string"
          )
            this.onGroupKeyChanged(this.cmbGroupKey, {
              value: (this._originalProps || new AscCommon.CContentControlPr()).get_NewKey(),
            })
          else
            this.onKeyChanged(this.cmbKey, {
              value: (this._originalProps || new AscCommon.CContentControlPr()).get_NewKey(),
            })
        },
        ...
        disableListButtons: function () {
          const rec = this.list.getSelectedRec()
          const idx = rec ? this.list.store.indexOf(rec) : -1
          ...
          this.btnListDown.setDisabled(
            idx < 0 || idx > this.list.store.length - 2 || this._state.DisabledControls,
          )
        },

        onImagePositionChange: function (type, field, newValue, oldValue) {
          const value = ((130 - 80) * newValue) / 100 - 1
          if (type === "x") {
        ...
        onRefreshRolesList: function (roles) {
          ...
          let lastrole = this.cmbRoles.getSelectedRecord()
          lastrole = lastrole ? lastrole.value : ""

          const arr = []
          roles?.forEach((item) => {
            const role = item.asc_getSettings()
            const color = role.asc_getColor()
            arr.push({
              displayValue: role.asc_getName() || this.textAnyone,
        ...
        updateDateFormats: function (lang) {
          if (this._originalDateProps) {
            const props = this._originalDateProps
            const formats = props.get_FormatsExamples()
            const arr = []
            ...
            if (this.api && !this._noApply) this.onDateFormatChanged()
          }
        },
        ...
        onThemeChanged: function () {
          ...
          if (!this._themeChanged && this.cmbRoles) {
            ...
            this.cmbRoles.cmpEl?.find(".form-control").css("width", `${width}px`)
          }
        },
      },
