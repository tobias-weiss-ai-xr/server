...
define([
  ...
  DE.Views.FormsTab = Common.UI.BaseView.extend(
    _.extend(
      (() => {
        const template =
        ...
        function setEvents() {
          this.btnTextField?.on("click", (b, e) => {
            this.fireEvent("forms:insert", ["text", { fixed: b.options.fieldType === "fixed" }])
          })
          this.btnTextField?.menu.on("item:click", (menu, item) => {
            const oldType = this.btnTextField.options.fieldType
            const newType = item.value

            if (newType !== oldType) {
              this.btnTextField.changeIcon({
                next: item.options.iconClsForMainBtn,
                curr: this.btnTextField.menu.items.filter((mnu) => mnu.value === oldType)[0].options
                  .iconClsForMainBtn,
              })
              this.btnTextField.updateHint(item.options.hintForMainBtn)
              this.btnTextField.options.fieldType = newType
              Common.localStorage.setBool("de-text-form-fixed", newType === "fixed")
            }
            this.fireEvent("forms:insert", ["text", { fixed: newType === "fixed" }])
          })
          ...
          this.btnSignField?.on("click", (b, e) => {
            Common.UI.TooltipManager.closeTip("signature")
            this.fireEvent("forms:insert", ["signature"])
          })
          this.btnComplexField?.on("click", (b, e) => {
            this.fireEvent("forms:insert", ["complex"])
          })
          ...
          if (this.btnViewFormRoles) {
            this.btnViewFormRoles.on("click", (b, e) => {
              let item
              if (b.menu) {
                item = b.menu.getChecked()
                if (item) {
                  item = item.caption
                } else if (this._state.roles && this._state.roles.length > 0) {
                  item = this._state.roles[0].asc_getSettings().asc_getName()
                }
              }
              this.fireEvent("forms:preview", [b.pressed, item])
            })
            if (this.btnViewFormRoles.menu) {
              this.btnViewFormRoles.menu.on(
                "item:click",
                _.bind((menu, item) => {
                  if (item.checked) {
                    this.btnViewFormRoles.toggle(true, true)
                    this.fireEvent("forms:preview", [true, item.caption])
                  }
                }, this),
              )
              this.btnViewFormRoles.menu.on("show:after", (menu) => {
                this.fillRolesMenu()
              })
            }
          }
          this.btnFinal?.on("click", (b, e) => {
            // me.fireEvent('forms:final', [b.pressed, true]);
          })
          this.btnManager?.on("click", (b, e) => {
            this.fireEvent("forms:manager")
          })
          this.cmbRoles?.on("selected", (combo, record) => {
            this.fireEvent("forms:currentrole", [combo, record])
          })
          ...
          if (this.fieldPages) {
            this.fieldPages.on("changed:after", () => {
              this.fireEvent("forms:gopage", ["", Number.parseInt(this.fieldPages.getValue())])
            })
            this.fieldPages.on("inputleave", function () {
              Common.NotificationCenter.trigger("edit:complete", this)
            })
            this.fieldPages.cmpEl?.on("focus", "input.form-control", () => {
              setTimeout(() => {
                this.fieldPages._input?.select()
              }, 1)
            })
          }
          this.btnFirstPage?.on("click", () => {
            this.fireEvent("forms:gopage", ["first"])
          })
          this.btnLastPage?.on("click", () => {
            this.fireEvent("forms:gopage", ["last"])
          })
          this.btnPrevPage?.on("click", () => {
            this.fireEvent("forms:gopage", ["prev"])
          })
          this.btnNextPage?.on("click", () => {
            this.fireEvent("forms:gopage", ["next"])
          })
        }
        ...
        return {
          ...
          initialize: function (options) {
            ...
            if (this.appConfig.isRestrictedEdit && this.appConfig.canFillForms) {
                  ...
                  validation: (value) => {
                    if (/(^[0-9]+$)/.test(value)) {
                      ...
                      if (value === undefined || value === null || value < 1)
                        this.fieldPages.setValue((this.api ? this.api.getCurrentPage() : 0) + 1)
                    } else this.fieldPages.setValue((this.api ? this.api.getCurrentPage() : 0) + 1)
              ...
              const isfixed = Common.localStorage.getBool("de-text-form-fixed", true)
              ...
              this.btnViewFormRoles = new Common.UI.Button({
                ...
                menu: Common.UI.FeaturesManager.isFeatureEnabled("roles", true)
                  ? new Common.UI.Menu({
                      cls: "menu-roles",
                      maxHeight: 270,
            ...
            if (this.appConfig.canSubmitForms) {
              ...
              this.btnSaveForm = new Common.UI.Button({
                ...
                caption:
                  this.appConfig.canRequestSaveAs || !!this.appConfig.saveAsUrl
                    ? this.capBtnSaveForm
                    : this.appConfig.isOffline
                      ? this.capBtnSaveFormDesktop
                      : this.capBtnDownloadForm,
                // disabled: this.appConfig.isEdit && this.appConfig.canFeatureContentControl && this.appConfig.canFeatureForms, // disable only for edit mode,
                dataHint: "1",
                dataHintDirection: "bottom",
                dataHintOffset: "small",
          ...
          onAppReady: function (config) {
            new Promise((accept, reject) => {
              ...
              if (config.isEdit && config.canFeatureContentControl && config.canFeatureForms) {
                ...
                // }
                const menuTemplate = _.template(
                  '<a id="<%= id %>" tabindex="-1" type="menuitem" class="menu-item">' +
                ...
                this.btnTextField.setMenu(
                  new Common.UI.Menu({
                    style: "max-width: 300px;",
                    items: [
                      {
                        caption: this.txtInlineText,
                        template: menuTemplate,
                        description: this.txtInlineDesc,
                        iconCls: "menu__icon btn-field",
                        value: "inline",
                        iconClsForMainBtn: "btn-text-field",
                        hintForMainBtn: [this.tipInlineText, this.tipTextField],
                      },
                      {
                        caption: this.txtFixedText,
                        template: menuTemplate,
                        description: this.txtFixedDesc,
                        iconCls: "menu__icon btn-fixed-field",
                        value: "fixed",
                        iconClsForMainBtn: "btn-text-fixed-field",
                        hintForMainBtn: [this.tipFixedText, this.tipTextField],
                      },
                    ],
                  }),
                )
                this.btnTextField.updateHint([
                  Common.localStorage.getBool("de-text-form-fixed", true)
                    ? this.tipFixedText
                    : this.tipInlineText,
                  this.tipTextField,
                ])
                ...
                this.btnSignField.updateHint(this.tipSignField)
                this.btnViewFormRoles.updateHint(this.tipViewForm)
                this.btnFinal.updateHint(this.tipFinalForm)
                this.btnManager.updateHint(this.tipManager)
                this.btnEmailField.updateHint(this.tipEmailField)
                this.btnPhoneField.updateHint(this.tipPhoneField)
                this.btnComplexField.updateHint(this.tipComplexField)
              ...
              this.btnSaveForm?.updateHint(
                config.canRequestSaveAs || !!config.saveAsUrl
                  ? this.tipSaveForm
                  : this.tipDownloadForm,
              )
          ...
          getPanel: function () {
            ...
            if (this.appConfig.isRestrictedEdit && this.appConfig.canFillForms) {
              this.btnSubmit
                ? this.btnSubmit.render($("#slot-btn-header-form-submit"))
                : $("#slot-btn-header-form-submit").hide()
              if (this.appConfig.isPDFForm) {
            ...
            this.btnSaveForm?.render($host.find("#slot-btn-form-save"))
            ;((this.btnSubmit &&
              !(this.appConfig.isRestrictedEdit && this.appConfig.canFillForms)) ||
              this.btnSaveForm) &&
              $host.find(".save-separator").show()
            this.appConfig.isRestrictedEdit &&
              this.appConfig.canFillForms &&
              this.appConfig.isPDFForm &&
              this.showFillingForms(false)

            return this.$el
          },

          fillRolesMenu: function (roles, lastViewRole) {
            if (!this.btnViewFormRoles?.menu?.isVisible()) {
            ...
            roles?.forEach((item, index) => {
              ...
              if (role.asc_getName() === lastViewRole) checkedIndex = index
              this.btnViewFormRoles.menu.addItem(
                new Common.UI.MenuItem({
                  caption: role.asc_getName() || this.textAnyone,
                  color: color
            ...
            const len = this.btnViewFormRoles.menu.items.length > 0
            len && this.btnViewFormRoles.menu.items[checkedIndex].setChecked(true, true)
            Common.Utils.lockControls(Common.enumLock.formsNoRoles, !len, {
              array: [this.btnViewFormRoles],
            })
          },
          ...
          fillFillForCombo: function (roles, lastRoleInList) {
            ...
            const rec = this.cmbRoles.store.findWhere({ value: lastrole })
          ...
          setPreviewMode: function (state) {
            ...
            this.btnSubmit?.setVisible(!state)
            this.btnSaveForm?.setVisible(!state)
          },
          ...
          showFillingForms: function (visible) {
            ...
            visible
              ? this.btnPrevForm.$el.parents(".group").show().prev(".separator").show()
          ...
          tipFormKey:
            "You can assign a key to a field or a group of fields. When a user fills in the data, it will be copied to all the fields with the same key.",
          tipFormGroupKey:
            "Group radio buttons to make the filling process faster. Choices with the same names will be synchronized. Users can only tick one radio button from the group.",
          tipFieldSettings:
            "You can configure selected fields on the right sidebar. Click this icon to open the field settings.",
          tipHelpRoles:
            "Use the Manage Roles feature to group fields by purpose and assign the responsible team members.",
