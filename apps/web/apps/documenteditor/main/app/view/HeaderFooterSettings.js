...
define([
  ...
  DE.Views.HeaderFooterSettings = Backbone.View.extend(
    _.extend(
      {
        ...
        ChangeSettings: function (prop) {
          ...
          if (prop) {
            ...
            if (this._state.PositionType !== value) {
              ...
              else this.lblPosition[0].innerHTML = this.textHeaderFromTop
            ...
            if (this._state.Numbering !== value && value !== null) {
              if (value < 0) this.radioPrev.setValue(true, true)
              else {
        ...
        onBtnPositionClick: function (btn, eOpts) {
          if (this.api) this.api.put_PageNum(btn.options.posWhere, btn.options.posAlign)
        ...
        onRadioFrom: function (field, newValue, eOpts) {
          if (newValue && this.api) {
            if (_.isEmpty(this.numFrom.getValue())) this.numFrom.setValue(1, true)
        ...
        updateMetricUnit: function () {
          if (this.spinners) {
            ...
            this.numPosition?.setValue(
              Common.Utils.Metric.fnRecalcFromMM(this._state.Position),
              true,
            )
          }
        },
        ...
        createDelayedControls: function () {
          const _arrPosition = [
            [
              c_pageNumPosition.PAGE_NUM_POSITION_TOP,
              c_pageNumPosition.PAGE_NUM_POSITION_LEFT,
              "btn-page-number-top-left",
              "headerfooter-button-top-left",
              this.textTopLeft,
            ],
            [
              c_pageNumPosition.PAGE_NUM_POSITION_TOP,
              c_pageNumPosition.PAGE_NUM_POSITION_CENTER,
              "btn-page-number-top-center",
              "headerfooter-button-top-center",
              this.textTopCenter,
            ],
            [
              c_pageNumPosition.PAGE_NUM_POSITION_TOP,
              c_pageNumPosition.PAGE_NUM_POSITION_RIGHT,
              "btn-page-number-top-right",
              "headerfooter-button-top-right",
              this.textTopRight,
            ],
            [
              c_pageNumPosition.PAGE_NUM_POSITION_BOTTOM,
              c_pageNumPosition.PAGE_NUM_POSITION_LEFT,
              "btn-page-number-bottom-left",
            ...
            [
              c_pageNumPosition.PAGE_NUM_POSITION_BOTTOM,
              c_pageNumPosition.PAGE_NUM_POSITION_CENTER,
              "btn-page-number-bottom-center",
              "headerfooter-button-bottom-center",
              this.textBottomCenter,
            ],
            [
              c_pageNumPosition.PAGE_NUM_POSITION_BOTTOM,
              c_pageNumPosition.PAGE_NUM_POSITION_RIGHT,
              "btn-page-number-bottom-right",
              "headerfooter-button-bottom-right",
              this.textBottomRight,
            ],
          ]
          ...
          this.numPosition.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          ...
          this._arrNumbers = [
            ...
            {
              displayValue: "- 1 -, - 2 -, - 3 -,...",
              value: Asc.c_oAscNumberingFormat.NumberInDash,
            },
        ...
        fillFormatCombo: function (format) {
          ...
          const store = [].concat(this._arrNumbers)
          this.recentNumTypes.forEach((item) => {
            if (item !== null && item !== undefined) {
              item = Number.parseInt(item)
              store.push({
                displayValue: `${AscCommon.IntToNumberFormat(1, item, this.mode.lang)}, ${AscCommon.IntToNumberFormat(2, item, this.mode.lang)}, ${AscCommon.IntToNumberFormat(3, item, this.mode.lang)},...`,
                value: item,
              })
            }
          })
          ...
          this.cmbFormat.setValue(
            format !== Asc.c_oAscNumberingFormat.None && format !== undefined && format !== null
              ? format
              : "",
          )
        },
        ...
        onFormatSelect: function (combo, record) {
          if (record.value === -2) {
            ...
            this.api?.asc_SetSectionPageNumFormat(record.value)
            this.fireEvent("editcomplete", this)
          }
        },

        loadRecent: function () {
          let sRecents = Common.localStorage.getItem("de-recent-header-formats")
          if (sRecents !== "") {
        ...
        addNewListType: function () {
          ...
          const win = new DE.Views.ListTypesAdvanced({
            ...
            handler: (result, value) => {
              btn = result
              if (result === "ok") {
                this.api?.asc_SetSectionPageNumFormat(value)
              }
              this.fireEvent("editcomplete", this)
            },
          }).on("close", (obj) => {
            btn !== "ok" && this.cmbFormat.setValue(this._state.NumFormat)
          })
          win.show()
        },
