...
define([], () => {
  DE.Views.HyphenationDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 290,
        ...
        render: function () {
          ...
          this.spnLimit.on(
            ...
            _.bind((field, newValue, oldValue, eOpts) => {
              if (Number.parseInt(newValue) === 0) field.setValue(-1)
            }, this),
          )
          ...
          this.spnZone.on(
            ...
            _.bind(function (field, newValue, oldValue, eOpts) {
              if (this._changedProps) {
                const value = field.getNumberValue()
                this._changedProps.setHyphenationZone(
        ...
        getFocusedComponents: function () {
          return [this.chAuto, this.chCaps, this.spnZone, this.spnLimit].concat(
        ...
        setSettings: function (props) {
          if (props) {
            ...
            let value = props.getHyphenationLimit() || 0
            this.spnLimit.setValue(
              value !== null && value !== undefined ? (value === 0 ? -1 : value) : "",
              true,
            )
            value = props.getHyphenationZone()
            this.spnZone.setValue(
              value !== null && value !== undefined
                ? Common.Utils.Metric.fnRecalcFromMM((value * 25.4) / 20 / 72.0)
                : "",
              true,
            )
          }
