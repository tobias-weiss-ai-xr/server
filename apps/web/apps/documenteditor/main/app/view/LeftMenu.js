...
define([
  ...
  const SCALE_MIN = 40
  const MENU_SCALE_PART = 300

  DE.Views.LeftMenu = Common.UI.SideMenu.extend(
    _.extend(
      {
        el: "#left-menu",
        ...
        render: function () {
          ...
          this.btnSearchBar.on(
            "click",
            _.bind(function () {
              this.onBtnMenuClick(this.btnSearchBar)
              if (this.btnSearchBar.pressed) this.fireEvent("search:aftershow")
            }, this),
          )
          ...
          this.btnSupport.on(
            "click",
            _.bind(function () {
              const config = this.mode.customization
        ...
        onBtnMenuToggle: function (btn, state) {
          if (state) {
            btn.panel.show()
            if (!this._state.pluginIsRunning) this.$el.width(SCALE_MIN)
          } else {
            btn.panel.hide()
          }
          DE.getController("Toolbar").DisableToolbar(state === true)
          if (!this.supressEvents) Common.NotificationCenter.trigger("layout:changed", "leftmenu")
        },

        onBtnCommentsToggle: function (btn, state) {
          if (!state) this.fireEvent("comments:hide", this)
        },
        ...
        onBtnMenuClick: function (btn, e) {
          ...
          if (btn.pressed) {
            if (!(this.$el.width() > SCALE_MIN)) {
              this.$el.width(
                Number.parseInt(Common.localStorage.getItem("de-mainmenu-width")) ||
                  MENU_SCALE_PART,
              )
            }
            ...
            this.isVisible() &&
              width > SCALE_MIN &&
              Common.localStorage.setItem("de-mainmenu-width", width)
          ...
          btn.options.type !== "plugin" &&
            $(".left-panel .plugin-panel").toggleClass("active", false)
          ...
          if (this.mode.canCoAuthoring) {
            ...
            if (this.mode.canChat) {
              if (this.btnChat.pressed) {
                if (this.btnChat.$el.hasClass("notify")) this.btnChat.$el.removeClass("notify")

                this.panelChat.show()
                this.panelChat.focus()
              } else this.panelChat.hide()
            }
          }
          if (this.panelNavigation) {
            if (this.btnNavigation.pressed) {
              this.panelNavigation.show()
            } else this.panelNavigation.hide()
          }
        ...
        getFocusElement: function () {
          ...
          } else if (this.btnSearchBar?.pressed) {
        ...
        setOptionsPanel: function (name, panel) {
          ...
          } /** coauthoring end **/ else if (name === "history") {
            this.panelHistory = panel.render("#left-panel-history")
          } else if (name === "navigation" && !this.panelNavigation) {
        ...
        markCoauthOptions: function (opt, ignoreDisabled) {
          if (
            opt === "chat" &&
            (this.btnChat.isVisible() || this.isButtonInMoreMenu(this.btnChat)) &&
            !this.btnChat.isDisabled() &&
            !this.btnChat.pressed
          ) {
            this.btnChat.$el.addClass("notify")
          }
          if (
            opt === "comments" &&
            (this.btnComments.isVisible() || this.isButtonInMoreMenu(this.btnComments)) &&
            !this.btnComments.pressed &&
          ...
          if (this.menuFile.isVisible()) {
            ...
            if (this.mode?.canCoAuthoring) {
              if (this.mode.canViewComments) {
                ...
                if (this.btnComments.pressed) this.fireEvent("comments:hide", this)
        ...
        isOpened: function () {
          let isopened = this.btnSearchBar.pressed
          /** coauthoring begin **/
        ...
        showMenu: function (menu, opts, suspendAfter) {
          ...
          if (re[1] === "file") {
            ...
            if (menu === "chat") {
              ...
              if (
                (this.btnNavigation.isVisible() || this.isButtonInMoreMenu(this.btnNavigation)) &&
                !this.btnNavigation.isDisabled() &&
                !this.btnNavigation.pressed
              ) {
              ...
              if (
                (this.btnSearchBar.isVisible() || this.isButtonInMoreMenu(this.btnSearchBar)) &&
                !this.btnSearchBar.isDisabled() &&
                !this.btnSearchBar.pressed
              ) {
        ...
        showHistory: function () {
          ...
          this.panelHistory.$el.width(
            (Number.parseInt(Common.localStorage.getItem("de-mainmenu-width")) || MENU_SCALE_PART) -
              SCALE_MIN,
          )
        ...
        setDeveloperMode: function (mode, beta, version) {
          ...
          if (mode & Asc.c_oLicenseMode.Trial || mode & Asc.c_oLicenseMode.Developer) {
            if (!this.developerHint) {
              ...
              else if ((mode & Asc.c_oLicenseMode.Trial) !== 0) str = this.txtTrial
              else if ((mode & Asc.c_oLicenseMode.Developer) !== 0) str = this.txtDeveloper
              str = str.toUpperCase()
          ...
          this.developerHint?.toggleClass(
            "hidden",
            !(mode & Asc.c_oLicenseMode.Trial || mode & Asc.c_oLicenseMode.Developer),
          )

          if (beta) {
            if (!this.betaHint) {
              const style = mode ? 'style="margin-top: 4px;"' : ""
              const arr = (version || "").split(".")
              let ver = ""
              arr.length > 0 && (ver += `v. ${arr[0]}`)
              arr.length > 1 && (ver += `.${arr[1]}`)
              this.betaHint = $(`<div id="beta-hint"${style}>${ver} (beta)</div>`).appendTo(
                this.$el,
              )
          ...
          const btns = this.$el.find("button.btn-category:visible")
          const lastbtn = btns.length > 0 ? $(btns[btns.length - 1]) : null
          this.minDevPosition = lastbtn
            ? Common.Utils.getOffset(lastbtn).top -
              Common.Utils.getOffset(lastbtn.offsetParent()).top +
              lastbtn.height() +
              20
            : 20
          this.onWindowResize()
        ...
        setLimitMode: function () {
          ...
          if (!this.limitHint) {
            const str = this.txtLimit.toUpperCase()
            this.limitHint = $(
              `<div id="limit-hint" style="margin-top: 4px;">${str}</div>`,
            ).appendTo(this.$el)
            this.limitHeight = this.limitHint.outerHeight()
            !this.devHintInited && $(window).on("resize", _.bind(this.onWindowResize, this))
            this.devHintInited = true
          }
          this.limitHint?.toggleClass("hidden", false)
        ...
        onWindowResize: function () {
          const height = (this.devHeight || 0) + (this.betaHeight || 0) + (this.limitHeight || 0)
          let top = Math.max((this.$el.height() - height) / 2, this.minDevPosition)
          if (this.developerHint) {
