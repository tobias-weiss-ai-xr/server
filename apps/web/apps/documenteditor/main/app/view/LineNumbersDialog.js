...
define([], () => {
  DE.Views.LineNumbersDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 290,
        ...
        getFocusedComponents: function () {
          return [
            this.chAddLineNumbering,
            this.spnStartAt,
            this.spnFromText,
            this.spnCountBy,
            this.rbRestartEachPage,
        ...
        setSettings: function (props) {
          if (props) {
            const type = props.get_Restart()
            this.chAddLineNumbering.setValue(true)
            switch (type) {
              case Asc.c_oAscLineNumberRestartType.Continuous:
                this.rbContinuous.setValue(true, true)
                break
              case Asc.c_oAscLineNumberRestartType.NewPage:
                this.rbRestartEachPage.setValue(true, true)
                break
              case Asc.c_oAscLineNumberRestartType.NewSection:
                this.rbRestartEachSection.setValue(true, true)
                break
            }
            this.spnStartAt.setValue(
              props.get_Start() !== null && props.get_Start() !== undefined
                ? props.get_Start()
                : "",
              true,
            )
            this.spnFromText.setValue(
              props.get_Distance() !== null && props.get_Distance() !== undefined
                ? Common.Utils.Metric.fnRecalcFromMM((props.get_Distance() * 25.4) / 20 / 72.0)
                : -1,
              true,
            )
            this.spnCountBy.setValue(
              props.get_CountBy() !== null && props.get_CountBy() !== undefined
                ? props.get_CountBy()
                : "",
              true,
            )
          } else this.chAddLineNumbering.setValue(false)
        },
        ...
        getSettings: function () {
          ...
          if (this.chAddLineNumbering.getValue() === "checked") {
            ...
            props.put_Start(
              this.spnStartAt.getValue() !== "" ? this.spnStartAt.getNumberValue() : undefined,
            )
            const value = this.spnFromText.getNumberValue()
            props.put_Distance(
              value < 0
                ? null
            ...
            props.put_CountBy(
              this.spnCountBy.getValue() !== "" ? this.spnCountBy.getNumberValue() : undefined,
            )
          }
