...
define([
  ...
  DE.Views.MailMergeEmailDlg = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
        ...
        getSettings: function () {
          let filename = this.inputFileName.getValue()
          const mailformat = this.cmbFormat.getValue()
          if (mailformat !== Asc.c_oAscFileType.HTML) {
            if (_.isEmpty(filename)) filename = "attach"
            const idx = filename.lastIndexOf(".")
            const ext = idx > 0 ? filename.substring(idx, filename.length).toLowerCase() : ""
            if (mailformat === Asc.c_oAscFileType.PDF && ext !== ".pdf") filename += ".pdf"
            else if (mailformat === Asc.c_oAscFileType.DOCX && ext !== ".docx") filename += ".docx"
          }
        ...
        _setDefaults: function (props) {
          if (props) {
            if (props.fieldsList) {
              const arr = []
              _.each(props.fieldsList, (field, index) => {
                arr.push({ displayValue: `<${field}>`, value: field })
              })
              this.cmbTo.setData(arr)
              if (arr.length > 0) this.cmbTo.setValue(arr[0].value)
            }

            if (props.emailAddresses) {
              const arr = []
              _.each(props.emailAddresses, (field, index) => {
                arr.push({ displayValue: field, value: field })
              })
              this.cmbFrom.setData(arr)
              if (arr.length > 0) this.cmbFrom.setValue(arr[0].value)
            }
          }
        },
        ...
        onCmbFormatSelect: function (combo, record) {
          const attachDisable = record.value === Asc.c_oAscFileType.HTML
          this.inputFileName.setDisabled(attachDisable)
          this.lblFileName.toggleClass("disabled", attachDisable)
          attachDisable
            ? this.textareaMessage.attr("disabled", "disabled")
            : this.textareaMessage.removeAttr("disabled")
        ...
        textWarning: "Warning!",
        textWarningMsg:
          "Please note that mailing cannot be stopped once your click the 'Send' button.",
      },
