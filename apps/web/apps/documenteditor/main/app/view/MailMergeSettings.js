...
define([
  ...
  DE.Views.MailMergeSettings = Backbone.View.extend(
    _.extend(
      {
        ...
        setApi: function (api) {
          ...
          if (this.api) {
            this.api.asc_registerCallback(
              ...
              _.bind(this.onPreviewMailMergeResult, this),
            )
            this.api.asc_registerCallback(
              "asc_onEndPreviewMailMergeResult",
              _.bind(this.onEndPreviewMailMergeResult, this),
            )
            this.api.asc_registerCallback(
              "asc_onStartMailMerge",
              _.bind(this.onStartMailMerge, this),
            )
        ...
        createDelayedControls: function () {
          ...
          this.btnInsField = new Common.UI.Button({
            parentEl: $("#mmerge-btn-ins-field", this.$el),
          ...
          this.txtFieldNum = new Common.UI.InputField({
            ...
            validation: (value) => {
              if (/(^[0-9]+$)/.test(value)) {
                ...
                if (value === undefined || value === null || value < 1) this.txtFieldNum.setValue(1)
                else if (value > this._state.recipientsCount)
                  this.txtFieldNum.setValue(this._state.recipientsCount)
              } else this.txtFieldNum.setValue(1)
            ...
            const val = Number.parseInt(this.txtFieldNum.getValue())
            if (val !== Number.parseInt(oldValue)) {
              this.api.asc_PreviewMailMergeResult(val - 1)
          ...
          this.btnEditData = new Common.UI.Button({
            el: this.$el.find("#mmerge-button-edit-data"),
          ...
          this.chHighlight = new Common.UI.CheckBox({
            el: this.$el.find("#mmerge-switcher-highlight"),
          ...
          this.chPreview = new Common.UI.CheckBox({
            el: this.$el.find("#mmerge-switcher-preview"),
          ...
          this.btnFirst = new Common.UI.Button({
            parentEl: $("#mmerge-button-first", this.$el),
            cls: "btn-toolbar",
          ...
          this.btnPrev = new Common.UI.Button({
            parentEl: $("#mmerge-button-prev", this.$el),
            cls: "btn-toolbar",
          ...
          this.btnNext = new Common.UI.Button({
            parentEl: $("#mmerge-button-next", this.$el),
            cls: "btn-toolbar",
          ...
          this.btnLast = new Common.UI.Button({
            parentEl: $("#mmerge-button-last", this.$el),
            cls: "btn-toolbar",
          ...
          this.cmbMergeTo = new Common.UI.ComboBox({
            el: $("#mmerge-combo-merge-to", this.$el),
            cls: "input-group-nr",
            style: "width: 100%;",
          ...
          this.radioCurrent = new Common.UI.RadioBox({
            el: $("#mmerge-radio-current", this.$el),
          ...
          this.radioFromTo = new Common.UI.RadioBox({
            el: $("#mmerge-radio-from-to", this.$el),
            ...
            validation: (value) => {
              if (/(^[0-9]+$)/.test(value)) {
                ...
                if (value === undefined || value === null || value < 1)
                  this.txtFieldFrom.setValue(1)
                else if (value > this._state.recipientsCount)
                  this.txtFieldFrom.setValue(this._state.recipientsCount)
              } else this.txtFieldFrom.setValue(1)
              value = Number.parseInt(this.txtFieldFrom.getValue())
              const tomax = Math.min(this._state.recipientsCount, value + 99)
              if (Number.parseInt(this.txtFieldTo.getValue()) > tomax)
                this.txtFieldTo.setValue(tomax)

              if (this._checkFromToValues) {
                if (value > Number.parseInt(this.txtFieldTo.getValue())) return this.txtFromToError

                this._checkFromToValues = false
                this.txtFieldTo.checkValidate()
              }
              return true
            },
            dataHint: "1",
          ...
          this.txtFieldTo = new Common.UI.InputField({
            ...
            validation: (value) => {
              if (/(^[0-9]+$)/.test(value)) {
                value = Number.parseInt(value)
                if (value === undefined || value === null || value < 1) this.txtFieldTo.setValue(1)
                else {
                  const tomax = Math.min(
                    this._state.recipientsCount,
                    Number.parseInt(this.txtFieldFrom.getValue()) + 99,
                  )
                  if (value > tomax) this.txtFieldTo.setValue(tomax)
                }
              } else this.txtFieldTo.setValue(1)

              if (this._checkFromToValues) {
                if (
                  Number.parseInt(this.txtFieldFrom.getValue()) >
                  Number.parseInt(this.txtFieldTo.getValue())
                )
                  return this.txtFromToError

                this._checkFromToValues = false
                this.txtFieldFrom.checkValidate()
              }
              return true
            },
            dataHint: "1",
        ...
        ChangeSettings: function (props) {
          ...
          if (props) {
            ...
            if (this.btnInsField.menu.items.length < 1) {
              _.each(this._state.fieldsList, (field, index) => {
                const mnu = new Common.UI.MenuItem({
                  caption: `«${field}»`,
                  field: field,
                }).on("click", (item, e) => {
                  if (this.api) {
                    this.api.asc_AddMailMergeField(item.options.field)
                    this.fireEvent("editcomplete", this)
                  }
                })
                this.btnInsField.menu.addItem(mnu)
              })
            }

            let numfrom = Number.parseInt(this.txtFieldFrom.getValue())
            numfrom = Number.isNaN(numfrom) ? 0 : numfrom - 1
            if (numfrom < 0 || numfrom > this._state.recipientsCount - 1) numfrom = 0

            let numto = this._isToChanged
              ? Number.parseInt(this.txtFieldTo.getValue())
              : this._state.recipientsCount
            numto = Number.isNaN(numto) ? 0 : numto - 1
            if (numto < 0 || numto > this._state.recipientsCount - 1)
              numto = Math.max(this._state.recipientsCount - 1, 0)
            ...
            let num = Number.parseInt(this.txtFieldNum.getValue())
            num = Number.isNaN(num) ? 0 : num - 1
            if (num < 0) num = 0
        ...
        onEditData: function () {
          ...
          const mergeEditor = DE.getController("Common.Controllers.ExternalMergeEditor").getView(
          ...
          if (mergeEditor) {
            ...
            const merge = this.api.asc_getMailMergeData()
            if (merge) {
        ...
        onCheckPreviewChange: function (field, newValue, eOpts) {
          const enable_preview = field.getValue() === "checked"
          const value = Number.parseInt(this.txtFieldNum.getValue())
          if (this.api) {
            enable_preview
              ? this.api.asc_PreviewMailMergeResult(Number.isNaN(value) ? 0 : value - 1)
        ...
        checkFromToValues: function () {
          ...
          const checkfrom = this.txtFieldFrom.checkValidate()
          const checkto = this.txtFieldTo.checkValidate()
        ...
        onDownloadClick: function (type, btn, e) {
          if (this.api) {
            let from = 0
            let to = Math.min(Math.max(this._state.recipientsCount - 1, 0), 99)
            if (this.radioCurrent.getValue()) {
              from = to = Number.parseInt(this.txtFieldNum.getValue()) - 1
            } else if (this.radioFromTo.getValue()) {
              if (!this.checkFromToValues()) return

              from = Number.parseInt(this.txtFieldFrom.getValue()) - 1
              to = Number.parseInt(this.txtFieldTo.getValue()) - 1
            ...
            if (!this.api.asc_DownloadAsMailMerge(this.cmbMergeTo.getValue(), from, to, type)) {
              ...
              const maincontroller = DE.getController("Main")
        ...
        onSaveMailMerge: function (url, fileType) {
          const loadMask = DE.getController("Main").loadMask
          loadMask?.hide()
          ...
          if (this.cmbMergeTo.getValue() !== Asc.c_oAscFileType.HTML) {
            const defFileName =
              this.defFileName +
              (this.cmbMergeTo.getValue() === Asc.c_oAscFileType.PDF ? ".pdf" : ".docx")
            if (this.mode.canRequestSaveAs) {
              Common.Gateway.requestSaveAs(url, defFileName, fileType)
            } else {
              this._mailMergeDlg = new Common.Views.SaveAsDlg({
                saveFolderUrl: this.mode.mergeFolderUrl,
                saveFileUrl: url,
                defFileName: defFileName,
              })
              this._mailMergeDlg
                .on("saveasfolder", (obj, folder) => {
                  // save last folder
                })
        ...
        onSetEmailAddresses: function (opts) {
          ...
          if (opts.data.error) {
            const config = {
              ...
              iconCls: "warn",
              buttons: _.isEmpty(opts.data.createEmailAccountUrl)
                ? ["ok"]
                : [{ value: "custom", caption: this.textGoToMail }, "cancel"],
        ...
        showMergeMailDlg: function () {
          if (this._mailMergeDlg) return
          this._mailMergeDlg = new DE.Views.MailMergeEmailDlg({
            props: {
          ...
          this._mailMergeDlg.on("close", (obj) => {
            this._mailMergeDlg = undefined
          })
          this._mailMergeDlg.show()
        },
        ...
        sendMergeDataByEmail: function () {
          if (this.api) {
            ...
            const mmdata = new Asc.CMailMergeSendData()
        ...
        disableFieldBtns: function (num) {
          const disabled_cmn =
            this._state.recipientsCount < 1 || this.chPreview.getValue() !== "checked"
          let disabled = disabled_cmn || num < 1
          ...
          if (num >= 0) this.txtFieldNum.setValue(num + 1)
        },

        onPreviewMailMergeResult: function (num) {
          if (this.chPreview.getValue() !== "checked") this.chPreview.setValue(true)
          this.disableFieldBtns(num)
          this.disableEditing(true)
        },

        onEndPreviewMailMergeResult: function () {
          if (this.chPreview.getValue() === "checked") this.chPreview.setValue(false)
        ...
        onCmbMergeToSelect: function (combo, record) {
          const mergeVisible = record.value === Asc.c_oAscFileType.HTML
          this.btnMerge.setVisible(mergeVisible)
          this.btnPortal.setVisible(
            !mergeVisible && (this.mode.canRequestSaveAs || this.mode.mergeFolderUrl),
          )
        ...
        disableControls: function (disable) {
          ...
          this.lockControls(DE.enumLockMM.lostConnect, disable, {
            array: _.union(
              [this.btnEditData, this.btnInsField, this.chHighlight],
              this.mode.canRequestSaveAs || this.mode.mergeFolderUrl ? [this.btnPortal] : [],
            ),
            merge: true,
          })
        },
        ...
        setDocumentName: function (name) {
          ...
          const idx = this.defFileName.lastIndexOf(".")
          if (idx > 0) this.defFileName = this.defFileName.substring(0, idx)
        },
        ...
        textSendMsg:
          "All mail messages are ready and will be sent out within some time.<br>The speed of mailing depends on your mail service.<br>" +
