...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  DE.Views.NoteSettingsDialog = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 310,
        ...
        render: function () {
          ...
          this.radioFootnote.on("change", (field, newValue, eOpts) => {
            if (newValue) {
              this.changeNoteType(false)
              setTimeout(() => {
                this.cmbFootnote.focus()
              }, 1)
            }
          })
          ...
          this.radioEndnote.on("change", (field, newValue, eOpts) => {
            if (newValue) {
              this.changeNoteType(true)
              setTimeout(() => {
                this.cmbEndnote.focus()
              }, 1)
            }
          })
          ...
          this.spnStart.on("change", (field, newValue, oldValue, eOpts) => {
            if (
              field.getNumberValue() > 1 &&
              this.cmbNumbering.getValue() !== Asc.c_oAscFootnoteRestart.Continuous
            )
          ...
          this.cmbNumbering.on("selected", (combo, record) => {
            if (
              record.value === Asc.c_oAscFootnoteRestart.EachSect ||
              record.value === Asc.c_oAscFootnoteRestart.EachPage
            )
              this.spnStart.setValue(1, true)
          })
          ...
          this.txtCustom = new Common.UI.InputField({
            ...
            this.cmbFormat.setDisabled(value.length > 0)
            this.spnStart.setDisabled(value.length > 0)
            this.btnApply.setDisabled(value.length > 0)
          })

          const arr = this.hasSections ? [{ displayValue: this.textSection, value: 0 }] : []
          ...
          this.btnApply =
            _.find(
              ...
              (item) =>
                item.$el &&
                item.$el
                  .find("#note-settings-btn-apply")
                  .addBack()
                  .filter("#note-settings-btn-apply").length > 0,
        ...
        _setDefaults: function (props) {
          this.isEndNote
            ? this.radioEndnote.setValue(true, true)
            : this.radioFootnote.setValue(true, true)
          this.changeNoteType(this.isEndNote)

          if (props) {
            let val = props.get_Pos()
        ...
        getSettings: function () {
          ...
          let val = this.txtCustom.getValue()
          if (_.isEmpty(val)) {
        ...
        _handleInput: function (state) {
          this.handler?.call(
            this,
            state,
            state === "insert" || state === "apply" ? this.getSettings() : undefined,
          )
          this.close()
        },
        ...
        onFormatSelect: function (combo, record) {
          ...
          switch (record.value) {
            ...
            case Asc.c_oAscNumberingFormat.LowerRoman: // i, ii, iii, ...
              this.spnStart.options.toCustomFormat = (value) =>
                this._10toRome(value).toLocaleLowerCase()
              this.spnStart.options.fromCustomFormat = (value) =>
                this._Rometo10(value.toLocaleUpperCase())
              break
            ...
            case Asc.c_oAscNumberingFormat.LowerLetter: // a, b, c, ...
              this.spnStart.options.toCustomFormat = (value) =>
                this._10toS(value).toLocaleLowerCase()
              this.spnStart.options.fromCustomFormat = (value) =>
                this._Sto10(value.toLocaleUpperCase())
              break
            default: // 1, 2, 3, ...
              this.spnStart.options.toCustomFormat = (value) => value
              this.spnStart.options.fromCustomFormat = (value) => value
              break
          }
        ...
        _10toS: (value) => {
          ...
          const n = Math.ceil(value / 26)
          const code = String.fromCharCode(((value - 1) % 26) + "A".charCodeAt(0))
          let result = ""
        ...
        _Sto10: (str) => {
          if (str.length < 1 || new RegExp(`[^${str.charAt(0)}]`).test(str) || !/[A-Z]/.test(str))
            return 1

          const n = str.length - 1
          let result = str.charCodeAt(0) - "A".charCodeAt(0) + 1
          result += 26 * n
        ...
        _10toRome: (value) => {
          ...
          let val = digits[0][1]
          let div = Math.floor(value / val)
          let n = 0

          for (let i = 0; i < div; i++) result += digits[n][0]
          value -= div * val
          n++
        ...
        _Rometo10: (str) => {
          ...
          const n = str.length - 1
          let result = digits[str.charAt(n)]
          let prev = result

          for (let i = n - 1; i >= 0; i--) {
            let val = digits[str.charAt(i)]
            if (val < prev) {
        ...
        changeNoteType: function (isEndNote) {
          this._state[this.isEndNote ? "endnote" : "footnote"].start =
            this.spnStart.getNumberValue() // save prev start
          this._state[this.isEndNote ? "endnote" : "footnote"].numbering =
            this.cmbNumbering.getValue() // save prev numbering
          ...
          const state = this._state[isEndNote ? "endnote" : "footnote"]
          const arr = isEndNote ? this._arrNumbering.slice(0, 2) : this._arrNumbering
