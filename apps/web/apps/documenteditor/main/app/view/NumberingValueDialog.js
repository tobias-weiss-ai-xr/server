...
define([], () => {
  DE.Views.NumberingValueDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 214,
        ...
        onFormatSelect: function (format) {
          let maskExp = /[0-9]/
          let toCustomFormat = (value) =>
            value !== ""
              ? AscCommon.IntToNumberFormat(Number.parseInt(value), this.props.format)
              : value
          ...
          switch (format) {
            ...
            case Asc.c_oAscNumberingFormat.LowerRoman: // i, ii, iii, ...
              convertValue = (value) =>
                /\D/.test(value) ? AscCommon.RomanToInt(value) : Number.parseInt(value)
            ...
            case Asc.c_oAscNumberingFormat.LowerLetter: // a, b, c, ...
              convertValue = (value) =>
                /\D/.test(value) ? AscCommon.LatinNumberingToInt(value) : Number.parseInt(value)
              maskExp = /[A-Za-z0-9]/
              break
            case Asc.c_oAscNumberingFormat.RussianLower: // а, б, в, ...
            case Asc.c_oAscNumberingFormat.RussianUpper: // А, Б, В, ...
              convertValue = (value) =>
                /\D/.test(value) ? AscCommon.RussianNumberingToInt(value) : Number.parseInt(value)
              maskExp = /[А-Яа-я0-9]/
              break
            default: // 1, 2, 3, ...
              toCustomFormat = (value) => value
          ...
          this.spnStart.options.fromCustomFormat = (value) => {
            const res = convertValue(value)
            return Number.isNaN(res) ? "1" : res.toString()
          }
          this.spnStart.on("changing", (cmp, newValue) => {
            const res = convertValue(newValue)
            if (Number.isNaN(res)) {
              cmp.setValue(1)
            }
          })
