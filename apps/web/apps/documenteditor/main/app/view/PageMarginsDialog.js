...
define([], () => {
  DE.Views.PageMarginsDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 404,
            ...
            _.bind(function (field, newValue, oldValue) {
              if (this.api) {
                this.properties = this.properties
                  ? this.properties
                  : new Asc.CDocumentSectionProps()
                this.properties.put_TopMargin(
            ...
            _.bind(function (field, newValue, oldValue) {
              if (this.api) {
                this.properties = this.properties
                  ? this.properties
                  : new Asc.CDocumentSectionProps()
                this.properties.put_BottomMargin(
                  Common.Utils.Metric.fnRecalcToMM(field.getNumberValue()),
                )
                this.api.SetDrawImagePreviewMargins("page-margins-preview", this.properties)
              }
            ...
            _.bind(function (field, newValue, oldValue) {
              if (this.api) {
                this.properties = this.properties
                  ? this.properties
                  : new Asc.CDocumentSectionProps()
                this.properties.put_LeftMargin(
                  Common.Utils.Metric.fnRecalcToMM(field.getNumberValue()),
                )
                this.api.SetDrawImagePreviewMargins("page-margins-preview", this.properties)
              }
            ...
            _.bind(function (field, newValue, oldValue) {
              if (this.api) {
                this.properties = this.properties
                  ? this.properties
                  : new Asc.CDocumentSectionProps()
                this.properties.put_RightMargin(
                  Common.Utils.Metric.fnRecalcToMM(field.getNumberValue()),
                )
                this.api.SetDrawImagePreviewMargins("page-margins-preview", this.properties)
              }
                ...
                if (this.properties.get_Orientation() !== record.value) {
                  this.properties.put_TopMargin(
                    Common.Utils.Metric.fnRecalcToMM(
                      record.value ? this.spnLeft.getNumberValue() : this.spnRight.getNumberValue(),
                    ),
                  )
                  this.properties.put_BottomMargin(
                    Common.Utils.Metric.fnRecalcToMM(
                      record.value ? this.spnRight.getNumberValue() : this.spnLeft.getNumberValue(),
                    ),
                  )
                  this.properties.put_LeftMargin(
                    Common.Utils.Metric.fnRecalcToMM(
                      record.value ? this.spnBottom.getNumberValue() : this.spnTop.getNumberValue(),
                    ),
                  )
                  this.properties.put_RightMargin(
                    Common.Utils.Metric.fnRecalcToMM(
                      record.value ? this.spnTop.getNumberValue() : this.spnBottom.getNumberValue(),
                    ),
                  )

                  const h = this.properties.get_H()
                  this.properties.put_H(this.properties.get_W())
                  this.properties.put_W(h)
                  this.properties.put_Orientation(record.value)

                  this.maxMarginsH = Common.Utils.Metric.fnRecalcFromMM(
                    this.properties.get_H() - 2.6,
                  )
                  this.maxMarginsW = Common.Utils.Metric.fnRecalcFromMM(
                    this.properties.get_W() - 12.7,
                  )
                  ...
                  this.spnTop.setValue(
                    Common.Utils.Metric.fnRecalcFromMM(this.properties.get_TopMargin()),
                    true,
                  )
                  this.spnBottom.setValue(
                    Common.Utils.Metric.fnRecalcFromMM(this.properties.get_BottomMargin()),
                    true,
                  )
                  this.spnLeft.setValue(
                    Common.Utils.Metric.fnRecalcFromMM(this.properties.get_LeftMargin()),
                    true,
                  )
                  this.spnRight.setValue(
                    Common.Utils.Metric.fnRecalcFromMM(this.properties.get_RightMargin()),
                    true,
                  )
        ...
        getFocusedComponents: function () {
          return [
            ...
            this.spnRight,
            this.spnGutter,
            this.cmbGutterPosition,
            this.cmbOrientation,
            this.cmbMultiplePages,
        ...
        _handleInput: function (state) {
          if (this.options.handler) {
            if (state === "ok") {
              let errmsg = null
              if (
                this.spnLeft.getNumberValue() +
                  this.spnRight.getNumberValue() +
                  (this.cmbGutterPosition.getValue() ? 0 : this.spnGutter.getNumberValue()) >
                this.maxMarginsW
              )
                errmsg = this.txtMarginsW
              else if (
                Math.abs(
                  this.spnTop.getNumberValue() +
                    this.spnBottom.getNumberValue() +
                    (this.cmbGutterPosition.getValue() ? this.spnGutter.getNumberValue() : 0),
                ) > this.maxMarginsH
              )
        ...
        setSettings: function (props) {
          if (props) {
            ...
            this.spnBottom.setValue(
              Common.Utils.Metric.fnRecalcFromMM(props.get_BottomMargin()),
              true,
            )
            this.spnLeft.setValue(Common.Utils.Metric.fnRecalcFromMM(props.get_LeftMargin()), true)
            this.spnRight.setValue(
              Common.Utils.Metric.fnRecalcFromMM(props.get_RightMargin()),
              true,
            )
            ...
            const mirrorMargins = props.get_MirrorMargins()
        ...
        getSettings: function () {
          ...
          props.put_GutterAtTop(!!this.cmbGutterPosition.getValue())
          props.put_MirrorMargins(!!this.cmbMultiplePages.getValue())
