...
define([], () => {
  DE.Views.PageSizeDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 215,
        ...
        render: function () {
          ...
          this.spnWidth.on(
            ...
            _.bind(function (field, newValue, oldValue, eOpts) {
              if (!this._noApply && this.cmbPreset.getValue() > -1) this.cmbPreset.setValue(-1)
            }, this),
          )
          ...
          this.cmbPreset.on(
            ...
            _.bind(function (combo, record) {
              ...
              if (record.value < 0) {
                ...
                const w = Common.Utils.Metric.fnRecalcFromMM(
                  this.isOrientPortrait ? record.size[0] : record.size[1],
                )
                const h = Common.Utils.Metric.fnRecalcFromMM(
                  this.isOrientPortrait ? record.size[1] : record.size[0],
                )
        ...
        _handleInput: function (state) {
          if (this.options.handler) {
            if (state === "ok" && this.options.checkPageSize) {
              const props = this.getSettings()
              if (this.options.checkPageSize(props[0], props[1])) return
            }
        ...
        setSettings: function (props) {
          if (props) {
            ...
            this.spnWidth.setMinValue(
              Common.Utils.Metric.fnRecalcFromMM(
                props.get_LeftMargin() + props.get_RightMargin() + 12.7,
              ),
            )
            this.spnWidth.setValue(Common.Utils.Metric.fnRecalcFromMM(props.get_W()), true)
            this.spnHeight.setMinValue(
              Common.Utils.Metric.fnRecalcFromMM(
                props.get_TopMargin() + props.get_BottomMargin() + 2.6,
              ),
            )
            this.spnHeight.setValue(Common.Utils.Metric.fnRecalcFromMM(props.get_H()), true)
            const width = this.isOrientPortrait ? props.get_W() : props.get_H()
            const height = this.isOrientPortrait ? props.get_H() : props.get_W()
            const rec = this.cmbPreset.store.find((item) => {
