...
define([
  ...
  DE.Views.RightMenu = Common.UI.SideMenu.extend(
    _.extend(
      {
        el: "#right-menu",
        ...
        render: function (mode) {
          ...
          if (
            mode?.canCoAuthoring &&
            mode.canUseMailMerge &&
            Common.UI.LayoutManager.isElementVisible("toolbar-collaboration-mailmerge")
          ) {
          ...
          if (mode?.canFeatureContentControl && mode.canEditContentControl && mode.isFormCreator) {
            ...
            this._settings[Common.Utils.documentSettingsType.Form] = {
              panel: "id-form-settings",
              btn: this.btnForm,
            }
            this.btnForm.setElement($markup.findById("#id-right-menu-form"), false)
            this.btnForm.render().setVisible(true)
          ...
          if (open) {
            $markup
              .findById("#id-paragraph-settings")
              .closest(".right-panel")
        ...
        setApi: function (api) {
          ...
          this.paragraphSettings
            .setApi(api)
            .on("editcomplete", _fire_editcomplete)
          ...
          if (this.mergeSettings)
            this.mergeSettings.setApi(api).on("editcomplete", _fire_editcomplete)
          if (this.signatureSettings)
            this.signatureSettings.setApi(api).on("editcomplete", _fire_editcomplete)
        ...
        SetActivePane: function (type, open) {
          ...
          if (this.minimizedMode) {
            ...
            const target_pane = this.$el.find(`#${this._settings[type].panel}`)
        ...
        GetActivePluginPane: function () {
          const active = this.$el.find(".plugin-panel.active")
          return this.minimizedMode || active.length === 0 ? null : active[0].id
        },

        clearSelection: function () {
          if (this.mergeSettings) this.mergeSettings.disablePreviewMode()
        ...
        setInnerWidth: function (value) {
          ...
          this.$el.css(
            "width",
            `${!Common.Utils.InternalSettings.get("de-hide-right-settings") ? MENU_SCALE_PART : SCALE_MIN}px`,
          )
        },
