...
define([], () => {
  DE.Views.RoleDeleteDlg = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 300,
        ...
        _setDefaults: function (props) {
          if (props) {
            const arr = []
            props.roles.each((item, index) => {
              const name = item.get("name")
              if (name !== props.excludeName) arr.push({ value: name, displayValue: name })
            })
        ...
        textLabel:
          "To delete this role, you  need to move the fields associated with it to another role.",
