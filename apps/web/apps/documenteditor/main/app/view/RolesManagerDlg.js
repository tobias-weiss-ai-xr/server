...
define([
  ...
  DE.Views.RolesManagerDlg = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
        ...
        render: function () {
          ...
          this.rolesList
            .on("item:select", _.bind(this.onSelectRoleItem, this))
        ...
        _setDefaults: function (props) {
          ...
          this.api.asc_registerCallback(
            ...
            this.wrapEvents.onRefreshRolesList,
          )
        },
        ...
        refreshRolesList: function (roles, selectedRole) {
          ...
          if (selectedRole === undefined && this.rolesList.store.length > 0) {
            const rec = this.rolesList.getSelectedRec()
          ...
          if (this.rolesList.store.length > 0) {
            ...
            setTimeout(() => {
              this.rolesList.scrollToRecord(rec || this.rolesList.store.at(0))
            }, 50)
          }
          this.updateButtons()
        },
        ...
        getIconCls: function (index) {
          if (this.rolesList.store.length === 1) return "Point"
          return index === 0
            ? "StartPoint"
            : index === this.rolesList.store.length - 1
              ? "EndPoint"
              : "MiddlePoint"
        },
        ...
        onDeleteRole: function () {
          const store = this.rolesList.store
          const rec = this.rolesList.getSelectedRec()

          if (!rec) return

          const callback = (toRole) => {
            this.lastSelectedRole = store.indexOf(rec)
            this.oformManager.asc_removeRole(rec.get("name"), toRole) // remove role and move it's fields
          }
          ...
          if (store.length === 1 || rec.get("fields") < 1) {
            ...
            Common.UI.warning({
              msg: Common.Utils.String.format(
                store.length === 1 ? this.textDeleteLast : this.warnDelete,
                Common.Utils.String.htmlEncode(rec.get("name")),
              ),
              maxwidth: 600,
              buttons: ["ok", "cancel"],
              callback: (btn) => {
            ...
            const xy = Common.Utils.getOffset(this.$window)
            const win = new DE.Views.RoleDeleteDlg({
        ...
        onMoveClick: function (up) {
          ...
          if (rec) {
            ...
            up
              ? this.oformManager.asc_moveUpRole(rec.get("name"))
              : this.oformManager.asc_moveDownRole(rec.get("name"))
          }
        },
        ...
        updateButtons: function () {
          ...
          this.updateMoveButtons()
          this.rolesList.scroller?.update()
        },

        updateMoveButtons: function () {
          const rec = this.rolesList.getSelectedRec()
          const index = rec ? this.rolesList.store.indexOf(rec) : -1
        ...
        onDblClickItem: function (lisvView, record, e) {
          if (!this.btnEditRole.isDisabled()) this.onEditRole(true)
        },
        ...
        textDelete: "Delete",
        textEmpty:
          "No roles have been created yet.<br>Create at least one role and it will appear in this field.",
        ...
        textDescription:
          "Add roles and set the order in which the fillers receive and sign the document",
        textAnyone: "Anyone",
        textDeleteLast:
          "Are you sure you want to delete the role {0}?<br>Once deleted, the default role will be created.",
      },
