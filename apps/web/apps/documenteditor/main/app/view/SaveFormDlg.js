...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  ...
  DE.Views.SaveFormDlg = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          alias: "SaveFormDlg",
        ...
        initialize: function (options) {
          _.extend(
            ...
            {
              title: this.txtTitle,
              buttons: [{ value: "ok", caption: this.saveButtonText }, "cancel"],
        ...
        getDefaultFocusableComponent: function () {
          return _.find(
            ...
            (item) => item.$el && item.$el.find(".primary").addBack().filter(".primary").length > 0,
          )
        },
        ...
        refreshRolesList: function (roles, selectedItem) {
          if (roles) {
            ...
            for (let i = 0; i < this.roles.length; i++) {
              ...
              const fields = role.asc_getFieldCount()
              fields > 0 &&
                arr.push({
        ...
        textDescription:
          "When saving to the pdf, only roles with fields are added to the filling list",
