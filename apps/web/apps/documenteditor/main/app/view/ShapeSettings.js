...
define([
  ...
  DE.Views.ShapeSettings = Backbone.View.extend(
    _.extend(
      {
          ...
          switch (record.value) {
            ...
            case Asc.c_oAscFill.FILL_TYPE_PATT:
              ...
              if (!this._noApply) {
                ...
                fill
                  .get_fill()
                  .put_color_bg(Common.Utils.ThemeColor.getRgbColor(this.BGColor.Color))
                props.put_fill(fill)
                this.imgprops.put_ShapeProperties(props)
                this.api.ImgApply(this.imgprops)
              }
        ...
        ChangeSettings: function (props) {
          ...
          else this.imgprops.put_ImageUrl(null)

          if (this._initSettings) this.createDelayedElements()

          let control_props = this.api.asc_IsContentControl()
          ...
          } else control_props = false

          if (props?.get_ShapeProperties()) {
            ...
            // background colors
            let rec = null
            let fill = shapeprops.get_fill()
            const fill_type = fill.get_type()
            let color = null
            let transparency = fill.get_transparent()
            if (
            ...
            if (fill === null || fill_type === null) {
              ...
              fill = fill.get_fill()
              const gradfilltype = fill.get_grad_type() // null - не совпадают у нескольких фигур
              ...
              if (this.GradFillType === Asc.c_oAscFillGradType.GRAD_LINEAR) {
                const value = Math.floor(fill.get_linear_angle() / 60000)
              ...
              const colors = fill.get_colors()
              const positions = fill.get_positions()
            ...
            // border colors
            const stroke = shapeprops.get_stroke()
            let strokeType = stroke.get_type()
            let borderType
            const update =
          ...
          this.ShapeOnlySeparator.toggleClass(
            "hidden",
            (control_props ||
              this._state.HideShapeOnlySettings ||
              this._state.HideRotationSettings) === true,
          )
        },
          ...
          if (texture && texture.length > 0) {
            ...
            _.each(texture, (item) => {
              this._texturearray.push({
                ...
                name: this.textureNames[item.get_id()],
                tip: this.textureNames[item.get_id()],
                type: item.get_id(),
                //                        allowSelected : false,
                selected: false,
              })
        ...
        onSelectShadowColor: function (picker, color) {
          const shapeProps = new Asc.asc_CShapeProperty()
          let shadowProps = this._originalProps.get_ShapeProperties().asc_getShadow()
