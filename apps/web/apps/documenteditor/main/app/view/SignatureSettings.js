...
define([
  ...
  DE.Views.SignatureSettings = Backbone.View.extend(
    _.extend(
      {
          ...
          this.viewValidList = new Common.UI.DataView({
            ...
            itemTemplate: _.template(
              [
                ...
                '<div class="caret-button <% if (name == "" || date == "") { %>' +
                  "nomargin" +
                  "<% } %> <% if (isForm) { %>" +
                  "hidden" +
        ...
        setMode: function (mode) {
          this.mode = mode
          this.$el?.find(".invisible-sign").toggleClass("hidden", !this.mode.isSignatureSupport)
        },
        ...
        updateSignatures: function (valid, requested) {
          ...
          let name_index = 1
          let hasForm = false
          let hasValidNotForm = false

          _.each(requested, (item, index) => {
            const name = item.asc_getSigner1()
            requestedSignatures.push({
              name: name !== "" ? name : `${this.strSigner} ${name_index++}`,
              guid: item.asc_getGuid(),
              requested: true,
            })
          })
          _.each(valid, (item, index) => {
            const item_date = item.asc_getDate()
            const isForm = item.asc_getIsForm()
            const sign = {
          ...
          this._state.hasValid = validSignatures.length > 0
          this._state.hasInvalid = invalidSignatures.length > 0
          this._state.hasRequested = requestedSignatures.length > 0
          ...
          this.$el.find(".requested").toggleClass("hidden", !this._state.hasRequested)
          this.$el.find(".valid").toggleClass("hidden", !this._state.hasValid)
          this.$el
            .find("#signature-valid-header")
            .closest(".valid")
            .toggleClass("hidden", !this._state.hasValid || !hasValidNotForm) // hide header if document has only form signature or has not valid signatures
          this.$el.find(".invalid").toggleClass("hidden", !this._state.hasInvalid)

          this.disableEditing(this._state.hasValid || this._state.hasInvalid)
        },

        onItemContextMenu: function (picker, item, record, e) {
          const menu = this.signatureMenu
          if (menu.isVisible()) {
            menu.hide()
          }
          if (record.get("isForm")) return

          const offsetParent = Common.Utils.getOffset($(this.el))
          const showPoint = [
            e.clientX * Common.Utils.zoom() - offsetParent.left + 5,
            e.clientY * Common.Utils.zoom() - offsetParent.top + 5,
          ]
        ...
        onSelectSignature: function (picker, item, record, e) {
          ...
          if ($(e.target).closest(".caret-button").length) {
            ...
            const offset = Common.Utils.getOffset(currentTarget)
            const offsetParent = Common.Utils.getOffset($(this.el))
            const showPoint = [
        ...
        showSignatureMenu: function (record, showPoint) {
          ...
          let menuContainer = parent.find("#menu-signature-container")
          if (!menu.rendered) {
            if (menuContainer.length < 1) {
              menuContainer = $(
                '<div id="menu-signature-container" style="position: absolute; z-index: 10000;"><div class="dropdown-toggle" data-toggle="dropdown"></div></div>',
                menu.id,
              )
          ...
          const requested = record.get("requested")
          const signed = this._state.hasValid || this._state.hasInvalid
          const signSupport = this.mode.isSignatureSupport
          const isForm = record.get("isForm")
          ...
          menu.items[2].setVisible(
            (requested || !record.get("invisible")) && signSupport && !isForm,
          )
        ...
        onMenuSignatureClick: function (menu, item) {
          const guid = menu.cmpEl.attr("data-value")
          switch (item.value) {
            case 0:
            ...
            case 2:
              Common.NotificationCenter.trigger(
                "protect:signature",
                "visible",
                !!Number.parseInt(item.cmpEl.attr("data-value")) || this._protected,
                guid,
              ) // can edit settings for requested signature
              break
            case 3: {
              Common.UI.warning({
        ...
        onDocumentReady: function () {
          ...
          this.showSignatureTooltip(
            this._state.hasValid,
            this._state.hasInvalid,
            this._state.hasRequested,
          )
        },

        showSignatureTooltip: function (hasValid, hasInvalid, hasRequested) {
          let tip = this._state.tip
          const hasForm = this._state.hasForm
          ...
          const showLink = (hasValid || hasInvalid) && !hasForm
          let tipText = ""
          if (hasForm) {
            tipText = this.txtSignedForm
          } else {
            tipText = hasInvalid ? this.txtSignedInvalid : hasValid ? this.txtSigned : ""
            if (hasRequested) tipText = `${this.txtRequestedSignatures}<br><br>${tipText}`
          }
          if (tip?.isVisible() && (tipText !== tip.text || showLink !== tip.showLink)) {
            tip.close()
            this._state.tip = undefined
          }
        ...
        onThemeChanged: function () {
          ...
          if (!this._themeChanged) {
            const marginLeft = `-${this.parentPanel.css("padding-left")}`
            const marginRight = `-${this.parentPanel.css("padding-right")}`
        ...
        strSigner: "Signer",
        txtRemoveWarning:
          "Are you sure you want to remove this signature?<br>This action cannot be undone.",
