...
define([], () => {
  DE.Views.TableFormulaDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 300,
        ...
        render: function () {
          ...
          this.cmbFunction.on(
            ...
            _.bind(function (combo, record) {
              ...
              const _input = me.inputFormula._input
              const end = _input[0].selectionEnd
              _input.val(
                _input.val().substring(0, end) +
                  record.displayValue +
                  (record.value ? "()" : "") +
                  _input.val().substring(end),
              )
              _input.focus()
              _input[0].selectionStart = _input[0].selectionEnd =
                end + record.displayValue.length + record.value
          ...
          this.cmbBookmark.on(
            ...
            _.bind(function (combo, record) {
              ...
              const end = _input[0].selectionEnd
              _input.val(
                _input.val().substring(0, end) + record.displayValue + _input.val().substring(end),
              )
              _input.focus()
        ...
        getFocusedComponents: function () {
          return [this.inputFormula, this.cmbFormat, this.cmbFunction, this.cmbBookmark].concat(
            this.getFooterButtons(),
          )
        },
        ...
        _setDefaults: function () {
          const arr = []
          _.each(this.api.asc_GetTableFormulaFormats(), (item) => {
            arr.push({ value: item, displayValue: item })
          })
          this.cmbFormat.setData(arr)
          const formula = this.api.asc_ParseTableFormulaInstrLine(this.api.asc_GetTableFormula())
        ...
        refreshBookmarks: function () {
          const arr = []
          if (this.bookmarks) {
            const count = this.bookmarks.asc_GetCount()
            for (let i = 0; i < count; i++) {
              const name = this.bookmarks.asc_GetName(i)
        ...
        getSettings: function () {
          return this.api.asc_CreateInstructionLine(
            this.inputFormula.getValue(),
            this.cmbFormat.getValue(),
          )
        },
        ...
        _handleInput: function (state) {
          if (state === "ok" && this.btnOk.isDisabled()) return
