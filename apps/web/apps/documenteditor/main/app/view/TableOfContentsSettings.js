...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  DE.Views.TableOfContentsSettings = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 500,
        ...
        initialize: function (options) {
          const height = options.type ? 300 : 375
          _.extend(
            this.options,
            {
              ...
              contentTemplate: _.template(
                [
                  ...
                  `<label class="input-label vertical-align-baseline">${this.textLeader}</label>`,
                  ...
                  '<td class="padding-small">',
                  "<% if (type == 1) { %>",
                  `<label class="input-label padding-small" style="display: block;">${this.textBuildTableOF}</label>`,
                  ...
                  "<% } else { %>",
                  `<label class="input-label padding-small" style="display: block;">${this.textBuildTable}</label>`,
                  ...
                  `<label class="input-label vertical-align-baseline">${this.textLevels}</label>`,
                  ...
                  "</td>",
                  '<td class="padding-small" style="vertical-align: top;">',
                  `<label class="input-label margin-left-10 vertical-align-baseline">${this.textStyles}</label>`,
        ...
        render: function () {
          ...
          this.chPages.on(
            ...
            _.bind(function (field, newValue, oldValue, eOpts) {
              ...
              const centered =
                this.type === 1 && this.cmbStyles.getValue() === Asc.c_oAscTOFStylesType.Centered
            ...
            this.radioStyles.on(
              ...
              _.bind(function (field, newValue, eOpts) {
                if (newValue) {
                  ...
                  if (this._needUpdateStyles) this.synchronizeLevelsFromOutline()
                  this.stylesList.scroller.update({ alwaysVisibleY: true })
                  setTimeout(() => {
                    const rec = me.stylesLevels.findWhere({ checked: true })
                    if (rec) me.stylesList.scrollToRecord(rec)
                  }, 10)
                }
              }, this),
            )
                ...
                headers: [
                  {
                    name: me.textStyle,
                    width: 144,
                    style: Common.UI.isRTL() ? "margin-right: 16px;" : "margin-left: 16px;",
                  },
                  { name: me.textLevel },
                ],
          ...
          this.cmbStyles.setValue(
            this.type === 1 ? Asc.c_oAscTOFStylesType.Current : Asc.c_oAscTOCStylesType.Current,
          )
          this.cmbStyles.on(
            "selected",
            _.bind(function (combo, record) {
              if (this.api && !this._noApply) {
                ...
                if (this.type === 1) {
                  const checked = record.value !== Asc.c_oAscTOFStylesType.Centered
        ...
        close: function () {
          this.type === 1
            ? this.api.SetDrawImagePlaceTableOfFigures(null)
        ...
        _setDefaults: function (props) {
          ...
          if (props) {
            let value = props.get_Hyperlink()
            this.chLinks.setValue(
            ...
            if (this.chPages.getValue() === "checked") {
              ...
              this.type === 1 &&
                this.cmbStyles.getValue() === Asc.c_oAscTOFStylesType.Centered &&
                this.chAlign.setValue(false)
            }
          }
          ...
          if (!props) {
            if (this.type === 1) {
              ...
              this._originalProps.put_IncludeLabelAndNumber(
                this.chFullCaption.getValue() === "checked",
              )
            } else {
          ...
          this.type === 1
            ? this.api.SetDrawImagePlaceTableOfFigures("tableofcontents-img", this._originalProps)
        ...
        initListHeaders: function () {
          const headersArr = this.stylesList.headerEl.find(".table-header-item")
          const widthHeader = this.stylesList.headerEl.width()
          const widthFirstHeader = $(headersArr[0]).width()
          const marginFirstHeader = Number.parseFloat($(headersArr[0]).css("margin-left"))

          $(headersArr[0]).addClass("hidden")
          const widthLastHeader = $(headersArr[1]).width()
        ...
        fillTOCProps: function (props) {
          const docStyles = this.api.asc_GetStylesArray()
          const styles = []
          let checkStyles = false
          _.each(docStyles, (style) => {
            const name = style.get_Name()
            const level = this.api.asc_GetHeadingLevel(name)
          ...
          if (props) {
            ...
            let end = props.get_OutlineEnd()
            const count = props.get_StylesCount()
            ...
            let disable_outlines = false
            for (let i = 0; i < count; i++) {
              const style = props.get_StyleName(i)
              const level = props.get_StyleLevel(i)
              const rec = _.findWhere(styles, { name: style })
              if (rec) {
                rec.checked = true
                rec.value = level
                if (rec.headerLevel !== level) disable_outlines = true
              } else {
                styles.push({
                  name: style,
                  displayValue: style,
            ...
            if (start > 0 && end > 0) {
              for (let i = start; i <= end; i++) {
                const rec = _.findWhere(styles, { headerLevel: i })
                if (rec) {
                  rec.checked = true
                  rec.value = i
                }
            ...
            let empty_index = -1
            for (let i = 0; i < 9; i++) {
              const rec = _.findWhere(styles, { headerLevel: i + 1 })
              if (rec) {
                const headerLevel = rec.headerLevel
                const level = rec.value
                if (headerLevel === level) {
                  if (empty_index < 1) {
                    if (new_start < 1) new_start = level
                    new_end = level
                  } else {
                    new_start = new_end = -1
                    disable_outlines = true
          ...
          styles.sort((a, b) => {
            const aname = a.displayValue.toLocaleLowerCase()
            const bname = b.displayValue.toLocaleLowerCase()
          ...
          if (checkStyles) {
            ...
            this.stylesList.scroller.update({ alwaysVisibleY: true })
            const rec = this.stylesLevels.findWhere({ checked: true })
            if (rec) this.stylesList.scrollToRecord(rec)
          }
        },
        ...
        fillTOFProps: function (props) {
          ...
          // 0 - not removable
          arr = arr.concat([
            { value: this.textEquation, displayValue: this.textEquation },
            { value: this.textFigure, displayValue: this.textFigure },
            { value: this.textTable, displayValue: this.textTable },
          ])
          arr.sort((a, b) => {
            const sa = a.displayValue.toLowerCase()
            const sb = b.displayValue.toLowerCase()
          ...
          if (props) {
            ...
            const rec = this.cmbCaptions.store.findWhere({ value: props.get_Caption() })
            rec && (value = rec.get("value"))
          }
          arr.length > 0 && this.cmbCaptions.setValue(value ? value : this.textFigure)

          arr = []
          _.each(this.api.asc_getAllUsedParagraphStyles(), (style, index) => {
            arr.push({
              displayValue: style.get_TranslatedName(),
          ...
          if (props) {
            const count = props.get_StylesCount()
            if (count > 0) {
              const rec = this.cmbTOFStyles.store.findWhere({ styleName: props.get_StyleName(0) })
              rec && (value = rec.get("value"))
            }
          }
        ...
        synchronizeLevelsFromOutline: function () {
          const start = 1
          const end = this.spnLevels.getNumberValue()
          this.stylesLevels.each((style) => {
            const header = style.get("headerLevel")
        ...
        synchronizeLevelsFromStyles: function (reset) {
          ...
          for (let i = 0; i < 9; i++) {
            const rec = this.stylesLevels.findWhere({ headerLevel: i + 1 })
            if (rec) {
              const headerLevel = rec.get("headerLevel")
              const level = rec.get("value")
              if (headerLevel === level) {
                if (empty_index < 1) {
                  if (new_start < 1) new_start = level
                  new_end = level
        ...
        getSettings: function () {
          ...
          if (this.type === 1) {
            if (this.radioCaption.getValue()) {
              ...
              const rec = this.cmbTOFStyles.getSelectedRecord()
            ...
            if (!this._needUpdateStyles)
              // if this._needUpdateStyles==true - fill only OutlineRange
              this.stylesLevels.each((style) => {
                if (style.get("checked")) props.add_Style(style.get("name"), style.get("value"))
              })
        ...
        addEvents: function (listView, itemView, record) {
          const input = itemView.$el.find("input")
          input.on("keypress", (e) => {
          ...
          input.on("input", (e) => {
            ...
            const isEmpty = _.isEmpty(input.val())
            if (record.get("checked") !== !isEmpty) {
              record.set("checked", !isEmpty)
            }
            record.set("value", isEmpty ? "" : Number.parseInt(input.val()))
            this._needUpdateOutlineLevels = true
            ...
            if (this.api && !this._noApply) {
              ...
              if (properties.get_StylesCount() > 0) properties.put_OutlineRange(-1, -1)
              else properties.put_OutlineRange(1, 9)
              this.api.SetDrawImagePlaceContents("tableofcontents-img", properties)
              this.scrollerY.update()
            }
          })
        ...
        onItemChange: function (listView, itemView, record) {
          ...
          const inp = itemView.$el.find("input")
          setTimeout(() => {
            inp.focus()
        ...
        disableButtons: function () {
          this.type &&
            this.btnOk.setDisabled(
              (this.radioCaption.getValue() && this.cmbCaptions.getValue() === null) ||
                (this.radioStyle.getValue() && this.cmbTOFStyles.length < 1),
            )
        },
