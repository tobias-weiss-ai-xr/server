...
define([
  ...
  DE.Views.TableSettings = Backbone.View.extend(
    _.extend(
      {
        ...
        onCheckTemplateChange: function (type, field, newValue, oldValue, eOpts) {
          if (this.api) {
            ...
            const look = this._originalLook ? this._originalLook : new Asc.CTablePropLook()
            switch (type) {
              case 0:
        ...
        setApi: function (o) {
          ...
          if (o) {
            this.api.asc_registerCallback(
              ...
              _.bind(this.onInitTableTemplates, this),
            )
            this.api.asc_registerCallback(
              "asc_onBeginTableStylesPreview",
              _.bind(this.onBeginTableStylesPreview, this),
            )
            this.api.asc_registerCallback(
              "asc_onAddTableStylesPreview",
              _.bind(this.onAddTableStylesPreview, this),
            )
            this.api.asc_registerCallback(
              "asc_onEndTableStylesPreview",
              _.bind(this.onEndTableStylesPreview, this),
            )
          }
          return this
        },
        ...
        createDelayedControls: function () {
          ...
          this.btnDistributeRows = new Common.UI.Button({
            parentEl: $("#table-btn-distrub-rows", this.$el),
            cls: "btn-toolbar",
          ...
          this.btnDistributeCols = new Common.UI.Button({
            parentEl: $("#table-btn-distrub-cols", this.$el),
            cls: "btn-toolbar",
        ...
        ChangeSettings: function (props) {
          ...
          if (props) {
            ...
            let value = props.get_ColumnWidth()
            if (
              ((this._state.Width === undefined || value === undefined) &&
                this._state.Width !== value) ||
              Math.abs(this._state.Width - value) > 0.001
            ) {
              this.numWidth.setValue(
                value !== null && value !== undefined
                  ? Common.Utils.Metric.fnRecalcFromMM(value)
                  : "",
                true,
              )
              this._state.Width = value
            ...
            if (
              ((this._state.Height === undefined || value === undefined) &&
                this._state.Height !== value) ||
              Math.abs(this._state.Height - value) > 0.001
            ) {
              this.numHeight.setValue(
                value !== null && value !== undefined
                  ? Common.Utils.Metric.fnRecalcFromMM(value)
                  : "",
                true,
            ...
            if (this._state.TemplateId !== value || this._isTemplatesChanged) {
              ...
              const template = this.api.asc_getTableStylesPreviews(false, [this._state.TemplateId])
              if (template && template.length > 0)
                this.$el.find(".icon-template-table").css({
                  "background-image": `url(${template[0].asc_getImage()})`,
                  height: "52px",
                  width: "72px",
            ...
            const look = props.get_TableLook()
            if (look) {
            ...
            // background colors
            const background = props.get_CellsBackground()
            if (background) {
            ...
            const type1 = typeof this.CellColor.Color
            const type2 = typeof this._state.BackColor
            if (
              type1 !== type2 ||
              (type1 === "object" &&
                (this.CellColor.Color.effectValue !== this._state.BackColor.effectValue ||
                  this._state.BackColor.color.indexOf(this.CellColor.Color.color) < 0)) ||
        ...
        _UpdateBordersStyle: function (border) {
          ...
          const updateBorders = this.CellBorders
        ...
        _UpdateBorderStyle: function (border, visible) {
          ...
          if (visible && this.BorderSize > 0) {
            const size = Number.parseFloat(this.BorderSize)
            border.put_Value(1)
        ...
        selectCurrentTableStyle: function () {
          ...
          let rec = this.mnuTableTemplatePicker.store.findWhere({
        ...
        onBeginTableStylesPreview: function (count) {
          ...
          this._state.groups = {
            "menu-table-group-custom": {
              id: "menu-table-group-custom",
              caption: this.txtGroupTable_Custom,
              index: 0,
              templateCount: 0,
            },
            "menu-table-group-plain": {
              id: "menu-table-group-plain",
              caption: this.txtGroupTable_Plain,
              index: 1,
              templateCount: 0,
            },
            "menu-table-group-grid": {
              id: "menu-table-group-grid",
              caption: this.txtGroupTable_Grid,
              index: 2,
              templateCount: 0,
            },
            "menu-table-group-list": {
              id: "menu-table-group-list",
              caption: this.txtGroupTable_List,
              index: 3,
              templateCount: 0,
            },
            "menu-table-group-bordered-and-lined": {
              id: "menu-table-group-bordered-and-lined",
              caption: this.txtGroupTable_BorderedAndLined,
              index: 4,
              templateCount: 0,
            },
            "menu-table-group-no-name": {
              id: "menu-table-group-no-name",
              caption: "&nbsp",
              index: 5,
              templateCount: 0,
            },
        ...
        onEndTableStylesPreview: function () {
          ...
          if (this.mnuTableTemplatePicker) {
            ...
            if (this.mnuTableTemplatePicker.isVisible())
              this.mnuTableTemplatePicker.scrollToRecord(
                this.mnuTableTemplatePicker.getSelectedRec(),
              )
          }
        },
          ...
          _.each(Templates, (template) => {
            ...
            if (template.asc_getType() === 0) {
              ...
              ;[
                ...
                const str = `txtTable_${item.replace("&", "And").replace(/ /g, "")}`
                if (this[str]) tip = tip.replace(item, this[str])
              })
            } else {
            ...
            for (const group in this._state.groups) {
              if (this._state.groups[group].index <= this._state.groups[groupItem].index) {
                templateIndex += this._state.groups[group].templateCount
              }
            }

            if (this._state.beginPreviewStyles) {
              this._state.beginPreviewStyles = false
              this.mnuTableTemplatePicker?.groups.reset(this._state.groups[groupItem])
              this.mnuTableTemplatePicker?.store.reset(templateObj)
              this.mnuTableTemplatePicker.groups.comparator = (item) => item.get("index")
            } else {
              if (this._state.groups[groupItem].templateCount === 0) {
                this.mnuTableTemplatePicker?.groups.add(this._state.groups[groupItem])
              }
              this.mnuTableTemplatePicker?.store.add(templateObj, { at: templateIndex })
            }

            this._state.groups[groupItem].templateCount += 1
          })
          !this._state.currentStyleFound && this.selectCurrentTableStyle()
        },
        ...
        _onInitTemplates: function () {
          ...
          if (!this.btnTableTemplate) {
            this.btnTableTemplate = new Common.UI.Button({
              ...
              menu: new Common.UI.Menu({
                ...
                items: [
                  {
                    template: _.template(
                      '<div id="id-table-menu-template" class="menu-table-template"></div>',
                    ),
                  },
                ],
              }),
            ...
            this.btnTableTemplate.on("render:after", (btn) => {
              this.mnuTableTemplatePicker = new Common.UI.DataView({
                ...
                itemTemplate: _.template(
                  '<div id="<%= id %>" class="item"><img src="<%= imageUrl %>" height="52" width="72"></div>',
                ),
            ...
            this.btnTableTemplate.cmpEl.find(".icon-template-table").css({
              height: "52px",
              width: "72px",
              "background-position": "center",
              "background-size": "auto 52px",
            ...
            this.mnuTableTemplatePicker.on(
              "item:click",
              _.bind(this.onTableTemplateSelect, this, this.btnTableTemplate),
            )
          }
        ...
        onAddFormula: function (e) {
          ...
          if (this.api && !this._locked) {
            new DE.Views.TableFormulaDialog({
              api: this.api,
              bookmarks: this.api.asc_GetBookmarksManager(),
              handler: (result, value) => {
                if (result === "ok") {
                  if (this.api) {
                    this.api.asc_AddTableFormula(value)
                  }
                }
                this.fireEvent("editcomplete", this)
              },
            ...
            new DE.Views.TableToTextDialog({
              handler: (dlg, result) => {
                if (result === "ok") {
                  ...
                  this.api.asc_ConvertTableToText(
                    settings.type,
                    settings.separator,
                    settings.nested,
                  )
