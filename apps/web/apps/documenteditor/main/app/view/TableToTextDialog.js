...
define([], () => {
  DE.Views.TableToTextDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 300,
        ...
        getSettings: function () {
          const type = this.rbPara.getValue() ? 1 : this.rbTabs.getValue() ? 2 : 3
          const separator = this.rbSemi.getValue()
            ? ";".charCodeAt(0)
            : this.rbOther.getValue()
              ? this.inputOther.getValue().charCodeAt(0)
              : undefined

          return {
            type: type,
            separator: separator,
            nested: this.chNested.getValue() === "checked",
          }
        },
