...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  DE.Views.TextToTableDialog = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 300,
        ...
        render: function () {
          ...
          this.spnColumns = new Common.UI.MetricSpinner({
            ...
            if (this.props) {
              const size = this.props.put_ColsCount(field.getNumberValue(), true)
              size && this.spnRows.setValue(size[0], true)
            }
          })
          ...
          }).on("changing", (input, newValue) => {
            if (this.props && newValue) {
              this.props.put_SeparatorType(3, true)
              const size = this.props.put_Separator(newValue.charCodeAt(0), true)
              if (size) {
        ...
        _setDefaults: function (props) {
          if (props) {
            let val = props.get_Size()
            this.spnColumns.setValue(val[1], true)
            this.spnRows.setValue(val[0], true)

            val = props.get_AutoFitType()
            val === 1
              ? this.rbFixed.setValue(true, true)
              : val === 2
                ? this.rbContents.setValue(true, true)
                : this.rbWindow.setValue(true, true)
            val === 1 &&
              this.spnWidth.setValue(
                props.get_Fit() > -1 ? Common.Utils.Metric.fnRecalcFromMM(props.get_Fit()) : -1,
                true,
              )
        ...
        getSettings: function () {
          if (this.props) {
            this.props.put_AutoFitType(
              this.rbFixed.getValue() ? 1 : this.rbContents.getValue() ? 2 : 3,
            )
            this.rbFixed.getValue() &&
              this.props.put_Fit(Common.Utils.Metric.fnRecalcToMM(this.spnWidth.getNumberValue()))
            this.rbOther.getValue() &&
              Common.Utils.InternalSettings.set(
                "de-text-to-table-separator",
                String.fromCharCode(this.props.get_Separator()),
              )
          }
        ...
        onRadioAutofitChange: function (field, newValue, eOpts) {
          ...
          if (newValue && this.props) {
            this.spnWidth.setDisabled(value !== 1)
            value === 1 &&
              setTimeout(() => {
                this.spnWidth.focus()
              }, 1)
