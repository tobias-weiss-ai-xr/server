...
define([
  ...
  DE.Views.ViewTab = Common.UI.BaseView.extend(
    _.extend(
      (() => {
        const template =
        ...
        return {
          ...
          setEvents: function () {
            const me = this
            me.btnNavigation.on("click", (btn, e) => {
              me.fireEvent("viewtab:navigation", [btn.pressed])
            })
            me.btnsFitToPage.forEach((btn) => {
              btn.on("click", () => {
                me.fireEvent("zoom:topage", [btn])
              })
            })
            me.btnsFitToWidth.forEach((btn) => {
              btn.on("click", () => {
            ...
            me.btnDarkDocument.on(
              "click",
              _.bind((e) => {
                me.fireEvent("darkmode:change", [e.pressed])
              }, me),
            )
            me.btnMultiplePages.on(
              "click",
              _.bind((e) => {
            ...
            me.btnMacros?.on("click", () => {
              me.fireEvent("macros:click")
            })
            me.btnRecMacro?.on("click", () => {
              me.fireEvent("macros:record")
            })
            me.btnPauseMacro?.on("click", () => {
          ...
          getPanel: function () {
            ...
            this.btnSelectTool?.render($host.find("#slot-btn-select-tool-view"))
            this.btnHandTool?.render($host.find("#slot-btn-hand-tool-view"))
            this.btnRecMacro?.render($host.find("#slot-btn-macro-start"))
            this.btnPauseMacro?.render($host.find("#slot-btn-macro-pause"))

            if (this.toolbar?.$el) {
              this.btnsFitToPage = Common.Utils.injectButtons(
                this.toolbar.$el.find(".slot-btn-ftp"),
                "tlbtn-btn-ftp-",
                "toolbar__icon btn-ic-zoomtopage",
                this.textFitToPage,
                ...
                "left",
                "medium",
              ).concat(this.btnsFitToPage)
              this.btnsFitToWidth = Common.Utils.injectButtons(
                this.toolbar.$el.find(".slot-btn-ftw"),
                "tlbtn-btn-ftw-",
                "toolbar__icon btn-ic-zoomtowidth",
                this.textFitToWidth,
                [Common.enumLock.lostConnect, Common.enumLock.disableOnStart],
                false,
                false,
            ...
            const created = this.btnsFitToPage.concat(this.btnsFitToWidth).concat(this.cmbsZoom)
          ...
          onAppReady: function (config) {
            ...
            this.btnsFitToPage.forEach((btn) => {
              btn.updateHint(this.tipFitToPage)
            })
            this.btnsFitToWidth.forEach((btn) => {
              btn.updateHint(this.tipFitToWidth)
            })
            this.btnMacros?.updateHint(this.tipMacros)
            this.btnRecMacro?.updateHint(this.tipRecMacro)
            this.btnPauseMacro?.updateHint(this.tipPauseMacro)
            ...
            if (config.isEdit || config.isRestrictedEdit) {
              ...
              this.btnSelectTool?.updateHint(this.toolbar.tipSelectTool)
              this.btnHandTool?.updateHint(this.toolbar.tipHandTool)
            }
          },
          ...
          turnNavigation: function (state) {
            this.btnNavigation?.toggle(state, true)
          },
