...
define([
  ...
  DE.Views.WatermarkText = new (function () {
    ...
    const _load = (callback) => {
      langs = []
      Common.Utils.loadConfig("resources/watermark/wm-text.json", (langJson) => {
        for (const lang in langJson) {
          const val = Common.util.LanguageInfo.getLocalLanguageCode(lang)
          if (val) {
            langs.push({
              code: val,
  ...
  DE.Views.WatermarkSettingsDialog = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 405,
        ...
        initialize: function (options) {
          ...
          this.lang = val
            ...
            : { value: "en", displayValue: "English", default: true }
        ...
        getDefaultFocusableComponent: function () {
          if (!this.cmbLang.isDisabled()) return this.cmbLang
          if (!this.btnSelectImage.isDisabled()) return this.btnSelectImage

          return this.radioNone
        },

        focusControls: function () {
          const el = this.getDefaultFocusableComponent()
          el &&
            setTimeout(() => {
              el.focus()
            }, 10)
        ...
        afterRender: function () {
          ...
          const insertImageFromStorage = (data) => {
            if (data?._urls && data.c === "watermark") {
              this.props.put_ImageUrl(data._urls[0], data.token)
            }
          }
          Common.NotificationCenter.on("storage:image-insert", insertImageFromStorage)

          this.on("close", (obj) => {
            this.api.asc_unregisterCallback("asc_onWatermarkImageLoaded", onApiWMLoaded)
        ...
        loadLanguages: function () {
          ...
          const callback = (languages) => {
            ...
            if (data.length) {
              ...
              const res = this.loadWMText(this.lang.value)
              if (res && (this.lang.default || res.foundSameLang)) this.cmbLang.setValue(res.value)
              else this.cmbLang.setValue(this.lang.displayValue)
              this.cmbLang.setDisabled(!this.radioText.getValue())
              this.text && this.cmbText.setValue(this.text)
            } else this.cmbLang.setDisabled(true)
            focus && this.focusControls()
          }
          const languages = DE.Views.WatermarkText.get()
          focus = !languages
          if (languages) callback(languages)
          else DE.Views.WatermarkText.load(callback)
        },
        ...
        loadWMText: function (lang) {
          ...
          const data = []
          let foundSameLang = true
          let item = this.cmbLang.store.findWhere({ value: lang })
          if (!item) item = this.cmbLang.store.findWhere({ value: lang.split(/[\-\_]/)[0] })
          if (!item) {
            foundSameLang = false
            item = this.cmbLang.store.findWhere({ value: "en" })
          }
          if (!item) item = this.cmbLang.store.at(0)

          item?.get("wmtext").forEach((item) => {
            data.push({ value: item })
          })
          if (data.length > 0) {
            this.cmbText.setData(data)
              ...
              handler: (result, value) => {
                if (result === "ok") {
                  ...
                  if (!_.isEmpty(checkUrl)) {
                    this.props.put_ImageUrl(checkUrl)
                  }
                }
              },
            })
              .on("close", () => {
                this.btnSelectImage.focus()
              })
              .show()
        ...
        _setDefaults: function (props) {
          ...
          if (props) {
            ...
            if (type === Asc.c_oAscWatermarkType.None) {
              ...
              this.cmbScale.setValue(
                val < 0 ? -1 : Math.round(val * 100),
                `${Math.round(val * 100)} %`,
              )
            } else {
              ...
              if (val) {
                const lang = Common.util.LanguageInfo.getLocalLanguageName(val.get_Lang())
                this.lang = { value: lang[0], displayValue: lang[1] }
                let langKey = lang[0]
                const cmbLangLi = this.cmbLang.store.findWhere({ value: langKey })
                if (!cmbLangLi) langKey = langKey.split(/[\-\_]/)[0]
                this.cmbLang.setValue(langKey, lang[1])
                this.loadWMText(lang[0])

                const font = val.get_FontFamily().get_Name()
                if (font) {
        ...
        getSettings: function () {
          ...
          let val = this.props.get_Type()
          if (val === Asc.c_oAscWatermarkType.Image) {
            val = Number.parseInt(this.cmbScale.getValue())
            ...
            if (val) {
              ...
              const font = new AscCommon.asc_CTextFontFamily()
              ...
              val.put_Lang(
                Number.parseInt(Common.util.LanguageInfo.getLocalLanguageCode(this.lang.value)),
              )
        ...
        disableControls: function (type) {
          // 0 - none, 1 - text, 2 - image
          let disable = type !== Asc.c_oAscWatermarkType.Image
          _.each(this.imageControls, (item) => {
            item.setDisabled(disable)
          })

          disable = type !== Asc.c_oAscWatermarkType.Text
          _.each(this.textControls, (item) => {
            item.setDisabled(disable)
          })
