...
;(function () {
  ...
  lunr.Pipeline.prototype.run = function (tokens) {
    ...
    for (let i = 0; i < stackLength; i++) {
      ...
      tokens = tokens.reduce((memo, token, j) => {
        const result = fn(token, j, tokens)
  ...
  lunr.Pipeline.prototype.runString = function (str) {
    const token = new lunr.Token(str)
  ...
  lunr.Index.prototype.query = function (fn) {
      ...
      for (let m = 0; m < terms.length; m++) {
        ...
        for (let j = 0; j < expandedTerms.length; j++) {
          ...
          for (let k = 0; k < clause.fields.length; k++) {
             ...
             * together.
             */
            queryVectors[field].upsert(termIndex, 1 * clause.boost, (a, b) => a + b)
    ...
    const matchingFieldRefs = Object.keys(matchingFields)
    const results = []
    ...
    for (let i = 0; i < this._fields.length; i++) {
      ...
      for (let j = 0; j < terms.length; j++) {
        ...
        if (this.invertedIndex[term] === undefined) {
          ...
          for (let k = 0; k < this._fields.length; k++) {
  ...
  lunr.Builder.prototype.createFieldVectors = function () {
    ...
    for (let i = 0; i < fieldRefsLength; i++) {
      ...
      for (let j = 0; j < termsLength; j++) {
        ...
        score =
          (idf * ((this._k1 + 1) * tf)) /
          (this._k1 * (1 - this._b + this._b * (fieldLength / this.averageFieldLength[field])) + tf)
  ...
  lunr.QueryParser.prototype.parse = function () {
    ...
    let state = lunr.QueryParser.parseFieldOrTerm
  ...
  lunr.QueryParser.parseFieldOrTerm = (parser) => {
