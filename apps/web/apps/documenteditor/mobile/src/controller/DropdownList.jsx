...
class DropdownListController extends Component {
  ...
  initDropdownList(obj) {
    ...
    this.specProps = this.isComboBox
      ? this.propsObj.get_ComboBoxPr()
      : this.propsObj.get_DropDownListPr()
  ...
  initListItems() {
    ...
    if (this.formProps) {
      if (!this.formProps.get_Required() || count < 1) {
        // for required or empty dropdown/combobox form control always add placeholder item
        const text = this.propsObj.get_PlaceholderText()
  ...
  onChangeItemList(value) {
    ...
    if (value !== -1) {
      ...
      if (this.isCorePDF === true) api.asc_SelectPDFFormListItem(value)
