...
class MainController extends Component {
  constructor(props) {
    ...
    ;[
      ...
      this.fallbackSdkTranslations[item] = item
    })
  ...
  initSdk() {
    const on_script_load = () => {
      ...
      const loadConfig = (data) => {
        ...
        this.props.storeApplicationSettings.changeMacrosRequest(
        ...
        if (this.editorConfig.canRequestRefreshFile) {
          ...
          this.api.asc_registerCallback(
            ...
            this.onRequestRefreshFile.bind(this),
          )
        }
      }
      ...
      const loadDocument = (data) => {
        ...
        this.api.asc_registerCallback(
          ...
          this.onMacrosPermissionRequest.bind(this),
        )
        this.api.asc_registerCallback(
          "asc_onRunAutostartMacroses",
          this.onRunAutostartMacroses.bind(this),
        )
        ...
        if (data.doc) {
          ...
          if (data.doc.info) {
            data.doc.info.author &&
              console.log(
                "Obsolete: The 'author' parameter of the document 'info' section is deprecated. Please use 'owner' instead.",
              )
            data.doc.info.created &&
              console.log(
                "Obsolete: The 'created' parameter of the document 'info' section is deprecated. Please use 'uploaded' instead.",
              )
          }
        }
      }
      ...
      const onEditorPermissions = (params) => {
        ...
        if (
          ...
          f7.dialog
            .create({
              title:
                Asc.c_oLicenseResult.NotBefore === licType
                  ? _t.titleLicenseNotActive
                  : _t.titleLicenseExp,
              text:
                Asc.c_oLicenseResult.NotBefore === licType
                  ? _t.warnLicenseBefore
                  : _t.warnLicenseExp,
            })
            .open()
        ...
        if (
          customization?.mobileForceView !== undefined &&
          customization?.mobileForceView !== null
        ) {
          console.warn(
            "Obsolete: The mobileForceView parameter is deprecated. Please use the forceView parameter from customization.mobile block",
          )
        }

        storeAppOptions.setPermissionOptions(
          this.document,
          licType,
          params,
          this.permissions,
      ...
      const onDocumentContentReady = () => {
        ...
        if (appOptions.canFillForms) {
          this.api.asc_registerCallback(
            ...
            this.showSignatureTooltip.bind(this),
          )
        }

        Common.Notifications.trigger("preloader:close")
        Common.Notifications.trigger(
          "preloader:endAction",
          Asc.c_oAscAsyncActionType.BlockInteraction,
          this.LoadingDocument,
        )

        appOptions.isRestrictedEdit &&
        ...
        const zf =
          value !== null
            ...
            : customization?.zoom
              ? Number.parseInt(customization.zoom)
              : 100
        zf === -1
        ...
        value = LocalStorage.getBool(
          "de-mobile-spellcheck",
          !(customization && customization.spellcheck === false),
        )
        ...
        value = LocalStorage.itemExists("mobile-view")
          ...
          : !(customization?.mobile?.standardView ?? false)
              ...
              buttons: [
                {
                  ...
                  onClick: () => {
                    this.api.asc_DownloadAs(
                      new Asc.asc_CDownloadOptions(Asc.c_oAscFileType.PDF, false),
                    )
                  },
                },
                {
        ...
        if (appOptions.isRestrictedEdit && appOptions.canFillForms && appOptions.isForm) {
      ...
      processArrayScripts(dep_scripts, promise_get_script).then(
        () => {
          ...
          if (
            !(
              typeof _translate === "object" &&
              _translate !== null &&
              Object.keys(_translate).length > 0
            )
          ) {
          ...
          Common.Gateway.on("internalcommand", (data) => {
            if (data.command === "hardBack") {
              ...
              } else Common.Gateway.internalMessage("hardBack", true)
            }
          })
  ...
  applyMode(appOptions) {
    ...
    this.appOptions.isCorePDF &&
      this.api.asc_setPdfViewer(!appOptions.isEdit && !appOptions.isRestrictedEdit)
    appOptions.isRestrictedEdit &&
      appOptions.canComments &&
    ...
    value =
      value !== null
        ? Number.parseInt(value)
        : appOptions.customization?.unit
          ? Common.Utils.Metric.c_MetricUnits[appOptions.customization.unit.toLocaleLowerCase()]
    ...
    this.api.asc_registerCallback(
      ...
      this.onDocumentModifiedChanged.bind(this),
    )
    this.api.asc_registerCallback(
      "asc_onDocumentCanSaveChanged",
      this.onDocumentCanSaveChanged.bind(this),
    )
  ...
  onLicenseChanged(params) {
    ...
    if (
      licType !== undefined &&
      (appOptions.canEdit || appOptions.isRestrictedEdit) &&
      ...
      (licType === Asc.c_oLicenseResult.Connections ||
        ...
        (licType === Asc.c_oLicenseResult.SuccessLimit &&
          (appOptions.trialMode & Asc.c_oLicenseMode.Limited) !== 0))
    )
    ...
    if (this._isDocReady && this._state.licenseType) this.applyLicense()
  }
  ...
  applyLicense() {
    ...
    if (appOptions.config.mode === "view") {
      ...
      if (
        ...
        license =
          license === Asc.c_oLicenseResult.Connections
            ? _t.tipLicenseExceeded
            : _t.tipLicenseUsersExceeded
      } else {
        license =
          license === Asc.c_oLicenseResult.ConnectionsOS ? warnNoLicense : warnNoLicenseUsers
        buttons = [
          {
            text: _t.textBuyNow,
            bold: true,
      ...
      if (
        !appOptions.isDesktopApp &&
        !appOptions.canBrandingExt &&
        appOptions.config &&
        appOptions.config.customization &&
        (appOptions.config.customization.loaderName || appOptions.config.customization.loaderLogo)
      ) {
        f7.dialog
          .create({
  ...
  onServerVersion(buildVersion) {
    ...
    if (About.appVersion() !== buildVersion && !About.compareVersions()) {
      ...
      f7.dialog.alert(_t.errorServerVersion, _t.titleServerVersion, () => {
        setTimeout(() => {
  ...
  bindEvents() {
    ...
    $$(window).on(
      "popover:open popup:open sheet:open actions:open dialog:open searchbar:enable",
      () => {
    ...
    if (this.appOptions.isCorePDF) {
      ...
      this.api.asc_registerCallback("asc_onHidePdfFormsActions", () => {
        if (this.cmpCalendar?.opened) this.cmpCalendar.close()
      })
    }
    ...
    this.api.asc_registerCallback("asc_onShowContentControlsActions", (obj, x, y) => {
      ...
      if (
        (!storeAppOptions.isEdit &&
          !(storeAppOptions.isRestrictedEdit && storeAppOptions.canFillForms)) ||
        this.props.users.isDisconnected ||
        (isViewer && !isForm)
      )
        return
    ...
    this.api.asc_registerCallback("asc_onTextHighLight", (color) => {
      const textPr = this.api.get_TextProps().get_TextPr()
    ...
    //chart settings
    EditorUIController.updateChartStyles?.(
      this.props.storeChartSettings,
      this.props.storeFocusObjects,
    )
    ...
    this.api.asc_registerCallback(
      ...
      (type, advOptions, mode, formatOptions) => {
        const { t } = this.props
        ...
        if (type === Asc.c_oAscAdvancedOptionsID.DRM) {
          ...
          onAdvancedOptions(
            ...
            _t,
            this._isDocReady,
            this.props.storeAppOptions.canRequestClose,
            this.isDRM,
          )
          this.isDRM = true
        }
    ...
    this.api.asc_registerCallback(
      ...
      this.onChangeProtectDocument.bind(this),
    )
  ...
  insertImageFromStorage(data) {
    if (data?._urls && (!data.c || data.c === "add") && data._urls.length > 0) {
  ...
  onChangeProtectDocument(userId) {
    ...
    if (isProtected) {
      ...
      if (
        (recUser &&
          recUser.asc_getIdOriginal() !== this.props.users.currentUser.asc_getIdOriginal()) ||
        this._state.forceDocProtect
      ) {
        this._state.protectWarn?.close()
        this._state.protectWarn = f7.dialog
          .create({
  ...
  onApiTextReplaced(found, replaced) {
    ...
    if (found) {
      f7.dialog.alert(
        null,
        !(found - replaced > 0)
          ? t("Main.textReplaceSuccess").replace(/\{0\}/, `${replaced}`)
          : t("Main.textReplaceSkipped").replace(/\{0\}/, `${found - replaced}`),
      )
    } else {
  ...
  onShowDateActions(obj, x, y) {
    ...
    if (!this.appOptions.isCorePDF) {
      ...
      if (specProps) val = specProps.get_FullDate()
    } else {
      if (obj) val = obj.asc_GetValue()
    }
  ...
  onShowImageActions(obj, x, y) {
    if (!Device.isPhone) {
      ...
      if (dropdownListTarget.length < 1) {
        dropdownListTarget = $$(
          '<div id="dropdown-image-list-target" style="position: absolute;"></div>',
        )
  ...
  onShowListActions(obj, x, y) {
    ...
    if (!this.appOptions.isCorePDF) Common.Notifications.trigger("openDropdownList", obj)
  ...
  onUpdateVersion(callback) {
    ...
    Common.Notifications.trigger(
      "preloader:endAction",
      Asc.c_oAscAsyncActionType.BlockInteraction,
      Asc.c_oAscAsyncAction.Open,
    )

    f7.dialog.alert(_t.errorUpdateVersion, _t.titleUpdateVersion, () => {
      Common.Gateway.updateVersion()
      if (callback) {
        callback.call(this)
      }
  ...
  onPrint() {
    ...
    if (this.api) this.api.asc_Print()
