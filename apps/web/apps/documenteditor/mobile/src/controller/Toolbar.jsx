...
const ToolbarController = inject(
  "storeAppOptions",
  "users",
  "storeReview",
  "storeFocusObjects",
  ...
  observer((props) => {
    ...
    const stateDisplayMode = !!(displayMode === "final" || displayMode === "original")
    const displayCollaboration =
      props.users.hasEditUsers ||
      appOptions.canViewComments ||
      appOptions.canReview ||
      appOptions.canViewReview
    ...
    const showEditDocument =
      !appOptions.isEdit && appOptions.canEdit && appOptions.canRequestEditRights
    ...
    const scrollHandler = (offset) => {
      const api = Common.EditorApi.get()
      const navbarHeight = getNavbarTotalHeight()
      const isSearchbarEnabled = document
        .querySelector(".subnavbar .searchbar")
        ?.classList.contains("searchbar-enabled")
    ...
    const loadConfig = (data) => {
      if (
        data?.config &&
        data.config?.canBackToFolder !== false &&
        data.config?.customization &&
        data.config?.customization.goback
      ) {
        const canback =
          data.config.customization.close === undefined
            ? data.config.customization.goback.url ||
              (data.config.customization.goback.requestClose && data.config.canRequestClose)
    ...
    const saveForm = () => {
      ...
      const isSavePdf =
        appOptions.canDownload && appOptions.canFillForms && !appOptions.canSubmitForms
    ...
    const saveAsPdf = () => {
      ...
      if (appOptions.isOffline) {
        ...
        const options = new Asc.asc_CDownloadOptions(Asc.c_oAscFileType.PDF, isFromBtnDownload)
