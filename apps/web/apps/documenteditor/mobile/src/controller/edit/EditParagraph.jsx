...
class EditParagraphController extends Component {
  ...
  onDistanceBefore(distance, isDecrement) {
    ...
    if (api) {
      ...
      api.put_LineSpacingBeforeAfter(
        0,
        isDecrement && newDistance < 0
          ? -1
          : !isDecrement && newDistance > -1 && newDistance < 0
            ? 0
            : Common.Utils.Metric.fnRecalcToMM(newDistance),
      )
    }
  }
  ...
  onSpinFirstLine(paragraphProperty, isDecrement) {
    ...
    if (api) {
      ...
      const newParagraphProp = new Asc.asc_CParagraphProperty()
