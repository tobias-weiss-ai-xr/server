...
class EditTableController extends Component {
  ...
  onOptionResize(value) {
    ...
    if (api) {
      ...
      properties.put_TableLayout(
        value ? Asc.c_oAscTableLayout.AutoFit : Asc.c_oAscTableLayout.Fixed,
      )
      api.tblApply(properties)
    }
  }
  ...
  onWrapMoveText(value) {
    ...
    if (api) {
      ...
      position.put_Value(
        tableObject.get_Value_Y(value ? Asc.c_oAscVAnchor.Text : Asc.c_oAscVAnchor.Page),
      )
  ...
  onChangeTableDimension(type, value, isDecrement) {
    ...
    const newValue = isDecrement ? Math.max(0, value - step) : Math.min(maxValue, value + step)
