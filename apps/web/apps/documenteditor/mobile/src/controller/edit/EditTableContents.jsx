...
class EditTableContentsController extends Component {
  ...
  getStylesImages() {
    ...
    const arrValuesStyles = [
      Asc.c_oAscTOCStylesType.Simple,
      Asc.c_oAscTOCStylesType.Web,
      Asc.c_oAscTOCStylesType.Standard,
      Asc.c_oAscTOCStylesType.Modern,
      Asc.c_oAscTOCStylesType.Classic,
    ]

    arrValuesStyles.forEach((value, index) => {
      const canvasElem = document.querySelector(`#image-toc-style${index}`)
  ...
  fillTOCProps(props) {
    ...
    let disableOutlines = false
    const styles = []

    docStyles.forEach((style) => {
      const name = style.get_Name()
      const level = api.asc_GetHeadingLevel(name)
    ...
    if (props) {
      ...
      for (let i = 0; i < count; i++) {
        const styleName = props.get_StyleName(i)
        const level = props.get_StyleLevel(i)
        const rec = styles.find((style) => style.name === styleName)

        if (rec) {
          rec.checked = true
          rec.value = level
          if (rec.headerLevel !== level) disableOutlines = true
        } else {
          styles.push({
            name: styleName,
      ...
      if (start > 0 && end > 0) {
        for (let i = start; i <= end; i++) {
          const rec = styles.find((style) => style.headerLevel === i)
      ...
      for (let i = 0; i < 9; i++) {
        const rec = styles.find((style) => style.headerLevel === i + 1)
  ...
  onUpdateTableContents(type, currentTOC) {
    const api = Common.EditorApi.get()
    const props = api.asc_GetTableOfContentsPr(currentTOC)
