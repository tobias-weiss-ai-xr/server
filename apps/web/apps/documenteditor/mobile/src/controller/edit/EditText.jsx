...
class EditTextController extends Component {
  ...
  updateBulletsNumbers(type) {
    ...
    const arrayElements =
      type === 0
        ? storeTextSettings.getBulletsList()
        : type === 1
          ? storeTextSettings.getNumbersList()
          : storeTextSettings.getMultiLevelList()
  ...
  updateListType() {
    ...
    const numformat =
      listId !== null
        ? api.asc_GetNumberingPr(listId).get_Lvl(api.asc_GetCurrentNumberingLvl()).get_Format()
        : Asc.c_oAscNumberingFormat.None

    this.props.storeTextSettings.resetListType(
