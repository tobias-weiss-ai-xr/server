...
class DocumentInfoController extends Component {
  ...
  updatePdfInfo(props) {
    ...
    if (props) {
      ...
      if (value) this.docInfoObject.dateCreated = this.getModified(new Date(value))

      value = props.ModDate
      if (value) this.docInfoObject.modifyDate = this.getModified(new Date(value))
      ...
      if (
        ...
        let heigth = props.PageHeight
        switch (Common.Utils.Metric.getCurrentMetric()) {
          case Common.Utils.Metric.c_MetricUnits.cm:
            width = Number.parseFloat(((width * 25.4) / 72000).toFixed(2))
            heigth = Number.parseFloat(((heigth * 25.4) / 72000).toFixed(2))
            break
          case Common.Utils.Metric.c_MetricUnits.pt:
            width = Number.parseFloat((width / 100).toFixed(2))
            heigth = Number.parseFloat((heigth / 100).toFixed(2))
            break
          case Common.Utils.Metric.c_MetricUnits.inch:
            width = Number.parseFloat((width / 7200).toFixed(2))
            heigth = Number.parseFloat((heigth / 7200).toFixed(2))
            break
        }
      ...
      value = props.Tagged
      if (value !== undefined) this.docInfoObject.tagged = value === true ? _t.textYes : _t.textNo
  ...
  getAppProps() {
    ...
    if (appProps) {
      ...
      if (appName && appProps.asc_getAppVersion()) appName += ` ${appProps.asc_getAppVersion()}`
  ...
  changeTitleHandler() {
    ...
    f7.dialog
      .create({
        ...
        content: Device.ios
          ? '<div class="input-field"><input type="text" class="modal-text-input" name="modal-title" id="modal-title"></div>'
          : '<div class="input-field modal-title"><div class="inputs-list list inline-labels"><ul><li><div class="item-content item-input"><div class="item-inner"><div class="item-input-wrap"><input type="text" name="modal-title" id="modal-title"></div></div></div></li></ul></div></div>',
...
export default inject(
  "storeAppOptions",
  "storeDocumentInfo",
)(observer(withTranslation()(DocumentInfoController)))
