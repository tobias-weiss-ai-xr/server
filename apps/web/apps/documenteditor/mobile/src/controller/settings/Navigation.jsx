...
class NavigationController extends Component {
  ...
  updateViewerNavigation(bookmarks) {
    ...
    for (let i = 0; i < count; i++) {
      const level = bookmarks[i].level - 1
      let hasParent = true

      if (level > prevLevel && i > 0) arrHeaders[i - 1].hasSubItems = true

      if (headerLevel < 0 || level <= headerLevel) {
        if (i > 0 || firstHeader) headerLevel = level
        hasParent = false
      }
  ...
  updateNavigation() {
    ...
    let prevLevel = -1
    let headerLevel = -1
    const firstHeader = !navigationObject.isFirstItemNotHeader()

    for (let i = 0; i < count; i++) {
      const level = navigationObject.get_Level(i)
      let hasParent = true
      if (level > prevLevel && i > 0) arrHeaders[i - 1].hasSubItems = true
      if (headerLevel < 0 || level <= headerLevel) {
        if (i > 0 || firstHeader) headerLevel = level
