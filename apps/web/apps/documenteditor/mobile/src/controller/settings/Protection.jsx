...
const ProtectionController = (props) => {
      ...
      if (propsProtection && isPassword) {
        f7.dialog
          .create({
            ...
            content: Device.ios
              ? `<div class="input-field modal-password"><input type="password" maxlength="15" class="modal-text-input" name="protection-password" placeholder="${t("Settings.advDRMPassword")}" id="protection-password"><i class="modal-password__icon icon icon-show-password"></i></div>`
              : `<div class="input-field modal-password"><div class="inputs-list list inline-labels"><ul><li><div class="item-content item-input"><div class="item-inner"><div class="item-input-wrap"><input type="password" name="protection-password" maxlength="15" id="protection-password" placeholder=${t("Settings.advDRMPassword")}><i class="modal-password__icon icon icon-show-password"></i></div></div></div></li></ul></div></div>`,
        ...
        if (!propsProtection) propsProtection = new AscCommonWord.CDocProtect()
  ...
  return (
    <>
      ...
      <Snackbar
        isShowSnackbar={isSnackbarVisible}
        message={t("Settings.textProtectTurnOff")}
        closeCallback={() => setSnackbarVisible(false)}
      />
    </>
  )
}
