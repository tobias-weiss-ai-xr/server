...
export const updateChartStyles = (storeChartSettings, storeFocusObjects) => {
  ...
  api.asc_registerCallback("asc_onUpdateChartStyles", () => {
    if (storeChartSettings.chartObject?.get_ChartProperties()) {
...
export const ContextMenu = {
  ...
  mapMenuItems(controller) {
    ...
    const contextTypeHandlers = {
      [Asc.c_oAscTypeSelectElement.Header]: (val, state) => {
        state.locked = val.get_Locked()
      },
      [Asc.c_oAscTypeSelectElement.Paragraph]: (val, state) => {
        state.locked = val.get_Locked()
        state.isText = true
      },
      [Asc.c_oAscTypeSelectElement.Image]: (val, state) => {
        state.locked = val.get_Locked()
        state.isObject = true
      },
      [Asc.c_oAscTypeSelectElement.Table]: (val, state) => {
        state.locked = val.get_Locked()
        state.isObject = true
      },
      [Asc.c_oAscTypeSelectElement.Hyperlink]: (val, state) => {
        state.isLink = true
      },
    }
