...
export class storeAppOptions {
  ...
  setConfigOptions(config, _t) {
    ...
    this.canRenameAnonymous = !(
      typeof this.customization === "object" &&
      typeof this.customization.anonymous === "object" &&
      this.customization.anonymous.request === false
    )
    this.guestName =
      typeof this.customization === "object" &&
      typeof this.customization.anonymous === "object" &&
      typeof this.customization.anonymous.label === "string" &&
      this.customization.anonymous.label.trim() !== ""
        ? Common.Utils.String.htmlEncode(this.customization.anonymous.label)
        : _t.textGuest

    const value = this.canRenameAnonymous ? LocalStorage.getItem("guest-username") : null
    this.user = Common.Utils.fillUserInfo(
      config.user,
      config.lang,
      value ? `${value} (${this.guestName})` : _t.textAnonymous,
      LocalStorage.getItem("guest-id") || `uid-${Date.now()}`,
    )
  ...
  setPermissionOptions(document, licType, params, permissions, isSupportEditFeature) {
    if (params.asc_getRights() !== Asc.c_oRights.Edit) permissions.edit = permissions.review = false
    ...
    this.canEdit =
      (permissions.edit !== false || permissions.review === true) && // can edit or review
    ...
    this.canSendEmailAddresses =
      this.canLicense && this.config.canSendEmailAddresses && this.canEdit && this.canCoAuthoring
    this.canComments =
      this.canLicense &&
      (permissions.comment === undefined ? this.isEdit : permissions.comment) &&
      this.config.mode !== "view"
    this.canComments =
      this.canComments &&
      !(typeof this.customization === "object" && this.customization.comments === false)
    ...
    if (typeof this.customization === "object" && this.customization.commentAuthorOnly === true) {
      ...
      if (
        permissions.editCommentAuthorOnly === undefined &&
        permissions.deleteCommentAuthorOnly === undefined
      )
    ...
    this.canFillForms =
      this.canLicense &&
      this.typeForm &&
      (permissions.fillForms === undefined ? this.isEdit : permissions.fillForms) &&
      this.config.mode !== "view"
    ...
    this.canSubmitForms =
      ...
      !this.isOffline &&
      typeof this.customization === "object" &&
      !!this.customization.submitForm &&
      (typeof this.customization.submitForm !== "object" ||
        this.customization.submitForm.visible !== false)
    this.isEditableForms = this.isForm && this.canSubmitForms
    this.isRestrictedEdit =
      !this.isEdit && (this.canComments || this.canFillForms) && isSupportEditFeature
    if (this.isRestrictedEdit && this.canComments && this.canFillForms)
    ...
    this.canBrandingExt =
      params.asc_getCanBranding() && (typeof this.customization === "object" || this.config.plugins)

    this.canFavorite =
      document.info &&
      document.info?.favorite !== undefined &&
      document.info?.favorite !== null &&
      !this.isOffline
    ...
    this.canUseReviewPermissions =
      this.canLicense &&
      (!!permissions.reviewGroups ||
        (this.customization?.reviewPermissions &&
          typeof this.customization.reviewPermissions === "object"))
    ...
    this.canUseCommentPermissions &&
      AscCommon.UserInfoParser.setCommentPermissions(permissions.commentGroups)
    this.canUseUserInfoPermissions &&
      AscCommon.UserInfoParser.setUserInfoPermissions(permissions.userInfoGroups)
