...
export class storeTableSettings {
  ...
  setStyles(arrStyles, typeStyles) {
    const styles = []
    for (const template of arrStyles) {
      styles.push({
  ...
  getMoveText(tableObject) {
    return (
      ...
      tableObject.get_PositionV().get_RelativeFrom() === Asc.c_oAscVAnchor.Text
    )
  }
      ...
      if (background.get_Value() === 0) {
        ...
        if (color) {
          if (color.get_type() === Asc.c_oAscColor.COLOR_TYPE_SCHEME) {
            ...
            fillColor = Common.Utils.ThemeColor.getHexColor(
              color.get_r(),
              color.get_g(),
              color.get_b(),
            )
  ...
  updateBorderStyle(border, visible) {
    ...
    if (visible && this.cellBorderWidth > 0) {
      const size = Number.parseFloat(this.cellBorderWidth)
      border.put_Value(1)
