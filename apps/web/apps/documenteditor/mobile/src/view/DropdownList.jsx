          ...
          <>
            <List className="dropdown-list">
              <ListItem
                radio
                radioIcon="end"
                checked={enteredValue.length}
                title={enteredValue || t("Edit.textEnterYourOption")}
                name="custom-option"
          ...
          {listItems.length &&
            listItems.map((item, index) => (
              <ListItem
                radioIcon="end"
                radio
                checked={item.value === curValue && !enteredValue}
                key={index}
                name="dropdown-option"
                className={`no-indicator ${index === 0 ? "dropdown-list__placeholder" : ""}`}
                title={item.caption}
                onClick={() => props.onChangeItemList(item.value)}
              />
            ))}
        </List>
      </Page>
...
class DropdownListView extends Component {
  render() {
    return Device.isPhone ? (
      <Popup
        id="dropdown-list-popup"
        className="dropdown-list-popup"
        closeByOutsideClick={true}
        swipeToClose={true}
        onPopupClosed={() => this.props.closeModal()}
      >
        <PageDropdownList
      ...
      <Popover
        id="dropdown-list-popover"
        className="popover__titled"
        closeByOutsideClick={true}
        onPopoverClosed={() => this.props.closeModal()}
      >
        <PageDropdownList
          listItems={this.props.listItems}
          onChangeItemList={this.props.onChangeItemList}
