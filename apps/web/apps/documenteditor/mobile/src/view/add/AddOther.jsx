      ...
      <List>
        ...
        >
          {Device.ios ? (
            ...
            <SvgIcon
              slot="media"
              symbolId={IconStringBreakAndroid.id}
              className={"icon icon-svg"}
            />
          )}
        </ListItem>
        <ListItem
          title={_t.textSectionBreak}
          link={"/add-section-break/"}
          routeProps={{
        ...
        >
          {Device.ios ? (
            ...
            <SvgIcon
              slot="media"
              symbolId={IconSectionBreakAndroid.id}
              className={"icon icon-svg"}
            />
          )}
        </ListItem>
  ...
  return (
    <Page>
          ...
          <div slot="after">
            <Segmented>
              <Button
                ...
                onClick={() => {
                  setStartAt(props.onFootnoteStartAt(stateStartAt, true))
                }}
              >
                {isAndroid ? (
              ...
              <Button
                ...
                onClick={() => {
                  setStartAt(props.onFootnoteStartAt(stateStartAt, false))
                }}
              >
                {isAndroid ? (
      ...
      <List className="buttons-list">
        <ListButton
          className={"button-fill button-raised"}
          title={_t.textInsertFootnote}
          onClick={() => {
...
const AddOther = (props) => {
  ...
  const isText = storeFocusObjects.settings.indexOf("text") > -1
  const isChart = storeFocusObjects.settings.indexOf("chart") > -1
  const isHyperLink = storeFocusObjects.settings.indexOf("hyperlink") > -1
  const isHeader = storeFocusObjects.settings.indexOf("header") > -1

  const disabledAddBreak =
    props.paragraphLocked ||
    props.inFootnote ||
    props.inControl ||
    props.richEditLock ||
    props.plainEditLock ||
    props.richDelLock ||
    props.plainDelLock
  const disabledAddFootnote =
    props.paragraphLocked || props.controlPlain || props.richEditLock || props.plainEditLock
  ...
  return (
    <List>
      {isText && canComments && (
        ...
        >
          {Device.ios ? (
            ...
            <SvgIcon
              slot="media"
              symbolId={IconInsertCommentAndroid.id}
              className={"icon icon-svg"}
            />
          )}
        </ListItem>
      ...
      {isText && !disabledAddLink && (
        <ListItem
          title={_t.textLink}
          href={isHyperLink ? "/edit-link/" : "/add-link/"}
          routeProps={{
            isNavigate: true,
          }}
        >
      ...
      {!disabledAddPageNumber && (
        <ListItem
          title={_t.textPageNumber}
          link={"/add-page-number/"}
          routeProps={{
      ...
      {!isHeader && (
        <ListItem title={_t.textTableContents} link="/add-table-contents/">
          {Device.ios ? (
            ...
            <SvgIcon
              slot="media"
              symbolId={IconTableContentsAndroid.id}
              className={"icon icon-svg"}
            />
          )}
        </ListItem>
...
const AddOtherContainer = inject(
  "storeComments",
  "storeFocusObjects",
  "storeLinkSettings",
  "storeAppOptions",
