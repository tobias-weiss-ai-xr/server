...
import { Icon, Link, NavRight, NavTitle, Navbar, Page, Tab, Tabs, f7 } from "framework7-react"
...
const AddLayoutNavbar = ({ tabs, storeTableSettings }) => {
  ...
  const getTableStylesPreviews = () => {
    if (!storeTableSettings.arrayStylesDefault.length) {
      ...
      setTimeout(
        () => storeTableSettings.setStyles(api.asc_getTableStylesPreviews(true), "default"),
        1,
      )
    }
  }
      ...
      {tabs.length > 1 ? (
        <div className="tab-buttons tabbar">
          {tabs.map((item, index) => (
            <Link
              key={`de-link-${item.id}`}
              onClick={() => getTableStylesPreviews()}
              tabLink={`#${item.id}`}
          ...
          {isAndroid && (
            <span className="tab-link-highlight" style={{ width: `${100 / tabs.lenght}%` }} />
          )}
        </div>
      ) : (
...
const AddingPage = inject(
  ...
  observer((props) => {
    ...
    let canAddTable = true
    let canAddImage = true
    let paragraphLocked = false
    let inFootnote = false
    let inControl = false
    let controlProps = false
    let lockType = false
    let controlPlain = false
    let contentLocked = false
    let richDelLock = false
    let richEditLock = false
    let plainDelLock = false
    let plainEditLock = false
    ...
    if (paragraphObj) {
      ...
      lockType =
        inControl && controlProps ? controlProps.get_Lock() : Asc.c_oAscSdtLockType.Unlocked
      controlPlain =
        inControl && controlProps
          ? controlProps.get_ContentControlType() === Asc.c_oAscSdtLevelType.Inline
          : false
      contentLocked =
        lockType === Asc.c_oAscSdtLockType.SdtContentLocked ||
        lockType === Asc.c_oAscSdtLockType.ContentLocked
    ...
    if (!showPanels && options.indexOf("text") > -1) {
      needDisable =
        !canAddTable || controlPlain || richEditLock || plainEditLock || richDelLock || plainDelLock
