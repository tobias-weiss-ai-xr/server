...
const EditHyperlink = (props) => {
  ...
  if (linkObject) {
    link = linkObject.get_Value() ? linkObject.get_Value().replace(/ /g, "%20") : ""
    ...
    <Page>
      <Navbar className="navbar-link-settings">
        <NavLeft>
          ...
          >
            {Device.android && <SvgIcon symbolId={IconClose.id} className={"icon icon-svg"} />}
          </Link>
        </NavLeft>
