...
const PageAdvancedSettings = (props) => {
  ...
  if (paragraphObj) {
    ...
    firstLine = Number.parseFloat(
      Common.Utils.Metric.fnRecalcFromMM(paragraphObj.get_Ind().get_FirstLine()).toFixed(2),
    )
    ...
    spaceBeforeFix = Number.parseFloat(spaceBefore.toFixed(2))
    spaceAfterFix = Number.parseFloat(spaceAfter.toFixed(2))
          ...
          <div slot="after">
            <Segmented>
              <Button
                ...
                className="decrement item-link"
                onClick={() => {
                  props.onSpinFirstLine(paragraphObj, true)
                }}
              >
                {isAndroid ? (
                  <SvgIcon symbolId={IconExpandDownAndroid.id} className={"icon icon-svg"} />
              ...
              <Button
                ...
                className="increment item-link"
                onClick={() => {
                  props.onSpinFirstLine(paragraphObj, false)
                }}
              >
                {isAndroid ? (
                  <SvgIcon symbolId={IconExpandUp.id} className={"icon icon-svg"} />
  ...
  const changeColor = (color, effectId, effectValue) => {
    if (color !== "empty") {
      ...
      props.f7router.navigate("/edit-paragraph-custom-color/", {
        props: { onBackgroundColor: props.onBackgroundColor },
      })
    }
  }
  return (
    <Page>
      ...
      <List>
        <ListItem
          ...
          link={"/edit-paragraph-custom-color/"}
          routeProps={{
...
const EditParagraph = (props) => {
  ...
  const curBackColor = storeParagraphSettings.backColor
    ? storeParagraphSettings.backColor
    : storeParagraphSettings.getBackgroundColor(paragraph)
  const background =
    curBackColor !== "transparent"
      ? `#${typeof curBackColor === "object" ? curBackColor.color : curBackColor}`
      : ""
  ...
  return (
    <Fragment>
      ...
      <List style={{ marginBottom: 0 }}>
        <ListItem
          link="/edit-paragraph-style/"
          title={!curStyle && t("Edit.textCustomStyle")}
          routeProps={{
        ...
        >
          {curStyle && (
            <div
              slot="inner"
              style={{
                backgroundImage: `url(${curStyle.image})`,
                width: "100%",
      ...
      <List>
        <ListItem
          title={_t.textBackground}
          link="/edit-paragraph-back-color/"
          routeProps={{
            onBackgroundColor: props.onBackgroundColor,
          }}
        >
          <span className="color-preview" slot="after" style={{ background: background }} />
        </ListItem>
      </List>
      <List>
        <ListItem
          title={_t.textAdvancedSettings}
          link="/edit-paragraph-adv/"
          routeProps={{
...
const EditParagraphStyle = (props) => {
  ...
  return (
    <Page>
      ...
      <List style={{ marginBottom: 0 }}>
        <ListItem
          className="create-style-link"
          title={t("Edit.textCreateTextStyle")}
          href="/create-text-style/"
          routeProps={{
            onSaveStyle: props.onSaveStyle,
          }}
        >
  ...
  return (
    <Page>
      <Navbar title={t("Edit.textCreateTextStyle")} backLink={t("Edit.textBack")}>
        <Link
          slot="right"
          className={`${!titleNewStyle.trim() && "disabled"}`}
          onClick={() => {
            const title = titleNewStyle.trim()
            if (title) {
      ...
      <List>
        <ListItem
          title={!nextParagraphStyle ? t("Edit.textSameCreatedNewStyle") : nextParagraphStyle}
          href="/change-next-paragraph-style/"
          routeProps={{
            nextParagraphStyle,
            setParagraph,
          }}
...
const ChangeNextParagraphStyle = (props) => {
  ...
  return (
    <Page>
      <Navbar title={t("Edit.textNextParagraphStyle")} backLink={_t.textBack} />
      <List>
        <ListItem
          style={{ paddingLeft: "5px" }}
          radio
          radioIcon="start"
          checked={!newParagraph}
          onClick={() => {
            if (newParagraph) {
              setParagraph("")
              props.setParagraph("")
...
const EditParagraphContainer = inject(
  "storeParagraphSettings",
  "storeFocusObjects",
)(observer(EditParagraph))
const ParagraphAdvSettings = inject(
  "storeParagraphSettings",
  "storeFocusObjects",
)(observer(PageAdvancedSettings))
const PageParagraphBackColor = inject(
  "storeParagraphSettings",
  "storePalette",
)(observer(PageBackgroundColor))
const PageParagraphCustomColor = inject(
  "storeParagraphSettings",
  "storePalette",
)(observer(PageCustomBackColor))
...
const PageChangeNextParagraphStyle = inject("storeParagraphSettings")(
  observer(ChangeNextParagraphStyle),
)
