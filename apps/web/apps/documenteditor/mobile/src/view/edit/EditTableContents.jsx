...
const EditTableContents = (props) => {
  ...
  const [leaderValue, setLeaderValue] = useState(
    propsTableContents.get_TabLeader()
      ? propsTableContents.get_TabLeader()
      : Asc.c_oAscTabLeader.Dot,
  )
    ...
    <Fragment>
      <List>
        <ListItem
          ...
          link="/edit-style-table-contents/"
          after={activeStyle ? activeStyle.displayValue : ""}
          routeProps={{
      ...
      <List>
        ...
        {pageNumbers && rightAlign && (
          <ListItem
            title={t("Edit.textLeader")}
            link="/edit-leader-table-contents/"
            after={activeLeader ? activeLeader.displayValue : arrLeaders[0].displayValue}
            routeProps={{
        ...
        <ListItem
          title={t("Edit.textStructure")}
          link="/edit-structure-table-contents/"
          after={stylesCount ? t("Edit.textStyles") : t("Edit.textLevels")}
          routeProps={{
      ...
      <List className="buttons-list">
        <ListButton
          ...
          title={t("Edit.textRefresh")}
          onClick={() => openActionsButtonsRefresh()}
        />
        <ListButton
          className="button-red button-fill button-raised"
          title={t("Edit.textRemoveTableContent")}
          onClick={() => props.onRemoveTableContents()}
        />
      </List>
    </Fragment>
  )
        ...
        {props.arrStyles.map((style, index) => {
          return (
            ...
            >
              ...
              <div
                className={`style-toc__image ${style.value === styleValue ? "style-toc__image_active" : ""}`}
              >
                <canvas
                  style={{ width: widthImage, height: "10px" }}
                  id={`image-toc-style${index}`}
                />
              </div>
            </div>
          )
        ...
        {props.arrLeaders.map((leader, index) => {
          return (
            <ListItem
              key={index}
              radio
              title={leader.displayValue}
              checked={leaderValue === leader.value}
              onClick={() => {
...
const PageEditStructureTableContents = (props) => {
  ...
  const addNewStyle = (style) => {
    const indexStyle = chosenStyles.findIndex((currentStyle) => currentStyle.name === style.name)
    ...
    <Page>
      ...
      <List>
        <ListItem
          radio
          checked={structure === 0}
          title={t("Edit.textLevels")}
          onClick={() => setStructure(0)}
        />
        <ListItem
          radio
          checked={structure === 1}
          title={t("Edit.textStyles")}
          ...
          {styles.map((style, index) => {
            return (
              <ListItem
                checkbox
                key={index}
                title={style.displayValue}
                checked={style.checked && style.value >= 1}
              >
