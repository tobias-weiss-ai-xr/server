...
const ProtectionDocumentView = (props) => {
  ...
  const isDisabledProtection = isRequirePassword
    ? !password.length || !passwordRepeat.length || stateTypeProtection === null
    : stateTypeProtection === null
  ...
  return (
    <Page>
      <Navbar title={t("Settings.textProtectDocument")} backLink={_t.textBack}>
        <NavRight>
          <Link
            text={isIos && t("Settings.textSave")}
            className={isDisabledProtection && "disabled"}
            onClick={changeHanlder}
          >
            {Device.android && <SvgIcon symbolId={IconCheck.id} className="icon icon-svg" />}
          </Link>
        </NavRight>
      </Navbar>
      <List>
          ...
          <div className="inputs-list list inline-labels">
            <ul>
              ...
              <PasswordField
                ...
                handlerChange={changeRepeationPassword}
                maxLength={15}
              />
            </ul>
          </div>
      ...
      <List>
        <ListItem
          radio
          checked={stateTypeProtection === Asc.c_oAscEDocProtect.ReadOnly}
          title={t("Settings.textNoChanges")}
          onClick={() => {
        ...
        <ListItem
          radio
          checked={stateTypeProtection === Asc.c_oAscEDocProtect.Forms}
          title={t("Settings.textFillingForms")}
          onClick={() => {
        ...
        <ListItem
          radio
          checked={stateTypeProtection === Asc.c_oAscEDocProtect.TrackedChanges}
          title={t("Settings.textTrackedChanges")}
          onClick={() => {
        ...
        <ListItem
          radio
          checked={stateTypeProtection === Asc.c_oAscEDocProtect.Comments}
          title={t("Settings.textComments")}
          onClick={() => {
