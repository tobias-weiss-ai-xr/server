...
import { Icon, Link, List, ListItem, NavRight, Navbar, Page, Toggle } from "framework7-react"
...
const SettingsPage = inject(
  ...
  observer((props) => {
    ...
    let _canDownloadOrigin = false
    let _canAbout = true
    let _canHelp = true
    let _canPrint = false
    let _canFeedback = true
    let _canDisplayInfo = true

    if (appOptions.isDisconnected) {
      _isEdit = false
      if (!appOptions.enableDownload) _canPrint = _canDownload = _canDownloadOrigin = false
    } else {
        ...
        <List>
          {isShowBack && (
            <ListItem
              ...
              onClick={() => Common.Notifications.trigger("goback")}
            >
              {Device.ios ? (
          ...
          {isEditableForms
            ? [
                isFavorite !== undefined && isFavorite !== null ? (
                  <ListItem
                    key="add-to-favorites-link"
                    title={
                      isFavorite
                        ? t("Settings.textRemoveFromFavorites")
                        : t("Settings.textAddToFavorites")
                    }
                    link="#"
                    className="no-indicator"
                    onClick={settingsContext.toggleFavorite}
                  >
                    {isFavorite ? (
                      <SvgIcon
                        slot="media"
                        symbolId={IconRemoveFavorites.id}
                        className={"icon icon-svg"}
                      />
                    ) : (
                      <SvgIcon
                        slot="media"
                        symbolId={IconAddFavorites.id}
                        className={"icon icon-svg"}
                      />
                    )}
                  </ListItem>
                ...
                <ListItem
                  key="clear-all-fields-link"
                  title={t("Settings.textClearAllFields")}
                  link="#"
                  className="no-indicator"
                  onClick={settingsContext.clearAllFields}
                  disabled={isSignatureForm}
                >
          ...
          {(Device.phone || isEditableForms) && (
            <ListItem
              title={!_isEdit || isViewer ? _t.textFind : _t.textFindAndReplace}
              link="#"
              searchbarEnable=".searchbar"
              onClick={settingsContext.closeModal}
              className="no-indicator"
            >
          ...
          {_isEdit && !isHistoryDisabled && canUseHistory && (
            <ListItem
              title={t("Settings.textVersionHistory")}
              link={!Device.phone ? "/version-history" : ""}
              onClick={() => {
                if (Device.phone) {
          ...
          {!isEditableForms ? (
            <ListItem
              title={t("Settings.textNavigation")}
              link={!Device.phone ? "/navigation" : "#"}
              onClick={() => {
                if (Device.phone) {
          ...
          {Device.sailfish && _isEdit && (
            <ListItem
              title={_t.textSpellcheck}
              onClick={() => settingsContext.onOrthographyCheck()}
              className="no-indicator"
              link="#"
            >
          ...
          {_canDownload && (
            <ListItem
              title={isEditableForms ? t("Settings.textExport") : _t.textDownload}
              link="/download/"
            >
              {isEditableForms ? (
          ...
          {_canDownloadOrigin && (
            <ListItem
              title={_t.textDownload}
              link="#"
              onClick={settingsContext.onDownloadOrigin}
              className="no-indicator"
            >
              <SvgIcon slot="media" symbolId={IconDownload.id} className={"icon icon-svg"} />
            </ListItem>
          ...
          {_canPrint && (
            <ListItem
              title={_t.textPrint}
              onClick={settingsContext.onPrint}
              link="#"
              className="no-indicator"
            >
          ...
          {_canFeedback && (
            ...
            >
              {Device.ios ? (
                <SvgIcon
                  slot="media"
                  symbolId={IconFeedbackForIos.id}
                  className={"icon icon-svg"}
                />
              ) : (
                <SvgIcon
                  slot="media"
                  symbolId={IconFeedbackForAndroid.id}
                  className={"icon icon-svg"}
                />
              )}
            </ListItem>
          ...
          {canCloseEditor && (
            <ListItem
              title={closeButtonText ?? t("Settings.textClose")}
              link="#"
              className="close-editor-btn no-indicator"
              onClick={() => Common.Notifications.trigger("close")}
            />
          )}
        </List>
      </Page>
