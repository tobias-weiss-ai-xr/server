...
define([
  ...
  PDFE.Controllers.LeftMenu = Backbone.Controller.extend(
    _.extend(
      {
        ...
        initialize: function () {
          ...
          Common.NotificationCenter.on(
            "search:resetmode",
            _.bind(this.onSetDefaultSearchMode, this),
          )
        },
        ...
        setApi: function (api) {
          ...
          if (this.mode.canCoAuthoring) {
            ...
            if (this.mode.canComments) {
              ...
              const resolved = Common.Utils.InternalSettings.get("pdfe-settings-resolvedcomment")
        ...
        _saveAsFormat: function (menu, format, ext, textParams, wopiPath) {
          ...
          if (menu) {
            ...
            if (format === Asc.c_oAscFileType.TXT || format === Asc.c_oAscFileType.RTF) {
              ...
              if (
                !Common.Utils.InternalSettings.get("pdfe-settings-compatible") &&
                !Common.localStorage.getBool("pdfe-hide-save-compatible") &&
        ...
        saveAsInWopi: function (menu, format, ext) {
          ...
          const fileExt =
            format === undefined && ext === "true"
              ? idx > 0
                ? defFileName.substring(idx)
                : ""
              : ext
          ...
          new Common.Views.TextInputDialog({
            ...
            handler: (result, value) => {
              if (result === "ok") {
                if (typeof fileExt === "string") value = value + fileExt
        ...
        applySettings: function (menu) {
          ...
          let fast_coauth = Common.Utils.InternalSettings.get("pdfe-settings-coauthmode")
          const canPDFSave = this.mode.isPDFAnnotate || this.mode.isPDFEdit
          /** coauthoring begin **/
          ...
          if (this.mode.isEdit && canPDFSave) {
            if (this.mode.canChangeCoAuthoring || !fast_coauth) {
              ...
              value = Number.parseInt(Common.localStorage.getItem("pdfe-settings-autosave"))
          ...
          this.api.put_ShowSnapLines(
            Common.Utils.InternalSettings.get("pdfe-settings-showsnaplines"),
          )
          ...
          /* update zoom */
          const newZoomValue = Common.localStorage.getItem("pdfe-settings-zoom")
          const oldZoomValue = Common.Utils.InternalSettings.get("pdfe-settings-zoom")
          const lastZoomValue = Common.Utils.InternalSettings.get("pdfe-last-zoom")
        ...
        SetDisabled: function (disable, options) {
          ...
          if (!options || options.thumbnails?.disable)
        ...
        onAppAddComment: function (sender) {
          if (this.api.can_AddQuotedComment() === false) {
            new Promise((resolve, reject) => {
              ...
              const ctrl = PDFE.getController("Common.Controllers.Comments")
        ...
        onShortcut: function (s, e) {
          ...
          switch (s) {
            ...
            case "escape": {
              ...
              const statusbar = PDFE.getController("Statusbar")
