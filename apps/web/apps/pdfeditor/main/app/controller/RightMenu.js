...
define(["core", "pdfeditor/main/app/view/RightMenu"], () => {
  PDFE.Controllers.RightMenu = Backbone.Controller.extend({
    ...
    onFocusObject: function (SelectedObjects, forceSignature, forceOpen) {
      if (!(this.editMode && this.mode.isPDFEdit) && !forceSignature) return

      const open = this._initSettings
        ? !Common.localStorage.getBool(
            "pdfe-hide-right-settings",
      ...
      if (!this.rightmenu.minimizedMode || open) {
        ...
        } else this.rightmenu.clearSelection()
      }
        ...
        if (disabled) {
          ...
          if (selectedElements.length > 0)
            this.onFocusObject(
              selectedElements,
              false,
              !Common.Utils.InternalSettings.get("pdfe-hide-right-settings") &&
    ...
    onInsertForm: function () {
      const idx = this._priorityArr.indexOf(Common.Utils.documentSettingsType.Form)
      if (idx >= 0) this._priorityArr.splice(idx, 1)
