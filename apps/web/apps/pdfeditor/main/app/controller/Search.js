...
define(["core", "common/main/lib/view/SearchPanel"], () => {
  PDFE.Controllers.Search = Backbone.Controller.extend(
    _.extend(
      {
        sdkViewName: "#id_main",
        ...
        setApi: function (api) {
          if (api) {
            ...
            this.api.asc_registerCallback(
              "asc_onUpdateRedactState",
              _.bind(this.onUpdateRedactState, this),
            )
          }
          return this
        },
        ...
        onQueryReplace: function (textSearch, textReplace) {
          if (textSearch !== "") {
            const str = this.api.asc_GetErrorForReplaceString(textReplace)
            if (str) {
              setTimeout(() => {
                Common.UI.warning({
        ...
        onMark: function (textSearch) {
          ...
          if (this.resultItems[this._state.currentResult + 1]) {
            ...
            this.view.disableRedactButtons(
              this.api.asc_GetRedactSearchInfo(this._state.currentResult),
            )
          }
        },
