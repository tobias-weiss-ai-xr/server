...
define([
  ...
  PDFE.Views.FileMenu = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#file-menu-panel",
        rendered: false,
        ...
        events: function () {
          return {
            "click .fm-btn": _.bind(function (event) {
              const item = _.findWhere(this.items, { el: event.currentTarget })
              ...
              const proceedOpening = function () {
                ...
                if (item) {
                  const panel = this.panels[item.options.action]
              ...
              if (item?.options && typeof item.options.openInterceptor === "function") {
        ...
        render: function () {
          ...
          if (this.options.miRename) {
            ...
            this.options.miRename = undefined
          }
          ...
          if (this.options.miProtect) {
            ...
            this.options.miProtect = undefined
          }
          ...
          if (this.options.miHistory) {
            ...
            this.options.miHistory = undefined
          }
          ...
          if (Common.Controllers.Desktop.isActive()) {
            Common.NotificationCenter.trigger(
              "update:recents",
              Common.Controllers.Desktop.recentFiles(),
            )
          }
          ...
          if (this.api) {
            ...
            if (this.panels.protect) this.panels.protect.setApi(this.api)
          }
        ...
        show: function (panel, opts) {
          if (
            (this.isVisible() && panel === undefined) ||
            !this.mode ||
          ...
          const proceedOpening = function () {
            const defPanel =
              this.mode.canDownload && (!this.mode.isDesktopApp || !this.mode.isOffline)
                ? "saveas"
                : "info"
            if (!panel) panel = this.active || defPanel
            this.$el.show()
            this.scroller.update()
        ...
        applyMode: function () {
          ...
          if (!this.panels) {
            this.panels = {
              opts: new PDFE.Views.FileMenuPanels.Settings({ menu: this }).render(
                this.$el.find("#panel-settings"),
              ),
              info: new PDFE.Views.FileMenuPanels.DocumentInfo({ menu: this }).render(
                this.$el.find("#panel-info"),
              ),
              rights: new PDFE.Views.FileMenuPanels.DocumentRights({ menu: this }).render(
                this.$el.find("#panel-rights"),
              ),
            }
            this._state.infoPreviewMode &&
              this.panels.info.setPreviewMode(this._state.infoPreviewMode)
          }

          if (!this.mode) return

          let isVisible = Common.UI.LayoutManager.isElementVisible("toolbar-file-close")
          this.miClose[isVisible ? "show" : "hide"]()

          this.miDownload[
            (this.mode.canDownload || this.mode.canDownloadOrigin) &&
            (!this.mode.isDesktopApp || !this.mode.isOffline)
              ? "show"
              : "hide"
          ]()
          const isBCSupport = Common.Controllers.Desktop.isActive()
            ? Common.Controllers.Desktop.call("isBlockchainSupport")
            : false
          this.miSaveCopyAs[
            (this.mode.canDownload || this.mode.canDownloadOrigin) &&
            (!this.mode.isDesktopApp || !this.mode.isOffline) &&
          ...
          this.miSave[
            ...
            this.mode.canSaveToFile &&
            Common.UI.LayoutManager.isElementVisible("toolbar-file-save")
              ? "show"
              : "hide"
          ]()
          this.miEdit[
            !this.mode.isEdit && this.mode.canEdit && this.mode.canRequestEditRights
              ? "show"
              : "hide"
          ]()
          ...
          this.miProtect[
            this.mode.isSignatureSupport || this.mode.isPasswordSupport ? "show" : "hide"
          ]()
          ...
          isVisible =
            ...
            this.document &&
            this.document.info &&
            ((this.document.info.sharingSettings &&
              this.document.info.sharingSettings.length > 0) ||
              this.mode.sharingSettingsUrl?.length ||
          ...
          if (!this.customizationDone) {
            ...
            this.mode.canBack &&
              this.mode.customization.goback.text &&
              typeof this.mode.customization.goback.text === "string" &&
              this.miBack.setCaption(this.mode.customization.goback.text)
          }
          ...
          if (this.mode.canCreateNew) {
            if (this.mode.templates?.length) {
              !this.panels.new &&
                (this.panels.new = new PDFE.Views.FileMenuPanels.CreateNew({
                  menu: this,
                  docs: this.mode.templates,
                  blank: this.mode.canRequestCreateNew || !!this.mode.createUrl,
                }).render())
            }
          }
          ...
          if (this.mode.canOpenRecent && this.mode.recent) {
            !this.panels.recent &&
              (this.panels.recent = new Common.Views.RecentFiles({
          ...
          if (this.mode.isSignatureSupport || this.mode.isPasswordSupport) {
            !this.panels.protect &&
              (this.panels.protect = new PDFE.Views.FileMenuPanels.ProtectDoc({
                menu: this,
              }).render())
          ...
          if (this.mode.canDownload) {
            !this.panels.saveas &&
              (this.panels.saveas = new PDFE.Views.FileMenuPanels.ViewSaveAs({
                menu: this,
          ...
          if (this.mode.canDownload && (this.mode.canRequestSaveAs || this.mode.saveAsUrl)) {
            !this.panels["save-copy"] &&
              (this.panels["save-copy"] = new PDFE.Views.FileMenuPanels.ViewSaveCopy({
                menu: this,
                fileType: this.document.fileType,
                mode: this.mode,
              }).render())
          ...
          if (this.mode.canPrint) {
            const printPanel = PDFE.getController("Print").getView("PrintWithPreview")
            printPanel.menu = this
            !this.panels.printpreview &&
              (this.panels.printpreview = printPanel.render(this.$el.find("#panel-print")))
          }

          if (Common.Controllers.Desktop.isActive()) {
            if (this.$el.find("#fm-btn-local-open").length < 1) {
              $('<li id="fm-btn-local-open" class="fm-btn"/>').insertBefore(
                $("#fm-btn-recent", this.$el),
              )
              this.items.push(
            ...
            if (this.$el.find("#fm-btn-exit").length < 1) {
              $('<li class="devider" />' + '<li id="fm-btn-exit" class="fm-btn"/>').insertAfter(
                $("#fm-btn-back", this.$el),
              )
              this.items.push(
                new Common.UI.MenuItem({
            ...
            $('<li class="devider" />' + '<li id="fm-btn-close" class="fm-btn"/>').insertAfter(
              $("#fm-btn-back", this.$el),
            )
            this.items.push(
              new Common.UI.MenuItem({
          ...
          if (this.mode.canSwitchToMobile && this.$el.find("#fm-btn-switchmobile").length < 1) {
            $('<li id="fm-btn-switchmobile" class="fm-btn"></li>').insertBefore(
              $("#fm-btn-settings", this.$el),
            )
            this.items.push(
              new Common.UI.MenuItem({
        ...
        selectMenu: function (menu, opts, defMenu) {
          if (menu) {
            let item = this._getMenuItem(menu)
            let panel = this.panels[menu]
            ...
            if (item && panel) {
              ...
              if (this.scroller) {
                const itemTop = Common.Utils.getPosition(item.$el).top
                const itemHeight = item.$el.outerHeight()
                const listHeight = this.$el.outerHeight()
                if (itemTop < 0 || itemTop + itemHeight > listHeight) {
                  let height =
                    this.scroller.$el.scrollTop() + itemTop + (itemHeight - listHeight) / 2
        ...
        _getMenuItem: function (action) {
          return _.find(this.items, (item) => item.options.action === action)
        },

        SetDisabled: function (disable, options) {
          const _btn_protect = this.getButton("protect")
          const _btn_history = this.getButton("history")

          options?.protect && _btn_protect.setDisabled(disable)
          options?.history && _btn_history.setDisabled(disable)
          options?.info &&
            (this.panels
              ? this.panels.info.setPreviewMode(disable)
              : (this._state.infoPreviewMode = disable))
        },
        ...
        getButton: function (type) {
          if (!this.rendered) {
            if (type === "save") {
              return this.options.miSave
                ? this.options.miSave
                : (this.options.miSave = new Common.UI.MenuItem({}))
            }
            if (type === "rename") {
              return this.options.miRename
                ? this.options.miRename
                : (this.options.miRename = new Common.UI.MenuItem({}))
            }
            if (type === "protect") {
              return this.options.miProtect
                ? this.options.miProtect
                : (this.options.miProtect = new Common.UI.MenuItem({}))
            }
            if (type === "history") {
              return this.options.miHistory
                ? this.options.miHistory
                : (this.options.miHistory = new Common.UI.MenuItem({}))
            }
          } else {
            if (type === "save") {
              return this.miSave
