...
define([], () => {
  ...
  PDFE.Views.FileMenuPanels.ViewSaveAs = Common.UI.BaseView.extend({
    ...
    render: function () {
      if (/^pdf$/.test(this.fileType)) {
        !this.mode?.isForm && this.formats[0].splice(1, 1, { name: "PDF", imgCls: "pdf", type: "" }) // remove pdf
  ...
  PDFE.Views.FileMenuPanels.Settings = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-settings",
        menu: undefined,
        ...
        setMode: function (mode) {
          ...
          const fast_coauth = Common.Utils.InternalSettings.get("pdfe-settings-coauthmode")
          const canPDFSave =
            mode.isEdit && (mode.isPDFAnnotate || mode.isPDFEdit) && mode.canSaveToFile

          $("tr.edit", this.el)[mode.isEdit ? "show" : "hide"]()
          $("tr.edit-pdf", this.el)[mode.isPDFEdit ? "show" : "hide"]()
          $("tr.autosave", this.el)[
            canPDFSave && (mode.canChangeCoAuthoring || !fast_coauth) ? "show" : "hide"
          ]()
          $("tr.forcesave", this.el)[mode.canForcesave && canPDFSave ? "show" : "hide"]()
          $("tr.editsave", this.el)[
            canPDFSave && (mode.canChangeCoAuthoring || !fast_coauth || mode.canForcesave)
              ? "show"
              : "hide"
          ]()
          ...
          $("tr.coauth.changes-mode", this.el)[
            !mode.isOffline && mode.canCoAuthoring && mode.canChangeCoAuthoring && canPDFSave
              ? "show"
              : "hide"
          ]()
          $("tr.coauth.changes-show", this.el)[
            !mode.isOffline && mode.canCoAuthoring && canPDFSave ? "show" : "hide"
          ]()
        ...
        updateSettings: function () {
          this.chUseAltKey.setValue(
            Common.Utils.InternalSettings.get("pdfe-settings-show-alt-hints"),
          )
          this.chScreenReader.setValue(
            Common.Utils.InternalSettings.get("app-settings-screen-reader"),
          )
          this.chUseAnnotateBar.setValue(
            Common.Utils.InternalSettings.get("pdfe-settings-annot-bar"),
          )

          let value = Common.Utils.InternalSettings.get("pdfe-settings-zoom")
          value =
            value !== null
          ...
          value = Common.Utils.InternalSettings.get(
            fast_coauth ? "pdfe-settings-showchanges-fast" : "pdfe-settings-showchanges-strict",
          )
        ...
        applySettings: function () {
          const canPDFSave =
            (this.mode.isPDFAnnotate || this.mode.isPDFEdit) && this.mode.canSaveToFile
          ...
          Common.localStorage.setItem(
            "pdfe-settings-show-alt-hints",
            this.chUseAltKey.isChecked() ? 1 : 0,
          )
          Common.Utils.InternalSettings.set(
            "pdfe-settings-show-alt-hints",
            Common.localStorage.getBool("pdfe-settings-show-alt-hints"),
          )
          ...
          Common.localStorage.setItem(
            "pdfe-settings-annot-bar",
            this.chUseAnnotateBar.isChecked() ? 1 : 0,
          )
          Common.Utils.InternalSettings.set(
            "pdfe-settings-annot-bar",
            Common.localStorage.getBool("pdfe-settings-annot-bar"),
          )

          /** coauthoring begin **/
          Common.Utils.InternalSettings.set(
            "pdfe-settings-livecomment",
            this.chLiveComment.isChecked(),
          )
          Common.Utils.InternalSettings.set(
            "pdfe-settings-resolvedcomment",
            this.chResolvedComment.isChecked(),
          )
          if (this.mode.isEdit && !this.mode.isOffline && this.mode.canCoAuthoring && canPDFSave) {
            this.mode.canChangeCoAuthoring &&
              Common.localStorage.setItem(
                "pdfe-settings-coauthmode",
  ...
  PDFE.Views.FileMenuPanels.CreateNew = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-createnew",
        menu: undefined,
  ...
  PDFE.Views.FileMenuPanels.DocumentInfo = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-info",
        menu: undefined,
        ...
        render: function (node) {
          ...
          this.authorTpl =
            '<tr><td><div style="display: inline-block;width: 200px;"><input type="text" spellcheck="false" class="form-control" readonly="true" value="{0}"></div><div class="tool close img-commonctrl img-colored" data-hint="2" data-hint-direction="right" data-hint-offset="small"></div></td></tr>'
          ...
          if (this.coreProps) {
            ...
            if (value) {
              ...
              try {
                ...
                this.lblDate.text(
                  `${value.toLocaleString(lang, { year: "numeric", month: "2-digit", day: "2-digit" })} ${value.toLocaleString(lang, { timeStyle: "short" })}`,
                )
              } catch (e) {
                lang = "en"
                this.lblDate.text(
          ...
          if (props) {
            ...
            if (
              ...
              const pageSizeTemplate = !Common.UI.isRTL()
                ...
                : `<span dir="ltr"><span dir="ltr">${Common.Utils.Metric.getCurrentMetricName()} </span><span dir="ltr">${h} x </span><span dir="ltr">${Common.Utils.Metric.getCurrentMetricName()} </span><span dir="ltr">${w}</span></span>`
        ...
        SetDisabled: function () {
          const disable = !this.mode.isEdit || this._state._locked || this._state.disableEditing
  ...
  PDFE.Views.FileMenuPanels.DocumentRights = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-rights",
        menu: undefined,
  ...
  PDFE.Views.FileMenuPanels.ProtectDoc = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-protect",
        menu: undefined,
        ...
        render: function () {
          ...
          const protection = PDFE.getController("Common.Controllers.Protection").getView()
        ...
        updateSignatures: function () {
          ...
          const tipText = hasInvalid ? this.txtSignedInvalid : hasValid ? this.txtSigned : ""
          this.cntSignatureView.html(
            this.templateSignature({ tipText: tipText, hasSigned: hasValid || hasInvalid }),
          )
  ...
  PDFE.Views.PrintWithPreview = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-print",
        menu: undefined,
        ...
        render: function (node) {
            ...
            data: [
              {
                ...
                size: /^(ca|us)$/i.test(Common.Utils.InternalSettings.get("pdfe-config-region"))
                  ? [25.4, 25.4, 25.4, 25.4]
                  : [20, 30, 20, 15],
              },
          ...
          this.cmbContent = new Common.UI.ComboBox({
            ...
            data: [
              ...
              {
                value: AscPDF.PRINT_CONTENT_TYPES.docAndMarkups,
                displayValue: this.txtDocumentAndMarkups,
              },
              {
                value: AscPDF.PRINT_CONTENT_TYPES.docAndStamps,
                displayValue: this.txtDocumentAndStamps,
              },
        ...
        applyMode: function () {
          ...
          this.mode.isDesktopApp &&
            Common.Utils.isMac &&
            this.$el.find(".desktop-settings.not-macos").hide()
        },
