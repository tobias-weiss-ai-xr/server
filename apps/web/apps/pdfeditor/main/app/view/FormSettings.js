...
define(["text!pdfeditor/main/app/template/FormSettings.template"], (menuTemplate) => {
  PDFE.Views.FormSettings = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#id-form-settings",
        ...
        initialize: function () {
          ...
          this.defFormat = {
            FormatType: AscPDF.FormatType.NONE,
            decimal: 2,
            separator: AscPDF.SeparatorStyle.COMMA_DOT,
            negative: AscPDF.NegativeStyle.BLACK_MINUS,
            symbol: "",
            location: true,
            dateformat: "m/d/yy",
            timeformat: 0,
            special: AscPDF.SpecialFormatType.PHONE,
            regexp: ".",
          }
          this.render()
        },
        ...
        createDelayedElements: function () {
          ...
          $markup
            .findById("#form-advanced-link")
            .on("click", _.bind(this.openAdvancedSettings, this))
          ...
          this.textareaTip.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          ...
          this.cmbFormat = new Common.UI.ComboBox({
            ...
            editable: false,
            data: [
              { displayValue: this.textNone, value: AscPDF.FormatType.NONE },
          ...
          this.txtMask.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          this.txtMask.cmpEl.on("focus", "input.form-control", () => {
            setTimeout(() => {
              this.txtMask._input?.select()
            }, 1)
          })

          let arr = []
          this.api.asc_getFieldDateFormatOptions().forEach((item) => {
            arr.push({
              value: item,
              displayValue: item,
            })
          ...
          arr = []
          const timearr = this.api.asc_getFieldTimeFormatOptions()
          for (const str in timearr) {
          ...
          this.spnCombChars.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          ...
          this.txtExport.on("inputleave", () => {
            this.fireEvent("editcomplete", this)
          })
          this.txtExport.cmpEl.on("focus", "input.form-control", () => {
            setTimeout(() => {
              this.txtExport._input?.select()
            }, 1)
          })
          ...
          this.cmbBehavior = new Common.UI.ComboBox({
            ...
            editable: false,
            data: [
              { displayValue: this.textNone, value: AscPDF.BUTTON_HIGHLIGHT_TYPES.none },
          ...
          this.cmbState = new Common.UI.ComboBox({
            ...
            editable: false,
            data: [
              { displayValue: this.textNormal, value: AscPDF.APPEARANCE_TYPES.normal },
          ...
          } else this.btnSelectImage.menu.items[2].setVisible(false)
          ...
          this.cmbScale = new Common.UI.ComboBox({
            ...
            editable: false,
            data: [
              { displayValue: this.textAlways, value: AscPDF.Api.Types.scaleWhen.always },
          ...
          this.cmbHowScale = new Common.UI.ComboBox({
            ...
            data: [
              {
                displayValue: this.textProportional,
                value: AscPDF.Api.Types.scaleHow.proportional,
              },
          ...
          this.imagePositionPreview_offset =
            this.imagePositionPreview.parent().width() - this.imagePositionPreview.width()
        ...
        onMaxCharsChange: function (field, newValue, oldValue, eOpts) {
          if (this.api && !this._noApply) {
            this.api.SetTextFieldCharLimit(
              this.chMaxChars.getValue() === "checked" ? field.getNumberValue() || 10 : 0,
            )
          }
        },
        ...
        onCombCharsChange: function (field, newValue, oldValue, eOpts) {
          if (this.api && !this._noApply) {
            this.api.SetTextFieldCharLimit(
              this.chComb.getValue() === "checked" ? field.getNumberValue() || 10 : 0,
            )
          }
        },
        ...
        onChDefValue: function (field, newValue, oldValue, eOpts) {
          if (this.api && !this._noApply) {
            ...
            this.api.SetFieldDefaultValue(
              field.getValue() === "checked" ? this.txtExport.getValue() : undefined,
            )
            this.fireEvent("editcomplete", this)
          }
        },
        ...
        insertImageFromStorage: function (data) {
          if (data?._urls && data.c === "control") {
            this._originalSpecProps?.put_ImageUrl(data._urls[0], this._state.State)
          }
        },
        ...
        onImageSelect: function (menu, item) {
          if (item.value === 1) {
                ...
                if (result === "ok") {
                  if (this._originalSpecProps) {
                    ...
                    if (!_.isEmpty(checkUrl)) {
                      this._originalSpecProps.put_ImageUrl(checkUrl, this._state.State)
                    }
                  }
                }
                this.fireEvent("editcomplete", this)
            ...
            this._originalSpecProps?.showFileDialog(this._state.State)
            this.fireEvent("editcomplete", this)
          }
        },
        ...
        onImagePositionChange: function (type, field, newValue, oldValue) {
          const value = (this.imagePositionPreview_offset * newValue) / 100 - 1
          if (type === "x") {
          ...
          this.imagePositionLabel.text(
            `${Math.round(this._state.imgPositionX)},${Math.round(100 - this._state.imgPositionY)}`,
          )
        },
        ...
        imgPositionApplyFunc: function (type) {
          if (this.api && !this._noApply) {
            this.api.SetButtonFieldIconPos(
              this._state.imgPositionX / 100,
              (100 - this._state.imgPositionY) / 100,
            )
            this.fireEvent("editcomplete", this)
          }
        },
        ...
        applyFormatSettings: function (_p) {
          switch (_p.FormatType) {
            ...
            case AscPDF.FormatType.SPECIAL:
              _p.special === -1
                ? this.api.SetFieldMask(_p.mask)
                : this.api.SetFieldSpecialFormat(_p.special)
              break
        ...
        onSpecialChanged: function (combo, record) {
          if (this.api && !this._noApply) {
            ...
            } else this.api.SetFieldSpecialFormat(record.value)
          }
          this.fireEvent("editcomplete", this)
        },
        ...
        ChangeSettings: function (props, isShape) {
          ...
          if (props) {
            ...
            let val = props.asc_getPropLocked()
            ...
            let forceShowHide = false
            const type = props.asc_getType()
            const specProps = props.asc_getFieldProps()
            this._originalSpecProps = specProps

            // common props
            const data = this.api.GetAvailableFieldsNames(type)
            if (
              !this._state.arrName ||
              this._state.arrName.length !== data.length ||
              _.difference(this._state.arrName, data).length > 0
            ) {
              const arr = []
              data.forEach((item) => {
            ...
            if (this._state.Required !== val) {
              this.chRequired.setValue(
                val !== null && val !== undefined ? !!val : "indeterminate",
                true,
              )
            ...
            const color = props.asc_getStroke()
            if (color) {
            ...
            if (
              ...
              this.BorderColor !== "transparent" &&
                this.mnuColorPicker.selectByRGB(
                  typeof this.BorderColor === "object" ? this.BorderColor.color : this.BorderColor,
                  true,
                )
              this._state.BorderColor = this.BorderColor
            }

            const bgColor = props.asc_getFill()
            if (bgColor) {
            ...
            if (
              ...
              this.BackgroundColor !== "transparent" &&
                this.mnuBGColorPicker.selectByRGB(
                  typeof this.BackgroundColor === "object"
                    ? this.BackgroundColor.color
                    : this.BackgroundColor,
                  true,
                )
            ...
            this.cmbLineWidth.setDisabled(
              this._state.DisabledControls || this._state.BorderColor === "transparent",
            )
            ...
            if (type === AscPDF.FIELD_TYPES.text && specProps) {
              ...
              const isComb = specProps.asc_getComb()
              const isMulti = specProps.asc_getMultiline()
              const isPwd = specProps.asc_getPassword()
              const isScroll = specProps.asc_getScrollLongText()
              const isFormatSelected =
                specProps.asc_getFormat() &&
                specProps.asc_getFormat().asc_getType() !== AscPDF.FormatType.NONE

              let combChanged = false
              if (this._state.Comb !== isComb) {
                this.chComb.setValue(
                  isComb !== null && isComb !== undefined ? !!isComb : "indeterminate",
                  true,
                )
              ...
              const isCharLimits = val && val > 0 && !isComb
              if (
              ...
              this.chComb.setDisabled(
                isMulti || isScroll || isCharLimits || isPwd || this._state.DisabledControls,
              )
              ...
              if (this._state.Multi !== isMulti) {
                this.chMulti.setValue(
                  isMulti !== null && isMulti !== undefined ? !!isMulti : "indeterminate",
                  true,
                )
                this._state.Multi = isMulti
              }
              this.chMulti.setDisabled(
                isComb || isPwd || isFormatSelected || this._state.DisabledControls,
              )
              ...
              if (this._state.Scroll !== isScroll) {
                this.chScroll.setValue(
                  isScroll !== null && isScroll !== undefined ? !!isScroll : "indeterminate",
                  true,
                )
            ...
            if (type === AscPDF.FIELD_TYPES.text || type === AscPDF.FIELD_TYPES.combobox) {
              if (specProps) {
                ...
                const format = specProps.asc_getFormat()
                val =
                  format === undefined
                    ? AscPDF.FormatType.NONE
                    : format === null
                      ? null
                      : format.asc_getType() // undefined - none, null - different types
                ...
                if (
                  (this._state.FormatType === AscPDF.FormatType.REGULAR ||
                    this._state.FormatType === AscPDF.FormatType.SPECIAL) &&
              ...
              this.cmbFormat.setDisabled(
                (type === AscPDF.FIELD_TYPES.text && this._state.Multi) ||
            ...
            if (type === AscPDF.FIELD_TYPES.combobox || type === AscPDF.FIELD_TYPES.listbox) {
              this.labelFormName.text(
                type === AscPDF.FIELD_TYPES.combobox ? this.textCombobox : this.textListBox,
              )
              if (specProps) {
                const options = specProps.asc_getOptions()
                const arr = []
            ...
            if (type === AscPDF.FIELD_TYPES.button) {
              ...
              if (specProps) {
                const layout = specProps.asc_getLayout()
                ...
                if (this._state.Behavior !== val) {
                  ...
                  this.cmbState.setDisabled(
                    val !== AscPDF.BUTTON_HIGHLIGHT_TYPES.push || this._state.DisabledControls,
                  )
                }
                if (this._state.Behavior === AscPDF.BUTTON_HIGHLIGHT_TYPES.push) {
                  ...
                  this._state.ButtonCaption[AscPDF.APPEARANCE_TYPES.normal] =
                    specProps.asc_getNormalCaption()
                  this._state.ButtonCaption[AscPDF.APPEARANCE_TYPES.mouseDown] =
                    specProps.asc_getDownCaption()
                  this._state.ButtonCaption[AscPDF.APPEARANCE_TYPES.rollover] =
                    specProps.asc_getHoverCaption()
                } else {
                ...
                if (layout !== AscPDF.Api.Types.position.textOnly) {
                  ...
                  this.cmbHowScale.setDisabled(
                    this._state.Scale === AscPDF.Api.Types.scaleWhen.never ||
                      this._state.DisabledControls,
                  )
                }
              }
            ...
            if (type === AscPDF.FIELD_TYPES.checkbox || type === AscPDF.FIELD_TYPES.radiobutton) {
              const isCheckbox = type === AscPDF.FIELD_TYPES.checkbox
            ...
            if (this.type !== type) this.fireEvent("updatescroller", this)
            this.type = type
          }
        },
        ...
        disableControls: function (disable) {
          ...
          this.btnListAdd.setDisabled(this.txtNewValue.length < 1 || this._state.DisabledControls)
          const isComb = this._state.Comb
          const isMulti = this._state.Multi
          const isPwd = this._state.Pwd
          const isScroll = this._state.Scroll
          this.chAutofit.setDisabled(isComb || this._state.DisabledControls)
          this.chMulti.setDisabled(
            isComb ||
              isPwd ||
              (this.type === AscPDF.FIELD_TYPES.text &&
                this._state.FormatType !== AscPDF.FormatType.NONE) ||
              this._state.DisabledControls,
          )
          this.chScroll.setDisabled(isComb || this._state.DisabledControls)
          this.chPwd.setDisabled(isComb || isMulti || this._state.DisabledControls)
          this.cmbFormat.setDisabled(
            (this.type === AscPDF.FIELD_TYPES.text && isMulti) || this._state.DisabledControls,
          )
          ...
          this.spnMaxChars.setDisabled(
            isComb || this.chMaxChars.getValue() !== "checked" || this._state.DisabledControls,
          )
          this.cmbHowScale.setDisabled(
            this._state.Scale === AscPDF.Api.Types.scaleWhen.never || this._state.DisabledControls,
          )
          this.cmbState.setDisabled(
            this._state.Behavior !== AscPDF.BUTTON_HIGHLIGHT_TYPES.push ||
              this._state.DisabledControls,
          )
        ...
        showHideControls: function (type, specProps) {
          const isCombobox = type === AscPDF.FIELD_TYPES.combobox
          const isText = type === AscPDF.FIELD_TYPES.text
          const isListbox = type === AscPDF.FIELD_TYPES.listbox
          const isCheck = type === AscPDF.FIELD_TYPES.checkbox
          const isRadio = type === AscPDF.FIELD_TYPES.radiobutton
          const isButton = type === AscPDF.FIELD_TYPES.button
          const isImage =
            isButton && specProps.asc_getLayout() !== AscPDF.Api.Types.position.textOnly
          const isButtonText =
            isButton && specProps.asc_getLayout() !== AscPDF.Api.Types.position.iconOnly
          ...
          this.TextSpecialSettings.toggleClass(
            "hidden",
            !(isCombobox || isText) || this._state.FormatType !== AscPDF.FormatType.SPECIAL,
          )
          this.MaskSettings.toggleClass(
            "hidden",
            !(isCombobox || isText) ||
              !(
                this._state.FormatType === AscPDF.FormatType.REGULAR ||
                (this._state.FormatType === AscPDF.FormatType.SPECIAL &&
                  this._state.SpecialType === -1)
              ),
          )
          this.DateSettings.toggleClass(
            "hidden",
            !(isCombobox || isText) || this._state.FormatType !== AscPDF.FormatType.DATE,
          )
          this.TimeSettings.toggleClass(
            "hidden",
            !(isCombobox || isText) || this._state.FormatType !== AscPDF.FormatType.TIME,
          )
