...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  PDFE.Views.FormatSettingsDialog = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 300,
        ...
        initialize: function (options) {
          ...
          this._state = {
            ...
            DateFormat: "",
            TimeFormat: 0,
            NegStyle: AscPDF.NegativeStyle.BLACK_MINUS,
            Mask: "*",
            RegExp: ".",
          }
        ...
        render: function () {
          ...
          this.api.asc_getNumberFormatCurrencySymbols().forEach((item) => {
            currencySymbolsData.push({
              value: item,
              displayValue: item,
            })
          })
          currencySymbolsData.unshift({ value: null, displayValue: this.textNone })
          ...
          this.inputCustomFormat = new Common.UI.InputField({
            ...
            this._state.DateFormat = value
            this.lblExample.text(this.api.asc_getFieldDateTimeFormatExample(value))
          })
          ...
          this.txtMask = new Common.UI.InputField({
            ...
            this._state[this.FormatType === AscPDF.FormatType.REGULAR ? "RegExp" : "Mask"] = value
          })
        ...
        _setDefaults: function (specProps) {
          if (specProps) {
            ...
            this.cmbFormat.setValue(
              val !== null && val !== undefined ? val : AscPDF.FormatType.NONE,
              "",
            )
            this.FormatType = val
            ...
            if (
              this.FormatType === AscPDF.FormatType.DATE ||
              this.FormatType === AscPDF.FormatType.TIME
            ) {
              val = format.asc_getFormat()
              let selectedItem = this.dateTimeList.store.findWhere({ value: val })
              this.inputCustomFormat.setVisible(
                this.FormatType === AscPDF.FormatType.DATE && !selectedItem,
              )
              if (this.FormatType === AscPDF.FormatType.DATE && !selectedItem) {
                selectedItem = this.dateTimeList.store.findWhere({ isCustom: true })
                selectedItem?.set({ value: val })
              ...
              this._state[
                this.FormatType === AscPDF.FormatType.DATE ? "DateFormat" : "TimeFormat"
              ] = val
            } else if (
              this.FormatType === AscPDF.FormatType.NUMBER ||
              this.FormatType === AscPDF.FormatType.PERCENTAGE
            ) {
              ...
              if (this.FormatType === AscPDF.FormatType.NUMBER) {
                ...
                else this.cmbLocation.setValue(format.asc_getCurrencyPrepend() ? 0 : 2)
                val = format.asc_getNegStyle()
                this._state.NegStyle = val
                this.chParens.setValue(
                  val === AscPDF.NegativeStyle.PARENS_BLACK ||
                    val === AscPDF.NegativeStyle.PARENS_RED,
                  true,
                )
                this.chRed.setValue(
                  val === AscPDF.NegativeStyle.RED_MINUS || val === AscPDF.NegativeStyle.PARENS_RED,
                  true,
                )
              }
            }
          ...
          switch (this.FormatType) {
            case AscPDF.FormatType.NUMBER: {
              ...
              return {
                ...
                decimal: this.spnDecimal.getNumberValue(),
                separator: this.cmbSeparator.getValue(),
                negative: this._state.NegStyle,
                symbol: symbol || "",
                location: location < 2,
              }
            }
            ...
            case AscPDF.FormatType.SPECIAL:
              return {
                FormatType: this.FormatType,
                special: this.cmbSpecial.getValue(),
                mask: this.txtMask.getValue(),
              }
        ...
        updateFormatExample: function () {
          ...
          switch (this.FormatType) {
            case AscPDF.FormatType.NUMBER: {
              ...
              str = this.api.asc_getFieldNumberFormatExample(
                this.spnDecimal.getNumberValue(),
                this.cmbSeparator.getValue(),
                this._state.NegStyle,
                symbol || "",
                location < 2,
              )
              break
            ...
            case AscPDF.FormatType.PERCENTAGE:
              str = this.api.asc_getFieldPercentFormatExample(
                this.spnDecimal.getNumberValue(),
                this.cmbSeparator.getValue(),
              )
              break
            case AscPDF.FormatType.DATE:
              ...
              break
            case AscPDF.FormatType.TIME:
              str = this.api.asc_getFieldDateTimeFormatExample(
                this.timeVal2Str[this._state.TimeFormat],
              )
              break
          }
          this.lblExample.toggleClass(
            "red-color",
            this.FormatType === AscPDF.FormatType.NUMBER &&
              (this._state.NegStyle === AscPDF.NegativeStyle.PARENS_RED ||
                this._state.NegStyle === AscPDF.NegativeStyle.RED_MINUS),
          )
        ...
        onNegativeChange: function (field, newValue, oldValue, eOpts) {
          const isRed = this.chRed.getValue() === "checked"
          const isParens = this.chParens.getValue() === "checked"
          this._state.NegStyle = isRed
            ? isParens
              ? AscPDF.NegativeStyle.PARENS_RED
              : AscPDF.NegativeStyle.RED_MINUS
            : isParens
              ? AscPDF.NegativeStyle.PARENS_BLACK
              : AscPDF.NegativeStyle.BLACK_MINUS
          this.updateFormatExample()
        },

        onDateTimeListSelect: function (listView, itemView, record) {
          if (!record) return
          const isCustom = this.FormatType === AscPDF.FormatType.DATE && !!record.get("isCustom")
          this.inputCustomFormat.setVisible(isCustom)
          isCustom && this.inputCustomFormat.setValue(this._state.DateFormatCustom)
          this._state[this.FormatType === AscPDF.FormatType.DATE ? "DateFormat" : "TimeFormat"] =
            isCustom ? this.inputCustomFormat.getValue() : record.get("value")
          this.updateFormatExample()
        },
        ...
        onFormatSelect: function (combo, record, specProps) {
          ...
          const isNumber = this.FormatType === AscPDF.FormatType.NUMBER
          const isPercent = this.FormatType === AscPDF.FormatType.PERCENTAGE
          ...
          this._maskPanel.toggleClass(
            "hidden",
            !(
              this.FormatType === AscPDF.FormatType.REGULAR ||
              (this.FormatType === AscPDF.FormatType.SPECIAL && this._state.SpecialType === -1)
            ),
          )
          if (this.FormatType === AscPDF.FormatType.REGULAR)
            this.txtMask.setValue(this._state.RegExp)
          ...
          if (
            ...
            this.dateTimeList.$el[0].style.height =
              this.FormatType === AscPDF.FormatType.DATE ? "184px" : "116px"
            this.dateTimeList.store.reset(
              this.FormatType === AscPDF.FormatType.DATE ? this.dateFormats : this.timeFormats,
            )
            this.dateTimeList.selectRecord(
              this.dateTimeList.store.at(this.FormatType === AscPDF.FormatType.DATE ? 1 : 0),
            )
            this.dateTimeList.scrollToRecord(this.dateTimeList.store.at(0))
            this.inputCustomFormat.setVisible(false)
          }
          this._examplePanel.toggleClass(
            "hidden",
            this.FormatType === AscPDF.FormatType.SPECIAL ||
              this.FormatType === AscPDF.FormatType.REGULAR ||
              this.FormatType === AscPDF.FormatType.NONE,
          )
          this.updateFormatExample()
