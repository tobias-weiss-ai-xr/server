...
PE.ApplicationController = new (function () {
      ...
      if (data.get_Type() === 1) {
        ...
        if (!$ttEl) {
          ...
          $ttEl.on("shown.bs.tooltip", (e) => {
            ...
            let pos = $ttEl.ttpos[1] - $tooltip.height() + ttOffset[1]
            pos < 0 && (pos = 0)
            $tooltip.css({
  ...
  function onDocumentContentReady() {
    ...
    config.customization?.slidePlayerBackground &&
      api.asc_setDemoBackgroundColor(config.customization.slidePlayerBackground)
    ...
    const zf = config.customization?.zoom ? Number.parseInt(config.customization.zoom) : -1
    ...
    else if (!embedConfig.embedUrl && !embedConfig.fullscreenUrl) $(dividers[2]).hide()
    ...
    PE.ApplicationView.tools.get("#idt-fullscreen").on("click", () => {
      common.utils.openLink(embedConfig.fullscreenUrl)
    })

    PE.ApplicationView.tools.get("#idt-download").on("click", () => {
      if (!!embedConfig.saveUrl && permissions.download !== false) {
    ...
    PE.ApplicationView.tools.get("#idt-print").on("click", () => {
      api.asc_Print(
        new Asc.asc_CDownloadOptions(
          null,
          $.browser.chrome ||
    ...
    PE.ApplicationView.tools.get("#idt-close").on("click", () => {
    ...
    PE.ApplicationView.tools.get("#idt-search").on("click", () => {
  ...
  function onPlayStart(e) {
    if (!isplaymode) {
      ...
      isplaymode === "play" ? api.DemonstrationPause() : api.DemonstrationPlay()
    }

    isplaymode !== "play"
      ? ($("#btn-play button").addClass("pause"), (isplaymode = "play"))
  ...
  function onPlaySlideChanged(number) {
    if (number++ < maxPages) $("#page-number").val(number)
  }
  ...
  function onDownloadAs() {
    ...
    if (api) {
      const options = new Asc.asc_CDownloadOptions(Asc.c_oAscFileType.PPTX, true)
