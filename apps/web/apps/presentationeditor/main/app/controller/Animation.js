...
define([
  ...
  PE.Controllers.Animation = Backbone.Controller.extend(
    _.extend(
      {
        models: [],
        collections: [],
        views: ["PE.Views.Animation"],
        options: {
          alias: "Animation",
        },
        sdkViewName: "#id_main",
        ...
        setApi: function (api) {
          ...
          this.api.asc_registerCallback(
            "asc_onAnimPreviewStarted",
            _.bind(this.onAnimPreviewStarted, this),
          )
          this.api.asc_registerCallback(
            "asc_onAnimPreviewFinished",
            _.bind(this.onAnimPreviewFinished, this),
          )
          this.api.asc_onShowAnimTab(!!this._state.onactivetab)
          this.api.asc_registerCallback(
            "asc_onCloseAnimPane",
            _.bind(this.onApiCloseAnimPane, this),
          )
        ...
        onPreviewClick: function () {
          if (this._state.playPreview) this.api.asc_StopAnimationPreview()
          else this.api.asc_StartAnimationPreview()
        },
        ...
        onParameterClick: function (value, toggleGroup) {
          if (this.api && this.AnimationProperties) {
            if (toggleGroup === "animateeffects") {
              ...
              this.api.asc_SetAnimationProperties(
                this.AnimationProperties,
                !Common.Utils.InternalSettings.get("pe-animation-no-auto-preview"),
              )
            } else if (toggleGroup === "custompath") {
              const groupName = _.findWhere(this.EffectGroups, {
                value: AscFormat.PRESET_CLASS_PATH,
              }).id
              this.addNewEffect(
                AscFormat.MOTION_CUSTOM_PATH,
                AscFormat.PRESET_CLASS_PATH,
                groupName,
                true,
                value,
              )
              ...
              const groupName = _.findWhere(this.EffectGroups, {
                value: this._state.EffectGroup,
              }).id
              this.addNewEffect(
                value,
                this._state.EffectGroup,
                groupName,
                true,
        ...
        onSelectColor: function (color) {
          const groupName = _.findWhere(this.EffectGroups, { value: this._state.EffectGroup }).id
          this.addNewEffect(
            this._state.Effect,
            this._state.EffectGroup,
            groupName,
        ...
        onAnimationAdditional: function (replace) {
          // replace or add new additional effect
        ...
        onAddAnimation: function (picker, record) {
          const type = record.get("value")
          const group = _.findWhere(this.EffectGroups, { id: record.get("group") }).value
          ...
          if (group === AscFormat.PRESET_CLASS_PATH && type === AscFormat.MOTION_CUSTOM_PATH) {
            Common.Utils.lockControls(Common.enumLock.noAnimation, false, {
              array: [this.view.btnParameters],
            })
            Common.Utils.lockControls(Common.enumLock.noAnimationParam, false, {
              array: [this.view.btnParameters],
            })
          }
        },
        ...
        addNewEffect: function (type, group, groupName, replace, parametr, preview, color) {
          const parameter = this.view.setMenuParameters(type, groupName, parametr)
          this.api.asc_AddAnimation(
            group,
            type,
            parameter !== undefined ? parameter : 0,
            color ? color : null,
            replace,
            !Common.Utils.InternalSettings.get("pe-animation-no-auto-preview"),
          )
        ...
        onDurationChange: function (before, combo, record, e) {
          ...
          if (before) {
            ...
            if (!item) {
              const expr = new RegExp(`^\\s*(\\d*(\\.|,)?\\d+)\\s*(${this.view.txtSec})?\\s*$`)
              if (!expr.exec(record.value)) {
                combo.setValue(
                  this._state.Duration,
                  this._state.Duration >= 0 ? `${this._state.Duration} ${this.view.txtSec}` : "",
                )
                e.preventDefault()
                return false
              }
            ...
            if (!record.displayValue)
              value = value > 600 ? 600 : value < 0 ? 0.01 : Number.parseFloat(value.toFixed(2))
        ...
        setDuration: function (valueRecord) {
          if (this.api && this.AnimationProperties) {
            const value = valueRecord < 0 ? valueRecord : valueRecord * 1000
        ...
        onRepeatChange: function (before, combo, record, e) {
          ...
          if (before) {
            ...
            value = Common.Utils.String.parseFloat(record.value)
            if (!record.displayValue)
              value = value > 9999 ? 9999 : value < 1 ? 1 : Math.floor((value + 0.4) * 2) / 2
        ...
        onRepeatComboOpen: (needfocus, combo, e, params) => {
        ...
        onEffectSelect: function (combo, record) {
          if (this.api) {
            ...
            let group = _.findWhere(this.EffectGroups, { id: record.get("group") })
            group = group ? group.value : undefined
            const prevEffect = this._state.Effect
            this._state.Effect = undefined
            this.addNewEffect(
              type,
              group,
              record.get("group"),
              prevEffect !== AscFormat.ANIM_PRESET_NONE,
            )
            if (group === AscFormat.PRESET_CLASS_PATH && type === AscFormat.MOTION_CUSTOM_PATH) {
              Common.Utils.lockControls(Common.enumLock.noAnimation, false, {
                array: [this.view.btnParameters],
        ...
        onFocusObject: function (selectedObjects) {
          ...
          const animatedObjects = this.api.asc_getAnimatedObjectsStack()
          for (let i = 0; i < animatedObjects.length; i++) {
            const type = animatedObjects[i].get_ObjectType()
            if (type === Asc.c_oAscTypeSelectElement.Animation) {
              ...
              const value = animatedObjects[i].get_ObjectValue()
              this._state.lockEmphasis =
                this._state.lockEmphasis ||
                value.get_FromGroup() ||
                value.get_FromChart() ||
                value.get_FromSmartArt() ||
                value.get_FromSmartArtInternal()
            } else if (type === Asc.c_oAscTypeSelectElement.Table) {
              this._state.lockEmphasis = true
            }
          }
          if (this._state.onactivetab) this.setSettings()
        },

        setSettings: function () {
          this._state.noGraphic =
            this._state.noAnimation =
            this._state.noAnimationParam =
            this._state.noTriggerObjects =
            this._state.noMoveAnimationLater =
            this._state.noMoveAnimationEarlier =
              true
          this._state.noAnimationPreview = !this.api.asc_canStartAnimationPreview()
          const view = this.view
          const store = view.listEffects.store
          const fieldStore = view.listEffects.fieldPicker.store
          if (this.AnimationProperties) {
            ...
            let item
            let value = this.AnimationProperties.asc_getType()
            let group = this.AnimationProperties.asc_getClass()
            ...
            if (this._state.Effect !== value || this._state.EffectGroup !== group) {
              ...
              const effect = _.findWhere(
                view.allEffects,
                group
                  ? { group: group.get("id"), value: this._state.Effect }
                  : { value: this._state.Effect },
              )
              const familyEffect = effect ? effect.familyEffect : undefined
              item = !familyEffect
                ? store.findWhere(
                    group ? { group: group.get("id"), value: value } : { value: value },
                  )
                : store.findWhere(
                    group
                      ? { group: group.get("id"), familyEffect: familyEffect }
                      : { familyEffect: familyEffect },
                  )
              if (item) {
                let forceFill = false
                if (!item.get("isCustom")) {
                  // remove custom effect from list if not-custom is selected
                  const rec = store.findWhere({ isCustom: true })
                  forceFill = !!rec
                  rec && store.remove(rec)
                }
                if (this._state.Effect !== AscFormat.ANIM_PRESET_MULTIPLE) {
                  // remove "multiple" item if one effect is selected
                  const rec = fieldStore.findWhere({ value: AscFormat.ANIM_PRESET_MULTIPLE })
                ...
                if (this._state.Effect === AscFormat.ANIM_PRESET_MULTIPLE) {
                  ...
                  // add custom effect to appropriate group
                  if (group) {
                    const items = store.where({ group: group.get("id") })
                    const index =
                      items && items.length > 0
                        ? store.indexOf(items[items.length - 1])
                        : store.length - 1
                    const rec = _.findWhere(Common.define.effectData.getEffectFullData(), {
                      group: group.get("id"),
                      value: this._state.Effect,
                    })
                    item = store.add(
                      new Common.UI.DataViewModel({
                        group: group.get("id"),
                        value: this._state.Effect,
                        iconCls: rec?.iconCls ? rec.iconCls : group.get("iconClsCustom"),
            ...
            if (
              this._state.EffectOption !== null &&
              this._state.Effect !== AscFormat.ANIM_PRESET_MULTIPLE &&
              this._state.Effect !== AscFormat.ANIM_PRESET_NONE
            ) {
              const rec = _.findWhere(this.EffectGroups, { value: this._state.EffectGroup })
              view.setMenuParameters(
                this._state.Effect,
              ...
              this._state.noAnimationParam =
                view.btnParameters.menu.getItemsLength() === view.startIndexParam && !view.isColor
            }

            value = this.AnimationProperties.asc_getDuration()
            this._state.Duration = value >= 0 ? value / 1000 : value // undefined or <0
            if (this._state.noAnimationDuration) view.cmbDuration.setValue("")
            else
            ...
            if (
              Math.abs(this._state.Delay - value) > 0.001 ||
              ((this._state.Delay === null || value === null) && this._state.Delay !== value) ||
              ((this._state.Delay === undefined || value === undefined) &&
                this._state.Delay !== value)
            ) {
              this._state.Delay = value
              view.numDelay.setValue(
                this._state.Delay !== null && this._state.Delay !== undefined
                  ? this._state.Delay / 1000
                  : "",
                true,
              )
            }
            ...
            if (this._state.noAnimationRepeat) view.cmbRepeat.setValue("")
            else
              view.cmbRepeat.setValue(
                this._state.Repeat,
                this._state.Repeat >= 0 ? this._state.Repeat : 1,
              )

            this._state.StartSelect = this.AnimationProperties.asc_getStartType()
            view.cmbStart.setValue(
              this._state.StartSelect !== undefined
                ? this._state.StartSelect
                : AscFormat.NODE_TYPE_CLICKEFFECT,
            )
            ...
            if (this.view?.listEffects) this.view.listEffects.fieldPicker.deselectAll()
          }
          if (!this.arrEmphasis) {
            this.arrEmphasis = store.filter(
              (item, index) =>
                item.get("group") === "menu-effect-group-emphasis" &&
                !(
                  item.get("value") === AscFormat.EMPHASIS_GROW_SHRINK ||
                  item.get("value") === AscFormat.EMPHASIS_SPIN ||
                  item.get("value") === AscFormat.EMPHASIS_TRANSPARENCY ||
                  item.get("value") === AscFormat.EMPHASIS_PULSE ||
                  item.get("value") === AscFormat.EMPHASIS_TEETER ||
                  item.get("value") === AscFormat.EMPHASIS_BLINK
                ),
            )
          }
          this.arrEmphasis.forEach((item) => {
            item.set("disabled", !!this._state.lockEmphasis)
          })
          fieldStore.each((item) => {
            if (
              item.get("group") === "menu-effect-group-emphasis" &&
              !(
                item.get("value") === AscFormat.EMPHASIS_GROW_SHRINK ||
        ...
        setTriggerList: function () {
          ...
          const btnMemnu = this.view.btnClickOf.menu
          this.objectNames.forEach((item) => {
            btnMemnu.addItem({
              caption: item,
              checkable: true,
              toggleGroup: "animtrigger",
              checked: item === this._state.TriggerValue,
            })
          })
          this.view.cmbTrigger.menu.items[0].setChecked(
        ...
        setViewRepeatAndDuration: function (group, type) {
          ...
          if (
            (group === AscFormat.PRESET_CLASS_ENTR && type === AscFormat.ENTRANCE_APPEAR) ||
            (group === AscFormat.PRESET_CLASS_EXIT && type === AscFormat.EXIT_DISAPPEAR)
          ) {
            this._state.noAnimationDuration = this._state.noAnimationRepeat = true
          } else if (
            group === AscFormat.PRESET_CLASS_EMPH &&
            ...
            if (this.view.cmbDuration.store.length === 6) {
              this.view.cmbDuration.store.add([
          ...
          if (
            this.view.cmbDuration.store.length === 8 &&
            (this._state.EffectGroup !== AscFormat.PRESET_CLASS_EMPH ||
              (this._state.EffectGroup === AscFormat.PRESET_CLASS_EMPH &&
                this._state.Effect !== AscFormat.EMPHASIS_BOLD_REVEAL &&
                this._state.Effect !== AscFormat.EMPHASIS_TRANSPARENCY))
          ) {
        ...
        onAddEffectShow: function (picker) {
          if (!this.arrEmphasisAddEffect) {
            this.arrEmphasisAddEffect = picker.store.filter(
              (item, index) =>
                item.get("group") === "menu-effect-group-emphasis" &&
                !(
                  item.get("value") === AscFormat.EMPHASIS_GROW_SHRINK ||
          ...
          this.arrEmphasisAddEffect.forEach((item) => {
            item.set("disabled", !!this._state.lockEmphasis)
          })
        ...
        setLocked: function () {
          ...
          if (this._state.noMoveAnimationEarlier !== undefined)
            this.lockToolbar(
              Common.enumLock.noMoveAnimationEarlier,
              this._state.noMoveAnimationEarlier,
            )
        ...
        getAnimationPanelTip: function (effect) {
          ...
          if (effect) {
            const nodeType =
              effect[0] === AscFormat.NODE_TYPE_CLICKEFFECT
                ? this.view.textStartOnClick
                : effect[0] === AscFormat.NODE_TYPE_WITHEFFECT
                  ? this.view.textStartWithPrevious
                  : effect[0] === AscFormat.NODE_TYPE_AFTEREFFECT
                    ? this.view.textStartAfterPrevious
                    : ""
            let presetClass = _.findWhere(Common.define.effectData.getEffectGroupData(), {
              value: effect[1],
            })
            const presetId = presetClass ? presetClass.id : ""
            presetClass = presetClass ? presetClass.caption : ""
            let preset = _.findWhere(Common.define.effectData.getEffectFullData(), {
              group: presetId,
              value: effect[2],
            })
            preset = preset ? preset.displayValue : ""
            const name = Common.Utils.String.htmlEncode(effect[3]) || ""
            result = `${nodeType}\n${presetClass}\n${preset} : ${name}`
          }
          return result
        },
