...
define([
  ...
  PE.Controllers.LeftMenu = Backbone.Controller.extend(
    _.extend(
      {
        ...
        applySettings: function (menu) {
          let fast_coauth = Common.Utils.InternalSettings.get("pe-settings-coauthmode")
          /** coauthoring begin **/
          ...
          if (this.mode.isEdit) {
            if (this.mode.canChangeCoAuthoring || !fast_coauth) {
              ...
              value = Number.parseInt(Common.localStorage.getItem("pe-settings-autosave"))
            ...
            value = Number.parseInt(Common.localStorage.getItem("pe-settings-paste-button"))
          ...
          /* update zoom */
          const newZoomValue = Common.localStorage.getItem("pe-settings-zoom")
          const oldZoomValue = Common.Utils.InternalSettings.get("pe-settings-zoom")
          const lastZoomValue = Common.Utils.InternalSettings.get("pe-last-zoom")
        ...
        clickToolbarTab: function (tab, e) {
          if (tab === "file") this.leftMenu.menuFile.show()
        ...
        aboutShowHide: function (value) {
          ...
          if (value && this.leftMenu._state.pluginIsRunning) {
            ...
            this.leftMenu.$el.width(
              Common.localStorage.getItem("pe-mainmenu-width") || MENU_SCALE_PART,
            )
            if (this.mode.canCoAuthoring) {
        ...
        onShortcut: function (s, e) {
          ...
          switch (s) {
            ...
            case "escape": {
              ...
              const statusbar = PE.getController("Statusbar")
            ...
            case "chat":
              if (
                ...
                (!previewPanel || !previewPanel.isVisible())
              ) {
                Common.UI.Menu.Manager.hideAll()
        ...
        onMenuChange: function (value) {
          if ("hide" === value) {
            if (this.leftMenu.btnComments.isActive() && this.api) {
              ...
              if (this.leftMenu._state.pluginIsRunning)
                // hide comments panel when plugin is running
        ...
        showHistory: function () {
          if (!this.mode.wopi) {
            const maincontroller = PE.getController("Main")
        ...
        onChangeViewMode: function (mode) {
          if (mode === "master") {
            if (this.leftMenu.btnComments?.pressed) {
              this.leftMenu.close()
            }
          }
