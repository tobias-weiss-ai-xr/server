...
define(["core", "presentationeditor/main/app/view/RightMenu"], () => {
  PE.Controllers.RightMenu = Backbone.Controller.extend({
    ...
    onRightMenuAfterRender: function (rightMenu) {
      ...
      this._settings[Common.Utils.documentSettingsType.Slide] = {
    ...
    onRightMenuClick: function (menu, type, minimized, event) {
      if (!minimized && this.editMode) {
        if (event) {
          // user click event
          const idx = this._priorityArr.indexOf(type)
          if (idx >= 0) this._priorityArr.splice(idx, 1)
    ...
    onFocusObject: function (SelectedObjects, forceSignature, forceOpen) {
      ...
      this._settings[Common.Utils.documentSettingsType.Slide].hidden =
        SelectedObjects.length > 0 ? 0 : 1
      ...
      for (i = 0; i < SelectedObjects.length; i++) {
        ...
        const settingsType = this.getDocumentSettingsType(eltype)
        if (
      ...
      if (this._settings[Common.Utils.documentSettingsType.Slide].locked) {
        // all elements are locked if the slide is locked
        for (i = 0; i < this._settings.length; i++) {
          if (this._settings[i]) this._settings[i].locked = true
        }
      }
      ...
      for (i = 0; i < this._settings.length; i++) {
        ...
        if (pnl.hidden) {
          ...
          if (
            i !== Common.Utils.documentSettingsType.Slide &&
          ...
          if (pnl.needShow) {
            ...
            i !== Common.Utils.documentSettingsType.Slide ||
            this.rightmenu._settings[i].isCurrent
          ) {
            if (activePane === pnl.panelId) currentactive = i
          }

          if (i === Common.Utils.documentSettingsType.Slide) {
            if (pnl.locked !== undefined)
              this.rightmenu.slideSettings.setLocked(
                this._state.no_slides || pnl.lockedBackground || pnl.locked,
                this._state.no_slides || pnl.lockedHeader || pnl.locked,
                pnl.inMaster,
              )
          } else pnl.panel.setLocked(pnl.locked)
        }
      }
      ...
      if (!this.rightmenu.minimizedMode || open) {
        ...
        if (priorityactive < 0) {
          if (this._priorityArr.length > 0) {
            for (i = 0; i < this._priorityArr.length; i++) {
              const type = this._priorityArr[i]
              const pnl = this._settings[type]
              if (
                pnl === undefined ||
                pnl.btn === undefined ||
                pnl.panel === undefined ||
        ...
        else if (!this.rightmenu.GetActivePluginPane())
          active = Common.Utils.documentSettingsType.Slide
        ...
        if (disabled) {
          ...
          if (selectedElements.length > 0)
            this.onFocusObject(
              selectedElements,
              false,
              !Common.Utils.InternalSettings.get("pe-hide-right-settings") &&
    ...
    onInsertTable: function () {
      ...
      const idx = this._priorityArr.indexOf(Common.Utils.documentSettingsType.Table)
      if (idx >= 0) this._priorityArr.splice(idx, 1)
    ...
    onInsertImage: function () {
      ...
      const idx = this._priorityArr.indexOf(Common.Utils.documentSettingsType.Image)
      if (idx >= 0) this._priorityArr.splice(idx, 1)
    ...
    onInsertChart: function () {
      ...
      const idx = this._priorityArr.indexOf(Common.Utils.documentSettingsType.Chart)
      if (idx >= 0) this._priorityArr.splice(idx, 1)
    ...
    onInsertShape: function () {
      ...
      const idx = this._priorityArr.indexOf(Common.Utils.documentSettingsType.Shape)
      if (idx >= 0) this._priorityArr.splice(idx, 1)
    ...
    onInsertTextArt: function () {
      ...
      const idx = this._priorityArr.indexOf(Common.Utils.documentSettingsType.TextArt)
      if (idx >= 0) this._priorityArr.splice(idx, 1)
    ...
    onApiUpdateSignatures: function (valid) {
      ...
      const disabled = !valid || valid.length < 1
    ...
    onApiCountPages: function (count) {
      if (this._state.no_slides !== count <= 0 && this.editMode) {
        ...
        if (this._state.no_slides && !this.rightmenu.minimizedMode) this.rightmenu.clearSelection()
        this._settings[Common.Utils.documentSettingsType.Slide].btn.setDisabled(
          this._state.no_slides,
        )
        this.rightmenu.setDisabledMoreMenuItem(
          this._settings[Common.Utils.documentSettingsType.Slide].btn,
          this._state.no_slides,
        )
      }
    },
    ...
    onChangeViewMode: function (mode) {
      if (this.rightmenu?.slideSettings)
