...
define(["core", "presentationeditor/main/app/view/SlideMasterTab"], () => {
  PE.Controllers.SlideMasterTab = Backbone.Controller.extend(
    _.extend(
      {
        models: [],
        collections: [],
        views: ["SlideMasterTab"],
        sdkViewName: "#id_main",

        initialize: function () {
          this.checkInsertAutoshape = (e) => {
            const cmp = $(e.target)
            ...
            if (cmp.attr("id") !== "editor_sdk" && cmp_sdk.length <= 0) {
              if (
                this.view.btnInsertPlaceholder.pressed &&
                this.view.btnInsertPlaceholder.id !== btn_id
              ) {
        ...
        setApi: function (api) {
          if (api) {
            ...
            this.api.asc_registerCallback(
              "asc_onLayoutFooter",
              _.bind(this.onApiLayoutFooter, this),
            )
        ...
        onApiFocusObject: function (selectedObjects) {
          ...
          Common.Utils.lockControls(Common.enumLock.inSlideMaster, in_slide_master, {
            array: [this.view.btnInsertPlaceholder, this.view.chTitle, this.view.chFooters],
          })
        },
        ...
        onBtnInsertPlaceholder: function (btn, e) {
          btn.menu.getItems(true).forEach((item) => {
            if (item.value === btn.options.currentType) item.setChecked(true)
          })
          if (!btn.pressed) {
        ...
        onMenuInsertPlaceholder: function (btn, e) {
          const oldType = btn.options.currentType
          ...
          if (newType !== oldType) {
            ...
            btn.changeIcon({
              ...
              curr: btn.menu.getItems(true).filter((item) => item.value === oldType)[0].options
                .iconClsForMainBtn,
            })
