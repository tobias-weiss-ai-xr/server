...
define([
  ...
  PE.Controllers.Toolbar = Backbone.Controller.extend(
    _.extend(
      {
        models: [],
        collections: ["SlideThemes"],
        views: ["Toolbar"],
        ...
        initialize: function () {
          ...
          this.slideSizeArr = [
            {
              ...
              ratio: 6858000 / 9144000,
              type: Asc.c_oAscSlideSZType.SzScreen4x3,
            },
            {
              size: [12192000, 6858000],
              ratio: 6858000 / 12192000,
          ...
          const checkInsertAutoshape = (e) => {
            ...
            if (cmp.attr("id") !== "editor_sdk" && cmp_sdk.length <= 0) {
              if (
                (this.toolbar.btnsInsertText.pressed() &&
                  !this.toolbar.btnsInsertText.contains(btn_id)) ||
                this.toolbar.cmbsInsertShape.some(
                  (cmb) => cmb.isComboViewRecActive() && cmb.id !== btn_id,
                )
              ) {
                this._isAddingShape = false
                ...
                this.toolbar.cmbsInsertShape.forEach((cmb) => {
          ...
          this.onApiEndAddShape = function () {
            ...
            if (this.toolbar.btnsInsertText.pressed()) {
              this.toolbar.btnsInsertText.toggle(false, true)
              this.toolbar.btnsInsertText.forEach((button) => {
                button.menu.clearAll(true)
              })
            }

            this.toolbar.cmbsInsertShape.forEach((cmb) => {
              if (cmb.isComboViewRecActive()) cmb.deactivateRecords()
            })
        ...
        onLaunch: function () {
          ...
          PE.getCollection("SlideLayouts").bind({
            reset: this.onResetSlides.bind(this),
          })
        },
        ...
        setMode: function (mode) {
          ...
          Common.UI.FeaturesManager.isFeatureEnabled("featuresTips", true) &&
            Common.UI.TooltipManager.addTips({
              ...
              masterTab: {
                name: "pe-help-tip-master-tab",
                placement: "bottom-right",
                offset: { x: Common.UI.isRTL() ? -10 : 10, y: 0 },
                text: this.helpMasterTab,
                header: this.helpMasterTabHeader,
                target: "li.ribtab #slideMaster",
                automove: true,
                maxwidth: 300,
                closable: false,
                isNewFeature: true,
              ...
              gifPlayback: {
                name: "pe-help-tip-gif-payback",
                placement: "bottom",
                text: this.helpGifPlayback,
                header: this.helpGifPlaybackHeader,
                target: "#toolbar",
                maxwidth: 300,
                automove: true,
                noArrow: true,
        ...
        attachUIEvents: function (toolbar) {
          ...
          toolbar.btnPreview.on(
            "disabled",
            _.bind(this.onBtnChangeState, this, "startover:disabled"),
          )
          ...
          toolbar.btnInsAudio?.on("click", _.bind(this.onAddAudio, this))
          toolbar.btnInsVideo?.on("click", _.bind(this.onAddVideo, this))
        ...
        setApi: function (api) {
          ...
          if (this.mode.isEdit) {
            ...
            this.api.asc_registerCallback(
              ...
              _.bind(this.onApiPageSize, this),
            )
            ...
            this.api.asc_registerCallback(
              "asc_onLockSlideHdrFtrApplyToAll",
              _.bind(this.onApiLockSlideHdrFtrApplyToAll, this, true),
            )
            this.api.asc_registerCallback(
              "asc_onUnLockSlideHdrFtrApplyToAll",
              _.bind(this.onApiLockSlideHdrFtrApplyToAll, this, false),
            )
            ...
            this.api.asc_registerCallback(
              "asc_onCountPages",
              _.bind(this.onApiCountPagesRestricted, this),
            )
          }
          this.api.asc_registerCallback(
            "onPluginToolbarMenu",
        ...
        onChangeCompactView: function (view, compact, suppressSave) {
          ...
          !suppressSave &&
            Common.localStorage.setBool(
              this.mode.isEdit ? "pe-compact-toolbar" : "pe-view-compact-toolbar",
              compact,
            )
        ...
        onApiCanAddHyperlink: function (value) {
          if (this._state.can_hyper !== value && this.editMode) {
            this.toolbar.lockToolbar(Common.enumLock.hyperlinkLock, !value, {
        ...
        onApiPageSize: function (width, height, type, firstNum) {
          if (
            Math.abs(this.currentPageSize.width - width) > 0.001 ||
            ...
            let idx = -1
            for (let i = 0; i < this.slideSizeArr.length; i++) {
        ...
        onApiCountPages: function (count) {
          if (this._state.no_slides !== count <= 0) {
            ...
            this.toolbar.lockToolbar(Common.enumLock.noSlides, this._state.no_slides, {
              array: this.toolbar.cmbsInsertShape.concat([
                this.toolbar.btnChangeSlide,
                this.toolbar.btnPreview,
                this.toolbar.btnPrint,
                this.toolbar.btnCopy,
                this.toolbar.btnCut,
                this.toolbar.btnSelectAll,
                this.toolbar.btnReplace,
                this.toolbar.btnPaste,
                this.toolbar.btnCopyStyle,
                this.toolbar.btnInsertTable,
                this.toolbar.btnInsertChart,
                this.toolbar.btnInsertSmartArt,
                this.toolbar.btnColorSchemas,
                this.toolbar.btnShapeAlign,
                this.toolbar.btnShapesMerge,
                this.toolbar.btnShapeArrange,
                this.toolbar.btnSlideSize,
                this.toolbar.listTheme,
                this.toolbar.btnEditHeader,
                this.toolbar.btnInsDateTime,
                this.toolbar.btnInsSlideNum,
              ]),
            })
            this.toolbar.lockToolbar(Common.enumLock.noSlides, this._state.no_slides, {
              array: this.toolbar.btnsInsertImage.concat(
                this.toolbar.btnsInsertText,
                this.toolbar.btnInsertEquation,
                this.toolbar.btnInsertTextArt,
                this.toolbar.btnInsAudio,
                this.toolbar.btnInsVideo,
              ),
            })
            if (this.btnsComment)
              this.toolbar.lockToolbar(Common.enumLock.noSlides, this._state.no_slides, {
        ...
        onApiFocusObject: function (selectedObjects) {
          ...
          let slide_deleted = undefined
          let slide_layout_lock = undefined
          ...
          this.toolbar.lockToolbar(Common.enumLock.noParagraphObject, !in_para, {
            array: [this.toolbar.btnLineSpace],
          })
          ...
          if (this._state.prcontrolsdisable !== paragraph_locked) {
            ...
            this.toolbar.lockToolbar(Common.enumLock.paragraphLock, paragraph_locked === true, {
              array: [
                this.toolbar.btnInsDateTime,
                this.toolbar.btnInsSlideNum,
                this.toolbar.btnInsertEquation,
          ...
          if (this._state.no_paragraph !== no_paragraph) {
            ...
            this.toolbar.lockToolbar(Common.enumLock.noParagraphSelected, no_paragraph, {
              array: this.toolbar.paragraphControls,
            })
            this.toolbar.lockToolbar(Common.enumLock.noParagraphSelected, no_paragraph, {
              array: [this.toolbar.btnCopyStyle],
            })
          }

          if (this._state.no_text !== no_text) {
            if (this._state.activated) this._state.no_text = no_text
            this.toolbar.lockToolbar(Common.enumLock.noTextSelected, no_text, {
              array: this.toolbar.paragraphControls,
            })
          }
          ...
          if (this._state.no_object !== no_object) {
            ...
            this.toolbar.lockToolbar(Common.enumLock.noObjectSelected, no_object, {
              array: [this.toolbar.btnVerticalAlign],
            })
          }
          ...
          if (this._state.no_drawing_objects !== no_drawing_objects) {
            ...
            this.toolbar.lockToolbar(Common.enumLock.noDrawingObjects, no_drawing_objects, {
              array: [
                this.toolbar.btnShapeAlign,
                this.toolbar.btnShapeArrange,
                this.toolbar.btnShapesMerge,
          ...
          if (shape_locked !== undefined && this._state.shapecontrolsdisable !== shape_locked) {
            ...
            this.toolbar.lockToolbar(Common.enumLock.shapeLock, shape_locked, {
              array: this.toolbar.shapeControls.concat(this.toolbar.paragraphControls),
            })
          }

          if (shape_locked === undefined && !this._state.no_drawing_objects) {
            // several tables selected
            this.toolbar.lockToolbar(Common.enumLock.shapeLock, false, {
              array: this.toolbar.shapeControls,
            })
          }
          ...
          if (
            ...
            this.toolbar.lockToolbar(Common.enumLock.slideLock, slide_layout_lock, {
              array: [this.toolbar.btnChangeSlide],
            })
          }
          ...
          if (slide_deleted !== undefined && this._state.slidecontrolsdisable !== slide_deleted) {
            ...
            this.toolbar.lockToolbar(Common.enumLock.slideDeleted, slide_deleted, {
              array: this.toolbar.slideOnlyControls.concat(this.toolbar.paragraphControls),
            })
          }
          ...
          if (this._state.in_equation !== in_equation) {
            ...
            this.toolbar.lockToolbar(Common.enumLock.inEquation, in_equation, {
              array: [this.toolbar.btnSuperscript, this.toolbar.btnSubscript],
            })
          }

          if (this._state.no_columns !== no_columns) {
            if (this._state.activated) this._state.no_columns = no_columns
            this.toolbar.lockToolbar(Common.enumLock.noColumns, no_columns, {
              array: [this.toolbar.btnColumns],
            })
          }
          ...
          if (this.toolbar.btnChangeSlide) {
            if (this.toolbar.btnChangeSlide.mnuSlidePicker)
              ...
              this.toolbar.btnChangeSlide.mnuSlidePicker = {
                options: { layout_index: layout_index },
              }
          }

          if (this._state.in_smartart !== in_smartart) {
            this.toolbar.lockToolbar(Common.enumLock.inSmartart, in_smartart, {
              array: this.toolbar.paragraphControls,
            })
          ...
          if (this._state.in_smartart_internal !== in_smartart_internal) {
            this.toolbar.lockToolbar(Common.enumLock.inSmartartInternal, in_smartart_internal, {
              array: this.toolbar.paragraphControls,
            })
        ...
        onApiUpdateThemeIndex: function (v) {
          if (this._state.themeId !== v) {
            const listStyle = this.toolbar.listTheme
            const listStylesVisible = listStyle.rendered

            if (listStylesVisible) {
              listStyle.suspendEvents()

              const styleRec = listStyle.menuPicker.store.findWhere({
                themeId: v,
              })
        ...
        onApiLockDocumentTheme: function () {
          this.toolbar.lockToolbar(Common.enumLock.themeLock, true, {
            array: [this.toolbar.btnColorSchemas, this.toolbar.listTheme],
          })
        },

        onApiUnLockDocumentTheme: function () {
          this.toolbar.lockToolbar(Common.enumLock.themeLock, false, {
            array: [this.toolbar.btnColorSchemas, this.toolbar.listTheme],
          })
        },
        ...
        onDuplicateCheck: function (menu) {
          if (this.api) menu.items[2].setDisabled(this.api.getCountPages() < 1)
        },
        ...
        onPreview: (slidenum, presenter, isCurrent) => {
          Common.NotificationCenter.trigger(
            "preview:start",
            _.isNumber(slidenum) ? slidenum : 0,
            presenter,
            false,
            isCurrent,
          )
              ...
              if (selectedElements && _.isArray(selectedElements)) {
                for (let i = 0; i < selectedElements.length; i++) {
                  if (Asc.c_oAscTypeSelectElement.Slide === selectedElements[i].get_ObjectType()) {
                    ...
                    const transition = elValue.get_transition()
                    if (transition) loop = transition.get_ShowLoop()
                  }
                }
              }
        ...
        onSave: function (e) {
          ...
          if (this.api?.asc_isDocumentCanSave) {
          ...
          toolbar.btnSave &&
            toolbar.lockToolbar(
              ...
              !toolbar.mode.forcesave && !toolbar.mode.canSaveDocumentToBinary,
              ...
              if (selectedElements && selectedElements.length > 0) {
                ...
                for (let i = selectedElements.length - 1; i >= 0; i--) {
                  ...
                  if (Asc.c_oAscTypeSelectElement.Shape === elType) {
                    const win = new PE.Views.ShapeSettingsAdvanced({
                      shapeProps: elValue,
        ...
        onShapeAlign: function (menu, item) {
          if (this.api) {
            const value = this.toolbar.mniAlignToSlide.isChecked()
              ? Asc.c_oAscObjectsAlignType.Slide
        ...
        onMenuInsertTextClick: function (btn, e) {
          ...
          if (newType !== oldType) {
            this.toolbar.btnsInsertText.forEach((button) => {
              button.updateHint([e.caption, this.views.Toolbar.prototype.tipInsertText])
              button.changeIcon({
                next: e.options.iconClsForMainBtn,
        ...
        onColorSchemaShow: function (menu) {
          if (this.api) {
            const value = this.api.asc_GetCurrentColorSchemeIndex()
            const item = _.find(menu.getItems(true), (item) => item.value === value)
        ...
        onSlideSize: function (menu, item) {
          if (item.value !== "advanced") {
            const newwidth = this.currentPageSize.height / this.slideSizeArr[item.value].ratio
            this.currentPageSize = {
            ...
            if (this.api)
              this.api.changeSlideSize(
                ...
                this.currentPageSize.type,
                this.currentPageSize.firstNum,
              )
            ...
            const handlerDlg = (dlg, result) => {
              if (result === "ok") {
                ...
                const ratio = this.currentPageSize.height / this.currentPageSize.width
                let idx = -1
                for (let i = 0; i < this.slideSizeArr.length; i++) {
                  if (Math.abs(this.slideSizeArr[i].ratio - ratio) < 0.001) {
                    idx = i
                ...
                this.toolbar.btnSlideSize.menu.items[0].setChecked(idx === 0)
                this.toolbar.btnSlideSize.menu.items[1].setChecked(idx === 1)
                if (this.api) this.api.changeSlideSize(props[1], props[2], props[0], props[3])
              }
        ...
        onListThemeSelect: function (combo, record) {
          ...
          if (this.api && record) this.api.ChangeTheme(record.get("themeId"))
        ...
        onResetAutoshapes: function () {
          const collection = PE.getCollection("ShapeGroups")
          const onComboShowBefore = (menu) => {
            this.toolbar.updateComboAutoshapeMenu(collection)
            menu.off("show:before", onComboShowBefore)
          }
          const onComboClick = (btn, record, cancel) => {
            if (cancel) {
              this._addAutoshape(false)
              return
            }
        ...
        _onInitEditorThemes: function (editorThemes /*array */, documentThemes) {
          ...
          if (!this.toolbar.listTheme) {
            this.themes = [editorThemes, documentThemes]
            return
          }

          const defaultThemes = editorThemes || []
          const docThemes = documentThemes || []

          this.toolbar.listTheme.menuPicker.store.reset([]) // remove all

          const themeStore = this.getCollection("SlideThemes")
          const mainController = this.getApplication().getController("Main")
          if (themeStore) {
            const arr1 = []
            const arr2 = []
            _.each(defaultThemes, (theme, index) => {
              const tip =
                mainController.translationTable[(theme.get_Name() || "").toLocaleLowerCase()] ||
                theme.get_Name()
              arr1.push(
            ...
            _.each(docThemes, (theme) => {
              const image = theme.get_Image()
              const tip =
                mainController.translationTable[(theme.get_Name() || "").toLocaleLowerCase()] ||
                theme.get_Name()
              arr1.push(
            ...
            this.toolbar.listTheme.menuPicker.store.reset(arr2)
          }

          if (
            this.toolbar.listTheme.menuPicker.store.length > 0 &&
            this.toolbar.listTheme.rendered
          ) {
            this.toolbar.listTheme.fillComboView(
              this.toolbar.listTheme.menuPicker.store.at(0),
              true,
            )
        ...
        DisableToolbar: function (disable, viewMode) {
          ...
          if (toolbar.btnAddSlide)
            // toolbar buttons are rendered
            this.toolbar.lockToolbar(Common.enumLock.menuFileOpen, disable, {
              array: [toolbar.btnAddSlide, toolbar.btnChangeSlide, toolbar.btnPreview],
            })

          const hkComments = Common.Utils.isMac ? "command+alt+a" : "alt+h"
          const hkPreview = Common.Utils.isMac ? "command+shift+enter" : "ctrl+f5"
          if (disable) {
        ...
        onAppShowed: function (config) {
          ...
          if (
            Common.localStorage.itemExists(
              editmode ? "pe-compact-toolbar" : "pe-view-compact-toolbar",
            )
          ) {
          ...
          if (config.isEdit) {
            ...
            if ($panel) {
              ...
              this.btnsDrawTab = drawtab.getView().getButtons()
              Array.prototype.push.apply(this.toolbar.lockControls, this.btnsDrawTab)
              Array.prototype.push.apply(this.toolbar.slideOnlyControls, this.btnsDrawTab)
            }

            const transitController = this.getApplication().getController("Transitions")
            transitController
              .setApi(this.api)
              .setConfig({ toolbar: this, mode: config })
              .createToolbarPanel()
            Array.prototype.push.apply(
              this.toolbar.lockControls,
              transitController.getView().getButtons(),
            )
            Array.prototype.push.apply(
              this.toolbar.slideOnlyControls,
              transitController.getView().getButtons(),
            )

            const animationController = this.getApplication().getController("Animation")
            animationController
              .setApi(this.api)
              .setConfig({ toolbar: this, mode: config })
              .createToolbarPanel()
            Array.prototype.push.apply(
              this.toolbar.slideOnlyControls,
              animationController.getView().getButtons(),
            )

            tab = {
              caption: this.toolbar.textTabSlideMaster,
              action: "slideMaster",
              extcls: config.isEdit ? "canedit" : "",
              layoutname: "toolbar-slidemaster",
              dataHintTitle: "M",
            }
            const slideMasterTab = this.getApplication().getController("SlideMasterTab")
            slideMasterTab.setApi(this.api).setConfig({ toolbar: this, mode: config })
            $panel = slideMasterTab.createToolbarPanel()
            if ($panel) {
              const visible = Common.UI.LayoutManager.isElementVisible("toolbar-slidemaster")
              this.toolbar.addTab(tab, $panel, 6)
              this.toolbar.setVisible("slideMaster", !visible)
              !editmode &&
                !compactview &&
                visible &&
              ...
              Array.prototype.push.apply(
                this.toolbar.lockControls,
                slideMasterTab.getView("SlideMasterTab").getButtons(),
              )
              Array.prototype.push.apply(
                this.toolbar.slideOnlyControls,
                slideMasterTab.getView("SlideMasterTab").getButtons(),
            ...
            if (config.canProtect && config.isDesktopApp) {
              if (config.isSignatureSupport || config.isPasswordSupport) {
                // don't add protect panel to toolbar
                tab = {
                  action: "protect",
          ...
          if ($panel) {
            ...
            this.toolbar.addTab(tab, $panel, 9)
        ...
        onAppReady: function (config) {
          ...
          if (config.canCoAuthoring && config.canComments) {
            ...
            this.btnsComment = Common.Utils.injectButtons(
              ...
              [_set.lostConnect, _set.noSlides, _set.slideMasterMode],
              undefined,
              undefined,
              undefined,
              "1",
            ...
            if (this.btnsComment.length) {
              const _comments = PE.getController("Common.Controllers.Comments").getView()
        ...
        onAddAudio: function () {
          this.api?.asc_AddAudio()
        },

        onAddVideo: function () {
          this.api?.asc_AddVideo()
        },
        ...
        onApiBeginSmartArtPreview: function (type) {
          ...
          this.smartArtGroups = this.docHolderMenu
            ...
            : this.toolbar.btnInsertSmartArt.menu.getItems(true)
        ...
        onChangeViewMode: function (mode) {
          ...
          this.toolbar.lockToolbar(Common.enumLock.slideMasterMode, isMaster, {
            array: this.btnsComment.concat([this.toolbar.btnInsertHyperlink]),
          })
        ...
        showStaticElements: function () {
          const showingStaticElements = [
            ...
            {
              el: this.toolbar.btnChangeSlide
                ? this.toolbar.btnChangeSlide.$el.closest(".group")
                : null,
              tabs: ["home", "design"],
              viewMode: "normal",
            },
            {
              el: this.toolbar.btnAddSlideMaster
                ? this.toolbar.btnAddSlideMaster.$el.closest(".group")
                : null,
              tabs: ["home", "design", "ins"],
              viewMode: "master",
            },
          ]
          let countShowingButtons = 0

          showingStaticElements.forEach((item) => {
            if (!item.el) return

            if (
              item.tabs.includes(this._state.activeTab) &&
              (item.viewMode === this._state.viewMode || !item.viewMode)
            ) {
              item.el.show()
          ...
          this.toolbar.$el
            .find("#id-toolbar-separator-end-static")
            [countShowingButtons > 0 ? "show" : "hide"]()
        },
      },
