...
define([], () => {
  PE.Views.AnimationDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 515,
        ...
        initialize: function (options) {
          ...
          this.EffectGroupData.forEach((item) => {
            item.displayValue = item.caption
          })
          if (
            this.options.activeEffect !== undefined &&
            this.options.activeEffect !== AscFormat.ANIM_PRESET_NONE &&
            this.options.activeEffect !== AscFormat.ANIM_PRESET_MULTIPLE
          ) {
            this._state.activeEffect = this.options.activeEffect
            this._state.activeGroupValue = this.options.groupValue
            const group = _.findWhere(this.EffectGroupData, { value: this._state.activeGroupValue })
        ...
        getFocusedComponents: function () {
          return [this.cmbGroup, this.pickerEffectList /*, this.chPreview*/].concat(
        ...
        fillEffect: function () {
          let arr = _.where(this.allEffects, { group: this._state.activeGroup })
          const lockEmphasis = this.lockEmphasis
          let currentIndex
          arr = _.reject(arr, (item) => !!item.notsupported)
          if (this._state.activeGroup === "menu-effect-group-emphasis") {
            arr.forEach((item, index) => {
              if (
                lockEmphasis &&
                !(
                  item.value === AscFormat.EMPHASIS_GROW_SHRINK ||
                  item.value === AscFormat.EMPHASIS_SPIN ||
                  item.value === AscFormat.EMPHASIS_TRANSPARENCY ||
                  item.value === AscFormat.EMPHASIS_PULSE ||
                  item.value === AscFormat.EMPHASIS_TEETER ||
                  item.value === AscFormat.EMPHASIS_BLINK
                )
              )
                item.disabled = lockEmphasis
              else if (currentIndex === undefined) currentIndex = index
            })
          }
          ...
          this.pickerEffectList.groups.reset(
            Common.define.effectData.getLevelEffect(
              this._state.activeGroup === "menu-effect-group-path",
            ),
          )
          this.pickerEffectList.store.reset(arr)
          let item = this.pickerEffectList.store.findWhere({ value: this._state.activeEffect })
          if (!item) {
        ...
        onPreviewChange: (field, newValue, oldValue, eOpts) => {
          Common.Utils.InternalSettings.set(
            "pe-animation-no-preview",
