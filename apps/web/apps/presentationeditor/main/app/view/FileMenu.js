...
define([
  ...
  PE.Views.FileMenu = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#file-menu-panel",
        rendered: false,
        ...
        applyMode: function () {
          ...
          if (!this.panels) {
            this.panels = {
              opts: new PE.Views.FileMenuPanels.Settings({ menu: this }).render(
                this.$el.find("#panel-settings"),
              ),
              info: new PE.Views.FileMenuPanels.DocumentInfo({ menu: this }).render(
                this.$el.find("#panel-info"),
              ),
              rights: new PE.Views.FileMenuPanels.DocumentRights({ menu: this }).render(
          ...
          this.miSaveAs[
            this.mode.canDownload && this.mode.isDesktopApp && this.mode.isOffline ? "show" : "hide"
          ]()
          this.miSave[
          ...
          this.panels.printpreview?.setMode(this.mode)
          ...
          if (this.mode.isSignatureSupport || this.mode.isPasswordSupport) {
            !this.panels.protect &&
              (this.panels.protect = new PE.Views.FileMenuPanels.ProtectDoc({
                menu: this,
              }).render())
          ...
          if (this.mode.canDownload) {
            !this.panels.saveas &&
              (this.panels.saveas = new PE.Views.FileMenuPanels.ViewSaveAs({
                menu: this,
          ...
          if (this.mode.canDownload && (this.mode.canRequestSaveAs || this.mode.saveAsUrl)) {
            !this.panels["save-copy"] &&
              (this.panels["save-copy"] = new PE.Views.FileMenuPanels.ViewSaveCopy({
                menu: this,
                fileType: this.document.fileType,
              }).render())
          }
          ...
          if (this.mode.canPreviewPrint) {
            const printPanel = PE.getController("Print").getView("PrintWithPreview")
        ...
        setMode: function (mode, delay) {
          if (mode.isDisconnected) {
            ...
            if (!mode.enableDownload) this.mode.canPrint = this.mode.canDownload = false
          } else {
            this.mode = mode
          }
        ...
        disableMenu: function (menu, status) {
          if (menu) {
            const item = this._getMenuItem(menu)
            if (item) {
        ...
        onDocumentName: function (name) {
          ...
          if (this.rendered) this.panels.info.updateInfo(this.document)
        },
