...
define([
  ...
  PE.Views.GridSettings = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 214,
        ...
        render: function () {
          ...
          const metric = Common.Utils.Metric.getCurrentMetric()
          this.spnSpacing = new Common.UI.MetricSpinner({
            el: $window.find("#grid-spacing-spin"),
            step: metric === Common.Utils.Metric.c_MetricUnits.pt ? 1 : 0.01,
            width: 86,
            defaultUnit: Common.Utils.Metric.getCurrentMetricName(),
            value:
              metric === Common.Utils.Metric.c_MetricUnits.inch
                ? '1 "'
                : metric === Common.Utils.Metric.c_MetricUnits.pt
                  ? "36 pt"
                  : "1 cm",
            maxValue:
              metric === Common.Utils.Metric.c_MetricUnits.inch
          ...
          this.spnSpacing.on(
            ...
            _.bind(function (field, newValue, oldValue, eOpts) {
              const value = this.spnSpacing.getNumberValue()
              let idx = -1
              for (let i = 0; i < this.arrSpacing.length; i++) {
                const item = this.arrSpacing[i]
                if (
                  (item.spacing < 1 && Math.abs(item.spacing - value) < 0.005) ||
                  (item.spacing >= 1 && Math.abs(item.spacing - value) < 0.001)
                ) {
                  idx = i
                  break
                }
