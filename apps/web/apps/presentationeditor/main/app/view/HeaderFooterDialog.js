...
define([
  ...
  PE.Views.HeaderFooterDialog = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 360,
            ...
            this.options,
            {
              ...
              template: _.template(
                [
                  ...
                  `<label  style="display:block;" class="input-label margin-left-15">${this.textPreview}</label>`,
        ...
        render: function () {
          ...
          this.btnApplyToAll =
            _.find(
              ...
              (item) =>
                item.$el &&
                item.$el
                  .find("#hf-dlg-btn-apply-to-all")
                  .addBack()
                  .filter("#hf-dlg-btn-apply-to-all").length > 0,
          ...
          this.btnApply =
            _.find(
              ...
              (item) =>
                item.$el &&
                item.$el.find("#hf-dlg-btn-apply").addBack().filter("#hf-dlg-btn-apply").length > 0,
        ...
        getDefaultFocusableComponent: function () {
          if (!this.cmbFormat.isDisabled()) return this.cmbFormat
          if (!this.inputFixed.isDisabled()) return this.inputFixed
          if (!this.inputFooter.isDisabled()) return this.inputFooter
        ...
        afterRender: function () {
          const value = Common.Utils.InternalSettings.get("pe-settings-datetime-default")
          const arr = value ? value.split(";") : []
          this.defaultFormats = []
          arr.forEach((item) => {
            const pair = item.split(" ")
            this.defaultFormats[Number.parseInt(pair[0])] = pair[1]
          })
        ...
        setType: function (type, field, newValue) {
          ...
          if (type === "date") {
            _.each(this.dateControls, (item) => {
              item.setDisabled(!newValue)
            })
            newValue &&
              this.setDateTimeType(this.radioFixed.getValue() ? "fixed" : "update", null, true)
            ...
            newValue &&
              setTimeout(() => {
                this.inputFooter.cmpEl.find("input").focus()
              }, 50)
          } else if (type === "header") {
            ...
            newValue &&
              setTimeout(() => {
                this.inputHeader.cmpEl.find("input").focus()
              }, 50)
          }
        ...
        updateFormats: function (lang, format) {
          const props = new AscCommonSlide.CAscDateTime()
          props.put_Lang(lang)
          const data = props.get_DateTimeExamples()
          const arr = []
        ...
        fillHFSettings: function () {
          ...
          let val = this.props.get_ShowDateTime()
          ...
          let format
          const datetime = this.props.get_DateTime()
          const item = this.cmbLang.store.findWhere({
            value: datetime ? datetime.get_Lang() || this.lang : this.lang,
          })
          ...
          this.inputFixed.setValue(
            datetime.get_CustomDateTime() ||
              (this.inited[this.type] ? "" : this.cmbFormat.getRawValue()),
          )
