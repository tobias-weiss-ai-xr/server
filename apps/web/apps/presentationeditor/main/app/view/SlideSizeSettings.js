...
define(["common/main/lib/component/Window", "common/main/lib/component/ComboBox"], () => {
  PE.Views.SlideSizeSettings = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 250,
        ...
        render: function () {
          ...
          this.cmbSlideSize = new Common.UI.ComboBox({
            ...
            data: [
              {
                value: Asc.c_oAscSlideSZType.SzScreen4x3,
                displayValue: this.txtStandard,
                size: [9144000, 6858000],
              },
              {
                value: Asc.c_oAscSlideSZType.SzScreen16x9,
                displayValue: `${this.txtWidescreen} (16:9)`,
                size: [9144000, 5143500],
              },
              {
                value: Asc.c_oAscSlideSZType.SzScreen16x10,
                displayValue: `${this.txtWidescreen} (16:10)`,
                size: [9144000, 5715000],
              },
              {
                value: Asc.c_oAscSlideSZType.SzLetter,
                displayValue: this.txtLetter,
                size: [9144000, 6858000],
              },
              {
                value: Asc.c_oAscSlideSZType.SzLedger,
                displayValue: this.txtLedger,
                size: [12179300, 9134475],
              },
              {
                value: Asc.c_oAscSlideSZType.SzA3,
                displayValue: this.txtA3,
                size: [12801600, 9601200],
              },
              {
                value: Asc.c_oAscSlideSZType.SzA4,
                displayValue: this.txtA4,
                size: [9906000, 6858000],
              },
              {
                value: Asc.c_oAscSlideSZType.SzB4ISO,
                displayValue: this.txtB4,
                size: [10826750, 8120063],
              },
              {
                value: Asc.c_oAscSlideSZType.SzB5ISO,
                displayValue: this.txtB5,
                size: [7169150, 5376863],
              },
              {
                value: Asc.c_oAscSlideSZType.Sz35mm,
                displayValue: this.txt35,
                size: [10287000, 6858000],
              },
              {
                value: Asc.c_oAscSlideSZType.SzOverhead,
                displayValue: this.txtOverhead,
                size: [9144000, 6858000],
              },
              {
                value: Asc.c_oAscSlideSZType.SzBanner,
                displayValue: this.txtBanner,
                size: [7315200, 914400],
              },
              {
                value: Asc.c_oAscSlideSZType.SzWidescreen,
                displayValue: this.txtWidescreen,
                size: [12192000, 6858000],
              },
              {
                value: Asc.c_oAscSlideSZType.SzCustom,
                displayValue: this.txtCustom,
                size: [10058400, 7772400],
              },
            ],
          })
          this.cmbSlideSize.setValue(Asc.c_oAscSlideSZType.SzScreen4x3)
          this.cmbSlideSize.on(
            "selected",
            _.bind(function (combo, record) {
              ...
              if (record.value < 0) {
                ...
                const w = record.size[0] / 36000
                const h = record.size[1] / 36000
                const orient = this.cmbSlideOrientation.getValue()
          ...
          this.spnWidth.on(
            ...
            _.bind(function (field, newValue, oldValue, eOpts) {
              if (!this._noApply) {
                ...
                const w = this.spnWidth.getNumberValue()
                const h = this.spnHeight.getNumberValue()
        ...
        setSettings: function (type, pagewitdh, pageheight, slidenum) {
          ...
          const portrait = pageheight > pagewitdh
          const w = portrait ? pageheight : pagewitdh
          const h = portrait ? pagewitdh : pageheight
          const store = this.cmbSlideSize.store
          const arr = []
          let preset = Asc.c_oAscSlideSZType.SzCustom
          for (let i = 0; i < store.length; i++) {
            const item = store.at(i)
            if (
              Math.abs(item.get("size")[0] - w) < 0.001 &&
              Math.abs(item.get("size")[1] - h) < 0.001
            )
              arr.push(item.get("value"))
          }
          if (arr.length > 1) {
            for (let i = 0; i < arr.length; i++) arr[i] === type && (preset = arr[i])
          } else if (arr.length > 0) preset = arr[0]
          this.cmbSlideSize.setValue(preset)
          this.spnSlideNum.setValue(slidenum, true)
        },

        getSettings: function () {
          const type = this.cmbSlideSize.getValue()
          let width
          let height
          if (type === Asc.c_oAscSlideSZType.SzCustom) {
            ...
            const record = this.cmbSlideSize.getSelectedRecord()
            const orient = this.cmbSlideOrientation.getValue()
