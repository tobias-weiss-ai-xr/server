...
define([
  ...
  PE.Views.ViewTab = Common.UI.BaseView.extend(
    _.extend(
      (() => {
        const template =
        ...
        return {
          ...
          setEvents: function () {
            ...
            this.btnSlideMaster?.on(
              "toggle",
              _.bind((btn, state) => {
                this.fireEvent("mode:master", [state])
              }, this),
            )
            this.btnFitToSlide?.on("click", () => {
              this.fireEvent("zoom:toslide", [this.btnFitToSlide])
            })
            this.btnFitToWidth?.on("click", () => {
              this.fireEvent("zoom:towidth", [this.btnFitToWidth])
            })
            this.chToolbar?.on(
              "change",
              _.bind((checkbox, state) => {
                this.fireEvent("toolbar:setcompact", [this.chToolbar, state !== "checked"])
              }, this),
            )
            this.chStatusbar?.on(
              "change",
              _.bind((checkbox, state) => {
                this.fireEvent("statusbar:hide", [this.chStatusbar, state !== "checked"])
              }, this),
            )
            this.chRulers?.on(
              "change",
              _.bind((checkbox, state) => {
                this.fireEvent("rulers:change", [state === "checked"])
              }, this),
            )
            this.chNotes?.on(
              "change",
              _.bind((checkbox, state) => {
                this.fireEvent("notes:change", [this.chNotes, state === "checked"])
              }, this),
            )
            this.cmbZoom
            ...
            this.btnGuides.on(
              "toggle",
              _.bind((btn, state) => {
                this.fireEvent("guides:show", [state])
              }, this),
            )
            this.btnGuides.menu.on(
              "item:click",
              _.bind((menu, item) => {
                if (item.value === "add-vert" || item.value === "add-hor")
                  this.fireEvent("guides:add", [item.value])
                else if (item.value === "clear") this.fireEvent("guides:clear")
                else if (item.value === "smart") this.fireEvent("guides:smart", [item.isChecked()])
                else this.fireEvent("guides:show", [item.isChecked()])
              }, this),
            )
            this.btnGuides.menu.on(
              "show:after",
              _.bind((btn, state) => {
                this.fireEvent("guides:aftershow")
              }, this),
            )

            this.btnGridlines.on(
              "toggle",
              _.bind((btn, state) => {
                this.fireEvent("gridlines:show", [state])
              }, this),
            )
            this.btnGridlines.menu.on(
              "item:click",
              _.bind((menu, item) => {
                if (item.value === "custom") this.fireEvent("gridlines:custom")
                else if (item.value === "snap") this.fireEvent("gridlines:snap", [item.isChecked()])
                else if (item.value === "show") this.fireEvent("gridlines:show", [item.isChecked()])
                else this.fireEvent("gridlines:spacing", [item.value])
              }, this),
            )
            this.btnGridlines.menu.on(
              "show:after",
              _.bind((btn, state) => {
                this.fireEvent("gridlines:aftershow")
              }, this),
            )
            this.chLeftMenu.on(
              "change",
            ...
            this.btnSelectTool?.on(
              "toggle",
              _.bind((btn, state) => {
                state && this.fireEvent("pointer:select")
              }, this),
            )
            this.btnHandTool?.on(
              "toggle",
              _.bind((btn, state) => {
                state && this.fireEvent("pointer:hand")
              }, this),
            )
          },
          ...
          initialize: function (options) {
            ...
            this.chNotes = new Common.UI.CheckBox({
              ...
              value: !Common.localStorage.getBool(
                "pe-hidden-notes",
                this.appConfig.customization && this.appConfig.customization.hideNotes === true,
              ),
              lock: [_set.disableOnStart],
              dataHint: "1",
              dataHintDirection: "left",
          ...
          getPanel: function () {
            ...
            this.btnSelectTool?.render($host.find("#slot-btn-select-tool"))
            this.btnHandTool?.render($host.find("#slot-btn-hand-tool"))
              ...
              me.btnGuides.setMenu(
                new Common.UI.Menu({
                  ...
                  items: [
                    ...
                    { caption: "--" },
                    {
                      caption: me.textAddVGuides,
                      iconCls: "menu__icon btn-vertical-guide",
                      value: "add-vert",
                    },
                    {
                      caption: me.textAddHGuides,
