...
class AddOtherController extends Component {
  ...
  onStyleClick(type) {
    ...
    const dialog = f7.dialog
      .create({
        ...
        text: "",
        content: `<div class="content-block"><div class="row row-picker"><div class="col-50">${_t.textColumns}</div><div class="col-50">${_t.textRows}</div></div><div id="picker-table-size"></div></div>`,
        buttons: [
          {
            text: _t.textCancel,
          },
          {
            ...
            onClick: () => {
              ...
              api.put_Table(
                Number.parseInt(size[0]),
                Number.parseInt(size[1]),
                undefined,
                type.toString(),
              )
            },
  ...
  onGetTableStylesPreviews = () => {
    if (this.props.storeTableSettings.arrayStylesDefault.length === 0) {
      ...
      setTimeout(
        () =>
          this.props.storeTableSettings.setStyles(api.asc_getTableStylesPreviews(true), "default"),
        1,
      )
    }
  }
  ...
  hideAddComment() {
    ...
    if (stack.length > 0) {
      ...
      let objectLocked =
        typeof topObjectValue.get_Locked === "function" ? topObjectValue.get_Locked() : false
      !objectLocked &&
        (objectLocked =
          typeof topObjectValue.get_LockDelete === "function"
            ? topObjectValue.get_LockDelete()
            : false)
      if (!objectLocked) {
