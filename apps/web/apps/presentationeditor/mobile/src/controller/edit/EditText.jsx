...
class EditTextController extends Component {
  ...
  onDistanceBefore(distance, isDecrement) {
    ...
    newDistance = Number.parseFloat(newDistance.toFixed(2))

    api.put_LineSpacingBeforeAfter(
      0,
      newDistance < 0 ? -1 : Common.Utils.Metric.fnRecalcToMM(newDistance),
    )
  }
  ...
  onBullet(type) {
    const api = Common.EditorApi.get()
    api.put_ListType(0, Number.parseInt(type))
  }

  onNumber(type) {
    const api = Common.EditorApi.get()
    api.put_ListType(1, Number.parseInt(type))
  }
  ...
  getIconsBulletsAndNumbers(arrayElements, type) {
    ...
    arrayElements.forEach((item) => {
      arr.push({
        numberingInfo: {
          bullet: item.numberingInfo === "undefined" ? undefined : JSON.parse(item.numberingInfo),
        },
        divId: item.id,
      })
    })
