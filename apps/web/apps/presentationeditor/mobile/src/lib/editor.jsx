...
export const initEditorStyles = (storeSlideSettings) => {
  ...
  api.asc_registerCallback("asc_onInitEditorStyles", (styles) => {
    const standardThemes = styles[0] || []
    const customThemes = styles[1] || []
    const themes = []
...
export const initFocusObjects = (storeFocusObjects) => {
  ...
  storeFocusObjects.intf.filterFocusObjects = () => {
    ...
    for (const object of storeFocusObjects._focusObjects) {
      ...
      if (Asc.c_oAscTypeSelectElement.Paragraph === type) {
        ...
        if (
          !value.get_LockLayout() &&
          !value.get_LockBackground() &&
          !value.get_LockTransition() &&
          !value.get_LockTiming()
        ) {
          arr.push("slide")
        }
...
export const ContextMenu = {
  ...
  mapMenuItems(controller) {
    ...
    if (stack.length > 0) {
      ...
      const lastValue = lastItem.get_ObjectValue()
  ...
  handleMenuItemClick(controller, action) {
    ...
    const actionHandlers = {
      ...
      openlink: () => {
        const link = api
          .getSelectedElements()
          .find((item) => item.get_ObjectType() === Asc.c_oAscTypeSelectElement.Hyperlink)
