...
export class storeFocusObjects {
  ...
  get paragraphLocked() {
    ...
    for (const object of this._focusObjects) {
    ...
    if (this._focusObjects.length > 0) {
      ...
      this._focusObjects.forEach((object) => {
        ...
        if (type === Asc.c_oAscTypeSelectElement.Slide) {
          ...
          slide_lock =
            objectValue.get_LockLayout() ||
            objectValue.get_LockBackground() ||
            objectValue.get_LockTransition() ||
            objectValue.get_LockTiming()
