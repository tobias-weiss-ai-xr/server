  ...
  return (
    <Page>
      <Navbar className="navbar-link-settings" title={_t.textLinkType} backLink={_t.textBack}>
        {Device.phone && (
          <NavRight>
            <Link icon="icon-close" popupClose={!isNavigate ? "#add-link-popup" : ".add-popup"} />
          </NavRight>
        )}
      </Navbar>
      <List>
  ...
  return (
    <Page>
      <Navbar className="navbar-link-settings">
        <NavLeft>
          <Link
            text={Device.ios ? t("View.Add.textCancel") : ""}
            onClick={() => {
              isNavigate
        ...
        <NavRight>
          <Link
            className={`${typeLink === 1 && link.length < 1 && "disabled"}`}
            onClick={() => {
              props.onInsertLink(
                typeLink,
                typeLink === 1
...
export { ObservablePageLink, PageLinkTo, PageTypeLink }
