  ...
  return (
    <Fragment>
      <List>
        ...
        />
        <ListItem
          title={t("View.Edit.textTransitions")}
          link="/transition/"
          routeProps={{
      ...
      <List>
        ...
        />
        <ListItem
          className={isLockResetBackground ? "disabled" : ""}
          title={_t.textResetBackground}
          onClick={() => {
      ...
      <List className="buttons-list">
        <ListButton className="button-fill button-raised" onClick={props.onDuplicateSlide}>
          {_t.textDuplicateSlide}
        </ListButton>
        <ListButton className="button-red button-fill button-raised" onClick={props.onRemoveSlide}>
          {_t.textDeleteSlide}
        </ListButton>
      </List>
    </Fragment>
        ...
        <List className="multi-column slide-theme__list">
          {arrayThemes.map((theme) => {
            return (
              <ListItem
                key={theme.themeId}
                className={theme.themeId === slideThemeIndex ? "item-theme active" : "item-theme"}
                style={{
                  backgroundPosition: `0 -${theme.offsety}px`,
                  backgroundImage: theme.imageUrl && `url(${theme.imageUrl})`,
                }}
                onClick={() => {
            ...
            <List className="multi-column slide-layout__list" key={index}>
              {layouts.map((layout) => {
                return (
                  <ListItem
                    key={layout.type}
                    className={slideLayoutIndex === layout.type ? "active" : ""}
                    onClick={() => {
...
const PageTransition = (props) => {
  ...
  const _arrEffectType = [
    ...
    {
      displayValue: _t.textHorizontalIn,
      value: Asc.c_oAscSlideTransitionParams.Split_HorizontalIn,
    },
    {
      displayValue: _t.textHorizontalOut,
    ...
    {
      displayValue: _t.textCounterclockwise,
  ...
  const getEffectName = (effect) => {
    for (let i = 0; i < _arrEffect.length; i++) {
  ...
  const getEffectTypeName = (type) => {
    for (let i = 0; i < _arrCurrentEffectTypes.length; i++) {
  ...
  const _effectDelay = transitionObj.get_SlideAdvanceDuration()

  const [stateRange, changeRange] = useState(
    _effectDelay !== null && _effectDelay !== undefined ? Number.parseInt(_effectDelay / 1000) : 0,
  )
  ...
  useEffect(() => {
    changeRange(
      _effectDelay !== null && _effectDelay !== undefined
        ? Number.parseInt(_effectDelay / 1000)
        : 0,
    )
  }, [_effectDelay])
  ...
  return (
    <Page className="slide-transition">
      ...
      <List>
        ...
        ></ListItem>
        <ListItem
          title={_t.textDuration}
          disabled={_effect === Asc.c_oAscSlideTransitionTypes.None}
        >
          {!isAndroid && (
            <div slot="after-start">
              <label>
                {_effectDuration !== null && _effectDuration !== undefined
                  ? `${Number.parseInt(_effectDuration / 1000)} ${_t.textSec}`
                  : ""}
              </label>
            </div>
          )}
          <div slot="after" className="splitter">
            <Segmented>
              <Button
                ...
                onClick={() => {
                  let duration = Number.parseInt(_effectDuration / 1000)
      ...
      <List className="buttons-list">
        <ListButton className="button-fill button-raised" onClick={props.onApplyAll}>
          {_t.textApplyAll}
        </ListButton>
      </List>
    </Page>
          ...
          {_arrEffect.map((elem, index) => {
            return (
              <ListItem
                key={index}
                radio
                name="editslide-effect"
                title={elem.displayValue}
                value={elem.value}
                checked={elem.value === currentEffect}
                onChange={() => {
                  setEffect(elem.value)
                  const valueEffectTypes = props.fillEffectTypes(elem.value)
          ...
          {_arrCurrentEffectTypes.map((elem, index) => {
            return (
              <ListItem
                key={index}
                radio
                name="editslide-effect-type"
                title={elem.displayValue}
                value={elem.value}
                checked={elem.value === currentType}
                onChange={() => {
                  setType(elem.value)
...
const StyleFillColor = inject(
  "storeSlideSettings",
  "storePalette",
  "storeFocusObjects",
)(observer(PageFillColor))
const CustomFillColor = inject(
  "storeSlideSettings",
  "storePalette",
  "storeFocusObjects",
