...
define(["core", "spreadsheeteditor/main/app/view/CellEditor"], (Viewport) => {
  SSE.Controllers.CellEditor = Backbone.Controller.extend({
    ...
    setApi: function (api) {
      ...
      this.api.asc_registerCallback(
        ...
        _.bind(this.onCellEditorTextChange, this),
      )
      this.api.asc_registerCallback(
    ...
    setMode: function (mode) {
      ...
      this.editor.btnNamedRanges.setVisible(
        this.mode.isEdit &&
      ...
      !this.mode.isEditDiagram &&
        this.api.asc_registerCallback(
          ...
          _.bind(this.onApiCellSelection, this),
        )
    },

    onInputKeyDown: (e) => {
      if (
        Common.UI.Keys.UP === e.keyCode ||
        Common.UI.Keys.DOWN === e.keyCode ||
        Common.UI.Keys.TAB === e.keyCode ||
        Common.UI.Keys.RETURN === e.keyCode ||
        Common.UI.Keys.ESC === e.keyCode ||
        Common.UI.Keys.LEFT === e.keyCode ||
        Common.UI.Keys.RIGHT === e.keyCode
      ) {
        const menu = $("#menu-formula-selection") // for formula menu
        if (menu.hasClass("open")) menu.find(".dropdown-menu").trigger("keydown", e)
      }
    },
    ...
    onLaunch: function () {
      ...
      const val = Common.localStorage.getItem("sse-celleditor-height")
      this.editor.keep_height = val !== null && Number.parseInt(val) > 0 ? Number.parseInt(val) : 74
      ...
      this.editor.btnNamedRanges.menu
        .on("item:click", _.bind(this.onNamedRangesMenu, this))
    ...
    onApiEditCell: function (state) {
      ...
      this.editor.$btnfunc.toggleClass(
        "disabled",
        state === Asc.c_oAscCellEditorState.editText || this.isUserProtected,
      )
    },
    ...
    onApiSelectionChanged: function (info) {
      ...
      const coauth_disable =
        !this.mode.isEditMailMerge && !this.mode.isEditDiagram && !this.mode.isEditOle
      ...
      this.editor.$btnfunc.toggleClass(
        "disabled",
        is_image || is_mode_2 || coauth_disable || this.isUserProtected,
      )
    },
    ...
    onApiDisconnect: function () {
      ...
      const controller = this.getApplication().getController("FormulaDialog")
      if (controller) {
    ...
    onCellsRange: function (status) {
      ...
      this.editor.$btnfunc.toggleClass(
        "disabled",
        status !== Asc.c_oAscSelectionDialogType.None || this.isUserProtected,
      )
    },
    ...
    onCellName: function (e) {
      if (e.keyCode === Common.UI.Keys.RETURN) {
        const name = this.editor.$cellname.val()
        if (name?.length) {
    ...
    onBlurCellEditor: function () {
      if (this.api.isCEditorFocused === "clear") this.api.isCEditorFocused = undefined
      else if (this.api.isCellEdited) this.api.isCEditorFocused = true
    ...
    expandEditorField: function () {
      let editorMinHeight = Number.parseFloat(this.editor.$el.css("min-height")) || 20
      editorMinHeight -= Number.parseFloat(this.editor.$el.css("border-bottom-width")) || 0
    ...
    onNamedRangesMenu: function (menu, item) {
      if (item.options.value === "manager") {
        ...
        new SSE.Views.NameManagerDlg({
          ...
          .on("close", (win) => {
            this.rangeListSort = win.getSettings()
          })
          .show()
      } else {
    ...
    onNameBeforeShow: function () {
      const names = this.api.asc_getDefinedNames(
        Asc.c_oAscGetDefinedNamesList.WorksheetWorkbook,
        true,
      )
      const rangesMenu = this.editor.btnNamedRanges.menu
      let prev_name = ""

      rangesMenu.removeItems(2, rangesMenu.items.length - 1)
      names.sort((item1, item2) => {
        const n1 = item1.asc_getName(true).toLowerCase()
        const n2 = item2.asc_getName(true).toLowerCase()
        if (n1 === n2) return 0
        return n1 < n2 ? -1 : 1
      })
      _.each(names, (field, index) => {
        const name = field.asc_getName(true)
        if (prev_name !== name) {
          rangesMenu.addItem(
    ...
    SetDisabled: function (disabled) {
      this.editor.$btnfunc[
        !disabled && this.mode && this.mode.isEdit && !this.isUserProtected
          ? "removeClass"
          : "addClass"
      ]("disabled")
      this.editor.btnNamedRanges.setVisible(
    ...
    setPreviewMode: function (mode) {
      ...
      this.viewmode = mode
      this.editor.$btnfunc[
        !mode && this.mode && this.mode.isEdit && !this.isUserProtected ? "removeClass" : "addClass"
      ]("disabled")
      this.editor.cellNameDisabled(
        mode &&
          !(
            this.mode?.isEdit &&
            !this.mode.isEditDiagram &&
