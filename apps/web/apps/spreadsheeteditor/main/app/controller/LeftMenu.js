...
define([
  ...
  SSE.Controllers.LeftMenu = Backbone.Controller.extend(
    _.extend(
      {
        ...
        initialize: function () {
          ...
          Common.NotificationCenter.on(
            "script:loaded:spellcheck",
            _.bind(this.initializeSpellcheck, this),
          )
        },
        ...
        setApi: function (api) {
          ...
          if (this.mode.canCoAuthoring) {
            ...
            if (this.mode.canComments) {
              ...
              const comments = this.getApplication().getController(
                "Common.Controllers.Comments",
              ).groupCollection
              for (const name in comments) {
                const collection = comments[name]
                const resolved = Common.Utils.InternalSettings.get("sse-settings-resolvedcomment")
        ...
        initializeSpellcheck: function () {
          if (this.mode.isEdit && Common.UI.FeaturesManager.canChange("spellcheck")) {
            Common.UI.LayoutManager.isElementVisible("leftMenu-spellcheck") &&
              this.leftMenu.btnSpellcheck.show()
            this.leftMenu.setOptionsPanel(
              "spellcheck",
              this.getApplication().getController("Spellcheck").getView("Spellcheck"),
            )
        ...
        clickMenuFileItem: function (menu, action, isopts) {
          ...
          switch (action) {
            ...
            case "export-pdf":
              ...
              break
            case "print":
              Common.NotificationCenter.trigger("print", this.leftMenu)
              break
            case "exit":
        ...
        clickSaveAsFormat: function (menu, format) {
          ...
          if (format === Asc.c_oAscFileType.CSV) {
            this.showLostDataWarning(() => {
              Common.NotificationCenter.trigger(
                "download:advanced",
                Asc.c_oAscAdvancedOptionsID.CSV,
                this.api.asc_getAdvancedOptions(),
                2,
                new Asc.asc_CDownloadOptions(format),
              )
              menu.hide()
            })
          ...
          new Common.Views.TextInputDialog({
            ...
            handler: (result, value) => {
              if (result === "ok") {
                ...
                this.clickSaveCopyAsFormat(menu, format, ext, value)
              }
            },
          }).show()
        },
        ...
        applySettings: function (menu) {
          let value = Common.localStorage.getBool("sse-settings-cachemode", true)
          ...
          let fast_coauth = Common.Utils.InternalSettings.get("sse-settings-coauthmode")
          ...
          if (this.mode.isEdit) {
            if (this.mode.canChangeCoAuthoring || !fast_coauth) {
              ...
              value = Number.parseInt(Common.localStorage.getItem("sse-settings-autosave"))
              Common.Utils.InternalSettings.set("sse-settings-autosave", value)
              this.api.asc_setAutoSaveGap(value)
            }

            value = Number.parseInt(Common.localStorage.getItem("sse-settings-paste-button"))
          ...
          let reg = Common.localStorage.getItem("sse-settings-reg-settings")
          const baseRegSettings = Common.Utils.InternalSettings.get(
          ...
          if (baseRegSettings) {
            this.api.asc_setLocale(Number.parseInt(reg), undefined, undefined)
          } else {
            this.api.asc_setLocale(
              Number.parseInt(reg),
              Common.localStorage.getItem("sse-settings-decimal-separator"),
              Common.localStorage.getItem("sse-settings-group-separator"),
            )
          }
          ...
          /* update zoom */
          const newZoomValue = Common.localStorage.getItem("sse-settings-zoom")
          if (newZoomValue > 0) {
            const oldZoomValue = Common.Utils.InternalSettings.get("sse-settings-zoom")
            if (
              oldZoomValue === null ||
              oldZoomValue === -3 ||
              oldZoomValue / 100 === this.api.asc_getZoom()
            ) {
        ...
        applySpellcheckSettings: function (menu) {
          if (this.mode.isEdit && this.api && Common.UI.FeaturesManager.canChange("spellcheck")) {
            let value = Common.localStorage.getBool("sse-spellcheck-ignore-uppercase-words")
            ...
            if (value) {
              this.api.asc_setDefaultLanguage(Number.parseInt(value))
            }
          }
        ...
        onApiAddComments: function (data) {
          ...
          for (let i = 0; i < data.length; ++i) {
            if (
              ...
              AscCommon.UserInfoParser.canViewComment(data[i].asc_getUserName())
            ) {
              this.leftMenu.markCoauthOptions("comments")
              break
            }
        ...
        onAppAddComment: function (sender, to_doc) {
          if (to_doc) {
            new Promise((resolve, reject) => {
              ...
              const ctrl = SSE.getController("Common.Controllers.Comments")
        ...
        commentsShowHide: function (state) {
          if (this.api) {
            const value = Common.Utils.InternalSettings.get("sse-settings-livecomment")
            const resolved = Common.Utils.InternalSettings.get("sse-settings-resolvedcomment")
            ...
            if (state) {
              ...
              Common.UI.TooltipManager.getNeedShow("commentFilter") &&
        ...
        menuFilesShowHide: function (state) {
          ...
          if (this.dlgSearch) {
            if (state === "show") this.dlgSearch.suspendKeyEvents()
            else Common.Utils.asyncCall(this.dlgSearch.resumeKeyEvents, this.dlgSearch)
          }
        },
          ...
          switch (s) {
            ...
            case "search":
              ...
              if (!this.leftMenu.btnSearchBar.isDisabled()) {
                ...
                if (this.leftMenu.menuFile.isVisible()) this.leftMenu.menuFile.hide()
                ...
                if (this.isSearchPanelVisible()) {
                  ...
                  this.leftMenu.fireEvent("search:aftershow", [
            ...
            case "save":
              ...
              return false
            case "help":
              if (this.mode.canHelp && !this.isEditFormula) {
                // TODO: unlock 'help' panel for 'view' mode
                Common.UI.Menu.Manager.hideAll()
                this.api.asc_closeCellEditor()
            ...
            case "escape": {
              ...
              const statusbar = SSE.getController("Statusbar")
              ...
              if (
                this.mode.canPlugins &&
                this.leftMenu.panelPlugins &&
              ...
              if (this.mode.isEditDiagram || this.mode.isEditMailMerge || this.mode.isEditOle) {
                const searchBarBtn =
                  (this.mode.isEditMailMerge || this.mode.isEditOle) &&
                  this.getApplication().getController("Toolbar").toolbar.btnSearch
                const isSearchOpen = searchBarBtn?.pressed
        ...
        onCellsRange: function (status) {
          const isRangeSelection = status !== Asc.c_oAscSelectionDialogType.None
          ...
          if (this.mode.canPlugins && this.leftMenu.panelPlugins) {
            Common.Utils.lockControls(Common.enumLock.selRangeEdit, isRangeSelection, {
              array: this.leftMenu.panelPlugins.lockedControls,
            })
        ...
        onApiEditCell: function (state) {
          ...
          if (this.mode.canPlugins && this.leftMenu.panelPlugins) {
            Common.Utils.lockControls(Common.enumLock.editFormula, this.isEditFormula, {
              array: this.leftMenu.panelPlugins.lockedControls,
            })
        ...
        warnDownloadCsvSheets:
          "The CSV format does not support saving a multi-sheet file.<br>To keep the selected format and save only the current sheet, press Save.<br>To save the current spreadsheet, click Cancel and save it in a different format.",
        textSave: "Save",
