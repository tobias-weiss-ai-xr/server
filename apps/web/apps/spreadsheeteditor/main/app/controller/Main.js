...
define([
  ...
  SSE.Controllers.Main = Backbone.Controller.extend(
    _.extend(
      (() => {
        ...
        return {
          ...
          initialize: function () {
            ...
            var styleNames = [
              ...
              translate = {
                ...
                "Multi-Select (Alt+S)":
                  this.txtMultiSelect +
                  Common.Utils.String.platformKey(
                    "Alt+S",
                    " (" + (Common.Utils.isMac ? Common.Utils.String.textCtrl + "+" : "") + "{0})",
                  ),
                "Clear Filter (Alt+C)":
                  this.txtClearFilter +
                  Common.Utils.String.platformKey(
            ...
            styleNames.forEach((item) => {
              translate[item] = this["txtStyle_" + item.replace(/ /g, "_")] || item
            })
            schemeNames.forEach((item) => {
              translate[item] = this["txtScheme_" + item.replace(/[ -]/g, "_")] || item
            })
            translate["Currency [0]"] = this.txtStyle_Currency + " [0]"
            translate["Comma [0]"] = this.txtStyle_Comma + " [0]"

            for (var i = 1; i < 7; i++) {
              translate["Accent" + i] = this.txtAccent + i
              translate["20% - Accent" + i] = "20% - " + this.txtAccent + i
              translate["40% - Accent" + i] = "40% - " + this.txtAccent + i
              translate["60% - Accent" + i] = "60% - " + this.txtAccent + i
            }
            this.translationTable = translate
          },
          ...
          onLaunch: function () {
            ...
            this.api.asc_setFontRenderingMode(Number.parseInt(value))
            ...
            this.api.asc_registerCallback(
              "asc_onFrameEditorReady",
              _.bind(this.onFrameEditorReady, this),
            )
            ...
            Common.NotificationCenter.on(
              "protectedrange:locked",
              _.bind(this.onProtectedRangeLocked, this),
            )
            ...
            $(document.body).on("focus", "input, textarea:not(#ce-cell-content)", (e) => {
              if (this.isAppDisabled === true) return

              if (e && e.target && !/area_id/.test(e.target.id)) {
                if (/msg-reply/.test(e.target.className)) this.dontCloseDummyComment = true
                else if (/textarea-control/.test(e.target.className)) this.inTextareaControl = true
                else if (
            ...
            $(document.body)
              .on("blur", "input, textarea", (e) => {
                if (this.isAppDisabled === true || this.isFrameClosed) return

                if (
                  (!Common.Utils.ModalWindow.isVisible() ||
                    $(".asc-window.enable-key-events:visible").length > 0) &&
                  !(this.loadMask && this.loadMask.isVisible())
                ) {
                  ...
                  if (
                    ...
                    if (/msg-reply/.test(e.target.className)) this.dontCloseDummyComment = false
              ...
              .on("dragover", (e) => {
                ...
                if (
                  ...
                  !(
                    $(event.target).is("#statusbar_bottom") ||
                    $.contains($("#statusbar_bottom"), $(event.target))
                  )
                ) {
            ...
            Common.NotificationCenter.on({
              ...
              "menu:show": (e) => {},
              "menu:hide": (menu, isFromInputControl) => {
                if (
                  !Common.Utils.ModalWindow.isVisible() &&
                  (!menu || !menu.cmpEl.hasClass("from-cell-edit")) &&
                  !isFromInputControl
                ) {
                  this.api.asc_InputClearKeyboardElement()
                  this.api.asc_enableKeyEvents(true)
                }
              },
          ...
          loadConfig: function (data) {
            ...
            if (this.appOptions.customization.font) {
              ...
              if (this.appOptions.customization.font.size) {
                var size = Number.parseInt(this.appOptions.customization.font.size)
                !isNaN(size) &&
                  document.documentElement.style.setProperty(
            ...
            this.appOptions.canCreateNew =
              ...
              (this.editorConfig.templates && this.editorConfig.templates.length)
            ...
            if (
              ...
              this.appOptions.canRenameAnonymous &&
              !this.appOptions.isEditDiagram &&
              !this.appOptions.isEditMailMerge &&
              !this.appOptions.isEditOle
            )
            ...
            this.headerView = this.getApplication()
              .getController("Viewport")
              .getView("Common.Views.Header")
            this.headerView.setCanBack(
            ...
            var reg = Common.localStorage.getItem("sse-settings-reg-settings"),
              isUseBaseSeparator = Common.localStorage.getBool(
            ...
            if (reg !== null) this.api.asc_setLocale(Number.parseInt(reg), decimal, group)
            else {
              reg = this.appOptions.region
              reg = Common.util.LanguageInfo.getLanguages().hasOwnProperty(reg)
                ? reg
                : Common.util.LanguageInfo.getLocalLanguageCode(reg)
              if (reg !== null) reg = Number.parseInt(reg)
              else
                reg = this.editorConfig.lang
                  ? Number.parseInt(
                      Common.util.LanguageInfo.getLocalLanguageCode(this.editorConfig.lang),
                    )
                  : 0x0409
            ...
            if (
              ...
              $("#editor_sdk").append(
                '<div class="doc-placeholder">' +
                  '<div class="columns"></div>'.repeat(2) +
                  "</div>",
              )
            }
            ...
            Common.Utils.InternalSettings.set(
              ...
              value !== null ? Number.parseInt(value) : 0,
            )
          ...
          loadDocument: function (data) {
            ...
            if (data.doc) {
              ...
              var coEditMode = !(
                this.editorConfig.coEditing && typeof this.editorConfig.coEditing == "object"
              )
                ? "fast"
                : // fast by default
                  this.editorConfig.mode === "view" && this.editorConfig.coEditing.change !== false
                  ? "fast"
              ...
              if (this.editorConfig.customization) {
                if (
                  this.editorConfig.customization.showVerticalScroll !== undefined &&
                  this.editorConfig.customization.showVerticalScroll !== null
                )
                  docInfo.asc_putShowVerticalScroll(
                    this.editorConfig.customization.showVerticalScroll,
                  )
                if (
                  this.editorConfig.customization.showHorizontalScroll !== undefined &&
                  this.editorConfig.customization.showHorizontalScroll !== null
                )
                  docInfo.asc_putShowHorizontalScroll(
                    this.editorConfig.customization.showHorizontalScroll,
                  )
              }
          ...
          onProcessRightsChange: function (data) {
            if (data && data.enabled === false) {
              var old_rights = this._state.lostEditingRights
          ...
          onDownloadAs: function (format) {
            ...
            if (!_format || _supported.indexOf(_format) < 0) _format = Asc.c_oAscFileType.XLSX
          ...
          onSetFavorite: function (favorite) {
            this.appOptions.canFavorite &&
              this.headerView &&
              this.headerView.setFavorite(!!favorite)
          },
              ...
              if (
                ...
                !(
                  (
                    ...
                    id == Asc.c_oAscAsyncAction["LoadImage"] ||
                    id == Asc.c_oAscAsyncAction["UploadImage"]
                  )
          ...
          onDocumentContentReady: function () {
            ...
            if (me.appOptions.isEdit && !me.appOptions.canModifyFilter)
              Common.UI.TooltipManager.showTip({
                step: "cantModifyFilter",
                text: me.warnModifyFilter,
                target: "#toolbar",
                maxwidth: 400,
                showButton: false,
                automove: true,
            ...
            value =
              ...
              this.appOptions.isEditOle
                ? 100
                : Common.localStorage.getItem("sse-settings-zoom")
            Common.Utils.InternalSettings.set("sse-settings-zoom", value)
            var zf =
              value !== null
                ? Number.parseInt(value) / 100
                : this.appOptions.customization && this.appOptions.customization.zoom
                  ? Number.parseInt(this.appOptions.customization.zoom) / 100
                  : 1
            value = Common.localStorage.getItem("sse-last-zoom")
            var lastZoom = value !== null ? Number.parseInt(value) / 100 : 1
            ...
            if (Common.UI.FeaturesManager.canChange("spellcheck")) {
              ...
              var ignoreUppercase = Common.localStorage.getBool(
                "sse-spellcheck-ignore-uppercase-words",
                true,
              )
              Common.Utils.InternalSettings.set(
                "sse-spellcheck-ignore-uppercase-words",
              ...
              var ignoreNumbers = Common.localStorage.getBool(
                "sse-spellcheck-ignore-numbers-words",
                true,
              )
              Common.Utils.InternalSettings.set(
                "sse-spellcheck-ignore-numbers-words",
            ...
            me.api.asc_registerCallback(
              ...
              _.bind(me.onActiveSheetChanged, me),
            )
            ...
            if (
              !me.appOptions.isEditMailMerge &&
              !me.appOptions.isEditDiagram &&
              !me.appOptions.isEditOle
            ) {
              pluginsController.setApi(me.api)
              this.api &&
                this.api.asc_setFrozenPaneBorderType(
                  Common.localStorage.getBool("sse-freeze-shadow", true)
                    ? Asc.c_oAscFrozenPaneBorderType.shadow
                    : Asc.c_oAscFrozenPaneBorderType.line,
                )
              application
            ...
            if (me.appOptions.isEdit) {
              ...
              Common.Utils.InternalSettings.set("sse-settings-paste-button", Number.parseInt(value))
              me.api.asc_setVisiblePasteButton(!!Number.parseInt(value))
              ...
              var timer_sl = setInterval(() => {
                if (
                  ...
                  Common.NotificationCenter.trigger("comments:updatefilter", [
                    "doc",
                    "sheet" + me.api.asc_getActiveWorksheetId(),
                  ])
          ...
          onEditingDisable: function (disable, options, type) {
            ...
            if (options.header) {
              ...
              this.headerView &&
                this.headerView.lockHeaderBtns(
                  "undo",
                  options.viewMode,
          ...
          onOpenDocument: function (progress) {
            ...
            proc =
              ...
              ": " +
              Common.Utils.String.fixedDigits(Math.min(Math.round(proc * 100), 100), 3, "  ") +
              "%"
          ...
          onEditorPermissions: function (params) {
            ...
            if (
              ...
              if (params.asc_getRights() !== Asc.c_oRights.Edit) this.permissions.edit = false
              ...
              this.appOptions.canFavorite =
                this.appOptions.spreadsheet.info &&
                this.appOptions.spreadsheet.info.favorite !== undefined &&
                this.appOptions.spreadsheet.info.favorite !== null
              this.appOptions.canFavorite &&
                this.headerView &&
                this.headerView.setFavorite(this.appOptions.spreadsheet.info.favorite)
              ...
              this.appOptions.canUseReviewPermissions =
                ...
                (!!this.permissions.reviewGroups ||
                  (this.appOptions.canLicense &&
                    ...
                    typeof this.editorConfig.customization.reviewPermissions == "object"))
            ...
            } else this.appOptions.canModifyFilter = true

            this.appOptions.canRequestEditRights = this.editorConfig.canRequestEditRights
            this.appOptions.canEdit =
              this.permissions.edit !== false && // can edit
            ...
            if (
              ...
              if (!this.appOptions.canChangeCoAuthoring) {
                ...
                if (value === null) {
                  ...
                  if (
                    value === null &&
                    !Common.localStorage.itemExists("sse-settings-autosave") &&
                    this.appOptions.customization &&
                    this.appOptions.customization.autosave === false
                  ) {
              ...
              fastCoauth = value === null || Number.parseInt(value) == 1
              ...
              if (!this.appOptions.canChangeCoAuthoring || value === null) {
                ...
                value =
                  this.editorConfig.coEditing && this.editorConfig.coEditing.mode === "strict"
                    ? 0
                    : 1
              }
              fastCoauth = Number.parseInt(value) == 1
            } else {
              fastCoauth = false
              autosave = 0
            }
          ...
          applyModeCommonElements: function () {
                  ...
                  $.proxy(function (e) {
                    if (this.isDiagramDrag || this.isDiagramResize) {
                      Common.Gateway.internalMessage("processMouse", {
                        event: "mouse:move",
                        pagex: e.pageX * Common.Utils.zoom(),
                        pagey: e.pageY * Common.Utils.zoom(),
                      })
                    }
                  }, this),
                )
            ...
            if (
              ...
              this.api.asc_registerCallback(
                ...
                _.bind(this.onSendFromFrameToGeneralEditor, this),
              )
            ...
            formulasDlgController &&
              formulasDlgController.setMode(this.appOptions).setApi(this.api, true)
          },
          ...
          applyModeEditorElements: function (prevmode) {
            ...
            reviewController
              ...
              .loadDocument({ doc: this.appOptions.spreadsheet })

            var value = Common.localStorage.getItem("sse-settings-unit")
            value =
              value !== null
                ? Number.parseInt(value)
                : this.appOptions.customization && this.appOptions.customization.unit
            ...
            var toolbarController = application.getController("Toolbar")
            toolbarController && toolbarController.setApi(this.api)

            if (this.appOptions.isRestrictedEdit) {
              application
                .getController("WBProtection")
                .setMode(this.appOptions)
                .setConfig({ config: this.editorConfig }, this.api)
            } else if (this.appOptions.isEdit) {
              // set api events for toolbar in the Restricted Editing mode
              var statusbarController = application.getController("Statusbar"),
                rightmenuController = application.getController("RightMenu"),
                fontsControllers = application.getController("Common.Controllers.Fonts")

              fontsControllers && fontsControllers.setApi(this.api)
              //                    statusbarController && statusbarController.setApi(me.api);
              rightmenuController && rightmenuController.setApi(this.api)
              ...
              if (statusbarController) {
                statusbarController.getView("Statusbar").changeViewMode(this.appOptions)
                ;(this.appOptions.isEditOle || this.appOptions.isEditDiagram) &&
                  statusbarController.onChangeViewMode(null, true, true) // set compact status bar for ole editing mode
              }
              ...
              if (
                ...
                !this.appOptions.isEditDiagram &&
                !this.appOptions.isEditOle &&
                this.appOptions.canFeaturePivot
              )
                application.getController("PivotTable").setMode(this.appOptions)

              if (
                !this.appOptions.isEditMailMerge &&
                !this.appOptions.isEditDiagram &&
                !this.appOptions.isEditOle &&
                this.appOptions.canFeatureTable
              )
                application.getController("TableDesignTab").setMode(this.appOptions)

              var viewport = this.getApplication().getController("Viewport").getView("Viewport")
              viewport.applyEditorMode()
              rightmenuController.getView("RightMenu").setMode(this.appOptions).setApi(this.api)
              ...
              if (
                ...
                var options = {}
                JSON.parse(Common.localStorage.getItem("sse-hidden-formula")) &&
                  (options.formula = true)
                application.getController("Toolbar").hideElements(options)
              } else rightmenuController.getView("RightMenu").hide()
              ...
              this.api.asc_registerCallback(
                ...
                _.bind(this.onConvertEquationToMath, this),
              )
              ...
              this.api.asc_setFilteringMode &&
                this.api.asc_setFilteringMode(this.appOptions.canModifyFilter)
            ...
            if (!this.appOptions.isEdit)
              window.onbeforeunload = _.bind(this.onBeforeUnloadView, this)
          },
          ...
          onError: function (id, level, errData, callback) {
            ...
            switch (id) {
              ...
              case Asc.c_oAscError.ID.DataValidate:
                ...
                if (icon !== undefined) {
                  config.iconCls =
                    icon == Asc.c_oAscEDataValidationErrorStyle.Stop
                      ? "error"
                      : icon == Asc.c_oAscEDataValidationErrorStyle.Information
                        ? "info"
                        : "warn"
                }
                errData &&
                  errData.asc_getErrorTitle() &&
                  (config.title = Common.Utils.String.htmlEncode(errData.asc_getErrorTitle()))
                config.buttons = ["ok", "cancel"]
                config.msg =
                  errData && errData.asc_getError()
                    ? Common.Utils.String.htmlEncode(errData.asc_getError())
                    : this.errorDataValidate
                config.maxwidth = 600
                break
              ...
              case Asc.c_oAscError.ID.RemoveDuplicates:
                ...
                config.buttons = ["ok"]
                config.msg =
                  errData.asc_getDuplicateValues() !== null &&
                  errData.asc_getUniqueValues() !== null
                    ? Common.Utils.String.format(
                        this.errRemDuplicates,
                        errData.asc_getDuplicateValues(),
                        errData.asc_getUniqueValues(),
                      )
                    : this.errNoDuplicates
                config.maxwidth = 600
                break
              ...
              case Asc.c_oAscError.ID.FillAllRowsWarning:
                ...
                if (this.appOptions.isDesktopApp && this.appOptions.isOffline) {
                  config.msg =
                    fill > have
                      ? Common.Utils.String.format(
                          this.textFormulaFilledAllRowsWithEmpty,
                          fillWithSeparator,
                        )
                      : Common.Utils.String.format(this.textFormulaFilledAllRows, fillWithSeparator)
                  config.buttons = [
                    { caption: this.textFillOtherRows, primary: true, value: "fillOther" },
                    "close",
                  ]
                } else {
                  config.msg =
                    fill >= have
                      ? Common.Utils.String.format(
                          this.textFormulaFilledFirstRowsOtherIsEmpty,
                          fillWithSeparator,
                        )
                      : Common.Utils.String.format(
                          this.textFormulaFilledFirstRowsOtherHaveData,
                          fillWithSeparator,
                          (have - fill).toLocaleString(lang),
                        )
                  config.buttons = ["ok"]
                }
                config.maxwidth = 400
              ...
              case Asc.c_oAscError.ID.ConvertationOpenFormat:
                ...
                if (Common.Utils.InternalSettings.get("import-xml-start"))
                  ...
                  config.msg = this.errorInconsistentExtPdf.replace(
                    "%1",
                    this.appOptions.spreadsheet.fileType || "",
                  )
              ...
              case Asc.c_oAscError.ID.MacroUnavailableWarning:
                config.msg = this.errorMacroUnavailableWarning.replace(
                  "%1",
                  errData ? "'" + errData + "'" : "",
                )
                config.maxwidth = 600
            ...
            if (level == Asc.c_oAscError.Level.Critical) {
              ...
              config.callback = _.bind(function (btn) {
                if (
                  ...
                  id == Asc.c_oAscError.ID.FillAllRowsWarning &&
                  btn === "fillOther" &&
          ...
          showTips: function (strings, options) {
            ...
            function showNextTip() {
              ...
              if (str_tip) {
                if (!(options && options.hideCloseTip)) str_tip += "\n" + me.textCloseTip
            ...
            if (this._state.isDocModified !== change || force) {
              if (this.headerView) {
                ...
                if (change) {
                  ...
                  if (this._state.fastCoauth && this._state.usersCount > 1) {
                    this._state.timerCaption = setTimeout(() => {
                      this.headerView.setDocumentCaption(
                        this.headerView.getDocumentCaption(),
                        false,
                      )
                    }, 500)
                  } else
          ...
          disableSaveButton: function (isCanSave) {
            ...
            var forcesave = this.appOptions.forcesave || this.appOptions.canSaveDocumentToBinary,
              isSyncButton = this.toolbarView.btnCollabChanges.rendered
                ? this.toolbarView.btnCollabChanges.cmpEl.hasClass("notify")
                : false,
              isDisabled =
                (!isCanSave && !isSyncButton && !forcesave) ||
                this._state.isDisconnected ||
                (this._state.fastCoauth && this._state.usersCount > 1 && !forcesave)
            this.toolbarView.lockToolbar(Common.enumLock.cantSave, isDisabled, {
              array: [this.toolbarView.btnSave],
            })
          },
          ...
          onBeforeUnload: function () {
            ...
            var isEdit =
              this.permissions.edit !== false &&
              this.editorConfig.mode !== "view" &&
              this.editorConfig.mode !== "editdiagram" &&
              this.editorConfig.mode !== "editmerge" &&
              this.editorConfig.mode !== "editole"
          ...
          onAdvancedOptions: function (type, advOptions, mode, formatOptions) {
            ...
            if (type == Asc.c_oAscAdvancedOptionsID.CSV) {
              const fileName = this.api && this.api.asc_getDocumentName(),
            ...
            if (!this.appOptions.isEditMailMerge && window.editor_elements_prepared) {
              ...
              if (
                ...
                Common.NotificationCenter.trigger(
                  ...
                  ["doc", "sheet" + this.api.asc_getWorksheetId(index)],
                  false,
                ) //  hide popover
              }
            }
            ...
            switch (id) {
              ...
              case Asc.c_oAscConfirm.ConfirmReplaceFormulaInTable:
                Common.UI.warning({
                  title: this.notcriticalErrorTitle,
                  msg:
                    id == Asc.c_oAscConfirm.ConfirmReplaceRange
                      ? this.confirmMoveCellRange
                      : this.confirmReplaceFormulaInTable,
                  buttons: ["yes", "no"],
                  primary: "yes",
                  callback: _.bind((btn) => {
                    if (apiCallback) {
                    ...
                    if (btn == "yes") {
                      this.onEditComplete(
                        this.application.getController("DocumentHolder").getView("DocumentHolder"),
                      )
                    }
                  }, this),
                })
              ...
              case Asc.c_oAscConfirm.ConfirmChangeProtectRange:
                var win = new Common.Views.OpenDialog({
                  ...
                  validatePwd: false,
                  handler: (result, value) => {
                    if (this.api && apiCallback) {
                      if (result == "ok") {
                        this.api.asc_checkProtectedRangesPassword(
                          value.drmOptions.asc_getPassword(),
                          data,
                          (res) => {
                            apiCallback(res, false)
                          },
                        )
                      } else apiCallback(false, true)
                    }
                    this.onEditComplete(
                      this.application.getController("DocumentHolder").getView("DocumentHolder"),
                    )
              ...
              case Asc.c_oAscConfirm.ConfirmAddCellWatches:
                Common.UI.warning({
                  title: this.notcriticalErrorTitle,
                  msg:
                    data > Asc.c_nAscMaxAddCellWatchesCount
                      ? Common.Utils.String.format(
                          this.confirmAddCellWatchesMax,
                          Asc.c_nAscMaxAddCellWatchesCount,
                        )
                      : Common.Utils.String.format(this.confirmAddCellWatches, data),
                  buttons: ["yes", "no"],
                  primary: "yes",
                  callback: _.bind((btn) => {
                    if (apiCallback) {
              ...
              case Asc.c_oAscConfirm.ConfirmReplaceHeaderFooterPicture:
                Common.UI.warning({
                  ...
                  buttons: [
                    { value: "ok", caption: this.textReplace, primary: true },
                    { value: "cancel", caption: this.textKeep },
                  ],
                  maxwidth: 600,
                  callback: _.bind((btn) => {
                    if (apiCallback) {
          ...
          checkProtectedRange: function (callback, scope, args) {
            var result =
              this.api.asc_isProtectedSheet() && this.api.asc_checkLockedCells()
                ? this.api.asc_checkProtectedRange()
                : false
            if (result === null) {
              this.onError(
            ...
            if (result) {
              var win = new Common.Views.OpenDialog({
                ...
                handler: (result, value) => {
                  if (result == "ok") {
                    if (this.api) {
                      this.api.asc_checkActiveCellPassword(
                        value.drmOptions.asc_getPassword(),
                        (res) => {
                          if (res) {
                            callback &&
          ...
          fillAutoShapes: function (groupNames, shapes) {
            ...
            _.each(groupNames, (groupName, index) => {
              ...
              _.each(shapes[index], (shape, idx) => {
                var name = this["txtShape_" + shape.Type]
                arr.push({
            ...
            setTimeout(() => {
              this.getApplication().getController("Toolbar").onApiAutoShapes()
            }, 50)
          ...
          updateThemeColors: function () {
            !this.appOptions.isEditOle &&
              !this.appOptions.isEditDiagram &&
              setTimeout(() => {
                this.getApplication().getController("RightMenu").UpdateThemeColors()
              }, 50)

            !this.appOptions.isEditDiagram &&
              setTimeout(() => {
                this.getApplication().getController("Toolbar").updateThemeColors()
              }, 50)

            setTimeout(() => {
              this.getApplication().getController("Statusbar").updateThemeColors()
            }, 50)
          },
          ...
          onSendThemeColors: function (colors, standart_colors) {
            ...
            if (window.styles_loaded) {
              if (!this.appOptions.isEditMailMerge) this.updateThemeColors()
          ...
          setLanguages: function () {
            ...
            sLangs.forEach((lang) => {
              ...
              rec && (rec = this.languages.indexOf(rec.toString()) < 0 ? null : lang)
              if (!rec) {
                rec = Common.util.LanguageInfo.getDefaultLanguageCode(lang)
                rec = rec && this.languages.indexOf(rec.toString()) > -1 ? rec : null
                rec &&
                  (rec = Common.util.LanguageInfo.getLocalLanguageName(Number.parseInt(rec))[0])
              }
              if (!rec) {
                rec = _.find(
                  this.languages,
                  (code) =>
                    Common.util.LanguageInfo.getLocalLanguageName(Number.parseInt(code))[0].indexOf(
                      lang.toLowerCase(),
                    ) === 0,
                )
                rec &&
            ...
            const recentKey = "app-settings-recent-langs",
            ...
            if (data) {
              switch (data.command) {
                ...
                case "getChartData":
                  ...
                  break
                case "setMergeData":
                  this.setMergeData(data.data)
                  break
                case "getMergeData":
                  this.getMergeData()
                  break
                case "setOleData":
                  this.setOleData(data.data)
                  break
                case "getOleData":
                  this.getOleData()
                  break
                case "setAppDisabled":
                  if (this.isAppDisabled === undefined && !data.data) {
                    // first editor opening
                    Common.NotificationCenter.trigger("layout:changed", "main")
                ...
                case "queryClose":
                  ...
                  } else Common.Gateway.internalMessage("canClose", { answer: false })
                  break
                case "window:drag":
          ...
          setChartData: function (chart) {
            if (typeof chart === "object" && this.api) {
              ...
              if (this.lastFrameError) {
                this.onError(
                  this.lastFrameError.id,
                  this.lastFrameError.level,
                  this.lastFrameError.errData,
                  this.lastFrameError.callback,
                )
          ...
          getChartData: function () {
            if (this.api) {
              this.api.asc_getBinaryFromDiagramFrame().then((chartData) => {
          ...
          getOleData: function () {
            if (this.api) {
              this.api.asc_getBinaryInfoOleObject().then((oleData) => {
          ...
          onSendFromFrameToGeneralEditor: (data) => {
          ...
          onShowProtectedChartPopup: function (value) {
            if (value) {
              if (!this._state.chartProtectTip) {
                var me = this,
                  text = (value.asc_getSource() || "").replace(/%20/g, " "),
                ...
                showLink &&
                  this._state.chartProtectTip.on("dontshowclick", function () {
          ...
          _compareActionStrong: (obj1, obj2) => obj1.id === obj2.id && obj1.type === obj2.type,

          _compareActionWeak: (obj1, obj2) => obj1.type === obj2.type,
          ...
          applySettings: function () {
            if (
              ...
              this._state.fastCoauth = value === null || Number.parseInt(value) == 1
            ...
            if (!this.iframePrint) {
              ...
              this.iframePrint.onload = () => {
                try {
                  ...
                  opts.asc_setAdvancedOptions(
                    this.getApplication().getController("Print").getPrintParams(),
                  )
          ...
          loadAutoCorrectSettings: function () {
            ...
            this.api.asc_refreshOnStartAutoCorrectMathSymbols(arrRem, arrAdd, value)
            ...
            this.api.asc_refreshOnStartAutoCorrectMathFunctions(arrRem, arrAdd)
            ...
            this.api.asc_setIncludeNewRowColTable(value)
            ...
            this.api.asc_SetAutoCorrectHyperlinks(value)
          },
          ...
          onRefreshHistory: function (opts) {
            ...
            if (opts.data.error || !opts.data.history) {
              ...
              Common.UI.alert({
                title: this.notcriticalErrorTitle,
                msg: opts.data.error
                  ? Common.Utils.String.htmlEncode(opts.data.error)
              ...
              this.headerView.getButton("share") &&
                this.headerView.getButton("share").setVisible(false)
              ...
              if (historyStore) {
                ...
                for (var ver = versions.length - 1, index = 0; ver >= 0; ver--, index++) {
                  ...
                  if (version) {
                    ...
                    if (!user) {
                      var color = Common.UI.ExternalUsers.getColor(
          ...
          onCollaborativeChanges: function () {
            ...
            if (this.appOptions.isEdit)
              this.getApplication()
                .getController("Statusbar")
                .setStatusCaption(this.textNeedSynchronize, true)
          },
          ...
          onConvertEquationToMath: function (equation) {
            ...
            win.$window.find("#id-equation-convert-help").on("click", (e) => {
              win && win.close()
              this.getApplication()
                .getController("LeftMenu")
                .getView("LeftMenu")
                .showMenu("file:help", "UsageInstructions/InsertEquation.htm#convertequation")
            })
          },
          ...
          onRefreshFile: function (data) {
            if (data) {
              ...
              var _options = $.extend(
                {},
                this.appOptions.spreadsheet.options,
              ...
              docInfo.put_DirectUrl(
                data.document && data.document.directUrl
                  ? data.document.directUrl
                  : this.appOptions.spreadsheet.directUrl,
              )
              docInfo.put_VKey(
                data.document && data.document.vkey
                  ? data.document.vkey
                  : this.appOptions.spreadsheet.vkey,
              )
              docInfo.put_EncryptedInfo(
                data.editorConfig && data.editorConfig.encryptionKeys
