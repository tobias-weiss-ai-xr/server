...
define(["core", "spreadsheeteditor/main/app/view/RightMenu"], () => {
  SSE.Controllers.RightMenu = Backbone.Controller.extend({
    ...
    onRightMenuAfterRender: function (rightMenu) {
      ...
      this._settings[Common.Utils.documentSettingsType.Pivot] = {
        panelId: "id-pivot-settings",
        panel: rightMenu.pivotSettings,
        btn: rightMenu.btnPivot,
        hidden: 1,
        locked: false,
      }
      ...
      this._settings[Common.Utils.documentSettingsType.Cell] = {
        panelId: "id-cell-settings",
        panel: rightMenu.cellSettings,
        btn: rightMenu.btnCell,
        hidden: 1,
        locked: false,
      }
      this._settings[Common.Utils.documentSettingsType.Slicer] = {
        panelId: "id-slicer-settings",
        panel: rightMenu.slicerSettings,
        btn: rightMenu.btnSlicer,
        hidden: 1,
        locked: false,
      }
    },
      ...
      if (!minimized && this.editMode) {
        if (event) {
          ...
          if (type === Common.Utils.documentSettingsType.Pivot)
            ...
            type === Common.Utils.documentSettingsType.Chart &&
            !this._settings[Common.Utils.documentSettingsType.Cell].hidden
          ) {
            //sparkline
        ...
        if (props && panel) {
          panel.ChangeSettings.call(
            panel,
            type === Common.Utils.documentSettingsType.Signature ? undefined : props,
            this._state.wsLock,
            this._state.wsProps,
          )
    ...
    onSelectionChanged: function (info) {
      ...
      let SelectedObjects = []
      const selectType = info.asc_getSelectionType()
      const sparkLineInfo = info.asc_getSparklineInfo()
      const cellInfo = info
      const pivotInfo = info.asc_getPivotTableInfo()

      if (
        selectType === Asc.c_oAscSelectionType.RangeImage ||
        selectType === Asc.c_oAscSelectionType.RangeShape ||
        selectType === Asc.c_oAscSelectionType.RangeSlicer ||
        selectType === Asc.c_oAscSelectionType.RangeChart ||
        selectType === Asc.c_oAscSelectionType.RangeChartText ||
    ...
    onFocusObject: function (SelectedObjects, cellInfo, sparkLineInfo, pivotInfo, forceSignature) {
      ...
      const isCellLocked = cellInfo?.asc_getLocked() || this._state.wsProps.FormatCells
      const isSparkLocked =
        (cellInfo && cellInfo.asc_getLockedSparkline() === true) || this._state.wsLock
      const isPivotLocked =
        (cellInfo && cellInfo.asc_getLockedPivotTable() === true) || this._state.wsProps.PivotTables
      const isUserProtected = cellInfo && cellInfo.asc_getUserProtected() === true
      ...
      for (i = 0; i < SelectedObjects.length; ++i) {
        ...
        if (settingsType === Common.Utils.documentSettingsType.Image) {
          ...
          if (value.asc_getChartProperties() !== null) {
            ...
            if (value.asc_getShapeProperties().asc_getTextArtProperties()) {
              ...
              this._settings[Common.Utils.documentSettingsType.TextArt].locked =
                value.asc_getLocked() ||
                (this._state.wsProps.Objects && value.asc_getProtectionLockText())
            }
          ...
          this._settings[settingsType].locked =
            value.asc_getLocked() ||
            (this._state.wsProps.Objects && value.asc_getProtectionLocked())
        } else {
        ...
        if (!this._settings[Common.Utils.documentSettingsType.Paragraph].locked)
          // lock Paragraph, if at least one shape has locked text
          this._settings[Common.Utils.documentSettingsType.Paragraph].locked =
            this._state.wsProps.Objects && locktext
      }
      ...
      if (SelectedObjects.length <= 0 && cellInfo) {
        // cell is selected
      ...
      if (!this.rightmenu.minimizedMode || this._openRightMenu) {
        ...
        if (priorityactive < 0 && this._lastVisibleSettings !== undefined) {
          const pnl = this._settings[this._lastVisibleSettings]
        ...
        if (
          ...
          const pivotactive = Common.Utils.InternalSettings.get("sse-rightpanel-active-pivot")
          const sparkactive = Common.Utils.InternalSettings.get("sse-rightpanel-active-spark")

          if (!this._settings[Common.Utils.documentSettingsType.Chart].hidden) {
            priorityactive =
              sparkactive === 0
                ? Common.Utils.documentSettingsType.Cell
                : Common.Utils.documentSettingsType.Chart
          }
          if (!this._settings[Common.Utils.documentSettingsType.Pivot].hidden)
            priorityactive =
              pivotactive === 0
                ? Common.Utils.documentSettingsType.Cell
                : Common.Utils.documentSettingsType.Pivot
        }
        ...
        if (active === undefined && this._openRightMenu && lastactive >= 0) active = lastactive
    ...
    createDelayedElements: function () {
      if (this.api) {
        this._openRightMenu = !Common.localStorage.getBool(
    ...
    onDoubleClickOnObject: function (obj) {
      ...
      const eltype = obj.asc_getObjectType()
      let settingsType = this.getDocumentSettingsType(eltype)
      if (
        settingsType === undefined ||
        settingsType >= this._settings.length ||
      ...
      const value = obj.asc_getObjectValue()
      if (settingsType === Common.Utils.documentSettingsType.Image) {
        if (value.asc_getChartProperties() !== null) {
          settingsType = Common.Utils.documentSettingsType.Chart
        } else if (value.asc_getShapeProperties() !== null) {
    ...
    SetDisabled: function (disabled, allowSignature) {
      ...
      if (this.rightmenu?.paragraphSettings) {
        ...
        if (this.rightmenu.signatureSettings) {
          ...
          allowSignature &&
            disabled &&
            this.onFocusObject([], undefined, undefined, undefined, true) // force press signature button
        }
    ...
    onRightMenuHide: function (view, status) {
      if (this.rightmenu) {
        if (!status) {
          // remember last active pane
          const active = this.rightmenu.GetActivePane()
          let type
          if (active) {
            for (let i = 0; i < this._settings.length; i++) {
    ...
    tryToShowRightMenu: function () {
      if (
        ...
        !this.rightmenu.mode.isEditMailMerge &&
        !this.rightmenu.mode.isEditDiagram &&
        !this.rightmenu.mode.isEditOle
      )
