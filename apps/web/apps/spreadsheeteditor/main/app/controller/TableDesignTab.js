...
define(["core", "spreadsheeteditor/main/app/view/TableDesignTab"], () => {
  SSE.Controllers.TableDesignTab = Backbone.Controller.extend(
    _.extend(
      {
        models: [],
        collections: [],
        views: ["TableDesignTab"],
        sdkViewName: "#id_main",
                ...
                if (
                  ...
                  Common.UI.warning({
                    ...
                    callback: (btn) => {
                      if (btn === "ok")
                        setTimeout(() => {
                          this.api.asc_changeTableRange(this._state.TableName, settings.range)
                        }, 1)
                      Common.NotificationCenter.trigger("edit:complete", this)
                    },
                  })
                else this.api.asc_changeTableRange(this._state.TableName, settings.range)
              }

              Common.NotificationCenter.trigger("edit:complete", this)
            }
            const win = new SSE.Views.TableOptionsDialog({
              handler: handlerDlg,
            })
            win.show()
            win.setSettings({
              ...
              title: this.textResize,
            })
          }
        },

        onTableNameChanged: function (input, newValue, oldValue) {
          const oldName = this._state.TableName
          ...
          const me = this
          let isvalid = this.api.asc_checkDefinedName(newValue, null)
        ...
        ChangeSettings: function (props) {
          if (props) {
            ...
            let value = props.asc_getTableName()
            ...
            Common.Utils.lockControls(
              Common.enumLock.isFilterAvailable,
              !this._state.CheckHeader || value === null,
              { array: [this.view.chFilterButton] },
            )

            if (needTablePictures) this.onApiInitTableStyles(this.api.asc_getTablePictures(props))
            ...
            if (this._state.TemplateName !== value || this._isTemplatesChanged) {
              ...
              const rec = view.tableStyles.menuPicker.store.findWhere({
                name: value,
              })
              ...
              if (this._isTemplatesChanged) {
                ...
                else view.tableStyles.fillComboView(view.tableStyles.menuPicker.store.at(0), true)
              }
        ...
        onApiInitTableStyles: function (Templates) {
          const styles = this.view.tableStyles
            ...
            let groups = [
              {
                ...
                caption: this.view.txtGroupTable_Custom,
                templates: [],
              },
              {
                id: "menu-table-group-light",
                caption: this.view.txtGroupTable_Light,
                templates: [],
              },
              {
                id: "menu-table-group-medium",
                caption: this.view.txtGroupTable_Medium,
                templates: [],
              },
              { id: "menu-table-group-dark", caption: this.view.txtGroupTable_Dark, templates: [] },
        ...
        openAdvancedSettings: function (e) {
          ...
          if (this.api && !this._locked) {
            new SSE.Views.TableSettingsAdvanced({
              tableProps: this._originalProps,
              api: this.api,
              handler: (result, value) => {
                if (result === "ok" && this.api && value) {
                  this.api.asc_changeFormatTableInfo(
                    this._state.TableName,
                    Asc.c_oAscChangeTableStyleInfo.advancedSettings,
                    value,
                  )
                }
        ...
        onInsertSlicer: function () {
          const props = this.api.asc_beforeInsertSlicer()
          if (props) {
        ...
        onSelectionChanged: function (info) {
          ...
          const tableInfo = info.asc_getFormatTableInfo()
          if (tableInfo) this.ChangeSettings(tableInfo)
        },
