    ...
    _.extend(
      {
        ...
        initialize: function () {
          this.addListeners({
            Toolbar: {
              "change:compact": this.onClickChangeCompact.bind(this),
              "add:chart": this.onSelectChart,
              "add:spark": this.onSelectSpark,
              "insert:textart": this.onInsertTextart,
              "change:scalespn": this.onClickChangeScaleInMenu.bind(this),
              "click:customscale": this.onScaleClick.bind(this),
            ...
            "Common.Views.Header": {
              ...
              downloadas: function (opts) {
                ...
                else _main.api.asc_DownloadAs(new Asc.asc_CDownloadOptions(_format))
              },
              "go:editor": () => {
          ...
          Common.NotificationCenter.on(
            "tab:set-active",
            _.bind(function (action, needUnfold) {
          ...
          var checkInsertAutoshape = (e, action) => {
            var cmp = $(e.target),
              cmp_sdk = cmp.closest("#editor_sdk"),
              btn_id = cmp.closest("button").attr("id")
            if (btn_id === undefined) btn_id = cmp.closest(".btn-group").attr("id")

            if (this.api && this.api.asc_isAddAutoshape()) {
              if (cmp_sdk.length <= 0 || action == "cancel") {
                if (
                  (this.toolbar.btnInsertText.pressed && btn_id != this.toolbar.btnInsertText.id) ||
                  (this.toolbar.btnInsertShape.pressed && btn_id != this.toolbar.btnInsertShape.id)
                ) {
                  this._isAddingShape = false
                  ...
                  this.toolbar.btnInsertShape.pressed &&
                  btn_id == this.toolbar.btnInsertShape.id
                ) {
                  _.defer(() => {
        ...
        setMode: function (mode) {
          ...
          var url = "https://www.Word Office.com/blog/2025/10/docs-9-1-released"

          Common.UI.FeaturesManager.isFeatureEnabled("featuresTips", true) &&
            Common.UI.TooltipManager.addTips({
              ...
              //                   closable: false, isNewFeature: true, link: {text: _main.textLearnMore, url: url}},
              tableTab: {
                name: "sse-help-tip-table-tab",
                placement: "bottom",
                offset: { x: Common.UI.isRTL() ? -10 : 10, y: 0 },
                text: this.helpTableTab,
                header: this.helpTableTabHeader,
                target: "li.ribtab #tabledesign",
                automove: true,
                maxwidth: 300,
                closable: false,
                isNewFeature: true,
                link: { text: _main.textLearnMore, url: url },
              },
              chartElements: {
                name: "help-tip-chart-elements",
                placement: "bottom",
                text: this.helpChartElements,
                header: this.helpChartElementsHeader,
                target: "#id-document-holder-btn-chart-element",
                maxwidth: 300,
                automove: true,
                noHighlight: true,
                noArrow: true,
              ...
              solver: {
                name: "sse-help-tip-solver",
                placement: "bottom-left",
                text: this.helpSolver,
                header: this.helpSolverHeader,
                target: "#slot-btn-solver",
                maxwidth: 300,
                automove: true,
                closable: false,
                isNewFeature: true,
              ...
              cellFormat: {
                name: "sse-help-tip-cellFormat",
                placement: "bottom-left",
                text: this.helpCellFormat,
                header: this.helpCellFormatHeader,
                target: "#slot-btn-cell-format",
                maxwidth: 300,
                automove: true,
                closable: false,
                isNewFeature: true,
        ...
        attachUIEvents: function (toolbar) {
           ...
           * UI Events
           */
          if (this.appConfig.isEditDiagram) {
            ...
            toolbar.cmbNumberFormat.on(
              "show:before",
              _.bind(this.onNumberFormatOpenBefore, this, true),
            )
            if (toolbar.cmbNumberFormat.cmpEl)
              toolbar.cmbNumberFormat.cmpEl.on(
                "click",
                "#id-toolbar-mnu-item-more-formats a",
                _.bind(this.onNumberFormatSelect, this),
              )
          ...
          } else if (this.appConfig.isEditMailMerge) {
            toolbar.btnUndo.on("click", _.bind(this.onUndo, this))
            toolbar.btnRedo.on("click", _.bind(this.onRedo, this))
            toolbar.btnCopy.on("click", _.bind(this.onCopyPaste, this, "copy"))
            toolbar.btnPaste.on("click", _.bind(this.onCopyPaste, this, "paste"))
            toolbar.btnSearch.on("toggle", _.bind(this.onSearch, this))
            toolbar.btnSortDown.on(
              "click",
              _.bind(this.onSortType, this, Asc.c_oAscSortOptions.Ascending),
            )
            toolbar.btnSortUp.on(
              "click",
              _.bind(this.onSortType, this, Asc.c_oAscSortOptions.Descending),
            )
          ...
          } else if (this.appConfig.isEditOle) {
            toolbar.btnUndo.on("click", _.bind(this.onUndo, this))
            toolbar.btnRedo.on("click", _.bind(this.onRedo, this))
            toolbar.btnCopy.on("click", _.bind(this.onCopyPaste, this, "copy"))
            toolbar.btnPaste.on("click", _.bind(this.onCopyPaste, this, "paste"))
            ...
            if (toolbar.btnBorders.rendered) {
              ...
              $("#id-toolbar-menu-auto-bordercolor").on(
                "click",
                _.bind(this.onAutoBorderColor, this),
              )
            }
            toolbar.btnMerge.on(
              "click",
              _.bind(
                this.onMergeCellsMenu,
                this,
                toolbar.btnMerge.menu,
                toolbar.btnMerge.menu.items[0],
              ),
            )
            ...
            toolbar.btnHorizontalAlign.menu.on(
              "item:click",
              _.bind(this.onHorizontalAlignMenu, this),
            )
            ...
            toolbar.btnBackColor.on("eyedropper:start", _.bind(this.onEyedropperStart, this))
            toolbar.btnBackColor.on("eyedropper:end", _.bind(this.onEyedropperEnd, this))
            toolbar.mnuFormatCellFill &&
              toolbar.mnuFormatCellFill.on("click", _.bind(this.onFormatCellFill, this))
            this.mode.isEdit &&
              Common.NotificationCenter.on("eyedropper:start", _.bind(this.eyedropperStart, this))
            toolbar.btnBorders.on("click", _.bind(this.onBorders, this))
            if (toolbar.btnBorders.rendered) {
              toolbar.btnBorders.menu.on("item:click", _.bind(this.onBordersMenu, this))
              toolbar.mnuBorderWidth.on("item:toggle", _.bind(this.onBordersWidth, this))
            ...
            toolbar.btnAlignLeft.on(
              "click",
              _.bind(this.onHorizontalAlign, this, AscCommon.align_Left),
            )
            toolbar.btnAlignCenter.on(
              "click",
              _.bind(this.onHorizontalAlign, this, AscCommon.align_Center),
            )
            toolbar.btnAlignRight.on(
              "click",
              _.bind(this.onHorizontalAlign, this, AscCommon.align_Right),
            )
            toolbar.btnAlignJust.on(
              "click",
              _.bind(this.onHorizontalAlign, this, AscCommon.align_Justify),
            )
            toolbar.btnMerge.on(
              "click",
              _.bind(
            ...
            toolbar.btnMerge.menu.on("item:click", _.bind(this.onMergeCellsMenu, this))
            toolbar.btnAlignTop.on(
              "click",
              _.bind(this.onVerticalAlign, this, Asc.c_oAscVAlign.Top),
            )
            toolbar.btnAlignMiddle.on(
              "click",
              _.bind(this.onVerticalAlign, this, Asc.c_oAscVAlign.Center),
            )
            toolbar.btnAlignBottom.on(
              "click",
              _.bind(this.onVerticalAlign, this, Asc.c_oAscVAlign.Bottom),
            )
            toolbar.btnWrap.on("click", _.bind(this.onWrap, this))
            ...
            toolbar.btnInsertSymbol.menu.items[2].on(
              "click",
              _.bind(this.onInsertSymbolClick, this),
            )
            toolbar.mnuInsertSymbolsPicker.on(
              "item:click",
              _.bind(this.onInsertSymbolItemClick, this),
            )
            ...
            toolbar.btnsEditHeader.forEach((button) => {
              button.on("click", _.bind(this.onEditHeaderClick, this, undefined))
            })
            ...
            if (toolbar.btnCondFormat.rendered) {
              toolbar.btnCondFormat.menu.on(
                "show:before",
                _.bind(this.onShowBeforeCondFormat, this, this.toolbar, "toolbar"),
              )
            }
            if (toolbar.btnFormatCell.rendered) {
              toolbar.btnFormatCell.menu.on(
                "show:before",
                _.bind(this.onShowBeforeCellFormat, this, this.toolbar),
              )
              ...
              $("#id-toolbar-menu-new-color", this.toolbar.$el).on(
                "click",
                _.bind(this.onNewTabColor, this),
              )
            }
            ...
            toolbar.btnFillNumbers.menu.on(
              "show:before",
              _.bind(this.onShowBeforeFillNumMenu, this),
            )
        ...
        setApi: function (api) {
          ...
          if (config.isEdit) {
            ...
            this.api.asc_registerCallback(
              ...
              _.bind(this.onApiInitEditorStyles, this),
            )
            this.api.asc_registerCallback(
            ...
            this.api.asc_registerCallback(
              "asc_onUnLockCFManager",
              _.bind(this.onUnLockCFManager, this),
            )
            ...
            this.api.asc_registerCallback(
              ...
              _.bind(this.onApiSelectionChangedRestricted, this),
            )
        ...
        onPrintMenu: function (btn, e) {
          ...
          if (newType != oldType) {
            this.toolbar.btnPrint.changeIcon({
              ...
              curr: this.toolbar.btnPrint.menu.items.filter((item) => item.value == oldType)[0]
                .options.iconClsForMainBtn,
            })
        ...
        onSave: function (e) {
          if (this.api) {
            ...
            var isSyncButton =
              this.toolbar.btnCollabChanges &&
              this.toolbar.btnCollabChanges.cmpEl.hasClass("notify")
            if (
              !isModified &&
              !isSyncButton &&
              !this.toolbar.mode.forcesave &&
              !this.toolbar.mode.canSaveDocumentToBinary
            )
              return
          ...
          if (this.api) {
            ...
            if (!res) {
              ...
              if (
                ...
                (type === "paste" || this.toolbar.mode.canCopy)
              ) {
            ...
            } else Common.component.Analytics.trackEvent("ToolBar", "Copy Warning")
          }
          Common.NotificationCenter.trigger("edit:complete", this.toolbar)
        },

        onSelectAll: function (e) {
          if (this.api) this.api.asc_EditSelectAll()
        ...
        onIncreaseFontSize: function (e) {
          if (this.api) this.api.asc_increaseFontSize()
        ...
        onDecreaseFontSize: function (e) {
          if (this.api) this.api.asc_decreaseFontSize()
        ...
        onChangeCase: function (menu, item, e) {
          if (this.api) this.api.asc_ChangeTextCase(item.value)
          Common.NotificationCenter.trigger("edit:complete", this.toolbar, { restorefocus: true })
        },

        onBold: function (btn, e) {
          this._state.bold = undefined
          if (this.api) this.api.asc_setCellBold(btn.pressed)
        ...
        onItalic: function (btn, e) {
          ...
          if (this.api) this.api.asc_setCellItalic(btn.pressed)
        ...
        onUnderline: function (btn, e) {
          ...
          if (this.api) this.api.asc_setCellUnderline(btn.pressed)
        ...
        onStrikeout: function (btn, e) {
          ...
          if (this.api) this.api.asc_setCellStrikeout(btn.pressed)
        ...
        onSubscriptMenu: function (menu, item) {
          ...
          Common.component.Analytics.trackEvent(
            "ToolBar",
            item.value == "sub" ? "Subscript" : "Superscript",
          )
        },
        ...
        onTextColor: function () {
          this.toolbar.mnuTextColorPicker.trigger(
            "select",
            this.toolbar.mnuTextColorPicker,
            this.toolbar.mnuTextColorPicker.currentColor,
          )
        },

        onBackColor: function () {
          this.toolbar.mnuBackColorPicker.trigger(
            "select",
            this.toolbar.mnuBackColorPicker,
            this.toolbar.mnuBackColorPicker.currentColor,
          )
        },
        ...
        onTextColorSelect: function (btn, color) {
          ...
          if (this.api) {
            this.api.asc_setCellTextColor(
              color.isAuto ? color.color : Common.Utils.ThemeColor.getRgbColor(color),
            )
          }
        ...
        onBackColorSelect: function (btn, color) {
          ...
          if (this.api) {
            this.api.asc_setCellBackgroundColor(
              color == "transparent" ? null : Common.Utils.ThemeColor.getRgbColor(color),
            )
          }
        ...
        onFormatCellFill: function (picker, color) {
          this.getApplication()
            ...
            .onRightMenuOpen(Common.Utils.documentSettingsType.Cell)
        },
        ...
        onBorders: function (btn) {
          ...
          _.each(btn.menu.getItems(true), (item) => {
        ...
        onBordersMenu: function (menu, item) {
          if (this.api && !_.isUndefined(item.options.borderId)) {
            var btnBorders = this.toolbar.btnBorders,
              new_borders = [],
            ...
            if (item.options.borderId == "inner") {
              new_borders[Asc.c_oAscBorderOptions.InnerV] = new Asc.asc_CBorder(
                bordersWidth,
                bordersColor,
              )
              new_borders[Asc.c_oAscBorderOptions.InnerH] = new Asc.asc_CBorder(
                bordersWidth,
                bordersColor,
              )
              ...
              new_borders[Asc.c_oAscBorderOptions.Left] = new Asc.asc_CBorder(
                bordersWidth,
                bordersColor,
              )
              new_borders[Asc.c_oAscBorderOptions.Top] = new Asc.asc_CBorder(
                bordersWidth,
                bordersColor,
              )
              new_borders[Asc.c_oAscBorderOptions.Right] = new Asc.asc_CBorder(
                bordersWidth,
                bordersColor,
              )
              new_borders[Asc.c_oAscBorderOptions.Bottom] = new Asc.asc_CBorder(
                bordersWidth,
                bordersColor,
              )
        ...
        onBordersColor: function (picker, color) {
          $("#id-toolbar-mnu-item-border-color > a .menu-item-icon").css(
            "border-color",
          ...
          var clr_item = this.toolbar.btnBorders.menu.$el.find(
            "#id-toolbar-menu-auto-bordercolor > a",
          )
        ...
        onMergeCellsMenu: function (menu, item) {
          ...
          if (me.api) {
            ...
            if (
              merged !== Asc.c_oAscMergeOptions.Merge &&
              me.api.asc_mergeCellsDataLost(item.value)
            ) {
              Common.UI.warning({
        ...
        onTextDirShowAfter: (menu, item) => {
          Common.UI.TooltipManager.closeTip("rtlDirection")
        },

        onUpdateTextDir: function (menu, item) {
          if (this.api)
            menu.items[4].setChecked(this.api.asc_getSheetViewSettings().asc_getRightToLeft())
        },

        onWrap: function (btn, e) {
          this._state.wrap = undefined
          if (this.api) this.api.asc_setCellTextWrap(btn.pressed)
        ...
        onTextOrientationMenu: function (menu, item) {
          ...
          if (this.api) this.api.asc_setCellAngle(angle)
        ...
        onBtnInsertTableClick: function (btn, e) {
          if (this.api) this._setTableFormat(this.api.asc_getDefaultTableStyle())
          Common.NotificationCenter.trigger("edit:complete", this.toolbar)
        ...
        onBtnPasteOptionsClick: function (btn, e) {
          ...
          this.api.asc_getPasteOptions((options) => {
            this.handlePasteOptions(options)
          })
        },
        ...
        handlePasteOptions: function (options) {
          if (options === null) {
            ...
            var mnu = new Common.UI.MenuItem({
              caption: this.txtPasteNoOptions,
              disabled: true,
            })
          ...
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.paste] = [this.txtPaste, 0]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.pasteOnlyFormula] = [
            this.txtPasteFormulas,
            0,
          ]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.formulaNumberFormat] = [
            this.txtPasteFormulaNumFormat,
            0,
          ]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.formulaAllFormatting] = [
            this.txtPasteKeepSourceFormat,
            0,
          ]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.formulaWithoutBorders] = [
            this.txtPasteBorders,
            0,
          ]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.formulaColumnWidth] = [
            this.txtPasteColWidths,
            0,
          ]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.mergeConditionalFormating] = [
            this.txtPasteMerge,
            0,
          ]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.pasteOnlyValues] = [this.txtPasteValues, 1]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.valueNumberFormat] = [
            this.txtPasteValNumFormat,
            1,
          ]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.valueAllFormating] = [
            this.txtPasteValFormat,
            1,
          ]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.pasteOnlyFormating] = [
            this.txtPasteFormat,
            2,
          ]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.link] = [this.txtPasteLink, 2]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.transpose] = [this.txtPasteTranspose, 2]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.picture] = [this.txtPastePicture, 2]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.linkedPicture] = [
            this.txtPasteLinkPicture,
            2,
          ]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.sourceformatting] = [
            this.txtPasteSourceFormat,
            2,
          ]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.destinationFormatting] = [
            this.txtPasteDestFormat,
            2,
          ]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.keepTextOnly] = [this.txtKeepTextOnly, 2]
          this._arrSpecialPaste[Asc.c_oSpecialPasteProps.useTextImport] = [this.txtUseTextImport, 3]
          ...
          if (pasteItems.length > 0) {
            ...
            _.each(pasteItems, (menuItem, index) => {
              if (this._arrSpecialPaste[menuItem]) {
                var mnu = new Common.UI.MenuItem({
                  caption:
                    this._arrSpecialPaste[menuItem][0] +
                    (this.hkSpecPaste[menuItem] ? " (" + this.hkSpecPaste[menuItem] + ")" : ""),
                  value: menuItem,
                  checkable: true,
                ...
                if (menuItem == Asc.c_oSpecialPasteProps.paste) {
                  ...
                  groups[this._arrSpecialPaste[menuItem][1]].push(mnu)
                }

                this._arrSpecialPaste[menuItem][2] = mnu
              }
            })
            var groupTitles = [this.txtFormula, this.txtValue, this.txtOther]
            ...
            for (var i = 0; i < 3; i++) {
              if (groups[i].length > 0) {
                ...
                _.each(groups[i], (menuItem, index) => {
              ...
              var mnu = new Common.UI.MenuItem({
                ...
                new SSE.Views.SpecialPasteDialog({
                  ...
                  handler: (result, settings) => {
                    if (result == "ok") {
                      if (this && this.api) {
                        this.api.asc_SpecialPaste(settings, true)
                      }
                    }
                  },
                }).show()
              ...
              handler: (result, settings) => {
                if (result == "ok") {
                  if (this && this.api) {
                    ...
                    this.api.asc_SpecialPaste(props, true)
                  }
                }
              },
            }).show()
                ...
                if (result == "ok") {
                  if (this.api) {
                    ...
                    if (!_.isEmpty(checkUrl)) {
                      this.toolbar.fireEvent("insertimage", this.toolbar)
                      this.api.asc_addImageDrawingObject([checkUrl])
        ...
        insertImageFromStorage: function (data) {
          if (data && data._urls && (!data.c || data.c == "add")) {
            this.toolbar.fireEvent("insertimage", this.toolbar)
            data._urls.length > 0 &&
              this.api.asc_addImageDrawingObject(data._urls, undefined, data.token) // for loading from storage
            Common.component.Analytics.trackEvent("ToolBar", "Image")
          }
        },
        ...
        onHyperlinkCallback: function (btn) {
          ...
          if (this.api) {
            ...
            while (++i < wc) {
              items.push({
                name: this.api.asc_getWorksheetName(i),
                hidden: this.api.asc_isWorksheetHidden(i),
              })
            }

            var handlerDlg = (dlg, result) => {
              if (result == "ok") {
            ...
            win.setSettings({
              sheets: items,
              ranges: this.api.asc_getDefinedNames(Asc.c_oAscGetDefinedNamesList.All, true),
              currentSheet: this.api.asc_getActiveWorksheetIndex(),
              props: props,
              text: cell.asc_getText(),
              isLock: cell.asc_getLockText(),
              allowInternal:
                seltype !== Asc.c_oAscSelectionType.RangeChart &&
        ...
        onEditChartData: function (btn) {
          ...
          var props
          if (this.api) {
            props = this.api.asc_getFrameChartSettings()
            if (props) {
        ...
        onSelectChart: function (group, type) {
          ...
          var info = this.api.asc_getCellInfo(),
          ...
          if (this.api) {
            ...
            var ischartedit =
              seltype == Asc.c_oAscSelectionType.RangeChart ||
              seltype == Asc.c_oAscSelectionType.RangeChartText
            props = this.api.asc_getChartSettings(true) // don't lock chart object
            if (props) {
              if (ischartedit) props.changeType(type)
              else {
                props.putType(type)
                var range = props.getRange(),
                  isvalid = !_.isEmpty(range)
                ...
                if (isvalid == Asc.c_oAscError.ID.No) {
                  this.api.asc_addChartDrawingObject(props)
                } else {
                  var msg = this.txtInvalidRange
                  switch (isvalid) {
        ...
        onSelectSpark: function (type) {
          ...
          if (this.api) {
            if (
              seltype == Asc.c_oAscSelectionType.RangeCells ||
              seltype == Asc.c_oAscSelectionType.RangeCol ||
              seltype == Asc.c_oAscSelectionType.RangeRow ||
              seltype == Asc.c_oAscSelectionType.RangeMax
            ) {
              ...
              if (!!sparkLineInfo) {
                ...
                new SSE.Views.CreateSparklineDialog({
                  api: this.api,
                  props: { selectedCells: this._state.selectedCells },
                  handler: (result, settings) => {
                    if (result == "ok" && settings) {
                      this.view && this.view.fireEvent("insertspark", this.view)
                      if (settings.destination)
                        this.api.asc_addSparklineGroup(type, settings.source, settings.destination)
                    }
                    Common.NotificationCenter.trigger("edit:complete", this)
                  },
                }).show()
        ...
        onMenuInsertTextClick: function (btn, e) {
          ...
          if (newType != oldType) {
            this.toolbar.btnInsertText.changeIcon({
              next: e.options.iconClsForMainBtn,
              curr: this.toolbar.btnInsertText.menu
                .getItems(true)
                .filter((item) => item.value == oldType)[0].options.iconClsForMainBtn,
            })
            this.toolbar.btnInsertText.updateHint([
              e.caption,
              this.views.Toolbar.prototype.tipInsertText,
            ])
        ...
        onInsertText: function (type, btn, e) {
          if (this.api) this._addAutoshape(this.toolbar.btnInsertText.pressed, type)
        ...
        onSortType: function (type, btn) {
          Common.NotificationCenter.trigger("protect:check", this.onSortTypeCallback, this, [
            type,
            btn,
          ])
        },
        ...
        onSortTypeCallback: function (type, btn) {
          if (this.api) {
            if (
              this.api.asc_getCellInfo().asc_getSelectionType() ==
              Asc.c_oAscSelectionType.RangeSlicer
            ) {
                    ...
                    elValue
                      ...
                      .asc_setSortOrder(
                        type == Asc.c_oAscSortOptions.Ascending
                          ? Asc.ST_tabularSlicerCacheSortOrder.Ascending
                          : Asc.ST_tabularSlicerCacheSortOrder.Descending,
                      )
              ...
              switch (res) {
                ...
                case Asc.c_oAscSelectionSortExpand.showLockMessage:
                  var config = {
                    ...
                    callback: (btn) => {
                      btn == "yes" &&
                        this.api.asc_sortColFilter(type, "", undefined, undefined, false)
                    },
                  }
                  Common.UI.alert(config)
                  break
                ...
                case Asc.c_oAscSelectionSortExpand.notExpandAndNotShowMessage:
                  this.api.asc_sortColFilter(
                    type,
                    "",
                    undefined,
                    undefined,
                    res === Asc.c_oAscSelectionSortExpand.expandAndNotShowMessage,
                  )
                  break
              }
            }
        ...
        onAutoFilter: function (btn) {
          ...
          if (this.api) {
            if (this._state.tablename || state)
              this.api.asc_changeAutoFilter(
                this._state.tablename,
                Asc.c_oAscChangeFilterOptions.filter,
                !state,
              )
            else this.api.asc_addAutoFilter()
          }
        ...
        onClearFilter: function (btn) {
          if (this.api) this.api.asc_clearFilter()
        ...
        onNumberFormat: function (btn) {
          if (this.api) this.api.asc_setCellStyle(btn.options.styleName)
        ...
        onNumberFormatMenu: function (menu, item) {
          if (this.api) {
            if (item.value == -1) {
              // show more formats
              if (
                this._state.numformatinfo &&
                this._state.numformatinfo.asc_getType() == Asc.c_oAscNumFormatType.Accounting
              )
                this.onCustomNumberFormat()
              else {
                var value = this.api.asc_getLocale()
                !value &&
                  (value = this.toolbar.mode.lang
                    ? Number.parseInt(
                        Common.util.LanguageInfo.getLocalLanguageCode(this.toolbar.mode.lang),
                      )
                    : 0x0409)
              ...
              var format = this.api.asc_getFormatCells(info)
              if (format && format.length > 0) this.api.asc_setCellFormat(format[0])
            }
          }
        ...
        onNumberFormatSelect: function (combo, record) {
          if (record) {
            if (this.api) this.api.asc_setCellFormat(record.format)
          } else {
            this.onCustomNumberFormat()
          }
          Common.NotificationCenter.trigger("edit:complete", this.toolbar)
        ...
        onCustomNumberFormat: function (format, formatInfo) {
          ...
          new Common.Views.FormatSettingsDialog({
            ...
            handler: (result, settings) => {
              if (settings) {
                this.api.asc_setCellFormat(settings.format)
              }
              Common.NotificationCenter.trigger("edit:complete", this.toolbar)
            },
            props: {
              format: format ? format : this._state.numformat,
              formatInfo: formatInfo ? formatInfo : this._state.numformatinfo,
              langId: value,
            },
          }).show()
          Common.NotificationCenter.trigger("edit:complete", this.toolbar)
        ...
        onNumberFormatOpenBefore: function (combo) {
          if (this.api) {
            ...
            if (this._state.langId !== value) {
              ...
              this.toolbar.numFormatData.forEach((item, index) => {
                this.toolbar.numFormatData[index].format = arr[index]
              })
            }

            this.toolbar.numFormatData.forEach((item, index) => {
              item.exampleval = this.api.asc_getLocaleExample(item.format)
            })
            this.toolbar.cmbNumberFormat.setData(this.toolbar.numFormatData)
            this.toolbar.cmbNumberFormat.setValue(this._state.numformattype, this.toolbar.txtCustom)
          }
        },

        onDecrement: function (btn) {
          if (this.api) this.api.asc_decreaseCellDigitNumbers()
        ...
        onIncrement: function (btn) {
          if (this.api) this.api.asc_increaseCellDigitNumbers()
        ...
        onInsertFormulaMenu: function (menu, item, e) {
          if (this.api) {
            if (item.value === "more") {
              ...
              this.toolbar.fireEvent("function:apply", [
                { name: this.api.asc_getFormulaLocaleName(item.value), origin: item.value },
                true,
              ])
          ...
          if (this.api) {
            if (item.value === "paste") {
              new SSE.Views.NamedRangePasteDlg({
                handler: (result, settings) => {
                  if (result == "ok" && settings) {
                    this.api.asc_insertInCell(
                      settings.asc_getName(true),
                      settings.asc_getType() === Asc.c_oAscDefNameType.table
                        ? Asc.c_oAscPopUpSelectorType.Table
                        : Asc.c_oAscPopUpSelectorType.Range,
                      false,
                    )
                ...
                ranges: this.api.asc_getDefinedNames(
                  Asc.c_oAscGetDefinedNamesList.WorksheetWorkbook,
                ), // names only for current sheet and workbook
              }).show()
              Common.component.Analytics.trackEvent("ToolBar", "Paste Named Range")
            } else {
              var wc = this.api.asc_getWorksheetsCount(),
              ...
              if (item.value === "new") {
                ...
                new SSE.Views.NameManagerDlg({
                  ...
                  locked: this._state.namedrange_locked,
                  sheets: items,
        ...
        onNamedRangeMenuOpen: function (menu) {
          if (this.api && menu) {
            var names = this.api.asc_getDefinedNames(
              Asc.c_oAscGetDefinedNamesList.WorksheetWorkbook,
            )
        ...
        onClearStyleMenu: function (menu, item, e) {
          if (
            (item.value == Asc.c_oAscCleanOptions.Format &&
              (!this._state.wsProps["FormatCells"] || !this.api.asc_checkLockedCells())) ||
            ...
            this._state.wsProps["Objects"]
              ? Common.NotificationCenter.trigger(
                  "showerror",
                  Asc.c_oAscError.ID.ChangeOnProtectedSheet,
        ...
        onClearStyleMenuCallback: function (menu, item, e) {
          if (this.api) {
            if (item.value == Asc.c_oAscCleanOptions.Comments) {
              ...
              this.api.asc_emptyCells(
                item.value,
                item.value == Asc.c_oAscCleanOptions.All && !this.mode.canDeleteComments,
              )
          }
        ...
        onCopyStyleToggle: function (btn, state, e) {
          if (this.api) this.api.asc_formatPainter(state ? 1 : 0)
          Common.NotificationCenter.trigger("edit:complete", this.toolbar)
          this.modeAlwaysSetStyle = state
        },

        onCellInsertMenu: function (menu, item, e) {
          if (this.api) this.api.asc_insertCells(item.value)
        ...
        onCellDeleteMenu: function (menu, item, e) {
          if (this.api) this.api.asc_deleteCells(item.value)
        ...
        onShowBeforeCellFormat: function (cmp, item, e) {
          if (!(e && e.target === e.currentTarget) || !this.toolbar) return
          ...
          toolbar.lockToolbar(_set["FormatRows"], !!this._state.wsProps["FormatRows"], {
            array: [toolbar.mnuRowHeight, toolbar.mniHideRows, toolbar.mniShowRows],
          })
          toolbar.lockToolbar(_set["FormatColumns"], !!this._state.wsProps["FormatColumns"], {
            array: [toolbar.mnuColumnWidth, toolbar.mniHideCols, toolbar.mniShowCols],
          })
          toolbar.lockToolbar(_set.wsLock, this._state.wsLock, { array: [toolbar.mniLockCell] })
          toolbar.lockToolbar(_set.userProtected, this._state.isUserProtected, {
            array: [toolbar.mniLockCell],
          })
          ...
          tabIndexes.forEach((item) => {
            if (this.api.asc_isWorksheetLockedOrDeleted(item)) issheetlocked = true
          })
          ...
          toolbar.btnFormatCell.menu.items[9].setCaption(
            this.api.asc_isProtectedSheet() ? toolbar.textUnProtectSheet : toolbar.textProtectSheet,
          ) // protect sheet
          toolbar.btnFormatCell.menu.items[11].setChecked(
            this.api.asc_getCellInfo().asc_getXfs().asc_getLocked(),
          ) // lock cell

          const hiddenItems = SSE.getController("Statusbar").statusbar.getHiddenWorksheets()
          toolbar.mnuShowSheets.menu.removeAll()
          toolbar.mnuShowSheets.setVisible(hiddenItems.length)
          if (hiddenItems.length) {
            hiddenItems.forEach((item) => {
              toolbar.mnuShowSheets.menu.addItem(
                new Common.UI.MenuItem({
          ...
          toolbar.mnuHide.setDisabled(
            toolbar.mniHideRows.isDisabled() &&
              toolbar.mniHideCols.isDisabled() &&
              toolbar.mniHideSheets.isDisabled(),
          )
          toolbar.mnuShow.setDisabled(
            toolbar.mniShowRows.isDisabled() &&
              toolbar.mniShowCols.isDisabled() &&
              (hiddenItems.length < 1 || toolbar.mnuShowSheets.isDisabled()),
          )
        ...
        onCellFormatMenu: function (menu, item, e) {
          ...
          const tabIndex = SSE.getController("Statusbar").getSelectTabs()
        ...
        onColorSchemaShow: function (menu) {
          if (this.api) {
            ...
            var item = _.find(menu.getItems(true), (item) => item.value == value)
                ...
                Common.UI.warning({
                  ...
                  callback: _.bind(function (btn) {
                    if (btn == "yes") {
                      ...
                      this.toolbar.cmbFontName.setValue(
                        this.api.asc_getCellInfo().asc_getXfs().asc_getFontName(),
                      )
                    }
                    Common.NotificationCenter.trigger("edit:complete", this.toolbar, {
                      restorefocus: true,
        ...
        onFontSizeSelect: function (combo, record) {
          ...
          if (this.api) this.api.asc_setCellFontSize(record.value)
        ...
        onFontSizeChanged: function (before, combo, record, e) {
          ...
          if (before) {
            ...
            value = value > 409 ? 409 : value < 1 ? 1 : Math.floor((value + 0.4) * 2) / 2

            combo.setRawValue(value)

            this._state.fontsize = undefined
            if (this.api) this.api.asc_setCellFontSize(value)
        ...
        onShowBeforeCondFormat: function (cmp, id) {
          if (cmp.mnuDataBars.menu.items.length > 0)
            // menu is inited
            return

          cmp.btnCondFormat.menu.on("item:click", _.bind(this.onCondFormatMenu, this))
          for (var i = 0; i < 7; i++) {
            cmp.btnCondFormat.menu.items[i].menu.on(
              "item:click",
              _.bind(this.onCondFormatMenu, this),
            )
          }
          cmp.btnCondFormat.menu.items[15].menu.on(
            "item:click",
            _.bind(this.onCondFormatMenu, this),
          )
          ...
          menuItem.menu.addItem(
            new Common.UI.MenuItem({
              template: _.template(
                '<div id="id-' +
                  id +
                  '-menu-databar" class="menu-shapes margin-left-5" style="width: 203px;"></div>',
              ),
            }),
          )
          var picker = new Common.UI.DataViewSimple({
            ...
            itemTemplate: _.template(
              '<div class="item-databar" id="<%= id %>"><svg width="25" height="25" class="icon uni-scale"><use xlink:href="#bar-<%= data.name %>"></use></svg></div>',
            ),
          })
            ...
            if (this.api) {
              if (record) {
                this.api.asc_setCF(
                  [],
                  [],
                  [Asc.c_oAscCFRuleTypeSettings.dataBar, record.get("data").index],
                )
              }
              if (e.type !== "click") cmp.btnCondFormat.menu.hide()
          ...
          menuItem.menu.addItem(
            new Common.UI.MenuItem({
              template: _.template(
                '<div id="id-' +
                  id +
                  '-menu-colorscales" class="menu-shapes margin-left-5" style="width: 136px;"></div>',
              ),
            }),
          )
          picker = new Common.UI.DataViewSimple({
            ...
            parentMenu: menuItem.menu,
            itemTemplate: _.template(
              '<div class="item-colorscale" id="<%= id %>"><svg width="25" height="25" class="icon uni-scale"><use xlink:href="#color-scale-<%= data.name %>"></use></svg></div>',
            ),
          })
          picker.on("item:click", (picker, item, record, e) => {
            if (this.api) {
              if (record) {
                this.api.asc_setCF(
                  [],
                  [],
                  [Asc.c_oAscCFRuleTypeSettings.colorScale, record.get("data").index],
                )
              }
              if (e.type !== "click") cmp.btnCondFormat.menu.hide()
              Common.NotificationCenter.trigger("edit:complete", cmp, cmp.btnCondFormat)
          ...
          menuItem.menu.addItem(
            new Common.UI.MenuItem({
              template: _.template(
                '<div id="id-' +
                  id +
                  '-menu-iconsets" class="menu-iconsets" style="width: 227px;"></div>',
              ),
            }),
          )
          arr = []
          var indexes = [
            Asc.EIconSetType.Arrows3,
            Asc.EIconSetType.Arrows3Gray,
            Asc.EIconSetType.Triangles3,
            Asc.EIconSetType.Arrows4Gray,
            Asc.EIconSetType.Arrows4,
            Asc.EIconSetType.Arrows5Gray,
            Asc.EIconSetType.Arrows5,
          ]
          for (var i = 0; i < indexes.length; i++) {
            arr.push({
              group: "menu-iconset-group-direct",
              data: {
                index: indexes[i],
                iconSet: collectionPresets.at([indexes[i]]).get("icons"),
                icons: collectionIcons,
              },
            })
          }
          indexes = [
            Asc.EIconSetType.Traffic3Lights1,
            Asc.EIconSetType.Traffic3Lights2,
            Asc.EIconSetType.Signs3,
            Asc.EIconSetType.Traffic4Lights,
            Asc.EIconSetType.RedToBlack4,
          ]
          for (var i = 0; i < indexes.length; i++) {
            arr.push({
              group: "menu-iconset-group-shape",
              data: {
                index: indexes[i],
                iconSet: collectionPresets.at([indexes[i]]).get("icons"),
                icons: collectionIcons,
          ...
          indexes = [
            Asc.EIconSetType.Symbols3,
            Asc.EIconSetType.Symbols3_2,
            Asc.EIconSetType.Flags3,
          ]
          for (var i = 0; i < indexes.length; i++) {
            arr.push({
              group: "menu-iconset-group-indicator",
              data: {
                index: indexes[i],
                iconSet: collectionPresets.at([indexes[i]]).get("icons"),
                icons: collectionIcons,
          ...
          indexes = [
            Asc.EIconSetType.Stars3,
            Asc.EIconSetType.Rating4,
            Asc.EIconSetType.Quarters5,
            Asc.EIconSetType.Rating5,
            Asc.EIconSetType.Boxes5,
          ]
          for (var i = 0; i < indexes.length; i++) {
            arr.push({
              group: "menu-iconset-group-rating",
              data: {
                index: indexes[i],
                iconSet: collectionPresets.at([indexes[i]]).get("icons"),
                icons: collectionIcons,
            ...
            if (this.api) {
              if (record) {
                this.api.asc_setCF(
                  [],
                  [],
                  [Asc.c_oAscCFRuleTypeSettings.icons, record.get("data").index],
                )
              }
              if (e.type !== "click") cmp.btnCondFormat.menu.hide()
              Common.NotificationCenter.trigger("edit:complete", cmp, cmp.btnCondFormat)
        ...
        onCondFormatMenu: function (menu, item) {
          ...
          if (item.value == "manage") {
            new SSE.Views.FormatRulesManagerDlg({
              api: this.api,
              langId: value,
              locked: !!this._state.cf_locked[this.api.asc_getActiveWorksheetIndex()],
              handler: (result, settings) => {
                if (this && this.api && result == "ok") {
                  this.api.asc_setCF(settings.rules, settings.deleted)
                }
              },
            }).show()
          } else if (item.value == "clear") {
            this.api.asc_clearCF(item.options.type)
          } else {
        ...
        createDelayedElements: function () {
          ...
          if (
            !this.appConfig.isEditDiagram &&
            !this.appConfig.isEditMailMerge &&
            !this.appConfig.isEditOle
          ) {
            this.api.asc_registerCallback(
              "asc_onSheetsChanged",
              _.bind(this.onApiSheetChanged, this),
            )
            this.api.asc_registerCallback(
            ...
            this.api.asc_registerCallback(
              ...
              _.bind(this.onApiEditorSelectionChanged, this),
            )
            this.api.asc_registerCallback(
              "asc_onUpdateDocumentProps",
              _.bind(this.onUpdateDocumentProps, this),
            )
            this.api.asc_registerCallback(
          ...
          if (this.appConfig.isEditOle) {
            this.api.asc_registerCallback(
              "asc_onEditorSelectionChanged",
              _.bind(this.onApiEditorSelectionChanged_OleEditor, this),
            )
          }
          ...
          this.api.asc_registerCallback(
            ...
            _.bind(this.onApiCanRevert, this, "undo"),
          )
          this.api.asc_registerCallback(
            "asc_onCanRedoChanged",
            _.bind(this.onApiCanRevert, this, "redo"),
          )
          this.api.asc_registerCallback("asc_onEditCell", _.bind(this.onApiEditCell, this))
          this.api.asc_registerCallback(
            "asc_onStopFormatPainter",
            _.bind(this.onApiStyleChange, this),
          )
          this.api.asc_registerCallback(
            "asc_onSelectionChanged",
          ...
          var shortcuts = {
            "command+l,ctrl+l": (e) => {
              if (
                this.editMode &&
                !this._state.multiselect &&
                this.appConfig.canModifyFilter &&
                !this._state.wsLock
              ) {
            ...
            "command+shift+l,ctrl+shift+l": (e) => {
              if (
                this.editMode &&
                this.api &&
                !this._state.multiselect &&
            ...
            "command+k,ctrl+k": (e) => {
              if (
                ...
                !this._state.multiselect &&
                !this._state.inpivot &&
                !this.getApplication().getController("LeftMenu").leftMenu.menuFile.isVisible() &&
                !this._state.wsProps["InsertHyperlinks"]
              ) {
                var cellinfo = this.api.asc_getCellInfo(),
                ...
                if (
                  ...
                  ((selectionType !== Asc.c_oAscSelectionType.RangeShapeText &&
                    ...
                    this.api.asc_canAddShapeHyperlink() !== false)
                )
                  this.onHyperlink()
              }
              e.preventDefault()
            ...
            "command+1,ctrl+1": (e) => {
              if (
                ...
                !this.toolbar.mode.isEditOle &&
                !this.api.isCellEdited &&
                !this.toolbar.cmbNumberFormat.isDisabled()
              ) {
                this.onCustomNumberFormat()
              }
            ...
            "shift+f3": (e) => {
              if (this.editMode && !this.toolbar.btnInsertFormula.isDisabled()) {
                var controller = this.getApplication().getController("FormulaDialog")
                if (controller) {
                  controller.showDialog()
                }
          ...
          shortcuts[
            "command+shift+=,ctrl+shift+=,command+shift+numplus,ctrl+shift+numplus" +
              (Common.Utils.isGecko ? ",command+shift+ff=,ctrl+shift+ff=" : "") +
              (Common.Utils.isMac ? ",command+shift+0,ctrl+shift+0" : "")
          ] = (e) => {
            if (Common.Utils.isMac && e.keyCode === Common.UI.Keys.ZERO && e.key !== "=")
              return false
            if (
              this.editMode &&
              !this.toolbar.mode.isEditMailMerge &&
              !this.toolbar.mode.isEditDiagram &&
              !this.toolbar.mode.isEditOle &&
              !this.toolbar.btnAddCell.isDisabled()
            ) {
              var cellinfo = this.api.asc_getCellInfo(),
                selectionType = cellinfo.asc_getSelectionType()
              if (
                ...
                selectionType === Asc.c_oAscSelectionType.RangeRow &&
                  !this.toolbar.btnAddCell.menu.items[2].isDisabled() &&
                  this.api.asc_insertCells(Asc.c_oAscInsertOptions.InsertRows)
                selectionType === Asc.c_oAscSelectionType.RangeCol &&
                  !this.toolbar.btnAddCell.menu.items[3].isDisabled() &&
                  this.api.asc_insertCells(Asc.c_oAscInsertOptions.InsertColumns)
                Common.NotificationCenter.trigger("edit:complete", this.toolbar)
              } else {
                var items = this.toolbar.btnAddCell.menu.getItems(true),
                  arr = [],
                  enabled = false
                for (var i = 0; i < 4; i++) {
                  arr.push({
                    ...
                    value: items[i].value,
                    disabled: items[i].isDisabled(),
                  })
                  !items[i].isDisabled() && (enabled = true)
                }
                enabled &&
                  new Common.Views.OptionsDialog({
                    title: this.txtInsertCells,
                    items: arr,
                    handler: (dlg, result) => {
                      if (result == "ok") {
                        this.api.asc_insertCells(dlg.getSettings())
                      }
                      Common.NotificationCenter.trigger("edit:complete", this.toolbar)
                    },
                  }).show()
          ...
          shortcuts[
            "command+shift+-,ctrl+shift+-,command+shift+numminus,ctrl+shift+numminus" +
              (Common.Utils.isGecko ? ",command+shift+ff-,ctrl+shift+ff-" : "")
          ] = (e) => {
            if (
              this.editMode &&
              ...
              !this.toolbar.mode.isEditDiagram &&
              !this.toolbar.mode.isEditOle &&
              !this.toolbar.btnDeleteCell.isDisabled()
            ) {
              var cellinfo = this.api.asc_getCellInfo(),
                selectionType = cellinfo.asc_getSelectionType()
              if (
                ...
                this.api.asc_deleteCells(
                  selectionType === Asc.c_oAscSelectionType.RangeRow
                    ? Asc.c_oAscDeleteOptions.DeleteRows
                    : Asc.c_oAscDeleteOptions.DeleteColumns,
                )
                Common.NotificationCenter.trigger("edit:complete", this.toolbar)
              } else {
                var items = this.toolbar.btnDeleteCell.menu.getItems(true),
                  arr = [],
                  enabled = false
                for (var i = 0; i < 4; i++) {
                  arr.push({
                ...
                enabled &&
                  new Common.Views.OptionsDialog({
                    title: this.txtDeleteCells,
                    items: arr,
                    handler: (dlg, result) => {
                      if (result == "ok") {
                        this.api.asc_deleteCells(dlg.getSettings())
                      }
                      Common.NotificationCenter.trigger("edit:complete", this.toolbar)
                    },
                  }).show()
        ...
        onChangeViewMode: function (item, compact, suppressSave) {
          ...
          !suppressSave &&
            Common.localStorage.setBool(
              this.mode.isEdit ? "sse-compact-toolbar" : "sse-view-compact-toolbar",
              compact,
            )
        ...
        fillTableTemplates: function () {
          ...
          function createPicker(element, menu) {
            var picker = new Common.UI.DataView({
              ...
              itemTemplate: _.template(
                '<div class="item-template"><img src="<%= imageUrl %>" id="<%= id %>" style="width:60px;height:44px;"></div>',
              ),
              delayRenderTips: true,
            })

            picker.on("item:click", (picker, item, record) => {
              if (me.api) {
          ...
          if (_.isUndefined(this.toolbar.mnuTableTemplatePicker)) {
            this.toolbar.mnuTableTemplatePicker = createPicker(
              $("#id-toolbar-menu-table-templates"),
              this.toolbar.btnTableTemplate.menu,
            )
          }
        ...
        onTableTplMenuOpen: function (menu) {
          this.onApiInitTableTemplates(
            this.api.asc_getTablePictures(this.api.asc_getCellInfo().asc_getFormatTableInfo()),
          )
          ...
          if (menu && this.toolbar.mnuTableTemplatePicker) {
            ...
            if (picker.cmpEl) {
              var itemEl = $(picker.cmpEl.find(".dataview.inner .item-template").get(0)).parent(),
                itemMargin = 8,
                itemWidth = itemEl.is(":visible") ? Number.parseFloat(itemEl.css("width")) : 60

              var menuWidth = columnCount * (itemMargin + itemWidth) + 11, // for scroller
                menuMargins =
                  Number.parseFloat(picker.cmpEl.css("margin-left")) +
                  Number.parseFloat(picker.cmpEl.css("margin-right"))
              if (menuWidth + menuMargins > Common.Utils.innerWidth())
                menuWidth =
                  Math.max(
                    Math.floor(
                      (Common.Utils.innerWidth() - menuMargins - 11) / (itemMargin + itemWidth),
                    ),
                    2,
                  ) *
          ...
          var val = this.toolbar.mnuTableTemplatePicker.store.findWhere({
            name: this._state.tablestylename,
          })
          if (val) this.toolbar.mnuTableTemplatePicker.selectRecord(val)
          else this.toolbar.mnuTableTemplatePicker.deselectAll()
        },

        onSendThemeColors: function () {
          // get new table templates
          if (
            this.toolbar.btnTableTemplate.rendered &&
            this.toolbar.btnTableTemplate.cmpEl.hasClass("open")
          )
        ...
        onApiInitTableTemplates: function (images) {
          ...
          if (store) {
            var templates = []
            var groups = [
              { id: "menu-table-group-custom", caption: this.txtGroupTable_Custom, templates: [] },
              { id: "menu-table-group-light", caption: this.txtGroupTable_Light, templates: [] },
              { id: "menu-table-group-medium", caption: this.txtGroupTable_Medium, templates: [] },
              { id: "menu-table-group-dark", caption: this.txtGroupTable_Dark, templates: [] },
            ...
            _.each(images, (item) => {
              ...
              if (item.asc_getType() == 0) {
                ...
                tip = this[arr] ? this[arr] + " " + lastWordInTip : tip
            ...
            this.toolbar.mnuTableTemplatePicker.groups.reset(groups)
        ...
        onCellStyleMenuOpen: function (menu) {
          if (menu && this.toolbar.mnuCellStylePicker) {
            ...
            if (picker.cmpEl) {
              var itemEl = $(picker.cmpEl.find(".dataview.inner .item").get(0)),
                itemMargin =
                  Number.parseFloat(itemEl.css("margin-left")) +
                  Number.parseFloat(itemEl.css("margin-right")),
                itemWidth = itemEl.is(":visible") ? Number.parseFloat(itemEl.css("width")) : 106
          ...
          if (val) this.toolbar.mnuCellStylePicker.selectRecord(val)
          else this.toolbar.mnuCellStylePicker.deselectAll()
        },
        ...
        onApiInitEditorStyles: function (styles) {
          ...
          if (this.toolbar.mode.isEditOle) {
            ...
            function createPicker(element, menu) {
              var picker = new Common.UI.DataView({
                ...
                itemTemplate: _.template(
                  '<div class="style"><img src="<%= imageUrl %>" id="<%= id %>" style="width:100px;height:20px;"></div>',
                ),
                delayRenderTips: true,
              })
            ...
            if (_.isUndefined(this.toolbar.mnuCellStylePicker)) {
              this.toolbar.mnuCellStylePicker = createPicker(
                $("#id-toolbar-menu-cell-styles"),
                this.toolbar.btnCellStyle.menu,
              )
            }
          }

          var listStyles = this.toolbar.mode.isEditOle
            ? this.toolbar.mnuCellStylePicker
            : this.toolbar.listStyles
          ...
          var groups = []
          for (var i = 0; i < 4; i++) {
            groups.push("menu-style-group-color")
          }
          for (var i = 0; i < 8; i++) {
            groups.push("menu-style-group-model")
          }
          for (var i = 0; i < 6; i++) {
            groups.push("menu-style-group-title")
          }
          for (var i = 0; i < 24; i++) {
            groups.push("menu-style-group-themed")
          }
          for (var i = 0; i < 5; i++) {
            groups.push("menu-style-group-number")
          }
          ...
          if (count > 0 && count == styles.length) {
            ...
            if (countCustomStyles == 0) {
              groupStore = groupStore.filter((item) => item.id != "menu-style-group-custom")
            }
            if (hasNoNameGroup === false) {
              groupStore = groupStore.filter((item) => item.id != "menu-style-group-no-name")
            }
        ...
        onHomeOpen: function () {
          ...
          if (
            listStyles &&
            listStyles.needFillComboView &&
            listStyles.menuPicker.store.length > 0 &&
            listStyles.rendered
          ) {
            var styleRec
            if (this._state.prstyle)
              styleRec = listStyles.menuPicker.store.findWhere({ name: this._state.prstyle })
        ...
        setDisabledComponents: (components, disable) => {
          _.each([].concat(components), (component) => {
        ...
        onEditCell: function (state, keepState) {
          ...
          if (toolbar.mode.isEditDiagram || toolbar.mode.isEditMailMerge) {
            ...
            toolbar.lockToolbar(
              Common.enumLock.editCell,
              state == Asc.c_oAscCellEditorState.editStart,
              {
                array: [
                  toolbar.btnDecDecimal,
                  toolbar.btnIncDecimal,
                  toolbar.cmbNumberFormat,
                  toolbar.btnEditChartData,
                ],
              },
            )
            ...
            if (
              state == Asc.c_oAscCellEditorState.editStart ||
              state == Asc.c_oAscCellEditorState.editEnd
            ) {
              var is_cell_edited = state == Asc.c_oAscCellEditorState.editStart
              toolbar.lockToolbar(
                Common.enumLock.editCell,
                state == Asc.c_oAscCellEditorState.editStart,
                {
                  array: [
                    ...
                    toolbar.btnWrap,
                    toolbar.btnMerge,
                    toolbar.btnBackColor,
                    toolbar.btnBorders,
                    toolbar.btnTableTemplate,
                    toolbar.btnHorizontalAlign,
                    toolbar.btnVerticalAlign,
                  ],
                  merge: true,
                  clear: !keepState
                    ? [Common.enumLock.editFormula, Common.enumLock.editText]
                    : undefined,
                },
              )
              is_cell_edited
                ? Common.util.Shortcuts.suspendEvents(
                    "command+l, ctrl+l, command+shift+l, ctrl+shift+l",
                  )
                : Common.util.Shortcuts.resumeEvents(
                    "command+l, ctrl+l, command+shift+l, ctrl+shift+l",
                  )
            } else {
              if (state == Asc.c_oAscCellEditorState.editText)
                var is_text = true,
                  is_formula = false
              else if (state == Asc.c_oAscCellEditorState.editFormula)
                is_text = !(is_formula = true)
              else if (state == Asc.c_oAscCellEditorState.editEmptyCell)
              ...
              toolbar.lockToolbar(Common.enumLock.editFormula, is_formula, {
                array: [
                  toolbar.cmbFontName,
                  toolbar.cmbFontSize,
                  toolbar.btnTextFormatting,
                  toolbar.btnTextColor,
                ],
              })
              toolbar.lockToolbar(Common.enumLock.editText, is_text, {
            ...
            is_cell_edited
              ? Common.util.Shortcuts.suspendEvents(
                  "command+l, ctrl+l, command+shift+l, ctrl+shift+l, command+k, ctrl+k, command+1, ctrl+1, " +
                    hkComments,
                )
              : Common.util.Shortcuts.resumeEvents(
                  "command+l, ctrl+l, command+shift+l, ctrl+shift+l, command+k, ctrl+k, command+1, ctrl+1, " +
            ...
            if (
              this.appConfig.isDesktopApp &&
              (this.appConfig.isSignatureSupport || this.appConfig.isPasswordSupport)
            ) {
              this.getApplication()
                .getController("Common.Controllers.Protection")
                .SetDisabled(is_cell_edited, false)
            }
          } else {
            if (state == Asc.c_oAscCellEditorState.editText)
              var is_text = true,
                is_formula = false
            else if (state == Asc.c_oAscCellEditorState.editFormula) is_text = !(is_formula = true)
            else if (state == Asc.c_oAscCellEditorState.editEmptyCell) is_text = is_formula = false

            toolbar.lockToolbar(Common.enumLock.editFormula, is_formula, {
              array: [
                toolbar.cmbFontName,
                toolbar.cmbFontSize,
                toolbar.btnIncFontSize,
                toolbar.btnDecFontSize,
                toolbar.btnChangeCase,
                toolbar.btnBold,
                toolbar.btnItalic,
                toolbar.btnUnderline,
                toolbar.btnStrikeout,
                toolbar.btnSubscript,
                toolbar.btnTextColor,
              ],
            })
            toolbar.lockToolbar(Common.enumLock.editText, is_text, {
              array: [toolbar.btnInsertFormula].concat(toolbar.btnsFormula),
            })
          }
        ...
        onApiSheetChanged: function () {
          ...
          this.onChangeScaleSettings(
            opt.asc_getFitToWidth(),
            opt.asc_getFitToHeight(),
            opt.asc_getScale(),
          )
          ...
          this.api.asc_isLayoutLocked(currentSheet)
            ? this.onApiLockDocumentProps(currentSheet)
            : this.onApiUnLockDocumentProps(currentSheet)
          this.toolbar.lockToolbar(
            Common.enumLock.printAreaLock,
            this.api.asc_isPrintAreaLocked(currentSheet),
            { array: [this.toolbar.btnPrintArea] },
          )
          this.toolbar.lockToolbar(
            Common.enumLock.pageBreakLock,
            this.api.asc_GetPageBreaksDisableType(currentSheet) ===
              Asc.c_oAscPageBreaksDisableType.all,
            { array: [this.toolbar.btnPageBreak] },
          )
        },

        onUpdateDocumentProps: function (nIndex) {
          if (nIndex == this.api.asc_getActiveWorksheetIndex()) this.onApiSheetChanged()
        },
        ...
        onApiPageSize: function (w, h) {
          ...
          if (
            Math.abs(this._state.pgsize[0] - w) > 0.1 ||
            ...
            if (this.toolbar.mnuPageSize) {
              ...
              _.each(
                this.toolbar.mnuPageSize.getItems(true),
                (item) => {
                  if (
                    item.value &&
                    typeof item.value == "object" &&
                    Math.abs(item.value[0] - w) < 0.1 &&
                    Math.abs(item.value[1] - h) < 0.1
                  ) {
        ...
        onApiPageMargins: function (props) {
          if (props) {
            ...
            if (
              !this._state.pgmargins ||
              Math.abs(this._state.pgmargins[0] - top) > 0.1 ||
              Math.abs(this._state.pgmargins[1] - left) > 0.1 ||
              Math.abs(this._state.pgmargins[2] - bottom) > 0.1 ||
              ...
              if (this.toolbar.btnPageMargins.menu) {
                ...
                _.each(
                  this.toolbar.btnPageMargins.menu.getItems(true),
                  (item) => {
                    if (
                      item.value &&
                      typeof item.value == "object" &&
                      Math.abs(item.value[0] - top) < 0.1 &&
                      Math.abs(item.value[1] - left) < 0.1 &&
                      Math.abs(item.value[2] - bottom) < 0.1 &&
                      Math.abs(item.value[3] - right) < 0.1
                    ) {
                      item.setChecked(true)
                      return false
                    }
        ...
        onApiPageOrient: function (orient) {
          if (this._state.pgorient !== orient) {
            this.toolbar.btnPageOrient.menu.items[
              orient == Asc.c_oAscPageOrientation.PagePortrait ? 0 : 1
            ].setChecked(true)
          ...
          if (this.toolbar.btnScale.menu) {
            if (width !== undefined) {
              ...
              if (scale !== undefined) {
                ...
                this.toolbar.setValueCustomScale(
                  this.api.asc_getPageOptions().asc_getPageSetup().asc_getScale(),
                )
              }
              this.toolbar.menuWidthScale.clearAll(true)
              this.toolbar.menuWidthScale.getItems(true).forEach((item) => {
                if (item.value === width) {
                  item.setChecked(true)
                  isWidth = true
                  return false
              ...
              this.toolbar.menuHeightScale.getItems(true).forEach((item) => {
              ...
              if (this.toolbar.mnuScalePicker) {
                this.toolbar.mnuScalePicker.setDisabled(
                  !(!this._state.scaleWidth && !this._state.scaleHeight),
                )
                this.toolbar.mnuCustomScale.setDisabled(
                  !(!this._state.scaleWidth && !this._state.scaleHeight),
                )
              }
            }
        ...
        onApiLockDocumentProps: function (nIndex) {
          if (this._state.lock_doc !== true && nIndex == this.api.asc_getActiveWorksheetIndex()) {
            this.toolbar.lockToolbar(Common.enumLock.docPropsLock, true, {
              array: [
                this.toolbar.btnPageSize,
                this.toolbar.btnPageMargins,
                this.toolbar.btnPageOrient,
                this.toolbar.btnPageBreak,
                this.toolbar.btnScale,
                this.toolbar.btnPrintTitles,
              ],
            })
        ...
        onApiEditorSelectionChanged: function (fontobj) {
          ...
          /* read font params */
          if (
            !toolbar.mode.isEditMailMerge &&
            !toolbar.mode.isEditDiagram &&
            !toolbar.mode.isEditOle
          ) {
              ...
              if (color.get_auto()) {
                ...
                if (
                  ...
                  (type1 != "object" &&
                    ...
                    this._state.clrtext.indexOf(clr) < 0)
                ) {
        ...
        onApiTextDirection: function (xfs, selectionType) {
          ...
          toolbar.btnTextDir.menu.items[2].setDisabled(
            selectionType === Asc.c_oAscSelectionType.RangeShape ||
              selectionType === Asc.c_oAscSelectionType.RangeShapeText ||
              selectionType === Asc.c_oAscSelectionType.RangeChartText ||
              selectionType === Asc.c_oAscSelectionType.RangeChart,
        ...
        onApiSelectionChanged: function (info) {
          var toolbar = this.toolbar
          if (!this.editMode || $(".asc-window.enable-key-events:visible").length > 0 || !info)
            return
          if (this.toolbar.mode.isEditDiagram) return this.onApiSelectionChanged_DiagramEditor(info)
          else if (this.toolbar.mode.isEditMailMerge)
            return this.onApiSelectionChanged_MailMergeEditor(info)
          else if (this.toolbar.mode.isEditOle) return this.onApiSelectionChanged_OleEditor(info)
          ...
          toolbar.lockToolbar(
            Common.enumLock.cantHyperlink,
            (selectionType === Asc.c_oAscSelectionType.RangeShapeText ||
              selectionType === Asc.c_oAscSelectionType.RangeShape ||
              selectionType === Asc.c_oAscSelectionType.RangeImage) &&
              this.api.asc_canAddShapeHyperlink() === false,
            { array: [toolbar.btnInsertHyperlink] },
          )
          ...
          toolbar.lockToolbar(Common.enumLock.selRange, need_disable, {
            array: [
              toolbar.btnImgAlign,
              toolbar.btnImgBackward,
              toolbar.btnImgForward,
              toolbar.btnImgGroup,
              toolbar.btnShapesMerge,
            ],
          })

          var cangroup = this.api.asc_canGroupGraphicsObjects(),
            canungroup = this.api.asc_canUnGroupGraphicsObjects()
          toolbar.lockToolbar(Common.enumLock.cantGroupUngroup, !cangroup && !canungroup, {
            array: [toolbar.btnImgGroup],
          })
          toolbar.btnImgGroup.menu.items[0].setDisabled(!cangroup)
          toolbar.btnImgGroup.menu.items[1].setDisabled(!canungroup)
          toolbar.lockToolbar(Common.enumLock.cantGroup, !cangroup, {
            array: [toolbar.btnImgAlign],
          })
          ...
          need_disable =
            ...
            selectionType === Asc.c_oAscSelectionType.RangeSlicer
          toolbar.lockToolbar(
            Common.enumLock.wsLockFormat,
            (need_disable && !!this._state.wsProps["Objects"] && !!this._state.is_lockText) ||
              (!need_disable && !!this._state.wsProps["FormatCells"]),
          )
          toolbar.lockToolbar(
            Common.enumLock.wsLockFormatFill,
            (need_disable && !!this._state.wsProps["Objects"] && !!this._state.is_lockShape) ||
              (!need_disable && !!this._state.wsProps["FormatCells"]),
          )
          ...
          toolbar.lockToolbar(
            Common.enumLock.commentLock,
            (selectionType == Asc.c_oAscSelectionType.RangeCells &&
              (!info.asc_getComments() ||
                info.asc_getComments().length > 0 ||
                info.asc_getLocked())) ||
              selectionType != Asc.c_oAscSelectionType.RangeCells,
          ...
          toolbar.lockToolbar(
            Common.enumLock.pageBreakLock,
            this.api.asc_GetPageBreaksDisableType(this.api.asc_getActiveWorksheetIndex()) ===
              Asc.c_oAscPageBreaksDisableType.all,
            { array: [toolbar.btnPageBreak] },
          )
          ...
          if (!editOptionsDisabled && !paragraphColorPicker.isDummy) {
            ...
            if (
              type1 !== type2 ||
              (type1 == "object" &&
                (clr.effectValue !== this._state.clrback.effectValue ||
                  this._state.clrback.color.indexOf(clr.color) < 0)) ||
              (type1 != "object" &&
                this._state.clrback !== undefined &&
                this._state.clrback.indexOf(clr) < 0)
            ) {
          ...
          if (!toolbar.mode.isEditDiagram) {
            ...
            if (!editOptionsDisabled && !in_chart && !toolbar.mode.isEditMailMerge) {
              ...
              need_disable =
                this._state.controlsdisabled.filters ||
                formatTableInfo !== null ||
                (filterInfo && filterInfo.asc_getIsAutoFilter() === null)
              //                (need_disable !== toolbar.btnMerge.isDisabled()) && toolbar.btnMerge.setDisabled(need_disable);
              toolbar.lockToolbar(Common.enumLock.ruleMerge, need_disable, {
                array: [toolbar.btnMerge, toolbar.btnInsertTable],
              })
            ...
            toolbar.lockToolbar(Common.enumLock.ruleFilter, need_disable, {
              array: toolbar.btnsSetAutofilter
                .concat(
                  toolbar.btnCustomSort,
                  toolbar.btnTableTemplate,
                  toolbar.btnInsertTable,
                  toolbar.btnDataValidation,
                )
                .concat(toolbar.btnsRemoveDuplicates),
            })

            toolbar.lockToolbar(
              Common.enumLock.tableHasSlicer,
              filterInfo && filterInfo.asc_getIsSlicerAdded(),
              { array: toolbar.btnsSetAutofilter },
            )

            need_disable =
              selectionType !== Asc.c_oAscSelectionType.RangeSlicer &&
              (this._state.controlsdisabled.filters || val === null)
            toolbar.lockToolbar(Common.enumLock.cantSort, need_disable, {
              array: toolbar.btnsSortDown.concat(toolbar.btnsSortUp),
            })
            ...
            need_disable =
              this._state.controlsdisabled.filters ||
              !filterInfo ||
              filterInfo.asc_getIsApplyAutoFilter() !== true
            toolbar.lockToolbar(Common.enumLock.ruleDelFilter, need_disable, {
              array: toolbar.btnsClearAutofilter,
            })
            ...
            toolbar.lockToolbar(Common.enumLock.multiselect, this._state.multiselect, {
              array: [toolbar.btnTableTemplate, toolbar.btnInsertHyperlink, toolbar.btnInsertTable],
            })
            ...
            if (this._state.inpivot !== inpivot) {
              if (!inpivot && this.toolbar.isTabActive("pivot")) this.toolbar.setTab("home")
              this.toolbar.setVisible("pivot", !!inpivot)
              if (inpivot && this._state.showPivotTab) this.toolbar.setTab("pivot")
              this._state.inpivot = inpivot
            }
            toolbar.lockToolbar(Common.enumLock.editPivot, this._state.inpivot, {
              array: toolbar.btnsSetAutofilter
                .concat(
                  toolbar.btnMerge,
                  toolbar.btnInsertHyperlink,
                  toolbar.btnInsertTable,
                  toolbar.btnDataValidation,
                )
                .concat(toolbar.btnsRemoveDuplicates),
            ...
            toolbar.lockToolbar(
              Common.enumLock.noSlicerSource,
              !(this._state.inpivot || formatTableInfo),
              { array: [toolbar.btnInsertSlicer] },
            )

            need_disable = !this.appConfig.canModifyFilter
            toolbar.lockToolbar(Common.enumLock.cantModifyFilter, need_disable, {
              array: toolbar.btnsSetAutofilter
                .concat(
                  toolbar.btnsSortDown,
                  toolbar.btnsSortUp,
                  toolbar.btnCustomSort,
                  toolbar.btnTableTemplate,
                  toolbar.btnClearStyle.menu.items[0],
                  toolbar.btnClearStyle.menu.items[2],
                  toolbar.btnInsertTable,
                  toolbar.btnDataValidation,
                )
                .concat(toolbar.btnsRemoveDuplicates)
                .concat(toolbar.btnsTableDesign),
            })
          }
          ...
          if (!editOptionsDisabled && !in_chart) {
            ...
            if (this._state.angle !== val) {
              ...
              switch (val) {
                ...
                case 90:
                  ...
                  break
                case -90:
                  toolbar.btnTextOrient.menu.items[5].setChecked(true, true)
                  break
                case 0:
            ...
            need_disable =
              selRow || (selMax && this._state.wsLock) || this._state.wsProps["InsertColumns"]
            toolbar.btnAddCell.menu.items[3].setDisabled(need_disable)

            need_disable =
              selRow ||
              (selMax && this._state.wsLock) ||
              (!selCol && this._state.wsLock) ||
              this._state.wsProps["DeleteColumns"]
            toolbar.btnDeleteCell.menu.items[3].setDisabled(need_disable)

            need_disable =
              selCol || (selMax && this._state.wsLock) || this._state.wsProps["InsertRows"]
            toolbar.btnAddCell.menu.items[2].setDisabled(need_disable)

            need_disable =
              selCol ||
              (selMax && this._state.wsLock) ||
              (!selRow && this._state.wsLock) ||
              this._state.wsProps["DeleteRows"]
            ...
            need_disable =
              selRow ||
              val ||
              (!(selCol || selMax) && this._state.wsLock) ||
              (selCol && this._state.wsProps["InsertColumns"]) ||
              (selMax && this._state.wsProps["InsertColumns"] && this._state.wsProps["InsertRows"])
            ...
            need_disable =
              ...
              val ||
              (!(selCol || selMax) && this._state.wsLock) ||
              (selCol && this._state.wsProps["DeleteColumns"]) ||
              (selMax && this._state.wsProps["DeleteColumns"] && this._state.wsProps["DeleteRows"])
            toolbar.btnDeleteCell.menu.items[0].setDisabled(need_disable)

            need_disable =
              selCol ||
              val ||
              (!(selRow || selMax) && this._state.wsLock) ||
              (selRow && this._state.wsProps["InsertRows"]) ||
              (selMax && this._state.wsProps["InsertColumns"] && this._state.wsProps["InsertRows"])
            ...
            need_disable =
              ...
              val ||
              (!(selRow || selMax) && this._state.wsLock) ||
              (selRow && this._state.wsProps["DeleteRows"]) ||
              (selMax && this._state.wsProps["DeleteColumns"] && this._state.wsProps["DeleteRows"])
            ...
            toolbar.lockToolbar(Common.enumLock.itemsDisabled, !enabled, {
              array: [toolbar.btnAddCell],
            })

            items = this.toolbar.btnDeleteCell.menu.getItems(true)
            enabled = false
            for (var i = 0; i < 4; i++) {
              !items[i].isDisabled() && (enabled = true)
            }
            toolbar.lockToolbar(Common.enumLock.itemsDisabled, !enabled, {
              array: [toolbar.btnDeleteCell],
            })
          }

          toolbar.lockToolbar(Common.enumLock.headerLock, info.asc_getLockedHeaderFooter(), {
            array: this.toolbar.btnsEditHeader,
          })
          ...
          if (
            toolbar.btnFormatCell &&
            !toolbar.btnFormatCell.isDisabled() &&
            toolbar.isTabActive("home")
          )
        ...
        onApiSelectionChangedRestricted: function (info) {
          ...
          this.toolbar.lockToolbar(Common.enumLock["Objects"], !!this._state.wsProps["Objects"], {
            array: this.btnsComment,
          })
        },
        ...
        onApiSelectionChanged_DiagramEditor: function (info) {
          ...
          var _disableEditOptions = (seltype, coauth_disable) => {
            ...
            if (is_mode_2) {
              var selectedObjects = this.api.asc_getGraphicObjectProps()
              is_objLocked = selectedObjects.some(
                (object) =>
                  object.asc_getObjectType() == Asc.c_oAscTypeSelectElement.Image &&
                  object.asc_getObjectValue().asc_getLocked(),
              )
            }
            ...
            this.toolbar.lockToolbar(type, type != seltype, {
              clear: [
                _set.selImage,
                _set.selChart,
                _set.selChartText,
            ...
            this.toolbar.lockToolbar(Common.enumLock.coAuthText, is_objLocked)
          ...
          if (val) {
            ...
            val = val.asc_getType()
            if (this._state.numformattype !== val) {
              this.toolbar.cmbNumberFormat.setValue(val, this.toolbar.txtCustom)
              this._state.numformattype = val
            }
          }
        },
        ...
        onApiSelectionChanged_MailMergeEditor: function (info) {
          ...
          if (!this.toolbar.mode.isEditDiagram) {
            ...
            val = filterInfo ? filterInfo.asc_getIsAutoFilter() : null
            if (this._state.filter !== val) {
              this.toolbar.btnSetAutofilter.toggle(val === true, true)
              this._state.filter = val
            }

            need_disable = this._state.controlsdisabled.filters || val === null
            this.toolbar.lockToolbar(Common.enumLock.ruleFilter, need_disable, {
              array: [
                this.toolbar.btnSetAutofilter,
                this.toolbar.btnSortDown,
                this.toolbar.btnSortUp,
              ],
            })
            ...
            need_disable =
              ...
              !filterInfo ||
              filterInfo.asc_getIsApplyAutoFilter() !== true
            this.toolbar.lockToolbar(Common.enumLock.ruleDelFilter, need_disable, {
              array: [this.toolbar.btnClearAutofilter],
            })
          }
        },
        ...
        onApiSelectionChanged_OleEditor: function (info) {
          ...
          var _disableEditOptions = (seltype, coauth_disable) => {
            ...
            this._state.controlsdisabled.filters = is_image || is_mode_2
          ...
          if (this._state.pralign !== fontparam) {
            ...
            if (toolbar.btnHorizontalAlign.rendered && toolbar.btnHorizontalAlign.$icon) {
              toolbar.btnHorizontalAlign.$icon
                .removeClass(toolbar.btnHorizontalAlign.options.icls)
                .addClass(align)
          ...
          if (this._state.valign !== fontparam) {
            ...
            if (toolbar.btnVerticalAlign.rendered && toolbar.btnVerticalAlign.$icon) {
              toolbar.btnVerticalAlign.$icon
                .removeClass(toolbar.btnVerticalAlign.options.icls)
                .addClass(align)
          ...
          need_disable = this._state.controlsdisabled.filters || val === null
          toolbar.lockToolbar(Common.enumLock.ruleFilter, need_disable, {
            array: [toolbar.btnTableTemplate],
          })
        ...
        updateThemeColors: function () {
          var updateColors = (picker, defaultColor) => {
            if (picker) {
              var clr
          ...
          if (
            this.toolbar.btnTextColor.currentColor === undefined ||
            !this.toolbar.btnTextColor.currentColor.isAuto
          ) {
          ...
          if (this.toolbar.btnBackColor.currentColor === undefined) {
            ...
            this.toolbar.btnBackColor.currentColor = this.toolbar.mnuBackColorPicker.currentColor
              ? this.toolbar.mnuBackColorPicker.currentColor.color ||
                this.toolbar.mnuBackColorPicker.currentColor
              : this.toolbar.mnuBackColorPicker.currentColor
          ...
          if (this.toolbar.mnuBorderColorPicker) {
            ...
            this.toolbar.btnBorders.options.borderscolor = currentColor
              ? currentColor.color || currentColor
              : currentColor
            $("#id-toolbar-mnu-item-border-color > a .menu-item-icon").css(
              "border-color",
              "#" + this.toolbar.btnBorders.options.borderscolor,
            )
          }
        ...
        onApiAutoShapes: function () {
          var onShowBefore = (menu) => {
            this.fillAutoShapes()
            menu.off("show:before", onShowBefore)
          }
          this.toolbar.btnInsertShape.menu.on("show:before", onShowBefore)
        },

        fillAutoShapes: function () {
          var menuitem = new Common.UI.MenuItem({
            template: _.template(
              '<div id="id-toolbar-menu-insertshape" class="menu-insertshape"></div>',
            ),
          })
          this.toolbar.btnInsertShape.menu.addItem(menuitem, true)
          ...
          var shapePicker = new Common.UI.DataViewShape({
            ...
            groups: this.getApplication().getCollection("ShapeGroups"),
            parentMenu: this.toolbar.btnInsertShape.menu,
            outerMenu: { menu: this.toolbar.btnInsertShape.menu, index: 0 },
            restoreHeight: 652,
            textRecentlyUsed: this.textRecentlyUsed,
            recentShapes: recents ? JSON.parse(recents) : null,
          })
          this.toolbar.btnInsertShape.menu.setInnerMenu([{ menu: shapePicker, index: 0 }])
          shapePicker.on("item:click", (picker, item, record, e) => {
            if (this.api) {
              if (record) {
                this._addAutoshape(true, record.get("data").shapeType)
              ...
              if (e.type !== "click") this.toolbar.btnInsertShape.menu.hide()
              Common.NotificationCenter.trigger(
                "edit:complete",
                this.toolbar,
                this.toolbar.btnInsertShape,
        ...
        fillEquations: function () {
          if (
            !this.toolbar.btnInsertEquation.rendered ||
            this.toolbar.btnInsertEquation.menu.getItemsLength(true) > 0
          )
            return

          var equationsStore = this.getApplication().getCollection("EquationGroups")
          var onShowAfter = (menu) => {
            for (var i = 0; i < equationsStore.length; ++i) {
              ...
              equationPicker.on("item:click", (picker, item, record, e) => {
                if (this.api) {
                  ...
                  if (e.type !== "click") this.toolbar.btnInsertEquation.menu.hide()
                  Common.NotificationCenter.trigger(
                    "edit:complete",
                    this.toolbar,
                    this.toolbar.btnInsertEquation,
                  )
          ...
          this.toolbar.btnInsertEquation.menu.on("show:after", onShowAfter)
          ...
          for (var i = 0; i < equationsStore.length; ++i) {
            ...
            var menuItem = new Common.UI.MenuItem({
              ...
              menu: new Common.UI.Menu({
                ...
                items: [
                  {
                    template: _.template(
                      '<div id="id-toolbar-menu-equationgroup' +
                        i +
                        '" class="menu-shape margin-left-5" style="width:' +
                        (equationGroup.get("groupWidth") + 8) +
                        "px; " +
                        equationGroup.get("groupHeightStr") +
                        '"></div>',
                    ),
                  },
                ],
            ...
            this.toolbar.btnInsertEquation.menu.addItem(menuItem, true)
          }
        },
        ...
        onInsertSymbolClick: function () {
          Common.NotificationCenter.trigger(
            "protect:check",
            this.onInsertSymbolClickCallback,
            this,
            [],
          )
        },
            ...
            var selected = this.api.asc_GetSelectedText(),
              win = new Common.Views.SymbolTableDialog({
                ...
                handler: (dlg, result, settings) => {
                  ...
                  } else Common.NotificationCenter.trigger("edit:complete", this.toolbar)
                },
              })
            win.show()
        ...
        onInsertSlicerClick: function () {
          var props = this.api.asc_beforeInsertSlicer()
          if (props) {
            new SSE.Views.SlicerAddDialog({
              props: props,
              handler: (result, settings) => {
                if (this && this.api && result == "ok") {
                  this.api.asc_insertSlicer(settings)
                }
                Common.NotificationCenter.trigger("edit:complete", this.toolbar)
              },
            }).show()
        ...
        onApiMathTypes: function (equation) {
          ...
          var onShowBefore = (menu) => {
            this.onMathTypes(this._equationTemp)
            if (this._equationTemp && this._equationTemp.get_Data().length > 0) this.fillEquations()
            this.toolbar.btnInsertEquation.menu.off("show:before", onShowBefore)
          }
          this.toolbar.btnInsertEquation.menu.on("show:before", onShowBefore)
        },
            ...
            if (count) {
              for (var j = 0; j < count; ++j) {
                ...
                if (store) {
                  ...
                  var normHeight = Number.parseInt(370 / (height + 10)) * (height + 10)
        ...
        _disableEditOptions: function (seltype, coauth_disable) {
          ...
          if (coauth_disable) {
            ...
            toolbar.lockToolbar(
              Common.enumLock.coAuthText,
              is_objLocked &&
                (seltype == Asc.c_oAscSelectionType.RangeChart ||
                  seltype == Asc.c_oAscSelectionType.RangeChartText),
              { array: [toolbar.btnInsertChart, toolbar.btnInsertChartRecommend] },
            )
        ...
        _setTableFormat: function (fmtname) {
          if (this.api.isRangeSelection !== true) {
            if (!this.api.asc_getCellInfo().asc_getFormatTableInfo()) {
              var handlerDlg = (dlg, result) => {
                if (result == "ok") {
                  this._state.filter = undefined
                  this.api.asc_setSelectionDialogMode(Asc.c_oAscSelectionDialogType.None)

                  if (this._state.tablename)
                    this.api.asc_changeAutoFilter(
                      this._state.tablename,
                      Asc.c_oAscChangeFilterOptions.style,
                      fmtname,
                    )
                  else {
                    var settings = dlg.getSettings()
                    if (
                      settings.selectionType == Asc.c_oAscSelectionType.RangeMax ||
                      settings.selectionType == Asc.c_oAscSelectionType.RangeRow ||
                      settings.selectionType == Asc.c_oAscSelectionType.RangeCol
                    )
                      Common.UI.warning({
                        title: this.textLongOperation,
                        msg: this.warnLongOperation,
                        buttons: ["ok", "cancel"],
                        callback: (btn) => {
                          if (btn == "ok")
                            setTimeout(() => {
                              this.toolbar.fireEvent("inserttable", this.toolbar)
                              this.api.asc_addAutoFilter(fmtname, settings.range)
                            }, 1)
                          Common.NotificationCenter.trigger("edit:complete", this.toolbar)
                        },
                      })
              ...
              win.setSettings({
                api: this.api,
                selectionType: this.api.asc_getCellInfo().asc_getSelectionType(),
              })
            } else {
              this._state.filter = undefined
              if (this._state.tablename)
                ...
                var selectionType = this.api.asc_getCellInfo().asc_getSelectionType()
                if (
                  selectionType == Asc.c_oAscSelectionType.RangeMax ||
                  selectionType == Asc.c_oAscSelectionType.RangeRow ||
                  selectionType == Asc.c_oAscSelectionType.RangeCol
                  ...
                  Common.UI.warning({
                    ...
                    callback: (btn) => {
                      if (btn == "ok")
                        setTimeout(() => {
                          this.toolbar.fireEvent("inserttable", this.toolbar)
                          this.api.asc_addAutoFilter(fmtname)
                        }, 1)
                      Common.NotificationCenter.trigger("edit:complete", this.toolbar)
                    },
                  })
        ...
        onSetupCopyStyleButton: function () {
          ...
          Common.NotificationCenter.on({
            "edit:complete": (cmp) => {
              if (this.api && this.modeAlwaysSetStyle && cmp !== "tab") {
          ...
          $(this.toolbar.btnCopyStyle.cmpEl).dblclick(() => {
            if (this.api) {
              this.modeAlwaysSetStyle = true
              this.toolbar.btnCopyStyle.toggle(true, true)
              this.api.asc_formatPainter(AscCommon.c_oAscFormatPainterState.kMultiple)
            }
          })
        },
        ...
        onCellsRange: function (status) {
          ...
          this.onEditCell(
            this.api.isRangeSelection || this.api.isCellEdited
              ? Asc.c_oAscCellEditorState.editStart
              : Asc.c_oAscCellEditorState.editEnd,
            true,
          )
          ...
          this.setDisabledComponents(
            [toolbar.btnUndo],
            this.api.isRangeSelection || !this.api.asc_getCanUndo(),
          )
          this.setDisabledComponents(
            [toolbar.btnRedo],
            this.api.isRangeSelection || !this.api.asc_getCanRedo(),
          )
        ...
        onLockDefNameManager: function (state) {
          ...
          this.toolbar.lockToolbar(
            Common.enumLock.printAreaLock,
            this.api.asc_isPrintAreaLocked(this.api.asc_getActiveWorksheetIndex()),
            { array: [this.toolbar.btnPrintArea] },
          )
          this.toolbar.lockToolbar(Common.enumLock.namedRangeLock, this._state.namedrange_locked, {
            array: [
              this.toolbar.btnPrintArea.menu.items[0],
              this.toolbar.btnPrintArea.menu.items[2],
            ],
          })
        },
        ...
        onAppShowed: function (config) {
          ...
          if (!config.isEditDiagram && !config.isEditMailMerge && !config.isEditOle) {
            if (
              Common.localStorage.itemExists(
                editmode ? "sse-compact-toolbar" : "sse-view-compact-toolbar",
              )
            ) {
          ...
          this.toolbar.render(_.extend({ isCompactView: editmode ? compactview : true }, config))

          if (!config.isEditDiagram && !config.isEditMailMerge && !config.isEditOle) {
            var tab = {
              action: "review",
              caption: this.toolbar.textTabCollaboration,
              layoutname: "toolbar-collaboration",
              dataHintTitle: "U",
            }
            var $panel = this.getApplication()
              .getController("Common.Controllers.ReviewChanges")
              .createToolbarPanel()
            if ($panel) {
              this.toolbar.addTab(tab, $panel, 6)
              this.toolbar.setVisible(
                "review",
                (config.isEdit ||
                  config.canViewReview ||
                  (config.canCoAuthoring && config.canComments)) &&
          ...
          if (config.isEdit) {
            ...
            this.toolbar.btnSave &&
              this.toolbar.btnSave.on(
                "disabled",
                _.bind(this.onBtnChangeState, this, "save:disabled"),
              )
            this.toolbar.btnUndo &&
              this.toolbar.btnUndo.on(
                "disabled",
                _.bind(this.onBtnChangeState, this, "undo:disabled"),
              )
            this.toolbar.btnRedo &&
              this.toolbar.btnRedo.on(
                "disabled",
                _.bind(this.onBtnChangeState, this, "redo:disabled"),
              )
            this.toolbar.btnPrint &&
              this.toolbar.btnPrint.on(
                "disabled",
                _.bind(this.onBtnChangeState, this, "print:disabled"),
              )
            this.toolbar.setApi(this.api)

            if (!config.isEditDiagram && !config.isEditMailMerge && !config.isEditOle) {
              var drawtab = this.getApplication().getController("Common.Controllers.Draw")
              drawtab.setApi(this.api).setMode(config)
              $panel = drawtab.createToolbarPanel()
              if ($panel) {
                tab = {
                  action: "draw",
                  caption: this.toolbar.textTabDraw,
                  extcls: "canedit",
                  layoutname: "toolbar-draw",
                  dataHintTitle: "C",
                }
                this.toolbar.addTab(tab, $panel, 2)
                this.toolbar.setVisible(
                  "draw",
                  Common.UI.LayoutManager.isElementVisible("toolbar-draw"),
                )
                Array.prototype.push.apply(
                  this.toolbar.lockControls,
                  drawtab.getView().getButtons(),
                )
              }

              var datatab = this.getApplication().getController("DataTab")
              datatab.setApi(this.api).setConfig({ toolbar: this })

              datatab = datatab.getView("DataTab")
              Array.prototype.push.apply(this.toolbar.lockControls, datatab.getButtons())
              this.toolbar.btnsSortDown = datatab.getButtons("sort-down")
              this.toolbar.btnsSortUp = datatab.getButtons("sort-up")
              this.toolbar.btnsSetAutofilter = datatab.getButtons("set-filter")
              this.toolbar.btnsClearAutofilter = datatab.getButtons("clear-filter")
              this.toolbar.btnCustomSort = datatab.getButtons("sort-custom")
              this.toolbar.btnsRemoveDuplicates = [datatab.getButtons("rem-duplicates")]
              this.toolbar.btnDataValidation = datatab.getButtons("data-validation")

              var formulatab = this.getApplication().getController("FormulaDialog")
              formulatab.setConfig({ toolbar: this })
              formulatab = formulatab.getView("FormulaTab")
              this.toolbar.btnsFormula = formulatab.getButtons("formula")
              var namedRange = formulatab.getButtons("range")
              this.toolbar.itemsNamedRange =
                namedRange && namedRange.menu && namedRange.menu.items
                  ? [namedRange.menu.items[0], namedRange.menu.items[1]]
                  : []
              Array.prototype.push.apply(this.toolbar.lockControls, formulatab.getButtons())

              if (config.canFeaturePivot) {
                tab = { action: "pivot", caption: this.textPivot, dataHintTitle: "B", aux: true }
                var pivottab = this.getApplication().getController("PivotTable")
                pivottab.setApi(this.api).setConfig({ toolbar: this })
                $panel = pivottab.createToolbarPanel()
                if ($panel) {
                  this.toolbar.addTab(tab, $panel)
                  this._state.inpivot && this.toolbar.setVisible("pivot", true)
                  Array.prototype.push.apply(
                    this.toolbar.lockControls,
                    pivottab.getView("PivotTable").getButtons(),
                  )
                }
              }
              ...
              if (config.canFeatureTable) {
                tab = {
                  caption: this.toolbar.textTabTableDesign,
                  action: "tabledesign",
                  extcls: config.isEdit ? "canedit" : "",
                  layoutname: "toolbar-tabledesign",
                  dataHintTitle: "B",
                  aux: true,
                }
                var tabledesigntab = this.getApplication().getController("TableDesignTab")
                tabledesigntab.setApi(this.api).setConfig({ toolbar: this })
                ...
                if ($panel) {
                  this.toolbar.addTab(tab, $panel)
                  this._state.intabledesign && this.toolbar.setVisible("tabledesign", true)
                  Array.prototype.push.apply(this.toolbar.lockControls, tabledesignbuttons)
                }
                this.toolbar.btnsRemoveDuplicates.push(view.getButtons("rem-duplicates"))
                this.toolbar.btnsTableDesign = tabledesignbuttons
              }

              if (!config.compactHeader) {
                // hide 'print' and 'save' buttons group and next separator
                this.toolbar.btnPrint.$el.parents(".group").hide().next().hide()

                // hide 'undo' and 'redo' buttons and get container
                var $box = this.toolbar.btnUndo.$el.hide().next().hide().parent()

                // move 'paste' button to the container instead of 'undo' and 'redo'
                this.toolbar.btnPaste.$el.detach().appendTo($box)
                this.toolbar.btnPaste.$el.find("button").attr("data-hint-direction", "bottom")
                this.toolbar.btnCopy.$el.removeClass("split")
              ...
              if (config.canProtect) {
                var tab = {
                  action: "protect",
                  caption: this.toolbar.textTabProtect,
                  layoutname: "toolbar-protect",
                  dataHintTitle: "T",
                }
                var $panel = this.getApplication()
                  .getController("Common.Controllers.Protection")
                  .createToolbarPanel()
                if ($panel) {
                  ;(config.isSignatureSupport || config.isPasswordSupport) &&
                    $panel.append($('<div class="separator long"></div>'))
                  var wbtab = this.getApplication().getController("WBProtection")
                  $panel.append(wbtab.createToolbarPanel())
                  this.toolbar.addTab(tab, $panel, 7)
                  this.toolbar.setVisible(
                    "protect",
                    Common.UI.LayoutManager.isElementVisible("toolbar-protect"),
                  )
                  Array.prototype.push.apply(
                    this.toolbar.lockControls,
                    wbtab.getView("WBProtection").getButtons(),
                  )
                }
              }

              this.getApplication()
                .getController("Common.Controllers.ExternalLinks")
                .setConfig({ toolbar: this })
                .setApi(this.api)
            }
          }
          if (!config.isEditDiagram && !config.isEditMailMerge && !config.isEditOle) {
            ...
            var viewtab = this.getApplication().getController("ViewTab")
            ...
            config.isEdit &&
              Array.prototype.push.apply(
                this.toolbar.lockControls,
                viewtab.getView("ViewTab").getButtons(),
              )
          }
        },
        ...
        onAppReady: function (config) {
          ...
          if (config.canCoAuthoring && config.canComments) {
            var _set = Common.enumLock
            this.btnsComment = Common.Utils.injectButtons(
              this.toolbar.$el.find(".slot-comment"),
              "tlbtn-addcomment-",
              "toolbar__icon btn-big-add-comment",
              this.toolbar.capBtnComment,
              [_set.lostConnect, _set.commentLock, _set.editCell, _set["Objects"]],
              undefined,
              undefined,
              undefined,
              "1",
              ...
              this.btnsComment.forEach((btn) => {
                ...
                btn.on("click", (btn, e) => {
                  Common.NotificationCenter.trigger(
                    ...
                    "toolbar",
                    this.api.asc_getCellInfo().asc_getSelectionType() !=
                      Asc.c_oAscSelectionType.RangeCells,
                  )
                })
                if (btn.cmpEl.closest("#review-changes-panel").length > 0)
                  btn.setCaption(this.toolbar.capBtnAddComment)
              }, this)
              if (_comments.buttonAddNew) {
                _comments.buttonAddNew.options.lock = [
                  _set.lostConnect,
                  _set.commentLock,
                  _set.editCell,
                  _set["Objects"],
          ...
          if (
            config.isEdit &&
            !config.isEditDiagram &&
            !config.isEditMailMerge &&
            !config.isEditOle
          ) {
            if (
              this.toolbar.btnTextDir &&
              !this.toolbar.btnTextDir.isDisabled() &&
              this.toolbar.isTabActive("home")
            )
              Common.UI.TooltipManager.showTip("rtlDirection")
            if (
              this.toolbar.btnFormatCell &&
              !this.toolbar.btnFormatCell.isDisabled() &&
              this.toolbar.isTabActive("home")
            )
              Common.UI.TooltipManager.showTip("cellFormat")
          }
        ...
        onPageMarginsSelect: function (menu, item) {
          if (this.api) {
            ...
            if (item.value !== "advanced") {
              ...
              this.api.asc_changePageMargins(
                props,
                undefined,
                undefined,
                undefined,
              ...
              win = new SSE.Views.PageMarginsDialog({
                handler: (dlg, result) => {
                  if (result == "ok") {
                    ...
                    this.toolbar.btnPageMargins.menu.items[0].setChecked(true)
              ...
              win.show()
              win.setSettings(this.api.asc_getPageOptions(this.api.asc_getActiveWorksheetIndex()))
            }
        ...
        onPageOrientSelect: function (menu, item) {
          ...
          if (this.api && item.checked) {
            this.api.asc_changePageOrient(
              item.value == Asc.c_oAscPageOrientation.PagePortrait,
              this.api.asc_getActiveWorksheetIndex(),
            )
          }
        ...
        onImgGroupSelect: function (menu, item) {
          if (this.api)
            this.api[
              item.value == "grouping" ? "asc_groupGraphicsObjects" : "asc_unGroupGraphicsObjects"
            ]()
          Common.NotificationCenter.trigger("edit:complete", this.toolbar)
        ...
        onImgArrangeSelect: function (menu, item) {
          if (this.api) {
            ...
            else this.api.asc_setSelectedDrawingObjectLayer(item.value)
          }
          Common.NotificationCenter.trigger("edit:complete", this.toolbar)
        ...
        onPageBreakClick: function (menu, item) {
          if (this.api) {
            if (item.value === "ins") this.api.asc_InsertPageBreak()
            else if (item.value === "del") this.api.asc_RemovePageBreak()
            else if (item.value === "reset") this.api.asc_ResetAllPageBreaks()
        ...
        onPageBreakMenuOpen: function () {
          if (this.api) {
            ...
            this.toolbar.btnPageBreak.menu.items[0].setDisabled(
              type === Asc.c_oAscPageBreaksDisableType.insertRemove,
            )
            this.toolbar.btnPageBreak.menu.items[1].setDisabled(
              type === Asc.c_oAscPageBreaksDisableType.insertRemove,
            )
            this.toolbar.btnPageBreak.menu.items[2].setDisabled(
              type === Asc.c_oAscPageBreaksDisableType.reset,
            )
          }
        },
        ...
        onEditHeaderClick: function (pageSetup, btn) {
          if (_.isUndefined(this.fontStore)) {
            ...
            var fonts = this.toolbar.cmbFontName.store.toJSON()
            var arr = []
          ...
          var win = new SSE.Views.HeaderFooterDialog({
            ...
            handler: (dlg, result) => {
              if (result === "ok") {
                this.getApplication().getController("Print").updatePreview()
              }
        ...
        onScaleClick: function (type, menu, item, event, scale) {
          if (this.api) {
            if (type === "width" && item.value !== "more") {
              if (this._state.scaleHeight === undefined || this._state.scaleHeight === null) {
                this._state.scaleHeight = 0
              }
              this.api.asc_SetPrintScale(item.value, this._state.scaleHeight, 100)
              this.onChangeScaleSettings(item.value, this._state.scaleHeight, 100)
            } else if (type === "height" && item.value !== "more") {
              if (this._state.scaleWidth === undefined || this._state.scaleWidth === null) {
                this._state.scaleWidth = 0
              }
              this.api.asc_SetPrintScale(this._state.scaleWidth, item.value, 100)
              this.onChangeScaleSettings(this._state.scaleWidth, item.value, 100)
              ...
              var win = new SSE.Views.ScaleDialog({
                ...
                handler: (dlg, result) => {
                  if (dlg == "ok") {
                    if (this.api && result) {
                      this.api.asc_SetPrintScale(result.width, result.height, result.scale)
                      this.onChangeScaleSettings(result.width, result.height, result.scale)
                    }
                  } else {
                    this.onChangeScaleSettings(
                      this._state.scaleWidth,
                      this._state.scaleHeight,
                      this._state.scale,
                    )
                  }
                  Common.NotificationCenter.trigger("edit:complete")
        ...
        onPrintTitlesClick: function (btn) {
          if (this.api) {
            ...
            win = new SSE.Views.PrintTitlesDialog({
              api: this.api,
              sheet: this.api.asc_getActiveWorksheetIndex(),
              handler: (dlg, result) => {
                if (result == "ok") {
                  props = dlg.getSettings()
                  this.api.asc_changePrintTitles(
                    props.width,
                    props.height,
                    this.api.asc_getActiveWorksheetIndex(),
                  )
        ...
        onChangeProtectSheet: function (props) {
          ...
          if (props) {
            ...
            this.toolbar.lockToolbar(
              Common.enumLock["InsertHyperlinks"],
              this._state.wsProps["InsertHyperlinks"],
              { array: [this.toolbar.btnInsertHyperlink] },
            )
            this.appConfig && this.appConfig.isEdit
              ? this.onApiSelectionChanged(this.api.asc_getCellInfo())
              : this.onApiSelectionChangedRestricted(this.api.asc_getCellInfo())
          }
        },
        ...
        onVisibleAreaClose: function (btn) {
          if (this.api) this.api.asc_toggleChangeVisibleAreaOleEditor(false)
        ...
        onApiEndSmartArtPreview: function () {
          ...
          if (this.currentSmartArtMenu) {
            ...
            this.currentSmartArtMenu.cmpEl &&
              this.currentSmartArtMenu.cmpEl.attr("data-preview-loaded", true)
          }
        ...
        onShowProtectedChartPopup: function (value) {
          this.toolbar.lockToolbar(Common.enumLock.externalChartProtected, value, {
            array: [
              this.toolbar.btnPaste,
              this.toolbar.btnInsertFormula,
              this.toolbar.btnDecDecimal,
              this.toolbar.btnIncDecimal,
              this.toolbar.cmbNumberFormat,
              this.toolbar.btnTextDir,
            ],
          })
        },
        ...
        onChartRecommendedClick: function () {
          var recommended = this.api.asc_getRecommendedChartData()
          if (!recommended) {
            Common.UI.warning({
              msg: this.warnNoRecommended,
              maxwidth: 600,
              callback: (btn) => {
                Common.NotificationCenter.trigger("edit:complete", this.toolbar)
              },
          ...
          new SSE.Views.ChartWizardDialog({
            ...
            handler: (result, value) => {
              if (result == "ok") {
                this.api && this.api.asc_addChartSpace(value)
              }
              Common.NotificationCenter.trigger("edit:complete", this.toolbar)
            },
          }).show()
          ...
          if (this.api) {
            if (item.value === Asc.c_oAscFillType.series) {
              new SSE.Views.FillSeriesDialog({
                handler: (result, settings) => {
                  if (result == "ok" && settings) {
                    this.api.asc_FillCells(Asc.c_oAscFillType.series, settings)
                  }
                  Common.NotificationCenter.trigger("edit:complete", this.toolbar)
                },
                props: this.api.asc_GetSeriesSettings(),
              }).show()
            } else {
        ...
        onActiveTab: function (tab) {
          if (tab !== "home") Common.UI.TooltipManager.closeTip("rtlDirection")
          else if (this.toolbar && this.toolbar.btnTextDir && !this.toolbar.btnTextDir.isDisabled())
            setTimeout(() => {
              Common.UI.TooltipManager.showTip("rtlDirection")
            }, 10)

          if (tab !== "home") Common.UI.TooltipManager.closeTip("cellFormat")
          else if (
            this.toolbar &&
            this.toolbar.btnFormatCell &&
            !this.toolbar.btnFormatCell.isDisabled()
          ...
          tab === "data"
            ? Common.UI.TooltipManager.showTip("solver")
            : Common.UI.TooltipManager.closeTip("solver")
          tab === "tabledesign"
            ? Common.UI.TooltipManager.showTip("tableTab")
            : Common.UI.TooltipManager.closeTip("tableTab")
        },
