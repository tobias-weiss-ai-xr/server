    ...
    _.assign(
      {
        ...
        initialize: function () {
          ...
          this.addListeners({
            ...
            Statusbar: {
              "view:compact": (statusbar, state) => {
                const height = Number.parseInt(
                  window.getComputedStyle(document.body).getPropertyValue("--statusbar-height") ||
                    25,
                )
                this.viewport.vlayout.getItem("statusbar").height = state ? height : height * 2
              },
            },
            Toolbar: {
              "render:before": (toolbar) => {
                const config = SSE.getController("Main").appOptions
                ...
                if (this.appConfig?.isEdit && !config.compactHeader && toolbar.btnCollabChanges)
        ...
        onAppShowed: function (config) {
          ...
          if (
            ...
            this.viewport.vlayout.getItem("toolbar").height = 41
            document.body.classList.add("inner-simple-editor")
          }

          if (
            config.twoLevelHeader &&
            ...
            const toolbar = this.viewport.vlayout.getItem("toolbar")
          ...
          $("#toolbar, #app-title").on("mousewheel", (e) => {
            e.preventDefault()
          })
        },
        ...
        onLaunch: function () {
          ...
          this.getApplication()
            .getController("CellEditor")
            .createView("CellEditor", { el: "#cell-editing-box" })
            .render()
        ...
        onThemeChanged: function () {
          if (Common.UI.Themes.available()) {
            ...
            if (!config.isEditDiagram && !config.isEditMailMerge && !config.isEditOle) {
              ...
              $filemenu.css(
                ...
                (Common.UI.LayoutManager.isElementVisible("toolbar")
                  ? _intvars.get("toolbar-height-tabs")
                  : 0) +
                  (config.twoLevelHeader && !config.compactHeader
                    ? _intvars.get("document-title-height")
                    : 0),
              )
            ...
            const height = Number.parseInt(computed_style.getPropertyValue("--statusbar-height"))
            const NoCompact = $(".statusbar").hasClass("no-compact")
            this.viewport.vlayout.getItem("statusbar").height = NoCompact ? height * 2 : height

            const celleditor = this.viewport.celayout.getItem("celleditor")
            const celleditorResizeOptions = this.viewport.getCelleditorResizeOptions()
