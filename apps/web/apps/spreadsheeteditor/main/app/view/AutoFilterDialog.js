...
define([], () => {
  ...
  SSE.Views.DigitalFilterDialog = Common.UI.Window.extend(
    _.extend(
      {
        ...
        render: function () {
          ...
          this.conditions = [
            ...
            { value: Asc.c_oAscCustomAutoFilter.doesNotEqual, displayValue: this.capCondition2 },
            {
              ...
              displayValue: this.type !== "date" ? this.capCondition3 : this.capCondition30,
            },
            {
              value: Asc.c_oAscCustomAutoFilter.isGreaterThanOrEqualTo,
              displayValue: this.type !== "date" ? this.capCondition4 : this.capCondition40,
            },
            {
              value: Asc.c_oAscCustomAutoFilter.isLessThan,
              displayValue: this.type !== "date" ? this.capCondition5 : this.capCondition50,
            },
            {
              value: Asc.c_oAscCustomAutoFilter.isLessThanOrEqualTo,
              displayValue: this.type !== "date" ? this.capCondition6 : this.capCondition60,
            },
          ]
          ...
          const comparator = (item1, item2) => {
            let n1 = item1.get("intval")
            let n2 = item2.get("intval")
            const isN1 = n1 !== undefined
            const isN2 = n2 !== undefined
            if (isN1 !== isN2) return isN1 ? -1 : 1
            !isN1 &&
              (n1 = item1.get("value").toLowerCase()) &&
        ...
        loadDefaults: function () {
          if (
            ...
            const arr = []
            this.properties.asc_getValues().forEach((item) => {
              const value = item.asc_getText()
              if (!_.isEmpty(value)) {
                arr.push({
                  value: value,
                  displayValue: value,
            ...
            const filterObj = this.properties.asc_getFilterObj()
            if (filterObj.asc_getType() === Asc.c_oAscAutoFilterTypes.CustomFilters) {
              const customFilter = filterObj.asc_getFilter()
              const customFilters = customFilter.asc_getCustomFilters()

              customFilter.asc_getAnd() ? this.rbAnd.setValue(true) : this.rbOr.setValue(true)

              this.cmbCondition1.setValue(
                customFilters[0].asc_getOperator() || Asc.c_oAscCustomAutoFilter.equals,
              )
              this.cmbCondition2.setValue(
                customFilters.length > 1 ? customFilters[1].asc_getOperator() || 0 : 0,
              )

              this.cmbValue1.setValue(
                null === customFilters[0].asc_getVal() ? "" : customFilters[0].asc_getVal(),
              )
              this.cmbValue2.setValue(
                customFilters.length > 1
                  ? null === customFilters[1].asc_getVal()
                    ? ""
                    : customFilters[1].asc_getVal()
                  : "",
              )
            }
          }
        ...
        save: function () {
          if (
            ...
            const customFilter = filterObj.asc_getFilter()
            customFilter.asc_setCustomFilters(
              this.cmbCondition2.getValue() === 0
                ? [new Asc.CustomFilter()]
                : [new Asc.CustomFilter(), new Asc.CustomFilter()],
            )
        ...
        showDatePicker: function (btn) {
          ...
          const id = btn.$el.attr("id")
          const index = Number.parseInt(id.slice(-1))
          let $picker
          const cmb = index === 1 ? this.cmbValue1 : this.cmbValue2
          if (!this.datePickers[index]) {
            $picker = $(
              `<div id="date-picker-${index}"><div id="date-picker-control${index}"></div></div>`,
            )
  ...
  SSE.Views.Top10FilterDialog = Common.UI.Window.extend(
    _.extend(
      {
        initialize: function (options) {
          const _options = {}
          ...
          this.template =
            ...
            [
              ...
              `<label id="id-top10-lblby" class="input-group-nr" style="min-width: 40px; text-align: center; display: inline-block; vertical-align: middle;">${this.txtBy}</label>`,
        ...
        getFocusedComponents: function () {
          return [this.cmbType, this.spnCount, this.cmbItem, this.cmbFields].concat(
        ...
        loadDefaults: function () {
          if (this.properties) {
            let isTop10Sum = false
            if (this.type === "value") {
              const pivotObj = this.properties.asc_getPivotObj()
              const idx = pivotObj.asc_getDataFieldIndexFilter()
              const fields = pivotObj.asc_getDataFields()
            ...
            if (filterObj.asc_getType() === Asc.c_oAscAutoFilterTypes.Top10) {
              const top10Filter = filterObj.asc_getFilter()
              const type = top10Filter.asc_getTop()
              const percent = top10Filter.asc_getPercent()
        ...
        save: function () {
          if (this.api && this.properties) {
            ...
            if (this.type === "value") {
              ...
              pivotObj.asc_setDataFieldIndexFilter(
                this.type === "value" ? this.cmbFields.getValue() + 1 : 0,
              )
            }
  ...
  SSE.Views.PivotDigitalFilterDialog = Common.UI.Window.extend(
    _.extend(
      {
        initialize: function (options) {
          const _options = {}
          ...
          this.template =
            ...
            [
              ...
              `<div id="id-cond-digital-combo" class="input-group-nr margin-right-10 display-inline-block-middle" style="width:${this.type === "label" ? 225 : (this.type === "date") ? 200 : 110}px;"></div>`,
              `<div id="id-input-digital-value1" class="display-inline-block-middle" style="width:${this.type === "date" ? 250 : 225}px;"></div>`,
              '<% if (type === "date") {%>',
              ...
              `<label style="display:block;">${this.textUse1}</label>`,
              `<label style="display:block;">${this.textUse2}</label>`,
              "</div>",
              "<% } %>",
              "</div>",
              "</div>",
        ...
        render: function () {
          ...
          this.cmbCondition1.setValue(Asc.c_oAscCustomAutoFilter.equals)
          this.cmbCondition1.on(
            "selected",
            _.bind(function (combo, record) {
              const isBetween = record.value === -2 || record.value === -3
              this.cmbValue2?.setVisible(isBetween)
              this.cmbValue1?.$el.width(isBetween ? 100 : this.type === "date" ? 250 : 225)
              ...
              this.btnDatePicker2?.setVisible(isBetween)
              setTimeout(() => {
                if (this.inputValue) this.inputValue.focus()
                else if (this.cmbValue1) this.cmbValue1.focus()
              }, 10)
            }, this),
          )
        ...
        loadDefaults: function () {
          if (
            ...
            this.cmbCondition1 &&
            this.cmbFields &&
            (this.cmbValue1 || this.inputValue)
          ) {
            const pivotObj = this.properties.asc_getPivotObj()
            const idx = pivotObj.asc_getDataFieldIndexFilter()
            const fields = pivotObj.asc_getDataFields()
            ...
            if (filterObj.asc_getType() === Asc.c_oAscAutoFilterTypes.CustomFilters) {
              const customFilter = filterObj.asc_getFilter()
              const customFilters = customFilter.asc_getCustomFilters()
              let value = customFilters[0].asc_getOperator()

              if (customFilters.length > 1) {
                const isAnd = customFilter.asc_getAnd()
                const isBetween =
                  customFilters[0].asc_getOperator() ===
                    Asc.c_oAscCustomAutoFilter.isGreaterThanOrEqualTo &&
                  customFilters[1].asc_getOperator() ===
                    Asc.c_oAscCustomAutoFilter.isLessThanOrEqualTo
                const isNotBetween =
                  customFilters[0].asc_getOperator() === Asc.c_oAscCustomAutoFilter.isLessThan &&
                  customFilters[1].asc_getOperator() === Asc.c_oAscCustomAutoFilter.isGreaterThan

                if (isAnd && isBetween) value = -2
                else if (!isAnd && isNotBetween) value = -3
              }
              this.cmbCondition1.setValue(value || Asc.c_oAscCustomAutoFilter.equals)
              this.inputValue?.setValue(
                null === customFilters[0].asc_getVal() ? "" : customFilters[0].asc_getVal(),
              )
              this.inputValue?.$el.width(
                value === -2 || value === -3 ? 100 : this.type === "date" ? 250 : 225,
              )
              this.cmbValue1?.setValue(
                null === customFilters[0].asc_getVal() ? "" : customFilters[0].asc_getVal(),
              )
              this.cmbValue1?.$el.width(
                value === -2 || value === -3 ? 100 : this.type === "date" ? 250 : 225,
              )

              this.lblAnd.toggleClass("hidden", !(value === -2 || value === -3))
              this.inputValue2?.setVisible(value === -2 || value === -3)
              this.inputValue2?.setValue(
                customFilters.length > 1
                  ? null === customFilters[1].asc_getVal()
                    ? ""
                    : customFilters[1].asc_getVal()
              ...
              this.cmbValue2?.setVisible(value === -2 || value === -3)
              this.cmbValue2?.setValue(
                customFilters.length > 1
                  ? null === customFilters[1].asc_getVal()
                    ? ""
              ...
              this.btnDatePicker2?.setVisible(value === -2 || value === -3)
            }
          }
        },
        save: function () {
          if (
            ...
            const customFilter = filterObj.asc_getFilter()
            const type = this.cmbCondition1.getValue()
            if (type === -2) {
              ...
              customFilters[0].asc_setVal(
                this.inputValue ? this.inputValue.getValue() : this.cmbValue1.getValue(),
              )
              customFilters[1].asc_setOperator(Asc.c_oAscCustomAutoFilter.isLessThanOrEqualTo)
              customFilters[1].asc_setVal(
                this.inputValue2 ? this.inputValue2.getValue() : this.cmbValue2.getValue(),
              )
            } else if (type === -3) {
              customFilter.asc_setCustomFilters([new Asc.CustomFilter(), new Asc.CustomFilter()])
  ...
  SSE.Views.SortFilterDialog = Common.UI.Window.extend(
    _.extend(
      {
        initialize: function (options) {
          const _options = {}
        ...
        render: function () {
          ...
          this.radioNoSort.on(
            "change",
            _.bind(function (field, newValue) {
        ...
        loadDefaults: function () {
          if (this.properties) {
            let idx = 0
            let sort = Asc.c_oAscSortOptions.Ascending

            const arr = []
            if (this.properties.filter) {
              const filter = this.properties.filter
              const pivotObj = filter.asc_getPivotObj()
              const fields = pivotObj.asc_getDataFields()
              ...
              this.cmbFieldsAsc.setDisabled(
                sort === Asc.c_oAscSortOptions.Descending || sort === null,
              )
              ...
              let pivotObj = this.properties.rowFilter.asc_getPivotObj()
              let fields = pivotObj.asc_getDataFields()
              const idxRow = pivotObj.asc_getDataFieldIndexSorting()
              arr.push({
                value: 0,
                displayValue: fields[0],
                filter: this.properties.rowFilter,
                indexSorting: 1,
              })

              pivotObj = this.properties.colFilter.asc_getPivotObj()
              fields = pivotObj.asc_getDataFields()
              const idxCol = pivotObj.asc_getDataFieldIndexSorting()
              arr.push({
                value: 1,
                displayValue: fields[0],
                filter: this.properties.colFilter,
                indexSorting: 1,
              })
        ...
        save: function () {
          if (this.api && this.properties) {
            const isNoSort = this.radioNoSort.getValue()
            if (isNoSort) {
              const filter = this.properties.filter
              if (filter) {
              ...
              const combo = this.radioAsc.getValue() ? this.cmbFieldsAsc : this.cmbFieldsDesc
              const rec = combo.getSelectedRecord()
              if (rec) {
                const filter = rec.filter
                const pivotObj = filter.asc_getPivotObj()
                pivotObj.asc_setDataFieldIndexSorting(rec.indexSorting)
                filter.asc_setSortState(
                  this.radioAsc.getValue()
  ...
  SSE.Views.AutoFilterDialog = Common.UI.Window.extend(
    _.extend(
      {
        initialize: function (options) {
          const _options = {}
        ...
        render: function () {
          ...
          const $border = this.$window.find(".resize-border")
          ...
          this.miSortCellColor = new Common.UI.MenuItem({
            ...
            menu: new Common.UI.Menu({
              ...
              items: [
                {
                  template: _.template(
                    '<div id="filter-dlg-sort-cells-color" style="max-width: 147px; max-height: 120px;"></div>',
                  ),
                },
              ],
            }),
          ...
          this.miSortFontColor = new Common.UI.MenuItem({
            ...
            menu: new Common.UI.Menu({
              ...
              items: [
                {
                  template: _.template(
                    '<div id="filter-dlg-sort-font-color" style="max-width: 147px; max-height: 120px;"></div>',
                  ),
                },
              ],
            }),
              ...
              items: [
                {
                  ...
                  caption: this.txtEquals,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.CustomFilters,
                },
                {
                  value: Asc.c_oAscCustomAutoFilter.doesNotEqual,
                  caption: this.txtNotEquals,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.CustomFilters,
                },
                { caption: "--" },
                {
                  value: Asc.c_oAscCustomAutoFilter.isGreaterThan,
                  caption: this.txtGreater,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.CustomFilters,
                },
                {
                  value: Asc.c_oAscCustomAutoFilter.isGreaterThanOrEqualTo,
                  caption: this.txtGreaterEquals,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.CustomFilters,
                },
                {
                  value: Asc.c_oAscCustomAutoFilter.isLessThan,
                  caption: this.txtLess,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.CustomFilters,
                },
                {
                  value: Asc.c_oAscCustomAutoFilter.isLessThanOrEqualTo,
                  caption: this.txtLessEquals,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.CustomFilters,
                },
                {
                  value: -2,
                  caption: this.txtBetween,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.CustomFilters,
                },
                { caption: "--" },
                {
                  ...
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.Top10,
                },
                {
                  value: Asc.c_oAscDynamicAutoFilter.aboveAverage,
                  caption: this.txtAboveAve,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                {
                  value: Asc.c_oAscDynamicAutoFilter.belowAverage,
                  caption: this.txtBelowAve,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                { caption: "--" },
          ...
          let items = this.miNumFilter.menu.items
          for (let i = 0; i < items.length; i++) {
            items[i].on(
              "click",
              _.bind(
                items[i].options.type === Asc.c_oAscAutoFilterTypes.CustomFilters
                  ? this.onNumCustomFilterItemClick
                  : items[i].options.type === Asc.c_oAscAutoFilterTypes.DynamicFilter
                    ? this.onNumDynamicFilterItemClick
                    : this.onTop10FilterItemClick,
                this,
              ),
            )
          }
          ...
          this.miTextFilter = new Common.UI.MenuItem({
            ...
            menu: new Common.UI.Menu({
              ...
              items: [
                ...
                { caption: "--" },
                {
                  value: Asc.c_oAscCustomAutoFilter.beginsWith,
                  caption: this.txtBegins,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.CustomFilters,
                },
                {
                  value: Asc.c_oAscCustomAutoFilter.doesNotBeginWith,
                  caption: this.txtNotBegins,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.CustomFilters,
                },
                {
                  value: Asc.c_oAscCustomAutoFilter.endsWith,
                  caption: this.txtEnds,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.CustomFilters,
                },
                {
                  value: Asc.c_oAscCustomAutoFilter.doesNotEndWith,
                  caption: this.txtNotEnds,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.CustomFilters,
                },
                { caption: "--" },
                {
                  value: Asc.c_oAscCustomAutoFilter.contains,
                  caption: this.txtContains,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.CustomFilters,
                },
                {
                  value: Asc.c_oAscCustomAutoFilter.doesNotContain,
                  caption: this.txtNotContains,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.CustomFilters,
                },
                { caption: "--" },
          ...
          this.miDateFilter = new Common.UI.MenuItem({
            ...
            menu: new Common.UI.Menu({
              ...
              items: [
                ...
                {
                  value: Asc.c_oAscCustomAutoFilter.isLessThan,
                  caption: this.txtBefore,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.CustomFilters,
                },
                {
                  value: Asc.c_oAscCustomAutoFilter.isGreaterThan,
                  caption: this.txtAfter,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.CustomFilters,
                },
                {
                ...
                { caption: "--" },
                {
                  value: Asc.c_oAscDynamicAutoFilter.tomorrow,
                  caption: this.txtTomorrow,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                {
                  value: Asc.c_oAscDynamicAutoFilter.today,
                  caption: this.txtToday,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                {
                  value: Asc.c_oAscDynamicAutoFilter.yesterday,
                  caption: this.txtYesterday,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                { caption: "--" },
                {
                  value: Asc.c_oAscDynamicAutoFilter.nextWeek,
                  caption: this.txtNextWeek,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                {
                  value: Asc.c_oAscDynamicAutoFilter.thisWeek,
                  caption: this.txtThisWeek,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                {
                  value: Asc.c_oAscDynamicAutoFilter.lastWeek,
                  caption: this.txtLastWeek,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                { caption: "--" },
                {
                  value: Asc.c_oAscDynamicAutoFilter.nextMonth,
                  caption: this.txtNextMonth,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                {
                  value: Asc.c_oAscDynamicAutoFilter.thisMonth,
                  caption: this.txtThisMonth,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                {
                  value: Asc.c_oAscDynamicAutoFilter.lastMonth,
                  caption: this.txtLastMonth,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                { caption: "--" },
                {
                  value: Asc.c_oAscDynamicAutoFilter.nextQuarter,
                  caption: this.txtNextQuarter,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                {
                  value: Asc.c_oAscDynamicAutoFilter.thisQuarter,
                  caption: this.txtThisQuarter,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                {
                  value: Asc.c_oAscDynamicAutoFilter.lastQuarter,
                  caption: this.txtLastQuarter,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                { caption: "--" },
                {
                  value: Asc.c_oAscDynamicAutoFilter.nextYear,
                  caption: this.txtNextYear,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                {
                  value: Asc.c_oAscDynamicAutoFilter.thisYear,
                  caption: this.txtThisYear,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                {
                  value: Asc.c_oAscDynamicAutoFilter.lastYear,
                  caption: this.txtLastYear,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                { caption: "--" },
                {
                  value: Asc.c_oAscDynamicAutoFilter.yearToDate,
                  caption: this.txtYearToDate,
                  checkable: true,
                  type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                },
                (this.miDatesInThePeriod = new Common.UI.MenuItem({
                  ...
                  menu: new Common.UI.Menu({
                    ...
                    items: [
                      {
                        value: Asc.c_oAscDynamicAutoFilter.q1,
                        caption: this.txtQuarter1,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                      {
                        value: Asc.c_oAscDynamicAutoFilter.q2,
                        caption: this.txtQuarter2,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                      {
                        value: Asc.c_oAscDynamicAutoFilter.q3,
                        caption: this.txtQuarter3,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                      {
                        value: Asc.c_oAscDynamicAutoFilter.q4,
                        caption: this.txtQuarter4,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                      { caption: "--" },
                      {
                        value: Asc.c_oAscDynamicAutoFilter.m1,
                        caption: this.txtJanuary,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                      {
                        value: Asc.c_oAscDynamicAutoFilter.m2,
                        caption: this.txtFebruary,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                      {
                        value: Asc.c_oAscDynamicAutoFilter.m3,
                        caption: this.txtMarch,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                      {
                        value: Asc.c_oAscDynamicAutoFilter.m4,
                        caption: this.txtApril,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                      {
                        value: Asc.c_oAscDynamicAutoFilter.m5,
                        caption: this.txtMay,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                      {
                        value: Asc.c_oAscDynamicAutoFilter.m6,
                        caption: this.txtJune,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                      {
                        value: Asc.c_oAscDynamicAutoFilter.m7,
                        caption: this.txtJuly,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                      {
                        value: Asc.c_oAscDynamicAutoFilter.m8,
                        caption: this.txtAugust,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                      {
                        value: Asc.c_oAscDynamicAutoFilter.m9,
                        caption: this.txtSeptember,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                      {
                        value: Asc.c_oAscDynamicAutoFilter.m10,
                        caption: this.txtOctober,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                      {
                        value: Asc.c_oAscDynamicAutoFilter.m11,
                        caption: this.txtNovember,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                      {
                        value: Asc.c_oAscDynamicAutoFilter.m12,
                        caption: this.txtDecember,
                        checkable: true,
                        type: Asc.c_oAscAutoFilterTypes.DynamicFilter,
                      },
                    ],
          ...
          this.miFilterCellColor = new Common.UI.MenuItem({
            ...
            menu: new Common.UI.Menu({
              ...
              items: [
                {
                  template: _.template(
                    '<div id="filter-dlg-filter-cells-color" style="max-width: 147px; max-height: 120px;"></div>',
                  ),
                },
              ],
            }),
          ...
          this.miFilterFontColor = new Common.UI.MenuItem({
            ...
            menu: new Common.UI.Menu({
              ...
              items: [
                {
                  template: _.template(
                    '<div id="filter-dlg-filter-font-color" style="max-width: 147px; max-height: 120px;"></div>',
                  ),
                },
              ],
            }),
          ...
          const menuContainer = this.$window.find("#menu-container-filters")
          ...
          this.mnuSortColorCellsPicker.on(
            "select",
            _.bind(this.onSortColorSelect, this, Asc.c_oAscSortOptions.ByColorFill),
          )
          ...
          this.mnuSortColorFontPicker.on(
            "select",
            _.bind(this.onSortColorSelect, this, Asc.c_oAscSortOptions.ByColorFont),
          )
          ...
          this.input = new Common.UI.InputField({
            ...
            if (value.length) {
              ...
              this.filter = new RegExp(value, "ig")
            } else {
          ...
          if (this.cells) {
            this.cellsList = new Common.UI.TreeView({
              ...
              enableKeyEvents: true,
              template: _.template(
                ['<div class="treeview inner" style="height:auto;"></div>'].join(""),
              ),
            ...
            this.cellsList.store.comparator = (item1, item2) => {
              ...
              if (isN1 !== isN2) return isN1 ? -1 : 1
              !isN1 &&
                (n1 = item1.get("cellvalue").toLowerCase()) &&
                (n2 = item2.get("cellvalue").toLowerCase())
              if (n1 === n2) return 0
              return n2 === "" || (n1 !== "" && n1 < n2) ? -1 : 1
            }
            this.cellsList.on({
          ...
          const checkDocumentClick = (e) => {
            if (this._skipCheckDocumentClick) return
            if ($(e.target).closest(".filter-dlg").length <= 0) this.close()
          }
          this.on("close", () => {
          ...
          this.cellsList.scroller.update({
            minScrollbarLength: this.cellsList.minScrollbarLength,
            alwaysVisibleY: true,
        ...
        onSortType: function (type) {
          if (this.api && this.configTo) {
            this.api.asc_sortColFilter(
              type,
              this.configTo.asc_getCellId(),
              this.configTo.asc_getDisplayName(),
            )
          }
        ...
        onNumCustomFilterItemClick: function (item) {
          const filterObj = this.configTo.asc_getFilterObj()
          let value1 = ""
          let value2 = ""
          let cond1 = Asc.c_oAscCustomAutoFilter.equals
          let cond2 = 0
          let isAnd = true
          if (filterObj.asc_getType() === Asc.c_oAscAutoFilterTypes.CustomFilters) {
            const customFilter = filterObj.asc_getFilter()
          ...
          if (item.value !== -1) {
            const newCustomFilter = new Asc.CustomFilters()
            newCustomFilter.asc_setCustomFilters(
              item.value === -2 || item.value === -3
                ? [new Asc.CustomFilter(), new Asc.CustomFilter()]
                : [new Asc.CustomFilter()],
            )

            const newCustomFilters = newCustomFilter.asc_getCustomFilters()
            newCustomFilters[0].asc_setOperator(
              item.value === -2
                ? Asc.c_oAscCustomAutoFilter.isGreaterThanOrEqualTo
                : item.value === -3
                  ? Asc.c_oAscCustomAutoFilter.isLessThan
                  : item.value,
            )

            if (item.value === -2) {
              const isBetween =
                cond1 === Asc.c_oAscCustomAutoFilter.isGreaterThanOrEqualTo &&
                cond2 === Asc.c_oAscCustomAutoFilter.isLessThanOrEqualTo
              ...
              const isNotBetween =
                cond1 === Asc.c_oAscCustomAutoFilter.isLessThan &&
                cond2 === Asc.c_oAscCustomAutoFilter.isGreaterThan
          ...
          let dlgDigitalFilter
          if (item.options.pivottype === "label" || item.options.pivottype === "value")
            dlgDigitalFilter = new SSE.Views.PivotDigitalFilterDialog({
              api: this.api,
              type: item.options.pivottype,
            }).on({
              close: () => {
                this.close()
              },
        ...
        onDataFilterMenuClick: function (item) {
          const pivotObj = this.configTo.asc_getPivotObj()
          const filterObj = this.configTo.asc_getFilterObj()
          let value1 = ""
          let value2 = ""
          let cond1 = Asc.c_oAscCustomAutoFilter.equals
          ...
          const dlgDigitalFilter = !pivotObj
            ? new SSE.Views.DigitalFilterDialog({ api: this.api, type: "date" }).on({
                close: () => {
                  this.close()
                },
              })
        ...
        onTop10FilterItemClick: function (item) {
          const dlgTop10Filter = new SSE.Views.Top10FilterDialog({
            api: this.api,
            type: item.options.pivottype,
          }).on({
            close: () => {
        ...
        onLabelFilterMenuClick: function (menu, item) {
          if (
            item.value === Asc.c_oAscCustomAutoFilter.isGreaterThan ||
            item.value === Asc.c_oAscCustomAutoFilter.isGreaterThanOrEqualTo ||
            item.value === Asc.c_oAscCustomAutoFilter.isLessThan ||
            item.value === Asc.c_oAscCustomAutoFilter.isLessThanOrEqualTo ||
            item.value === -2 ||
            item.value === -3
          )
            this.onNumCustomFilterItemClick(item)
          else this.onTextFilterMenuClick(menu, item)
        },
        ...
        onValueFilterMenuClick: function (menu, item) {
          ...
          if (fields.length < 2) {
            ...
            if (item.options.type === Asc.c_oAscAutoFilterTypes.CustomFilters)
              this.onNumCustomFilterItemClick(item)
            else this.onTop10FilterItemClick(item)
          }
        },
        ...
        onFilterColorSelect: function (isCellColor, picker, color) {
          ...
          const colorFilter = filterObj.asc_getFilter()
          colorFilter.asc_setCellColor(isCellColor ? null : false)
          colorFilter.asc_setCColor(
            (isCellColor && color === "transparent") || (!isCellColor && color === "#000000")
              ? null
        ...
        onSortColorSelect: function (type, picker, color) {
          if (this.api && this.configTo) {
            const isCellColor = type === Asc.c_oAscSortOptions.ByColorFill
            this.api.asc_sortColFilter(
              type,
              this.configTo.asc_getCellId(),
              this.configTo.asc_getDisplayName(),
        ...
        updateDateLevelCheck: function (check) {
          let selectAllState = check || this.cells.at(this.cells.length - 1).get("check")
          let i
          let cell
          let state = []
          let lastLevel = -1
          ...
          for (i = this.cells.length - 1; i >= 0; i--) {
            ...
            if (cell.get("isDate")) {
              if (lastLevel === 0) {
                const lastState = state[lastLevel]
                state = [lastState]
              }
              const curLevel = cell.get("level")
              const curCheck = cell.get("check")
              const throughIndex = cell.get("throughIndex")
              if (curLevel > lastLevel) {
                // new branch, new date item
                ...
                if (
                  ...
                  (state[lastLevel] === "indeterminate" || curLevel === 0)
                ) {
                  const groupLevel = state[lastLevel] === "indeterminate" ? lastLevel : 0
                  for (let l = 0; l < curLevelCells.length; l++) {
        ...
        updateCellCheck: function (listView, record) {
          if (record && listView) {
            ...
            const idxs = this.filter ? this.filteredIndexes : this.throughIndexes
            if ("0" === record.get("groupid")) {
              this.cells.each((cell) => {
                if ("2" !== cell.get("groupid")) {
                  cell.set("check", check)
                  if (cell.get("throughIndex") > 1)
                    idxs[Number.parseInt(cell.get("throughIndex"))] = check
                }
              })
            } else {
              record.set("check", check)
              let selectAllState = check
              let i
              let cell
              idxs[Number.parseInt(record.get("throughIndex"))] = check

              if (record.get("isDate")) {
                const level = record.get("level")
                const startInnerItems = record.get("fullDate").slice(0, level + 1)
                for (i = 0; i < this.cells.length; i++) {
                  cell = this.cells.at(i)
                  if ("1" === cell.get("groupid")) {
                    if (cell.get("isDate")) {
                      if (
                        cell.get("level") > level &&
                        startInnerItems.toString() ===
                          cell
                            .get("fullDate")
                            .slice(0, level + 1)
                            .toString()
                      ) {
                        // change checking of inner items
                        cell.set("check", check)
                        idxs[Number.parseInt(cell.get("throughIndex"))] = check
                      }
                    }
            ...
            listView.scroller.update({
              minScrollbarLength: listView.minScrollbarLength,
              alwaysVisibleY: true,
              suppressScrollX: true,
            })
          }
        ...
        onClear: function () {
          if (this.api && this.configTo)
            this.api.asc_clearFilterColumn(
              this.configTo.asc_getCellId(),
              this.configTo.asc_getDisplayName(),
            )
          this.close()
        ...
        _setDefaults: function () {
          ...
          const menuPanel = this.$window.find(".menu-panel")
          const pivotObj = this.configTo.asc_getPivotObj()
          const isPivot = !!pivotObj
          let isValueFilter = false
          ...
          const filterObj = this.configTo.asc_getFilterObj()
          const isCustomFilter = this.initialFilterType === Asc.c_oAscAutoFilterTypes.CustomFilters
          const isDynamicFilter = this.initialFilterType === Asc.c_oAscAutoFilterTypes.DynamicFilter
          const isTop10 = this.initialFilterType === Asc.c_oAscAutoFilterTypes.Top10
          const isTextFilter = this.configTo.asc_getIsTextFilter()
          const isDateFilter = this.configTo.asc_getIsDateFilter()
          const colorsFill = this.configTo.asc_getColorsFill()
          const colorsFont = this.configTo.asc_getColorsFont()
          const sort = this.configTo.asc_getSortState()
          let sortColor = this.configTo.asc_getSortColor()

          if (sortColor)
            sortColor = Common.Utils.ThemeColor.getHexColor(
              sortColor.get_r(),
              sortColor.get_g(),
              sortColor.get_b(),
            ).toLocaleUpperCase()
          ...
          this.miNumFilter.setChecked(
            (isCustomFilter || isDynamicFilter || isTop10) && !isTextFilter,
            true,
          )
          this.miDateFilter.setChecked(
            (isDynamicFilter || isCustomFilter) && isDateFilter && !isValueFilter,
            true,
          )

          this.miValueFilter.setChecked(isPivot && isValueFilter, true)
          this.miLabelFilter.setChecked(
            isPivot && !isValueFilter && (isCustomFilter || isTop10),
            true,
          )
          ...
          if (isCustomFilter) {
            ...
            const customFilters = customFilter.asc_getCustomFilters()
            const isAnd = customFilter.asc_getAnd()
            const cond1 = customFilters[0].asc_getOperator()
            const cond2 = customFilters.length > 1 ? customFilters[1].asc_getOperator() || 0 : 0
            const items = isPivot
              ? isValueFilter
                ? this.miValueFilter.menu.items
                : isDateFilter
                  ? this.miDateFilter.menu.items
                  : this.miLabelFilter.menu.items
              : isTextFilter
                ? this.miTextFilter.menu.items
                : isDateFilter
                  ? this.miDateFilter.menu.items
                  : this.miNumFilter.menu.items
            let isCustomConditions = true

            if (customFilters.length === 1)
              items.forEach((item) => {
                const checked =
                  item.options.type === Asc.c_oAscAutoFilterTypes.CustomFilters &&
                  item.value === cond1
                item.setChecked(checked, true)
                if (checked) isCustomConditions = false
              })
            else if (
              (isPivot || !isTextFilter) &&
              cond1 === Asc.c_oAscCustomAutoFilter.isGreaterThanOrEqualTo &&
              cond2 === Asc.c_oAscCustomAutoFilter.isLessThanOrEqualTo
            ) {
            ...
            if (isCustomConditions) items[items.length - 1].setChecked(true, true)
          } else if (this.initialFilterType === Asc.c_oAscAutoFilterTypes.ColorFilter) {
            const colorFilter = filterObj.asc_getFilter()
            let filterColor = colorFilter.asc_getCColor()
            if (filterColor)
              filterColor = Common.Utils.ThemeColor.getHexColor(
                filterColor.get_r(),
                filterColor.get_g(),
                filterColor.get_b(),
              ).toLocaleUpperCase()
            ...
            const dynType = isDynamicFilter ? filterObj.asc_getFilter().asc_getType() : null
            let items = isDateFilter
              ? this.miDateFilter.menu.items
              : isPivot
                ? this.miValueFilter.menu.items
                : this.miNumFilter.menu.items
            items.forEach((item) => {
              item.setChecked(
                (isDynamicFilter &&
                  item.options.type === Asc.c_oAscAutoFilterTypes.DynamicFilter &&
                  item.value === dynType) ||
                  (isTop10 && item.options.type === Asc.c_oAscAutoFilterTypes.Top10),
                true,
              )
            })
            if (isDateFilter) {
              const item = this.miDateFilter.menu.getChecked()
              if (!item) {
                let isChecked = false
                items = this.miDatesInThePeriod.menu.items
                items.forEach((item) => {
                  const value = item.value === dynType
          ...
          this.btnOk.setDisabled(
            this.initialFilterType !== Asc.c_oAscAutoFilterTypes.Filters &&
              this.initialFilterType !== Asc.c_oAscAutoFilterTypes.None,
          )
          ...
          if (Common.Utils.InternalSettings.get("sse-settings-size-filter-window")) {
            width =
              Common.Utils.InternalSettings.get("sse-settings-size-filter-window")[0] +
              this.menuPanelWidth
          }
          if (isPivot && pivotObj.asc_getIsPageFilter()) {
            this.setResizable(true, [
              this.initConfig.minwidth - this.menuPanelWidth,
              this.initConfig.minheight,
            ])
        ...
        setupDataCells: function () {
          ...
          const idxs = this.filter ? this.filteredIndexes : this.throughIndexes
          let isdate
          const isTimeFormat = this.configTo.asc_getTimeFormat()
          let lastDate
          const monthList = [
            this.txtJanuary,
            this.txtFebruary,
          ...
          this.configTo.asc_getValues().forEach((item) => {
            ...
            if (isDate) {
              isdate = true
              if (this.filter && index === 0) index += 1
              const year = item.asc_getYear()
              const month = item.asc_getMonth()
              const day = item.asc_getDay()
              const hour = item.asc_getHour()
              const minute = item.asc_getMinute()
              const second = item.asc_getSecond()
              if (isTimeFormat) {
              ...
              displayDate.forEach((item, index) => {
            ...
            if (this.filter) {
              const val = !isDate ? value : dateText
              if (null === val.match(this.filter)) {
                applyfilter = false
              }
              idxs[throughIndex] = applyfilter
            } else if (idxs[throughIndex] === undefined) idxs[throughIndex] = item.asc_getVisible()
            ...
            if (applyfilter) {
              if (isDate) {
                ...
                for (let i = dateLevel; i <= lastDateLevel; i++) {
                  index += 1
                  arr.push(
                ...
                arr.push(
                  new Common.UI.TreeViewModel({
                    ...
                    allowSelected: true,
                    cellvalue: value,
                    value: isnumber ? value : value.length > 0 ? value : this.textEmptyItem,
                    intval: isnumber ? Number.parseFloat(value) : undefined,
          ...
          this.cellsList.cmpEl[countYears > 0 ? "addClass" : "removeClass"]("shifted-right")

          if (selectedCells === arr.length) selectAllState = true
          else if (selectedCells > 0) selectAllState = "indeterminate"

          if (this.filter || idxs[0] === undefined) idxs[0] = true
          if (!this.filter || arr.length > 0)
            arr.unshift(
              new Common.UI.TreeViewModel({
                id: ++index,
                selected: false,
                allowSelected: true,
                value: this.filter ? this.textSelectAllResults : this.textSelectAll,
                groupid: "0",
                check: idxs[0],
                throughIndex: 0,
              }),
            )
          if (this.filter && arr.length > 1) {
            if (idxs[1] === undefined) idxs[1] = false
            arr.splice(
              1,
              0,
              new Common.UI.TreeViewModel({
        ...
        testFilter: function () {
          ...
          if (this.cells) {
            if (this.filter && this.filteredIndexes[1]) isValid = true
            else
        ...
        save: function () {
          if (this.api && this.configTo && this.cells && this.filterExcludeCells) {
            const arr = this.configTo.asc_getValues()
            let isValid = false
            if (this.filter && this.filteredIndexes[1]) {
              ...
              this.cells.each((cell) => {
                if ("1" === cell.get("groupid")) {
                  arr[Number.parseInt(cell.get("throughIndex")) - 2].asc_setVisible(
                    cell.get("check"),
                  )
                }
              })
              ...
              arr.forEach((item, index) => {
                ...
                if (item.asc_getIsDateFormat()) {
                  const group = this.dateTimeGroup[index + 2]
                  let value
                  switch (group) {
                    case 0:
