    ...
    _.extend(
      {
        ...
        render: function () {
          ...
          this.btnNamedRanges = new Common.UI.Button({
            ...
            menu: new Common.UI.Menu({
              ...
              maxHeight: 250,
              items: [{ caption: this.textManager, value: "manager" }, { caption: "--" }],
            }),
          })
          ...
          this.$cellname.on("focus", (e) => {
            const txt = this.$cellname[0]
          ...
          this.$btnfunc.on("app:scaling", (e, info) => {
        ...
        cellNameDisabled: function (disabled) {
          if (this.mode?.isEditDiagram) disabled = true
          disabled
            ? this.$cellname.attr("disabled", "disabled")
            : this.$cellname.removeAttr("disabled")
          this.btnNamedRanges.setDisabled(disabled)
        },

        applyScaling: function (ratio) {
          if (ratio > 2 && !this.$btnfunc.find("svg.icon").length) {
            const icon_name = "btn-function"
            const svg_icon =
              '<svg class="icon uni-scale"><use href="#%iconname"></use></svg>'.replace(
                "%iconname",
                icon_name,
              )
        ...
        cellEditorTextChange: function () {
          ...
          const cellcontent = this.$cellcontent[0]

          if (cellcontent.clientHeight !== cellcontent.scrollHeight) {
            if (!this._isScrollShow) {
              this._isScrollShow = true
              const scrollBarWidth = cellcontent.offsetWidth - cellcontent.clientWidth
