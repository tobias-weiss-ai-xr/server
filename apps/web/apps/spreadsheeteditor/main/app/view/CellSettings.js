...
define([
  ...
  SSE.Views.CellSettings = Backbone.View.extend(
    _.extend(
      {
        el: "#id-cell-settings",
        ...
        onBtnBordersClick: function (btn, eOpts) {
          if (this.api) {
            const new_borders = []
            const bordersWidth = this.BorderType
            let bordersColor
        ...
        createDelayedControls: function () {
          ...
          this.btnDirection = new Common.UI.Button({
            ...
            menu: new Common.UI.Menu({
              ...
              items: [
                {
                  template: _.template(
                    '<div id="id-cell-menu-direction" style="width: 175px; margin: 0 5px;"></div>',
                  ),
                },
              ],
            }),
          ...
          const global_hatch_menu_map = [
            -1, -1, -1, -1, -1, -1, -1, -1, -1, 8, 9, 10, 11, -1, -1, -1, 21, -1, -1, -1, -1, 20,
            22, 23, -1, -1, 27, 28, 29, 30, -1, -1, 33, -1, 35, -1, -1, -1, -1, -1, 41, -1, 43, -1,
            -1, 46, -1, -1, -1, -1,
          ]
          ...
          const _arrBorderPosition = [
            [
              Asc.c_oAscBorderOptions.Left,
              "toolbar__icon btn-border-left",
              "cell-button-border-left",
              this.tipLeft,
              "bottom",
            ],
            [
              Asc.c_oAscBorderOptions.InnerV,
              "toolbar__icon btn-border-insidevert",
              "cell-button-border-inner-vert",
              this.tipInnerVert,
              "bottom",
            ],
            [
              Asc.c_oAscBorderOptions.Right,
              "toolbar__icon btn-border-right",
              "cell-button-border-right",
              this.tipRight,
              "bottom",
            ],
            [
              Asc.c_oAscBorderOptions.Top,
              "toolbar__icon btn-border-top",
              "cell-button-border-top",
              this.tipTop,
              "bottom",
            ],
            [
              Asc.c_oAscBorderOptions.InnerH,
              "toolbar__icon btn-border-insidehor",
              "cell-button-border-inner-hor",
              this.tipInnerHor,
              "bottom",
            ],
            [
              Asc.c_oAscBorderOptions.Bottom,
              "toolbar__icon btn-border-bottom",
              "cell-button-border-bottom",
              this.tipBottom,
              "bottom",
            ],
            [
              Asc.c_oAscBorderOptions.DiagU,
              "toolbar__icon btn-border-diagup",
              "cell-button-border-diagu",
              this.tipDiagU,
              "top",
            ],
            [
              Asc.c_oAscBorderOptions.DiagD,
              "toolbar__icon btn-border-diagdown",
              "cell-button-border-diagd",
              this.tipDiagD,
              "top",
            ],
            [
              "inner",
              "toolbar__icon btn-border-inside",
              "cell-button-border-inner",
              this.tipInner,
              "top",
            ],
            [
              "outer",
              "toolbar__icon btn-border-out",
              "cell-button-border-outer",
              this.tipOuter,
              "top",
            ],
        ...
        ChangeSettings: function (props) {
          ...
          if (props) {
            ...
            const xfs = props.asc_getXfs()
            let value = xfs.asc_getAngle()
            if (
              Math.abs(this._state.CellAngle - value) > 0.1 ||
              (this._state.CellAngle === undefined && this._state.CellAngle !== value)
            ) {
            ...
            if (
              Math.abs(this._state.CellIndent - value) > 0.1 ||
              (this._state.CellIndent === undefined && this._state.CellIndent !== value)
            ) {
            ...
            if (this.fill) {
              ...
              if (this.pattern === null && this.gradient === null) {
                ...
                if (this.pattern.asc_getType() === -1) {
                  const color = this.pattern.asc_getFgColor()
                  if (color) {
                  ...
                  let color = this.pattern.asc_getFgColor()
                  if (color) {
                    if (color.asc_getType() === Asc.c_oAscColor.COLOR_TYPE_SCHEME) {
                      this.FGColor = {
                        Value: 1,
                  ...
                  } else this.FGColor = { Value: 1, Color: { color: "000000", effectId: 24 } }
                ...
                const gradFillType = this.gradient.asc_getType()
                if (
                  this._state.GradFillType !== gradFillType ||
                ...
                if (this.GradFillType === Asc.c_oAscFillGradType.GRAD_LINEAR) {
                  const value = this.gradient.asc_getDegree()
                ...
                const length = gradientStops.length
                this.sldrGradient.setThumbs(length)
                this.GradColor.colors.length = 0
                this.GradColor.values.length = 0
                gradientStops.forEach((color) => {
                  const clr = color.asc_getColor()
                  const position = color.asc_getPosition()
                  this.GradColor.colors.push(
                    clr.asc_getType() === Asc.c_oAscColor.COLOR_TYPE_SCHEME
                      ? {
                          color: Common.Utils.ThemeColor.getHexColor(
                            clr.asc_getR(),
                            clr.asc_getG(),
                  ...
                  this.GradColor.values.push(position * 100)
                })
                ...
                for (let index = 0; index < length; index++) {
                  ...
                  arrGrCollors.push(
                    `${this.sldrGradient.getColorValue(index)} ${this.sldrGradient.getValue(index)}%`,
                  )
                }
              ...
              if (this._state.FillType !== this.OriginalFillType) {
                this.cmbFillSrc.setValue(
                  this.OriginalFillType === null ? "" : this.OriginalFillType,
                )
              ...
              let type1 = typeof this.CellColor.Color
              let type2 = typeof this._state.CellColor
              if (
                type1 !== type2 ||
                (type1 === "object" &&
                  (this.CellColor.Color.effectValue !== this._state.CellColor.effectValue ||
                    this._state.CellColor.color.indexOf(this.CellColor.Color) < 0)) ||
                (type1 !== "object" &&
                  this._state.CellColor !== undefined &&
                  this._state.CellColor.indexOf(this.CellColor.Color) < 0)
              ) {
              ...
              // Gradient colors
              const gradColor = this.GradColor.colors[this.GradColor.currentIdx]
              ...
              if (
                type1 !== type2 ||
                (type1 === "object" &&
                  (gradColor.effectValue !== this._state.GradColor.effectValue ||
                    this._state.GradColor.color.indexOf(gradColor.color) < 0)) ||
        ...
        onFillSrcSelect: function (combo, record) {
          ...
          switch (record.value) {
            case Asc.c_oAscFill.FILL_TYPE_SOLID:
              ...
              if (!this._noApply) {
                if (this.pattern == null) this.pattern = new Asc.asc_CPatternFill()
                this.pattern.asc_setType(-1)
                this.pattern.asc_setFgColor(
                  Common.Utils.ThemeColor.getRgbColor(
                    this.CellColor.Color === "transparent"
            ...
            case Asc.c_oAscFill.FILL_TYPE_GRAD:
              ...
              if (!this._noApply) {
                if (this.gradient == null) this.gradient = new Asc.asc_CGradientFill()
                ...
                if (this.OriginalFillType !== Asc.c_oAscFill.FILL_TYPE_GRAD) {
                  ...
                  if (this.GradColor.colors.length === 2) {
                    const HexColor0 = Common.Utils.ThemeColor.getRgbColor(this.GradColor.colors[0])
                      .get_color()
                      .get_hex()
                  ...
                  const arrGradStop = []
                  this.GradColor.colors.forEach((item, index) => {
                    const gradientStop = new Asc.asc_CGradientStop()
                    gradientStop.asc_setColor(
                      Common.Utils.ThemeColor.getRgbColor(this.GradColor.colors[index]),
                    )
                    gradientStop.asc_setPosition(this.GradColor.values[index] / 100)
          ...
          if (this.api) {
            if (this.GradFillType === Asc.c_oAscFillGradType.GRAD_LINEAR) {
              if (this.gradient == null) {
                ...
                this.gradient.asc_setType(this.GradFillType)
                const arrGradStop = []
                this.GradColor.values.forEach((item, index) => {
                  const gradientStop = new Asc.asc_CGradientStop()
                  gradientStop.asc_setColor(
                    Common.Utils.ThemeColor.getRgbColor(this.GradColor.colors[index]),
                  )
