...
define([
  ...
  SSE.Views.ChartSettingsDlg = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 340,
        ...
        render: function () {
          ...
          if (this.isDiagramMode) {
            this.btnChartType = new Common.UI.Button({
              ...
              menu: new Common.UI.Menu({
                ...
                items: [
                  {
                    template: _.template(
                      '<div id="id-chart-dlg-menu-type" class="menu-insertchart"></div>',
                    ),
                  },
                ],
              }),
            ...
            this.mnuChartTypePicker.on(
              "item:click",
              _.bind(this.onSelectType, this, this.btnChartType),
            )
          }
          ...
          this.btnSparkType = new Common.UI.Button({
            ...
            menu: new Common.UI.Menu({
              ...
              items: [
                {
                  template: _.template(
                    '<div id="id-spark-dlg-menu-type" class="menu-insertchart"></div>',
                  ),
                },
              ],
            }),
          ...
          this.btnSparkType.on("render:after", (btn) => {
          ...
          this.cmbSparkMinType = new Common.UI.ComboBox({
            ...
            "selected",
            _.bind(function (combo, record) {
              this.spnSparkMinValue.setDisabled(
                record.value !== Asc.c_oAscSparklineAxisMinMax.Custom,
              )
              if (this._changedProps) {
              ...
              if (
                record.value === Asc.c_oAscSparklineAxisMinMax.Custom &&
                _.isEmpty(this.spnSparkMinValue.getValue())
              )
          ...
          this.spnSparkMinValue = new Common.UI.MetricSpinner({
            ...
            "change",
            _.bind(function (field, newValue, oldValue) {
              if (this._changedProps) {
          ...
          this.cmbSparkMaxType = new Common.UI.ComboBox({
            ...
            "selected",
            _.bind(function (combo, record) {
              this.spnSparkMaxValue.setDisabled(
                record.value !== Asc.c_oAscSparklineAxisMinMax.Custom,
              )
              if (this._changedProps) {
              ...
              if (
                record.value === Asc.c_oAscSparklineAxisMinMax.Custom &&
                _.isEmpty(this.spnSparkMaxValue.getValue())
              )
        ...
        onCategoryClick: function (btn, index) {
          ...
          setTimeout(() => {
            switch (index) {
              ...
              case 6:
                this.btnSparkType.focus()
                break
              case 7:
                this.chShowAxis.focus()
                break
              case 8:
                this.radioTwoCell.focus()
                break
              case 9:
        ...
        updateSparkStyles: function (styles) {
          if (styles && styles.length > 1) {
            const picker = this.cmbSparkStyle.menuPicker
            const stylesStore = picker.store
        ...
        onSelectSparkType: function (btn, picker, itemView, record) {
          ...
          let changed = false
          const value = this.cmbEmptyCells.getValue()
          if (rawData.type !== Asc.c_oAscSparklineType.Line && this._arrEmptyCells.length > 2) {
            this._arrEmptyCells.pop()
            changed = true
          } else if (
            rawData.type === Asc.c_oAscSparklineType.Line &&
            this._arrEmptyCells.length < 3
          ) {
            this._arrEmptyCells.push({
              value: Asc.c_oAscEDispBlanksAs.Span,
          ...
          if (props) {
            ...
            if (this.isChart) {
              ...
              Common.UI.FocusManager.add(this, [
                this.btnSparkType,
                this.cmbEmptyCells,
                this.chShowEmpty, // 6 tab
                this.chShowAxis,
                this.chReverse,
                this.cmbSparkMinType,
                this.spnSparkMinValue,
                this.cmbSparkMaxType,
                this.spnSparkMaxValue,
              ]) // 7 tab

              this._state.SparkType = props.asc_getType()
              const record = this.mnuSparkTypePicker.store.findWhere({
                type: this._state.SparkType,
              })
              this.mnuSparkTypePicker.selectRecord(record, true)
              if (record) this.btnSparkType.setIconCls(`svgicon chart-${record.get("iconCls")}`)
              else this.btnSparkType.setIconCls("svgicon")

              this.updateSparkStyles(
                this.sparklineStyles ? this.sparklineStyles : props.asc_getStyles(),
              )

              if (this._state.SparkType !== Asc.c_oAscSparklineType.Line) this._arrEmptyCells.pop()
              this.cmbEmptyCells.setData(this._arrEmptyCells)

              const value = props.asc_getDisplayEmpty()
              this.cmbEmptyCells.setValue(
                this._state.SparkType !== Asc.c_oAscSparklineType.Line &&
                  value === Asc.c_oAscEDispBlanksAs.Span
                  ? this.textEmptyLine
                  : value,
              )
              ...
              this.spnSparkMinValue.setDisabled(
                props.asc_getMinAxisType() !== Asc.c_oAscSparklineAxisMinMax.Custom,
              )
              this.spnSparkMaxValue.setDisabled(
                props.asc_getMaxAxisType() !== Asc.c_oAscSparklineAxisMinMax.Custom,
              )
              this.spnSparkMinValue.setValue(
                props.asc_getManualMin() !== null ? props.asc_getManualMin() : "",
                true,
              )
              this.spnSparkMaxValue.setValue(
                props.asc_getManualMax() !== null ? props.asc_getManualMax() : "",
                true,
              )
        ...
        getSettings: function () {
          ...
          if (this.isChart) {
            let type = this.currentChartType
            if (this.isDiagramMode) {
              const rec = this.mnuChartTypePicker.getSelectedRec()
        ...
        onRadioSnapChange: function (field, newValue, eOpts) {
          if (newValue) {
            if (!this._changedImageProps) this._changedImageProps = new Asc.asc_CImgProperty()
        ...
        isRangeValid: function () {
          if (this.isChart) {
            let isvalid
            const range = this.chartSettings.getRange()
            if (!_.isEmpty(range)) {
              let type = this.currentChartType
              if (this.isDiagramMode) {
                const rec = this.mnuChartTypePicker.getSelectedRec()
        ...
        onSelectSparkData: function () {
          if (this.api) {
            const handlerDlg = (dlg, result) => {
              if (result === "ok") {
                this.sparkDataRangeValid = dlg.getSettings()
                this.txtSparkDataRange.setValue(this.sparkDataRangeValid)
                this.txtSparkDataRange.checkValidate()
              }
            }
            ...
            win.setSettings({
              api: this.api,
              range:
                !_.isEmpty(this.txtSparkDataRange.getValue()) &&
                this.txtSparkDataRange.checkValidate() === true
                  ? this.txtSparkDataRange.getValue()
                  : this.sparkDataRangeValid,
              type: Asc.c_oAscSelectionDialogType.Chart,
            })
          }
        ...
        onSelectLocationData: function () {
          if (this.api) {
            const handlerDlg = (dlg, result) => {
              if (result === "ok") {
                this.dataLocationRangeValid = dlg.getSettings()
                this.txtSparkDataLocation.setValue(this.dataLocationRangeValid)
                this.txtSparkDataLocation.checkValidate()
              }
            }
            ...
            win.setSettings({
              api: this.api,
              range:
                !_.isEmpty(this.txtSparkDataLocation.getValue()) &&
                this.txtSparkDataLocation.checkValidate() === true
                  ? this.txtSparkDataLocation.getValue()
                  : this.dataLocationRangeValid,
        ...
        openFormat: function (index, btn) {
          ...
          let value = this.api.asc_getLocale()
          const lang = Common.Utils.InternalSettings.get("sse-config-lang")
          !value &&
            (value = lang
              ? Number.parseInt(Common.util.LanguageInfo.getLocalLanguageCode(lang))
              : 0x0409)
