...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  ...
  SSE.Views.ChartWizardDialog = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 455,
        ...
        initialize: function (options) {
          const charts = []
          const groups = []
          const chartData = Common.define.chartData.getChartData()
          this._currentTabIndex = undefined
          this._arrSeriesGroups = []
          this._arrSeriesType = []
          if (options.props.recommended) {
            for (const type in options.props.recommended) {
              if (Number.isNaN(Number.parseInt(type))) continue
              for (let i = 0; i < chartData.length; i++) {
                if (Number.parseInt(type) === chartData[i].type) {
          ...
          charts.length > 0 &&
            groups.push({
              panelId: "id-chart-recommended-rec",
              panelCaption: this.textRecommended,
              groupId: "rec",
              charts: charts,
              categoryIcon: "toolbar__icon " + "btn-chartcategory-recommended",
              categoryCls: "png-icons",
            })
          Common.define.chartData.getChartGroupData().forEach((group) => {
            const charts = []
            chartData.forEach((item) => {
              if (group.id === item.group) {
                charts.push(item)
                options.type === item.type && (this._currentTabIndex = groups.length)
              }
            })
            groups.push({
              panelId: `id-chart-recommended-${group.id}`,
              panelCaption: group.caption,
              groupId: group.id,
              charts: charts,
              categoryIcon: `toolbar__icon btn-chartcategory-${charts[0].iconCls}`,
              categoryCls: "png-icons",
            })
            group.id !== "menu-chart-group-combo" &&
              group.id !== "menu-chart-group-stock" &&
              this._arrSeriesGroups.push(group)
          })
        ...
        onCategoryClick: function (btn, index) {
          ...
          const buttons = item.chartButtons
          if (buttons.length > 0) {
            buttons[this._openedButtonIndex].toggle(true)
            setTimeout(() => {
              buttons[this._openedButtonIndex].focus()
            }, 10)
          }
        },
        ...
        renderChartTab: function (tab) {
          ...
          $window.find(`#${tab.panelId}`).append(
            $(
              _.template(this.chartTabTemplate)({
                group: tab,
                scope: this,
              }),
            ),
          ...
          tab.charts.forEach((chart, index) => {
            ...
            btn.on("toggle", (cmp, pressed) => {
              if (pressed) {
                $window.find(`#id-${tab.groupId}-lbl`).text(chart.tip)
                this._currentChartType = chart.type
                this.updatePreview()
              }
            })
            ...
            chart.type === this.options.type &&
              tab.groupId !== "rec" &&
              (this._openedButtonIndex = index)
            Common.UI.FocusManager.insert(this, btn, -1 * this.getFooterButtons().length)
          })
          ...
          tab.listPreview = new Common.UI.DataView({
            ...
            itemTemplate: _.template(
              ['<div class="style" id="<%= id %>" style="width:210px; height:120px;"></div>'].join(
                "",
              ),
            ),
            tabindex: 1,
          ...
          tab.listPreview.on({
            "item:select": (dataView, itemView, record) => {
              if (record) {
                this._currentChartSpace = record.get("data")
                if (this._currentTabSettings.groupId === "menu-chart-group-combo") {
                  this.updateSeriesList(this._currentChartSpace.asc_getSeries())
                }
              }
            },
            "item:add": (dataView, itemView, record) => {
              record?.get("data").updateView(record.get("id"))
            },
            entervalue: _.bind(this.onPrimary, this),
          })
          Common.UI.FocusManager.insert(this, tab.listPreview, -1 * this.getFooterButtons().length)

          if (tab.groupId === "menu-chart-group-combo") this.renderSeries(tab)
        ...
        addControls: function (listView, itemView, item) {
          ...
          const cmpEl = this.seriesList.cmpEl.find(`#chart-recommend-item-${i}`)
          const series = item.get("series")
          series.asc_drawPreviewRect(`chart-recommend-series-preview-${i}`)
          const combo = this.initSeriesType(`#chart-recommend-cmb-series-${i}`, i, item)
        ...
        fillPreviews: function (tab) {
          this._currentPreviews =
            tab.groupId === "rec" ? this.options.props.recommended : this.allCharts
          this._currentPreviewSize =
            tab.groupId === "menu-chart-group-combo" ? [280, 160] : [430, 258]
          this._currentTabSettings = tab
        },

        updatePreview: function (seriesIndex) {
          let charts = this._currentPreviews[this._currentChartType]
          if (charts === undefined && this._currentTabSettings.groupId !== "rec") {
            charts = this._currentPreviews[this._currentChartType] = this.api.asc_getChartData(
              this._currentChartType,
            )
          }
          this._currentTabSettings.divErrorEl.toggleClass("hidden", typeof charts !== "number")
          if (typeof charts === "number") {
            // show error
            let msg = ""
            switch (charts) {
          ...
          this._currentTabSettings.divPreviewEl.toggleClass(
            "hidden",
            !charts || charts.length !== 1,
          )
          this._currentTabSettings.groupId === "menu-chart-group-combo" &&
          ...
          if (charts) {
            if (charts.length === 1) {
              ...
              const store = this._currentTabSettings.listPreview.store
              const idx =
                seriesIndex !== undefined
                  ? _.indexOf(store.models, this._currentTabSettings.listPreview.getSelectedRec())
                  : 0
              const arr = []
            ...
            charts.length > 0 &&
              this._currentTabSettings.groupId === "menu-chart-group-combo" &&
              this.updateSeriesList(this._currentChartSpace.asc_getSeries(), seriesIndex)
          }
        ...
        errorStockChart:
          "Incorrect row order. To build a stock chart place the data on the sheet in the following order: opening price, max price, min price, closing price.",
