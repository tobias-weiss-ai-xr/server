...
define([], () => {
  SSE.Views.ConstraintDialog = Common.UI.Window.extend(
    _.extend(
      {
        ...
        render: function () {
          ...
          this.txtCellRef.on(
            "button:click",
            _.bind(this.onSelectData, this, Asc.c_oAscSelectionDialogType.Chart),
          )
          ...
          this.txtConstraint.on(
            "button:click",
            _.bind(this.onSelectData, this, Asc.c_oAscSelectionDialogType.Solver_Constraint),
          )
          ...
          this.cmbOperator = new Common.UI.ComboBox({
            ...
            data: [
              {
                value: AscCommonExcel.c_oAscOperator["<="],
                displayValue: this.constraintOperator[AscCommonExcel.c_oAscOperator["<="]],
              },
              {
                value: AscCommonExcel.c_oAscOperator["="],
                displayValue: this.constraintOperator[AscCommonExcel.c_oAscOperator["="]],
        ...
        getFocusedComponents: function () {
          return [this.txtCellRef, this.cmbOperator, this.txtConstraint].concat(
        ...
        _handleInput: function (state) {
          if (this.options.handler) {
            if (state === "ok" || state === "add") {
              if (this.isRangeValid() !== true) return
            }
        ...
        setSettings: function (settings) {
          if (settings) {
            ...
            const item = this.cmbOperator.store.findWhere({ value: settings.operator })
            if (item) this.cmbOperator.setValue(settings.operator)
              ...
              this.cmbOperator.setData([
                ...
                },
                {
                  ...
                  displayValue: this.constraintOperator[settings.operator],
                },
              ])
        ...
        getSettings: function () {
          return {
            cellRef: this.txtCellRef.getValue(),
            operator: this.cmbOperator.getValue(),
            constraint: this.txtConstraint.getValue(),
          }
        },
        ...
        isRangeValid: function () {
          ...
          value = this.api.asc_checkDataRange(
            Asc.c_oAscSelectionDialogType.Solver_CellReference,
            this.txtCellRef.getValue(),
            true,
          )
          ...
          // } else {
          value = this.api.asc_checkDataRange(
            Asc.c_oAscSelectionDialogType.Solver_Constraint,
            this.txtConstraint.getValue(),
            true,
          )
          if (value !== Asc.c_oAscError.ID.No) {
