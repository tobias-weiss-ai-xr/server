...
define([
  ...
  SSE.Views.FileMenu = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#file-menu-panel",
        rendered: false,
        ...
        applyMode: function () {
          ...
          if (!this.panels) {
            this.panels = {
              opts: new SSE.Views.FileMenuPanels.MainSettingsGeneral({ menu: this }).render(
                this.$el.find("#panel-settings"),
              ),
              info: new SSE.Views.FileMenuPanels.DocumentInfo({ menu: this }).render(
                this.$el.find("#panel-info"),
              ),
              rights: new SSE.Views.FileMenuPanels.DocumentRights({ menu: this }).render(
          ...
          const isBCSupport = window.AscDesktopEditor
          ...
          if (this.mode.isSignatureSupport || this.mode.isPasswordSupport) {
            !this.panels.protect &&
              (this.panels.protect = new SSE.Views.FileMenuPanels.ProtectDoc({
                menu: this,
              }).render())
          ...
          if (this.mode.canDownload) {
            !this.panels.saveas &&
              (this.panels.saveas = new SSE.Views.FileMenuPanels.ViewSaveAs({
                menu: this,
          ...
          if (this.mode.canDownload && (this.mode.canRequestSaveAs || this.mode.saveAsUrl)) {
            !this.panels["save-copy"] &&
              (this.panels["save-copy"] = new SSE.Views.FileMenuPanels.ViewSaveCopy({
                menu: this,
                fileType: this.document.fileType,
              }).render())
          }
          ...
          if (this.mode.disableEditing !== undefined) {
            ...
            this.mode.disableEditing = undefined
          }

          if (this.mode.canPrint) {
            const printPanel = SSE.getController("Print").getView("PrintWithPreview")
        ...
        SetDisabled: function (disable, options) {
          ...
          options?.protect && _btn_protect.setDisabled(disable || !this.mode.isEdit)
        },
