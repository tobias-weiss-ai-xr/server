...
define([], () => {
  ...
  SSE.Views.FileMenuPanels.ViewSaveAs = Common.UI.BaseView.extend({
    ...
    initialize: function (options) {
      ...
      Common.NotificationCenter.on({
        "window:resize": _.bind(function () {
          const divided = Common.Utils.innerWidth() >= this.maxWidth
    ...
    render: function () {
      this.$el.html(
        this.template({
          ...
          header: /*this.textDownloadAs*/ Common.Locale.get("btnDownloadCaption", {
            name: "SSE.Views.FileMenu",
            default: this.textDownloadAs,
          }),
        }),
      )
      ...
      const itemWidth = 70 + 24 // width + margin
      let maxCount = 0
      ...
      this.maxWidth =
        $("#file-menu-panel .panel-menu").outerWidth() + 20 + 10 + itemWidth * maxCount // menu + left padding + margin
    ...
    onFormatClick: function (e) {
      const type = e.currentTarget.attributes.format
      if (!_.isUndefined(type) && this.menu) {
        this.menu.fireEvent("saveas:format", [this.menu, Number.parseInt(type.value)])
      }
    },
  ...
  SSE.Views.FileMenuPanels.ViewSaveCopy = Common.UI.BaseView.extend({
    ...
    render: function () {
      this.$el.html(
        this.template({
          rows: this.formats,
          fileType: (this.fileType || "xlsx").toLowerCase(),
          header: /*this.textSaveCopyAs*/ Common.Locale.get("btnSaveCopyAsCaption", {
            name: "SSE.Views.FileMenu",
            default: this.textSaveCopyAs,
          }),
        }),
      )
      $(".btn-doc-format", this.el).on("click", _.bind(this.onFormatClick, this))
    ...
    onFormatClick: function (e) {
      const type = e.currentTarget.attributes.format
      const ext = e.currentTarget.attributes["format-ext"]
      if (!_.isUndefined(type) && !_.isUndefined(ext) && this.menu) {
        this.menu.fireEvent("savecopy:format", [this.menu, Number.parseInt(type.value), ext.value])
      }
    },
  ...
  SSE.Views.FileMenuPanels.MainSettingsGeneral = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-settings",
        menu: undefined,
        ...
        render: function (node) {
          const $markup = $(this.template({ scope: this }))
          ...
          ;(Common.Utils.isIE || (Common.Utils.isMac && Common.Utils.isGecko)) &&
            this.chUseAltKey.$el.parent().parent().hide()
          ...
          }).on("change", (field, newValue, eOpts) => {
            newValue && this.chAutosave.setValue(1)
          })
          this.rbCoAuthModeFast.$el.parent().on("click", () => {
            this.rbCoAuthModeFast.setValue(true)
          })
          ...
          this.rbCoAuthModeStrict.$el.parent().on("click", () => {
            this.rbCoAuthModeStrict.setValue(true)
          })
          /** coauthoring end **/
          ...
          this.cmbFontRender = new Common.UI.ComboBox({
            ...
            data: [
              {
                value: Asc.c_oAscFontRenderingModeType.hintingAndSubpixeling,
          ...
          this.chAutosave = new Common.UI.CheckBox({
            ...
            if (field.getValue() !== "checked" && this.rbCoAuthModeFast.getValue()) {
              this.rbCoAuthModeStrict.setValue(true)
            }
          })
          ...
          const formula_arr = []
          SSE.Collections.formulasLangs.forEach((item) => {
            let str = item.replace(/[\-_]/, "")
            str = str.charAt(0).toUpperCase() + str.substring(1, str.length)
            formula_arr.push({
              value: item,
              displayValue: this[`txt${str}lang`] || this[`txt${str}`],
              exampleValue: this[`txtExample${str}`] || this.txtExampleEn,
            })
          })
          formula_arr.sort((a, b) => {
          ...
          this.cmbRegSettings = new Common.UI.ComboBox({
            ...
            const isBaseSettings = this.chSeparator.getValue()
            if (isBaseSettings === "checked") {
              this.inputDecimalSeparator.setValue(
                this.api.asc_getDecimalSeparator(record.value),
                true,
              )
              this.inputThousandsSeparator.setValue(
                this.api.asc_getGroupSeparator(record.value),
                true,
              )
            }
          })
          if (this.cmbRegSettings.scroller)
            this.cmbRegSettings.scroller.update({ alwaysVisibleY: true })
          ...
          this.chSeparator = new Common.UI.CheckBox({
            ...
            _.bind(function (field, newValue, oldValue, eOpts) {
              ...
              if (checked) {
                const lang = this.cmbRegSettings.getValue()
                const decimal = this.api.asc_getDecimalSeparator(
                  _.isNumber(lang) ? lang : undefined,
                )
                const group = this.api.asc_getGroupSeparator(_.isNumber(lang) ? lang : undefined)
          ...
          const $decimalSeparatorInput = this.inputDecimalSeparator.$el.find("input")
          ...
          const $thousandsSeparatorInput = this.inputThousandsSeparator.$el.find("input")
          ...
          $markup.find(".btn.primary").each((index, el) => {
            new Common.UI.Button({
              el: $(el),
            }).on("click", _.bind(this.applySettings, this))
          })
          ...
          this.inputMaxChange = new Common.UI.InputField({
            ...
            validation: (value) =>
              !_.isEmpty(value) &&
              (!/^(\d*(\.|,)?\d+)$|^(\d+(\.|,)?\d*)$/.test(value) ||
                Number.isNaN(Common.Utils.String.parseFloat(value)))
                ? this.txtErrorNumber
                : true,
          })
          ...
          Common.NotificationCenter.on({
            "window:resize": () => {
              this.isVisible() && this.updateScroller()
            },
          })
        ...
        updateScroller: function () {
          if (this.scroller) {
            ...
            const scrolled =
              this.$el.height() < this.pnlTable.parent().height() + 25 + this.pnlApply.height()
        ...
        setMode: function (mode) {
          ...
          const fast_coauth = Common.Utils.InternalSettings.get("sse-settings-coauthmode")

          $("tr.editsave", this.el)[mode.isEdit || mode.canForcesave ? "show" : "hide"]()
          $("tr.collaboration", this.el)[mode.canCoAuthoring ? "show" : "hide"]()
          $("tr.edit", this.el)[mode.isEdit ? "show" : "hide"]()
          $("tr.autosave", this.el)[
            mode.isEdit && (mode.canChangeCoAuthoring || !fast_coauth) ? "show" : "hide"
          ]()
          ...
          $("tr.coauth.changes", this.el)[
            mode.isEdit && !mode.isOffline && mode.canCoAuthoring && mode.canChangeCoAuthoring
              ? "show"
              : "hide"
          ]()
          $("tr.live-viewer", this.el)[
            mode.canLiveView && !mode.isOffline && mode.canChangeCoAuthoring ? "show" : "hide"
          ]()
          $("tr.macros", this.el)[
            mode.customization && mode.customization.macros === false ? "hide" : "show"
          ]()
          $("tr.quick-print", this.el)[
            mode.canQuickPrint && !(mode.compactHeader && mode.isEdit) ? "show" : "hide"
          ]()
          $("tr.tab-background", this.el)[
            !Common.Utils.isIE && Common.UI.FeaturesManager.canChange("tabBackground", true)
              ? "show"
              : "hide"
          ]()
          $("tr.tab-style", this.el)[
            !Common.Utils.isIE &&
            !Common.Controllers.Desktop.isWinXp() &&
            Common.UI.FeaturesManager.canChange("tabStyle", true)
              ? "show"
              : "hide"
          ]()
          ...
          $("tr.spellcheck", this.el)[
            Common.UI.FeaturesManager.canChange("spellcheck") && mode.isEdit ? "show" : "hide"
          ]()
        ...
        updateSettings: function () {
          let value = Common.Utils.InternalSettings.get("sse-settings-zoom")
          value =
            value !== null
              ? Number.parseInt(value)
              : this.mode.customization?.zoom
                ? Number.parseInt(this.mode.customization.zoom)
                : 100
          let item = this.cmbZoom.store.findWhere({ value: value })
          this.cmbZoom.setValue(
            item ? Number.parseInt(item.get("value")) : value > 0 ? `${value}%` : 100,
          )
          this.chUseAltKey.setValue(
            Common.Utils.InternalSettings.get("sse-settings-show-alt-hints"),
          )
          this.chScreenReader.setValue(
            Common.Utils.InternalSettings.get("app-settings-screen-reader"),
          )

          /** coauthoring begin **/
          this.chLiveComment.setValue(Common.Utils.InternalSettings.get("sse-settings-livecomment"))
          this.chResolvedComment.setValue(
            Common.Utils.InternalSettings.get("sse-settings-resolvedcomment"),
          )
          this.chR1C1Style.setValue(Common.Utils.InternalSettings.get("sse-settings-r1c1"))
          this.chSmoothScroll.setValue(
            !Common.Utils.InternalSettings.get("sse-settings-smooth-scroll"),
          )
          ...
          item = this.cmbFontRender.store.findWhere({ value: Number.parseInt(value) })
          this.cmbFontRender.setValue(
            item ? item.get("value") : Asc.c_oAscFontRenderingModeType.hintingAndSubpixeling,
          )
          ...
          this.cmbUnit.setValue(
            item ? Number.parseInt(item.get("value")) : Common.Utils.Metric.getDefaultMetric(),
          )
          ...
          if (value) {
            ...
            this.cmbRegSettings.setValue(
              item ? item.get("value") : Common.util.LanguageInfo.getLocalLanguageName(value)[1],
            )
          ...
          const isBaseSettings = Common.Utils.InternalSettings.get(
            "sse-settings-use-base-separator",
          )
          ...
          if (!isBaseSettings) {
            decimal =
              Common.Utils.InternalSettings.get("sse-settings-decimal-separator") ||
              this.api.asc_getDecimalSeparator()
            group =
              Common.Utils.InternalSettings.get("sse-settings-group-separator") ||
          ...
          item = this.cmbMacros.store.findWhere({
            value: Common.Utils.InternalSettings.get("sse-macros-mode"),
          })
          this.cmbMacros.setValue(item ? item.get("value") : 0)

          this.chPaste.setValue(Common.Utils.InternalSettings.get("sse-settings-paste-button"))
          this.chTooltip.setValue(
            Common.Utils.InternalSettings.get("sse-settings-function-tooltip"),
          )
          ...
          this.chTabBack.setValue(
            Common.Utils.InternalSettings.get("settings-tab-background") === "toolbar",
          )
          ...
          if (Common.UI.FeaturesManager.canChange("spellcheck") && this.mode.isEdit) {
            const arrLang = SSE.getController("Spellcheck").loadLanguages()
            const defaultShortName = "en-US"
            const allLangs = arrLang[0]
            const langs = arrLang[1]
            const change = arrLang[2]
            ...
            if (langs && langs.length > 0) {
              ...
              let item = this.cmbDictionaryLanguage.store.findWhere({ value: value })
              if (!item && allLangs[value]) {
                value = allLangs[value][0].split(/[\-\_]/)[0]
                item = this.cmbDictionaryLanguage.store.find(
                  (model) => model.get("shortName").indexOf(value) === 0,
                )
                if (!item)
              ...
              this.cmbDictionaryLanguage.setValue(
                Common.util.LanguageInfo.getLocalLanguageName(value)[1],
              )
              this.cmbDictionaryLanguage.setDisabled(true)
            }

            this.chIgnoreUppercase.setValue(
              Common.Utils.InternalSettings.get("sse-spellcheck-ignore-uppercase-words"),
            )
            this.chIgnoreNumbers.setValue(
              Common.Utils.InternalSettings.get("sse-spellcheck-ignore-numbers-words"),
            )
        ...
        applySettings: function () {
          ...
          Common.localStorage.setItem(
            "sse-settings-show-alt-hints",
            this.chUseAltKey.isChecked() ? 1 : 0,
          )
          Common.Utils.InternalSettings.set(
            "sse-settings-show-alt-hints",
            Common.localStorage.getBool("sse-settings-show-alt-hints"),
          )
          ...
          Common.localStorage.setItem(
            ...
            this.chScreenReader.isChecked() ? 1 : 0,
          )
          /** coauthoring begin **/
          Common.localStorage.setItem(
            "sse-settings-livecomment",
            this.chLiveComment.isChecked() ? 1 : 0,
          )
          Common.localStorage.setItem(
            "sse-settings-resolvedcomment",
            this.chResolvedComment.isChecked() ? 1 : 0,
          )
          if (
            this.mode.isEdit &&
            !this.mode.isOffline &&
            this.mode.canCoAuthoring &&
            this.mode.canChangeCoAuthoring
          )
            Common.localStorage.setItem(
              "sse-settings-coauthmode",
              this.rbCoAuthModeFast.getValue() ? 1 : 0,
            )
          else if (
            this.mode.canLiveView &&
            !this.mode.isOffline &&
            this.mode.canChangeCoAuthoring
          ) {
            // viewer
            Common.localStorage.setItem(
              "sse-settings-view-coauthmode",
              this.chLiveViewer.isChecked() ? 1 : 0,
            )
          }
          /** coauthoring end **/
          Common.localStorage.setItem("sse-settings-r1c1", this.chR1C1Style.isChecked() ? 1 : 0)
          Common.localStorage.setItem(
            "sse-settings-smooth-scroll",
            this.chSmoothScroll.isChecked() ? 0 : 1,
          )
          Common.localStorage.setItem("sse-settings-fontrender", this.cmbFontRender.getValue())
          const item = this.cmbFontRender.store.findWhere({ value: "custom" })
          Common.localStorage.setItem(
            "sse-settings-cachemode",
            item && !item.get("checked") ? 0 : 1,
          )
          Common.localStorage.setItem("sse-settings-unit", this.cmbUnit.getValue())
          if (
            this.mode.isEdit &&
            (this.mode.canChangeCoAuthoring ||
              !Common.Utils.InternalSettings.get("sse-settings-coauthmode"))
          )
            Common.localStorage.setItem(
              "sse-settings-autosave",
              this.chAutosave.isChecked() ? 1 : 0,
            )
          if (this.mode.canForcesave)
            Common.localStorage.setItem(
              "sse-settings-forcesave",
              this.chForcesave.isChecked() ? 1 : 0,
            )
          ...
          let value
          const isChecked = this.chSeparator.isChecked()
          if (!isChecked) {
          ...
          Common.localStorage.setItem(
            "sse-settings-function-tooltip",
            this.chTooltip.isChecked() ? 1 : 0,
          )
          Common.Utils.InternalSettings.set(
            "sse-settings-function-tooltip",
            this.chTooltip.isChecked() ? 1 : 0,
          ...
          this.mode.isEdit &&
            Common.localStorage.setBool("sse-settings-def-sheet-rtl", this.rbSheetRtl.getValue())
          ...
          if (Common.UI.FeaturesManager.canChange("spellcheck") && this.mode.isEdit) {
            let value = this.chIgnoreUppercase.isChecked()
          ...
          if (this.mode.isEdit) {
            ...
            if (value) {
              ...
              this.inputMaxChange.getValue() &&
                value.asc_setMaxChange(
                  Common.Utils.String.parseFloat(this.inputMaxChange.getValue()),
                )
        ...
        updateRegionalExample: function (landId) {
          if (this.api) {
            ...
            if (landId) {
              ...
              const arr = this.api.asc_getFormatCells(info) // all formats
        ...
        onFontRenderSelected: function (combo, record) {
          if (record.value === "custom") {
            const item = combo.store.findWhere({ value: "custom" })
            item?.set("checked", !record.checked)
        ...
        autoCorrect: function () {
          if (this.dlgAutoCorrect?.isVisible()) return
        ...
        customizeQuickAccess: function () {
          if (this.dlgQuickAccess?.isVisible()) return
          this.dlgQuickAccess = new Common.Views.CustomizeQuickAccessDialog({
            showSave:
              this.mode.showSaveButton && Common.UI.LayoutManager.isElementVisible("header-save"),
  ...
  SSE.Views.FileMenuPanels.CreateNew = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-createnew",
        menu: undefined,
        ...
        render: function () {
          ...
          const docs = (this.blank ? [{ title: this.txtBlank }] : []).concat(this.docs)
          const thumbsElm = this.$el.find(".thumb-wrap, .blank-document")
          _.each(thumbsElm, (tmb, index) => {
        ...
        _onBlankDocument: function () {
          if (this.menu) this.menu.fireEvent("create:new", [this.menu, "blank"])
        },

        _onDocumentTemplate: function (e) {
          if (this.menu)
            this.menu.fireEvent("create:new", [
              this.menu,
              e.currentTarget.attributes.template.value,
            ])
        },
  ...
  SSE.Views.FileMenuPanels.DocumentInfo = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-info",
        menu: undefined,
        ...
        render: function (node) {
          ...
          // edited info
          const keyDownBefore = (input, e) => {
            if (e.keyCode === Common.UI.Keys.ESC) {
              const newVal = input._input.val()
              const oldVal = input.getValue()
              if (newVal !== oldVal) {
          ...
          this.inputTitle = new Common.UI.InputField({
            ...
            .on("keydown:before", keyDownBefore)
            .on("changed:after", (_, newValue) => {
              this.coreProps.asc_putTitle(newValue)
          ...
          this.inputTags = new Common.UI.InputField({
            ...
            .on("keydown:before", keyDownBefore)
            .on("changed:after", (_, newValue) => {
              this.coreProps.asc_putKeywords(newValue)
              this.api.asc_setCoreProps(this.coreProps)
            })
          ...
          this.inputSubject = new Common.UI.InputField({
            ...
            .on("keydown:before", keyDownBefore)
            .on("changed:after", (_, newValue) => {
              this.coreProps.asc_putSubject(newValue)
              this.api.asc_setCoreProps(this.coreProps)
            })
          ...
          this.inputComment = new Common.UI.InputField({
            ...
            .on("keydown:before", keyDownBefore)
            .on("changed:after", (_, newValue) => {
              this.coreProps.asc_putDescription(newValue)
              this.api.asc_setCoreProps(this.coreProps)
            })
          ...
          this.authorTpl =
            '<tr><td><div style="display: inline-block;width: 200px;"><input type="text" spellcheck="false" class="form-control" readonly="true" value="{0}" ></div><div class="tool close img-colored" data-hint="2" data-hint-direction="right" data-hint-offset="small"></div></td></tr>'

          this.tblAuthor.on("click", (e) => {
            const btn = $markup.find(e.target)
            if (btn.hasClass("close") && !btn.hasClass("disabled")) {
              const el = btn.closest("tr")
              const idx = this.tblAuthor.find("tr").index(el)
              el.remove()
          ...
          this.inputAuthor = new Common.UI.InputField({
            ...
            .on("changed:after", (input, newValue, oldValue, e) => {
              ...
              if (!!val && val !== oldValue.trim()) {
                const isFromApply = e?.relatedTarget && e.relatedTarget.id === "fminfo-btn-apply"
                val.split(/\s*[,;]\s*/).forEach((item) => {
                  const str = item.trim()
                  if (str) {
                    ...
                    if (!isFromApply) {
                      ...
                      this.trAuthor.before(div)
                      this.updateScroller()
                    }
                  }
                })
                !isFromApply && this.inputAuthor.setValue("")
              }
        ...
        updateInfo: function (doc) {
          ...
          if (doc.info) {
            // server info
            if (doc.info.folder) this.lblPlacement.text(doc.info.folder)
            visible =
              this._ShowHideInfoItem(
                this.lblPlacement,
                doc.info.folder !== undefined && doc.info.folder !== null,
              ) || visible
            let value = doc.info.owner
            if (value) this.lblOwner.text(value)
            visible = this._ShowHideInfoItem(this.lblOwner, !!value) || visible
            value = doc.info.uploaded
            if (value) this.lblUploaded.text(value)
            visible = this._ShowHideInfoItem(this.lblUploaded, !!value) || visible
          } else this._ShowHideDocInfo(false)
          $("tr.divider.general", this.el)[visible ? "show" : "hide"]()

          let appname = this.api ? this.api.asc_getAppProps() : null
          if (appname) {
            appname =
              (appname.asc_getApplication() || "") +
              (appname.asc_getAppVersion() ? " " : "") +
              (appname.asc_getAppVersion() || "")
          ...
          if (this.coreProps) {
            const value = this.coreProps.asc_getCreated()
        ...
        updateFileInfo: function () {
          if (!this.rendered) return
          const props = this.api ? this.api.asc_getCoreProps() : null
          let value
          ...
          if (props) {
            ...
            if (value) this.lblModifyBy.text(AscCommon.UserInfoParser.getParsedName(value))
            ...
            value?.split(/\s*[,;]\s*/).forEach((item) => {
              const div = $(
                Common.Utils.String.format(this.authorTpl, Common.Utils.String.htmlEncode(item)),
              )
              this.trAuthor.before(div)
              this.authors.push(item)
            })
        ...
        tplCustomProperty: function (name, type, value) {
          ...
          return `<tr data-custom-property><td class="left"><label>${Common.Utils.String.htmlEncode(name)}</label></td><td class="right"><div class="custom-property-wrapper"><input type="text" spellcheck="false" class="form-control" readonly style="width: 200px;" value="${value}"><div class="tool close img-colored" data-hint="2" data-hint-direction="right" data-hint-offset="small"></div></div></td></tr>`
        },

        dateToString: function (value, hideTime) {
          let text = ""
          if (value) {
            let lang = (this.mode.lang || "en").replace("_", "-").toLowerCase()
            try {
              if (lang === "ar-SA".toLowerCase()) lang = `${lang}-u-nu-latn-ca-gregory`
              text =
                value.toLocaleString(lang, { year: "numeric", month: "2-digit", day: "2-digit" }) +
                (!hideTime ? ` ${value.toLocaleString(lang, { timeStyle: "short" })}` : "")
            } catch (e) {
              lang = "en"
              text =
                value.toLocaleString(lang, { year: "numeric", month: "2-digit", day: "2-digit" }) +
        ...
        renderCustomProperties: function () {
          ...
          const properties = this.api.asc_getAllCustomProperties()
          _.each(
            properties,
            _.bind(function (prop, idx) {
              const name = prop.asc_getName()
              const type = prop.asc_getType()
              const value = prop.asc_getValue()
              const $propertyEl = $(this.tplCustomProperty(name, type, value))

              $("tbody.properties-tab").append($propertyEl)

              $propertyEl.on("click", (e) => {
                if ($propertyEl.find("div.disabled").length) {
                  return
                }

                const btn = $propertyEl.find(e.target)
                if (btn.hasClass("close")) {
                  this.api.asc_removeCustomProperty(idx)
                  this.renderCustomProperties()
                } else if (btn.hasClass("form-control")) {
                  new Common.Views.DocumentPropertyDialog({
                    title: this.txtDocumentPropertyUpdateTitle,
                    lang: this.mode.lang,
                    defaultValue: {
                      name: name,
                      type: type,
                    ...
                    nameValidator: (newName) => {
                      if (
                        newName !== name &&
                        _.some(properties, (prop) => prop.asc_getName() === newName)
                      ) {
                        return this.txtPropertyTitleConflictError
                      }

                      return true
                    },
                    handler: (result, name, type, value) => {
                      if (result === "ok") {
                        this.api.asc_modifyCustomProperty(idx, name, type, value)
                        this.renderCustomProperties()
                      }
                    },
                  }).show()
        ...
        setDisabledCustomProperties: (disable) => {
          _.each($("tr[data-custom-property]"), (prop) => {
            $(prop)
              .find("div.custom-property-wrapper")
        ...
        onAddPropertyClick: function () {
          new Common.Views.DocumentPropertyDialog({
            ...
            nameValidator: (newName) => {
              const properties = this.api.asc_getAllCustomProperties()
              if (_.some(properties, (prop) => prop.asc_getName() === newName)) {
                return this.txtPropertyTitleConflictError
              }
            ...
            handler: (result, name, type, value) => {
              if (result === "ok") {
                this.api.asc_addCustomProperty(name, type, value)
                this.renderCustomProperties()
              }
            },
          }).show()
        ...
        SetDisabled: function () {
          const disable = !this.mode.isEdit || this._locked
  ...
  SSE.Views.FileMenuPanels.DocumentRights = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-rights",
        menu: undefined,
        ...
        render: function (node) {
          ...
          Common.NotificationCenter.on(
            "collaboration:sharingupdate",
            this.updateSharingSettings.bind(this),
          )
          Common.NotificationCenter.on(
            "collaboration:sharingdeny",
            this.onLostEditRights.bind(this),
          )
        ...
        updateInfo: function (doc) {
          ...
          if (doc.info) {
            ...
            this._ShowHideInfoItem(
              "rights",
              doc.info.sharingSettings !== undefined &&
                doc.info.sharingSettings !== null &&
                doc.info.sharingSettings.length > 0,
            )
            this._ShowHideInfoItem(
              "edit-rights",
              ((!!this.sharingSettingsUrl && this.sharingSettingsUrl.length) ||
                this.mode.canRequestSharingSettings) &&
        ...
        changeAccessRights: (btn, event, opts) => {
        ...
        updateSharingSettings: function (rights) {
          this._ShowHideInfoItem(
            "rights",
            this.doc.info.sharingSettings !== undefined &&
              this.doc.info.sharingSettings !== null &&
              this.doc.info.sharingSettings.length > 0,
          )
  ...
  SSE.Views.FileMenuPanels.Help = Common.UI.BaseView.extend({
      ...
      this.en_data = [
        {
          ...
          name: "Introducing Spreadsheet Editor user interface",
        ...
        },
        {
          ...
          name: "Collaborative spreadsheet editing",
          headername: "Spreadsheet co-editing",
        },
        {
        ...
        },
        {
          ...
          name: "Advanced settings of Spreadsheet Editor",
        },
    ...
    render: function () {
      ...
      this.viewHelpPicker.on("item:add", (dataview, itemview, record) => {
        if (record.has("headername")) {
          $(itemview.el).before(`<div class="header-name">${record.get("headername")}</div>`)
        }
      })

      this.viewHelpPicker.on("item:select", (dataview, itemview, record) => {
        this.onSelectItem(record.get("src"))
      })
      ...
      if (
        ...
        this.iFrame.onload = () => {
          try {
            this.findUrl(this.iFrame.contentWindow.location.href)
          } catch (e) {}
        }

      Common.Gateway.on("internalcommand", (data) => {
        if (data.type === "help:hyperlink") {
          this.findUrl(data.data)
        }
      })
    ...
    setLangConfig: function (lang) {
      const store = this.viewHelpPicker.store
      if (lang) {
        lang = lang.split(/[\-\_]/)[0]
        const config = {
          dataType: "json",
          error: () => {
            if (this.urlPref.indexOf("resources/help/{{DEFAULT_LANG}}/") < 0) {
              this.urlPref = "resources/help/{{DEFAULT_LANG}}/"
              store.url = "resources/help/{{DEFAULT_LANG}}/Contents.json"
              store.fetch(config)
            } else {
              this.urlPref = "resources/help/{{DEFAULT_LANG}}/"
              store.reset(this.en_data)
            }
          },
          success: () => {
            const rec = this.openUrl
              ? store.find((record) => this.openUrl.indexOf(record.get("src")) >= 0)
              : store.at(0)
            if (rec) {
              this.viewHelpPicker.selectRecord(rec, true)
              this.viewHelpPicker.scrollToRecord(rec)
            }
            this.onSelectItem(this.openUrl ? this.openUrl : rec.get("src"))
          },
        }

        if (Common.Controllers.Desktop.isActive()) {
          if (!Common.Controllers.Desktop.isHelpAvailable()) {
            this.noHelpContents = true
            this.iFrame.src = "../../common/main/resources/help/download.html"
          } else {
            this.urlPref = `${Common.Controllers.Desktop.helpUrl()}/`
            store.url = `${this.urlPref}Contents.json`
            store.fetch(config)
          }
        } else {
    ...
    findUrl: function (src) {
      const rec = this.viewHelpPicker.store.find((record) => src.indexOf(record.get("src")) >= 0)
      if (rec) {
        this.viewHelpPicker.selectRecord(rec, true)
        this.viewHelpPicker.scrollToRecord(rec)
      }
  ...
  SSE.Views.FileMenuPanels.ProtectDoc = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-protect",
        menu: undefined,
        ...
        initialize: function (options) {
          ...
          this.templateSignature = _.template(
            [
              ...
              '<div class="buttons">',
              `<label class="link signature-view-link margin-right-20" data-hint="2" data-hint-direction="bottom" data-hint-offset="medium">${this.txtView}</label>`,
              `<label class="link signature-edit-link <% if (!hasSigned) { %>hidden<% } %>" data-hint="2" data-hint-direction="bottom" data-hint-offset="medium">${this.txtEdit}</label>`,
              "</div>",
              "</div>",
            ].join(""),
          )
        ...
        render: function () {
          ...
          const protection = SSE.getController("Common.Controllers.Protection").getView()
        ...
        onEdit: function () {
          ...
          Common.UI.warning({
            ...
            callback: (btn) => {
              if (btn === "ok") {
                this.api.asc_RemoveAllSignatures()
              }
            },
          })
        },
        ...
        onView: function () {
          this.menu?.hide()
          SSE.getController("RightMenu").rightmenu.SetActivePane(
            Common.Utils.documentSettingsType.Signature,
            true,
          )
        },

        updateSignatures: function () {
          const requested = this.api.asc_getRequestSignatures()
          const valid = this.api.asc_getSignatures()
          const hasRequested = requested && requested.length > 0
          let hasValid = false
          let hasInvalid = false

          _.each(valid, (item, index) => {
            if (item.asc_getValid() === 0) hasValid = true
          ...
          let tipText = hasInvalid ? this.txtSignedInvalid : hasValid ? this.txtSigned : ""
          if (hasRequested)
            tipText = this.txtRequestedSignatures + (tipText !== "" ? "<br><br>" : "") + tipText

          this.cntSignatureView.html(
          ...
          const isAddedSignature = this.btnAddInvisibleSign.$el.find("button").hasClass("hidden")
          this.cntAddSignature.toggleClass("hidden", isAddedSignature)
          this.cntAddedSignature.toggleClass("hidden", !isAddedSignature)
        },

        updateEncrypt: function () {
          const isProtected = this.btnAddPwd.$el.find("button").hasClass("hidden")
        ...
        txtSigned:
          "Valid signatures has been added to the workbook. The workbook is protected from editing.",
        txtSignedInvalid:
          "Some of the digital signatures in workbook are invalid or could not be verified. The workbook is protected from editing.",
        txtRequestedSignatures: "This workbook needs to be signed.",
        notcriticalErrorTitle: "Warning",
        txtEditWarning:
          "Editing will remove the signatures from the workbook.<br>Are you sure you want to continue?",
        ...
        txtAddSignature:
          "Ensure the integrity of the spreadsheet by adding an<br>invisible digital signature",
        txtAddedSignature:
          "Valid signatures have been added to the spreadsheet.<br>The spreadsheet is protected from editing.",
      },
      SSE.Views.FileMenuPanels.ProtectDoc || {},
    ),
  )

  SSE.Views.PrintWithPreview = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#panel-print",
        menu: undefined,
        ...
        initialize: function (options) {
          ...
          this._defaultPaperSizeList = [
            {
              value: "215.9|279.4",
              displayValue: ["US Letter", "21,59", "27,94", "cm"],
              caption: "US Letter",
              size: [215.9, 279.4],
            },
            {
              value: "215.9|355.6",
              displayValue: ["US Legal", "21,59", "35,56", "cm"],
              caption: "US Legal",
              size: [215.9, 355.6],
            },
            {
              value: "210|297",
              displayValue: ["A4", "21", "29,7", "cm"],
              caption: "A4",
              size: [210, 297],
            },
            {
              value: "148|210",
              displayValue: ["A5", "14,8", "21", "cm"],
              caption: "A5",
              size: [148, 210],
            },
            {
              value: "176|250",
              displayValue: ["B5", "17,6", "25", "cm"],
              caption: "B5",
              size: [176, 250],
            },
            {
              value: "104.8|241.3",
              displayValue: ["Envelope #10", "10,48", "24,13", "cm"],
              caption: "Envelope #10",
              size: [104.8, 241.3],
            },
            {
              value: "110|220",
              displayValue: ["Envelope DL", "11", "22", "cm"],
              caption: "Envelope DL",
              size: [110, 220],
            },
            {
              value: "279.4|431.8",
              displayValue: ["Tabloid", "27,94", "43,18", "cm"],
              caption: "Tabloid",
              size: [279.4, 431.8],
            },
            {
              value: "297|420",
              displayValue: ["A3", "29,7", "42", "cm"],
              caption: "A3",
              size: [297, 420],
            },
            {
              value: "296.9|457.2",
              displayValue: ["Tabloid Oversize", "29,69", "45,72", "cm"],
              caption: "Tabloid Oversize",
              size: [296.9, 457.2],
            },
            {
              value: "196.8|273",
              displayValue: ["ROC 16K", "19,68", "27,3", "cm"],
              caption: "ROC 16K",
              size: [196.8, 273],
            },
            {
              value: "120|235",
              displayValue: ["Envelope Choukei 3", "12", "23,5", "cm"],
              caption: "Envelope Choukei 3",
              size: [120, 235],
            },
            {
              value: "305|487",
              displayValue: ["Super B/A3", "30,5", "48,7", "cm"],
              caption: "Super B/A3",
              size: [305, 487],
            },
          ]
        ...
        render: function (node) {
          const $markup = $(this.template({ scope: this, isRTL: Common.UI.isRTL() }))
          ...
          this.cmbSides = new Common.UI.ComboBox({
            ...
            data: [
              ...
              {
                value: "both-long",
                displayValue: this.txtBothSides,
                descValue: this.txtBothSidesLongDesc,
              },
              {
                value: "both-short",
                displayValue: this.txtBothSides,
                descValue: this.txtBothSidesShortDesc,
              },
            ],
            itemsTemplate: _.template(
              [
          ...
          const paperSizeItemsTemplate = !Common.UI.isRTL()
            ? _.template(
                [
                  "<% _.each(items, function(item) { %>",
          ...
          this.cmbPaperSize = new Common.UI.ComboBoxCustom({
            ...
            updateFormControl: function (record, customValue) {
              ...
              if (record || customValue) {
                const displayValue = customValue ? customValue : record.get("displayValue")
                if (typeof displayValue === "string") {
                  formcontrol[0].innerHTML = displayValue
                } else {
                  if (!Common.UI.isRTL()) {
                    formcontrol[0].innerHTML = `${displayValue[0]} (${displayValue[1]} ${displayValue[3]} x ${displayValue[2]} ${displayValue[3]})`
                  } else {
                    formcontrol[0].innerHTML = `<span dir="ltr">(<span dir="rtl">${displayValue[2]} ${displayValue[3]}</span><span> x </span><span dir="rtl">${displayValue[1]} ${displayValue[3]}</span>)<span> ${displayValue[0]}</span></span>`
                  }
                }
              } else formcontrol[0].innerHTML = ""
            },
          })
          ...
          if (this.mode.isDesktopApp) {
            ...
            for (let i = 0; i < 2; i++) {
              ...
              const tableTemplate = _.template(table)({ scope: this, index: i })
              $($markup.find(".footer")[i]).html(tableTemplate)
              this.btnsPrintPDF.push(
                new Common.UI.Button({
          ...
          this.txtNumberPage = new Common.UI.InputField({
            ...
            validation: (value) => {
              if (/(^[0-9]+$)/.test(value)) {
                ...
                if (undefined !== value && value > 0 && value <= this.pageCount) return true
              }
          ...
          this.$el.on(
            "click",
            "#print-header-footer-settings > span",
            _.bind(this.openHeaderSettings, this),
          )
        ...
        updateCmbPrinter: function (currentPrinter, printers, isWaitingForPrinters) {
          const cmbPrinterOptions = (printers || []).map((printer) => ({
            value: printer.name,
          ...
          if (
            !selectedPrinter &&
            _.some(cmbPrinterOptions, (option) => option.value === currentPrinter)
          ) {
        ...
        setCmbSidesOptions: function (isDuplexSupported) {
          let cmbValue = this.cmbSides.getValue()
          const list = [
            { value: "one", displayValue: this.txtOneSide, descValue: this.txtOneSideDesc },
          ]
        ...
        setCmbPaperSizeOptions: function (paperSizeList) {
          ...
          if (paperSizeList && paperSizeList.length > 0) {
            resultList = paperSizeList.map((item, index) => ({
              value: index,
          ...
          const prevSelectedOption = this.cmbPaperSize.store.findWhere({
          ...
          function findOptionBySize(list, width, height) {
            return _.find(
              list,
              (option) =>
                Math.abs(option.size[0] - width) < 0.1 && Math.abs(option.size[1] - height) < 0.1,
            )
          }
            ...
            if (prevSelectedOption) {
              newSelectedOption = {
                ...
                value: [
                  this.txtCustom,
                  Number.parseFloat(
                    Common.Utils.Metric.fnRecalcFromMM(prevSelectedOption.get("size")[0]).toFixed(
                      2,
                    ),
                  ),
                  Number.parseFloat(
                    Common.Utils.Metric.fnRecalcFromMM(prevSelectedOption.get("size")[1]).toFixed(
                      2,
                    ),
                  ),
              ...
              const _h = this._originalPageSize ? this._originalPageSize.h : 297
              ...
              if (!newSelectedOption) {
                newSelectedOption = {
                  ...
                  value: [
                    this.txtCustom,
                    Number.parseFloat(Common.Utils.Metric.fnRecalcFromMM(_w).toFixed(2)),
                    Number.parseFloat(Common.Utils.Metric.fnRecalcFromMM(_h).toFixed(2)),
                    Common.Utils.Metric.getCurrentMetricName(),
                  ],
                }
              }
        ...
        updateScroller: function () {
          if (this.scroller) {
            ...
            const scrolled =
              this.$el.height() <
              this.pnlTable.height() +
                25 +
                this.pnlApply.height() +
                this.$el.find(".main-header").outerHeight(true)
            this.pnlApply.toggleClass("hidden", !scrolled)
            this.trApply.toggleClass("hidden", scrolled)
            this.pnlSettings.css("overflow", scrolled ? "hidden" : "visible")
            this.scroller.update()
        ...
        updatePaperSizeMetricUnit: function () {
          ...
          const store = this.cmbPaperSize.store
          for (let i = 0; i < store.length; i++) {
            ...
            item.set("displayValue", [
              item.get("caption"),
              Number.parseFloat(Common.Utils.Metric.fnRecalcFromMM(pagewidth).toFixed(2)),
              Number.parseFloat(Common.Utils.Metric.fnRecalcFromMM(pageheight).toFixed(2)),
              Common.Utils.Metric.getCurrentMetricName(),
            ])
          }
        ...
        updateCountOfPages: function (count) {
          this.countOfPages.text(Common.Utils.String.format(this.txtOf, count))
          this.pageCount = count
        },

        updateActiveSheet: function (name) {
          this.txtActiveSheet.text(Common.Utils.String.format(this.txtSheet, name))
        },
