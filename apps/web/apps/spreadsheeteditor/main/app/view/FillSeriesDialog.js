...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  SSE.Views.FillSeriesDialog = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 350,
        ...
        render: function () {
          ...
          this.inputStep._input.on("input", (e) => {
            this.isInputStepFirstChange && this.inputStep.showError()
            this.isInputStepFirstChange = false
          })
          ...
          this.inputStop = new Common.UI.InputField({
            ...
            this.isStopChanged = true
          })
          this.inputStop._input.on("input", (e) => {
            this.isInputStopFirstChange && this.inputStop.showError()
            this.isInputStopFirstChange = false
          })
        ...
        _setDefaults: function (props) {
          if (props) {
            let value = props.asc_getSeriesIn()
        ...
        isValid: function () {
          if (this.isStepChanged) {
            const res = this._changedProps.asc_isValidStepValue(this.inputStep.getValue())
          ...
          if (this.isStopChanged) {
            const res = this._changedProps.asc_isValidStopValue(this.inputStop.getValue())
            if (res[0] !== Asc.c_oAscError.ID.No) {
        ...
        onRadioTypeChange: function (field, newValue, eOpts) {
          if (newValue) {
            ...
            const isDate = field.options.value === Asc.c_oAscSeriesType.date
            const isAuto = field.options.value === Asc.c_oAscSeriesType.autoFill
            const isTrend = this.chTrend.getValue() === "checked"
        ...
        onChangeTrend: function (field, newValue, eOpts) {
          ...
          const isDate = this.radioDate.getValue()
          const isAuto = this.radioAuto.getValue()
