...
define([
  ...
  SSE.Views.FormatRulesEditDlg = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
        ...
        render: function () {
          ...
          const rules = [
            {
              ...
              rules: [
                {
                  name: Common.define.conditionalData.textGreater,
                  subtype: Asc.c_oAscCFOperator.greaterThan,
                },
                {
                  name: Common.define.conditionalData.textGreaterEq,
                  subtype: Asc.c_oAscCFOperator.greaterThanOrEqual,
                },
                {
                  name: Common.define.conditionalData.textLess,
                  subtype: Asc.c_oAscCFOperator.lessThan,
                },
                {
                  name: Common.define.conditionalData.textLessEq,
                  subtype: Asc.c_oAscCFOperator.lessThanOrEqual,
                },
                {
                  name: Common.define.conditionalData.textEqual,
                  subtype: Asc.c_oAscCFOperator.equal,
                },
                {
                  name: Common.define.conditionalData.textNotEqual,
                  subtype: Asc.c_oAscCFOperator.notEqual,
                },
                {
                  name: Common.define.conditionalData.textBetween,
                  subtype: Asc.c_oAscCFOperator.between,
                },
                {
                  name: Common.define.conditionalData.textNotBetween,
                  subtype: Asc.c_oAscCFOperator.notBetween,
                },
              ],
            },
            {
            ...
            },
            {
              ...
              rules: [
                {
                  name: Common.define.conditionalData.textContains,
                  type: Asc.c_oAscCFType.containsText,
                },
                {
                  name: Common.define.conditionalData.textNotContains,
                  type: Asc.c_oAscCFType.notContainsText,
                },
                {
                  name: Common.define.conditionalData.textBegins,
            ...
            },
            {
              ...
              rules: [
                {
                  name: Common.define.conditionalData.textBlanks,
                  type: Asc.c_oAscCFType.containsBlanks,
                },
                {
                  name: Common.define.conditionalData.textNotBlanks,
                  type: Asc.c_oAscCFType.notContainsBlanks,
                },
                {
                  name: Common.define.conditionalData.textErrors,
                  type: Asc.c_oAscCFType.containsErrors,
                },
                {
                  name: Common.define.conditionalData.textNotErrors,
            ...
            },
            {
              ...
              rules: [
                {
                  name: Common.define.conditionalData.textDuplicate,
                  type: Asc.c_oAscCFType.duplicateValues,
                },
                {
                  name: Common.define.conditionalData.textUnique,
          ...
          _.each(rules, (rule, index) => {
            ...
            const store = new Backbone.Collection(arr)
            arrrules.push({
              name: rule.name,
              type: rule.type,
              rules: store,
          ...
          this.cmbCategory = new Common.UI.ComboBox({
            ...
            this.refreshRules(record.value)
          })
          ...
          this.cmbRule = new Common.UI.ComboBox({
            ...
            this.setControls(this.cmbCategory.getValue(), record.value)
          })
          ...
          this.cmbPercent = new Common.UI.ComboBox({
            ...
            const percent = !!record.value
            // me.numRank.setMaxValue(percent ? 100 : 1000);
            this.numRank.setValue(this.numRank.getNumberValue())
          })
          ...
          this.numRank.on(
            "change",
            _.bind((field, newValue, oldValue, eOpts) => {}, this),
          )
          Common.UI.FocusManager.add(this, this.numRank)

          // Format
          this.CFPresets = this.api.asc_getCFPresets()
          const formatPresets = this.CFPresets[Asc.c_oAscCFRuleTypeSettings.format]
          const color_data = []
          const presetTemplate = _.template(
            [
              '<a id="<%= id %>" tabindex="-1" type="menuitem" style="padding: 3px 10px;">',
              '<div style="height: 22px;padding: 4px 0;<%= options.styleStr %>"><%= caption %></div>',
              "</a>",
            ].join(""),
          )
          _.each(formatPresets, (preset, index) => {
            color_data.push({
              value: index,
              presetSettings: {
                fontColor: preset[0],
                ...
                styleObj: {
                  "background-color": preset[1] ? `#${preset[1]}` : "#ffffff",
                  color: preset[0] ? `#${preset[0]}` : "transparent",
                  border: preset[2] ? `1px solid #${preset[2]}` : "",
                  "text-align": "center",
                },
              },
          ...
          const initNewColor = (btn) => {
            btn.setMenu()
            btn.currentColor = "000000"
            const picker = btn.getPicker()
            ...
            menu: new Common.UI.Menu({
              items: [
                ...
                (this.mnuBorderColor = new Common.UI.MenuItem({
                  ...
                  template: _.template(
                    '<a id="<%= id %>" tabindex="-1" type="menuitem"><span class="menu-item-icon" style="background-image: none; width: 12px; height: 12px; border-style: solid; border-width: 3px; border-color: #000;"></span><%= caption %></a>',
                  ),
                  menu: new Common.UI.Menu({
                    menuAlign: "tl-tr",
                    items: [
                      {
                        template: _.template(
                          '<div id="format-rules-borders-menu-bordercolor" style="width: 164px; display: inline-block;"></div>',
                        ),
                      },
                      { caption: "--" },
                      {
                        ...
                        template: _.template(
                          `<a tabindex="-1" type="menuitem" style="${Common.UI.isRTL() ? "padding-right: 12px;" : "padding-left: 12px;"}">${this.textNewColor}</a>`,
                        ),
                      },
                    ],
                  }),
          ...
          this.mnuBorderColorPicker.on("close:extended", () => {
            setTimeout(() => {
              this.btnBorders.focus()
            }, 1)
          })
          $("#format-rules-borders-menu-new-bordercolor").on("click", () => {
            this.mnuBorderColorPicker.addNewColor()
          })
          ...
          if (this.api) {
            ...
            this.numFormatData.forEach((item, index) => {
              item.format = arr[index]
              item.exampleval = this.api.asc_getLocaleExample(item.format, 100)
            })
          }
        ...
        addNewIconsLine: function () {
          const me = this
          const i = this.iconsControls.length
          this.iconsControls.push({})

          let combo = new Common.UI.ComboBoxDataView({
            ...
            itemTemplate: _.template(
              '<img id="<%= id %>" class="item-icon" src="<%= imgUrl %>" style="width: 16px; height: 16px;">',
            ),
            takeFocusOnClose: true,
            updateFormControl: function (record) {
          ...
          menu
            .on("show:before", () => {
              if (!menu.items[0].isChecked()) {
                const rec = picker.store.findWhere({ value: picker.currentIconValue })
          ...
          combo = new Common.UI.ComboBox({
            ...
            cls: "input-group-nr",
            data: [
              { value: true, displayValue: ">=", prevOp: "<" },
              { value: false, displayValue: ">", prevOp: "<=" },
            ],
            type: i,
            takeFocusOnClose: true,
          }).on("selected", (combo, record) => {
          ...
          const range = new Common.UI.InputFieldBtn({
        ...
        renderIconsPanel: function () {
          ...
          this.iconsList = []
          this.collectionIcons.each((icon, index) => {
            this.iconsList.push({
          ...
          const arr = []
          const len = this.collectionPresets.length
          const iconsPresets = this.CFPresets[Asc.c_oAscCFRuleTypeSettings.icons]
          _.each(iconsPresets, (preset, index) => {
            if (index >= len) return
            const values = []
            for (let i = 0; i < preset.length; i++) {
              const formatValueObject = new Asc.asc_CConditionalFormatValueObject()
            ...
            arr.push({
              value: index,
              data: {
                iconSet: this.collectionPresets.at(index).get("icons"),
                values: values,
                icons: this.collectionIcons,
              },
            })
          })
          ...
          this.cmbIconsPresets = new Common.UI.ComboBoxIcons({
            ...
            .on("selected", (combo, record) => {
              ...
              this.fillIconsControls(record.value, record.data.values)
              _.delay(() => {
                this.iconsControls[this.iconsProps.iconsLength - 1].cmbOperator.focus()
              }, 50)
            })
            .on("hide:after", (combo) => {
              !combo.skipFocus &&
          ...
          this.chIconShow.on("change", (field, newValue, oldValue, eOpts) => {})
          Common.UI.FocusManager.add(this, this.chIconShow)

          this.btnReverse = new Common.UI.Button({
            el: $("#format-rules-edit-btn-icon-reverse"),
          })
          this.btnReverse.on("click", () => {
            this.iconsProps.isReverse = !this.iconsProps.isReverse
            this.reverseIconsControls()
          })
          ...
          const rec = this.cmbIconsPresets.getSelectedRecord()
        ...
        renderDataBarPanel: function () {
          ...
          for (let i = 0; i < 2; i++) {
            let arr = this.dataValTypes
            if (i === 0) {
              arr = [{ value: Asc.c_oAscCfvoType.Minimum, displayValue: this.textMinimum }].concat(
                arr,
              )
              arr.push({ value: Asc.c_oAscCfvoType.AutoMin, displayValue: this.textAutomatic })
            } else {
              arr = [{ value: Asc.c_oAscCfvoType.Maximum, displayValue: this.textMaximum }].concat(
                arr,
              )
              arr.push({ value: Asc.c_oAscCfvoType.AutoMax, displayValue: this.textAutomatic })
            }
            const combo = new Common.UI.ComboBox({
              ...
              this.barControls[combo.options.type].range.setDisabled(
                record.value === Asc.c_oAscCfvoType.Minimum ||
                  record.value === Asc.c_oAscCfvoType.Maximum ||
                  record.value === Asc.c_oAscCfvoType.AutoMin ||
                  record.value === Asc.c_oAscCfvoType.AutoMax,
              )
              this.setDefComboValue(
                combo.options.type,
                record.value,
                this.barControls[combo.options.type].range,
              )
            })
            const value = i === 0 ? Asc.c_oAscCfvoType.AutoMin : Asc.c_oAscCfvoType.AutoMax
            ...
            const range = new Common.UI.InputFieldBtn({
              ...
              type: i,
              disabled:
                value === Asc.c_oAscCfvoType.AutoMin ||
                value === Asc.c_oAscCfvoType.AutoMax ||
                value === Asc.c_oAscCfvoType.Minimum ||
                value === Asc.c_oAscCfvoType.Maximum,
            })
            this.setDefComboValue(i, value, range)
            range.on("button:click", _.bind(this.onSelectData, this))
          ...
          this.cmbFill = new Common.UI.ComboBox({
            ...
            if (this.barProps) {
              this.barProps.asc_setGradient(record.value)
              this.previewFormat()
            }
          })
          ...
          this.chFill.on("change", (field, newValue, oldValue, eOpts) => {
            this.btnNegFill.setDisabled(field.getValue() === "checked")
          })
          ...
          this.cmbBorder = new Common.UI.ComboBox({
            ...
            const hasBorder = !record.value
            this.btnPosBorder.setDisabled(!hasBorder)
            this.btnNegBorder.setDisabled(!hasBorder || this.chBorder.getValue() === "checked")
            this.chBorder.setDisabled(!hasBorder)
            if (this.barProps) {
              if (hasBorder) {
                this.barProps.asc_setBorderColor(
                  Common.Utils.ThemeColor.getRgbColor(this.btnPosBorder.colorPicker.currentColor),
                )
              } else this.barProps.asc_setBorderColor(null)
              this.previewFormat()
            }
          })
          ...
          this.chBorder.on("change", (field, newValue, oldValue, eOpts) => {
            this.btnNegBorder.setDisabled(field.getValue() === "checked")
          })
          ...
          this.cmbBarDirection = new Common.UI.ComboBox({
            ...
            if (this.barProps) {
              this.barProps.asc_setDirection(record.value)
              this.previewFormat()
            }
          })
          ...
          this.cmbAxisPos = new Common.UI.ComboBox({
            ...
            this.btnAxisColor.setDisabled(record.value === Asc.c_oAscDataBarAxisPosition.none)
          })
        ...
        renderScalesPanel: function () {
          ...
          for (let i = 0; i < 3; i++) {
            ...
            const combo = new Common.UI.ComboBox({
              ...
              this.scaleControls[combo.options.type].range.setDisabled(
                record.value === Asc.c_oAscCfvoType.Minimum ||
                  record.value === Asc.c_oAscCfvoType.Maximum,
              )
              this.setDefComboValue(
                combo.options.type,
                record.value,
                this.scaleControls[combo.options.type].range,
              )
            })
            ...
            const color = new Common.UI.ColorButton({
        ...
        _setDefaults: function (props) {
          ...
          const type = props ? props.asc_getType() : this.type
          let ruleType
          let subtype = this.subtype
          let isPercent = !!this.percent
          ...
          if (props) {
            ...
            switch (type) {
              ...
              case Asc.c_oAscCFType.aboveAverage: {
                const above = props.asc_getAboveAverage()
                const eq = props.asc_getEqualAverage()
                const stddev = props.asc_getStdDev()
                subtype = above ? 0 : 1
                if (eq) subtype += 2
                else if (stddev) {
                  subtype += 2 + stddev * 2
              ...
              case Asc.c_oAscCFType.colorScale: {
                ...
                const scales = value.asc_getCFVOs()
                const colors = value.asc_getColors()
                subtype = scales.length
                const arr =
                  scales.length === 2
                    ? [this.scaleControls[0], this.scaleControls[2]]
                    : this.scaleControls
                for (let i = 0; i < scales.length; i++) {
                  const scaletype = scales[i].asc_getType()
                  const val = scales[i].asc_getVal()
                  const color = colors[i]
                  const controls = arr[i]
                  controls.combo.setValue(scaletype)
                  controls.range.setDisabled(
                    scaletype === Asc.c_oAscCfvoType.Minimum ||
                      scaletype === Asc.c_oAscCfvoType.Maximum,
                  )
                  controls.range.setValue(
                    scaletype !== Asc.c_oAscCfvoType.Minimum &&
                      scaletype !== Asc.c_oAscCfvoType.Maximum &&
                      val !== null &&
                      val !== undefined
              ...
              case Asc.c_oAscCFType.dataBar: {
                ...
                value = props.asc_getColorScaleOrDataBarOrIconSetRule()
                const bars = value.asc_getCFVOs()
                const arr = this.barControls
                for (let i = 0; i < bars.length; i++) {
                  const bartype = bars[i].asc_getType()
                  const val = bars[i].asc_getVal()
                  const controls = arr[i]
                  controls.combo.setValue(bartype)
                  controls.range.setDisabled(
                    bartype === Asc.c_oAscCfvoType.Minimum ||
                      bartype === Asc.c_oAscCfvoType.Maximum ||
                      bartype === Asc.c_oAscCfvoType.AutoMin ||
                      bartype === Asc.c_oAscCfvoType.AutoMax,
                  )
                  controls.range.setValue(
                    bartype !== Asc.c_oAscCfvoType.Minimum &&
                      bartype !== Asc.c_oAscCfvoType.Maximum &&
                ...
                const color = value.asc_getBorderColor()
                ...
                if (color) {
                  ...
                  this.setColor(
                    value.asc_getNegativeBorderColor() || value.asc_getBorderColor(),
                    this.btnNegBorder,
                  )
                }
                this.chBorder.setValue(value.asc_getNegativeBarBorderColorSameAsPositive())
                this.chBorder.setDisabled(color === null)
                this.btnNegBorder.setDisabled(
                  color === null || value.asc_getNegativeBarBorderColorSameAsPositive(),
                )
                ...
                value.asc_getAxisColor() &&
                  this.setColor(value.asc_getAxisColor(), this.btnAxisColor)
                this.btnAxisColor.setDisabled(
                  value.asc_getAxisPosition() === Asc.c_oAscDataBarAxisPosition.none,
                )
                break
              }
              case Asc.c_oAscCFType.iconSet:
                ...
                this.fillIconsControls(
                  value.asc_getIconSet(),
                  value.asc_getCFVOs(),
                  value.asc_getIconSets(),
                )
                break
            }
          }
          ...
          if (!rec) rec = this.ruleStore.at(0)
          if (rec) {
          ...
          const xfs = this.xfsFormat ? this.xfsFormat : new Asc.asc_CellXfs()
          this.fillXfsFormatInfo(xfs)
          this.previewFormat()
          Common.UI.FocusManager.add(this, this.getFooterButtons())
        },

        setColor: (color, control, picker) => {
        ...
        fillXfsFormatInfo: function (xfs) {
          if (xfs) {
            ...
            let color = this.setColor(
              xfs.asc_getFontColor(),
              this.btnTextColor,
              this.mnuTextColorPicker,
            )
            color = this.setColor(
              xfs.asc_getFillColor(),
              this.btnFillColor,
              this.mnuFillColorPicker,
            )

            const val = xfs.asc_getNumFormatInfo()
        ...
        getSettings: function () {
          let props
          const rec = this.ruleStore.findWhere({ index: this.cmbCategory.getValue() })

          if (rec) {
            props = this._originalProps || new Asc.asc_CConditionalFormattingRule()
            const type = rec.get("type")
            const type_changed = type !== props.asc_getType()
            ...
            switch (type) {
              case Asc.c_oAscCFType.containsText: {
                const value = this.txtRange1.getValue()
              ...
              case Asc.c_oAscCFType.timePeriod:
                ...
                break
              case Asc.c_oAscCFType.aboveAverage: {
                const val = this.cmbRule.getValue()
                const above = !(val % 2)
              ...
              case Asc.c_oAscCFType.expression:
                ...
                break
              case Asc.c_oAscCFType.colorScale: {
                const scaleProps = !type_changed
                  ? props.asc_getColorScaleOrDataBarOrIconSetRule()
                  : new Asc.asc_CColorScale()
                const scalesCount = rec.get("num")
                const arr =
                  scalesCount === 2
                    ? [this.scaleControls[0], this.scaleControls[2]]
                    : this.scaleControls
                ...
                for (let i = 0; i < scalesCount; i++) {
                  const scale = new Asc.asc_CConditionalFormatValueObject()
                  const controls = arr[i]
                  scale.asc_setType(controls.combo.getValue())
                  scale.asc_setVal(controls.range.getValue())
                  scales.push(scale)
                  colors.push(
                    Common.Utils.ThemeColor.getRgbColor(controls.colorPicker.currentColor),
                  )
                }
              ...
              case Asc.c_oAscCFType.dataBar: {
                const barProps = !type_changed
                  ? props.asc_getColorScaleOrDataBarOrIconSetRule()
                  : new Asc.asc_CDataBar()
                type_changed && barProps.asc_setInterfaceDefault()
                const arr = this.barControls
                const bars = []
                for (let i = 0; i < arr.length; i++) {
                  const bar = new Asc.asc_CConditionalFormatValueObject()
                  const controls = arr[i]
                  const bartype = controls.combo.getValue()
                ...
                barProps.asc_setColor(
                  Common.Utils.ThemeColor.getRgbColor(this.btnPosFill.colorPicker.currentColor),
                )
                barProps.asc_setNegativeColor(
                  Common.Utils.ThemeColor.getRgbColor(
                    this.chFill.getValue() === "checked"
                      ? this.btnPosFill.colorPicker.currentColor
                      : this.btnNegFill.colorPicker.currentColor,
                  ),
                )
                barProps.asc_setNegativeBarColorSameAsPositive(this.chFill.getValue() === "checked")
                const hasBorder = !this.cmbBorder.getValue()
                if (hasBorder) {
                  barProps.asc_setBorderColor(
                    Common.Utils.ThemeColor.getRgbColor(this.btnPosBorder.colorPicker.currentColor),
                  )
                  barProps.asc_setNegativeBorderColor(
                    Common.Utils.ThemeColor.getRgbColor(
                      this.chBorder.getValue() === "checked"
                        ? this.btnPosBorder.colorPicker.currentColor
                        : this.btnNegBorder.colorPicker.currentColor,
                    ),
                  )
                  barProps.asc_setNegativeBarBorderColorSameAsPositive(
                    this.chBorder.getValue() === "checked",
                  )
                } else {
                ...
                const pos = this.cmbAxisPos.getValue()
                barProps.asc_setAxisPosition(pos)
                if (pos !== Asc.c_oAscDataBarAxisPosition.none) {
                  barProps.asc_setAxisColor(
                    Common.Utils.ThemeColor.getRgbColor(this.btnAxisColor.colorPicker.currentColor),
                  )
                } else barProps.asc_setAxisColor(null)
                props.asc_setColorScaleOrDataBarOrIconSetRule(barProps)
                break
              }
              case Asc.c_oAscCFType.iconSet: {
                const iconsProps = !type_changed
                  ? props.asc_getColorScaleOrDataBarOrIconSetRule()
                  : new Asc.asc_CIconSet()
                ...
                const arr = this.iconsControls
                const len = this.iconsProps.iconsLength
                const icons = !this.cmbIconsPresets.getSelectedRecord() ? [] : null
                const values = []
                for (let i = 0; i < len; i++) {
                  const controls = arr[i]
                  const value = new Asc.asc_CConditionalFormatValueObject()
                  ...
                  if (icons) {
                    if (controls.itemNoIcons.isChecked()) {
                      const icon = new Asc.asc_CConditionalFormatIconSet()
                      ...
                      let icon = controls.pickerIcons.currentIconValue + 1
                      for (let k = 0; k < this.collectionPresets.length; k++) {
                        const items = this.collectionPresets.at(k).get("icons")
        ...
        refreshRules: function (index, ruleType) {
          const rec = this.ruleStore.findWhere({ index: index })
          if (rec) {
            const rules = rec.get("rules")
            const cmbData = []
            rules?.each((rule, idx) => {
              cmbData.push({
                value: rule.get("type") !== undefined ? rule.get("type") : rule.get("subtype"),
                displayValue: rule.get("name"),
              })
            })
            this.cmbRule.setData(cmbData)
            cmbData.length > 0 &&
              this.cmbRule.setValue(ruleType !== undefined ? ruleType : cmbData[0].value)
          }
          ...
          if (rec) {
            ...
            if (
              ...
              ;(this.xfsFormat || this.xfsFormat === null) &&
                this._changedProps.asc_setDxf(this.xfsFormat)
        ...
        setControls: function (category, rule) {
          const hasformat = this.$window.find(".hasformat")
          ...
          this.txtRange2.setVisible(
            category === 0 &&
              (rule === Asc.c_oAscCFOperator.between || rule === Asc.c_oAscCFOperator.notBetween),
          )
          ...
          if (category < 7) focused = this.cmbRule
        ...
        onSelectData: function (cmp) {
          if (this.api) {
            ...
            win.setSettings({
              api: this.api,
              range:
                !_.isEmpty(cmp.getValue()) && cmp.checkValidate() === true ? cmp.getValue() : "",
        ...
        onFormatsSelect: function (menu, item) {
          const xfs = new Asc.asc_CellXfs()
          const settings = item.options.presetSettings
          settings?.fontColor &&
            xfs.asc_setFontColor(Common.Utils.ThemeColor.getRgbColor(settings.fontColor))
          settings?.fillColor &&
            xfs.asc_setFillColor(Common.Utils.ThemeColor.getRgbColor(settings.fillColor))
          if (settings?.borderColor) {
            const new_borders = []
            const bordersWidth = Asc.c_oAscBorderStyles.Thin
            const bordersColor = Common.Utils.ThemeColor.getRgbColor(settings.borderColor)
          ...
          this.xfsFormat = xfs
          this._changedProps?.asc_setDxf(xfs)
          this.fillXfsFormatInfo(xfs)
          this.previewFormat()
        },
        ...
        clearFormat: function () {
          this.xfsFormat = null
          this._changedProps?.asc_setDxf(null)
        ...
        onBordersColor: function (picker, color) {
          $("#format-rules-borders-border-color .menu-item-icon").css(
            "border-color",
        ...
        onBordersMenu: function (menu, item) {
          if (!_.isUndefined(item.options.borderId)) {
            const btnBorders = this.btnBorders
            const new_borders = []
            const bordersWidth = btnBorders.options.borderswidth
            const bordersColor = btnBorders.options.borderscolor
        ...
        onFormatTextColorSelect: function (btn, color, fromBtn) {
          ...
          !this.xfsFormat && (this.xfsFormat = new Asc.asc_CellXfs())
          this.xfsFormat.asc_setFontColor(
            Common.Utils.ThemeColor.getRgbColor(this.mnuTextColorPicker.currentColor),
          )
          this.previewFormat()
        },
        ...
        onFormatFillColorSelect: function (btn, color, fromBtn) {
          ...
          !this.xfsFormat && (this.xfsFormat = new Asc.asc_CellXfs())
          this.xfsFormat.asc_setFillColor(
            this.mnuFillColorPicker.currentColor === "transparent"
              ? null
              : Common.Utils.ThemeColor.getRgbColor(this.mnuFillColorPicker.currentColor),
          )
          this.previewFormat()
        },
        ...
        previewFormat: function () {
          if (this._changedProps) {
            const type = this._changedProps.asc_getType()
            if (
              type === Asc.c_oAscCFType.containsText ||
              type === Asc.c_oAscCFType.notContainsText ||
              type === Asc.c_oAscCFType.beginsWith ||
              ...
              this.xfsFormat &&
                !this._changedProps.asc_getDxf() &&
                this._changedProps.asc_setDxf(this.xfsFormat)
              this._changedProps.asc_getPreview(
                "format-rules-edit-preview-format",
                this.xfsFormat
                  ? Common.define.conditionalData.exampleText
        ...
        updateThemeColors: function () {
          if (this.panels.scale.rendered && !this.panels.scale.initColors) {
            for (let i = 0; i < this.scaleControls.length; i++) {
              const btn = this.scaleControls[i].color
              btn.setMenu()
              const colorPicker = btn.getPicker()
          ...
          if (this.panels.format.rendered && !this.panels.format.initColors) {
            this.mnuTextColorPicker.updateColors(
              Common.Utils.ThemeColor.getEffectColors(),
              Common.Utils.ThemeColor.getStandartColors(),
            )
            this.mnuFillColorPicker.updateColors(
              Common.Utils.ThemeColor.getEffectColors(),
              Common.Utils.ThemeColor.getStandartColors(),
            )
            this.mnuBorderColorPicker.updateColors(
              Common.Utils.ThemeColor.getEffectColors(),
              Common.Utils.ThemeColor.getStandartColors(),
            )
        ...
        onScaleColorsSelect: function (picker, color) {
          ...
          if (this.scaleProps) {
            const colors = this.scaleProps.asc_getColors()
            colors[Math.min(picker.options.type, colors.length - 1)] =
              Common.Utils.ThemeColor.getRgbColor(picker.colorPicker.currentColor)
            this.scaleProps.asc_setColors(colors)
            this.previewFormat()
          }
        },
        ...
        onBarColorsSelect: function (type, picker, color) {
          ...
          if (this.barProps && (type === "pos" || type === "pos-border")) {
            ...
            else this.barProps.asc_setBorderColor(Common.Utils.ThemeColor.getRgbColor(color))
            this.previewFormat()
          }
        },

        setDefComboValue: function (index, type, range) {
          const category = this.cmbCategory.getValue()
          let value = ""
          switch (type) {
          ...
          if (icons && icons.length > 0) {
            ...
            _.each(icons, (item) => {
              if (item.asc_getIconSet() === Asc.EIconSetType.NoIcons) {
                ...
                iconsIndexes.push(
                  this.collectionPresets.at(item.asc_getIconSet()).get("icons")[
                    item.asc_getIconId()
                  ],
                )
            })
            ...
            iconsIndexes = this.collectionPresets.at(iconSet).get("icons")
          }
          const isReverse = this.iconsProps.isReverse
          const len = iconsIndexes.length
          for (let i = 0; i < iconsIndexes.length; i++) {
            const controls = arr[isReverse ? len - i - 1 : i]
            const rec =
              iconsIndexes[i] === -1
                ? null
                : controls.pickerIcons.store.findWhere({ value: iconsIndexes[i] - 1 })
        ...
        fillIconsLabels: function () {
          const arr = this.iconsControls
          const len = this.iconsProps.iconsLength
          const regstr = /^\s*[-]?[0-9]+[,.]?[0-9]*\s*$/
          let val = arr[1].value.getValue()
          arr[0].label.text(
            Common.Utils.String.format(
              this.textIconLabelFirst,
              arr[1].cmbOperator.getSelectedRecord().prevOp,
              regstr.test(val) ? Number.parseFloat(val) : this.textFormula,
            ),
          )
          for (let i = 1; i < len - 1; i++) {
            ...
            arr[i].label.text(
              Common.Utils.String.format(
                this.textIconLabel,
                arr[i + 1].cmbOperator.getSelectedRecord().prevOp,
                regstr.test(val) ? Number.parseFloat(val) : this.textFormula,
              ),
            )
          }
        ...
        reverseIconsControls: function () {
          ...
          for (let i = 0; i < len / 2; i++) {
            const controls1 = arr[i]
            const controls2 = arr[len - i - 1]
            const icon1 = controls1.itemNoIcons.isChecked()
              ? -1
              : controls1.pickerIcons.currentIconValue
            const icon2 = controls2.itemNoIcons.isChecked()
              ? -1
              : controls2.pickerIcons.currentIconValue
            let rec = controls1.pickerIcons.store.findWhere({ value: icon2 })
          ...
          if (rec) {
            ...
            switch (type) {
              ...
              case Asc.c_oAscCFType.expression:
                ...
                break
              case Asc.c_oAscCFType.cellIs: {
                const subtype = this.cmbRule.getValue()
                if (this.txtRange1.getValue() === "") {
              ...
              case Asc.c_oAscCFType.top10: {
                const isPercent = !!this.cmbPercent.getValue()
                res = this.api.asc_isValidDataRefCf(type, [
                  [this.numRank.getNumberValue(), isPercent],
                ])
                res && (res = res[0])
                if (res === Asc.c_oAscError.ID.ErrorTop10Between)
                  msg = Common.Utils.String.format(
                    this.textErrorTop10Between,
                    isPercent ? 0 : 1,
                    isPercent ? 100 : 1000,
                  )
                break
              ...
              case Asc.c_oAscCFType.colorScale: {
                ...
                for (let i = 0; i < scalesCount; i++) {
                  if (
                    !scaleControls[i].range.isDisabled() &&
                    scaleControls[i].range.getValue() === ""
                  ) {
                    msg =
                      scaleControls[i].combo.getValue() === Asc.c_oAscCfvoType.Formula
                        ? this.textEmptyFormula
                        : this.textEmptyText
                ...
                if (!msg) {
                  ...
                  if (res) {
                    ...
                    if (res === Asc.c_oAscError.ID.ValueMustBeGreaterThen) {
                      msg = Common.Utils.String.format(
                        this.textErrorGreater,
                        index === 0
                          ? this.textMinpoint
                          : index === scalesCount - 1
                            ? this.textMaxpoint
                            : this.textMidpoint,
                        index === scalesCount - 1 && scalesCount === 3
                          ? this.textMidpoint
                          : this.textMinpoint,
                      )
                    } else if (res === Asc.c_oAscError.ID.NotValidPercentage)
                      msg = Common.Utils.String.format(
                        this.textNotValidPercentageExt,
                        index === 0
                          ? this.textMinpoint
                          : index === scalesCount - 1
                            ? this.textMaxpoint
                      ...
                      msg = Common.Utils.String.format(
                        this.textNotValidPercentileExt,
                        index === 0
                          ? this.textMinpoint
                          : index === scalesCount - 1
                            ? this.textMaxpoint
              ...
              case Asc.c_oAscCFType.dataBar: {
                const barControls = this.barControls
                for (let i = 0; i < barControls.length; i++) {
                  if (
                    !barControls[i].range.isDisabled() &&
                    barControls[i].range.getValue() === ""
                  ) {
                    msg =
                      barControls[i].combo.getValue() === Asc.c_oAscCfvoType.Formula
                        ? this.textEmptyFormula
                        : this.textEmptyText
                ...
                if (!msg) {
                  ...
                  if (res) {
                    ...
                    if (res === Asc.c_oAscError.ID.NotValidPercentage)
                      msg = Common.Utils.String.format(
                        this.textNotValidPercentageExt,
                        index ? this.textLongBar : this.textShortBar,
                      )
                    else if (res === Asc.c_oAscError.ID.NotValidPercentile)
                      msg = Common.Utils.String.format(
                        this.textNotValidPercentileExt,
              ...
              case Asc.c_oAscCFType.iconSet: {
                const iconsControls = this.iconsControls
                for (let i = 0; i < this.iconsProps.iconsLength; i++) {
                  if (
                    !iconsControls[i].value.isDisabled() &&
                    iconsControls[i].value.getValue() === ""
                  ) {
                    msg =
                      iconsControls[i].cmbType.getValue() === Asc.c_oAscCfvoType.Formula
                        ? this.textEmptyFormula
                        : this.textEmptyText
                ...
                if (!msg) {
                  for (let i = 0; i < this.iconsProps.iconsLength; i++) {
                    arr.push([
                      iconsControls[i].value.getValue(),
                      iconsControls[i].cmbType.getValue(),
                    ])
                  }
                  res = this.api.asc_isValidDataRefCf(type, arr)
                  if (res) {
            ...
            if (!msg && res) {
              const mainController = SSE.getController("Main")
              switch (res) {
        ...
        textEmptyFormulaExt:
          "The formula you entered does not evaluate to a number, date, time or string.",
        ...
        textIconsOverlap:
          "One or more icon data ranges overlap.<br>Adjust icon data range values so that the ranges do not overlap.",
        textSingleRef:
          "This type of reference cannot be used in a conditional formatting formula.<br>Change the reference to a single cell, or use the reference with a worksheet function, such as =SUM(A1:B5).",
        textRelativeRef:
