...
define([
  ...
  const _CustomItem = Common.UI.DataViewItem.extend({
    initialize: function (options) {
      ...
      this.listenTo(this.model, "change:lock", () => {
        this.render()
        this.trigger("change", this, this.model)
      })
      this.listenTo(this.model, "change:tip", () => {
        const el = this.$el || $(this.el)
        const tip = el.data("bs.tooltip")
        if (tip) {
          const zIndex = tip.options.zIndex
          el.removeData("bs.tooltip")
          el.tooltip({
            title: this.model.get("tip"),
            placement: "cursor",
            zIndex: zIndex,
          })
        }
  ...
  SSE.Views.FormatRulesManagerDlg = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
        ...
        initialize: function (options) {
          ...
          this.maxItemsCount = Number.parseInt(this.previewListHeight / this.itemHeight)
        ...
        render: function () {
          ...
          this.rulesList
            .on("item:select", _.bind(this.onSelectRule, this))
            ...
            .on(
              "entervalue",
              _.bind(function (e) {
                !!e.store.length && this.onEditRule(true)
              }, this),
            )
        ...
        refreshScopeList: function () {
          ...
          const items = [
            {
              value: Asc.c_oAscSelectionForCFType.selection,
              displayValue: this.textSelection,
              sheetIndex: -1,
            },
            {
              value: Asc.c_oAscSelectionForCFType.worksheet,
              displayValue: this.textThisSheet,
              sheetIndex: -1,
            },
            {
              value: Asc.c_oAscSelectionForCFType.table,
              displayValue: this.textThisTable,
              sheetIndex: -1,
            },
            {
              value: Asc.c_oAscSelectionForCFType.pivot,
              displayValue: this.textThisPivot,
              sheetIndex: -1,
            },
          ]
          // if (wc>1) {
        ...
        refreshRuleList: function (scope) {
          ...
          const sheetIndex = scope.sheetIndex > -1 ? scope.sheetIndex : this.currentSheet
          let ruleStore = this.rulesStores[sheetIndex]
          if (!ruleStore) {
            ruleStore = []

            const obj = this.api.asc_getCF(Asc.c_oAscSelectionForCFType.worksheet, sheetIndex)
            const rules = obj[0]
            ...
            if (rules) {
              for (let i = 0; i < rules.length; i++) {
                ...
                const location = rule.asc_getLocation()
                const idlock = rule.asc_getIsLock()
                ruleStore.push({
                  ...
                  lockuser: idlock
                    ? this.isUserVisible(idlock)
                      ? this.getUserName(idlock)
                      : this.lockText
                    : this.guestText,
                })
              }
        ...
        onScrollEnd: function () {
          ...
          const startPos = this.scrollerY.getScrollTop()
          let start = Math.floor(startPos / this.itemHeight + 0.5)
          let end = start + this.maxItemsCount - 1
        ...
        previewDataBlock: function (start, end) {
          ...
          const ruleStore = this.rulesStores[this.currentSheet]
          const arr = []
        ...
        getRuleName: function (rule) {
          ...
          switch (rule.asc_getType()) {
            case Asc.c_oAscCFType.aboveAverage: {
              name = this.textAbove
              const above = rule.asc_getAboveAverage()
              const eq = rule.asc_getEqualAverage()
              const stddev = rule.asc_getStdDev()
              subtype = above ? 0 : 1
              if (eq) subtype += 2
              else if (stddev) {
                subtype += 2 + stddev * 2
            ...
            case Asc.c_oAscCFType.cellIs: {
              ...
              const subtype = rule.asc_getOperator()
              let op
              switch (subtype) {
            ...
            case Asc.c_oAscCFType.timePeriod: {
              const subtype = rule.asc_getTimePeriod()
              switch (subtype) {
            ...
            case Asc.c_oAscCFType.top10:
              name = rule.asc_getBottom()
                ? Common.define.conditionalData.textBottom
                : Common.define.conditionalData.textTop
        ...
        addControls: function (listView, itemView, item) {
          if (!item) return
          const i = item.get("ruleIndex")
          const cmpEl = this.rulesList.cmpEl.find(`#format-manager-item-${i}`)

          if (!this.rules[i]) this.rules[i] = {}
          const rule = this.rules[i]
          const input = new Common.UI.InputFieldBtn({
            ...
            .on("changed:after", (input, newValue, oldValue, e) => {
              if (rule.dataRangeValid !== newValue) {
                if (this.isRangeValid(newValue)) {
                  ...
                  this.rulesStores[this.currentSheet][item.get("indexInStore")].ruleChanged = true
                  this.rulesStores[this.currentSheet][item.get("indexInStore")].range =
                    rule.dataRangeValid
                } else rule.txtDataRange.setValue(rule.dataRangeValid)
              }
            })
            .on("button:click", _.bind(this.onSelectData, this, rule, item))
          input.cmpEl.on("dblclick", "input", (e) => {
            e.stopPropagation()
          })
          Common.UI.FocusManager.add(this, input)

          const val = item.get("range")
          val !== null && input.setValue(val)
          rule.txtDataRange = input
          rule.dataRangeValid = val
          rule.previewDiv = `format-manager-item-preview-${i}`
          this.drawRulePreview(item)

          cmpEl.on("mousedown", "input", () => {
            this.rulesList.selectRecord(item)
          })
        },

        drawRulePreview: function (rule) {
          const props = rule.get("props")
        ...
        onSelectData: function (rule, item) {
          if (this.api) {
            const handlerDlg = (dlg, result) => {
              if (result === "ok") {
                if (this.isRangeValid(dlg.getSettings())) {
            ...
            win.setSettings({
              api: this.api,
              range: !_.isEmpty(rule.txtDataRange.getValue())
                ? rule.txtDataRange.getValue()
                : rule.dataRangeValid,
        ...
        onEditRule: function (isEdit) {
          const xy = Common.Utils.getOffset(this.$window)
          const rec = this.rulesList.getSelectedRec()
          let previewRec

          const win = new SSE.Views.FormatRulesEditDlg({
            api: this.api,
            ...
            handler: (result, settings) => {
              if (result === "ok" && settings) {
                const name = this.getRuleName(settings)
                if (isEdit) {
                  const item = this.rulesStores[this.currentSheet][rec.get("indexInStore")]
                  ...
                  settings.asc_setLocation(this.currentRange)
                  const ruleStore = this.rulesStores[this.currentSheet]
                  ...
                  this.updateRulesPriority(ruleStore)
                }
              }
            },
          }).on("close", () => {
            this.show()
            if (isEdit) {
              previewRec && this.drawRulePreview(previewRec)
            } else if (previewRec) {
              this.cmbScope.setValue(Asc.c_oAscSelectionForCFType.selection)
              this.refreshRuleList(this.cmbScope.getSelectedRecord())
            }
        ...
        onDeleteRule: function () {
          const store = this.rulesList.store
          const rec = this.rulesList.getSelectedRec()
          if (rec) {
            const id = rec.get("ruleId")
            ...
            this.updateRulesCount(true)
            let start = this.listSettings.start
            let end = start + this.maxItemsCount - 1
            if (end > this.listSettings.data.length - 1) {
              end = this.listSettings.data.length - 1
              start = Math.max(0, end - this.maxItemsCount + 1)
            ...
            this.previewDataBlock(start, end)
            this.listSettings.data.length > 0 &&
              this.rulesList.selectByIndex(Math.min(index, store.length - 1))
          }
          this.updateButtons()
        },
        ...
        onMoveClick: function (up) {
          const ruleStore = this.rulesStores[this.currentSheet]
          let rec = this.rulesList.getSelectedRec()
          const selectedIndex = this.rulesList.store.indexOf(rec)
          if (rec) {
            const index = rec.get("indexInStore")
            const newindex = up ? this.getPrevRuleIndex(index) : this.getNextRuleIndex(index)
            const newrec = ruleStore[newindex]
            const prioritynew = newrec.priority
            rec = ruleStore[index]
            ...
            let start =
              selectedIndex === 0 && up
                ? this.listSettings.start - 1
                : !up && selectedIndex >= this.maxItemsCount - 1
                  ? this.listSettings.start + 1
                  : this.listSettings.start
            let end = start + this.maxItemsCount - 1
            if (end > this.listSettings.data.length - 1) {
              end = this.listSettings.data.length - 1
              start = Math.max(0, end - this.maxItemsCount + 1)
        ...
        onSelectRule: function (lisvView, itemView, record) {
          const idx = _.indexOf(lisvView.store.models, record)
        ...
        updateButtons: function () {
          const rec = this.rulesList.getSelectedRec()
          const lock = rec ? rec.get("lock") : false
        ...
        updateMoveButtons: function () {
          const rec = this.rulesList.getSelectedRec()
          const index = rec ? this.rulesList.store.indexOf(rec) : -1
          this.btnUp.setDisabled(index + this.listSettings.start < 1 || this.locked)
          this.btnDown.setDisabled(
            index < 0 ||
              index + this.listSettings.start >= this.listSettings.data.length - 1 ||
              this.locked,
          )
        },
        ...
        getPrevRuleIndex: function (index) {
          ...
          const ruleStore = this.rulesStores[this.currentSheet]
          for (let i = index - 1; i >= this.listSettings.min; i--) {
        ...
        getNextRuleIndex: function (index) {
          ...
          const ruleStore = this.rulesStores[this.currentSheet]
          for (let i = index + 1; i <= this.listSettings.max; i++) {
            if (ruleStore[i].cls !== "hidden") {
              return i
            }
          }
        ...
        updateRulesCount: function (updateData) {
          const ruleStore = this.rulesStores[this.currentSheet]
          const scope = this.cmbScope.getSelectedRecord()
          updateData && (this.listSettings.data = [])
          this.listSettings.min = -1
          this.listSettings.max = 0

          ruleStore.forEach((item, index) => {
            const hidden =
              scope.value !== Asc.c_oAscSelectionForCFType.worksheet &&
              !item.props.asc_checkScope(scope.value)
            item.cls = hidden ? "hidden" : undefined
            item.indexInStore = index
            if (!hidden) {
              updateData && this.listSettings.data.push(index)
              this.listSettings.max = index
              this.listSettings.min < 0 && (this.listSettings.min = index)
            }
          })
          updateData &&
            this.previewInner.height(
              this.listSettings.data.length * this.itemHeight + this.rulesList.headerHeight,
            )
        },

        updateRulesPriority: (store) => {
        ...
        getSettings: function () {
          const sheets = []
          for (const sheet in this.rulesStores) {
            if (this.rulesStores.hasOwnProperty(sheet)) {
              const store = this.rulesStores[sheet]
              const arr = []
              store?.forEach((item) => {
                ...
                if (item.ruleChanged) arr.push(props)
              })
        ...
        onKeyEvent: function (list, e, repeat) {
          if (
            e.keyCode === Common.UI.Keys.DELETE &&
            !this.btnDelete.isDisabled() &&
            !/form-control/.test(e.target.className)
          ) {
            this.onDeleteRule()
          } else if (e.keyCode === Common.UI.Keys.UP || e.keyCode === Common.UI.Keys.DOWN) {
            const rec = list.getSelectedRec()
            let idx = _.indexOf(list.store.models, rec)
            if (idx >= 0) {
        ...
        close: function () {
          ...
          this.api.asc_unregisterCallback(
            ...
            this.wrapEvents.onUnLockCFManager,
          )
        ...
        onLockCFRule: function (index, ruleId, userId) {
          ...
          const ruleStore = this.rulesStores[this.currentSheet]
          let rec = _.findWhere(ruleStore, { ruleId: ruleId })
          if (rec) {
            rec.lockuser = userId
              ? this.isUserVisible(userId)
