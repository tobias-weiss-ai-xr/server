...
define([
  ...
  SSE.Views = SSE.Views || {}
  SSE.Views.FormulaDialog = Common.UI.Window.extend(
    _.extend(
      {
        ...
        initialize: function (options) {
          ...
          this.template =
            ...
            [
              ...
              `<label class="header">${this.textGroupDescription}</label>`,
              '<div id="formula-dlg-combo-group" class="input-group-nr" style=""></div>',
              `<label class="header" style="margin-top: 10px">${this.textListDescription}</label>`,
        ...
        render: function () {
          ...
          this.btnOk =
            _.find(
              ...
              (item) =>
                item.$el &&
                item.$el.find("#formula-dlg-btn-ok").addBack().filter("#formula-dlg-btn-ok")
                  .length > 0,
        ...
        getFocusedComponents: function () {
          return [this.inputSearch, this.cmbFuncGroup, this.cmbListFunctions].concat(
            this.getFooterButtons(),
          )
        },
        ...
        show: function (group) {
          if (this.$window) {
            ...
            let top
            let left
            const win_height = this.initConfig.height
            ...
            top = (Number.parseInt(main_height, 10) - Number.parseInt(win_height, 10)) / 2
            left =
              (Number.parseInt(main_width, 10) - Number.parseInt(this.initConfig.width, 10)) / 2
          ...
          ;(group || this.cmbFuncGroup.getValue() === "Last10") &&
            this.fillFunctions(this.cmbFuncGroup.getValue())
        ...
        hide: function () {
          let val = this.cmbFuncGroup.getValue()
          val === "Recommended" && (val = "Last10")
          if (
            this.cmbFuncGroup.store.length > 0 &&
            this.cmbFuncGroup.store.at(0).get("value") === "Recommended"
          ) {
        ...
        _handleInput: function (state) {
          if (this.handler && state === "ok") {
            if (this.btnOk.isDisabled()) return
        ...
        fillFormulasGroups: function () {
          if (this.formulasGroups) {
            let lang = Common.Utils.InternalSettings.get("sse-settings-func-locale")
            if (_.isEmpty(lang)) lang = "en"

            let i
            const groupsListItems = []
            const length = this.formulasGroups.length

            for (i = 0; i < length; ++i) {
              const group = this.formulasGroups.at(i)
            ...
            const group = this.formulasGroups.findWhere({ name: "All" })
        ...
        fillFunctions: function (name) {
          if (this.formulasGroups) {
            ...
            if (this.functions) {
              ...
              this.applyFunction = length
                ? { name: functions[0].get("name"), origin: functions[0].get("origin") }
                : undefined

              this.syntaxLabel.text(
                length ? this.applyFunction.name + functions[0].get("args") : "",
              )
        ...
        filterFormulas: function () {
          ...
          const filter_reg = new RegExp(this.filter, "ig")
          const arr = this.allFunctions.filter((item) => !!item.get("name").match(filter_reg))
          if (arr.length > 0) {
            if (this.cmbFuncGroup.store.at(0).get("value") !== "Recommended") {
              this.cmbFuncGroup.store.unshift({
                value: "Recommended",
                displayValue: this.txtRecommended,
              })
              this.cmbFuncGroup.onResetItems()
            }
            const idx = _.findIndex(arr, (item) => item.get("name").toLowerCase() === this.filter)
            if (idx > 0) {
        ...
        onKeyDown: function (e, event) {
          ...
          let record = null
          const parent = this
          const me = this.cmbListFunctions
          let charVal = ""
          let value = ""
          let firstRecord = null
          const recSelect = false
          let innerEl = null
          let isEqualSelectRecord = false
          let selectRecord = null
          const needNextRecord = false
          ...
          if (e.keyCode > 64 && e.keyCode < 91 && charVal && charVal.length) {
            ...
            if (selectRecord) {
              ...
              isEqualSelectRecord = value?.length && value[0].toLocaleLowerCase() === charVal
            }
        ...
        onScrollToRecordCustom: function (record) {
          ...
          const div = innerEl.find(`#${record.get("id")}`).parent()
