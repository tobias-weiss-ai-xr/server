...
define([], () => {
  SSE.Views.HeaderFooterDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 650,
        ...
        initialize: function (options) {
          ...
          this.template = [
            ...
            '<button type="button" class="btn btn-text-default auto" id="id-dlg-hf-btn-all">',
            this.textAll,
            "</button>",
            '<button type="button" class="btn btn-text-default auto hidden" id="id-dlg-hf-btn-odd">',
            this.textOdd,
            "</button>",
            '<button type="button" class="btn btn-text-default auto hidden" id="id-dlg-hf-btn-even">',
            this.textEven,
            "</button>",
            '<button type="button" class="btn btn-text-default auto hidden" id="id-dlg-hf-btn-first">',
            this.textFirst,
            "</button>",
            "</div>",
            ...
            "</tr>",
            "<tr>",
            '<td class="display-flex-row-center">',
            ...
            '<div id="id-dlg-h-bold" class="margin-left-2"></div>',
            '<div id="id-dlg-h-italic" class="margin-left-6"></div>',
            '<div id="id-dlg-h-underline" class="margin-left-6"></div>',
            '<div id="id-dlg-h-strikeout" class="margin-left-6"></div>',
            '<div id="id-dlg-h-subscript" class="margin-left-6"></div>',
            '<div id="id-dlg-h-superscript" class="margin-left-6"></div>',
            "</td>",
            "</tr>",
            "<tr>",
            "<td>",
            ...
            '<div id="id-dlg-f-bold" class="margin-left-2"></div>',
            '<div id="id-dlg-f-italic" class="margin-left-6"></div>',
            '<div id="id-dlg-f-underline" class="margin-left-6"></div>',
            '<div class="margin-left-6" id="id-dlg-f-strikeout"></div>',
            '<div id="id-dlg-f-subscript" class="margin-left-6"></div>',
            '<div id="id-dlg-f-superscript" class="margin-left-6"></div>',
            "</td>",
            "</tr>",
            "<tr>",
            "<td>",
        ...
        render: function () {
          ...
          this.chFirstPage.on(
            ...
            _.bind(function (field, newValue, oldValue, eOpts) {
              ...
              const id = this.HFObject ? this.HFObject.setDifferentFirst(checked) : null
              if (id) {
                this.showError(() => {
                  field.setValue(!checked, true)
                  _.delay(() => {
                    this.onCanvasClick(id)
                  }, 50)
                })
                return
              }
          ...
          this.chOddPage.on(
            ...
            _.bind(function (field, newValue, oldValue, eOpts) {
              const checked = field.getValue() === "checked"
              const id = this.HFObject ? this.HFObject.setDifferentOddEven(checked) : null
              if (id) {
                this.showError(() => {
                  field.setValue(!checked, true)
                  _.delay(() => {
              ...
              if (checked && this.btnAll.isActive()) this.btnOdd.toggle(true)
            }, this),
          )
          ...
          this.chScale.on(
            ...
            _.bind(function (field, newValue, oldValue, eOpts) {
              const checked = field.getValue() === "checked"
              if (this.HFObject) this.HFObject.setScaleWithDoc(checked)
            }, this),
          )
          ...
          this.chAlign.on(
            ...
            _.bind(function (field, newValue, oldValue, eOpts) {
              const checked = field.getValue() === "checked"
              if (this.HFObject) this.HFObject.setAlignWithMargins(checked)
            }, this),
          )
          ...
          this.btnAll.on(
            "toggle",
            _.bind(this.onPageTypeToggle, this, Asc.c_oAscHeaderFooterType.odd),
          )
          ...
          this.btnEven.on(
            "toggle",
            _.bind(this.onPageTypeToggle, this, Asc.c_oAscHeaderFooterType.even),
          )
          ...
          this.btnFirst.on(
            "toggle",
            _.bind(this.onPageTypeToggle, this, Asc.c_oAscHeaderFooterType.first),
          )

          Common.UI.GroupedButtons([this.btnAll, this.btnOdd, this.btnEven, this.btnFirst], {
            underline: true,
          })
          ...
          const initNewColor = (btn) => {
            ...
            btn.on("color:select", _.bind(this.onColorSelect, this))
            return picker
          }
          this.btnTextColor = []
          this.btnTextColor.push(
            new Common.UI.ButtonColored({
        ...
        initCanvas: function (name) {
          const el = this.$window.find(name)
        ...
        close: function () {
          this.api.asc_unregisterCallback(
            ...
            this.wrapEvents.onApiEditorSelectionChanged,
          )
          this.api.asc_unregisterCallback(
            "asc_resizeEditorHeight",
            this.wrapEvents.onApiResizeEditorHeight,
          )
          this.api.asc_unregisterCallback(
            "asc_updateEditorCursorPosition",
            this.wrapEvents.onUpdateEditorCursorPosition,
          )

          Common.UI.Window.prototype.close.apply(this, arguments)

          if (this.HFObject) this.HFObject.destroy(false, this.pageSetup)
        },
        ...
        afterRender: function () {
          ...
          _.delay(() => {
            this.onCanvasClick("#header-left-img")
          }, 500)
        },

        _setDefaults: function (props) {
          const presets = []
          this.HFObject.getTextPresetsArr().forEach((item, index) => {
            presets.push({ caption: item, value: index })
          })

          this.btnPresetsH.setMenu(
            new Common.UI.Menu({
          ...
          let value = this.chOddPage.getValue() === "checked"
        ...
        _handleInput: function (state) {
          if (this.HFObject) {
            const id = this.HFObject.destroy(state === "ok", this.pageSetup)
            if (id) {
              this.showError(() => {
                _.delay(() => {
                  this.onCanvasClick(id)
        ...
        scrollerUpdate: function () {
          for (const name in this.scrollers) {
            this.scrollers[name]?.update()
          }
        },
        ...
        onCanvasClick: function (id, event) {
          ...
          const diff = this.currentCanvas !== id
          if (diff) {
            this.currentCanvas = id
            this.isFooter =
              id === "#footer-left-img" || id === "#footer-center-img" || id === "#footer-right-img"
            this.headerControls.forEach((item) => {
              item.setDisabled(this.isFooter)
            })
            this.footerControls.forEach((item) => {
              item.setDisabled(!this.isFooter)
            })
          }
        ...
        onApiResizeEditorHeight: function (event) {
          ...
          if (this.scrollers[this.currentCanvas]) this.scrollers[this.currentCanvas].update()
        },

        onUpdateEditorCursorPosition: function (pos, height) {
          if (!this.editorCanvas) return
          const id = this.currentCanvas
          if (this.scrollers[id]) {
            const top = this.scrollers[id].getScrollTop()
            if (pos + height > top + this.canvasBoxHeight)
              this.scrollers[id].scrollTop(pos + height - this.canvasBoxHeight)
            else if (pos < top) this.scrollers[id].scrollTop(pos)
          }
        },

        onPresetSelect: function (footer, menu, item) {
          if (this.HFObject) this.HFObject.applyPreset(item.value, !!footer)
          this.onCanvasClick(footer ? "#footer-left-img" : "#header-left-img")
        },

        onObjectSelect: function (menu, item) {
          if (this.HFObject) this.HFObject.addField(item.value)
          this.onCanvasClick(this.currentCanvas)
        },

        onFontSelect: function (combo, record) {
          if (this.HFObject) this.HFObject.setFontName(record.name)
          this.onCanvasClick(this.currentCanvas)
        },

        onFontSizeSelect: function (combo, record) {
          if (this.HFObject) this.HFObject.setFontSize(record.value)
          this.onCanvasClick(this.currentCanvas)
        },
        ...
        onFontSizeChanged: function (before, combo, record, e) {
          ...
          if (before) {
            ...
            combo.setRawValue(value)
            if (this.HFObject) this.HFObject.setFontSize(value)
          }
        },
        ...
        onTextColor: function () {
          const mnuTextColorPicker = this.mnuTextColorPicker[this.isFooter ? 1 : 0]
          mnuTextColorPicker.trigger(
            "select",
            mnuTextColorPicker,
            mnuTextColorPicker.currentColor,
            true,
          )
        },
        ...
        onColorSelect: function (btn, color) {
          ...
          if (this.HFObject) this.HFObject.setTextColor(Common.Utils.ThemeColor.getRgbColor(color))
          this.onCanvasClick(this.currentCanvas)
        },
        ...
        onPageTypeToggle: function (type, btn, state) {
          ...
          if (state && this.HFObject) {
            const prev = this.HFObject.getPageType()
            const id = this.HFObject.switchHeaderFooterType(type)
            if (id) {
              this._pagetype = true
              this.showError(() => {
                switch (prev) {
                  case Asc.c_oAscHeaderFooterType.odd:
                    this.btnOdd.isVisible() ? this.btnOdd.toggle(true) : this.btnAll.toggle(true)
                    break
                  case Asc.c_oAscHeaderFooterType.even:
                    this.btnEven.toggle(true)
                    break
                  case Asc.c_oAscHeaderFooterType.first:
                    this.btnFirst.toggle(true)
                    break
                }
                _.delay(() => {
                  this.onCanvasClick(id)
        ...
        onApiEditorSelectionChanged: function (fontobj) {
          const idx = this.isFooter ? 1 : 0
          let val

          /* read font name */
          const fontparam = fontobj.asc_getFontName()
          ...
          if (this.btnBold[idx].isActive() !== val) this.btnBold[idx].toggle(val === true, true)

          val = fontobj.asc_getFontItalic()
          if (this.btnItalic[idx].isActive() !== val) this.btnItalic[idx].toggle(val === true, true)
          ...
          /* read font size */
          const str_size = fontobj.asc_getFontSize()
          ...
          let clr
          let color
          const fontColorPicker = this.mnuTextColorPicker[idx]
