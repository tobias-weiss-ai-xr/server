...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  SSE.Views.MacroDialog = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 250,
        ...
        render: function () {
          ...
          this.macroList = new Common.UI.ListView({
            ...
            itemTemplate: _.template(
              '<div id="<%= id %>" class="list-item" style="overflow: hidden; text-overflow: ellipsis;width:216px;"><%= Common.Utils.String.htmlEncode(value) %></div>',
            ),
          })
        ...
        onPrimary: function () {
          this.handler?.call(this, "ok", this.getSettings())
          this.close()
          return false
        },
        ...
        refreshList: function () {
          if (this.props?.macroList) {
            const list = this.props.macroList
            const arr = []
        ...
        findMacro: function (value) {
          const rec = this.macroList.store.findWhere({ value: value })
          if (rec) {
            this.macroList.selectRecord(rec)
            this.macroList.scrollToRecord(rec)
          } else this.macroList.deselectAll()
        },
