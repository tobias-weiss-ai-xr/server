...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  ...
  SSE.Views.NamedRangeEditDlg = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
        ...
        render: function () {
          ...
          this.inputName = new Common.UI.InputField({
            ...
            style: "width: 100%;",
            validation: (value) => {
              const isvalid = this.api.asc_checkDefinedName(
                value,
                this.cmbScope.getValue() === -255 ? null : this.cmbScope.getValue(),
              )
              if (isvalid.asc_getStatus() === true) return true

              switch (isvalid.asc_getReason()) {
                case Asc.c_oAscDefinedNameReason.IsLocked:
                  return this.textIsLocked
                  break
                case Asc.c_oAscDefinedNameReason.Existed:
                  return this.isEdit &&
                    this.props.asc_getName(true).toLowerCase() === value.toLowerCase()
                    ? true
                    : this.textExistName
                case Asc.c_oAscDefinedNameReason.NameReserved:
                  return this.isEdit ? this.textReservedName : true
                default:
                  return this.textInvalidName
              }
            },
          })
          this.inputName._input.on("input", (input, value) => {
            this.isInputFirstChange && this.inputName.showError()
          ...
          this.txtDataRange = new Common.UI.InputFieldBtn({
            ...
            validation: (value) => {
              ...
              return isvalid !== Asc.c_oAscError.ID.DataRangeError ||
                (this.isEdit && this.props.asc_getRef().toLowerCase() === value.toLowerCase())
                ? true
                : this.textInvalidRange
            },
          })
        ...
        _setDefaults: function (props) {
          this.cmbScope.setData(
            [{ value: -255, displayValue: this.strWorkbook }].concat(this.sheets),
          )

          if (props) {
            let val = props.asc_getScope()
            ...
            this.txtDataRange.setDisabled(
              this.isEdit &&
                (props.asc_getType() === Asc.c_oAscDefNameType.table ||
                  props.asc_getType() === Asc.c_oAscDefNameType.slicer),
            )
          } else this.cmbScope.setValue(-255)
        ...
        getSettings: function () {
          return new Asc.asc_CDefName(
            this.inputName.getValue(),
            this.txtDataRange.getValue(),
            this.cmbScope.getValue() === -255 ? null : this.cmbScope.getValue(),
            this.props.asc_getType(),
            undefined,
            undefined,
            undefined,
            true,
        ...
        onDlgBtnClick: function (event) {
          ...
          if (state === "ok") {
            ...
            const checkname = this.inputName.checkValidate()
            const checkrange = this.txtDataRange.checkValidate()
            if (checkname !== true) {
        ...
        onRefreshDefNameList: function (name) {
          if (this.isEdit && Common.Utils.InternalSettings.get("sse-settings-coauthmode")) {
            // fast co-editing
            if (
              name?.asc_getIsLock() &&
              name.asc_getName(true).toLowerCase() === this.props.asc_getName(true).toLowerCase() &&
              ((name.asc_getScope() === null && this.props.asc_getScope() === null) ||
                name.asc_getScope().toLowerCase() === this.props.asc_getScope().toLowerCase()) &&
              !this._listRefreshed
            ) {
