...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  ...
  SSE.Views.NamedRangePasteDlg = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
        ...
        _setDefaults: function () {
          if (this.ranges) {
            ...
            for (let i = 0; i < this.ranges.length; i++) {
              const name = this.ranges[i].asc_getName(true)
              if (name !== prev_name) {
            ...
            if (this.rangeList.store.length > 0) this.rangeList.selectByIndex(0)
            this.rangeList.scroller.update({ alwaysVisibleY: true })

            _.delay(
              () => {
                this.rangeList.focus()
              },
              100,
              this,
            )
