...
define([], () => {
  SSE.Views.PageMarginsDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 215,
        ...
        _handleInput: function (state) {
          if (this.options.handler) {
            if (state === "ok") {
              let result = null
              const top = this.spnTop.getNumberValue()
              const bottom = this.spnBottom.getNumberValue()
              const left = this.spnLeft.getNumberValue()
              const right = this.spnRight.getNumberValue()
              if (left > this.maxMarginsW) result = "left"
              else if (right > this.maxMarginsW - left) result = "right"
              else if (top > this.maxMarginsH) result = "top"
              else if (bottom > this.maxMarginsH - top) result = "bottom"
              if (result) {
                Common.UI.warning({
        ...
        setSettings: function (props) {
          if (props) {
            const margins = props.asc_getPageMargins()
            ...
            this.spnBottom.setValue(
              Common.Utils.Metric.fnRecalcFromMM(margins.asc_getBottom()),
              true,
            )
        ...
        getSettings: function () {
          const props = new Asc.asc_CPageMargins()
          ...
          return {
            margins: props,
            vertical: this.chVert.getValue() === "checked",
            horizontal: this.chHor.getValue() === "checked",
          }
        },
