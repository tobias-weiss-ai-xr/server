...
define([
  ...
  SSE.Views.ParagraphSettings = Backbone.View.extend(
    _.extend(
      {
        ...
        onNumSpacingBeforeChange: function (field, newValue, oldValue, eOpts) {
          if (this.api) {
            ...
            if (num < 0) this.api.asc_putLineSpacingBeforeAfter(0, -1)
            else
              this.api.asc_putLineSpacingBeforeAfter(
                0,
                Common.Utils.Metric.fnRecalcToMM(field.getNumberValue()),
              )
          }
        ...
        onNumSpacingAfterChange: function (field, newValue, oldValue, eOpts) {
          if (this.api) {
            const num = field.getNumberValue()
            if (num < 0) this.api.asc_putLineSpacingBeforeAfter(1, -1)
            else
              this.api.asc_putLineSpacingBeforeAfter(
                1,
                Common.Utils.Metric.fnRecalcToMM(field.getNumberValue()),
        ...
        onLineRuleSelect: function (combo, record) {
          if (this.api) this.api.asc_putPrLineSpacing(record.value, record.defaultValue)
        ...
        _onLineSpacing: function (value) {
          ...
          const linerule = value.asc_getLineRule()
          const line = value.asc_getLine()

          if (this._state.LineRule !== linerule) {
            let obj
            let rec = this.cmbLineRule.store.findWhere(((obj = {}), (obj.value = linerule), obj))
        ...
        ChangeSettings: function (prop) {
          ...
          if (prop) {
            ...
            if (this._state.LineRule !== Spacing.LineRule) {
              let obj
              let rec = this.cmbLineRule.store.findWhere(
                ((obj = {}), (obj.value = Spacing.LineRule), obj),
              )
              if (!rec) rec = this.cmbLineRule.store.at(0)
        ...
        updateMetricUnit: function () {
          ...
          if (this.cmbLineRule) {
            let rec = this.cmbLineRule.store.at(1)
            rec.set({
              defaultUnit: Common.Utils.Metric.getCurrentMetricName(),
              minValue: Number.parseFloat(Common.Utils.Metric.fnRecalcFromMM(0.3).toFixed(2)),
              step:
                Common.Utils.Metric.getCurrentMetric() === Common.Utils.Metric.c_MetricUnits.pt
                  ? 1
                  : 0.01,
            ...
            if (this._state.LineRule !== null) {
              let obj
              rec = this.cmbLineRule.store.findWhere(
                ((obj = {}), (obj.value = this._state.LineRule), obj),
              )
              if (!rec) rec = this.cmbLineRule.store.at(0)
              this.numLineHeight.setDefaultUnit(rec.get("defaultUnit"))
              this.numLineHeight.setStep(rec.get("step"))
        ...
        createDelayedControls: function () {
          ...
          $(this.el).on(
            ...
            _.bind(this.openAdvancedSettings, this),
          )
        },
