...
define([], () => {
  SSE.Views.PivotGroupDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 250,
        ...
        initialize: function (options) {
          const height = options.date ? 250 : 110
          ...
          this.template =
            ...
            [
              ...
              '<td class="padding-right-10" style="padding-bottom: 8px;width: 100%;">',
              `<label class="margin-left-22" style="margin-top:2px;">${this.textBy}</label>`,
              "</td>",
              '<td style="padding-bottom: 8px;">',
              ...
              `<label style="margin-top:2px;">${this.textNumDays}</label>`,
              "</td>",
              "<td>",
        ...
        render: function () {
          ...
          this.chStart.on("change", (field, newValue, oldValue, eOpts) => {
            if (newValue === "checked" && this.defRangePr) {
              this.inputStart.setValue(this.defRangePr.start)
            }
          })
          ...
          this.chEnd.on("change", (field, newValue, oldValue, eOpts) => {
            if (newValue === "checked" && this.defRangePr) {
              this.inputEnd.setValue(this.defRangePr.end)
            }
          })
          ...
          this.inputStart = new Common.UI.InputField({
            ...
            this.chStart.setValue(false, true)
          })
          ...
          this.inputEnd = new Common.UI.InputField({
            ...
            this.chEnd.setValue(false, true)
          })
          ...
          this.listDate = new Common.UI.ListView({
            ...
            itemTemplate: _.template(
              '<div id="<%= id %>" class="list-item" style="pointer-events:none;"><%= value %></div>',
            ),
          })
        ...
        onSelectDate: function (listView, itemView, record) {
          ...
          if (record.get("type") === Asc.c_oAscGroupBy.Days) this.isDays = record.get("selected")
          const selected = this.listDate.getSelectedRec().length
        ...
        setSettings: function (rangePr, dateTypes, defRangePr) {
          ...
          if (rangePr) {
            ...
            let value = dateTypes ? rangePr.asc_getStartDateText() : rangePr.asc_getStartNum()
            ...
            if (defRangePr) {
              this.defRangePr = {
                start: dateTypes ? defRangePr.asc_getStartDateText() : defRangePr.asc_getStartNum(),
                end: dateTypes ? defRangePr.asc_getEndDateText() : defRangePr.asc_getEndNum(),
              }
            }
          }
          if (dateTypes) {
            ...
            _.each(dateTypes, (item) => {
              const rec = this.listDate.store.findWhere({ type: item })
              rec && this.listDate.selectRecord(rec)
              if (item === Asc.c_oAscGroupBy.Days) isDays = true
            })
        ...
        getSettings: function () {
          if (this.rangePr) {
            ...
            if (this.dateTypes) {
              this.dateTypes = this.listDate.getSelectedRec().map((item) => item.get("type"))
              ...
              this.rangePr.asc_setStartNum(
                Number.parseFloat(this.inputStart.getValue().toString().replace(",", ".")),
              )
              this.rangePr.asc_setEndNum(
                Number.parseFloat(this.inputEnd.getValue().toString().replace(",", ".")),
              )
              this.rangePr.asc_setGroupInterval(
                Number.parseFloat(this.inputBy.getValue().toString().replace(",", ".")),
              )
            }
          }
        ...
        isRangeValid: function () {
          if (this.dateTypes) {
            if (this.rangePr) {
              this.rangePr.asc_setStartDateText(this.inputStart.getValue())
              const res1 = this.rangePr.asc_getStartDate()
              ...
              this.rangePr.asc_setEndDateText(this.inputEnd.getValue())
              const res2 = this.rangePr.asc_getEndDate()
            ...
            const res1 = this.inputStart.getValue().toString()
            if (!regstr.test(res1)) {
              this.inputStart.showError([this.textError])
              this.inputStart.focus()
              return false
            }
            const res2 = this.inputEnd.getValue().toString()
            if (!regstr.test(res2)) {
              this.inputEnd.showError([this.textError])
              this.inputEnd.focus()
              return false
            ...
            if (
              Number.parseFloat(res2.replace(",", ".")) < Number.parseFloat(res1.replace(",", "."))
            ) {
              Common.UI.warning({ msg: this.textGreaterError, maxwidth: 600 })
              this.inputEnd.focus()
              return false
