...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  SSE.Views.PivotInsertCalculatedItemDialog = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 290,
        ...
        render: function () {
          ...
          this.$window
            .find("#pivot-insert-calculated-help")
            .on("click", _.bind(this.showHelp, this))
        },

        getNewItemName: function () {
          let indexMax = 0
          const textTranslate = this.txtItem
          this.itemsList.store.each((item) => {
            const itemName = item.get("name")
            if (0 === itemName.indexOf(textTranslate)) {
              const index = Number.parseInt(itemName.substr(textTranslate.length))
              if (!Number.isNaN(index) && indexMax < index) indexMax = index
            }
          })
          indexMax++
        ...
        onAddItemInFormula: function (name) {
          let formulaValue = this.inputFormula.getValue()
          const formulaValueNoSpaces = formulaValue.trim().replaceAll(" ", "")
          if (
            formulaValueNoSpaces === "=0" ||
            formulaValueNoSpaces === "=" ||
            formulaValueNoSpaces === ""
          ) {
            formulaValue = "="
          }
        ...
        onDlgBtnClick: function (event) {
          ...
          if (this.handler) {
            if (state === "ok") {
              ...
              // Validate formula
              const convertedFormula = this.getConvertedFormula(returnedItem.formula)
        ...
        addTooltipItemsList: function () {
          this.itemsList.dataViewItems.forEach((itemView) => {
            ...
            const arr = []
            const btns = $(itemView.el).find(".list-item-icon")
            btns.tooltip({
              title: this.txtInsertIntoFormula,
              placement: "cursor",
              zIndex: this.itemsList.tipZIndex,
            })
            btns.each((idx, item) => {
        ...
        showHelp: function () {
          if (this.helpUrl === undefined) {
            let lang = Common.Locale.getCurrentLanguage()
            lang = lang ? lang.split(/[\-\_]/)[0] : "en"
            const name = "/UsageInstructions/PivotTables.htm"
        ...
        getConvertedFormula: function (formula) {
          ...
          if (formulaTrim[0] !== "=") {
            ...
            convertedFormula = this.pivotInfo.asc_convertCalculatedFormula(
              formulaTrim.slice(1),
        ...
        txtItems: "Items",
        txtDescription:
          "You can use Calculated Items for basic calculations between different items within a single field",
