...
define([
  ...
  SSE.Views.PivotSettings = Backbone.View.extend(
    _.extend(
      {
        ...
        getDragElement: function (value) {
          this._dragEl = $(
            `<div style="font-weight: bold;position: absolute;left:-10000px;z-index: 10000;">${Common.Utils.String.htmlEncode(value)}</div>`,
          )
        ...
        onFieldsDragStart: function (item, index, event) {
          ...
          !Common.Utils.isIE &&
            event.originalEvent.dataTransfer.setDragImage(
              this.getDragElement(item.model.get("value")),
              14,
              14,
            )
        ...
        onDragItemOver: function (listview, item, index, event) {
          if (
            this.pivotIndex === -2 &&
            (this.enterListView === "pivot-list-filters" ||
              this.enterListView === "pivot-list-values")
          ) {
            ...
            // scroll
            const heightListView = item.$el.parent().height()
            const positionTopItem = Common.Utils.getPosition(item.$el).top
            const heightItem = item.$el.outerHeight()
            const scrollTop = item.$el.parent().scrollTop()
            ...
            if (positionTopItem > heightListView - heightItem && index < listview.store.length) {
              listview.scrollToRecord(
                listview.store.at(index === listview.store.length - 1 ? index : index + 1),
              )
            }
        ...
        onDragOver: function (listview, event) {
          if (
            event.originalEvent.dataTransfer.types[0] === "Word Office" ||
            (this.pivotIndex === -2 &&
              (this.enterListView === "pivot-list-filters" ||
                this.enterListView === "pivot-list-values")) ||
            (this.fromListView === "pivot-list-fields" &&
              this.enterListView === "pivot-list-fields")
          ) {
            event.originalEvent.dataTransfer.dropEffect = "none"
          } else {
            event.preventDefault() // Necessary. Allows us to drop.
        ...
        onDrop: function (event) {
          ...
          } else if (this.enterListView === "pivot-list-fields") {
            //remove field
            if (_.isNumber(this.pivotIndex)) {
                ...
                switch (this.enterListView) {
                  case "pivot-list-columns":
                    this.onMove(
                      0,
                      this.itemIndex,
                      _.isNumber(this.indexMoveTo)
                        ? this.indexMoveTo !== 0 && this.itemIndex < this.indexMoveTo
                          ? this.indexMoveTo - 1
                          : this.indexMoveTo
                    ...
                    this.onMove(
                      ...
                      _.isNumber(this.indexMoveTo)
                        ? this.indexMoveTo !== 0 && this.itemIndex < this.indexMoveTo
                          ? this.indexMoveTo - 1
                          : this.indexMoveTo
                        : this.filtersList.store.length - 1,
                    )
                    break
                  case "pivot-list-values":
                    this.onMove(
                      ...
                      _.isNumber(this.indexMoveTo)
                        ? this.indexMoveTo !== 0 && this.itemIndex < this.indexMoveTo
                          ? this.indexMoveTo - 1
                          : this.indexMoveTo
                        : this.valuesList.store.length - 1,
                    )
                    break
                }
              }
        ...
        openAdvancedSettings: function (e) {
          ...
          if (this.api && !this._locked) {
            new SSE.Views.PivotSettingsAdvanced({
              props: this._originalProps,
              api: this.api,
              handler: (result, value) => {
                if (result === "ok" && this.api && value) {
                  this._originalProps.asc_set(this.api, value)
        ...
        ChangeSettings: function (props) {
          if (this._initSettings) this.createDelayedControls()
          ...
          if (props) {
            ...
            const isChecked = []
            const cache_names = props.asc_getCacheFields()
            const pivot_names = props.asc_getPivotFields()

            this._state.names = []
            pivot_names.forEach((item, index) => {
              this._state.names[index] = item.asc_getName() || cache_names[index].asc_getName()
            })

            const fillList = (propValue, list, eventIndex, getNameFunction) => {
              const arr = []
              const models = list.store.models
              const equalArr = list.store.length === (propValue ? propValue.length : 0)
              propValue?.forEach((item, index) => {
                const pivotIndex = item.asc_getIndex()
                const name = getNameFunction ? getNameFunction(pivotIndex) : item.asc_getName()
                if (equalArr) {
                  models[index].set({
                ...
                isChecked[getNameFunction ? name : this._state.names[pivotIndex]] = true
              })
              if (!equalArr) {
                list.store.reset(arr)
                list.scroller.update({
                  minScrollbarLength: list.minScrollbarLength,
                  alwaysVisibleY: true,
                  suppressScrollX: true,
                })
                list.dataViewItems.forEach((item, index) => {
                  item.$el.attr("draggable", true)
                  item.$el.on(
                    "dragstart",
                    _.bind(this.onItemsDragStart, this, eventIndex, list, item, index),
                  )
                  item.$el.on("dragenter", _.bind(this.onDragItemEnter, this, item, index))
                  item.$el.on("dragleave", _.bind(this.onDragItemLeave, this, item, index))
                  item.$el.on("dragover", _.bind(this.onDragItemOver, this, list, item, index))
                  item.$el.on("drop", _.bind(this.onDrop, this))
                  item.$el.on("dragend", _.bind(this.onDragEnd, this))
                })
                list.$el.find(".item").last().css({ "margin-bottom": "10px" })
              }
            }

            let value = props.asc_getColumnFields()
            value &&
              (value = _.filter(value, (item) => {
                const pivotIndex = item.asc_getIndex()
                return pivotIndex > -1 || pivotIndex === -2
              }))
            fillList(value, this.columnsList, 0, (pivotIndex) =>
              pivotIndex > -1 ? this._state.names[pivotIndex] : this.textValues,
            )
            ...
            fillList(value, this.rowsList, 1, (pivotIndex) =>
              pivotIndex > -1 ? this._state.names[pivotIndex] : this.textValues,
            )

            value = props.asc_getDataFields()
            value && (value = _.filter(value, (item) => item.asc_getIndex() > -1))
            fillList(value, this.valuesList, 2)

            value = props.asc_getPageFields()
            value && (value = _.filter(value, (item) => item.asc_getIndex() > -1))
            fillList(value, this.filtersList, 3, (pivotIndex) => this._state.names[pivotIndex])

            const arr = []
            const models = this.fieldsList.store.models
            const equalArr = this.fieldsList.store.length === this._state.names.length
            this._state.names.forEach((item, index) => {
              if (equalArr) {
                models[index].set({
                  value: item,
                  index: index,
            ...
            if (!equalArr) {
              ...
              this.fieldsList.scroller.update({
                minScrollbarLength: this.fieldsList.minScrollbarLength,
                alwaysVisibleY: true,
                suppressScrollX: true,
              })
              this.fieldsList.dataViewItems.forEach((item, index) => {
                item.$el.attr("draggable", true)
                item.$el.on("dragstart", _.bind(this.onFieldsDragStart, this, item, index))
                item.$el.on("dragend", _.bind(this.onDragEnd, this))
              })
            }
          }
            ...
            if (btn?.hasClass("listitem-icon")) {
              ...
              if (this.pivotFieldsMenu) {
                ...
                this.pivotFieldsMenu = new Common.UI.Menu({
                  menuAlign: "tr-br",
                  items: [this.miAddFilter, this.miAddRow, this.miAddColumn, this.miAddValues],
                })
              }

              const recIndex = record !== undefined ? record.get("index") : -1

              const menu = this.pivotFieldsMenu
              const currentTarget = $(event.currentTarget)
              const parent = $(this.el)

              let menuContainer = parent.find("#menu-pivot-fields-container")
              if (!menu.rendered) {
                if (menuContainer.length < 1) {
                  menuContainer = $(
                    '<div id="menu-pivot-fields-container" style="position: absolute; z-index: 10000;"><div class="dropdown-toggle" data-toggle="dropdown"></div></div>',
                    menu.id,
                  )
        ...
        onColumnsSelect: function (type, picker, item, record, e) {
          ...
          const recIndex = record !== undefined ? record.get("index") : -1
          const len = picker.store.length
          const pivotIndex = record.get("pivotIndex")
          ...
          const menu = this.fieldsMenu
          const currentTarget = $(e.currentTarget)
          const parent = $(this.el)

          let menuContainer = parent.find("#menu-pivot-container")
          if (!menu.rendered) {
            if (menuContainer.length < 1) {
              menuContainer = $(
                '<div id="menu-pivot-container" style="position: absolute; z-index: 10000;"><div class="dropdown-toggle" data-toggle="dropdown"></div></div>',
                menu.id,
              )
              parent.append(menuContainer)
            }
        ...
        onFieldSettings: function (record, type, e) {
          ...
          if (this.api && !this._locked && this._state.field) {
            if (this._state.field.type === 2) {
              // value field
              const dataIndex = this._state.field.record.get("index")
              const field = this._originalProps.asc_getDataFields()[dataIndex]
              new SSE.Views.ValueFieldSettingsDialog({
                ...
                handler: (result, value) => {
                  if (result === "ok" && this.api && value) {
                    field.asc_set(this.api, this._originalProps, dataIndex, value)
                    Common.NotificationCenter.trigger("edit:complete", this)
                  }
              ...
              const pivotIndex = this._state.field.record.get("pivotIndex")
              const pivotField = this._originalProps.asc_getPivotFields()[pivotIndex]
              new SSE.Views.FieldSettingsDialog({
                props: this._originalProps,
                fieldIndex: pivotIndex,
                api: this.api,
                type: this._state.field.type,
                handler: (result, value) => {
                  if (result === "ok" && this.api && value) {
                    pivotField.asc_set(this.api, this._originalProps, pivotIndex, value)
                    Common.NotificationCenter.trigger("edit:complete", this)
                  }
        ...
        onAddFilter: function (index, moveTo) {
          if (this.api && !this._locked && this._state.field) {
            this._originalProps.asc_addPageField(
              this.api,
              _.isNumber(index) ? index : this._state.field.record.get("index"),
              _.isNumber(moveTo) ? moveTo : undefined,
            )
            Common.NotificationCenter.trigger("edit:complete", this)
          }
        },
        ...
        onAddRow: function (index, moveTo) {
          if (this.api && !this._locked && this._state.field) {
            this._originalProps.asc_addRowField(
              this.api,
              _.isNumber(index) ? index : this._state.field.record.get("index"),
              _.isNumber(moveTo) ? moveTo : undefined,
            )
        ...
        onAddColumn: function (index, moveTo) {
          if (this.api && !this._locked && this._state.field) {
            this._originalProps.asc_addColField(
              this.api,
              _.isNumber(index) ? index : this._state.field.record.get("index"),
              _.isNumber(moveTo) ? moveTo : undefined,
            )
        ...
        onAddValues: function (index, moveTo) {
          if (this.api && !this._locked && this._state.field) {
            this._originalProps.asc_addDataField(
              this.api,
              _.isNumber(index) ? index : this._state.field.record.get("index"),
              _.isNumber(moveTo) ? moveTo : undefined,
            )
        ...
        onRemove: function (pivotindex, index) {
          if (this.api && !this._locked && this._state.field) {
            if (this._state.field.type === 2 || _.isNumber(index))
              // value
              this._originalProps.asc_removeDataField(
                this.api,
                _.isNumber(pivotindex) ? pivotindex : this._state.field.record.get("pivotIndex"),
                _.isNumber(index) ? index : this._state.field.record.get("index"),
              )
            else
              this._originalProps.asc_removeNoDataField(
                this.api,
                _.isNumber(pivotindex) ? pivotindex : this._state.field.record.get("pivotIndex"),
              )
            Common.NotificationCenter.trigger("edit:complete", this)
        ...
        onMoveUp: function () {
          if (this.api && !this._locked && this._state.field) {
            const index = this._state.field.record.get("index")
        ...
        onMoveTo: function (type, pivotindex, to) {
          if (this.api && !this._locked && this._state.field) {
            const pivotIndex = _.isNumber(pivotindex)
              ? pivotindex
              : this._state.field.record.get("pivotIndex")
            const index = _.isNumber(to) ? to : undefined
            const dataIndex =
              this._state.field.type === 2 ? this._state.field.record.get("index") : undefined
            switch (type) {
              case 0:
