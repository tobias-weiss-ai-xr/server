...
define([
  ...
  SSE.Views.PivotTable = Common.UI.BaseView.extend(
    _.extend(
      (() => {
        const template =
        ...
        function setEvents() {
          this.btnsAddPivot.forEach((button) => {
            button.on("click", (b, e) => {
              this.fireEvent("pivottable:create")
            })
          })

          this.btnPivotLayout.menu.on("item:click", (menu, item, e) => {
            this.fireEvent("pivottable:layout", [item.value])
          })

          this.btnPivotBlankRows.menu.on("item:click", (menu, item, e) => {
            this.fireEvent("pivottable:blankrows", [item.value])
          })

          this.btnPivotSubtotals.menu.on("item:click", (menu, item, e) => {
            this.fireEvent("pivottable:subtotals", [item.value])
          })

          this.btnPivotGrandTotals.menu.on("item:click", (menu, item, e) => {
            this.fireEvent("pivottable:grandtotals", [item.value])
          })

          this.btnRefreshPivot.on("click", (e) => {
            this.fireEvent("pivottable:refresh", ["current"])
          })

          this.btnRefreshPivot.menu.on("item:click", (menu, item, e) => {
            this.fireEvent("pivottable:refresh", [item.value])
          })

          this.btnSelectPivot.on("click", (e) => {
            this.fireEvent("pivottable:select")
          })

          this.btnCalculatedItems.on("click", (e) => {
            this.fireEvent("pivottable:calculated")
          })

          this.btnExpandField.on("click", (e) => {
            this.fireEvent("pivottable:expand")
          })

          this.btnCollapseField.on("click", (e) => {
            this.fireEvent("pivottable:collapse")
          })

          this.chRowHeader.on("change", (field, value) => {
            this.fireEvent("pivottable:rowscolumns", [0, value])
          })
          this.chColHeader.on("change", (field, value) => {
            this.fireEvent("pivottable:rowscolumns", [1, value])
          })
          this.chRowBanded.on("change", (field, value) => {
            this.fireEvent("pivottable:rowscolumns", [2, value])
          })
          this.chColBanded.on("change", (field, value) => {
            this.fireEvent("pivottable:rowscolumns", [3, value])
          })

          this.pivotStyles.on("click", (combo, record) => {
            this.fireEvent("pivottable:style", [record])
          })
          this.pivotStyles.openButton.menu.on("show:after", () => {
            this.pivotStyles.menuPicker.scroller.update({ alwaysVisibleY: true })
          })
        }
        ...
        return {
          ...
          initialize: function (options) {
            ...
            this.btnsAddPivot = Common.Utils.injectButtons(
              this.toolbar.$el.find(".btn-slot.slot-add-pivot"),
              "",
              "toolbar__icon btn-big-pivot-sum",
              this.txtPivotTable,
              [
                _set.lostConnect,
                _set.coAuth,
          ...
          onAppReady: function (config) {
            new Promise((accept, reject) => {
              ...
              this.btnsAddPivot.forEach((btn) => {
                btn.updateHint(this.tipCreatePivot)
              })
              this.btnSelectPivot.updateHint(this.tipSelect)
              this.btnCalculatedItems.updateHint(this.tipCalculatedItems)
              this.btnPivotLayout.updateHint(this.capLayout)
              this.btnPivotLayout.setMenu(
                new Common.UI.Menu({
                  items: [
              ...
              this.btnPivotBlankRows.updateHint(this.capBlankRows)
              this.btnPivotBlankRows.setMenu(
                new Common.UI.Menu({
                  items: [
              ...
              this.btnPivotSubtotals.updateHint(this.tipSubtotals)
              this.btnPivotSubtotals.setMenu(
                new Common.UI.Menu({
                  items: [
              ...
              this.btnPivotGrandTotals.updateHint(this.tipGrandTotals)
              this.btnPivotGrandTotals.setMenu(
                new Common.UI.Menu({
                  items: [
              ...
              this.btnRefreshPivot.setMenu(
                new Common.UI.Menu({
                  items: [
                    { caption: this.txtRefresh, value: "current" },
                    { caption: this.txtRefreshAll, value: "all" },
                  ],
                }),
              )
              this.btnRefreshPivot.updateHint([this.tipRefreshCurrent, this.tipRefresh])
          ...
          getPanel: function () {
            ...
            const _set = Common.enumLock
            this.btnsAddPivot = this.btnsAddPivot.concat(
              Common.Utils.injectButtons(
                this.$el.find(".btn-slot.slot-add-pivot"),
                "",
                "toolbar__icon btn-big-pivot-sum",
                this.txtCreate,
                [
          ...
          SetDisabled: function (state) {
            this.btnsAddPivot.concat(this.lockedControls).forEach((button) => {
              if (button) {
