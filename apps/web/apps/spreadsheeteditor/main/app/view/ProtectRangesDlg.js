...
define([
  ...
  SSE.Views.ProtectRangesDlg = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
        ...
        render: function () {
          ...
          this.rangeList.store.comparator = (item1, item2) => {
            const n1 = item1.get("name").toLowerCase()
            const n2 = item2.get("name").toLowerCase()
            if (n1 === n2) return 0
            return n1 < n2 ? -1 : 1
          }
          this.rangeList
        ...
        getFocusedComponents: function () {
          return [this.btnNewRange, this.btnEditRange, this.btnDeleteRange, this.rangeList].concat(
        ...
        _setDefaults: function (props) {
          ...
          this.api.asc_registerCallback(
            "asc_onLockProtectedRangeManager",
            this.wrapEvents.onLockProtectedRangeManager,
          )
          this.api.asc_registerCallback(
            "asc_onUnLockProtectedRangeManager",
            this.wrapEvents.onUnLockProtectedRangeManager,
          )
          this.api.asc_registerCallback(
            "asc_onLockProtectedRange",
            this.wrapEvents.onLockProtectedRange,
          )
          this.api.asc_registerCallback(
            "asc_onUnLockProtectedRange",
            this.wrapEvents.onUnLockProtectedRange,
          )
        },
        ...
        refreshRangeList: function (ranges, selectedItem) {
          ...
          if (val > 0) {
            ...
            if (_.isNumber(selectedItem)) {
              ...
              // object
              const rec = store.findWhere({ name: selectedItem.asc_getName(true) })
              if (rec) {
        ...
        onEditRange: function (isEdit) {
          ...
          const me = this
          const xy = Common.Utils.getOffset(me.$window)
          let rec = this.rangeList.getSelectedRec()
          let props
          if (isEdit) props = rec.get("props")
          else {
            props = new Asc.CProtectedRange()
            props.asc_setSqref(me.api.asc_getActiveRangeStr(Asc.referenceType.A))
          }
          const names = []
          this.rangeList.store.each((item) => {
        ...
        onLockProtectedRange: function (index, rangeId, userId) {
          if (this.currentSheet !== index) return
          const store = this.rangeList.store
          const rec = store.findWhere({ rangeId: rangeId })
          if (rec) {
            rec.set(
              "lockuser",
              userId
        ...
        textRangesDesc:
          "Ranges unlocked by a password when sheet is protected (this works only for locked cells)",
        textTitle: "Title",
        textRange: "Range",
        textPwd: "Password",
        textNew: "New",
