...
define([], () => {
  ...
  SSE.Views.ProtectedRangesEditDlg = Common.UI.Window.extend(
    _.extend(
      {
        initialize: function (options) {
          const _options = {}
          ...
          this.template =
            ...
            [
              ...
              '<div class="input-row">',
              `<label class="font-weight-bold">${this.txtAccess}</label>`,
              "</div>",
        ...
        render: function () {
          ...
          this.inputRangeName = new Common.UI.InputField({
            ...
            validation: (value) => {
              ...
              return this.api.asc_checkUserProtectedRangeName(value) ===
                Asc.c_oAscDefinedNameReason.OK
                ? true
                : this.textInvalidName
            },
          })
          ...
          Common.Utils.isChrome &&
            this.cmbUser._input &&
            this.cmbUser._input.attr("autocomplete", "1") // Don't show browser menu with email addresses
        ...
        getFocusedComponents: function () {
          return [this.inputRangeName, this.txtDataRange, this.cmbUser, this.listUser].concat(
        ...
        close: function () {
          this.canRequestUsers &&
            Common.NotificationCenter.off("mentions:setusers", this.binding.onUserMenuCallback)
          Common.UI.Window.prototype.close.apply(this, arguments)
        },

        afterRender: function () {
          if (this.canRequestUsers) {
            if (!this.binding) this.binding = {}
        ...
        _setDefaults: function (props) {
          if (props) {
            ...
            const rangeUsers = this.props.asc_getUsers()
            if (rangeUsers && rangeUsers.length > 0) {
              const store = this.listUser.store
              let count = 1
              rangeUsers.forEach((item) => {
                const name = item.asc_getName() || this.textAnonymous
                item.asc_getId() !== this.currentUser.id &&
                  store.add({
        ...
        onUserChanging: function (combo, newValue) {
          ...
          this.timerUserList = setTimeout(() => {
            this.onUserMenu(newValue)
          }, 300)
        },
        ...
        onUserMenu: function (str, preventOpen) {
          ...
          if (typeof str === "string") {
            ...
            Common.UI.ExternalUsers.get(
              "protect",
              undefined,
              this.userSearch.from,
              this.userSearch.count,
              this.userSearch.str,
            )
          }
        },
        ...
        onUserMenuCallback: function (type, users, isPaginated) {
          ...
          const from = this.userSearch.from
          const isClientSearch = isPaginated === undefined

          if (this._initSettings) {
            if (users && _.find(users, (item) => item.id !== undefined && item.id !== null)) {
              // has id in user info
              this.cmbUser.setDisabled(false)
              this.cmbUser._input?.attr("placeholder", this.userPlaceholder)
            }
            !isClientSearch &&
              this.cmbUser.cmpEl.find("ul").on("scroll", (event) => {
                if (
                  this.userSearch &&
                  this.userSearch.requestNext > 0 &&
                  this.userSearch.requestNext < $(event.target).scrollTop()
                ) {
                  this.userSearch.scrollTop = $(event.target).scrollTop()
                  this.userSearch.requestNext = -1 // wait for response
                  this.onUserMenuNext()
                }
              })
          ...
          if (this.listUser.store.findWhere({ initSettings: true })) {
            users &&
              users.length > 0 &&
              this.listUser.store.each((item) => {
                if (item.get("value") === this.currentUser.id || !item.get("initSettings")) return
                const rec = _.findWhere(users, { id: item.get("value") })
                if (rec) {
          ...
          const arr = []
          const str = this.cmbUser.getRawValue()
          ...
          if (users && users.length > 0) {
            if (isClientSearch) {
              ...
              users = _.filter(users, (item) => {
                if (
                  item.id !== undefined &&
                  item.id !== null &&
                  (!strlc || (item.email && 0 === item.email.toLowerCase().indexOf(strlc)))
                )
                  return true
            ...
            _.each(users, (item, index) => {
              ...
              if (!item.hasAccess) divider = true
            })
          }
          if (isClientSearch || from === 0) {
            this.cmbUser.setData(arr)
          } else {
            let $items = this.cmbUser.cmpEl.find("ul > li").find("> a")
          ...
          if (this.cmbUser.store.length > 0) {
            this.cmbUser.openMenu(0, () => {
              if (!isClientSearch) {
                this.userSearch.requestNext =
                  users && users.length > 0
                    ? (1 - 10 / this.cmbUser.store.length) *
                      this.cmbUser.scroller.cmpEl.get(0).scrollHeight
                    : -1
              }
            })
          } else {
        ...
        onKeyDown: function (lisvView, record, e) {
          if (e.keyCode === Common.UI.Keys.DELETE) this.onDeleteUser()
          else if (e.keyCode === Common.UI.Keys.SPACE) {
            const rec = this.listUser.getSelectedRec()
            const btn = rec && !rec.get("isCurrent") ? rec.get("btnEdit") : null
        ...
        addControls: function (listView, itemView, item) {
          ...
          if (btn.menu) {
            btn.menu.on("item:click", (menu, mni) => {
              if (mni.value === "remove") {
                this.onHideMenu()
                ...
                item.set(
                  "type",
                  mni.value === "edit"
                    ? Asc.c_oSerUserProtectedRangeType.edit
                    : mni.value === "view"
                      ? Asc.c_oSerUserProtectedRangeType.view
                      : Asc.c_oSerUserProtectedRangeType.notView,
                )
                item
                  .get("btnEdit")
                  .setIconCls(
                    `toolbar__icon ${mni.value === "edit" ? "btn-edit" : mni.value === "view" ? "btn-sheet-view" : "btn-hide-password"}`,
                  )
              }
            })
