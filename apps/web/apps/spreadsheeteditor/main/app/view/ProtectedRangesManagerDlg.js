...
define([
  ...
  SSE.Views.ProtectedRangesManagerDlg = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          ...
          this.rangeList = new Common.UI.ListView({
            ...
            itemTemplate: _.template(
              [
                ...
                '<div style="width:70px;"><% if (type===Asc.c_oSerUserProtectedRangeType.edit) { %>',
                this.txtEdit,
                "<% } else if (type===Asc.c_oSerUserProtectedRangeType.view) { %>",
                this.txtView,
                "<% } else { %>",
                this.txtDenied,
                "<% } %></div>",
        ...
        _setDefaults: function () {
          ...
          this.api.asc_registerCallback(
            ...
            this.wrapEvents.onRefreshUserProtectedRangesList,
          )
          this.api.asc_registerCallback(
            "asc_onLockUserProtectedManager",
            this.wrapEvents.onLockUserProtectedManager,
          )
        ...
        refreshRangeList: function (selectedItem) {
          const sheetIndex = this.cmbFilter.getValue()
          const ranges = this.api.asc_getUserProtectedRanges(
            sheetIndex >= 0 ? this.api.asc_getWorksheetName(sheetIndex) : undefined,
          )
          if (ranges) {
            const arr = []
            const currentId = this.currentUser.id
            for (let i = 0; i < ranges.length; i++) {
              const id = ranges[i].asc_getIsLock()
              const users = ranges[i].asc_getUsers()
              let type = ranges[i].asc_getType()
              const user = _.find(users, (item) => item.asc_getId() === currentId)
          ...
          if (val > 0) {
            ...
            if (_.isNumber(selectedItem)) {
              ...
              // object
              const rec = store.findWhere({ rangeId: selectedItem.asc_getId() })
              if (rec) {
        ...
        onEditRange: function (isEdit) {
          ...
          if (isEdit) {
            ...
            props.asc_setRef(this.api.asc_getActiveRangeStr(Asc.referenceType.A))
          }

          const win = new SSE.Views.ProtectedRangesEditDlg({
            title: isEdit ? this.txtEditRange : this.txtNewRange,
            props: props,
            isEdit: isEdit,
            api: this.api,
            canRequestUsers: this.canRequestUsers,
            currentUser: {
              id: this.currentUser.id,
              name: this.getUserName(this.currentUser.id, true),
            },
            handler: (result, newprops) => {
              if (result === "ok") {
                this.currentRange = newprops
                if (isEdit) {
                  this.api.asc_changeUserProtectedRange(props, newprops)
                } else {
                  this.api.asc_addUserProtectedRange(newprops)
                }
              }
            },
          }).on("close", () => {
        ...
        onDeleteRange: function () {
          ...
          if (rec && rec.length > 0) {
            ...
            Common.UI.warning({
              msg:
                rec.length > 1
                  ? this.warnDeleteRanges
                  : Common.Utils.String.format(this.warnDelete, rec[0].get("name")),
              maxwidth: 500,
              buttons: ["ok", "cancel"],
              callback: (btn) => {
                if (btn === "ok") {
                  this.currentRange =
                    _.indexOf(this.rangeList.store.models, rec[rec.length - 1]) - rec.length + 1
                  this.api.asc_deleteUserProtectedRange(_.map(rec, (item) => item.get("props")))
                }
                setTimeout(() => {
                  this.getDefaultFocusableComponent().focus()
                }, 100)
        ...
        onDlgBtnClick: function (event) {
          this.handler?.call(this, event.currentTarget.attributes.result.value, this.getSettings())
          this.close()
        },
        ...
        getUserName: function (id, original) {
          ...
          if (usersStore) {
            const rec = original ? usersStore.findOriginalUser(id) : usersStore.findUser(id)
        ...
        onLockUserProtectedManager: function (lock) {
          ...
          if (
            this.locked &&
            this.userTooltip === true &&
            this.rangeList.cmpEl.find(".lock-user").length > 0
          )
            this.rangeList.cmpEl
              .on("mouseover", _.bind(this.onMouseOverLock, this))
              .on("mouseout", _.bind(this.onMouseOutLock, this))
        ...
        updateButtons: function () {
          const rec = this.rangeList.getSelectedRec()
          const lock = rec && rec.length > 0 ? _.find(rec, (item) => !!item.get("lock")) : true
          const length = this.rangeList.store.length
          const canEdit =
            rec && rec.length > 0 ? !_.find(rec, (item) => !item.get("canEdit")) : false
          this.btnDeleteRange.setDisabled(
            length < 1 || lock || !canEdit || !!this.currentUser.anonymous,
          )
          this.btnEditRange.setDisabled(
            length < 1 ||
              !(rec && rec.length === 1) ||
              lock ||
              !canEdit ||
        ...
        onApiSheetChanged: function () {
          const oldValue =
            this.cmbFilter.store.length > 0
              ? this.cmbFilter.getValue()
        ...
        textDelete: "Delete",
        textEmpty:
          "No protected ranges have been created yet.<br>Create at least one protected range and it will appear in this field.",
        guestText: "Guest",
        tipIsLocked: "This element is being edited by another user.",
        warnDelete:
          "Are you sure you want to delete the protected range {0}?<br>Anyone who has edit access to the spreadsheet will be able to edit content in the range.",
        warnDeleteRanges:
          "Are you sure you want to delete the protected ranges?<br>Anyone who has edit access to the spreadsheet will be able to edit content in those ranges.",
