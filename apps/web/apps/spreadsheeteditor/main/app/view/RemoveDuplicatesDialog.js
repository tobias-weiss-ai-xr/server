...
define([], () => {
  SSE.Views.RemoveDuplicatesDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 350,
        ...
        updateColumnsList: function () {
          ...
          if (store.length < 1) {
            this.props.asc_getColumnList().forEach((item, index) => {
              const visible = item.asc_getVisible()
              arr.push(
        ...
        getSettings: function () {
          const store = this.columnsList.store
          const props = this.props.asc_getColumnList()
