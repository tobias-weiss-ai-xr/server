...
define([], () => {
  SSE.Views.ScaleDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 215,
        ...
        changeWidthHeight: function (type, field) {
          ...
          if (
            (this._state.width === 0 || this._state.width === null) &&
            (this._state.height === 0 || this._state.height === null)
          ) {
            this.radioScaleTo.setValue(true, true)
          } else {
        ...
        _setDefaults: function (props) {
          if (this.api) {
            const pageProps = props ? props : this.api.asc_getPageOptions()
            const pageSetup = pageProps.asc_getPageSetup()
            let width = pageSetup.asc_getFitToWidth()
            let height = pageSetup.asc_getFitToHeight()
            const scale = pageSetup.asc_getScale()
        ...
        onChangeComboScale: function (type, combo, record, e) {
          ...
          const exprAuto = new RegExp(`^\\s*(${this.textAuto.toLowerCase()})\\s*$`)
          if (exprAuto.exec(value)) {
            value = 0
          } else {
            value = Number.parseInt(value)
            !Number.isNaN(value) && (textPage = this.getTextPages(value))
          }
          if (Number.isNaN(value) || value < 0 || value > 32767) {
            ...
            value = type === "width" ? this._state.width : this._state.height
            textPage = this.getTextPages(value)
          }
          if (value === null) value = 0
          if (type === "width") {
            if (_.findWhere(this.arrDataScale, { value: value })) {
              this.cmbScaleWidth.setValue(value)
            } else {
              this.cmbScaleWidth.setValue("")
              this.cmbScaleWidth.setRawValue(`${value.toString()} ${textPage}`)
            }
            this._state.width = value
          } else if (type === "height") {
            if (_.findWhere(this.arrDataScale, { value: value })) {
              this.cmbScaleHeight.setValue(value)
            } else {
              this.cmbScaleHeight.setValue("")
              this.cmbScaleHeight.setRawValue(`${value.toString()} ${textPage}`)
            }
            this._state.height = value
          }
        },
