...
define([], () => {
  SSE.Views.SetValueDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 214,
        ...
        render: function () {
          ...
          this.spnSize.setValue(
            this.startvalue !== null ? `${this.startvalue} ${this.defaultUnit}` : "",
          )

          if (this.startvalue !== null) {
            setTimeout(() => {
              const input = this.spnSize.$input[0]
              if (document.selection) {
                // IE
                this.spnSize.$input.select()
              } else {
                //FF and Webkit
                input.selectionStart = 0
                input.selectionEnd = this.startvalue.toString().length
              }
            }, 10)
          }
          ...
          if (this.options.rounding) this.spnSize.on("change", _.bind(this.onChange, this))
        ...
        onChange: function () {
          let val = this.spnSize.getNumberValue()
          val = val / this.step
