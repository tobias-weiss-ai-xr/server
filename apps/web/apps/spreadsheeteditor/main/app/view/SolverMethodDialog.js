 ...
 */
define([], () => {
  SSE.Views.SolverMethodDialog = Common.UI.Window.extend(
    _.extend(
      {
        options: {
          width: 290,
        ...
        render: function () {
          ...
          this.inputPrecision = new Common.UI.InputField({
            ...
            "changed:after",
            _.bind(function (input, newValue, oldValue) {
        ...
        getFocusedComponents: function () {
          return [
            this.inputPrecision,
            this.chAuto,
            /*this.chIgnore, this.inputOptimality, */ this.inputMaxTime,
            this.inputIterations,
        ...
        _handleInput: function (state) {
          if (this.options.handler) {
            if (state === "ok") {
              let val = this.inputPrecision.getValue()
              if (
                !Common.UI.isValidNumber(val) ||
                Number.parseFloat(val) >= 1 ||
              ...
              if (
                !_.isEmpty(val) &&
                (!Common.UI.isValidNumber(val) || Number.parseFloat(val) % 1 !== 0)
              ) {
        ...
        getSettings: function () {
          if (this._changedProps) {
            this._precisionChanged &&
              this._changedProps.asc_setConstraintPrecision(this.inputPrecision.getValue())
            ...
            this._iterationsChanged &&
              this._changedProps.asc_setIterations(this.inputIterations.getValue())
            this._autoChanged &&
              this._changedProps.asc_setAutomaticScaling(this.chAuto.getValue() === "checked")
