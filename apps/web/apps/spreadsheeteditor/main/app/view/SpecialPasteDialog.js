...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  SSE.Views.SpecialPasteDialog = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 350,
        ...
        _setDefaults: function (props) {
          props &&
            _.each(props, (menuItem, index) => {
              this.propControls[menuItem]?.setDisabled(false)
            })
        ...
        onRadioPasteChange: function (field, newValue, eOpts) {
          if (newValue && this._changedProps) {
            ...
            const disable =
              field.options.value === Asc.c_oSpecialPasteProps.pasteOnlyFormating ||
              field.options.value === Asc.c_oSpecialPasteProps.comments ||
              field.options.value === Asc.c_oSpecialPasteProps.columnWidth
            const disableTable = this.isTable && !!this._changedProps.asc_getTableAllowed()
