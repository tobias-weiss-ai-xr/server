...
define([
  ...
  SSE.Views.TextArtSettings = Backbone.View.extend(
    _.extend(
      {
        ...
        onFillSrcSelect: function (combo, record) {
          ...
          switch (record.value) {
            case Asc.c_oAscFill.FILL_TYPE_SOLID:
              ...
              if (!this._noApply) {
                const props = new Asc.asc_TextArtProperties()
        ...
        ChangeSettings: function (props) {
          ...
          this._state.isFromSmartArtInternal = props
            .asc_getShapeProperties()
            ?.asc_getFromSmartArtInternal()
          this._state.isFromChart = props.asc_getShapeProperties()?.get_FromChart()
          this.hideTransformSettings(this._state.isFromSmartArtInternal || this._state.isFromChart)

          if (props?.asc_getShapeProperties()?.get_TextArtProperties()) {
            const shapeprops = props.asc_getShapeProperties().get_TextArtProperties()
            ...
            const fill_type = fill ? fill.asc_getType() : null
            let color = null

            if (fill) {
              const transparency = fill.asc_getTransparent()
              if (
            ...
            if (fill === null || fill === undefined || fill_type === null) {
            ...
            // border colors
            const stroke = shapeprops.asc_getLine()
            const strokeType = stroke ? stroke.asc_getType() : null
            let borderType
            const update =
            ...
            if (stroke) {
              const transparency = stroke.asc_getTransparent()
              if (
            ...
            const form = shapeprops.asc_getForm()
            if (this._state.FormId !== form) {
              this.cmbTransform.suspendEvents()
              const rec = this.cmbTransform.menuPicker.store.findWhere({
                type: form,
              })
        ...
        createDelayedControls: function () {
          ...
          this.cmbTransform.openButton.menu.on("show:after", () => {
            this.cmbTransform.menuPicker.scroller.update({ alwaysVisibleY: true })
          })
          ...
          if (!this._initSettings && !this.btnTexture) {
            ...
            this.textureMenu = new Common.UI.Menu({
              items: [
                {
                  template: _.template(
                    '<div id="id-textart-menu-texture" style="width: 233px; margin: 0 5px;"></div>',
                  ),
                },
              ],
            })
        ...
        fillTextArt: function () {
          ...
          if (!this.cmbTextArt) {
            ...
            this.cmbTextArt.openButton.menu.on("show:after", () => {
              this.cmbTextArt.menuPicker.scroller.update({ alwaysVisibleY: true })
            })
            this.lockedControls.push(this.cmbTextArt)
          }

          const models = this.application.getCollection("Common.Collections.TextArt").models
          const count = this.cmbTextArt.menuPicker.store.length
          if (models.length < 1) {
            SSE.getController("Main").fillTextArt(this.api.asc_getTextArtPreviews())
            return
          }
          if (count > 0 && count === models.length) {
            const data = this.cmbTextArt.menuPicker.store.models
            _.each(models, (template, index) => {
