...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  SSE.Views.ValueFieldSettingsDialog = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
          contentWidth: 284,
            ...
            this.options,
            {
              ...
              contentTemplate: _.template(
                [
                  ...
                  `<label class="header margin-right-2" class="margin-right-4" style="vertical-align: middle;">${this.txtSourceName}</label>`,
                  ...
                  "<tr>",
                  '<td colspan="2" class="padding-large">',
                  '<label class="header" style="display:block;">',
                  this.txtShowAs,
                  "</label>",
        ...
        render: function () {
          ...
          this.cmbSummarize = new Common.UI.ComboBox({
            ...
            data: [
              ...
              {
                value: Asc.c_oAscDataConsolidateFunction.CountNums,
                displayValue: this.txtCountNums,
              },
        ...
        _setDefaults: function (props) {
          ...
          this.format.defFormats[Asc.c_oAscNumFormatType.General] = {
            formatStr: this.api.asc_getFormatCells(info)[0],
            formatInfo: info,
          }
          ...
          info.asc_setSeparator(false)
          this.format.defFormats[Asc.c_oAscNumFormatType.Percent] = {
            formatStr: this.api.asc_getFormatCells(info)[0],
            formatInfo: info,
          }
          ...
          if (props) {
            ...
            this.lblSourceName.html(
              Common.Utils.String.htmlEncode(this.cache_names[field.asc_getIndex()].asc_getName()),
            )
            this.inputCustomName.setValue(field.asc_getName())
            this.cmbSummarize.setValue(field.asc_getSubtotal())

            const show_as = field.asc_getShowDataAs()
            this.cmbShowAs.setValue(show_as)
            this.format.show_as = show_as

            let data = []
            this.pivot_names.forEach((item, index) => {
              data.push({
                value: index,
                displayValue: item.asc_getName() || this.cache_names[index].asc_getName(),
              })
            })

            const defValue = this.api.asc_getPivotShowValueAsInfo(Asc.c_oAscShowDataAs.Difference)
            this.cmbBaseField.setData(data)
            const disabled =
              show_as === Asc.c_oAscShowDataAs.Normal ||
              show_as === Asc.c_oAscShowDataAs.PercentOfTotal ||
              show_as === Asc.c_oAscShowDataAs.PercentOfRow ||
              show_as === Asc.c_oAscShowDataAs.PercentOfCol ||
              show_as === Asc.c_oAscShowDataAs.PercentOfParentRow ||
              show_as === Asc.c_oAscShowDataAs.PercentOfParentCol ||
              show_as === Asc.c_oAscShowDataAs.Index
            this.cmbBaseField.setDisabled(disabled)
            this.cmbBaseField.setValue(
              disabled && defValue ? defValue.asc_getBaseField() : field.asc_getBaseField(),
              "",
            )

            data = []
            const baseitems = this.pivot_names[field.asc_getBaseField()].asc_getBaseItemObject(
              this.cache_names[field.asc_getBaseField()],
            )
            baseitems.forEach((item, index) => {
              data.push({
                value: item.baseItem,
                displayValue: index === 0 ? this.textPrev : index === 1 ? this.textNext : item.name,
              })
            })
            this.cmbBaseItem.setData(data)
            this.cmbBaseItem.setDisabled(
              data.length < 1 ||
                (show_as !== Asc.c_oAscShowDataAs.Difference &&
                  show_as !== Asc.c_oAscShowDataAs.Percent &&
                  show_as !== Asc.c_oAscShowDataAs.PercentDiff),
            )
            this.cmbBaseItem.setValue(
              data.length > 0 &&
                (show_as === Asc.c_oAscShowDataAs.Difference ||
                  show_as === Asc.c_oAscShowDataAs.Percent ||
                  show_as === Asc.c_oAscShowDataAs.PercentDiff)
                ? field.asc_getBaseItem()
                : "",
              "",
            )

            this.btnFormat.setDisabled(
              this.props.asc_getFieldGroupType(field.asc_getIndex()) === Asc.c_oAscGroupType.Text,
            )
            if (this.getDefFormat(show_as) === Asc.c_oAscNumFormatType.General)
              this.format.defFormats[Asc.c_oAscNumFormatType.General] = {
                formatStr: this.format.formatStr,
                formatInfo: this.format.formatInfo,
              }
          }
        ...
        getSettings: function () {
          ...
          this.format.isChanged &&
            this.cmbShowAs.getSelectedRecord() &&
            field.asc_setNumFormat(this.format.formatStr)

          if (!this.cmbBaseField.isDisabled()) field.asc_setBaseField(this.cmbBaseField.getValue())
          if (!this.cmbBaseItem.isDisabled()) field.asc_setBaseItem(this.cmbBaseItem.getValue())
        ...
        onSummarizeSelect: function (combo, record) {
          this.inputCustomName.setValue(
            this.txtByField
              .replace("%1", record.displayValue)
              .replace("%2", this.lblSourceName.text()),
          )
        },

        onShowAsSelect: function (combo, record) {
          const show_as = record.value
          const disabled =
            show_as === Asc.c_oAscShowDataAs.Normal ||
            show_as === Asc.c_oAscShowDataAs.PercentOfTotal ||
            show_as === Asc.c_oAscShowDataAs.PercentOfRow ||
          ...
          if (!this.baseFieldChanged) {
            const defValue = this.api.asc_getPivotShowValueAsInfo(show_as)
            if (defValue) {
              this.cmbBaseField.setValue(defValue.asc_getBaseField(), "")
              this.cmbBaseField.getSelectedRecord() &&
                this.changeBaseField(defValue.asc_getBaseItem())
            }
          } else {
            this.cmbBaseItem.setValue(
              (show_as === Asc.c_oAscShowDataAs.Difference ||
                show_as === Asc.c_oAscShowDataAs.Percent ||
                show_as === Asc.c_oAscShowDataAs.PercentDiff) &&
                this.cmbBaseItem.store.length > 0
          ...
          this.cmbBaseItem.setDisabled(
            this.cmbBaseItem.store.length < 1 ||
              (show_as !== Asc.c_oAscShowDataAs.Difference &&
                show_as !== Asc.c_oAscShowDataAs.Percent &&
                show_as !== Asc.c_oAscShowDataAs.PercentDiff),
          )
          const newFormat = this.getDefFormat(show_as)
        ...
        changeBaseField: function (value) {
          const field = this.cmbBaseField.getValue()
          const baseitems = this.pivot_names[field].asc_getBaseItemObject(this.cache_names[field])
          const data = []
          baseitems.forEach((item, index) => {
            data.push({
              value: item.baseItem,
          ...
          const show_as = this.cmbShowAs.getValue()
          this.cmbBaseItem.setDisabled(
            data.length < 1 ||
              (show_as !== Asc.c_oAscShowDataAs.Difference &&
                show_as !== Asc.c_oAscShowDataAs.Percent &&
          ...
          const win = new Common.Views.FormatSettingsDialog({
            ...
            handler: (result, settings) => {
              if (result === "ok" && settings) {
                this.format.isChanged = true
                this.format.formatStr = settings.format
                this.format.formatInfo = settings.formatInfo
                if (this.getDefFormat(this.format.show_as) === Asc.c_oAscNumFormatType.General) {
                  this.format.defFormats[Asc.c_oAscNumFormatType.General].formatStr =
                    settings.format
                  this.format.defFormats[Asc.c_oAscNumFormatType.General].formatInfo =
                    settings.formatInfo
                }
              }
            },
            props: {
              format: this.format.formatStr,
              formatInfo: this.format.formatInfo,
              langId: value,
            },
          }).on("close", () => {
            this.btnFormat.cmpEl.focus()
          })
          win.show()
        },
