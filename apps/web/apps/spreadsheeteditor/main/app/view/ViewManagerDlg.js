...
define(["common/main/lib/view/AdvancedSettingsWindow"], () => {
  ...
  SSE.Views.ViewManagerDlg = Common.Views.AdvancedSettingsWindow.extend(
    _.extend(
      {
        options: {
        ...
        initialize: function (options) {
          _.extend(
            ...
            {
              title: this.txtTitle,
              buttons: [{ value: "ok", caption: this.textGoTo, primary: true }, "close"],
        ...
        render: function () {
          ...
          this.viewList
            .on("item:select", _.bind(this.onSelectItem, this))
        ...
        _setDefaults: function (props) {
          ...
          this.api.asc_registerCallback(
            ...
            this.wrapEvents.onRefreshNamedSheetViewList,
          )
        },
        ...
        refreshList: function (views, selectedItem) {
          ...
          const val = this.viewList.store.length
          ...
          if (val > 0) {
            ...
            if (_.isNumber(selectedItem)) {
              ...
              setTimeout(() => {
                this.viewList.scrollToRecord(this.viewList.store.at(selectedItem))
              }, 50)
            }
          ...
          _.delay(
            () => {
              this.viewList.cmpEl.find(".listview").focus()
              this.viewList.scroller.update({ alwaysVisibleY: true })
            },
            100,
            this,
          )
        ...
        onNew: function (duplicate) {
          const rec = duplicate ? this.viewList.getSelectedRec().get("view") : undefined
          this.currentView = this.viewList.store.length
          this.api.asc_addNamedSheetView(rec)
        },

        onDelete: function () {
          const rec = this.viewList.getSelectedRec()
          let warnMsg
          if (rec) {
            if (rec.get("active")) {
              warnMsg = this.warnDeleteView.replace(
                "%1",
        ...
        onRename: function () {
          ...
          if (rec) {
            new Common.Views.TextInputDialog({
              ...
              inputConfig: {
                allowBlank: false,
                blankError: this.textRenameError,
        ...
        onKeyDown: function (lisvView, record, e) {
          if (e.keyCode === Common.UI.Keys.DELETE && !this.btnDelete.isDisabled()) this.onDelete()
        },
        ...
        onPrimary: function () {
          ...
          if (this.handler?.call(this, "ok", this.getSettings())) return
        ...
        warnDeleteView:
          "You are trying to delete the currently enabled view '%1'.<br>Close this view and delete it?",
