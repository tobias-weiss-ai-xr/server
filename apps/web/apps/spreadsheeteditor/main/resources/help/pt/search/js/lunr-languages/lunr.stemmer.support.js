...
;((root, factory) => {
  ...
  return (lunr) => {
      ...
      SnowballProgram: () => {
        ...
        return {
          ...
          replace_s: function (c_bra, c_ket, s) {
            ...
            const left = current.substring(0, c_bra)
            const right = current.substring(c_ket)
    ...
    lunr.trimmerSupport = {
      generateTrimmer: (wordCharacters) => {
        ...
        return (token) => {
          ...
          if (typeof token.update === "function") {
            return token.update((s) => s.replace(startRegex, "").replace(endRegex, ""))
          }
