...
class ContextMenu extends ContextMenuController {
  ...
  isUserVisible(id) {
    const user = this.props.users.searchUserByCurrentId(id)
    return user
      ? user.asc_getIdOriginal() === this.props.users.currentUser.asc_getIdOriginal() ||
  ...
  onMenuItemClick(action) {
    ...
    if (EditorUIController.ContextMenu?.handleMenuItemClick(this, action)) return
    ...
    switch (action) {
      ...
      case "copy":
        if (
          !api.asc_Copy() &&
          !LocalStorage.getBool("sse-hide-copy-cut-paste-warning") &&
          this.props.canCopy
        ) {
          this.showCopyCutPasteModal()
        }
  ...
  initMenuItems() {
    ...
    if (!isDisconnected && !isVersionHistoryMode) {
      if (
        ...
        comments &&
        comments.length &&
        ((!isSolvedComment && !isResolvedComments) || isResolvedComments)
      ) {
        itemsText.push({
  ...
  onApiMouseMove(dataarray) {
    ...
    let index_locked
    let index_foreign
    const editorOffset = $$("#editor_sdk").offset()
    const XY = [
      editorOffset.left - $(window).scrollLeft(),
      editorOffset.top - $(window).scrollTop(),
    ]

    for (let i = dataarray.length; i > 0; i--) {
      if (dataarray[i - 1].asc_getType() === Asc.c_oAscMouseMoveType.LockedObject) index_locked = i
      if (dataarray[i - 1].asc_getType() === Asc.c_oAscMouseMoveType.ForeignSelect)
        index_foreign = i
    }
    ...
    if (index_locked && this.isUserVisible(dataarray[index_locked - 1].asc_getUserId())) {
      const data = dataarray[index_locked - 1]
      const X = data.asc_getX()
      const Y = data.asc_getY()
      const src = $$(`<div class="username-tip"></div>`)
      ...
      const showPoint = [$$(window).width() - (X + XY[0]), Y + XY[1]]
    ...
    if (index_foreign && this.isUserVisible(dataarray[index_foreign - 1].asc_getUserId())) {
      const data = dataarray[index_foreign - 1]
      const src = $$(`<div class="username-tip"></div>`)
      const color = data.asc_getColor()
      const foreignSelectX = data.asc_getX()
      const foreignSelectY = data.asc_getY()
