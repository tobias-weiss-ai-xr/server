...
class EncodingController extends Component {
  ...
  initEncoding(type, advOptions, mode, formatOptions) {
    ...
    if (type === Asc.c_oAscAdvancedOptionsID.CSV) {
      ...
      this.valueDelimeter = recommendedSettings?.asc_getDelimiter()
  ...
  initEncodeData() {
    for (const page of this.advOptions.asc_getCodePages()) {
  ...
  onSaveFormat(valueEncoding, valueDelimeter) {
    ...
    if (this.mode === 2) {
      this.formatOptions?.asc_setAdvancedOptions(
        new Asc.asc_CTextOptions(valueEncoding, valueDelimeter),
      )
