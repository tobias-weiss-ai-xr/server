...
const ErrorController = inject(
  "storeAppOptions",
  "storeSpreadsheetInfo",
)(({ storeAppOptions, storeSpreadsheetInfo, LoadingDocument }) => {
  const { t } = useTranslation()
  const _t = t("Error", { returnObjects: true })

  useEffect(() => {
    const on_engine_created = (k) => {
      k.asc_registerCallback("asc_onError", onError)
    }
  ...
  const onError = (id, level, errData) => {
    ...
    switch (id) {
      ...
      case Asc.c_oAscError.ID.DataValidate:
        ...
        config.msg = errData?.asc_getError()
          ? Common.Utils.String.htmlEncode(errData.asc_getError())
          : _t.errorDataValidate
        break
      ...
      case Asc.c_oAscError.ID.RemoveDuplicates:
        ...
        config.msg =
          errData.asc_getDuplicateValues() !== null && errData.asc_getUniqueValues() !== null
            ? t("Error.errRemDuplicates")
                .replace("{0}", errData.asc_getDuplicateValues())
                .replace("{1}", errData.asc_getUniqueValues())
            : t("Error.errNoDuplicates")
        break
      ...
      case Asc.c_oAscError.ID.FillAllRowsWarning: {
        const fill = errData[0]
        const have = errData[1]
        let lang = (storeAppOptions.lang || "en").replace("_", "-").toLowerCase()
        let fillWithSeparator
        ...
        if (storeAppOptions.isDesktopApp && storeAppOptions.isOffline) {
          ...
          config.buttons = [
            ...
            { caption: t("Error.textClose") },
          ]
        } else {
          config.msg =
            fill >= have
              ? t("Error.textFormulaFilledFirstRowsOtherIsEmpty").replace("{0}", fillWithSeparator)
              : t("Error.textFormulaFilledFirstRowsOtherHaveData")
                  .replace("{0}", fillWithSeparator)
                  .replace("{1}", (have - fill).toLocaleString(lang))
        }
        break
      }
      ...
      case Asc.c_oAscError.ID.ConvertationOpenFormat: {
        const docExt = storeSpreadsheetInfo.dataDoc
          ? storeSpreadsheetInfo.dataDoc.fileType || ""
          : ""
        if (errData === "pdf") config.msg = _t.errorInconsistentExtPdf.replace("%1", docExt)
        else if (errData === "docx") config.msg = _t.errorInconsistentExtDocx.replace("%1", docExt)
        else if (errData === "xlsx") config.msg = _t.errorInconsistentExtXlsx.replace("%1", docExt)
        else if (errData === "pptx") config.msg = _t.errorInconsistentExtPptx.replace("%1", docExt)
        else config.msg = _t.errorInconsistentExt
        break
      }
      ...
      case Asc.c_oAscError.ID.MacroUnavailableWarning:
        config.msg = t("Error.errorMacroUnavailableWarning").replace(
          "%1",
          errData ? `'${errData}'` : "",
        )
        break
    ...
    if (level === Asc.c_oAscError.Level.Critical) {
      ...
      if (storeAppOptions.canRequestClose) {
        ...
        storeAppOptions.canBackToFolder &&
        !storeAppOptions.isDesktopApp &&
        typeof id !== "string" &&
        storeAppOptions.customization.goback.url &&
        storeAppOptions.customization.goback.blank === false
      ) {
