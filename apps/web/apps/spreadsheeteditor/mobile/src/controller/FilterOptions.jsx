import { Popover, Sheet, f7 } from "framework7-react"
...
const FilterOptionsController = memo((props) => {
  ...
  const onApiFilterOptions = (config) => {
    ...
    if (
      (wsProps.PivotTables && config.asc_getPivotObj()) ||
      (wsProps.AutoFilter && !config.asc_getPivotObj())
    )
      return

    setCheckSort(
      (config.asc_getSortState() === Asc.c_oAscSortOptions.Ascending ? "down" : "") ||
    ...
    if (Device.phone) {
      ...
      const rect = config.asc_getCellCoord()
      const posX = rect.asc_getX() + rect.asc_getWidth() - 9
      const posY = rect.asc_getY() + rect.asc_getHeight() - 9

      const $target = $$("#idx-context-menu-target").css({ left: `${posX}px`, top: `${posY}px` })
  ...
  const onSort = (type) => {
    const api = Common.EditorApi.get()
    api.asc_sortColFilter(
      type === "sortdown" ? Asc.c_oAscSortOptions.Ascending : Asc.c_oAscSortOptions.Descending,
      configRef.current.asc_getCellId(),
      configRef.current.asc_getDisplayName(),
    )
  ...
  const setClearDisable = (config) => {
    const arr = config.asc_getValues()
    const lenCheck = arr.filter((item) => item.visible === true).length
  ...
  const setDataFilterCells = (config) => {
    ...
    config.asc_getValues().forEach((item, index) => {
      ...
      isnumber = !Number.isNaN(Number.parseFloat(value)) && Number.isFinite(value)
  ...
  const onUpdateCell = (id, state) => {
    ...
    if (listVal.some((item) => item.check)) {
      configRef.current
        .asc_getValues()
        .forEach((item, index) => item.asc_setVisible(listVal[index].check))
