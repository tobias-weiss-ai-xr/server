...
const LongActionsController = inject("storeAppOptions")(({ storeAppOptions }) => {
  ...
  const onLongActionEnd = (type, id, forceClose) => {
    ...
    action =
      stackLongActions.get({ type: Asc.c_oAscAsyncActionType.Information }) ||
      stackLongActions.get({ type: Asc.c_oAscAsyncActionType.BlockInteraction })

    if (action && !forceClose) {
      setLongActionView(action)
    } else {
      loadMask?.el?.classList.contains("modal-in")
        ? f7.dialog.close(loadMask.el)
  ...
  const setLongActionView = (action) => {
    ...
    if (action.type === Asc.c_oAscAsyncActionType.BlockInteraction) {
      if (loadMask?.el?.classList.contains("modal-in")) {
  ...
  const onConfirmAction = (id, apiCallback, data) => {
    ...
    if (
      ...
      id === Asc.c_oAscConfirm.ConfirmReplaceFormulaInTable
    ) {
      f7.dialog
        .create({
          ...
          text:
            id === Asc.c_oAscConfirm.ConfirmReplaceRange
              ? _t.confirmMoveCellRange
              : _t.confirmReplaceFormulaInTable,
          buttons: [
            {
              text: _t.textYes,
              onClick: () => {
