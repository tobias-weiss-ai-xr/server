...
class MainController extends Component {
  ...
  initSdk() {
    const on_load_scripts = () => {
      ...
      const loadConfig = (data) => {
        ...
        if (value !== null) {
          this.api.asc_setLocale(Number.parseInt(value))
        } else {
          value = appOptions.region
          value = Common.util.LanguageInfo.getLanguages().hasOwnProperty(value)
            ? value
            : Common.util.LanguageInfo.getLocalLanguageCode(value)
          if (value !== null) {
            value = Number.parseInt(value)
          } else {
            value = appOptions.lang
              ? Number.parseInt(Common.util.LanguageInfo.getLocalLanguageCode(appOptions.lang))
              : 0x0409
          }
      ...
      const onEditorPermissions = (params) => {
        ...
        if (
          ...
          f7.dialog
            .create({
              title:
                Asc.c_oLicenseResult.NotBefore === licType
                  ? t("Controller.Main.titleLicenseNotActive")
                  : t("Controller.Main.titleLicenseExp"),
              text:
                Asc.c_oLicenseResult.NotBefore === licType
                  ? t("Controller.Main.warnLicenseBefore")
                  : t("Controller.Main.warnLicenseExp"),
            })
            .open()
        ...
        const appOptions = this.props.storeAppOptions
        appOptions.setPermissionOptions(
          this.document,
          licType,
          params,
          this.permissions,
  ...
  bindEvents() {
    ...
    EditorUIController.initCellInfo?.(this.props)

    EditorUIController.initEditorStyles?.(this.props.storeCellSettings)

    EditorUIController.initFonts?.(this.props)
    ...
    this.api.asc_registerCallback(
      ...
      this.onChangeProtectSheet.bind(this),
    )
    ...
    this.api.asc_registerCallback(
      ...
      this.onEntriesListMenu.bind(this, true),
    )
    ...
    this.api.asc_registerCallback("asc_onInputMessage", (title, msg) => {
      if (msg) {
        ...
        if (
          rect.right - leftPosition <= widthCell ||
          rect.right - leftPosition <= tooltipCell.offsetWidth
        ) {
        ...
        if (
          rect.bottom - topPosition <= heightCell ||
          rect.bottom - topPosition <= tooltipCell.offsetHeight
        ) {
    ...
    this.api.asc_registerCallback(
      "asc_onNeedUpdateExternalReferenceOnOpen",
      this.onNeedUpdateExternalReference.bind(this),
    )

    const storeAppOptions = this.props.storeAppOptions
    this.api.asc_setFilteringMode?.(storeAppOptions.canModifyFilter)
  }
        ...
        buttons: [
          {
            ...
            onClick: () => {
              ...
              links?.length && this.api.asc_updateExternalReferences(links)
            },
          },
          {
  ...
  onEntriesListMenu(validation, textArr, addArr) {
    ...
    if (
      (!storeAppOptions.isEdit && !storeAppOptions.isRestrictedEdit) ||
    ...
    if (textArr?.length) {
      if (!Device.isPhone) {
        ...
        const coord = this.api.asc_getActiveCellCoord(validation)
  ...
  onRenameText(found, replaced) {
    ...
    if (this.api.isReplaceAll) {
      f7.dialog.alert(
        ...
        found
          ? !found - replaced
            ? t("Controller.Main.textReplaceSuccess").replace(/\{0\}/, `${replaced}`)
            : t("Controller.Main.textReplaceSkipped").replace(/\{0\}/, `${found - replaced}`)
          : t("Controller.Main.textNoMatches"),
      )
    }
  }
  ...
  onChangeProtectSheet() {
    ...
    const { wsLock, wsProps } = this.getWSProps(true)
  ...
  getWSProps(update) {
    ...
    if (update) {
      const wsLock = !!this.api.asc_isProtectedSheet()
      const wsProps = {}
  ...
  onDocumentContentReady() {
    ...
    let value =
      appOptions.isEditMailMerge || appOptions.isEditDiagram
        ? 100
        : LocalStorage.getItem("sse-settings-zoom")
    const zf =
      value !== null
        ? Number.parseInt(value) / 100
        : appOptions.customization?.zoom
          ? Number.parseInt(appOptions.customization.zoom) / 100
          : 1
  ...
  onBeforeUnload() {
    ...
    const isEdit =
      this.permissions.edit !== false && config.mode !== "view" && config.mode !== "editdiagram"
