...
import { BlockTitle, Link, List, ListItem, NavRight, Navbar, Page, Toggle } from "framework7-react"
import { f7 } from "framework7-react"
import { inject, observer } from "mobx-react"
import React, { useEffect, useState } from "react"
import { withTranslation } from "react-i18next"
import { SearchSettingsView, SearchView } from "../../../../common/mobile/lib/controller/Search"
      ...
      <Page>
        ...
        <List>
          <ListItem
            radio
            title={_t.textFind}
            name="find-replace-checkbox"
            checked={!this.state.useReplace}
            onClick={(e) => this.onFindReplaceClick("find")}
          />
          {isEdit && !isVersionHistoryMode
            ? [
                ...
                <ListItem
                  key="replace-all"
                  radio
                  title={_t.textFindAndReplaceAll}
                  name="find-replace-checkbox"
                  checked={this.state.isReplaceAll}
                  onClick={() => this.onFindReplaceClick("replace-all")}
                />,
              ]
            : null}
        </List>
        ...
        <List>
          <ListItem
            radio
            title={_t.textWorkbook}
            name="search-in-checkbox"
            value="0"
            checked={this.state.searchIn === 0}
            onClick={() =>
              this.setState({
                searchIn: 0,
              })
        ...
        <List>
          <ListItem
            radio
            title={_t.textByRows}
            name="search-by-checkbox"
            value="0"
            checked={this.state.searchBy === 0}
            onClick={() =>
              this.setState({
                searchBy: 0,
              })
          ...
          <ListItem
            radio
            title={_t.textByColumns}
            name="search-by-checkbox"
            value="1"
            checked={this.state.searchBy === 1}
            onClick={() =>
              this.setState({
                searchBy: 1,
              })
        ...
        <List>
          <ListItem
            radio
            title={_t.textFormulas}
            name="look-in-checkbox"
            value="0"
            checked={this.state.lookIn === 0}
            onClick={() =>
              this.setState({
                lookIn: 0,
              })
        ...
        <List>
          <ListItem title={_t.textMatchCase}>
            <Toggle
              slot="after"
              className="toggle-match-case"
              checked={this.state.isMatchCase}
              onToggleChange={() =>
                this.setState({
          ...
          <ListItem title={_t.textMatchCell}>
            <Toggle
              slot="after"
              className="toggle-match-cell"
              checked={this.state.isMatchCell}
              onToggleChange={() =>
                this.setState({
          ...
          <ListItem title={_t.textHighlightRes}>
            <Toggle
              slot="after"
              className="toggle-mark-results"
              defaultChecked
              onToggleChange={this.onToggleMarkResults}
            />
          </ListItem>
        </List>
      </Page>
...
class SESearchView extends SearchView {
  searchParams() {
    const params = super.searchParams()
    const $$ = Dom7

    const checkboxMatchCase = f7.toggle.get(".toggle-match-case")
    const checkboxMatchCell = f7.toggle.get(".toggle-match-cell")
    const checkboxMarkResults = f7.toggle.get(".toggle-mark-results")
    const checkboxSearchIn = $$('[name="search-in-checkbox"]:checked')[0]
    const checkboxSearchBy = $$('[name="search-by-checkbox"]:checked')[0]
    const checkboxLookIn = $$('[name="look-in-checkbox"]:checked')[0]
...
const Search = withTranslation()((props) => {
  ...
  const onSearchQuery = (params, isSearchByTyping) => {
    ...
    const lookIn = +params.lookIn === 0
    const searchIn = +params.searchIn
    const searchBy = +params.searchBy === 0
    ...
    api.asc_findText(options, (resultCount) => {
      if (!resultCount) {
...
const SearchSettingsWithTranslation = inject(
  "storeAppOptions",
  "storeVersionHistory",
)(observer(withTranslation()(SearchSettings)))
