...
const StatusbarController = inject(
  ...
  observer((props) => {
    ...
    const onApiEditCell = (state) => {
      const isDisable = state !== Asc.c_oAscCellEditorState.editEnd
    ...
    const onApiUpdateTabColor = (sheetindex) => {
      const api = Common.EditorApi.get()
      const tab = storeWorksheets.sheets.find((sheet) => sheet.index === sheetindex)
...
const Statusbar = inject(
  ...
  observer((props) => {
    ...
    const onTabClick = (i, target) => {
      ...
      const opened = $$(".document-menu.modal-in").length
    ...
    const renameWorksheet = () => {
      ...
      if (api.asc_getWorksheetsCount() > 0) {
        ...
        const current = api.asc_getWorksheetName(api.asc_getActiveWorksheetIndex())
            ...
            buttons: [
              {
                ...
                bold: true,
                onClick: () => {
                  const s = $$('input[name="modal-sheet-name"]').val()
                  let wc = api.asc_getWorksheetsCount()
                  const items = []
                  let err = !s.trim().length
                    ? _t.textErrNotEmpty
                    : !/^(\')|[:\\\/\*\?\[\]]|(\')$/.test(s)
                      ? null
                      : _t.textErrNameWrongChar
                  if (!err) {
                    while (wc--) {
                      if (sindex !== wc) {
                        items.push(api.asc_getWorksheetName(wc).toLowerCase())
                  ...
                  } else if (s !== current) api.asc_renameWorksheet(s)
                },
              },
              {
                text: _t.textCancel,
    ...
    const onTabMenu = (event) => {
      const api = Common.EditorApi.get()
      const index = storeWorksheets.sheets.find((sheet) => sheet.active).index
      ...
      switch (event) {
        case "del":
          ...
          break
        case "hide":
          hideWorksheet(true, index)
          break
        case "ins":
          api.asc_insertWorksheet(createSheetName())
          break
        case "copy": {
          const name = createCopyName(api.asc_getWorksheetName(api.asc_getActiveWorksheetIndex()))
        ...
        case "move":
          Device.phone
            ? f7.sheet.open(".move-sheet")
            : f7.popover.open("#idx-move-sheet-popover", targetRef.current)
          break
        case "tab-color":
          if (Device.phone) {
        ...
        case "showMore":
          ...
          break
        default: {
          const _re = /reveal\:(\d+)/.exec(event)
          if (_re && !!_re[1]) {
            hideWorksheet(false, Number.parseInt(_re[1]))
          }
        }
      }
    }
    ...
    const onMenuMoveClick = (index) => {
      ...
      const sheetsCount = api.asc_getWorksheetsCount()
      const activeIndex = api.asc_getActiveWorksheetIndex()
