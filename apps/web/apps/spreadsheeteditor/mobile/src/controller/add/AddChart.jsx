...
class AddChartController extends Component {
    ...
    if (settings) {
      if (isChartEdit) {
        ...
        if (isValid === Asc.c_oAscError.ID.No) {
          ...
          f7.dialog.alert(
            isValid === Asc.c_oAscError.ID.StockChartError
              ? _t.errorStockChart
              : isValid === Asc.c_oAscError.ID.MaxDataSeriesError
                ? _t.errorMaxRows
                : _t.txtInvalidRange,
  ...
  render() {
    return <AddChart onInsertChart={this.onInsertChart} />
  }
}
