...
class AddFilterController extends Component {
  ...
  componentDidMount() {
    ...
    api.asc_setFilteringMode?.(appOptions.canModifyFilter)
  ...
  onInsertSort(type) {
    ...
    const typeCheck =
      type === "down" ? Asc.c_oAscSortOptions.Ascending : Asc.c_oAscSortOptions.Descending
    const res = api.asc_sortCellsRangeExpand()
    switch (res) {
  ...
  render() {
    return (
      <AddSortAndFilter
        getIsAutoFilter={this.getIsAutoFilter}
...
export default inject(
  "storeWorksheets",
  "storeAppOptions",
)(observer(withTranslation()(AddFilterController)))
