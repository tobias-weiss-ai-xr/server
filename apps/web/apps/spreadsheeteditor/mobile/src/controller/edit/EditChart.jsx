...
class EditChartController extends Component {
  ...
  onBorderColor(color) {
    ...
    if (
      currentShape &&
      currentShape.get_stroke().get_type() === Asc.c_oAscStrokeType.STROKE_COLOR
    ) {
  ...
  onBorderSize(value) {
      ...
      if (_borderColor === "transparent")
        ...
        stroke.put_color(
          Common.Utils.ThemeColor.getRgbColor(
            Common.Utils.ThemeColor.colorValue2EffectId(_borderColor),
          ),
        )
    ...
    api.asc_setGraphicObjectProps(image)
    this.props.storeChartSettings.initBorderColor(
      this.props.storeFocusObjects.shapeObject.get_ShapeProperties(),
    ) // when select STROKE_NONE or change from STROKE_NONE to STROKE_COLOR
  }
  ...
  setLayoutProperty(propertyMethod, value) {
    const api = Common.EditorApi.get()
    const chartObject = api.asc_getChartSettings()
  ...
  getVerticalAxisProp() {
    const api = Common.EditorApi.get()
    const chartObject = api.asc_getChartSettings()
    const verAxisProps = chartObject.getVertAxisProps()

    return verAxisProps.getAxisType() === Asc.c_oAscAxisType.val
      ? verAxisProps
      : chartObject.getHorAxisProps()
  }
  ...
  onVerAxisMinValue(value) {
    const axisProps = this.getVerticalAxisProp()
    const axisRule = !value ? Asc.c_oAscValAxisRule.auto : Asc.c_oAscValAxisRule.fixed
  ...
  getHorizontalAxisProp() {
    const api = Common.EditorApi.get()
    const chartObject = api.asc_getChartSettings()
    const verHorProps = chartObject.getHorAxisProps()

    return verHorProps.getAxisType() === Asc.c_oAscAxisType.val
      ? chartObject.getVertAxisProps()
      : verHorProps
  }
  ...
  setHorizontalAxisProp(axisProps) {
    const api = Common.EditorApi.get()
    const chartObject = api.asc_getChartSettings()
    const verAxisProps = chartObject.getHorAxisProps()
  ...
  onHorAxisCrossType(value) {
    const axisProps = this.getHorizontalAxisProp()
