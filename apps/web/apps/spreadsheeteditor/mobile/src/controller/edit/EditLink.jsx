...
import { EditLink, PageEditSheet, PageEditTypeLink } from "../../view/edit/EditLink"
...
class EditLinkController extends Component {
  ...
  onEditLink(args) {
    ...
    const linkProps = new Asc.asc_CHyperlink()
    let sheet = ""
    let displayText = args.text
    ...
    if (args.type === Asc.c_oAscHyperlinkType.RangeLink) {
      const range = args.url
      let isValidRange = /^[A-Z]+[1-9]\d*:[A-Z]+[1-9]\d*$/.test(range)

      if (!isValidRange) isValidRange = /^[A-Z]+[1-9]\d*$/.test(range)
  ...
  render() {
    return !this.props.isNavigate ? (
      Device.phone ? (
        ...
        <Popover
          id="edit-link-popover"
          className="popover__titled"
          closeByOutsideClick={false}
          onPopoverClosed={() => this.props.closeOptions("edit-link")}
        >
