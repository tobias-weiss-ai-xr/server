...
class EditShapeController extends Component {
  ...
  onBorderColor(color) {
    ...
    if (
      _shapeObject &&
      _shapeObject.get_stroke().get_type() === Asc.c_oAscStrokeType.STROKE_COLOR
    ) {
      const shape = new Asc.asc_CShapeProperty()
      const image = new Asc.asc_CImgProperty()
  ...
  onBorderSize(value) {
    ...
    this.props.storeShapeSettings.initBorderColorView(
  ...
  onOpacity(value) {
    ...
    fill.asc_putTransparent(Number.parseInt(value * 2.55))
    shape.asc_putFill(fill)
