...
class ApplicationSettingsController extends Component {
  constructor(props) {
    ...
    this.props.storeApplicationSettings.changeUnitMeasurement(
  ...
  initFormulaLangsCollection() {
    ...
    const formulaLangsCollection = formulaLangs.map((lang) => {
      ...
      return {
        value: lang,
        displayValue: _t[`txt${str}lang`]
          ? t(`View.Settings.txt${str}lang`)
          : t(`View.Settings.txt${str}`),
        exampleValue: formulaLangsExamples[`txtExample${str}`] || formulaLangsExamples.txtExampleEn,
      }
    })

    formulaLangsCollection.sort((a, b) => {
  ...
  initRegSettings() {
    ...
    const text5 = api.asc_getLocaleExample(arr[5], Asc.cDate().getExcelDateWithTime(true), regCode)
    const text6 = api.asc_getLocaleExample(arr[7], Asc.cDate().getExcelDateWithTime(true), regCode)
  ...
  onChangeDisplayResolved(value) {
    const api = Common.EditorApi.get()
    const displayComments = LocalStorage.getBool("sse-mobile-settings-livecomment", true)
