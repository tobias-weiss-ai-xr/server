...
class SpreadsheetSettingsController extends Component {
  ...
  initSpreadsheetSettings() {
    ...
    this.props.storeSpreadsheetSettings.resetPortrait(
      opt.asc_getOrientation() === Asc.c_oAscPageOrientation.PagePortrait,
    )
  ...
  initSpreadsheetMargins() {
    ...
    const currentSheet = api.asc_getActiveWorksheetIndex()
    const props = api.asc_getPageOptions(currentSheet)

    this.localMarginProps = props.asc_getPageMargins()

    const left = this.localMarginProps.asc_getLeft()
    const top = this.localMarginProps.asc_getTop()
    const right = this.localMarginProps.asc_getRight()
    const bottom = this.localMarginProps.asc_getBottom()
  ...
  onPageMarginsChange(align, marginValue) {
    const api = Common.EditorApi.get()
    const changeProps = new Asc.asc_CPageMargins()
    ...
    switch (align) {
      case "left":
        changeProps.asc_setLeft(marginValue)
        break
      case "top":
        changeProps.asc_setTop(marginValue)
        break
      case "right":
        changeProps.asc_setRight(marginValue)
        break
      case "bottom":
        changeProps.asc_setBottom(marginValue)
        break
    }

    api.asc_changePageMargins(
      changeProps,
      undefined,
      undefined,
      undefined,
