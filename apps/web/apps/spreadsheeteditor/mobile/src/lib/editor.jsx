...
export const initCellInfo = (props) => {
  ...
  api.asc_registerCallback("asc_onSelectionChanged", (cellInfo) => {
    ...
    const graphicObjects = api.asc_getGraphicObjectProps()
...
export const ContextMenu = {
  ...
  mapMenuItems(controller) {
    ...
    const isSolved = comments[0]?.asc_getSolved()
    ...
    if (locked || api.isCellEdited || isDisconnected) {
      ...
      if (canViewComments) {
        if (comments?.length && ((!isSolved && !isResolvedComments) || isResolvedComments)) {
    ...
    if (cellInfo.asc_getHyperlink() && !cellInfo.asc_getMultiselect()) {
      ...
      !cellInfo.asc_getHyperlink() &&
      !cellInfo.asc_getMultiselect() &&
      !isPivot &&
      !wsProps.InsertHyperlinks &&
      !isVersionHistoryMode
    ) {
    ...
    if (Device.phone && itemsText.length > 2) {
      controller.extraItems = itemsText.splice(2, itemsText.length, {
        caption: _t.menuMore,
        event: "showActionSheet",
      })
    } else if (itemsText.length > 4) {
      controller.extraItems = itemsText.splice(3, itemsText.length, {
        caption: _t.menuMore,
        event: "showActionSheet",
      })
    }
  ...
  handleMenuItemClick(controller, action) {
    ...
    const deleteActions = {
      [Asc.c_oAscSelectionType.RangeRow]: () =>
        api.asc_deleteCells(Asc.c_oAscDeleteOptions.DeleteRows),
      [Asc.c_oAscSelectionType.RangeCol]: () =>
    ...
    const actionHandlers = {
      ...
      del: () =>
        (
          deleteActions[cellInfo.asc_getSelectionType()] ||
