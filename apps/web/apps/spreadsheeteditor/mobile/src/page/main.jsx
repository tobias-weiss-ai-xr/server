...
class MainPage extends Component {
  ...
  handleOptionsViewClosed = (opts) => {
    this.setState((state) => {
      if (opts === "edit") return { editOptionsVisible: false, isOpenModal: false }
      if (opts === "add")
        return { addOptionsVisible: false, addShowOptions: null, isOpenModal: false }
      if (opts === "settings") return { settingsVisible: false, isOpenModal: false }
      if (opts === "coauth") return { collaborationVisible: false, isOpenModal: false }
      if (opts === "add-link") return { addLinkSettingsVisible: false }
      if (opts === "edit-link") return { editLinkSettingsVisible: false }
      if (opts === "history") return { historyVisible: false }
    })
      ...
      <Themes>
        ...
        >
          <Page name="home" className={`editor${!isHideLogo ? " page-with-logo" : ""}`}>
            ...
            <CellEditor
              onClickToOpenAddOptions={(panels, button) =>
            ...
            {!this.state.addOptionsVisible ? null : (
              <AddingController showOptions={this.state.addShowOptions} />
            )}
            {!this.state.addLinkSettingsVisible ? null : (
              <AddLinkController closeOptions={this.handleOptionsViewClosed.bind(this)} />
            )}
            {!this.state.editLinkSettingsVisible ? null : (
              <EditLinkController closeOptions={this.handleOptionsViewClosed.bind(this)} />
            )}
            {!this.state.settingsVisible ? null : <SettingsController />}
            {!this.state.collaborationVisible ? null : (
              <CollaborationView closeOptions={this.handleOptionsViewClosed.bind(this)} />
            )}
            {!this.state.historyVisible ? null : (
              <VersionHistoryController
                onclosed={this.handleOptionsViewClosed.bind(this, "history")}
              />
            )}
            {appOptions.isDocReady && (
