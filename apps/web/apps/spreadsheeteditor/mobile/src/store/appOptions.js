...
export class storeAppOptions {
  ...
  setPermissionOptions(document, licType, params, permissions, isSupportEditFeature) {
    if (params.asc_getRights() !== Asc.c_oRights.Edit) permissions.edit = false
    ...
    this.canEdit =
      permissions.edit !== false && // can edit or review
      (this.config.canRequestEditRights || this.config.mode !== "view") &&
      isSupportEditFeature // if mode=="view" -> canRequestEditRights must be defined
    // (!this.isReviewOnly || this.canLicense) && // if isReviewOnly==true -> canLicense must be true
    this.isEdit =
      (this.canLicense || this.isEditDiagram || this.isEditMailMerge) &&
    ...
    this.canLiveView =
      !!params.asc_getLiveViewerSupport() && this.config.mode === "view" && isSupportEditFeature
