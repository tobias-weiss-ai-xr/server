...
export class storeChartSettings {
  ...
  initBorderColor(shapeProperties) {
    const stroke = shapeProperties.get_stroke()
    this.borderColor =
      stroke && stroke.get_type() === Asc.c_oAscStrokeType.STROKE_COLOR
        ? this.resetColor(stroke.get_color())
        : "transparent"
  }
  ...
  get styles() {
    ...
    const columns = Number.parseInt(widthContainer / 70) // magic
    let row = -1
    const styles = []
