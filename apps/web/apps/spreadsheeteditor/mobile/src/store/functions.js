    ...
    for (const g in groups) {
      ...
      for (const f in funcarr) {
        ...
        if (customFuncInfo) {
          ...
          args = `(${arrArgs.map((item) => (item.asc_getIsOptional() ? `[${item.asc_getName()}]` : item.asc_getName())).join(`${api.asc_getFunctionArgumentSeparator()} `)})`
          descr = customFuncInfo.asc_getDescription()
        } else {
          args = (data?.[funcName] ? data[funcName].a : "").replace(/[,;]/g, separator)
          descr = data?.[funcName] ? data[funcName].d : ""
        }
