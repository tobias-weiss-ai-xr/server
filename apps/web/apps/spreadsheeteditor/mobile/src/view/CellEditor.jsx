  ...
  const FunctionHint = ({ funcHint }) => {
    ...
    return (
      <div className="function-hint-card">
        <div className="name">
          <span className={funcHint.nameIsActive ? "contrast" : ""}>{funcHint.name}</span>(
  ...
  return (
    <>
      <View
        ...
        style={viewStyle}
        routes={routes}
        className={expanded ? "cell-editor expanded" : "cell-editor collapsed"}
      >
        ...
        <div className="ce-group group--content" style={contentStyle}>
          <div id="idx-list-target" className="target-function-list" />
...
const FunctionsList = (props) => {
  ...
  return (
    <div
      id="functions-list"
      className={isPhone ? "functions-list functions-list__mobile" : "functions-list"}
    >
      <List>
        {funcArr?.length &&
              ...
              >
                {(functions[elem.name] || hintArr[index]?.descr) && (
                  <div
                    ...
                    onClick={(e) => {
                      e.stopPropagation()
                      const functionInfo = functions[elem.name] || hintArr[index]

                      if (functionInfo) {
                        f7.views.current.router.navigate("/function-info/", {
                          props: {
                            functionInfo,
                            functionObj: elem,
                            insertFormula: props.insertFormula,
      ...
      <Navbar title={functionInfo.caption} backLink={_t.textBack} backLinkUrl="/functions-list/">
        <NavRight>
          <Link
            ...
            onClick={() => props.insertFormula(functionObj.name, functionObj.type)}
          />
        </NavRight>
      </Navbar>
