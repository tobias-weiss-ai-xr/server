...
const PageEncoding = (props) => {
  ...
  const [stateEncoding, setStateEncoding] = useState(
    props.valueEncoding === -1
      ? encodeData.find((encoding) => encoding.lcid === 65001).value
      : props.valueEncoding,
  )
  const [stateDelimeter, setStateDelimeter] = useState(props.valueDelimeter)
  const getIndexNameEncoding = () =>
    encodeData.findIndex((encoding) => encoding.value === stateEncoding)
    ...
    <View style={props.style} routes={routes}>
      <Page>
        ...
        <List className="buttons-list">
          ...
          <ListButton
            className="button-fill button-raised"
            title={mode === 2 ? _t.textDownload : _t.txtOk}
            onClick={() => props.onSaveFormat(stateEncoding, stateDelimeter)}
          />
        </List>
      </Page>
    </View>
      ...
      <List>
        {encodeData.map((encoding, index) => {
          return (
            <ListItem
              radio
              checked={currentEncoding === encoding.value}
              title={encoding.displayValue}
              key={index}
      ...
      <List>
        {namesDelimeter.map((name, index) => {
          return (
            <ListItem
              radio
              checked={currentDelimeter === valuesDelimeter[index]}
              title={name}
              key={index}
              value={valuesDelimeter[index]}
              onChange={() => {
