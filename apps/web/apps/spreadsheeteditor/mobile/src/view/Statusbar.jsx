      ...
      <Page>
        ...
        <List>
          <ListGroup style={Device.phone ? { paddingBottom: "44px" } : undefined}>
            ...
            <ListItem title={t("Statusbar.textMoveToEnd")} onClick={() => onMenuMoveClick(-255)} />
          </ListGroup>
        </List>
      </Page>
    </View>
  ...
  useEffect(() => {
    ...
    if (sheets.activeWorksheet !== -1) {
      if (tab.length === 0) {
        ...
        $$(".sheet-tabs").scrollLeft(
          $$(".sheet-tabs").scrollLeft() +
            (tab.offset().left + tab.width() - $$(".sheet-tabs").width()),
          500,
        )
      }
    }
      ...
      <Page>
        <List>
          {allSheets.map((model, sheetIndex) => (
            <ListItem
              className={`item-list ${model.active ? "active" : null}`}
              key={model.name}
              title={model.name}
              checkbox
              checked={model.active}
              onClick={() => onTabListClick(sheetIndex)}
            >
              <div slot="after">
                {model.hidden ? (
                  ...
                  <div
                    className="marker-color-sheet"
                    style={{
                      background: model.color
                        ? `#${Common.Utils.ThemeColor.getHexColor(model.color.get_r(), model.color.get_g(), model.color.get_b())}`
                        : "transparent",
                    }}
                  />
                )}
...
const PageCustomTabColor = inject("storePalette")(
  observer((props) => {
    const { t } = useTranslation()
...
const PageTabColor = inject("storePalette")(
  observer((props) => {
    const { t } = useTranslation()
    ...
    useEffect(() => {
      if (allSheets.length !== 0) {
        const color = sheets.at(activeIndex).color
        if (color !== null) {
          sheets.changeTabColor(
    ...
    const changeColor = (color, effectId, effectValue) => {
      if (color !== "empty") {
        ...
        Device.isPhone
          ? f7.sheet.close(".tab-color-sheet")
          : f7.popover.close("#idx-tab-color-popover")
      } else {
        f7.views.tabColorView.router.navigate("/sheet-tab-custom-color/", {
          props: { onSetWorkSheetColor: props.onSetWorkSheetColor, sheets },
        })
      }
    }
  ...
  observer((props) => {
    ...
    return (
      <Fragment>
        <View
          id="idx-statusbar"
          className={`statusbar ${isSheetRtl ? "rtl" : "ltr"}`}
          style={{
            marginBottom: storeAppOptions.isDrawMode
              ? "calc(44px + env(safe-area-inset-bottom) + env(keyboard-inset-top))"
              : undefined,
            ...viewStyle,
          }}
        >
          {isEdit && (
            <div
              id="idx-box-add-tab"
              className={`${isDisconnected || isWorkbookLocked ? "disabled box-tab" : "box-tab"}`}
            >
              <Link
                href={false}
                id="idx-btn-addtab"
                className={`tab${isDisabledEditSheet || isDisconnected || isWorkbookLocked || isProtectedWorkbook ? " disabled" : ""}`}
                onClick={props.onAddTabClicked}
              >
                {isAndroid ? (
              ...
              <Link
                href={false}
                id="idx-btn-all-list-tab"
                className={`tab${isDisabledEditSheet || isDisconnected || isWorkbookLocked ? " disabled" : ""}`}
                onClick={(e) =>
                  f7.popover.open("#idx-all-list", e.target.closest("#idx-btn-all-list-tab"))
                }
              >
                {isAndroid ? (
          ...
          <div className="statusbar--box-tabs">
            <ul className="sheet-tabs bottom">
              {allSheets.map((model, i) =>
                model.hidden ? null : (
                  <li
                    className={`tab${model.active ? " active" : ""} ${model.locked ? "locked" : ""}`}
                    key={i}
                    onClick={(e) => props.onTabClick(i, e.target)}
                  >
                    <a
                      style={{ boxShadow: setSheetColor(model) }}
                      className={`tab-color-${model.index}`}
                    >
                      {model.name}
                    </a>
                  </li>
        ...
        {isEdit ? (
          <Popover
            id="idx-tab-context-menu-popover"
        ...
        {isPhone || isAndroid ? (
          <Actions id="idx-tab-menu-actions" backdrop={true} closeByBackdropClick={true}>
            <ActionsGroup>
              ...
              <ActionsButton onClick={() => props.onTabMenu("tab-color")}>
                {_t.textTabColor}
              </ActionsButton>
              {hiddenSheets.length ? (
                <ActionsButton onClick={() => props.onTabMenu("unhide")}>
                  {_t.textUnhide}
                </ActionsButton>
              ) : null}
            </ActionsGroup>
