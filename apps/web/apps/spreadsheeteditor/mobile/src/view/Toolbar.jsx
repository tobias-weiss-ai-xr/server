...
import { Link, NavLeft, NavRight } from "framework7-react"
...
const ToolbarView = (props) => {
  ...
  const undo_box =
    props.isEdit && EditorUIController.toolbarOptions
      ? EditorUIController.toolbarOptions.getUndoRedo({
    ...
    <Fragment>
      <NavLeft>
        {props.isDrawMode && (
          <Link
            iconOnly
            text={Device.ios ? t("Toolbar.textOk") : ""}
            className="back-reader-mode"
            onClick={() => Common.Notifications.trigger("draw:stop")}
          >
            {Device.android && (
        ...
        {!props.isDrawMode && props.isShowBack && !isVersionHistoryMode && (
          <Link
            iconOnly
            className={`btn-doc-back${(props.disabledControls || isOpenModal) && " disabled"}`}
      ...
      <NavRight>
        ...
        {props.showEditDocument && !isVersionHistoryMode && (
          <Link
            iconOnly
            className={props.disabledControls || isOpenModal ? "disabled" : ""}
            href={false}
            onClick={props.onEditDocument}
          >
            {Device.ios ? (
        ...
        {!props.isDrawMode &&
          ...
          !isVersionHistoryMode &&
          EditorUIController.toolbarOptions.getEditOptions({
            disabled:
              props.disabledEditControls || props.disabledControls || isDisconnected || isOpenModal,
            wsProps,
            focusOn,
            isShapeLocked,
        ...
        {Device.phone ? null : (
          <Link
            iconOnly
            className={
              (props.disabledControls || props.disabledSearch || isOpenModal) && "disabled"
            }
            searchbarEnable=".searchbar"
            href={false}
          >
            <SvgIcon symbolId={IconSearch.id} className={"icon icon-svg"} />
          </Link>
        )}
        {!props.isDrawMode &&
        ...
        !isVersionHistoryMode ? (
          <Link
            iconOnly
            className={
              (props.disabledControls || props.disabledCollaboration || isOpenModal) && "disabled"
            }
            id="btn-coauth"
            href={false}
            onClick={() => props.openOptions("coauth")}
          >
            <SvgIcon symbolId={IconCollaboration.id} className={"icon icon-svg"} />
          </Link>
        ) : null}
        {isVersionHistoryMode ? (
          <Link
            ...
            onClick={() => props.openOptions("history")}
          >
            <SvgIcon symbolId={IconHistory.id} className={"icon icon-svg"} />
          </Link>
        ) : null}
        <Link
          iconOnly
          className={
            (props.disabledSettings || props.disabledControls || isDisconnected || isOpenModal) &&
            "disabled"
          }
          id="btn-settings"
          href={false}
