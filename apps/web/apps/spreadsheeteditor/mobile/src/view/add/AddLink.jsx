...
const AddLink = (props) => {
  ...
  const [stateAutoUpdate, setAutoUpdate] = useState(!stateDisplayText)
  ...
  return (
    <Page routes={props.routes}>
      <Navbar className="navbar-link-settings">
        ...
        <NavRight>
          <Link
            className={`${((typeLink === "ext" && link.length < 1) || (typeLink === "int" && range.length < 1)) && "disabled"}`}
            onClick={() => {
              props.onInsertLink(
                typeLink === "ext"
