...
const PageEditTypeLink = ({ curType, changeType, storeFocusObjects }) => {
  ...
  const settings = !storeFocusObjects.focusOn
    ? []
    : storeFocusObjects.focusOn === "obj"
      ? storeFocusObjects.objects
      : storeFocusObjects.selections
  ...
  return (
    <Page>
      ...
      <List>
        ...
        />
        <ListItem
          title={_t.textInternalDataRange}
          radio
          checked={typeLink === 2}
          onClick={() => {
            setTypeLink(2)
...
const EditLink = (props) => {
  ...
  const linkSheet =
    valueTypeLink === Asc.c_oAscHyperlinkType.RangeLink
      ? getLinkSheet()
        ? getLinkSheet().caption
        : ""
      : currentSheet

  const [typeLink, setTypeLink] = useState(valueTypeLink)
  const textType =
    typeLink !== Asc.c_oAscHyperlinkType.RangeLink ? _t.textExternalLink : _t.textInternalDataRange
  ...
  const [link, setLink] = useState(
    linkInfo.asc_getHyperlinkUrl() ? linkInfo.asc_getHyperlinkUrl().replace(/ /g, "%20") : "",
  )
  ...
  return (
    <Page>
      <Navbar className="navbar-link-settings">
        ...
        <NavRight>
          <Link
            className={`${(typeLink === 1 && !link.length) || (typeLink === 2 && (!range.length || !curSheet.length) && "disabled")}`}
            onClick={() => {
              props.onEditLink(
                typeLink === 1
...
export { EditLink, _PageEditTypeLink as PageEditTypeLink, _PageEditSheet as PageEditSheet }
