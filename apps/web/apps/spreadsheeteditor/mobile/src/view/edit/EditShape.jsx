...
const EditShape = (props) => {
  ...
  const disableRemove = storeFocusObjects.selections.indexOf("text") > -1
...
const PageStyle = (props) => {
  ...
  const displayBorderSize =
    borderType === Asc.c_oAscStrokeType.STROKE_NONE
      ? 0
  ...
  if (
    (!shapeObject || storeFocusObjects.chartObject || storeFocusObjects.focusOn === "cell") &&
    Device.phone
  ) {
    ...
    <Page>
      <Navbar backLink={_t.textBack}>
        <div className="tab-buttons tabbar">
          <Link key={"se-link-shape-fill"} tabLink={"#edit-shape-fill"} tabLinkActive={true}>
            {_t.textFill}
          </Link>
          <Link key={"se-link-shape-border"} tabLink={"#edit-shape-border"}>
            {_t.textBorder}
          </Link>
          <Link key={"se-link-shape-effects"} tabLink={"#edit-shape-effects"}>
            {_t.textEffects}
          </Link>
...
const PageReplace = (props) => {
  ...
  const storeFocusObjects = props.storeFocusObjects
  if (
    (!storeFocusObjects.shapeObject ||
      storeFocusObjects.chartObject ||
      storeFocusObjects.focusOn === "cell") &&
    Device.phone
  ) {
    $$(".sheet-modal.modal-in").length > 0 && f7.sheet.close()
    return null
