...
import { Link, NavRight, NavTitle, Navbar, Page, Tab, Tabs, f7 } from "framework7-react"
    ...
    <Navbar>
      {editors.length > 1 ? (
        <div className="tab-buttons tabbar">
          ...
          {isAndroid && (
            <span className="tab-link-highlight" style={{ width: `${100 / editors.length}%` }} />
          )}
        </div>
      ) : (
...
const EditLayoutContent = ({ editors }) => {
  ...
  return <Page>{editors[0].component}</Page>
}

const EditingPage = inject("storeFocusObjects")(
  observer((props) => {
    const { t } = useTranslation()
    const _t = t("View.Edit", { returnObjects: true })
