import { Block, BlockTitle, Icon, List, ListItem, Navbar, Page, Toggle, f7 } from "framework7-react"
...
const PageApplicationSettings = (props) => {
  ...
  return (
    <Page>
        ...
        <Fragment>
          ...
          <List>
            <ListItem
              radio
              radioIcon="end"
              title={_t.textCentimeter}
              name="unit-of-measurement"
              checked={unitMeasurement === 0}
              onChange={() => changeMeasureSettings(0)}
            />
            <ListItem
              radio
              radioIcon="end"
              title={_t.textPoint}
              name="unit-of-measurement"
              checked={unitMeasurement === 1}
              onChange={() => changeMeasureSettings(1)}
            />
            <ListItem
              radio
              radioIcon="end"
          ...
          <List mediaList>
            <ListItem
              title={currentFormulaLang.displayValue}
              subtitle={`${t("View.Settings.textExample")}: ${currentFormulaLang.exampleValue}`}
              link="/formula-languages/"
              routeProps={{
          ...
          <List mediaList>
            <ListItem
              title={currentRegSetting.displayName}
              subtitle={`${t("View.Settings.textExample")}: ${regExample}`}
      ...
      <List>
        <ListItem
          title={_t.textMacrosSettings}
          link="/macros-settings/"
          routeProps={{
...
const PageRegionalSettings = (props) => {
  ...
  return (
    <Page className="regional-settings">
      ...
      <List>
        {regData.length
          ? regData.map((elem, index) => {
              return (
                <ListItem
                  key={index}
                  radio
                  checked={elem.code === regCode}
                  onChange={() => {
                ...
                >
                  <div className="item-title-row">
                    <Icon slot="media" icon={`lang-flag ${elem.langName}`} />
        ...
        {formulaLangsColection.map((elem, index) => {
          return (
            <ListItem
              ...
              subtitle={`${t("View.Settings.textExample")}: ${elem.exampleValue}`}
              checked={elem.value === formulaLang}
              onChange={() => {
  ...
  return (
    <Page>
      ...
      <List mediaList>
        <ListItem
          radio
          name="macros-settings"
          title={_t.textDisableAll}
          text={_t.textDisableAllMacrosWithoutNotification}
          checked={macrosMode === 2}
          onChange={() => changeMacros(2)}
        />
        <ListItem
          radio
          name="macros-settings"
          title={_t.textShowNotification}
          text={_t.textDisableAllMacrosWithNotification}
          checked={macrosMode === 0}
          onChange={() => changeMacros(0)}
        />
        <ListItem
          radio
          name="macros-settings"
          title={_t.textEnableAll}
          text={_t.textEnableAllMacrosWithoutNotification}
          checked={macrosMode === 1}
          onChange={() => changeMacros(1)}
        />
      </List>
    </Page>
  )
...
export { ApplicationSettings, MacrosSettings, RegionalSettings, FormulaLanguage, ThemeSettings }
