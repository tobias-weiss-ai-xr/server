...
VE.ApplicationController = new (function () {
  ...
  function onPagesChanged() {
    ...
    const handlePages = function (e) {
      const $page = $(this)
      let index = $page.attr("id").match(/\d+$/)

      if (index.length > 0) {
        index = Number.parseInt(index[0])

        if (index > -1 && index < maxPages) api.goToPage(index)
      }
    }
    ...
    const tpl = '<li id="id-page{index}" tabtitle="{tabtitle}" {style}>{title}</li>'
    for (let i = 0; i < maxPages; i++) {
      // escape html
      const name = api.asc_getPageName(i) || me.txtPage + (i + 1)
  ...
  function setupScrollButtons() {
    const $container = $("#pages-container")
    const $prevButton = $("#pages-list-button-prev")
    const $nextButton = $("#pages-list-button-next")
    const $box = $("#id-pages")
    const rtlPage = window.isrtl
    ...
    const buttonWidth = $(".pages-list-buttons").outerWidth()
        ...
        $($box.children().get().reverse()).each(function () {
          ...
          if (right > rightBound) {
            $container.scrollLeft(
              $container.scrollLeft() + right - rightBound + ($container.width() > 400 ? 20 : 5),
            )
            return false
          }
        })
  ...
  function onDocumentContentReady() {
    ...
    VE.ApplicationView.tools.get("#idt-download").on("click", () => {
      if (!!embedConfig.saveUrl && permissions.download !== false) {
    ...
    VE.ApplicationView.tools.get("#idt-print").on("click", () => {
      api.asc_Print(
        new Asc.asc_CDownloadOptions(
          null,
          $.browser.chrome ||
    ...
    VE.ApplicationView.tools.get("#idt-close").on("click", () => {
  ...
  function onDownloadAs() {
    ...
    if (api) {
      const options = new Asc.asc_CDownloadOptions(Asc.c_oAscFileType.VSDX, true)
