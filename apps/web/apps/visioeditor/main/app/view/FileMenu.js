...
define([
  ...
  VE.Views.FileMenu = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#file-menu-panel",
        rendered: false,
        ...
        applyMode: function () {
          ...
          if (!this.panels) {
            this.panels = {
              opts: new VE.Views.FileMenuPanels.Settings({ menu: this }).render(
                this.$el.find("#panel-settings"),
              ),
              info: new VE.Views.FileMenuPanels.DocumentInfo({ menu: this }).render(
                this.$el.find("#panel-info"),
              ),
              rights: new VE.Views.FileMenuPanels.DocumentRights({ menu: this }).render(
          ...
          if (this.mode.canCreateNew) {
            if (this.mode.templates?.length) {
              !this.panels.new &&
                (this.panels.new = new VE.Views.FileMenuPanels.CreateNew({
                  menu: this,
          ...
          if (this.mode.canDownload) {
            !this.panels.saveas &&
              (this.panels.saveas = new VE.Views.FileMenuPanels.ViewSaveAs({
                menu: this,
          ...
          if (this.mode.canDownload && (this.mode.canRequestSaveAs || this.mode.saveAsUrl)) {
            !this.panels["save-copy"] &&
              (this.panels["save-copy"] = new VE.Views.FileMenuPanels.ViewSaveCopy({
                menu: this,
                fileType: this.document.fileType,
              }).render())
          }
          ...
          if (this.mode.canPreviewPrint) {
            const printPanel = VE.getController("Print").getView("PrintWithPreview")
