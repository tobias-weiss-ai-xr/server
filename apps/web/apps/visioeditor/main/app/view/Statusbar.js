...
define([
  ...
  function _onAppReady(config) {
    this.btnScrollBack.updateHint(this.tipPrev)
    this.btnScrollNext.updateHint(this.tipNext)
    this.btnZoomToPage.updateHint(this.tipFitPage)
    this.btnZoomToWidth.updateHint(this.tipFitWidth)
    this.cntSheetList.updateHint(this.tipListOfSheets)
    VE.getController("Common.Controllers.Shortcuts").updateShortcutHints({
      ZoomOut: {
        btn: this.btnZoomDown,
        label: this.tipZoomOut + Common.Utils.String.platformKey("Ctrl+-"),
      },
      ZoomIn: {
        btn: this.btnZoomUp,
        label: this.tipZoomIn + Common.Utils.String.platformKey("Ctrl++"),
      },
    })
  ...
  VE.Views.Statusbar = Common.UI.BaseView.extend(
    _.extend(
      {
        el: "#statusbar",
        ...
        setMode: function (mode) {
          ...
          this.tabBarDefPosition += Number.parseFloat($("#status-addtabs-box").css("width"))
        ...
        update: function () {
          ...
          if (this.api) {
            const wc = this.api.getCountPages()
            let i = -1
            const items = []
            let tab
            const sindex = this._state.pageCurrent
            ...
            while (++i < wc) {
              tab = {
                ...
                index: items.length,
                active: sindex === i,
                label: this.api.asc_getPageName(i) || this.txtPage + (i + 1),
                isLockTheDrag: true,
              }
              items.push(tab)
            }
            ...
            items.forEach((item) => {
              const hidden = false //me.api.asc_isWorksheetHidden(item.sheetindex);
            ...
            if (!this.tabbar.isTabVisible(sindex)) this.tabbar.setTabVisible(sindex)
