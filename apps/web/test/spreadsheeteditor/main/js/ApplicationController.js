...
SSE.ApplicationController = new (function () {
  ...
  function loadDocument(data) {
    ...
    if (docConfig) {
      ...
      const _permissions = $.extend({}, docConfig.permissions)
  ...
  function onSheetsChanged() {
    ...
    const handleWorksheet = function (e) {
      ...
      if (index.length > 0) {
        ...
        if (index > -1 && index < maxPages) setActiveWorkSheet(index)
      }
    }
    ...
    const tpl = '<li id="worksheet{index}">{title}</li>'
    for (let i = 0; i < maxPages; i++) {
      const item = tpl
        .replace(/\{index}/, i)
        .replace(/\{title}/, api.asc_getWorksheetName(i).replace(/\s/g, "&nbsp;"))
    ...
    if (type === Asc.c_oAscAsyncActionType.BlockInteraction) {
      switch (id) {
        case Asc.c_oAscAsyncAction.Open:
          if (api) {
            api.asc_Resize()
            const zf = config.customization?.zoom
              ? Number.parseInt(config.customization.zoom) / 100
              : 1
