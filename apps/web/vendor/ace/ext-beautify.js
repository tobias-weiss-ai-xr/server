ace.define(
  ...
  (e, t, n) => {
    ...
    const r = e("../token_iterator").TokenIterator
    ;(t.singletonTags = [
      "area",
      "base",
      "br",
      ...
      (t.beautify = (e) => {
        const n = new r(e, 0, 0)
        let s = n.getCurrentToken()
        const o = e.getTabString()
        const u = t.singletonTags
        const a = t.blockTags
        const f = t.formatOptions || {}
        let l
        let c = !1
        let h = !1
        let p = !1
        ...
        let B = !1
        const j = () => {
          l?.value && l.type !== "string.regexp" && (l.value = l.value.replace(/^\s*/, ""))
        }
        const F = () => {
          let e = d.length - 1
          for (;;) {
            ...
            ;(v = s.value),
              ...
              i(s, "tag-open")
                ? ((M = !0),
                  ...
                  v === "</" && (D && !c && C < 1 && C++, _ && (C = 1), (E = 1), (D = !1)))
                : i(s, "tag-close")
                  ? (M = !1)
                  : i(s, "comment.start")
                    ? (D = !0)
                    : i(s, "comment.end") && (D = !1),
              !M && !C && s.type === "paren.rparen" && s.value.substr(0, 1) === "}" && C++,
              N !== T && ((C = N), T && (C -= T))
            if (C) {
              I()
              for (; C > 0; C--) d += "\n"
              ;(c = !0),
                !i(s, "comment") && !s.type.match(/^(comment|string)$/) && (v = v.trimLeft())
            }
            if (v) {
              s.type === "keyword" && v.match(/^(if|else|elseif|for|foreach|while|switch)$/)
                ? ((H[g] = v),
                  j(),
                  (p = !0),
                  v.match(/^(else|elseif)$/) && d.match(/\}[\s]*$/) && (I(), (h = !0)))
                : s.type === "paren.lparen"
                  ? (j(),
                  ...
                  : s.type === "paren.rparen"
                    ? ((E = 1),
                      v.substr(0, 1) === "}" &&
                        (H[g - 1] === "case" && E++,
                        d.trimRight().substr(-1) === "{" ? I() : ((h = !0), _ && (C += 2))),
                      v.substr(0, 1) === "]" &&
                        d.substr(-1) !== "}" &&
                        d.trimRight().substr(-1) === "}" &&
                        ((h = !1), w++, I()),
              ...
              if (
                c &&
                (!s.type.match(/^(comment)$/) || !!v.substr(0, 1).match(/^[/#]$/)) &&
