ace.define(
  ...
  (require, exports, module) => {
    ...
    const getCurrentToken = (cursor) =>
      cvoxAce.editor.getSession().getTokenAt(cursor.row, cursor.column + 1)
    ...
    const speakTokenQueue = (token) => {
      speakToken_(token, 1)
    }
    const getTokenRule = (token) => {
      if (!token || !token.type) {
        return
      }
    ...
    const rowColToString = (row, col) => `row ${row + 1} column ${col + 1}`
