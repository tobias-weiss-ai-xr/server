ace.define(
  "ace/occur",
  [
    "require",
    "exports",
    "module",
    "ace/lib/oop",
    "ace/search",
    "ace/edit_session",
    "ace/search_highlight",
    "ace/lib/dom",
  ],
  function (e, t, n) {
    const r =
      (this && this.__extends) ||
      (() => {
        let e = (t, n) => (
          (e =
            Object.setPrototypeOf ||
            (Array.isArray({ __proto__: [] }) &&
              ((e, t) => {
                e.__proto__ = t
              })) ||
            ((e, t) => {
              for (const n in t) Object.prototype.hasOwnProperty.call(t, n) && (e[n] = t[n])
            })),
          e(t, n)
        )
        return (t, n) => {
          function r() {
            this.constructor = t
          }
          if (typeof n !== "function" && n !== null)
            throw new TypeError(`Class extends value ${String(n)} is not a constructor or null`)
          e(t, n),
            (t.prototype = n === null ? Object.create(n) : ((r.prototype = n.prototype), new r()))
        }
      })()
    const i = e("./lib/oop")
    const s = e("./search").Search
    const o = e("./edit_session").EditSession
    const u = e("./search_highlight").SearchHighlight
    const a = ((e) => {
      function t() {
        return e?.apply(this, arguments) || this
      }
      return (
        r(t, e),
        (t.prototype.enter = function (e, t) {
          if (!t.needle) return !1
          const n = e.getCursorPosition()
          this.displayOccurContent(e, t)
          const r = this.originalToOccurPosition(e.session, n)
          return e.moveCursorToPosition(r), !0
        }),
        (t.prototype.exit = function (e, t) {
          const n = t.translatePosition && e.getCursorPosition()
          const r = n && this.occurToOriginalPosition(e.session, n)
          return this.displayOriginalContent(e), r && e.moveCursorToPosition(r), !0
        }),
        (t.prototype.highlight = (e, t) => {
          const n = (e.$occurHighlight =
            e.$occurHighlight || e.addDynamicMarker(new u(null, "ace_occur-highlight", "text")))
          n.setRegexp(t), e._emit("changeBackMarker")
        }),
        (t.prototype.displayOccurContent = function (e, t) {
          this.$originalSession = e.session
          const n = this.matchingLines(e.session, t)
          const r = n.map((e) => e.content)
          const i = new o(r.join("\n"))
          ;(i.$occur = this),
            (i.$occurMatchingLines = n),
            e.setSession(i),
            (this.$useEmacsStyleLineStart = this.$originalSession.$useEmacsStyleLineStart),
            (i.$useEmacsStyleLineStart = this.$useEmacsStyleLineStart),
            this.highlight(i, t.re),
            i._emit("changeBackMarker")
        }),
        (t.prototype.displayOriginalContent = function (e) {
          e.setSession(this.$originalSession),
            (this.$originalSession.$useEmacsStyleLineStart = this.$useEmacsStyleLineStart)
        }),
        (t.prototype.originalToOccurPosition = (e, t) => {
          const n = e.$occurMatchingLines
          const r = { row: 0, column: 0 }
          if (!n) return r
          for (let i = 0; i < n.length; i++)
            if (n[i].row === t.row) return { row: i, column: t.column }
          return r
        }),
        (t.prototype.occurToOriginalPosition = (e, t) => {
          const n = e.$occurMatchingLines
          return !n || !n[t.row] ? t : { row: n[t.row].row, column: t.column }
        }),
        (t.prototype.matchingLines = (e, t) => {
          t = i.mixin({}, t)
          if (!e || !t.needle) return []
          const n = new s()
          return (
            n.set(t),
            n.findAll(e).reduce((t, n) => {
              const r = n.start.row
              const i = t[t.length - 1]
              return i && i.row === r ? t : t.concat({ row: r, content: e.getLine(r) })
            }, [])
          )
        }),
        t
      )
    })(s)
    const f = e("./lib/dom")
    f.importCssString(
      ".ace_occur-highlight {\n    border-radius: 4px;\n    background-color: rgba(87, 255, 8, 0.25);\n    position: absolute;\n    z-index: 4;\n    box-sizing: border-box;\n    box-shadow: 0 0 4px rgb(91, 255, 50);\n}\n.ace_dark .ace_occur-highlight {\n    background-color: rgb(80, 140, 85);\n    box-shadow: 0 0 4px rgb(60, 120, 70);\n}\n",
      "incremental-occur-highlighting",
      !1,
    ),
      (t.Occur = a)
  },
),
  ace.define(
    "ace/commands/occur_commands",
    [
      "require",
      "exports",
      "module",
      "ace/config",
      "ace/occur",
      "ace/keyboard/hash_handler",
      "ace/lib/oop",
    ],
    (e, t, n) => {
      function f() {}
      const r = e("../config")
      const i = e("../occur").Occur
      const s = {
        name: "occur",
        exec: (e, t) => {
          const n = !!e.session.$occur
          const r = new i().enter(e, t)
          r && !n && f.installIn(e)
        },
        readOnly: !0,
      }
      const o = [
        {
          name: "occurexit",
          bindKey: "esc|Ctrl-G",
          exec: (e) => {
            const t = e.session.$occur
            if (!t) return
            t.exit(e, {}), e.session.$occur || f.uninstallFrom(e)
          },
          readOnly: !0,
        },
        {
          name: "occuraccept",
          bindKey: "enter",
          exec: (e) => {
            const t = e.session.$occur
            if (!t) return
            t.exit(e, { translatePosition: !0 }), e.session.$occur || f.uninstallFrom(e)
          },
          readOnly: !0,
        },
      ]
      const u = e("../keyboard/hash_handler").HashHandler
      const a = e("../lib/oop")
      a.inherits(f, u),
        function () {
          ;(this.isOccurHandler = !0),
            (this.attach = function (e) {
              u.call(this, o, e.commands.platform), (this.$editor = e)
            })
          const e = this.handleKeyboard
          this.handleKeyboard = function (t, n, r, i) {
            const s = e.call(this, t, n, r, i)
            return s?.command ? s : undefined
          }
        }.call(f.prototype),
        (f.installIn = function (e) {
          const t = new this()
          e.keyBinding.addKeyboardHandler(t), e.commands.addCommands(o)
        }),
        (f.uninstallFrom = (e) => {
          e.commands.removeCommands(o)
          const t = e.getKeyboardHandler()
          t.isOccurHandler && e.keyBinding.removeKeyboardHandler(t)
        }),
        (t.occurStartCommand = s)
    },
  ),
  ace.define(
    "ace/commands/incremental_search_commands",
    [
      "require",
      "exports",
      "module",
      "ace/config",
      "ace/lib/oop",
      "ace/keyboard/hash_handler",
      "ace/commands/occur_commands",
    ],
    (e, t, n) => {
      function u(e) {
        this.$iSearch = e
      }
      const r = e("../config")
      const i = e("../lib/oop")
      const s = e("../keyboard/hash_handler").HashHandler
      const o = e("./occur_commands").occurStartCommand
      ;(t.iSearchStartCommands = [
        {
          name: "iSearch",
          bindKey: { win: "Ctrl-F", mac: "Command-F" },
          exec: (e, t) => {
            r.loadModule(["core", "ace/incremental_search"], (n) => {
              const r = (n.iSearch = n.iSearch || new n.IncrementalSearch())
              r.activate(e, t.backwards), t.jumpToFirstMatch && r.next(t)
            })
          },
          readOnly: !0,
        },
        {
          name: "iSearchBackwards",
          exec: (e, t) => {
            e.execCommand("iSearch", { backwards: !0 })
          },
          readOnly: !0,
        },
        {
          name: "iSearchAndGo",
          bindKey: { win: "Ctrl-K", mac: "Command-G" },
          exec: (e, t) => {
            e.execCommand("iSearch", { jumpToFirstMatch: !0, useCurrentOrPrevSearch: !0 })
          },
          readOnly: !0,
        },
        {
          name: "iSearchBackwardsAndGo",
          bindKey: { win: "Ctrl-Shift-K", mac: "Command-Shift-G" },
          exec: (e) => {
            e.execCommand("iSearch", {
              jumpToFirstMatch: !0,
              backwards: !0,
              useCurrentOrPrevSearch: !0,
            })
          },
          readOnly: !0,
        },
      ]),
        (t.iSearchCommands = [
          {
            name: "restartSearch",
            bindKey: { win: "Ctrl-F", mac: "Command-F" },
            exec: (e) => {
              e.cancelSearch(!0)
            },
          },
          {
            name: "searchForward",
            bindKey: { win: "Ctrl-S|Ctrl-K", mac: "Ctrl-S|Command-G" },
            exec: (e, t) => {
              ;(t.useCurrentOrPrevSearch = !0), e.next(t)
            },
          },
          {
            name: "searchBackward",
            bindKey: { win: "Ctrl-R|Ctrl-Shift-K", mac: "Ctrl-R|Command-Shift-G" },
            exec: (e, t) => {
              ;(t.useCurrentOrPrevSearch = !0), (t.backwards = !0), e.next(t)
            },
          },
          {
            name: "extendSearchTerm",
            exec: (e, t) => {
              e.addString(t)
            },
          },
          {
            name: "extendSearchTermSpace",
            bindKey: "space",
            exec: (e) => {
              e.addString(" ")
            },
          },
          {
            name: "shrinkSearchTerm",
            bindKey: "backspace",
            exec: (e) => {
              e.removeChar()
            },
          },
          {
            name: "confirmSearch",
            bindKey: "return",
            exec: (e) => {
              e.deactivate()
            },
          },
          {
            name: "cancelSearch",
            bindKey: "esc|Ctrl-G",
            exec: (e) => {
              e.deactivate(!0)
            },
          },
          {
            name: "occurisearch",
            bindKey: "Ctrl-O",
            exec: (e) => {
              const t = i.mixin({}, e.$options)
              e.deactivate(), o.exec(e.$editor, t)
            },
          },
          {
            name: "yankNextWord",
            bindKey: "Ctrl-w",
            exec: (e) => {
              const t = e.$editor
              const n = t.selection.getRangeOfMovements((e) => {
                e.moveCursorWordRight()
              })
              const r = t.session.getTextRange(n)
              e.addString(r)
            },
          },
          {
            name: "yankNextChar",
            bindKey: "Ctrl-Alt-y",
            exec: (e) => {
              const t = e.$editor
              const n = t.selection.getRangeOfMovements((e) => {
                e.moveCursorRight()
              })
              const r = t.session.getTextRange(n)
              e.addString(r)
            },
          },
          {
            name: "recenterTopBottom",
            bindKey: "Ctrl-l",
            exec: (e) => {
              e.$editor.execCommand("recenterTopBottom")
            },
          },
          {
            name: "selectAllMatches",
            bindKey: "Ctrl-space",
            exec: (e) => {
              const t = e.$editor
              const n = t.session.$isearchHighlight
              const r = n?.cache ? n.cache.reduce((e, t) => e.concat(t ? t : []), []) : []
              e.deactivate(!1), r.forEach(t.selection.addRange.bind(t.selection))
            },
          },
          {
            name: "searchAsRegExp",
            bindKey: "Alt-r",
            exec: (e) => {
              e.convertNeedleToRegExp()
            },
          },
        ].map(
          (e) => (
            (e.readOnly = !0),
            (e.isIncrementalSearchCommand = !0),
            (e.scrollIntoView = "animate-cursor"),
            e
          ),
        )),
        i.inherits(u, s),
        function () {
          ;(this.attach = function (e) {
            const n = this.$iSearch
            s.call(this, t.iSearchCommands, e.commands.platform),
              (this.$commandExecHandler = e.commands.on("exec", (t) => {
                if (!t.command.isIncrementalSearchCommand) return n.deactivate()
                t.stopPropagation(), t.preventDefault()
                const r = e.session.getScrollTop()
                const i = t.command.exec(n, t.args || {})
                return e.renderer.scrollCursorIntoView(null, 0.5), e.renderer.animateScrolling(r), i
              }))
          }),
            (this.detach = function (e) {
              if (!this.$commandExecHandler) return
              e.commands.off("exec", this.$commandExecHandler),
                (this.$commandExecHandler = undefined)
            })
          const e = this.handleKeyboard
          this.handleKeyboard = function (t, n, r, i) {
            if (((n === 1 || n === 8) && r === "v") || (n === 1 && r === "y")) return null
            const s = e.call(this, t, n, r, i)
            if (s?.command) return s
            if (n === -1) {
              const o = this.commands.extendSearchTerm
              if (o) return { command: o, args: r }
            }
            return !1
          }
        }.call(u.prototype),
        (t.IncrementalSearchKeyboardHandler = u)
    },
  ),
  ace.define(
    "ace/incremental_search",
    [
      "require",
      "exports",
      "module",
      "ace/range",
      "ace/search",
      "ace/search_highlight",
      "ace/commands/incremental_search_commands",
      "ace/lib/dom",
      "ace/commands/command_manager",
      "ace/editor",
      "ace/config",
    ],
    function (e, t, n) {
      function f(e) {
        return e instanceof RegExp
      }
      function l(e) {
        const t = String(e)
        const n = t.indexOf("/")
        const r = t.lastIndexOf("/")
        return { expression: t.slice(n + 1, r), flags: t.slice(r + 1) }
      }
      function c(e, t) {
        try {
          return new RegExp(e, t)
        } catch (n) {
          return e
        }
      }
      function h(e) {
        return c(e.expression, e.flags)
      }
      const r =
        (this && this.__extends) ||
        (() => {
          let e = (t, n) => (
            (e =
              Object.setPrototypeOf ||
              (Array.isArray({ __proto__: [] }) &&
                ((e, t) => {
                  e.__proto__ = t
                })) ||
              ((e, t) => {
                for (const n in t) Object.prototype.hasOwnProperty.call(t, n) && (e[n] = t[n])
              })),
            e(t, n)
          )
          return (t, n) => {
            function r() {
              this.constructor = t
            }
            if (typeof n !== "function" && n !== null)
              throw new TypeError(`Class extends value ${String(n)} is not a constructor or null`)
            e(t, n),
              (t.prototype = n === null ? Object.create(n) : ((r.prototype = n.prototype), new r()))
          }
        })()
      const i = e("./range").Range
      const s = e("./search").Search
      const o = e("./search_highlight").SearchHighlight
      const u = e("./commands/incremental_search_commands")
      const a = u.IncrementalSearchKeyboardHandler
      const p = ((e) => {
        function t() {
          const t = e.call(this) || this
          return (t.$options = { wrap: !1, skipCurrent: !1 }), (t.$keyboardHandler = new a(t)), t
        }
        return (
          r(t, e),
          (t.prototype.activate = function (e, t) {
            ;(this.$editor = e),
              (this.$startPos = this.$currentPos = e.getCursorPosition()),
              (this.$options.needle = ""),
              (this.$options.backwards = t),
              e.keyBinding.addKeyboardHandler(this.$keyboardHandler),
              (this.$originalEditorOnPaste = e.onPaste),
              (e.onPaste = this.onPaste.bind(this)),
              (this.$mousedownHandler = e.on("mousedown", this.onMouseDown.bind(this))),
              this.selectionFix(e),
              this.statusMessage(!0)
          }),
          (t.prototype.deactivate = function (e) {
            this.cancelSearch(e)
            const t = this.$editor
            t.keyBinding.removeKeyboardHandler(this.$keyboardHandler),
              this.$mousedownHandler &&
                (t.off("mousedown", this.$mousedownHandler), (this.$mousedownHandler = undefined)),
              (t.onPaste = this.$originalEditorOnPaste),
              this.message("")
          }),
          (t.prototype.selectionFix = (e) => {
            e.selection.isEmpty() && !e.session.$emacsMark && e.clearSelection()
          }),
          (t.prototype.highlight = function (e) {
            const t = this.$editor.session
            const n = (t.$isearchHighlight =
              t.$isearchHighlight || t.addDynamicMarker(new o(null, "ace_isearch-result", "text")))
            n.setRegexp(e), t._emit("changeBackMarker")
          }),
          (t.prototype.cancelSearch = function (e) {
            const t = this.$editor
            return (
              (this.$prevNeedle = this.$options.needle),
              (this.$options.needle = ""),
              e
                ? (t.moveCursorToPosition(this.$startPos), (this.$currentPos = this.$startPos))
                : t.pushEmacsMark?.(this.$startPos, !1),
              this.highlight(null),
              i.fromPoints(this.$currentPos, this.$currentPos)
            )
          }),
          (t.prototype.highlightAndFindWithNeedle = function (e, t) {
            if (!this.$editor) return null
            const n = this.$options
            t && (n.needle = t.call(this, n.needle || "") || "")
            if (n.needle.length === 0) return this.statusMessage(!0), this.cancelSearch(!0)
            n.start = this.$currentPos
            const r = this.$editor.session
            let s = this.find(r)
            const o = this.$editor.emacsMark
              ? !!this.$editor.emacsMark()
              : !this.$editor.selection.isEmpty()
            return (
              s &&
                (n.backwards && (s = i.fromPoints(s.end, s.start)),
                this.$editor.selection.setRange(i.fromPoints(o ? this.$startPos : s.end, s.end)),
                e && (this.$currentPos = s.end),
                this.highlight(n.re)),
              this.statusMessage(s),
              s
            )
          }),
          (t.prototype.addString = function (e) {
            return this.highlightAndFindWithNeedle(!1, (t) => {
              if (!f(t)) return t + e
              const n = l(t)
              return (n.expression += e), h(n)
            })
          }),
          (t.prototype.removeChar = function (e) {
            return this.highlightAndFindWithNeedle(!1, (e) => {
              if (!f(e)) return e.substring(0, e.length - 1)
              const t = l(e)
              return (t.expression = t.expression.substring(0, t.expression.length - 1)), h(t)
            })
          }),
          (t.prototype.next = function (e) {
            return (
              (e = e || {}),
              (this.$options.backwards = !!e.backwards),
              (this.$currentPos = this.$editor.getCursorPosition()),
              this.highlightAndFindWithNeedle(!0, function (t) {
                return e.useCurrentOrPrevSearch && t.length === 0 ? this.$prevNeedle || "" : t
              })
            )
          }),
          (t.prototype.onMouseDown = function (e) {
            return this.deactivate(), !0
          }),
          (t.prototype.onPaste = function (e) {
            this.addString(e)
          }),
          (t.prototype.convertNeedleToRegExp = function () {
            return this.highlightAndFindWithNeedle(!1, (e) => (f(e) ? e : c(e, "ig")))
          }),
          (t.prototype.convertNeedleToString = function () {
            return this.highlightAndFindWithNeedle(!1, (e) => (f(e) ? l(e).expression : e))
          }),
          (t.prototype.statusMessage = function (e) {
            const t = this.$options
            let n = ""
            ;(n += t.backwards ? "reverse-" : ""),
              (n += `isearch: ${t.needle}`),
              (n += e ? "" : " (not found)"),
              this.message(n)
          }),
          (t.prototype.message = function (e) {
            this.$editor.showCommandLine && (this.$editor.showCommandLine(e), this.$editor.focus())
          }),
          t
        )
      })(s)
      t.IncrementalSearch = p
      const d = e("./lib/dom")
      d.importCssString(
        "\n.ace_marker-layer .ace_isearch-result {\n  position: absolute;\n  z-index: 6;\n  box-sizing: border-box;\n}\ndiv.ace_isearch-result {\n  border-radius: 4px;\n  background-color: rgba(255, 200, 0, 0.5);\n  box-shadow: 0 0 4px rgb(255, 200, 0);\n}\n.ace_dark div.ace_isearch-result {\n  background-color: rgb(100, 110, 160);\n  box-shadow: 0 0 4px rgb(80, 90, 140);\n}",
        "incremental-search-highlighting",
        !1,
      )
      const v = e("./commands/command_manager")
      ;(function () {
        this.setupIncrementalSearch = function (e, t) {
          if (this.usesIncrementalSearch === t) return
          this.usesIncrementalSearch = t
          const n = u.iSearchStartCommands
          const r = t ? "addCommands" : "removeCommands"
          this[r](n)
        }
      }).call(v.CommandManager.prototype)
      const m = e("./editor").Editor
      e("./config").defineOptions(m.prototype, "editor", {
        useIncrementalSearch: {
          set: function (e) {
            this.keyBinding.$handlers.forEach(function (t) {
              t.setupIncrementalSearch?.(this, e)
            }),
              this._emit("incrementalSearchSettingChanged", { isEnabled: e })
          },
        },
      })
    },
  ),
  ace.define(
    "ace/keyboard/emacs",
    [
      "require",
      "exports",
      "module",
      "ace/lib/dom",
      "ace/incremental_search",
      "ace/commands/incremental_search_commands",
      "ace/keyboard/hash_handler",
      "ace/lib/keys",
    ],
    (e, t, n) => {
      function a(e, t) {
        const n = e[e.length - 1]
        if (n && n.row === t.row && n.column === t.column) return
        e.push(t)
      }
      const r = e("../lib/dom")
      e("../incremental_search")
      const i = e("../commands/incremental_search_commands")
      const s = e("./hash_handler").HashHandler
      ;(t.handler = new s()),
        (t.handler.isEmacs = !0),
        (t.handler.$id = "ace/keyboard/emacs"),
        r.importCssString(
          "\n.emacs-mode .ace_cursor{\n    border: 1px rgba(50,250,50,0.8) solid!important;\n    box-sizing: border-box!important;\n    background-color: rgba(0,250,0,0.9);\n    opacity: 0.5;\n}\n.emacs-mode .ace_hidden-cursors .ace_cursor{\n    opacity: 1;\n    background-color: transparent;\n}\n.emacs-mode .ace_overwrite-cursors .ace_cursor {\n    opacity: 1;\n    background-color: transparent;\n    border-width: 0 0 2px 2px !important;\n}\n.emacs-mode .ace_text-layer {\n    z-index: 4\n}\n.emacs-mode .ace_cursor-layer {\n    z-index: 2\n}",
          "emacsMode",
        )
      let o
      let u
      ;(t.handler.attach = function (e) {
        ;(o = e.session.$selectLongWords),
          (e.session.$selectLongWords = !0),
          (u = e.session.$useEmacsStyleLineStart),
          (e.session.$useEmacsStyleLineStart = !0),
          (e.session.$emacsMark = null),
          (e.session.$emacsMarkRing = e.session.$emacsMarkRing || []),
          (e.emacsMark = function () {
            return this.session.$emacsMark
          }),
          (e.setEmacsMark = function (e) {
            this.session.$emacsMark = e
          }),
          (e.pushEmacsMark = function (e, t) {
            const n = this.session.$emacsMark
            n && a(this.session.$emacsMarkRing, n),
              !e || t ? this.setEmacsMark(e) : a(this.session.$emacsMarkRing, e)
          }),
          (e.popEmacsMark = function () {
            const e = this.emacsMark()
            return e ? (this.setEmacsMark(null), e) : this.session.$emacsMarkRing.pop()
          }),
          (e.getLastEmacsMark = function (e) {
            return this.session.$emacsMark || this.session.$emacsMarkRing.slice(-1)[0]
          }),
          (e.emacsMarkForSelection = function (e) {
            const t = this.selection
            const n = this.multiSelect ? this.multiSelect.getAllRanges().length : 1
            const r = t.index || 0
            const i = this.session.$emacsMarkRing
            const s = i.length - (n - r)
            const o = i[s] || t.anchor
            return e && i.splice(s, 1, "row" in e && "column" in e ? e : undefined), o
          }),
          e.on("click", l),
          e.on("changeSession", f),
          (e.renderer.$blockCursor = !0),
          e.setStyle("emacs-mode"),
          e.commands.addCommands(d),
          (t.handler.platform = e.commands.platform),
          (e.$emacsModeHandler = this),
          e.on("copy", this.onCopy),
          e.on("paste", this.onPaste)
      }),
        (t.handler.detach = function (e) {
          ;(e.renderer.$blockCursor = !1),
            (e.session.$selectLongWords = o),
            (e.session.$useEmacsStyleLineStart = u),
            e.off("click", l),
            e.off("changeSession", f),
            e.unsetStyle("emacs-mode"),
            e.commands.removeCommands(d),
            e.off("copy", this.onCopy),
            e.off("paste", this.onPaste),
            (e.$emacsModeHandler = null)
        })
      const f = (e) => {
        e.oldSession &&
          ((e.oldSession.$selectLongWords = o), (e.oldSession.$useEmacsStyleLineStart = u)),
          (o = e.session.$selectLongWords),
          (e.session.$selectLongWords = !0),
          (u = e.session.$useEmacsStyleLineStart),
          (e.session.$useEmacsStyleLineStart = !0),
          e.session.hasOwnProperty("$emacsMark") || (e.session.$emacsMark = null),
          e.session.hasOwnProperty("$emacsMarkRing") || (e.session.$emacsMarkRing = [])
      }
      const l = (e) => {
        e.editor.session.$emacsMark = null
      }
      const c = e("../lib/keys").KEY_MODS
      const h = { C: "ctrl", S: "shift", M: "alt", CMD: "command" }
      const p = [
        "C-S-M-CMD",
        "S-M-CMD",
        "C-M-CMD",
        "C-S-CMD",
        "C-S-M",
        "M-CMD",
        "S-CMD",
        "S-M",
        "C-CMD",
        "C-M",
        "C-S",
        "CMD",
        "M",
        "S",
        "C",
      ]
      p.forEach((e) => {
        let t = 0
        e.split("-").forEach((e) => {
          t |= c[h[e]]
        }),
          (h[t] = `${e.toLowerCase()}-`)
      }),
        (t.handler.onCopy = (e, n) => {
          if (n.$handlesEmacsOnCopy) return
          ;(n.$handlesEmacsOnCopy = !0),
            t.handler.commands.killRingSave.exec(n),
            (n.$handlesEmacsOnCopy = !1)
        }),
        (t.handler.onPaste = (e, t) => {
          t.pushEmacsMark(t.getCursorPosition())
        }),
        (t.handler.bindKey = function (e, t) {
          typeof e === "object" && (e = e[this.platform])
          if (!e) return
          const n = this.commandKeyBinding
          e.split("|").forEach((e) => {
            ;(e = e.toLowerCase()), (n[e] = t)
            const r = e.split(" ").slice(0, -1)
            r.reduce((e, t, n) => {
              const r = e[n - 1] ? `${e[n - 1]} ` : ""
              return e.concat([r + t])
            }, []).forEach((e) => {
              n[e] || (n[e] = "null")
            })
          }, this)
        }),
        (t.handler.getStatusText = (e, t) => {
          let n = ""
          return t.count && (n += t.count), t.keyChain && (n += ` ${t.keyChain}`), n
        }),
        (t.handler.handleKeyboard = function (e, t, n, r) {
          if (r === -1) return undefined
          const i = e.editor
          i._signal("changeStatus")
          if (t === -1) {
            i.pushEmacsMark()
            if (e.count) {
              const s = new Array(e.count + 1).join(n)
              return (e.count = null), { command: "insertstring", args: s }
            }
          }
          const o = h[t]
          if (o === "c-" || e.count) {
            const u = Number.parseInt(n[n.length - 1])
            if (typeof u === "number" && !Number.isNaN(u))
              return (
                (e.count = Math.max(e.count, 0) || 0),
                (e.count = 10 * e.count + u),
                { command: "null" }
              )
          }
          o && (n = o + n), e.keyChain && (n = e.keyChain += ` ${n}`)
          let a = this.commandKeyBinding[n]
          e.keyChain = a === "null" ? n : ""
          if (!a) return undefined
          if (a === "null") return { command: "null" }
          if (a === "universalArgument") return (e.count = -4), { command: "null" }
          let f
          typeof a !== "string" &&
            ((f = a.args),
            a.command && (a = a.command),
            a === "goorselect" && ((a = i.emacsMark() ? f[1] : f[0]), (f = null)))
          if (typeof a === "string") {
            ;(a === "insertstring" || a === "splitline" || a === "togglecomment") &&
              i.pushEmacsMark(),
              (a = this.commands[a] || i.commands.commands[a])
            if (!a) return undefined
          }
          !a.readOnly && !a.isYank && (e.lastCommand = null),
            !a.readOnly && i.emacsMark() && i.setEmacsMark(null)
          if (e.count) {
            const u = e.count
            e.count = 0
            if (!a || !a.handlesCount)
              return {
                args: f,
                command: {
                  exec: (e, t) => {
                    for (let n = 0; n < u; n++) a.exec(e, t)
                  },
                  multiSelectAction: a.multiSelectAction,
                },
              }
            f || (f = {}), typeof f === "object" && (f.count = u)
          }
          return { command: a, args: f }
        }),
        (t.emacsKeys = {
          "Up|C-p": { command: "goorselect", args: ["golineup", "selectup"] },
          "Down|C-n": { command: "goorselect", args: ["golinedown", "selectdown"] },
          "Left|C-b": { command: "goorselect", args: ["gotoleft", "selectleft"] },
          "Right|C-f": { command: "goorselect", args: ["gotoright", "selectright"] },
          "C-Left|M-b": { command: "goorselect", args: ["gotowordleft", "selectwordleft"] },
          "C-Right|M-f": { command: "goorselect", args: ["gotowordright", "selectwordright"] },
          "Home|C-a": { command: "goorselect", args: ["gotolinestart", "selecttolinestart"] },
          "End|C-e": { command: "goorselect", args: ["gotolineend", "selecttolineend"] },
          "C-Home|S-M-,": { command: "goorselect", args: ["gotostart", "selecttostart"] },
          "C-End|S-M-.": { command: "goorselect", args: ["gotoend", "selecttoend"] },
          "S-Up|S-C-p": "selectup",
          "S-Down|S-C-n": "selectdown",
          "S-Left|S-C-b": "selectleft",
          "S-Right|S-C-f": "selectright",
          "S-C-Left|S-M-b": "selectwordleft",
          "S-C-Right|S-M-f": "selectwordright",
          "S-Home|S-C-a": "selecttolinestart",
          "S-End|S-C-e": "selecttolineend",
          "S-C-Home": "selecttostart",
          "S-C-End": "selecttoend",
          "C-l": "recenterTopBottom",
          "M-s": "centerselection",
          "M-g": "gotoline",
          "C-x C-p": "selectall",
          "C-Down": { command: "goorselect", args: ["gotopagedown", "selectpagedown"] },
          "C-Up": { command: "goorselect", args: ["gotopageup", "selectpageup"] },
          "PageDown|C-v": { command: "goorselect", args: ["gotopagedown", "selectpagedown"] },
          "PageUp|M-v": { command: "goorselect", args: ["gotopageup", "selectpageup"] },
          "S-C-Down": "selectpagedown",
          "S-C-Up": "selectpageup",
          "C-s": "iSearch",
          "C-r": "iSearchBackwards",
          "M-C-s": "findnext",
          "M-C-r": "findprevious",
          "S-M-5": "replace",
          Backspace: "backspace",
          "Delete|C-d": "del",
          "Return|C-m": { command: "insertstring", args: "\n" },
          "C-o": "splitline",
          "M-d|C-Delete": { command: "killWord", args: "right" },
          "C-Backspace|M-Backspace|M-Delete": { command: "killWord", args: "left" },
          "C-k": "killLine",
          "C-y|S-Delete": "yank",
          "M-y": "yankRotate",
          "C-g": "keyboardQuit",
          "C-w|C-S-W": "killRegion",
          "M-w": "killRingSave",
          "C-Space": "setMark",
          "C-x C-x": "exchangePointAndMark",
          "C-t": "transposeletters",
          "M-u": "touppercase",
          "M-l": "tolowercase",
          "M-/": "autocomplete",
          "C-u": "universalArgument",
          "M-;": "togglecomment",
          "C-/|C-x u|S-C--|C-z": "undo",
          "S-C-/|S-C-x u|C--|S-C-z": "redo",
          "C-x r": "selectRectangularRegion",
          "M-x": { command: "focusCommandLine", args: "M-x " },
        }),
        t.handler.bindKeys(t.emacsKeys),
        t.handler.addCommands({
          recenterTopBottom: (e) => {
            const t = e.renderer
            const n = t.$cursorLayer.getPixelPosition()
            const r = t.$size.scrollerHeight - t.lineHeight
            let i = t.scrollTop
            Math.abs(n.top - i) < 2
              ? (i = n.top - r)
              : Math.abs(n.top - i - r * 0.5) < 2
                ? (i = n.top)
                : (i = n.top - r * 0.5),
              e.session.setScrollTop(i)
          },
          selectRectangularRegion: (e) => {
            e.multiSelect.toggleBlockSelection()
          },
          setMark: {
            exec: (e, t) => {
              function u() {
                const t = e.popEmacsMark()
                t && e.moveCursorToPosition(t)
              }
              if (t?.count) {
                e.inMultiSelectMode ? e.forEachSelection(u) : u(), u()
                return
              }
              const n = e.emacsMark()
              const r = e.selection.getAllRanges()
              const i = r.map((e) => ({ row: e.start.row, column: e.start.column }))
              const s = !0
              const o = r.every((e) => e.isEmpty())
              if (s && (n || !o)) {
                e.inMultiSelectMode
                  ? e.forEachSelection({ exec: e.clearSelection.bind(e) })
                  : e.clearSelection(),
                  n && e.pushEmacsMark(null)
                return
              }
              if (!n) {
                i.forEach((t) => {
                  e.pushEmacsMark(t)
                }),
                  e.setEmacsMark(i[i.length - 1])
                return
              }
            },
            readOnly: !0,
            handlesCount: !0,
          },
          exchangePointAndMark: {
            exec: (t, n) => {
              const r = t.selection
              if (!n.count && !r.isEmpty()) {
                r.setSelectionRange(r.getRange(), !r.isBackwards())
                return
              }
              if (n.count) {
                const i = { row: r.lead.row, column: r.lead.column }
                r.clearSelection(), r.moveCursorToPosition(t.emacsMarkForSelection(i))
              } else r.selectToPosition(t.emacsMarkForSelection())
            },
            readOnly: !0,
            handlesCount: !0,
            multiSelectAction: "forEach",
          },
          killWord: {
            exec: (e, n) => {
              e.clearSelection(),
                n === "left" ? e.selection.selectWordLeft() : e.selection.selectWordRight()
              const r = e.getSelectionRange()
              const i = e.session.getTextRange(r)
              t.killRing.add(i), e.session.remove(r), e.clearSelection()
            },
            multiSelectAction: "forEach",
          },
          killLine: function (e) {
            e.pushEmacsMark(null), e.clearSelection()
            const n = e.getSelectionRange()
            let r = e.session.getLine(n.start.row)
            ;(n.end.column = r.length), (r = r.substr(n.start.column))
            const i = e.session.getFoldLine(n.start.row)
            i && n.end.row !== i.end.row && ((n.end.row = i.end.row), (r = "x")),
              /^\s*$/.test(r) &&
                (n.end.row++,
                (r = e.session.getLine(n.end.row)),
                (n.end.column = /^\s*$/.test(r) ? r.length : 0))
            const s = e.session.getTextRange(n)
            e.prevOp.command === this ? t.killRing.append(s) : t.killRing.add(s),
              e.session.remove(n),
              e.clearSelection()
          },
          yank: (e) => {
            e.onPaste(t.killRing.get() || ""), (e.keyBinding.$data.lastCommand = "yank")
          },
          yankRotate: (e) => {
            if (e.keyBinding.$data.lastCommand !== "yank") return
            e.undo(),
              e.session.$emacsMarkRing.pop(),
              e.onPaste(t.killRing.rotate()),
              (e.keyBinding.$data.lastCommand = "yank")
          },
          killRegion: {
            exec: (e) => {
              t.killRing.add(e.getCopyText()), e.commands.byName.cut.exec(e), e.setEmacsMark(null)
            },
            readOnly: !0,
            multiSelectAction: "forEach",
          },
          killRingSave: {
            exec: (e) => {
              e.$handlesEmacsOnCopy = !0
              const n = e.session.$emacsMarkRing.slice()
              const r = []
              t.killRing.add(e.getCopyText()),
                setTimeout(() => {
                  function t() {
                    const t = e.selection
                    const n = t.getRange()
                    const i = t.isBackwards() ? n.end : n.start
                    r.push({ row: i.row, column: i.column }), t.clearSelection()
                  }
                  ;(e.$handlesEmacsOnCopy = !1),
                    e.inMultiSelectMode ? e.forEachSelection({ exec: t }) : t(),
                    e.setEmacsMark(null),
                    (e.session.$emacsMarkRing = n.concat(r.reverse()))
                }, 0)
            },
            readOnly: !0,
          },
          keyboardQuit: (e) => {
            e.selection.clearSelection(), e.setEmacsMark(null), (e.keyBinding.$data.count = null)
          },
          focusCommandLine: (e, t) => {
            e.showCommandLine?.(t)
          },
        }),
        t.handler.addCommands(i.iSearchStartCommands)
      const d = t.handler.commands
      ;(d.yank.isYank = !0),
        (d.yankRotate.isYank = !0),
        (t.killRing = {
          $data: [],
          add: function (e) {
            e && this.$data.push(e), this.$data.length > 30 && this.$data.shift()
          },
          append: function (e) {
            const t = this.$data.length - 1
            let n = this.$data[t] || ""
            e && (n += e), n && (this.$data[t] = n)
          },
          get: function (e) {
            return (
              (e = e || 1),
              this.$data
                .slice(this.$data.length - e, this.$data.length)
                .reverse()
                .join("\n")
            )
          },
          pop: function () {
            return this.$data.length > 1 && this.$data.pop(), this.get()
          },
          rotate: function () {
            return this.$data.unshift(this.$data.pop()), this.get()
          },
        })
    },
  )
;(() => {
  ace.require(["ace/keyboard/emacs"], (m) => {
    if (typeof module === "object" && typeof exports === "object" && module) {
      module.exports = m
    }
  })
})()
