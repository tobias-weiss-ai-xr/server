ace.define(
  ...
  (e, t, n) => {
    function i(e, t, n) {
      ...
      const i = r.lead.row
      let s = r.lead.column
      const o = e.session.getLine(i)
      if (!o[s + t]) {
        const u = (n ? "selectWord" : "moveCursorShortWord") + (t === 1 ? "Right" : "Left")
        return e.selection[u]()
      }
      t === -1 && s--
      while (o[s]) {
        const a = f(o[s]) + f(o[s + t])
        s += t
        if (t === 1) {
    ...
    ;(t.handler = new r()),
      t.handler.addCommands([
        ...
        },
        {
          ...
          exec: (e) => {
            const t = e.selection.getCursor()
            e.session.remove({ start: t, end: { row: t.row, column: Number.POSITIVE_INFINITY } })
          },
          multiSelectAction: "forEach",
          scrollIntoView: "cursor",
        },
...
;(() => {
  ace.require(["ace/keyboard/sublime"], (m) => {
