ace.define(
  ...
  (e, t, n) => {
    ...
    const u = (t.supportFunction = "rgb|rgba|url|attr|counter|counters")
    const a = (t.supportConstant =
    ...
    const f = (t.supportConstantColor =
    ...
    const l = (t.supportConstantFonts =
    ...
    const c = (t.numRe = "\\-?(?:(?:[0-9]+(?:\\.[0-9]+)?)|(?:\\.[0-9]+))")
    const h = (t.pseudoElements =
      "(\\:+)\\b(after|before|first-letter|first-line|moz-selection|selection)\\b")
    const p = (t.pseudoClasses =
  ...
  ace.define("ace/mode/css_completions", ["require", "exports", "module"], (e, t, n) => {
    const r = {
      background: { "#$0": 1 },
    ...
    ;(function () {
      ;(this.completionsDefined = !1),
        (this.defineCompletions = function () {
          if (document) {
            const e = document.createElement("c").style
            for (const t in e) {
        ...
        (this.getCompletions = function (e, t, n, r) {
          ...
          if (e === "ruleset" || t.$mode.$id === "ace/mode/scss") {
            let i = t.getLine(n.row).substr(0, n.column)
            const s = /\([^)]*$/.test(i)
            return (
        ...
        (this.getPropertyCompletions = (e, t, n, i, s) => {
          s = s || !1
        ...
        (this.getPropertyValueCompletions = (e, t, n, i) => {
          const s = t.getLine(n.row).substr(0, n.column)
          const o = /([\w\-]+):[^:]*$/.exec(s)?.[1]
          if (!o) return []
          let u = []
          return (
            o in r && typeof r[o] === "object" && (u = Object.keys(r[o])),
            u.map((e) => ({ caption: e, snippet: e, meta: "property value", score: 1e6 }))
          )
        })
    }).call(i.prototype),
      (t.CssCompletions = i)
  ...
  ace.define(
    ...
    (e, t, n) => {
      const r = e("../../lib/oop")
      const i = e("../behaviour").Behaviour
      const s = e("./cstyle").CstyleBehaviour
      const o = e("../../token_iterator").TokenIterator
      const u = function () {
        this.inherit(s),
          this.add("colon", "insertion", (e, t, n, r, i) => {
            if (i === ":" && n.selection.isEmpty()) {
              const s = n.getCursorPosition()
              const u = new o(r, s.row, s.column)
              let a = u.getCurrentToken()
              a?.value.match(/\s+/) && (a = u.stepBackward())
              if (a && a.type === "support.type") {
                const f = r.doc.getLine(s.row)
                const l = f.substring(s.column, s.column + 1)
          ...
          this.add("colon", "deletion", (e, t, n, r, i) => {
            const s = r.doc.getTextRange(i)
            if (!i.isMultiLine() && s === ":") {
              const u = n.getCursorPosition()
              const a = new o(r, u.row, u.column)
              let f = a.getCurrentToken()
              f?.value.match(/\s+/) && (f = a.stepBackward())
              if (f && f.type === "support.type") {
                const l = r.doc.getLine(i.start.row)
                const c = l.substring(i.end.column, i.end.column + 1)
          ...
          this.add("semicolon", "insertion", (e, t, n, r, i) => {
            if (i === ";" && n.selection.isEmpty()) {
              const s = n.getCursorPosition()
              const o = r.doc.getLine(s.row)
              const u = o.substring(s.column, s.column + 1)
              if (u === ";") return { text: "", selection: [1, 1] }
            }
          }),
          this.add("!important", "insertion", (e, t, n, r, i) => {
            if (i === "!" && n.selection.isEmpty()) {
              const s = n.getCursorPosition()
              const o = r.doc.getLine(s.row)
  ...
  ace.define(
    ...
    (e, t, n) => {
      const r = e("../lib/oop")
      const i = e("./text").Mode
      const s = e("./css_highlight_rules").CssHighlightRules
      const o = e("./matching_brace_outdent").MatchingBraceOutdent
      const u = e("../worker/worker_client").WorkerClient
      const a = e("./css_completions").CssCompletions
      const f = e("./behaviour/css").CssBehaviour
        ...
        function () {
          ;(this.foldingRules = "cStyle"),
            ...
            (this.getNextLineIndent = function (e, t, n) {
              ...
              const i = this.getTokenizer().getLineTokens(t, e).tokens
            ...
            (this.createWorker = (e) => {
              const t = new u(["ace"], "ace/mode/css_worker", "Worker")
              return (
