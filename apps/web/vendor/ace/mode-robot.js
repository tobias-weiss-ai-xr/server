ace.define(
  ...
  (e, t, n) => {
        ...
        ;(this.$rules = {
          start: [
            ...
            },
            {
              ...
              caseInsensitive: !0,
              push: [
                { token: "storage.type.method.robot", regex: /^(?!\s*\.\.\.)/, next: "pop" },
            ...
            { token: "constant.language", regex: e, caseInsensitive: !0 },
            {
              ...
              regex: /[$@&%]\{\{?/,
              push: [
                { token: "entity.name.variable.wrapper", regex: /\}\}?(\s?=)?/, next: "pop" },
                { include: "$self" },
            ...
            },
            {
              ...
              regex: /^[^\s\t*$|]+|(?=^\|)\s+[^\s\t*$|]+/,
              push: [
                { token: "keyword.control.robot", regex: /(?=\s{2})|\t|$|\s+(?=\|)/, next: "pop" },
                { defaultToken: "keyword.control.robot" },
              ],
            },
            { token: "constant.numeric.robot", regex: /\b[0-9]+(?:\.[0-9]+)?\b/ },
            {
              token: "keyword",
            ...
            },
            {
              ...
              regex: /^(?:\s{2,}\s+)[^ \t*$@&%[.|]+/,
              push: [
                { token: "storage.type.function", regex: /(?=\s{2})|\t|$|\s+(?=\|)/, next: "pop" },
    ...
    ;(s.metadata = { fileTypes: ["robot"], name: "Robot", scopeName: "source.robot" }),
      r.inherits(s, i),
