;((QUnit) => {
  ...
  QUnit.test("new and parse", (assert) => {
    ...
    var Collection = Backbone.Collection.extend({
      parse: (data) => _.filter(data, (datum) => datum.a % 2 === 0),
    })
  ...
  QUnit.test("add; at should have preference over comparator", (assert) => {
    ...
    var Col = Backbone.Collection.extend({
      comparator: (m1, m2) => (m1.id > m2.id ? -1 : 1),
    })
  ...
  QUnit.test(
    "successful create with wait:true drops special error listener (#4284)",
    function (assert) {
      assert.expect(1)
      var collection = new Backbone.Collection()
      collection.url = "/test"
  ...
  QUnit.test(
    "successful create pre-existing with wait:true preserves other error bindings (#4262)",
    function (assert) {
      assert.expect(1)
      var model = new Backbone.Model()
      var collection = new Backbone.Collection([model])
  ...
  QUnit.test("mixin", (assert) => {
    Backbone.Collection.mixin({
      sum: (models, iteratee) => _.reduce(models, (s, m) => s + iteratee(m), 0),
    })

    var coll = new Backbone.Collection([{ a: 1 }, { a: 1, b: 2 }, { a: 2, b: 2 }, { a: 3 }])
  ...
  QUnit.test("`set` and model level `parse`", (assert) => {
    ...
    var Collection = Backbone.Collection.extend({
      model: Model,
      parse: (res) => _.map(res.models, "model"),
    })
  ...
  QUnit.test("`add` only `sort`s when necessary with comparator function", (assert) => {
    assert.expect(3)
    var collection = new (Backbone.Collection.extend({
      comparator: (m1, m2) => (m1.get("a") > m2.get("a") ? 1 : m1.get("a") < m2.get("a") ? -1 : 0),
    }))([{ id: 1 }, { id: 2 }, { id: 3 }])
    collection.on("sort", () => {
      assert.ok(true)
    })
  ...
  QUnit.test('Polymorphic models work with "simple" constructors', (assert) => {
    ...
    var C = Backbone.Collection.extend({
      model: (attrs) => (attrs.type === "a" ? new A(attrs) : new B(attrs)),
    })
  ...
  QUnit.test('Polymorphic models work with "advanced" constructors', (assert) => {
    ...
    var C = Backbone.Collection.extend({
      model: Backbone.Model.extend({
        constructor: (attrs) => (attrs.type === "a" ? new A(attrs) : new B(attrs)),
  ...
  QUnit.test(
    "Collection with polymorphic models receives id from modelId using model instance idAttribute",
    (assert) => {
      assert.expect(6)
      // When the polymorphic models use 'id' for the idAttribute, all is fine.
      var C1 = Backbone.Collection.extend({
        model: (attrs) => new Backbone.Model(attrs),
      })
      var c1 = new C1({ id: 1 })
  ...
  QUnit.test("#3961 - add events sends options.index that correspond to wrong index", (assert) => {
    var numModels = 4
    var models = _.each(["a", "b", "c", "d"], (val) => new Backbone.Model({ id: val }))
  ...
  QUnit.test("#4233 - can instantiate new model in ES class Collection", (assert) => {
    var model
    try {
      model = new Function(
        "return ({\n" +
