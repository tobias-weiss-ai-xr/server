;((QUnit) => {
  ...
  QUnit.test("#3391 - Empty root normalizes to single slash.", (assert) => {
    assert.expect(1)
    Backbone.history.stop()
    Backbone.history = _.extend(new Backbone.History(), {
      location: location,
  ...
  QUnit.test("#3391 - Use trailing slash on root when trailingSlash is true.", (assert) => {
    assert.expect(1)
    Backbone.history.stop()
    Backbone.history = _.extend(new Backbone.History(), {
      location: location,
  ...
  QUnit.test("initial non-matching root triggers notfound event", (assert) => {
    assert.expect(1)
    location.replace("http://example.com/root#foo")
    Backbone.history.stop()
    Backbone.history = _.extend(new Backbone.History(), { location: location })
    Backbone.history.on("notfound", () => {
      assert.ok(true)
    })
    var MyRouter = Backbone.Router.extend({
      routes: {
  ...
  QUnit.test("later non-matching root triggers notfound event", (assert) => {
    assert.expect(2)
    location.replace("http://example.com/root#foo")
    Backbone.history.stop()
    Backbone.history = _.extend(new Backbone.History(), { location: location })
  ...
  QUnit.test("initial non-matching route triggers notfound event", (assert) => {
    assert.expect(1)
  ...
  QUnit.test("later non-matching route triggers notfound event", (assert) => {
    assert.expect(2)
    location.replace("http://example.com/root#foo")
    Backbone.history.stop()
    Backbone.history = _.extend(new Backbone.History(), { location: location })
  ...
  QUnit.test("non-matching pushState route triggers notfound event", (assert) => {
    assert.expect(2)
    location.replace("http://example.com/root/foo")
    Backbone.history.stop()
    Backbone.history = _.extend(new Backbone.History(), { location: location })
  ...
  QUnit.test("non-matching navigate triggers notfound event", (assert) => {
    assert.expect(2)
    location.replace("http://example.com/root#foo")
    Backbone.history.stop()
    Backbone.history = _.extend(new Backbone.History(), { location: location })
