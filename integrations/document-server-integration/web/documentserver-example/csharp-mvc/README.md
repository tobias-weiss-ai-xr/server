...
This example will help you integrate World-Office Docs into your web application written in .Net (C# MVC).
...
See the detailed guide to learn how to install Document Server [for Windows](https://helpcenter.worldoffice.com/installation/docs-developer-install-windows.aspx), [for Linux](https://helpcenter.worldoffice.com/installation/docs-developer-install-ubuntu.aspx), or [for Docker](https://helpcenter.worldoffice.com/server/developer-edition/docker/docker-installation.aspx).

## Step 2. Download the .Net (C# MVC) code for the editors integration

Download the [.Net (C# MVC) example](https://api.worldoffice.com/docs/docs-api/samples/language-specific-examples/) from our site.
...
```
where the **documentserver** is the name of the server with the World-Office Document Server installed and the **storage-path** is the path where files will be created and stored. You can set an absolute path.

If you want to experiment with the editor configuration, modify the [parameters](https://api.worldoffice.com/docs/docs-api/usage-api/advanced-parameters/) in the *DocEditor.aspx* file.
...
Open the *web.appsettings.config* file and [specify the same secret key](https://helpcenter.worldoffice.com/installation/docs-configure-jwt.aspx) as used in your Document Server to enable JWT: 
