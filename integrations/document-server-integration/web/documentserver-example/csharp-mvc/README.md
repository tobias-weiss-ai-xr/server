...
This example will help you integrate WORLDOFFICE Docs into your web application written in .Net (C# MVC).
...
See the detailed guide to learn how to install Document Server [for Windows](https://helpcenter.world-office.com/installation/docs-developer-install-windows.aspx), [for Linux](https://helpcenter.world-office.com/installation/docs-developer-install-ubuntu.aspx), or [for Docker](https://helpcenter.world-office.com/server/developer-edition/docker/docker-installation.aspx).

## Step 2. Download the .Net (C# MVC) code for the editors integration

Download the [.Net (C# MVC) example](https://api.world-office.com/docs/docs-api/samples/language-specific-examples/) from our site.
...
```
where the **documentserver** is the name of the server with the WORLDOFFICE Document Server installed and the **storage-path** is the path where files will be created and stored. You can set an absolute path.

If you want to experiment with the editor configuration, modify the [parameters](https://api.world-office.com/docs/docs-api/usage-api/advanced-parameters/) in the *DocEditor.aspx* file.
...
Open the *web.appsettings.config* file and [specify the same secret key](https://helpcenter.world-office.com/installation/docs-configure-jwt.aspx) as used in your Document Server to enable JWT: 
