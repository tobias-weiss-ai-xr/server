...
// Hash a string using the same algorithm as String.prototype.hashCode (was in app.js)
function hashString(str) {
  const len = str.length;
  let ret = 0;
  for (let i = 0; i < len; i++) {
    ret = Math.imul(ret, 31) + str.charCodeAt(i);
  }
  return ret;
}

// get all the necessary values from modules
...
documentService.generateRevisionId = function generateRevisionId(expectedKey) {
  ...
  if (expKey.length > maxKeyLength) { // if the expected key length is greater than the max key length
    ...
    expKey = hashString(expKey).toString();
  }
