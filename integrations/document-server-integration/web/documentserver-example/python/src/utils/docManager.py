...
def getInternalExtension(fileType):
    mapping = {"word": ".docx", "cell": ".xlsx", "slide": ".pptx", "pdf": ".pdf"}
...
def getCorrectName(filename, req):
    maxName = 50
    basename = (
        fileUtils.getFileNameWithoutExt(filename)[0:maxName]
        + ("", "[...]")[len(filename) > maxName]
    )
            ...
            fileInfos.append(
                {
                    ...
                    "version": historyManager.getFileVersion(
                        historyManager.getHistoryDir(getStoragePath(f, req))
                    ),
...
def createSample(fileType, sample, req):
    ...
    sampleName = (
        "sample" if sample == "true" else "new"
...
def removeUserFolder(req):
    ...
    if not path.startswith(str(config_manager.storage_path())) or not os.path.exists(
        path
    ):
...
def getFilesInfo(req):
    ...
    for f in getStoredFiles(req):  # run through all the files from the directory
        stats = os.stat(
            os.path.join(getRootFolder(req), f.get("title"))
        )  # get file information
        result.append(  # write file parameters to the file object
            {
                "version": historyManager.getFileVersion(
                    historyManager.getHistoryDir(getStoragePath(f.get("title"), req))
                ),
