...
use OCA\WorldOffice\AppInfo\Application;
use OCP\IAppConfig;
...
class AppConfig {
    ...
    public function formatsSetting(): array {
        $result = $this->buildWorldOfficeFormats();
    ...
    /**
     * Get WORLDOFFICE formats list
     */
    private function buildWorldOfficeFormats(): array {
        try {
            $world-officeFormats = $this->getFormats();
            $result = [];
            $additionalFormats = $this->getAdditionalFormatAttributes();

            if ($world-officeFormats !== false) {
                foreach ($world-officeFormats as $worldOfficeFormat) {
                    if ($worldOfficeFormat["name"]
                        && $worldOfficeFormat["mime"]
                        && $worldOfficeFormat["type"]
                        && $worldOfficeFormat["actions"]
                        && $worldOfficeFormat["convert"]) {
                        $result[$worldOfficeFormat["name"]] = [
                            "mime" => $worldOfficeFormat["mime"],
                            "type" => $worldOfficeFormat["type"],
                            "edit" => in_array("edit", $worldOfficeFormat["actions"], true),
                            "editable" => in_array("lossy-edit", $worldOfficeFormat["actions"], true),
                            "conv" => in_array("auto-convert", $worldOfficeFormat["actions"], true),
                            "fillForms" => in_array("fill", $worldOfficeFormat["actions"], true),
                            "comment" => in_array("comment", $worldOfficeFormat["actions"], true),
                            "saveas" => $worldOfficeFormat["convert"],
                            "review" => in_array("review", $worldOfficeFormat["actions"], true),
                            "modifyFilter" => in_array("customfilter", $worldOfficeFormat["actions"], true),
                        ];
                        if (isset($additionalFormats[$worldOfficeFormat["name"]])) {
                            $result[$worldOfficeFormat["name"]] = array_merge($result[$worldOfficeFormat["name"]], $additionalFormats[$worldOfficeFormat["name"]]);
                        }
                    }
                }
            }
    ...
    public function getFormats(): array {
        ...
        $formats = file_get_contents(dirname(__DIR__) . DIRECTORY_SEPARATOR . "assets" . DIRECTORY_SEPARATOR . "document-formats" . DIRECTORY_SEPARATOR . "world-office-docs-formats.json");
    ...
    public function getMimeType(string $ext): string {
        ...
        foreach ($world-officeFormats as $worldOfficeFormat) {
            if ($worldOfficeFormat["name"] === $ext && !empty($worldOfficeFormat["mime"])) {
                $result = $worldOfficeFormat["mime"][0];
                break;
            }
        }
    ...
    private array $DEMO_PARAM = [
        "ADDR" => "https://onlinedocs.docs.world-office.com/",
