...
use OCA\WorldOffice\RemoteInstance;
...
class CallbackController extends Controller {
    ...
    public function emptyfile(string $doc): DataDownloadResponse|JSONResponse {
        ...
        // Validate JWT header only if Document Server secret is configured
        // The doc parameter already validates the request signature
        if (!empty($this->appConfig->getDocumentServerSecret())) {
            $header = $this->request->getHeader($this->appConfig->jwtHeader());
            if (!empty($header)) {
            ...
            // If header is empty but doc parameter is valid, allow the request
            // This handles cases where Document Server sends the token in the doc parameter only
        }
