...
namespace OCA\WorldOffice\Cron;
...
class EditorsCheck extends TimedJob {
    ...
    protected function run($argument): void {
        ...
        if ($host === "localhost" || $host === "127.0.0.1") {
            $this->logger->debug("Localhost is not alowed for cron editors availability check. Please provide server address for internal requests from WORLDOFFICE Docs");
            return;
        }

        $this->logger->debug("WORLDOFFICE check started by cron");

        [$error, $version] = $this->documentService->checkDocServiceUrl();

        if (!empty($error)) {
            $this->logger->info("WORLDOFFICE server is not available");
            $this->appConfig->setSettingsError($error);
            $this->notifyAdmins();
        } else {
            $this->logger->debug("WORLDOFFICE server availability check is finished successfully");
        }
    }
    ...
    private function notifyAdmins(): void {
        ...
        $notification->setApp($this->appName)
            ...
            ->setObject("editorsCheck", $this->trans->t("word-office server is not available"))
