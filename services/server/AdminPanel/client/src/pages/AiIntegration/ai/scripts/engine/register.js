...
function registerButtons(window, undefined)
{
	...
	function chatWindowShow(attachedText)
	{
		...
		let variation = {
			...
			isModal : false,
			isCanDocked: true,
			type: window.localStorage.getItem("world-office_ai_chat_placement") || "window",
		...
		chatWindow.attachEvent("onDockedChanged", async function(type) {
			window.localStorage.setItem("world-office_ai_chat_placement", type);
	...
	if (true)
	{
		...
		button4.attachOnClick(async function(){
			...
			const ls_lang_key = "world-office_ai_plugin_translate_lang";
	...
	var actions_key = "world-office_ai_actions_key";
