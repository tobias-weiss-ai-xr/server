    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", position: "relative" }}>
      ...
      {internalProvidersLoaded && currentWindow && (
        <div key={currentWindow.iframeId} style={{ width: "100%", height: "100%" }}>
          ...
          <iframe
            id={currentWindow.iframeId}
            title={currentWindow.description || ""}
            src={currentWindow.url}
            style={pluginWindowStyle}
          />
          {currentWindow.buttons && currentWindow.buttons.length > 0 && (
            <FixedSaveButtonGroup buttons={currentWindow.buttons} />
          )}
        </div>
      )}
    </div>
