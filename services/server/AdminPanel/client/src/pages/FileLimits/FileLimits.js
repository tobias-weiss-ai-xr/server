...
function FileLimits() {
  ...
  useEffect(() => {
    if (config) {
      ...
      settings.maxDownloadBytes = getNestedValue(
        config,
      ...
      const docxLimit = inputLimits.find((limit) => limit.type?.includes("docx"))
      const xlsxLimit = inputLimits.find((limit) => limit.type?.includes("xlsx"))
      const pptxLimit = inputLimits.find((limit) => limit.type?.includes("pptx"))
      const vsdxLimit = inputLimits.find((limit) => limit.type?.includes("vsdx"))
  ...
  const handleSave = async () => {
    ...
    configUpdate["FileConverter.converter.maxDownloadBytes"] = Number.parseInt(
      localSettings.maxDownloadBytes,
    )
