...
import { fetchConfiguration, fetchStatistics } from "../../api"
 ...
 */
const getCriticalStatus = (remaining, limit) =>
  remaining > limit * CRITICAL_THRESHOLD ? "normal" : "critical"
...
export default function Statistics() {
  ...
  // Build block
  const buildDate = licenseInfo.buildDate
    ? new Date(licenseInfo.buildDate).toLocaleDateString()
    : ""
  const isOpenSource = licenseInfo.packageType === 0
  const packageTypeLabel = isOpenSource
    ? "Open source"
    : licenseInfo.packageType === 1
      ? "Enterprise Edition"
      : "Developer Edition"
  const buildBlock = (
  ...
  const licenseBlock = (() => {
    ...
    const startDateStr = licenseInfo.startDate
      ? new Date(licenseInfo.startDate).toLocaleDateString()
      : ""
    const isStartCritical =
      licType === 16 || (licenseInfo.startDate ? new Date(licenseInfo.startDate) > srvDate : false)
    const trialText = licenseInfo.mode & 1 ? "Trial" : ""

    return (
      <TopBlock title="License">
        {startDateStr && (
          <div style={isStartCritical ? { color: CRITICAL_COLOR } : undefined}>
            Start date: {startDateStr}
          </div>
        )}
        <div>
          <span>{licValidText}</span>
          <span style={licValidColor ? { color: licValidColor } : undefined}>
  ...
  const currentTable = useMemo(() => {
    if (isUsersModel) {
      // Users model
      const days = Number.parseInt(licenseInfo.usersExpire / SECONDS_PER_DAY, 10) || 1
