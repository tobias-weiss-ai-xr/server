...
import { rotateWopiKeysAction, saveConfig, selectConfig } from "../../store/slices/configSlice"
...
function WOPISettings() {
  ...
  const handleWopiEnabledChange = (enabled) => {
    ...
    setHasChanges(
      enabled !== configWopiEnabled ||
        localRotateKeys ||
        localRefreshLockInterval !== configRefreshLockInterval,
    )
  ...
  const handleRotateKeysChange = (checked) => {
    ...
    setHasChanges(
      localWopiEnabled !== configWopiEnabled ||
        checked ||
        localRefreshLockInterval !== configRefreshLockInterval,
    )
  }
  ...
  const handleRefreshLockIntervalChange = (value) => {
    ...
    setHasChanges(
      localWopiEnabled !== configWopiEnabled ||
        localRotateKeys ||
        value !== configRefreshLockInterval,
    )
    ...
    <div className={`${styles.wopiSettings} ${styles.pageWithFixedSave}`}>
      ...
      {localWopiEnabled && (
        <>
          <Section
            title="Lock Settings"
            description="Configure document lock refresh interval for WOPI sessions."
          >
          ...
          >
            <div className={styles.noteWrapper}>
              <Note type="warning">
                Do not rotate keys more than once per 24 hours; storage may not refresh in time and
