...
const BOOTSTRAP_SECRET =
  process.env.ADMINPANEL_BOOTSTRAP_SECRET || crypto.randomBytes(32).toString("hex")

if (!process.env.ADMINPANEL_BOOTSTRAP_SECRET) {
  console.info(
    "ADMINPANEL_BOOTSTRAP_SECRET not set. Bootstrap codes will not work in cluster mode!",
  )
}
...
function generateShortCode(timestamp) {
  ...
  for (let i = 0; i < BOOTSTRAP_CODE_LENGTH; i++) {
    const byte = Number.parseInt(hmac.substring(i * 2, i * 2 + 2), 16)
