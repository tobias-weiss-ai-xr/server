...
Context.prototype.initTenantCache = async function () {
  ...
  if (!this.config) {
    ...
    this.config = utils.deepMergeObjects(
      {},
      moduleReloader.getBaseConfig(),
      runtimeConfig,
      tenantConfig,
    )
    ...
    if (this.config.persistentStorage && this.config.storage) {
      this.config.persistentStorage = Context.normalizePersistentStorageCfg(
        this.config.storage,
...
Context.prototype.cleanRuntimeConfigCache = () => {
  configCache = null
}
Context.prototype.cleanTenantConfigCache = (tenant) => {
  if (configCache) {
 ...
 */
Context.normalizePersistentStorageCfg = (storageCfg, persistentStorageCfg) =>
  utils.deepMergeObjects({}, storageCfg, persistentStorageCfg)
