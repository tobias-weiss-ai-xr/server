...
const cfgRabbitQueueConvertResponse = config.util.cloneDeep(
  config.get("rabbitmq.queueconvertresponse"),
)
const cfgRabbitExchangeConvertDead = config.util.cloneDeep(
  ...
  return co(function* () {
    ...
    try {
      ...
      if (isEmitDead) {
        ...
        const filteredDeadOptions = rabbitMQCore.filterQueueOptions(
...
function initSenderActive(sender, senderData) {
  const processEvent = (context, res) => {
  ...
  sender.on("released", (context) => {
    ...
    const err = new Error(
      `[AMQP] message is released (delivery_failed=${delivery_failed}, undeliverable_here=${undeliverable_here})`,
    )
  ...
  sender.on("modified ", (context) => {
    ...
    const undeliverable_here = context.delivery?.remote_state?.undeliverable_here
    const err = new Error(
      `[AMQP] message is modified (delivery_failed=${delivery_failed}, undeliverable_here=${undeliverable_here})`,
    )
    processEvent(context, err)
  })
}
...
TaskQueueRabbitMQ.prototype.addResponse = function (task) {
  return new Promise((resolve, reject) => {
    ...
    if (null != this.channelConvertResponse) {
      addResponse(this, content, (err, _ok) => {
        if (null != err) {
          reject(err)
        } else {
          resolve()
...
TaskQueueRabbitMQ.prototype.addDelayed = function (task, ttl) {
  return new Promise((resolve, reject) => {
    ...
    if (null != this.channelDelayed) {
      addDelayed(this, content, ttl, (err, _ok) => {
        if (null != err) {
          reject(err)
        } else {
          resolve()
      ...
      this.addDelayedStore.push({ task: content, ttl })
      resolve()
    }
  })
}
