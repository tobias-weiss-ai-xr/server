...
require("node:tls").DEFAULT_ECDH_CURVE = "auto"
...
exports.getConvertionTimeout = (opt_ctx) => {
  if (opt_ctx) {
...
exports.addSeconds = (date, sec) => {
  date.setSeconds(date.getSeconds() + sec)
}
exports.getMillisecondsOfHour = (date) =>
  (date.getUTCMinutes() * 60 + date.getUTCSeconds()) * 1000 + date.getUTCMilliseconds()
exports.encodeXml = (value) =>
  value.replace(/[<>&'"\r\n\t\xA0]/g, (c) => {
    switch (c) {
      case "<":
        return "&lt;"
...
exports.listFolders = (fsPath, optNoSubDir) =>
  co(function* () {
    let stats
    const list = []
    try {
...
exports.listObjects = (fsPath, optNoSubDir) =>
  co(function* () {
    let stats
    const list = []
    try {
...
async function httpRequest(
  ...
  try {
    ...
    const contentLength = headers["content-length"]
    if (opt_limit && contentLength && Number.parseInt(contentLength) > opt_limit) {
      const error = new Error(`EMSGSIZE: Error response: content-length:${contentLength}`)
      error.code = "EMSGSIZE"
      response.data.destroy(error)
      throw error
...
exports.mapAscServerErrorToOldError = (error) => {
  let res = -1
  switch (error) {
...
exports.compareStringByLength = (x, y) => {
  if (x && y) {
...
exports.promiseRedis = (client, func) => {
...
exports.containsAllAscii = (str) => /^[\000-\177]*$/.test(str)
 ...
 * @param {Express} app
 */
exports.expressifyIncomingMessage = (rawReq, app) => {
...
function changeWorldOfficeUrl(inputUrl, strPath, optFilename) {
  //world-office file server expects url end with file extension
  if (-1 === inputUrl.indexOf("?")) {
    inputUrl += "?"
  } else {
    inputUrl += "&"
  }
  return `${inputUrl + constants.WORLD_OFFICE_URL_PARAM}=${constants.OUTPUT_NAME}${path.extname(optFilename || strPath)}`
}
exports.changeWorldOfficeUrl = changeWorldOfficeUrl
/**
...
function getJwtHsKey(secret) {
  ...
  if (!res && secret != null) {
    try {
      res = jwtKeyCache[secret] = crypto.createSecretKey(
...
exports.getIndexFromUserId = (userId, userIdOriginal) =>
  Number.parseInt(userId.substring(userIdOriginal.length))
exports.checkPathTraversal = (ctx, docId, rootDirectory, filename) => {
...
exports.getConnectionInfo = (conn) => {
...
exports.getConnectionInfoStr = (conn) => JSON.stringify(exports.getConnectionInfo(conn))
exports.isLiveViewer = (conn) => conn.user?.view && "fast" === conn.coEditingMode
exports.isLiveViewerSupport = (licenseInfo) =>
  licenseInfo.connectionsView > 0 || licenseInfo.usersViewCount > 0
exports.canIncludeOutboxAuthorization = (ctx, url) => {
  ...
  if (tenTokenEnableRequestOutbox) {
    ...
    if (!new RegExp(escapeStringRegexp(tenTokenOutboxUrlExclusionRegex)).test(url)) {
      return true
    }
    ctx.logger.debug(
 ...
 */
exports.encryptPassword = async (ctx, password) => {
  ...
  const lowerNumber = 10 ** (iterationsLength - 1)
  const greaterNumber = 10 ** iterationsLength - 1
...
exports.decryptPassword = async (ctx, password) => {
  const pbkdf2Promise = util.promisify(crypto.pbkdf2)
  const tenSecret = ctx.getCfg("aesEncrypt.secret", cfgSecret)
  const tenAESConfig = ctx.getCfg("aesEncrypt.config", cfgAESConfig) ?? {}
  const {
...
exports.getDateTimeTicks = (date) => BigInt(date.getTime() * 10000) + 621355968000000000n
exports.convertLicenseInfoToFileParams = (licenseInfo) => {
  // todo
  // {
  // 	user_quota = 0;
  // 	portal_count = 0;
  ...
  // }
  const license = {}
  license.start_date = licenseInfo.startDate?.toJSON()
  license.end_date = licenseInfo.endDate?.toJSON()
...
exports.resolvePath = (object, path, defaultValue) =>
...
exports.getLicensePeriod = (startDate, now) => {
...
exports.removeIllegalCharacters = (filename) => filename?.replace(/[/\\?%*:|"<>]/g, "-") || filename
exports.getFunctionArguments = (func) =>
  func
    .toString()
    .replace(/[\r\n\s]+/g, " ")
    ...
    .join("")
    .split(/\s*,\s*/)
exports.isUselesSfc = (row, cmd) =>
  !(
    row &&
    commonDefines.FileStatus.SaveVersion === row.status &&
    cmd.getStatusInfoIn() === row.status_info
  )
exports.getChangesFileHeader = () => `CHANGES\t${commonDefines.buildVersion}\n`
exports.checksumFile = (hashName, path) => {
