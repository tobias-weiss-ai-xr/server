...
const defaultHttpsPort = 443 // Default ports (for http and https)
...
function updatePresenceCounters(ctx, conn, val) {
  return co(function* () {
    ...
    } else if (conn.isCloseCoAuthoring || conn.user?.view) {
...
function* setForceSave(ctx, docId, forceSave, cmd, success, url) {
  ...
  if (commonDefines.c_oAscForceSaveTypes.Command !== forceSaveType) {
    ...
    if (
      ...
      yield publish(
        ctx,
        { type: commonDefines.c_oPublishType.forceSave, ctx, docId, data },
...
async function startForceSave(
  ...
  const res = {
    code: commonDefines.c_oAscServerCommandErrors.NoError,
    time: null,
    inProgress: false,
  }
...
const resetForceSaveAfterChanges = co.wrap(
...
async function startWopiRPC(ctx, docId, userId, userIdOriginal, data) {
  ...
  if (row) {
    if (row.callback) {
      ...
      const uri = sqlBase.UserCallback.prototype.getCallbackByUserIndex(
        ctx,
        row.callback,
        userIndex,
      )
...
const unlockWopiDoc = co.wrap(function* (ctx, docId, opt_userIndex) {
  ...
  if (getRes?.wopiParams?.userAuth && "view" !== getRes.wopiParams.userAuth.mode) {
...
async function encryptPasswordParams(ctx, data) {
  ...
  if (dataWithPassword?.savepassword) {
...
exports.install = (server, app, callbackFunction) => {
  ...
  io.use((socket, next) => {
    co(function* () {
      ...
      try {
        ...
        if (tenTokenEnableBrowser || token) {
          const secretType = handshake?.auth?.session
            ? commonDefines.c_oAscSecretType.Session
  ...
  function isEditMode(permissions, mode) {
    ...
    //https://github.com/WORLDOFFICE/web-apps/blob/4a7879b4f88f315fe94d9f7d97c0ed8aa9f82221/apps/documenteditor/main/app/controller/Main.js#L1743
    //todo permissions in embed editor
    //https://github.com/WORLDOFFICE/web-apps/blob/72b8350c71e7b314b63b8eec675e76156bb4a2e4/apps/documenteditor/forms/app/controller/ApplicationController.js#L627
    return (
      (!mode || mode !== "view") &&
      (!permissions ||
  ...
  function* auth(ctx, conn, data) {
    ...
    if (data.token && data.user) {
      ...
      if (tenTokenEnableBrowser || token) {
        const secretType = data.jwtSession
          ? commonDefines.c_oAscSecretType.Session
          : commonDefines.c_oAscSecretType.Browser
      ...
      if (bIsRestore) {
        ...
        const format = data.openCmd?.format
      ...
      if (bIsRestore) {
        ...
        if (!conn.user.view) {
          ...
          try {
            ...
            if (puckerIndex > 0) {
              const objChangesDocument = yield* getDocumentChanges(
                ctx,
                docId,
                puckerIndex - 1,
                puckerIndex,
  ...
  function* onMessage(ctx, conn, data) {
    ...
    yield publish(
      ctx,
      { type: commonDefines.c_oPublishType.message, ctx, docId, userId, messages },
      docId,
      userId,
    )
  }
  ...
  function* onCursor(ctx, conn, data) {
    ...
    yield publish(
      ctx,
      { type: commonDefines.c_oPublishType.cursor, ctx, docId, userId, messages },
      docId,
      userId,
    )
  }
  ...
  async function getLock(ctx, conn, data, bIsRestore) {
    ...
    await publish(
      ctx,
      { type: commonDefines.c_oPublishType.getLock, ctx, docId, userId, documentLocks },
      docId,
      userId,
    )
    return true
    ...
    if (data.endSaveChanges) {
      ...
      if (data.isExcel && false !== data.isCoAuthoring && data.excelAdditionalInfo) {
        ...
        if (null !== oRecalcIndexColumns || null !== oRecalcIndexRows) {
          ...
          const docLockMod = _recalcLockArray(
            userId,
            docLock,
            oRecalcIndexColumns,
            oRecalcIndexRows,
    ...
    return co(function* () {
      ...
      try {
        ...
        for (let i = 0; i < connections.length; ++i) {
          ...
          yield ctx.initTenantCache()
          let tenExpSessionIdle =
            ms(ctx.getCfg("services.CoAuthoring.expire.sessionidle", cfgExpSessionIdle)) || 0
    ...
    return co(function* () {
      ...
      try {
        ...
        for (let i = 0; i < connections.length; ++i) {
          ...
          yield ctx.initTenantCache()
          const docId = conn.docId
          if (conn.user?.view || docIds.has(docId)) {
            continue
          }
          docIds.set(docId, 1)
...
exports.setLicenseInfo = async (globalCtx, data, original) => {
...
function* commandHandle(ctx, params, req, output) {
  ...
  switch (params.c) {
    case "info": {
      ...
      const selectRes = yield taskResult.select(ctx, docId)
      if (selectRes.length > 0) {
        const sendData = yield* bindEvents(
          ctx,
          docId,
          params.callback,
...
exports.commandFromServer = (req, res) =>
  co(function* () {
    const output = {
 ...
 */
exports.getShutdownStatus = (req, res) => {
  const ctx = new operationContext.Context()
  try {
    ctx.initFromRequest(req)
    res.setHeader("Content-Type", "application/json")
...
exports.getEditorConnectionsCount = (req, res) => {
  const ctx = new operationContext.Context()
  let count = 0
  try {
    ctx.initFromRequest(req)
    for (let i = 0; i < connections.length; ++i) {
      const conn = connections[i]
      if (!(conn.isCloseCoAuthoring || conn.user?.view)) {
        count++
      }
    }
