...
async function getOutputData(
  ...
  if (selectRes.length > 0) {
    ...
    creationDate = row.created_at?.getTime()
  ...
  switch (status) {
    ...
    case commonDefines.FileStatus.Ok: {
      ...
      } else if (optConn?.isCloseCoAuthoring) {
        outputData.setStatus(constants.FILE_STATUS_UPDATE_VERSION)
      } else if (optConn?.user.view) {
        outputData.setStatus("ok")
      } else if (
...
function addOriginFormat(ctx, cmd, row) {
  cmd.setOriginFormat(row?.change_id)
}
...
function changeFormatByOrigin(ctx, row, format) {
  ...
  const originFormat = row?.change_id
...
function* saveParts(ctx, cmd, filename) {
  ...
  if (SAVE_TYPE_COMPLETE_ALL !== saveType) {
    ...
    const saveIndex = Number.parseInt(cmd.getSaveIndex()) || 1 //prevent path traversal
...
function* commandOpenFillOutput(ctx, conn, cmd, outputData, opt_bIsRestore) {
  const status = yield getOutputData(
    ctx,
    cmd,
    outputData,
    cmd.getDocId(),
...
const commandSfctByCmd = co.wrap(
  ...
  if (strName) {
    ...
    if (-1 !== index) {
      if (index + findStr.length < strName.length) {
        const displayN = Number.parseInt(strName[index + findStr.length])
        if (!Number.isNaN(displayN)) {
...
function* commandImgurls(ctx, conn, cmd, outputData) {
  ...
  const tenDocumentTypesUpload = ctx.getCfg(
    "services.CoAuthoring.utils.limits_document_types_upload",
    cfgDocumentTypesUpload,
  )
  ...
  if (tenTokenEnableBrowser && token) {
    ...
    const checkJwtRes = yield docsCoServer.checkJwt(
      ctx,
      token,
    ...
    for (let i = 0; i < urls.length; ++i) {
      ...
      if (urlSource?.startsWith("data:")) {
        ...
        try {
          ...
          //todo stream
          const getRes = yield utils.downloadUrlPromise(
            ctx,
            urlSource,
      ...
      if (data) {
        ...
        if (constants.AVS_OFFICESTUDIO_FILE_UNKNOWN !== format) {
          ...
          constants.AVS_OFFICESTUDIO_FILE_SPREADSHEET_XLSX ===
          formatChecker.getOfficeZipFormatBySignature(data)
        ) {
          formatStr = "xlsx"
        ...
        if (isAllow) {
          ...
          if (urlParsed) {
            ...
            if (displayN > 0) {
              const displayedImageName = urlBasename.substring(
                0,
          ...
          const imgUrl = yield storage.getSignedUrl(
            ctx,
            conn.baseUrl,
            strPath,
            commonDefines.c_oAscUrlTypes.Session,
...
function* commandPathUrls(ctx, conn, data, outputData) {
  ...
  const urls = yield storage.getSignedUrlsArrayByArray(
    ctx,
    conn.baseUrl,
    listImages,
    commonDefines.c_oAscUrlTypes.Session,
...
function* commandPathUrl(ctx, conn, cmd, outputData) {
  ...
  const url = yield storage.getSignedUrl(
    ctx,
    conn.baseUrl,
    strPath,
    commonDefines.c_oAscUrlTypes.Temporary,
...
function* commandSetPassword(ctx, conn, cmd, outputData) {
  ...
  if (selectRes.length > 0) {
    ...
    if (commonDefines.FileStatus.Ok === row.status) {
      const documentPasswordCurEnc = sqlBase.DocumentPassword.prototype.getCurPassword(
        ctx,
        row.password,
      )
  ...
  //https://github.com/WORLDOFFICE/web-apps/blob/4a7879b4f88f315fe94d9f7d97c0ed8aa9f82221/apps/documenteditor/main/app/controller/Main.js#L1652
  //this.appOptions.isPasswordSupport = this.appOptions.isEdit && this.api.asc_isProtectionSupport() && (this.permissions.protect!==false);
  const isPasswordSupport =
  ...
  if (constants.EDITOR_CHANGES !== statusInfo || isSfcm) {
    ...
    if (uri && baseUrl && userLastChangeId) {
      ...
      if (formsData) {
        ...
        const formsDataUrl = yield storage.getSignedUrl(
          ctx,
          baseUrl,
          formsDataPath,
          commonDefines.c_oAscUrlTypes.Temporary,
      ...
      if (!isError || isErrorCorrupted) {
        try {
          ...
          if (isSendHistory && !isEncrypted) {
            ...
            const changeUrl = yield storage.getSignedUrl(
              ctx,
              baseUrl,
              savePathChanges,
              commonDefines.c_oAscUrlTypes.Temporary,
      ...
      if (isSfcm) {
        ...
        if (row && row.status === commonDefines.FileStatus.Ok) {
          ...
          if (forceSave && forceSaveType === commonDefines.c_oAscForceSaveTypes.Internal) {
            ...
            try {
              ...
              if (
                replyData &&
                commonDefines.c_oAscServerCommandErrors.NoError !== replyData.error
              ) {
        ...
        if (0 === editorsCount || (isEncrypted && 1 === editorsCount)) {
          ...
          if (updateIfRes.affectedRows > 0) {
            ...
            const forceSaveDate = actualForceSave?.time
              ? new Date(actualForceSave.time)
              : new Date()
...
function* processWopiPutFile(
  ...
  if (postRes) {
    ...
    if (body?.LastModifiedTime) {
      const lastModifiedTimeInfo = wopiClient.getWopiModifiedMarker(
        wopiParams,
...
exports.openDocument = (ctx, conn, cmd, opt_upsertRes, opt_bIsRestore) =>
  co(function* () {
    let outputData
    try {
      let startDate = null
    ...
    try {
      ...
      switch (cmd.getCommand()) {
        case "save":
          ...
          break
        case "savefromorigin":
          yield* commandSaveFromOrigin(ctx, cmd, outputData, row?.password)
          break
        case "sendmm":
        ...
        if (updateRes.affectedRows > 0) {
          ...
          if ("open" === command || "reopen" === command) {
            ...
            if (additionalOutput.row) {
              ...
              const userAuthStr = sqlBase.UserCallback.prototype.getCallbackByUserIndex(
                ctx,
                row.callback,
              )
              const wopiParams = wopiClient.parseWopiCallback(ctx, userAuthStr, row.callback)
