...
async function convertFromChanges(
  ...
  const commandSfctByCmdRes = await canvasService.commandSfctByCmd(
    ctx,
    cmd,
    opt_priority,
    opt_expiration,
  ...
  return co(function* () {
    ...
    try {
      ...
      if (constants.AVS_OFFICESTUDIO_FILE_UNKNOWN !== cmd.getOutputFormat()) {
        ...
        const status = yield* convertByCmd(
          ctx,
          cmd,
          async,
          fileTo,
...
function convertTo(req, res) {
  return co(function* () {
    ...
    try {
      ...
      if (pdfVer?.startsWith("PDF/A") && "pdf" === format) {
        format = "pdfa"
      }
      ...
      if (req.files?.length > 0 && req.files[0].originalname && req.files[0].buffer) {
        ...
        cmd.setDelimiter(
          "tsv" === filetype
            ? commonDefines.c_oAscCsvDelimiter.Tab
            : commonDefines.c_oAscCsvDelimiter.Comma,
        )
        if (lang) {
