...
const reservoirMaximum =
  cfgBottleneckGetChanges.reservoirIncreaseMaximum || cfgBottleneckGetChanges.reservoirRefreshAmount
...
function _insertChangesCallback(ctx, startIndex, objChanges, docId, index, user, callback) {
  ...
  let i = startIndex
  let lengthUtf8Current = sqlCommand.length
  let lengthUtf8Row = 0
    ...
    dbInstance.sqlQuery(
      ...
      (error, result) => {
        if (error) {
          ...
          resolve(!Number.isNaN(res) ? res : 0)
        }
      },
      false,
      false,
