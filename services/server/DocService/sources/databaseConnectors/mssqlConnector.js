...
const connectionConfiguration = {
  ...
  port: Number.parseInt(configSql.get("dbPort")),
...
function registerPlaceholderValues(values, statement) {
  if (values._typesMetadata !== undefined) {
    ...
    values._typesMetadata = undefined
  } else {
...
async function upsert(ctx, task) {
  ...
  const sqlMerge = `MERGE INTO ${cfgTableResult} AS target USING(VALUES(${insertValues})) AS source(${sourceColumns}) ON(${condition}) WHEN MATCHED THEN UPDATE SET ${updateColumns} WHEN NOT MATCHED THEN INSERT(${sourceColumns}) VALUES(${sourceValues}) OUTPUT $ACTION as action, INSERTED.user_index as insertId;`
