...
additionalOptions.thin = undefined
additionalOptions.libDir = undefined
...
function getColumnTypeMap(metaData) {
  ...
  if (!columnTypeMap) {
    ...
    for (let i = 0; i < metaData.length; i++) {
      ...
      const isClobColumn =
        col.dbType === oracledb.DB_TYPE_NCLOB || col.dbType === oracledb.DB_TYPE_CLOB
...
async function executeBunch(ctx, sqlCommand, values, options, noLog = false) {
  ...
  try {
    ...
    if (
      options?.batchErrors &&
      Array.isArray(result?.batchErrors) &&
    ...
    if (connection) {
      try {
        ...
        if (!noLog) {
          ctx.logger.error(
            `connection.close() error while executing batched query: ${sqlCommand}\n${error.stack}`,
          )
        }
      }
    }
 ...
 */
async function insertChangesAsync(
  ctx,
  tableChanges,
  startIndex,
  objChanges,
  ...
  const maxInsertStatementLength =
    `INSERT INTO ${tableChanges} VALUES()`.length +
  ...
  for (; currentIndex < objChanges.length; ++currentIndex, ++index) {
    ...
    if (
      (lengthUtf8Row + lengthUtf8Current >= cfgMaxPacketSize ||
        values.length >= MAX_EXECUTE_MANY_ROWS) &&
  ...
  const bindDefs = [
    ...
    maxChangeLen <= 2000
      ? {
          ...
          maxSize: Math.max(16, Math.min(maxChangeLen || 16, 2000)),
        }
  ...
  if (packetCapacityReached) {
    ...
    const r1 = await p1
    const r2 = await insertChangesAsync(
      ctx,
      tableChanges,
      currentIndex,
      objChanges,
