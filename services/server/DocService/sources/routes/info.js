...
const cfgEditorStatStorage =
  config.get("services.CoAuthoring.server.editorStatStorage") ||
  config.get("services.CoAuthoring.server.editorDataStorage")
...
async function buildLicenseInfoOutput(ctx, getConnections = null) {
  ...
  try {
    ...
    output.quota.edit.connectionsCount = await editorStat.getEditorConnectionsCount(
      ctx,
      connections,
    )
    ...
    output.quota.view.connectionsCount = await editorStat.getLiveViewerConnectionsCount(
      ctx,
      connections,
    )
