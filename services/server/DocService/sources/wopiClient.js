...
const { stat, lstat, readdir } = require("node:fs/promises")
...
function discovery(req, res) {
  return co(function* () {
    ...
    try {
      ...
      const tenDocumentFormatsFile = ctx.getCfg(
        "services.CoAuthoring.server.documentFormatsFile",
        cfgDocumentFormatsFile,
      )
      ...
      let templateEnd =
        "&lt;rs=DC_LLCC&amp;&gt;&lt;dchat=DISABLE_CHAT&amp;&gt;&lt;embed=EMBEDDED&amp;&gt;"
      templateEnd +=
        "&lt;fs=FULLSCREEN&amp;&gt;&lt;hid=HOST_SESSION_ID&amp;&gt;&lt;rec=RECORDING&amp;&gt;"
      templateEnd +=
          ...
          if (mimeTypes) {
            mimeTypes.forEach((value) => {
              ...
              xmlApp
                ...
                .up()
              xmlApp
                .ele("action", { name: "embedview", ext: "", urlsrc: urlTemplateEmbedView })
                .up()
              xmlApp
...
function isWopiModifiedMarker(url) {
  if (isWopiCallback(url)) {
    ...
    return obj.fileInfo?.LastModifiedTime
  }
}
...
function checkAndInvalidateCache(ctx, docId, fileInfo) {
  return co(function* () {
    ...
    if (selectRes.length > 0) {
      ...
      if (row.callback) {
        const commonInfoStr = sqlBase.UserCallback.prototype.getCallbackByUserIndex(
          ctx,
          row.callback,
          1,
        )
        if (isWopiCallback(commonInfoStr)) {
          ...
          const unlockMarkStr = sqlBase.UserCallback.prototype.getCallbackByUserIndex(
            ctx,
            row.callback,
          )
 ...
 */
async function prepareDocumentForEditing(
  ctx,
  wopiSrc,
  fileInfo,
  userAuth,
  ...
  do {
    ...
    if (!shutdownFlag) {
      const preOpenRes = await preOpen(
        ctx,
        checkRes.lockId,
        docId,
        fileInfo,
...
function getEditorHtml(req, res) {
  return co(function* () {
    ...
    try {
      ...
      const access_token_ttl = Number.parseInt(req.body.access_token_ttl) || 0
      ...
      const prepareResult = yield prepareDocumentForEditing(
        ctx,
        wopiSrc,
        fileInfo,
        userAuth,
...
function putRelativeFile(
  ...
  return co(function* () {
    ...
    try {
      ...
      const errorMsg = getWopiErrorMessage(err.statusCode)
      ctx.logger.error(
        "wopi putRelativeFile error status=%d (%s):%s",
        err.statusCode,
        errorMsg,
        err.stack,
      )
...
async function refreshFile(ctx, wopiParams, baseUrl) {
  ...
  try {
    ...
    const prepareResult = await prepareDocumentForEditing(
      ctx,
      userAuth.wopiSrc,
      fileInfo,
      userAuth,
...
async function checkFileInfo(ctx, wopiSrc, access_token, opt_sc) {
  ...
  try {
    ...
    //isInJwtToken is true because it passed checkIpFilter for wopi
    const isInJwtToken = true
    const getRes = await utils.downloadUrlPromise(
      ctx,
      uri,
      tenDownloadTimeout,
      undefined,
    ...
    const errorMsg = getWopiErrorMessage(err.statusCode)
    ctx.logger.error(
      "wopi checkFileInfo error status=%d (%s):%s",
      err.statusCode,
      errorMsg,
      err.stack,
    )
