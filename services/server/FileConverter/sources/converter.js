...
const cfgSigningKeyStorePath =
  config.get("FileConverter.converter.signing.keyStorePath") ||
  config.get("FileConverter.converter.signingKeyStorePath")
...
function mergeSigningMeta(signingCfg, jsonParams) {
  ...
  if (!meta || (!meta.reason && !meta.name && !meta.location && !meta.contactInfo))
    return jsonParams
...
TaskQueueDataConvert.prototype = {
  serialize(ctx, fsPath) {
    ...
    if (
      this.formatTo === constants.AVS_OFFICESTUDIO_FILE_DOCUMENT_OFORM_PDF &&
      commonDefines.c_oAscForceSaveTypes.Form === this.forceSaveType
    ) {
...
function* postProcess(ctx, cmd, dataConvert, tempDirs, childRes, error, isTimeout) {
  ...
  if (-1 !== exitCodesUpload.indexOf(error)) {
    ...
    if (dataConvert._cloudSigningCertPath) {
      try {
        const signingCfg = ctx.getCfg("FileConverter.converter.signing", cfgSigning)
        yield performCloudSigning(
          ctx,
          dataConvert.fileTo,
          dataConvert._cloudSigningCertPath,
          signingCfg,
        )
...
function* spawnProcess(
  ...
  let childArgs
  if (tenArgs.length > 0) {
    childArgs = tenArgs.trim().replace(/ {2,}/g, " ").split(" ")
  } else {
    childArgs = []
  }
  let processPath
