...
function parseCertificatesFromCredentialsInfo(data) {
  ...
  const certs =
    pick(data, ["certificates"]) ||
    pick(data?.cert, ["certificates"]) ||
    pick(data?.cert, ["certificate"])
  if (!certs) return []
...
function detectKeyTypeFromDer(leafDer) {
  try {
    ...
    return pub?.asymmetricKeyType ? String(pub.asymmetricKeyType) : null
  } catch {
    return null
  }
}
...
class CscSigner {
  ...
  async getAccessToken() {
    ...
    if (typeof this.cfg.oauth.tokenProvider === "function") {
      ...
      if (t && typeof t === "object" && t.access_token) {
        ...
        this._cachedTokenExp = expiresIn
          ? now + expiresIn * 1000 - 5000
          : now + DEFAULT_TOKEN_CACHE_MS
        return this._cachedToken
      }
    }
    ...
    try {
      ...
      throw new Error(
        `OAuth token request failed (HTTP ${status || "N/A"}): ${detail}${netErr} — url=${this.cfg.oauth.tokenUrl}`,
      )
    }
  }
  ...
  async resolveCredentialId(token) {
    ...
    const ids =
      ...
      (Array.isArray(resp?.data?.credentials)
        ? resp.data.credentials.map((x) => x.credentialID).filter(Boolean)
        : null) ||
      []
    ...
    const chosen =
      typeof this.cfg.credential.select === "function"
        ? this.cfg.credential.select(ids, resp.data)
        : ids[0]
  ...
  async authorizeCredential(token, credentialId, digestBuf) {
    ...
    try {
      ...
      const msg = String(
        data?.error_description || data?.error || data?.message || e?.message || "",
      )
      if (/otp|pin|2fa|second\s*factor/i.test(msg)) {
        throw new Error(
          "CSC provider requires OTP/PIN (interactive 2FA). This integration is configured for non-interactive signing only.",
        )
      }
      if (status)
        throw new Error(`credentials/authorize failed (HTTP ${status}): ${msg || "no details"}`)
      throw e
    }
  ...
  async signHash(token, credentialId, sad, digestBuf, algos) {
    ...
    if (!Array.isArray(sigs) || sigs.length === 0)
      throw new Error("signatures/signHash returned no signatures")
...
async function signPdfFile(inputPath, outputPath, config) {
  ...
  return signPdfWithSigner(inputPath, outputPath, { ...config, certificateChainDer }, (digest) =>
    signer.sign(digest),
  )
}
