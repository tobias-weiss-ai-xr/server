...
beforeAll(() => {
  ...
  execSync(
    `openssl req -x509 -newkey rsa:2048 -keyout "${keyPath}" -out "${certPath}" -days 1 -nodes -subj "/CN=Test/O=TestOrg"`,
    { stdio: "pipe" },
  )
  ...
  execSync(
    `openssl req -x509 -new -key "${ecKeyPath}" -out "${ecCertPath}" -days 1 -subj "/CN=TestEC/O=TestOrg"`,
    { stdio: "pipe" },
  )
...
function createTestPdf() {
  ...
  const content = `%PDF-1.4\n1 0 obj\n<< /Type /Catalog /Pages 2 0 R >>\nendobj\n2 0 obj\n<< /Type /Pages /Kids [] /Count 0 >>\nendobj\n3 0 obj\n<< /Type /Sig /Filter /Adobe.PPKLite /SubFilter /adbe.pkcs7.detached /ByteRange [0 ********** ********** **********] /Contents <${placeholder}> >>\nendobj\nxref\n0 4\n%%EOF\n`
