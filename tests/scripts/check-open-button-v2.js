...
;(async () => {
  ...
  if (fileRows.length > 0) {
    ...
    const menuItems = await page
      .locator(
        '.oc-context-menu li, .v-popper__inner li, [role="menuitem"], .oc-context-actions li',
      )
      .allTextContents()
    ...
    const allText = await page
      ...
      .catch(() => "")
    const hasOpenWith =
      allText.includes("Open with") ||
      allText.includes("Open in") ||
      allText.includes("World Office")
    console.log(`Has "Open with/Open in/World Office" text: ${hasOpenWith}`)

    // Look for any buttons/links with "open" in them
    const openElements = await page
      .locator('[class*="open" i], [data-test*="open" i], button:has-text("Open")')
      .allTextContents()
