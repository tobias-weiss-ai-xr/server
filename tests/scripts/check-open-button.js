...
;(async () => {
  ...
  page.on("request", (req) => {
    const url = req.url()
    if (
      url.includes("app-provider") ||
      url.includes("collaboration") ||
  ...
  await page.evaluate(
    async ({ token }) => {
      ...
      const doc =
        '<?xml version="1.0" encoding="UTF-8"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:r><w:t>Open with World Office!</w:t></w:r></w:p></w:body></w:document>'
      const docRels =
      ...
      function buildZip(entries) {
        ...
        for (const e of entries) {
          const n = Buffer.from(e.name, "utf8")
  ...
  if (await fileItem.isVisible().catch(() => false)) {
    ...
    const menuItems = await page
      .locator('.oc-context-menu, [role="menu"], .v-menu__content, .action-menu')
      .all()
      .catch(() => [])
    ...
    await page.screenshot({
      path: "C:/Users/Tobias/git/World-Office/test-results/context-menu.png",
    })
    console.log("   Screenshot: test-results/context-menu.png")
  } else {
    console.log("   File not visible in list")
    // Take screenshot of current state
    await page.screenshot({ path: "C:/Users/Tobias/git/World-Office/test-results/files-list.png" })
  }

  // Also check via the OCIS app-provider API
  console.log("[4] Check app-provider API...")
  const appProviderResult = await page.evaluate(
    async ({ token }) => {
      try {
  ...
  const graphResult = await page.evaluate(
    async ({ token }) => {
      try {
        ...
        const file = data.value?.find((f) => f.name === "open-test.docx")
        if (file?.webUrl) {
