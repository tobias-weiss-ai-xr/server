...
;(async () => {
  ...
  await page.goto("https://localhost:9200/files/app/files/WOPITest", {
  ...
  const session = await page.evaluate(
    ...
    {
      url: `https://localhost:9200/app/open?file_id=${encodeURIComponent(fileId)}&app_name=WorldOffice`,
      token,
    },
  )
  ...
  const editorFrames = page
    .frames()
    .filter((f) => f.url().includes("documenteditor/main/index.html"))
  ...
  const session2Json = await new Promise((resolve, reject) => {
    const url = new URL(
      `/app/open?file_id=${encodeURIComponent(fileId)}&app_name=WorldOffice`,
