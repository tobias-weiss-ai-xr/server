...
(async () => {
  ...
  }, { url: `https://localhost:9200/app/open?file_id=${encodeURIComponent(fileId)}&app_name=WorldOffice`, token });
  ...
  const session2Json = await new Promise((resolve, reject) => {
    const url = new URL(`/app/open?file_id=${encodeURIComponent(fileId)}&app_name=WorldOffice`, 'https://localhost:9200');
    const postData = '';
    const options = {
