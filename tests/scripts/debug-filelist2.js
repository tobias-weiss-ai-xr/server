...
;(async () => {
  ...
  const warningVisible = await page
    .locator('text="Your browser is not supported"')
    .isVisible()
    .catch(() => false)
  ...
  await page
    .waitForSelector("#oc-login-username", { state: "visible", timeout: 15000 })
    .catch(() => {
  ...
  if (token) {
    ...
    const doc = Buffer.from(
      '<?xml version="1.0"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:r><w:t>E2E Test Document</w:t></w:r></w:p></w:body></w:document>',
    )
    ...
    await page.goto("https://localhost:9200/files/spaces/personal/admin", {
    ...
    const tableRows = await page
      .locator('table tbody tr, [data-testid="resource-table"] tr')
      .count()
