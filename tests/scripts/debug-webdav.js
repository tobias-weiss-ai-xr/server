...
;(async () => {
  ...
  console.log(
    "All cookies:",
    cookies.map((c) => `${c.name}=${c.value.substring(0, 40)}... (${c.domain})`).join("\n"),
  )
  ...
  const dav2 = await page.evaluate(async () => {
    const res = await fetch("/remote.php/dav/files/admin/", {
      method: "PROPFIND",
      headers: { Depth: "1" },
    })
    return { status: res.status }
  ...
  const putResult2 = await page.evaluate(async () => {
    ...
    const res = await fetch("/remote.php/dav/files/admin/test-upload.txt", {
      method: "PUT",
      body: content,
    })
    return { status: res.status, ok: res.ok }
