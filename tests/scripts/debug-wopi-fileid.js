...
;(async () => {
  ...
  await page.evaluate(
    async ({ token }) => {
      ...
      const doc =
        '<?xml version="1.0" encoding="UTF-8"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:r><w:t>WOPI Test</w:t></w:r></w:p></w:body></w:document>'
      const docRels =
  ...
  const graphMeta = await page.evaluate(
    async ({ token }) => {
      try {
        ...
        const file = data.value?.find((f) => f.name === "wopi-debug.docx")
  ...
  // Test CheckFileInfo
  const wopiTests = await page.evaluate(
    async ({ token, fileId }) => {
      const results = []
      const ids = [fileId]
