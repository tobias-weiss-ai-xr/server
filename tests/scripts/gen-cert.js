...
fs.writeFileSync(
  path.join(certDir, "tls-cert.pem"),
  cert.export({ type: "pem", format: "der" }).toString("binary"),
  "binary",
)
fs.writeFileSync(
  path.join(certDir, "tls-key.pem"),
  privateKey.export({ type: "pkcs8", format: "der" }).toString("binary"),
  "binary",
)
