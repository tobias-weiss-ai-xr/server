...
;(async () => {
  ...
  const fileId =
    "27a0af90-399b-4075-9f24-8f5e31a79bb0$802e8371-3e52-43bb-a6aa-bd87e13423ea!5ae667ab-63e9-4a45-883d-ff7c5e0b5daa"

  // First, upload a fresh test file and get its file ID
  console.log("\n=== Ensure WOPITest folder exists ===")
  const mkcolResult = await page.evaluate(
    async ({ url, token }) => {
  ...
  console.log(
    `MKCOL WOPITest: ${mkcolResult.status} (${mkcolResult.status === 201 ? "created" : mkcolResult.status === 405 ? "exists" : "error"})`,
  )
  ...
  console.log("\n=== /app/open ===")
  const appOpenResult = await page.evaluate(
    async ({ url, token }) => {
      const res = await fetch(url, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
  ...
  try {
    appOpenJson = JSON.parse(appOpenResult.data)
  } catch (e) {
  ...
  try {
    const cfiRes = await fetch(
      `http://localhost:9300/wopi/files/${fileIdInWopi}?access_token=${wopiAccessToken}`,
    )
  ...
  try {
    const edRes = await fetch(editorUrl, {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: `access_token=${encodeURIComponent(wopiAccessToken)}`,
      redirect: "manual",
    })
    ...
    const hasEditor =
      edText.includes("editor") || edText.includes("iframe") || edText.includes("script")
  ...
  try {
    ...
    console.log(
      "Check: docker exec eo-docs tail -20 /var/log/world-office/documentserver/docservice/out.log",
    )
  } catch (e) {
