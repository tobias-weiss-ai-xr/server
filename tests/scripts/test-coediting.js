...
;(async () => {
  ...
  const uploadResult = await page.evaluate(
    ...
    {
      url: `https://localhost:9200/dav/files/admin/WOPITest/${filename}`,
      docxBase64: minimalDocxB64,
      token,
    },
  )
  ...
  // === Get file ID ===
  const propfindResult = await page.evaluate(
    async ({ url, token }) => {
      const body =
  ...
  const session1 = await page.evaluate(
    ...
    {
      url: `https://localhost:9200/app/open?file_id=${encodeURIComponent(freshFileId)}&app_name=WorldOffice`,
      token,
    },
  )
  ...
  console.log(
    `  Same WOPISrc file ID: ${fileId1 === fileId2 ? "✅" : "❌"} (${fileId1 === fileId2 ? "same" : "DIFFERENT"})`,
  )
  console.log(
    `  Different access tokens: ${token1 !== token2 ? "✅" : "❌"} (${token1 !== token2 ? "different" : "SAME"})`,
  )
  ...
  if (claims1 && claims2) {
    ...
    console.log(
      `  Session IDs differ: ${sessionId1 !== sessionId2 ? "✅" : "❌"} (${sessionId1.substring(0, 8)}... vs ${sessionId2.substring(0, 8)}...)`,
    )
    console.log(
      `  Access tokens differ: ${accessToken1 !== accessToken2 ? "✅" : "❌"} (same OIDC token, different WOPI token)`,
    )
    ...
    console.log(
      `  Same file reference: ${JSON.stringify(claims1.WopiContext?.FileReference) === JSON.stringify(claims2.WopiContext?.FileReference) ? "✅" : "⚠️"}`,
    )
  }
  ...
  const openEditor = async (wopiSrc, wopiToken, label) => {
    ...
    const editorFrames = frames.filter(
      (f) =>
    ...
    console.log(
      `  ${label}: frames=${frames.length}, editorFrames=${editorFrames.length}, canvas=${state.hasCanvas}, error=${state.isError}, title=${state.title.substring(0, 60)}`,
    )
    ...
    await editorPage.screenshot({
      path: `scripts/coedit-${label.toLowerCase()}.png`,
      fullPage: false,
    })
