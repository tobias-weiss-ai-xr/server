  ...
  try {
    ...
    if (fileVisible) {
      ...
      const menuItems = await page
        .locator('.v-menu, .oc-context-menu, [role="menu"], .context-menu')
        .allTextContents()
        .catch(() => [])
      console.log("\nContext menu items:", menuItems)

      // Check for action menu items
      const actionItems = await page
        .locator('[data-testid], [class*="action"], [class*="menu"]')
        .allTextContents()
        .catch(() => [])
