...
async function step1_Login() {
  ...
  try {
    ...
    log(
      "LOGIN",
      `${cookies.length} cookies, token: ${accessToken ? `${accessToken.substring(0, 30)}...` : "NONE"}`,
    )
...
async function step3_Upload(page, accessToken) {
  ...
  try {
    ...
    const davPaths = ["/dav/files/admin/WOPITest/", "/remote.php/dav/files/admin/WOPITest/"]
    ...
    if (uploadResult.status >= 200 && uploadResult.status < 300) {
      ...
      const propfind = await page.evaluate(
        async ({ url, token }) => {
          const body =
    ...
    log(
      "UPLOAD",
      `Upload failed. Response preview: ${JSON.stringify(uploadResult).substring(0, 200)}`,
    )
    return null
  } catch (e) {
  ...
  const tests = [
    {
      ...
      fn: () =>
        page.evaluate(
          async ({ url, fileId, filename, filePath, mimeType, token }) => {
            try {
    ...
    },
    {
      ...
      fn: () =>
        page.evaluate(
          async ({ url, fileId, filename, token }) => {
            try {
          ...
          async ({ url, fileId, filename, token }) => {
            try {
              const res = await fetch(
                `${url}/app-provider/v1/open?file_id=${encodeURIComponent(fileId)}&mime_type=application/vnd.openxmlformats-officedocument.wordprocessingml.document&filename=${encodeURIComponent(filename)}`,
                {
                  headers: { Authorization: `Bearer ${token}` },
                },
              )
...
async function step5_CollabService(fileId) {
  ...
  const tests = [
    ...
    {
      desc: "GET /wopi/files/{id}?access_token=X",
      url: `${COLLAB_URL}/wopi/files/${fileId}?access_token=test-token`,
    },
...
async function step6_Eodocs() {
  ...
  const tests = [
    ...
    {
      desc: "POST /hosting/wopi/word/edit",
      url: `${EODOCS_URL}/hosting/wopi/word/edit`,
      method: "POST",
    },
...
async function main() {
  ...
  // Step 4
  console.log("\n=== Step 4: App-provider ===")
  const editorResult = await step4_AppProvider(
    auth.page,
  ...
  console.log(
    `  Upload:       ${fileInfo ? `OK (${fileInfo.filename}, id=${fileInfo.fileId})` : "FAILED"}`,
  )
  console.log(
    `  App-provider: ${editorResult ? `OK (${editorResult.editorUrl.substring(0, 80)})` : "NO EDITOR URL"}`,
  )
  console.log("==========================================")
