...
;(async () => {
  ...
  if (!freshFileId) {
    console.log("❌ Cannot get file ID")
    await browser.close()
    process.exit(1)
  }
  ...
  console.log(
    `✅ CheckFileInfo: ${cfiData.BaseFileName}, ${cfiData.Size} bytes, Version=${cfiData.Version}`,
  )
  ...
  const putFileRes = await fetch(
    `http://localhost:9300/wopi/files/${fileIdInWopi}/contents?access_token=${wopiAccessToken}`,
    {
      method: "POST",
      headers: {
  ...
  putFileRes.headers.forEach((v, k) => {
    putFileHeaders[k] = v
  })
  ...
  getFileRes.headers.forEach((v, k) => {
    getFileHeaders[k] = v
  })
