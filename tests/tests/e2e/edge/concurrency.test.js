...
describe("Concurrency and Network Resilience", () => {
  ...
  describe("Concurrent File Operations", () => {
    test(
      "10 concurrent file open requests all succeed",
      async () => {
    ...
    test(
      "Concurrent writes to same file are prevented by lock mechanism",
      async () => {
    ...
    test(
      ...
      async () => {
        ...
        const verifyRequests = files.map(({ fileId }) => wopiRequest("GET", fileId))
  ...
  describe("Network Timeout Handling", () => {
    test(
      "Network timeout during save triggers retry or graceful error",
      async () => {
        const fileId = generateFileId()
    ...
    test(
      "Simulated network latency allows editor to remain responsive",
      async () => {
        const fileId = generateFileId()
  ...
  describe("Invalid Request Handling", () => {
    test(
      "Invalid file type (.exe) returns appropriate error (403 or 400)",
      async () => {
    ...
    test(
      "Invalid file type (.zip) returns appropriate error",
      async () => {
    ...
    test(
      "Invalid file type (.sh) returns appropriate error",
      async () => {
    ...
    test(
      "Missing file ID returns 404 Not Found",
      async () => {
  ...
  describe("Lock Cleanup and Recovery", () => {
    test(
      "Stale locks can be overridden with correct X-WOPI-OldLock",
      async () => {
        const fileId = generateFileId()
    ...
    test(
      "Lock refresh extends lock validity",
      async () => {
        const fileId = generateFileId()
        const lockToken = generateLockToken()
  ...
  describe("High Concurrency Scenarios", () => {
    test(
      "15 concurrent read requests all complete successfully",
      async () => {
        const concurrentRequests = 15
        const fileIds = Array.from({ length: concurrentRequests }, () => generateFileId())

        const requests = fileIds.map((fileId) => wopiRequest("GET", fileId, {}, null, 15000))
    ...
    test(
      "Concurrent lock and unlock operations maintain consistency",
      async () => {
        const fileId = generateFileId()
        const lockToken = generateLockToken()
  ...
  describe("Document Server Restart Scenarios", () => {
    ...
    test(
      "should handle DS restart during active editing gracefully",
      async () => {
