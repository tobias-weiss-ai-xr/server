...
describe('Empty / Large File Edge Cases', () => {
  ...
  describe('Empty file (0 bytes)', () => {
    test('opens successfully and reports Size: 0', async () => {
      if (skipAll) {
      ...
      const response = await wopiRequest('GET', `${fileId}?access_token=${accessToken}`, {
        'X-WOPI-Override': 'GET'
      }, null, LARGE_FILE_TIMEOUT);
  ...
  describe('Large file (50 MB)', () => {
    test('opens within timeout', async () => {
      if (skipAll) {
        console.warn('⏭ Skipping: OCIS not available');
        return;
      }
  ...
  describe('File at size limit (100 MB)', () => {
    test('returns 200 OK or proper 4xx error if limit exceeded', async () => {
      if (skipAll) {
        console.warn('⏭ Skipping: OCIS not available');
        return;
      }
  ...
  describe('Very long filename', () => {
    test('is handled gracefully — server responds (200 or 400, not 500)', async () => {
      if (skipAll) {
        console.warn('⏭ Skipping: OCIS not available');
        return;
      }
  ...
  describe('Special characters in filename', () => {
    ...
    test.each(specialFilenames)('$name — no path traversal, no server error', async ({ filename }) => {
      if (skipAll) {
        console.warn('⏭ Skipping: OCIS not available');
        return;
      }
      ...
      const response = await wopiRequest('GET', `${filename}?access_token=${accessToken}`, {
        'X-WOPI-Override': 'GET'
      }, null, DEFAULT_TIMEOUT);
