...
const PLACEHOLDER_TOKENS = {
  ...
  expired:
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJleHAiOjE2MDAwMDAwMDAsImlhdCI6MTYwMDAwMDAwMH0.expired_signature_placeholder",
  malformed: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.malformed_payload.invalid_signature",
  validButUnsigned:
    "eyJhbGciOiJub25lIiwidHlwIjoiSldUIn0.eyJzdWIiOiJ0ZXN0IiwiaWF0IjoxNTE2MjM5MDIyfQ.",
}
...
describe("JWT Token Validation", () => {
  describe("Negative Test Cases", () => {
    ...
    test(
      "should return 401 for malformed JWT with invalid signature",
      async () => {
        const response = await axios.get(`${TEST_URL}/coauthoring/CommandService.ashx`, {
          headers: {
    ...
    test(
      "should return 401 for unsigned JWT token (alg: none)",
      async () => {
        const response = await axios.get(`${TEST_URL}/coauthoring/CommandService.ashx`, {
          headers: {
  ...
  describe("Positive Test Cases", () => {
    test(
      "should return 200 OK for valid JWT token when JWT is enabled",
      async () => {
    ...
    test(
      "should allow access to public endpoints without JWT",
      async () => {
  ...
  describe("Edge Cases", () => {
    test(
      "should return 401 for empty Authorization header",
      async () => {
        const response = await axios.get(`${TEST_URL}/coauthoring/CommandService.ashx`, {
          headers: {
            Authorization: "",
    ...
    test(
      "should return 401 for wrong auth scheme (Basic instead of Bearer)",
      async () => {
        const response = await axios.get(`${TEST_URL}/coauthoring/CommandService.ashx`, {
          headers: {
    ...
    test(
      "should handle JWT with tampered payload",
      async () => {
        // Token with tampered payload section
        const tamperedToken =
          "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.dGFtcGVyZWRfcGF5bG9hZA.tampered_signature"
  ...
  describe("Security Headers Validation", () => {
    test(
      "should include appropriate security headers in 401 response",
      async () => {
        const response = await axios.get(`${TEST_URL}/coauthoring/CommandService.ashx`, {
          headers: {
            Authorization: `Bearer ${PLACEHOLDER_TOKENS.invalid}`,
